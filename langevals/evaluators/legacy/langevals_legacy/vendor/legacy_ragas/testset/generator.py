...
from legacy_ragas._analytics import TestsetGenerationEvent, track
from legacy_ragas.embeddings.base import BaseRagasEmbeddings, LangchainEmbeddingsWrapper
from legacy_ragas.exceptions import ExceptionInRunner
from legacy_ragas.executor import Executor
from legacy_ragas.llms import BaseRagasLLM, LangchainLLMWrapper
from legacy_ragas.run_config import RunConfig
from legacy_ragas.testset.docstore import Document, DocumentStore, InMemoryDocumentStore
from legacy_ragas.testset.evolutions import (
    ComplexEvolution,
    CurrentNodes,
    DataRow,
    Evolution,
...
from legacy_ragas.testset.extractor import KeyphraseExtractor
from legacy_ragas.testset.filters import EvolutionFilter, NodeFilter, QuestionFilter
from legacy_ragas.utils import check_if_sum_is_close, deprecated, get_feature_language, is_nan
...
class TestsetGenerator:
    ...
    def generate(
        ...
        if with_debugging_logs:
            from legacy_ragas.utils import patch_logger

            patch_logger("legacy_ragas.testset.evolutions", logging.DEBUG)
            patch_logger("legacy_ragas.testset.extractor", logging.DEBUG)
            patch_logger("legacy_ragas.testset.filters", logging.DEBUG)
            patch_logger("legacy_ragas.testset.docstore", logging.DEBUG)
            patch_logger("legacy_ragas.llms.prompt", logging.DEBUG)
