...
| `langyagent.workerIsolation`                   | Per-worker identity posture (ADR-130). "per-uid" gives each conversation's worker its own uid, which costs the pod root plus five capabilities. "none" runs every worker as the pod's own user, which is what makes the pod admissible under Pod Security Admission "restricted" — at the cost of one worker being able to read another's live credentials and conversation content. Any other value is refused at render time. Forcing the default posture non-root instead is refused too, because it renders a pod that is admitted and then fails every worker spawn.                                        | `per-uid`                        |
| `langyagent.acceptWorkerIsolationDisabled`     | Accept running every worker under one shared identity. Required for workerIsolation "none"; the render fails without it, so the weaker posture is never reached by leaving a field blank or by an override that replaced more than it named.                                                                                                                                                                                                                                              | `false`                          |
