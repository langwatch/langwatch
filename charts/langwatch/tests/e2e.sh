...
# ─────────────────────────────────────────────────────────────────────────────
# SUITE: LangWatchQL self-provisioning (issue #6635)
# Runs after test_app: the app pod's boot (`start:prepare:db` → lwql:provision)
# is what provisions everything asserted here, from the chart-wired
# LWQL_SELF_PROVISION env and the two autogenerated passwords. Asserts the
# whole model exists, that the restricted identity is genuinely restricted,
...
test_lwql() {
  ...
  # PostgreSQL bridge: named collection + engine tables on the ClickHouse
  # side, reader role + approved views on the PostgreSQL side.
  ...
  local engine_tables
  engine_tables=$(ch_query "$pod" "SELECT count() FROM system.tables WHERE database='langwatch' AND engine='PostgreSQL'")
  if [ "${engine_tables:-0}" -ge 1 ]; then
    pass "PostgreSQL-engine tables mapped ($engine_tables)"
  else
    fail "no PostgreSQL-engine tables in the langwatch database"
  fi
  assert_eq "reader role lwql_ro exists" \
    "$(pg_query "$pg_pod" "SELECT count(*) FROM pg_roles WHERE rolname='lwql_ro'")" "1"
  local approved
  approved=$(pg_query "$pg_pod" "SELECT count(*) FROM pg_views WHERE viewname LIKE 'lwql_%'")
  if [ "${approved:-0}" -ge 1 ]; then
    pass "approved lwql_ views exist in PostgreSQL ($approved)"
  else
    fail "no approved lwql_ views in PostgreSQL"
  fi
  ...
  # Idempotence AC: a second provisioning run converges — same single user,
  # no duplicate key-map rows (PR #6486's INCORRECT_RESULT_OF_SCALAR_SUBQUERY
  # regression class).
