  ...
  {{- if not .Values.postgresql.chartManaged }}
    ...
    {{- else }}
      {{- /* External PostgreSQL with an EXPLICIT (non-empty) bridge host: the
             ...
             their own PostgreSQL, dialing as the read-only role lwql_ro. But the
             app never runs role DDL against a PostgreSQL it does not own
             (LWQL_MANAGE_POSTGRES_READER is gated on postgresql.chartManaged, so
             it is absent here), and url-secret.yaml's autogenerated lwql_pg_password
             is a password nobody set on that PostgreSQL. Without a real reader
             credential the bridge is dead on arrival, silently. Require the
             operator to supply the reader password through a Secret they own
             (clickhouse.lwqlAccessModel.existingSecret, which redirects both this
             chart's lwqlSecretName and the subchart's mount at it), so ClickHouse
             dials lwql_ro with the password actually set on it. */}}
      {{- if empty $chLwql.existingSecret }}
        {{- $bridgeUser := $bridge.user | default "lwql_ro" }}
        {{- $bridgePwKey := $bridge.passwordSecretKey | default "lwql_pg_password" }}
        {{- $resolvedHost := tpl $bridge.host . }}
        {{- $errors = append $errors (printf "clickhouse.lwqlAccessModel.postgres.host is set (%[1]q) with postgresql.chartManaged=false, so the lwql_postgres bridge dials YOUR external PostgreSQL as the read-only role %[2]q — the chart never provisions that role on a PostgreSQL it does not own and will not autogenerate a password nobody set there. Before installing: (1) create %[2]s on your PostgreSQL, read-only, granted SELECT on the approved lwql_* views only — e.g. CREATE ROLE %[2]s LOGIN; ALTER ROLE %[2]s WITH LOGIN PASSWORD '<pw>' CONNECTION LIMIT <n>; ALTER ROLE %[2]s SET default_transaction_read_only = on; REVOKE ALL ON SCHEMA public FROM %[2]s; REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM %[2]s; GRANT USAGE ON SCHEMA public TO %[2]s; GRANT SELECT ON public.lwql_traces TO %[2]s (one GRANT SELECT per approved lwql_* view, never the superuser); and (2) supply that reader password via clickhouse.lwqlAccessModel.existingSecret pointing at a Secret you own that carries the %[3]q key — the same Secret ClickHouse mounts for the lwql_postgres collection. To disable the bridge instead, set clickhouse.lwqlAccessModel.postgres.host to \"\"." $resolvedHost $bridgeUser $bridgePwKey) }}
      {{- end }}
    {{- end }}
  {{- else }}
