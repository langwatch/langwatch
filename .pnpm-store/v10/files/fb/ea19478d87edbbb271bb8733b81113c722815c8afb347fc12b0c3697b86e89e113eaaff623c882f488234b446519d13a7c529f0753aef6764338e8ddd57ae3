...
class NodeSDK {
    ...
    start() {
        ...
        if (this._configuration?.spanProcessors) {
            ...
            spanProcessors = [
                (0, create_from_env_1.createBatchSpanProcessorFromEnv)(this._configuration.traceExporter, sdkMetricsEnabled ? this._meterProvider : undefined),
            ];
        }
        else {
            spanProcessors = (0, utils_1.getSpanProcessorsFromEnv)(sdkMetricsEnabled ? this._meterProvider : undefined);
        }
        ...
        if (!this._loggerProviderConfig) {
            this.configureLoggerProviderFromEnv(sdkMetricsEnabled ? this._meterProvider : undefined);
        }
        if (this._loggerProviderConfig) {
