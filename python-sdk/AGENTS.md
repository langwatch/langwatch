...
| Failing to follow [TESTING_PHILOSOPHY.md](../dev/docs/TESTING_PHILOSOPHY.md) hierarchy | Always start with a `.feature` file, drive from E2E tests in `tests/e2e/`, then add integration tests in `tests/` and unit tests as outlined. Examples in `examples/` serve as user demonstrations. |
