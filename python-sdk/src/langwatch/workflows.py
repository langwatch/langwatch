"""
API facade for managing LangWatch workflows via REST API.

Provides list, get, update, and delete operations for workflows with proper error handling.
...
def _raise_for_status(response: httpx.Response, *, operation: str = "") -> None:
    ...
    if status == 404:
        raise ValueError(
            f"Workflow not found: {detail}" if detail else "Workflow not found"
        )
    if status == 400:
...
class WorkflowsFacade:
    """
    Facade for managing LangWatch workflows via REST API.

    Provides list, get, update, and delete operations.
    """
    ...
    @classmethod
    def from_global(cls) -> "WorkflowsFacade":
        """Create a WorkflowsFacade using the global LangWatch configuration."""
        ensure_setup()
    ...
    def list(self) -> Dict[str, Any]:
        """
        List all workflows for the project.
