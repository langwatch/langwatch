...
describe("parseAllowedOrigins", () => {
  ...
  it("drops entries that are not usable origins", () => {
    expect(parseAllowedOrigins("not-an-origin, https://ok.test")).toEqual(["https://ok.test"]);
  });

  it("deduplicates equivalent origins", () => {
    expect(parseAllowedOrigins("https://a.test,https://a.test/")).toEqual(["https://a.test"]);
  });
});
...
describe("isOriginAllowed", () => {
  ...
  it("matches on scheme, host and port together", () => {
    ...
    expect(isOriginAllowed({ origin: "https://app.test:8443", allowedOrigins: allowlist })).toBe(
      false,
    );
    expect(isOriginAllowed({ origin: "https://other.test", allowedOrigins: allowlist })).toBe(
      false,
    );
  });
  ...
  it("does not let a hostname that merely ends in localhost through", () => {
    ...
    expect(isOriginAllowed({ origin: "https://evil-localhost.test", allowedOrigins: [] })).toBe(
      false,
    );
  });
  ...
  it("treats a rebound attacker hostname as unlisted", () => {
    ...
    expect(isOriginAllowed({ origin: "http://rebind.attacker.test", allowedOrigins: [] })).toBe(
      false,
    );
  });
});
