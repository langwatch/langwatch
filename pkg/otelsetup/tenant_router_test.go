...
func TestTenantRouter_OpsFallback(t *testing.T) {
	t.Run("an untenanted span reaches the ops pipeline with the internal resource", func(t *testing.T) {
		...
		// No api_key anywhere in context: a background/ops span — carrying an
		// attribute that must NOT survive, since a key-less span might be a
		// customer-path span that lost its context.
		_, span := tp.Tracer("test").Start(context.Background(), "startup-work")
		span.SetAttributes(attribute.String("workflow.input", "customer data"))
		span.End()
		...
		if n := len(ops.spans[0].Attributes()); n != 0 {
			t.Fatalf("ops copy must be the operational skeleton only, got %d span attributes", n)
		}
	})
