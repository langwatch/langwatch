...
// shape of the openai / gopenai instrumentations' config so the instrumentations
...
// Option configures a traced transport (NewTransport / NewHTTPClient).
...
// default it is set to "ollama" (the OTel GenAI semconv has no Ollama constant).
// Override it when pointing the client at a different backend with a value from
// the semconv (e.g. semconv.GenAIProviderNameKey.String("...")) or a custom
// provider key/value.
