...
// tracedClient builds an *http.Client whose transport traces requests to the
// given exporter's provider and then forwards to the mock round tripper. This is
// the client passed verbatim to genai via ClientConfig.HTTPClient.
func tracedClient(rt *mockRoundTripper, provider *sdktrace.TracerProvider, opts ...Option) *http.Client {
	allOpts := append([]Option{WithTracerProvider(provider)}, opts...)
	return &http.Client{Transport: NewTransportWithBase(rt, allOpts...)}
}
...
// attribute value. Unlike langwatch.input/output, these are a RAW JSON array of
// ChatMessage objects with NO {type,value} envelope.
...
// gen_ai-native content attribute keys. LLM messages and system instructions are
// recorded here, NOT under langwatch.input/output.
var (
	genAIInputKey        = semconv.GenAIInputMessagesKey
	genAIOutputKey       = semconv.GenAIOutputMessagesKey
	genAIInstructionsKey = semconv.GenAISystemInstructionsKey
)
