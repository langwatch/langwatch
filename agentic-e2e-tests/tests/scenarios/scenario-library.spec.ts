...
test.describe("Scenario Library", () => {
  ...
  test("view scenarios in list", async ({ page }) => {
  ...
  test("click scenario row to edit", async ({ page }) => {
  ...
  test("empty state when no scenarios", async ({ page }) => {
    ...
    const emptyState = page.getByText(/no scenarios|create your first|get started/i);
    const newScenarioButton = page.getByRole("button", {
      name: /new scenario/i,
    });
