...
// HostedCall is what a caller asks of a hosted service: the operation and the
// body it sent, as it sent it.
type HostedCall struct {
	Operation domain.HostedServiceOperation
	Body      []byte
}
...
// they skip the check.
func (a *App) CallHostedService(ctx context.Context, bundle *domain.Bundle, call HostedCall) (domain.HostedServiceResponse, error) {
	if a.hosted == nil {
	...
	if call.Operation == domain.HostedInstantEvalsClassify {
