...
// Cache creates an interceptor that decides the request's cache behavior and
// applies it to the body. Operator-defined cache rules take precedence; when
// no rule matches, the resolved provider's default applies so prompt caching
// happens out of the box on providers that need an explicit per-request
// marker (defaultCacheAction).
func Cache(evaluate EvaluateCacheFunc) Interceptor {
	return PreOnly("cache", func(ctx context.Context, call *Call) error {
		...
		action, ok := defaultCacheAction(call.Request.Resolved)
		if !ok {
			return nil
		}
		if err := call.MaterializeBody(); err != nil {
