...
//
// core has no such producer — every StatusCode it sets comes from fasthttp's
// resp.StatusCode() or a positive literal — so this pins the invariant rather
// than a live path: the two must not be able to disagree again if a vendor
// bump ever introduces one.
