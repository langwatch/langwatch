...
type Credentials struct {
	...
	// A `harness` field selected between two coding-agent harnesses until
	// ADR-131 removed the second one. A control plane that still sends it is
	// not an error: the field is simply not decoded, and there is nothing left
	// for it to select.
}
...
type CredentialSignature struct {
	...
	MirrorTier string
	// A normalized harness was folded in here until ADR-131, so that flipping
	// between the two harnesses replaced the worker instead of reusing one
	// built for the other. With a single harness there is nothing to compare:
	// every worker is built the same way, and keeping the field would have made
	// its value a constant that still had to be canonicalized identically in
	// two places to avoid recycling the fleet on deploy.
}
