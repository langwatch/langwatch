...
// terminal on the worker's event stream — e.g. its LLM call was
...
// cause chain crosses every wire (herr ⇄ the control plane's DomainError are
// the same model).
...
const (
	...
	ErrSessionNotFound = herr.Code("agent_session_not_found")
