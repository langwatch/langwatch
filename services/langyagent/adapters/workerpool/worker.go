...
func (w *Worker) PostMessage(ctx context.Context, system, prompt string) error {
	return postMessage(ctx, w.port, w.bearerToken, w.openCodeSessionID, system, prompt)
}
...
func (w *Worker) StreamEvents(ctx context.Context, sink app.ChatSink) error {
	return streamSessionEvents(ctx, w.port, w.bearerToken, w.openCodeSessionID, sink, sink.Flush)
}
...
// plaintext LangWatch API key; ${HOME}/work holds cloned repos. Without this,
// secrets and clones accumulate on the pod volume forever — the github.md
// skill's "the idle reaper cleans it with the session" guarantee lives here.
func removeWorkerHome(ctx context.Context, sessionsRoot, conversationID string) {
	...
	if err := os.RemoveAll(workerHome); err != nil {
		clog.Get(ctx).Warn("remove worker home failed",
