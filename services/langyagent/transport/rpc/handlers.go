...
type chatRequest struct {
	...
	// ProjectID and UserID are required above: every worker is principal-bound,
	// and every signed frame carries that same identity.
}
...
func chatHandler(application *app.App, maxBodyBytes int64, intent string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		creds.ProjectID = req.ProjectID
		creds.ActorUserID = req.UserID
