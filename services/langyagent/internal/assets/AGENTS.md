...
- **`--jq` is a small subset; your shell has the real tools.** The flag reads dot paths, `.items[]`, `.items[].field`, `.items[0]` and a terminal `| length`. It has no object construction, no `select`, no `map`. For anything past that, redirect the answer to a file (`langwatch ... --format json > results.json`) and narrow it in your shell, where `jq`, `python` and `python3` are all installed.
