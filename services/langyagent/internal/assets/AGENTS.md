...
- "Running `langwatch trace search --format json` and counting traces from the last 24h (default). I'll return the concise result and the LangWatch UI link." → run it and answer "N traces in last 24h. <url>" — never narrate the command
