...
A Python parameter with a default that is not a turn field is a run parameter. The platform offers it in the run dialog and in `--param`.
...
Do not set the environment by hand when the service already sets `APP_ENV` or `NODE_ENV`. Read those first.
...
- Do NOT hardcode an environment string when the service already reads `APP_ENV` or `NODE_ENV`. Pass the value the service uses.
