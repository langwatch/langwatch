...
For Python, a ladder, stopping at the first that works: `uv add langwatch` when the folder has a `uv.lock`; otherwise `.venv/bin/python -m pip install langwatch` when `.venv` exists, then `pip install langwatch`, `pip3 install langwatch`, `python3 -m pip install langwatch`, `python -m pip install langwatch`. A command not found (exit 127) moves to the next rung, never a retry: it is a missing spelling, never a missing capability. When no rung exists, ask before giving up: "Install uv for me" or "I'll set up Python myself".
