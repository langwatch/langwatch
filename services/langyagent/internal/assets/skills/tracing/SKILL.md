...
Before you start the process, check that the key is visible to it the way the project reads it: run the project's own loader in a one-line program and print only whether `LANGWATCH_API_KEY` is set, never its value, for example `uv run python -c "from dotenv import load_dotenv; load_dotenv(); import os; print(bool(os.getenv('LANGWATCH_API_KEY')))"`. A key that is set in the file and unset in the process is the order above.
