...
Get the session population from the coding-agent traces, then pull each interesting session's per-call rows:

```bash
langwatch trace export --origin coding_agent --format jsonl --limit 1000 -o coding-traces.jsonl
```
...
2. **Cache rebuilds**: model calls whose cache-creation tokens are the bulk of their input. Each one re-paid for context that was already paid for, at the cache-write rate, which is higher than fresh input.
