...
7. **Compare aggregates. Read them, never work them out yourself.** `langwatch workbench get-state <slug>` answers a block per column that already holds `overallPassRate`, each evaluator's `passRate`, `completedRows`, `totalRows`, `averageCost` and `averageLatency`, over the same denominator the page shows the user. Take the figures from there.
   ...
   **Quote the denominator next to the figure,** as "30 of 40 rows". A rate over a `completedRows` short of `totalRows` describes part of the dataset. If the two columns do not share the same `completedRows` and `totalRows`, run the short one to the end before you compare them at all, because the comparison is otherwise between two different dataset slices and says nothing.
