...
2. **Spans carry timing and operation attributes.**
   - Check: Take two or three trace ids from prerequisite 1 and run `langwatch trace get <trace_id> --format json` for each. Each trace must have spans with a type (`llm`, `agent`, `tool`, `chain`), a model on `llm` spans, and `finished_at` later than `started_at`. Then run `langwatch analytics query --metric p95-latency --format json` and confirm it returns a value greater than zero. That figure is trace-level completion time, so it only proves traces carry timing at all; the span checks above are what prove the spans are usable.
