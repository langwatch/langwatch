...
It returns the widget's render status, any compile/runtime error text, and the rendered markup (inline SVG for a recharts chart) — so you can check for an empty chart, NaN/undefined labels, overflow, or an error panel before calling the work done. Omit `widgetId` to list every widget on the open dashboard without markup. Receipts exist only while that tab is open; there is no away fallback.
