...
6. **Open the pull request** with `gh pr create --base <default> --title ... --body ...` when the workspace facts say `gh` is signed in. When it is not, or the folder is not a git repository, say so in one line and report the branch name instead.
