...
2\. **Never touch the user's working state.** Pick the branch name first, from BOTH lists: `git branch --list "langy/*" && git ls-remote --heads origin "langy/*"`. The first reads the names this folder carries and the second the names the remote carries; pick one that is in neither, so `checkout -b` runs once and the push is not rejected. A user who ran Langy before has leftovers on both sides, and a local list alone made the name look free until `git push` came back non-fast-forward. `git ls-remote` reaches the network, so run it in the same command as the `git fetch` you need anyway and both lists come back in one round trip. When the tree is dirty, leave it alone: create a worktree (`git worktree add ../<folder>-langy -b langy/<slug> origin/<default>`) and work there. When it is clean, `git fetch origin` and `git checkout -b langy/<slug> origin/<default>`. When the workspace facts say `git remote: none`, the repository has no remote: branch from the local default branch (`git checkout -b langy/<slug>`), and run no `git fetch` and no `git push`. When they say `git: not a repository`, there is no branch to make, and a dead end you know the unlock of is a question, never a stop: ask with the `question` tool, options "Create a repository for me" (`git init`, a `.gitignore` naming `.env` when there is none, `git add -A`, `git commit -m "Initial commit"`, then this step as the no-remote case) and "I'll choose another folder" (end the turn saying which folder to connect next), both plain answers with no `ref`. Never commit on the default branch, never stash, never reset.
