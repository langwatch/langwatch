...
// and in every execution_state_change frame (#3198). runCode/runIfElsePython
// now also scrub via nodeRun.storeOutput/storeError above (which additionally
// covers the run-scoped sandbox API key) — this call is the belt-and-suspenders
// backstop that catches every node kind, including any that skips nodeRun.
