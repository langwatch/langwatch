...
// AWS Bedrock counterpart to the OpenAI / Anthropic / Gemini / Azure
// live e2es. Owner brief flagged Bedrock as the second of the "tricky
// enterprise" providers — customer workflows configure it via
// litellm_params{aws_access_key_id, aws_secret_access_key,
// aws_region_name}, which the dispatcheradapter translates onto
// Bifrost's Bedrock key shape (access_key / secret_key / region).
//
// Build tag: live_bedrock (gated; needs AWS_ACCESS_KEY_ID +
// AWS_SECRET_ACCESS_KEY + AWS_DEFAULT_REGION in env). BEDROCK_MODEL
// override defaults to the EU cross-region inference profile that
// matches the matrix gateway test (eu.anthropic.claude-haiku-4-5...).
...
func setupStackWithLLM_bedrock(t *testing.T) *stack {
	t.Helper()
	...
	executor := liveBedrockExecutorAdapter{eng: eng}
...
type liveBedrockExecutorAdapter struct{ eng *engine.Engine }

func (a liveBedrockExecutorAdapter) ExecuteStream(ctx context.Context, req app.WorkflowRequest, opts app.WorkflowStreamOptions) (<-chan app.WorkflowStreamEvent, error) {
...
func (a liveBedrockExecutorAdapter) Execute(ctx context.Context, req app.WorkflowRequest) (*app.WorkflowResult, error) {
	wf, err := dsl.ParseWorkflow(req.WorkflowJSON)
	if err != nil {
...
func TestSync_RealWorkflowEndToEnd_Bedrock(t *testing.T) {
	...
	if model == "" {
		// EU cross-region inference profile — matches the matrix gateway
		// test default (the un-prefixed model id is rejected by the
		// langwatch-dev-bedrock-user IAM policy on eu-central-1).
