// End-to-end integration test for W3C trace-context + causality-depth
// propagation through nlpgo.
//
// Why this file exists: the original loop-prevention PR (#3976) shipped
// with function-level unit coverage of every individual piece —
// applyInboundCausality extracts headers, BaggageAttributeProcessor
// stamps spans, evaluatorblock injects the propagator on outbound —
// but nothing actually wired the whole chain together and observed it.
// rchaves caught that on post-merge dogfood: "did we ever prove the
// spans end up as a child of the parent and the full traceparent is
// working?"
//
// What this test proves end-to-end:
//   1. POSTing /go/studio/execute_sync with `traceparent` makes the
//      emitted spans share the inbound trace_id (trace continuity).
//   2. The studio root span's PARENT span_id equals the inbound
//      traceparent's span_id (parent linkage, not a fresh root).
//   3. EVERY span emitted during the run — root, children across
//      goroutines — carries `langwatch.reserved.causality_depth`
//      stamped from baggage by BaggageAttributeProcessor.
//   4. The outbound HTTP call from evaluatorblock to the fake LangWatch
//      app carries `traceparent` AND `baggage` AND
//      `X-LangWatch-Causality-Depth` headers, with the same trace_id
//      we started with.
//
// The wiring is exactly the production wiring (NewBaggageAttributeProcessor
// from pkg/otelsetup, the same TextMapPropagator composite), so a
