...
// Gemini counterpart to TestSync_RealWorkflowEndToEnd_OpenAI. Same
// chain, different provider — proves the credential mapping +
// dispatch path works for Gemini specifically when a real customer
// workflow is configured with `gemini/<model>`. Owner explicitly
// flagged Gemini alongside OpenAI as a top-priority provider in
// the migration brief.
//
// Build tag: live_gemini (gated; needs real GEMINI_API_KEY).
...
func setupStackWithLLM_gemini(t *testing.T) *stack {
	t.Helper()
	...
	executor := liveGeminiExecutorAdapter{eng: eng}
...
type liveGeminiExecutorAdapter struct{ eng *engine.Engine }

func (a liveGeminiExecutorAdapter) ExecuteStream(ctx context.Context, req app.WorkflowRequest, opts app.WorkflowStreamOptions) (<-chan app.WorkflowStreamEvent, error) {
...
func (a liveGeminiExecutorAdapter) Execute(ctx context.Context, req app.WorkflowRequest) (*app.WorkflowResult, error) {
	wf, err := dsl.ParseWorkflow(req.WorkflowJSON)
	if err != nil {
...
// TestSync_RealWorkflowEndToEnd_Gemini is the Gemini sibling of
...
// onto the right BifrostRouter provider so an `gemini/<model>`
// signature node lands on Gemini, not OpenAI.
func TestSync_RealWorkflowEndToEnd_Gemini(t *testing.T) {
	apiKey := os.Getenv("GEMINI_API_KEY")
	if apiKey == "" {
		t.Skip("GEMINI_API_KEY not set")
	}
	stack := setupStackWithLLM_gemini(t)

	body := `{
	  "type": "execute_flow",
	  "payload": {
	    "trace_id": "real-e2e-gemini",
	    "origin": "workflow",
	    "workflow": {
	      "workflow_id":"wf","api_key":"k","spec_version":"1.3","name":"GeminiMath","icon":"🧮","description":"x","version":"x",
	...
	assert.Equal(t, "real-e2e-gemini", res.TraceID)
