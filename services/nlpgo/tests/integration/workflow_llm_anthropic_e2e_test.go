...
func TestSync_RealWorkflowEndToEnd_Anthropic(t *testing.T) {
	    ...
	    "workflow": {
	      ...
	      "nodes":[
	        {"id":"entry","type":"entry","data":{
	          ...
	          "dataset":{"inline":{"records":{"question":["What is 5+5? Reply with just the digits."]}}},
	          "entry_selection":0,
	...
	// 5+5=10 — tightened from "any digit" so a wrong-but-numeric reply
	// can't silently mask a regression in prompt rendering or model
	// dispatch (matches the OpenAI/Vertex e2e pattern).
	assert.Contains(t, answer, "10",
		"expected the answer to contain 10, got %q", answer)
