...
// TestExecute_ToolCallOnConflictingModelDisablesReasoning pins the
// in-process half of the endpoint-scoped reasoning/tools rule. Everything
// this executor sends goes out on chat/completions, which is exactly the
// endpoint the gpt-5.6 family rejects the pair on.
//
// @scenario "a conflicting model sending tools has its reasoning turned off"
func TestExecute_ToolCallOnConflictingModelDisablesReasoning(t *testing.T) {
	gw := &fakeGateway{respStatus: 200, respBody: successResponse("ok")}
	exec := New(gw)
	...
	_, err := exec.Execute(context.Background(), app.LLMRequest{
		...
		ReasoningEffort: "high",
		Tools: []app.Tool{
			{Type: "function", Function: map[string]any{"name": "finish_test"}},
		},
		LiteLLMParams: map[string]any{"api_key": "k"},
	})
	if err != nil {
	...
	if got := bodyString(t, gw.lastReq.Body, "reasoning_effort"); got != "none" {
		t.Errorf("reasoning_effort = %q, want \"none\"", got)
	}
	if !hasField(t, gw.lastReq.Body, "tools") {
		t.Errorf("the tools must survive the correction, got: %s", gw.lastReq.Body)
	}
}

// The narrowness pin for the executor path: a reasoning model with no
// declared conflict keeps the effort the caller asked for, tools or not.
//
// @scenario "a reasoning model with no declared conflict keeps its reasoning"
func TestExecute_ToolCallOnUnconflictedReasoningModelKeepsEffort(t *testing.T) {
	gw := &fakeGateway{respStatus: 200, respBody: successResponse("ok")}
	exec := New(gw)
	...
	if got := bodyString(t, gw.lastReq.Body, "reasoning_effort"); got != "high" {
		t.Errorf("reasoning_effort = %q, want \"high\" (unchanged)", got)
	}
}

// A plain completion on a conflicting model is free to reason: the
// conflict is between reasoning AND tools, not reasoning alone.
//
// @scenario "a conflicting model with no tools keeps its reasoning"
func TestExecute_ToollessRequestOnConflictingModelKeepsEffort(t *testing.T) {
	gw := &fakeGateway{respStatus: 200, respBody: successResponse("ok")}
	exec := New(gw)
...
func bodyString(t *testing.T, body []byte, key string) string {
	t.Helper()
	var generic map[string]any
	if err := json.Unmarshal(body, &generic); err != nil {
		t.Fatalf("unmarshal body: %v", err)
	}
	value, _ := generic[key].(string)
	return value
}
