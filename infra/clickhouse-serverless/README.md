...
| `CH_REPLICATED` | `false` | Enable Keeper-coordinated ReplicatedMergeTree replication (requires keeper + data node env vars). Keeper replicates table data only — the LangWatchQL access model (users, profiles, row policies) is provisioned by the app via SQL, never stored in Keeper; see [ADR-141](../../dev/docs/adr/141-the-app-owns-the-lwql-access-model.md) |
