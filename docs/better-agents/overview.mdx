...
Better Agents is a CLI tool and a set of standards for building **reliable, testable, production-grade agents**, independent of which framework you use. It gives your coding assistant (Kilocode, Claude Code, Cursor, etc.) knowledge of any agent framework you choose (Agno, Mastra, LangGraph, etc.) and all their best practices.

Use your preferred stack: Agno, Mastra, Vercel AI, Google ADK, or anything else. Better Agents adds project structure, tests, and prompt versioning on top of it.

<Note>
  **Already have an agent?** You don't need Better Agents -- go to [LangWatch
  Skills](/skills/directory) to add tracing, evaluations, scenarios, and prompt versioning
...
<Check>
  Once you run your first scenario, you'll see results appear in your LangWatch project
  dashboard under the Simulations section.
</Check>
...
**The core of real agent reliability.** These are conversational test cases that simulate real tasks and validate agent behavior across iterations, updates, or model swaps.

Scenarios answer one question: _Does the agent still behave the way we expect?_
...
<Note>
  See the complete list of available evaluators in [Evaluators
  List](/evaluations/evaluators/list).
</Note>
...
<Note>
  For detailed scenario testing documentation, see [Agent
  Simulations](/agent-simulations/introduction).
</Note>
...
<Note>Learn more about evaluations in [LLM Evaluation](/evaluations/overview).</Note>

### Prompt Versioning

With Better Agents, prompts become:

- **Tracked** - Full version history with rollback
...
<Note>
  For comprehensive prompt management features, see [Prompt
  Management](/prompt-management/overview).
</Note>
...
<Note>Learn more about MCP integration in [LangWatch MCP](/integration/mcp).</Note>
