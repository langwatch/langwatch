...
LangWatch Cloud bills on **ingested events per billing period**. The list below is complete, so you can predict a bill from your own workload rather than from a sample.
...
Spans dominate the count whenever your app sends traces. A single trace of an agent turn typically contains several spans, one per step.

### Reading an experiment's cost

An experiment counts one event per step, not one for the whole run. A run over 100 dataset rows with 2 evaluators produces roughly `1 started + 100 target results + 200 evaluator results`, so around 301 events. This is worth knowing before a large sweep.
...
Instant Evals are not events. A judged query or run is charged per input token sent to the judge, on its own meter, and shows up as a spend event with request type `instant_eval` on the [billing events](/ai-gateway/billing-events) ledger. The rate and the free budget are on [Pricing](/pricing#instant-evals).
