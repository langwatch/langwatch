...
Under **Prompts or Agents**, click **Add**, pick **Prompt**, then **New Prompt**. Pick the model, write the coaching assistant's system prompt, keep `{{input}}` as the user message, and save.
</Step>
...
In **Variables**, map `input` to the dataset's `input` column and `output` to the prompt column's output. Save the evaluator.
</Step>
...
Click an evaluator chip to open its **Result** block. **Details** holds the judge's reasoning, including the criterion that failed. Read the failed rows first: a reason that names the same criterion on many rows points at what to change in the prompt.
...
Change the prompt, click **Run** again, then open **Results** and click **Compare** in the **Experiment Runs** sidebar to see both runs side by side. The pass rate tells you whether the change helped; the reasons on the rows that flipped tell you why.
