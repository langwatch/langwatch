...
`no_verdict` covers both `inconclusive` and `skipped` because a recorded row cannot tell them apart: they reach the platform under the same status, and naming either one in the export would be a guess. The reasoning cell says which it was, in the judge's own words. Read `verdict.status` in code when you need the two kept separate.
