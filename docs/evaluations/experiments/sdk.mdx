...
The winner comes back named with the target name you registered. Two candidates is not a different call from five: `compare()` judges however many the row has, and a target that produced nothing for the row, or only an empty output, is simply not a candidate.
...
| Column Pattern | Description | Example |
|----------------|-------------|---------|
| `{comparison}_winner` | Name of the winning target, `tie`, `unresolved`, or empty | `comparison_winner` → `gpt-5-mini` |
