...
```bash
curl -X POST "https://app.langwatch.ai/api/v1/evaluations/v3/{slug}/run" \
...
```bash
curl "https://app.langwatch.ai/api/v1/evaluations/v3/runs/{runId}" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```
...
```bash
curl "https://app.langwatch.ai/api/v1/evaluations/v3/runs/{runId}/results?experimentSlug={slug}" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```
