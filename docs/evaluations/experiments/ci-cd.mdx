...
Scenario suite runs accept the same field, where the values apply to every scenario in the run instead of every dataset row. See [Scenario run parameters](/agent-simulations/scenario-parameters).
