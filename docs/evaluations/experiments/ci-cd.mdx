...
<Frame>
  <img
    src="/images/evaluations/workbench-run-via-api.png"
    alt="The Run via API dialog with the Python, TypeScript and Shell tabs, the data source tabs and the Python snippet"
  />
</Frame>
...
The experiment runs with the configuration saved in LangWatch.
...
- `parameters`: constant inputs merged into every row (and the single row when there is no dataset), for example a feature flag or a model name
...
Test suite runs also accept a `parameters` field. There the values apply to every scenario in the run. See [Scenario run parameters](/agent-testing/run-parameters).
...
for idx, row in experiment.loop(dataset.iterrows()): # Call your LLM/agent
...
SDK experiments can compare different configurations in the same run:
...
for idx, row in experiment.loop(dataset.iterrows()):
    def compare(idx, row):
        ...
        with experiment.target("gpt-5-mini", {"model": "gpt-5-mini"}):
            ...
            experiment.evaluate("ragas/faithfulness", index=idx, settings={}, data={
                "input": row["input"],
                "output": response,
                "contexts": row["contexts"],
            })
...
<Tabs>
  <Tab title="Platform Experiment">
    ```yaml evaluate: stage: test image: python:3.11 script: - pip install langwatch - python -c "
    import langwatch result = langwatch.experiment.run('my-experiment') result.print_summary() "
    variables: LANGWATCH_API_KEY: $LANGWATCH_API_KEY ```
  </Tab>
  <Tab title="via SDK">
    ```yaml evaluate: stage: test image: python:3.11 script: - pip install langwatch openai - python
    scripts/run_evaluation.py variables: LANGWATCH_API_KEY: $LANGWATCH_API_KEY OPENAI_API_KEY:
    $OPENAI_API_KEY ```
  </Tab>
</Tabs>
...
For custom integrations, you can use the REST API directly. The paths below are also served under `/api/evaluations/v3/`, the prefix earlier SDK versions call.

### Start a Run

```bash
curl -X POST "https://app.langwatch.ai/api/experiments/{slug}/run" \
...
```bash
curl "https://app.langwatch.ai/api/experiments/runs/{runId}" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```
...
```bash
curl "https://app.langwatch.ai/api/experiments/runs/{runId}/results?experimentSlug={slug}" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```

Returns the per-row dataset entries (input, output, trace id, duration) and evaluation scores for the run. `experimentSlug` is optional for the first 24 hours after the run and required after that.
