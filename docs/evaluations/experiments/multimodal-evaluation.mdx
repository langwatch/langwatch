...
```bash
curl -X POST "https://app.langwatch.ai/api/v1/evaluations/evaluators/image-quality-scorer/evaluate" \
...
<Note>
  If a non-vision model is selected, the evaluator falls back to sending plain text descriptions.
  For accurate image evaluation, always select a vision-capable model.
</Note>
