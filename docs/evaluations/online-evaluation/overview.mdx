...
Online evaluation lets you continuously score your LLM's production traffic. It monitors your live application to catch quality issues, detect regressions, and ensure safety. To test before deployment instead, see [experiments](/evaluations/experiments/overview).

<Info>
  In the LangWatch platform, online evaluation is implemented through **Monitors** - automated rules
  that score incoming traces based on evaluators you configure.
</Info>
...
@langwatch.trace()
def my_llm_app(user_input): # Your LLM logic here
return response
...
1. Go to [Online Evaluations](https://app.langwatch.ai/@project/online-evaluations) in LangWatch
2. Click **New Online Evaluation**
