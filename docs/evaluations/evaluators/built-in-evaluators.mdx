...
`langwatch/api_keys_and_secrets_detection` flags leaked credentials in trace content: provider and cloud API keys, tokens, private keys, and database connection strings. It runs in-process with no analysis-service round-trip, using the same detection rules as secrets redaction, and works as a guardrail.
