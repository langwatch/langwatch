...
<Accordion title="Decision flowchart">
  ``` Do you have your own evaluation logic? ├─ Yes → Use Custom Scoring └─ No → Do you want to
  configure via UI and reuse? ├─ Yes → Use Saved Evaluators └─ No → Use Built-in Evaluators ```
</Accordion>
...
The evaluator picker in the platform has two more entries. **Custom (Code)** runs a Python function you write in a sandbox with `requests`, `httpx`, `pydantic` and the LangWatch SDK available, with a 10 minute limit per call by default. **Custom (from Workflow)** runs one of your workflows as the evaluator.
...
<Warning>
  **Important:** Always provide a descriptive `name` when running evaluators. This helps identify
  evaluation results in Analytics and traces.
</Warning>
