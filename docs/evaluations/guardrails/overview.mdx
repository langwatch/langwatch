...
Add `as_guardrail=True` to any evaluation to make it block when it fails:
...
See [Code Integration](/evaluations/guardrails/code-integration) for the full pattern — input + output guardrails, TypeScript, async, parallel execution, and error handling.
