...
@langwatch.trace()
def my_llm_app(user_input): # Run guardrail check
...
Use both to check the input and the output:
...
@langwatch.trace()
async def async_chatbot(user_input): # Async guardrail check
...
@langwatch.trace()
async def chatbot_with_parallel_guards(user_input): # Run multiple guardrails in parallel
...
@langwatch.trace()
def chatbot_with_error_handling(user_input):
    try:
        guardrail = langwatch.evaluation.evaluate(
            "azure/jailbreak",
            name="Jailbreak Detection",
