...
* **Historical Tracking**: A record of all your evaluation runs, so you can compare model versions and track improvements over time.
...
Now that you've set up evaluation for your structured data extraction, you can use [Workflows](/workflows/overview) to improve your extraction pipeline. Put the extraction prompt in an LLM node, attach the dataset, evaluate each prompt variation on the same rows and compare the results run by run.