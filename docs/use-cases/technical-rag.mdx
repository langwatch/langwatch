...
The goal is to verify that the chatbot provides accurate, relevant, and faithful answers based on the official documentation. We'll use LangWatch to automate this evaluation so it can run in a CI/CD workflow.
...
for idx, row in evaluation.loop(df.iterrows()):
    ...
    # This runs 'ragas/response_relevancy' to check if the answer is relevant to the question.
    evaluation.run(
...
- **Compare Experiments**: Compare the performance of `model-a-rag-evaluation-v1` against `v2` to see if your changes had a positive impact on metrics like faithfulness and accuracy.
...
- **Collaborate with Experts**: Share direct links to evaluation results with the domain experts who created the dataset, so they can close the feedback loop.
