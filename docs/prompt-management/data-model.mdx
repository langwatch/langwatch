...
<Note>
  **Workflows only**: You set these fields in the LLM node of a [workflow](/workflows/overview).
  They cannot be configured via the API.
</Note>
...
- `"bool"` - Boolean values
- `"image"` - Image data
- `"file"` - A file, for example a PDF, an audio clip or a text file
