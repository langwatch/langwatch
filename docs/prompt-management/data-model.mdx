...
<ResponseField name="scope" type="string" required default="PROJECT">
  - `PROJECT` - Prompts are only accessible within the project - `ORGANIZATION` - Prompts
  are shared across all projects in the organization
</ResponseField>

### Model Configuration

<ResponseField name="model" type="string" required>
  The LLM model to use (e.g., "openai/gpt-4o-mini"). Model names follow the litellm
...
<ResponseField name="parameters" type="object" default="{}">
  Arbitrary JSON object versioned alongside the prompt. Use it to store application
  settings that should change together with the prompt (thresholds, iteration counts,
  structured output schemas, feature switches). Changing `parameters` creates a new prompt
  version. See [Runtime
...
<Note>
  **Studio Only**: These advanced optimization features require the optimization studio
  interface for proper experimentation, performance measurement, and A/B testing. They
