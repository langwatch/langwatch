...
<Note>
  Available since Python SDK `0.26.0` and TypeScript SDK `0.33.0` (reading
  `prompt.parameters` works from Python SDK `0.25.0`; writing via `create`/`update`
  requires `0.26.0`). Changing `parameters` creates a new prompt version, just like
