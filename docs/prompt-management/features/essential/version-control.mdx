...
curl --request GET \
  --url "https://app.langwatch.ai/api/v1/prompts/{prompt_handle}/versions" \
...
curl --request GET \
  --url "https://app.langwatch.ai/api/v1/prompts/{prompt_handle}" \
  --header "X-Auth-Token: your-api-key"
...
curl --request GET \
  --url "https://app.langwatch.ai/api/v1/prompts/{prompt_handle}?version=2" \
  --header "X-Auth-Token: your-api-key"
```
...
<Tip>
  Combine these advanced features with LangWatch's optimization studio, where you can A/B test
  different configurations and measure their impact on performance metrics.
</Tip>
