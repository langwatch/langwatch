...
<Warning>
  **Scope Conflicts**: When updating an organization-scoped prompt, conflicts may arise if
  other projects have made changes. The system provides conflict information to help
  resolve differences.
</Warning>
...
<Note>
  **Field Structure**: All examples show the essential fields. Additional optional fields
  like `temperature`, `maxTokens`, `responseFormat`, `inputs`, `outputs`,
  `demonstrations`, and `promptingTechnique` can also be set. See the [Data
  Model](/prompt-management/data-model) page for complete field documentation.
</Note>
...
<Warning>
  **System Message Conflict**: You cannot set both a `prompt` (system message) and
  `messages` array with a system role in the same operation. Choose one approach to avoid
  errors.
</Warning>
...
<Warning>
  **System Message Conflict**: Same rule applies - you cannot set both a `prompt` and
  `messages` array with a system role in the same operation.
</Warning>
...
<Warning>
  **Permanent Deletion**: Deleting a prompt removes ALL versions permanently. This action
  cannot be undone.
</Warning>
...
<Warning>
  **Critical**: You cannot set both a `prompt` field and a `messages` array containing a
  system role in the same operation. This will throw an error.
</Warning>
...
<Tip>
  Combine these advanced features with LangWatch's optimization studio, where you can A/B
  test different configurations and measure their impact on performance metrics.
</Tip>
...
<Card
  title="Explore Optimization Studio"
  icon="rocket"
