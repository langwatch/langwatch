...
<Warning>
  **Scope Conflicts**: When updating an organization-scoped prompt, conflicts may arise if other
  projects have made changes. The system provides conflict information to help resolve differences.
</Warning>
...
<Note>
  **Field Structure**: All examples show the essential fields. Additional optional fields like
  `temperature`, `maxTokens`, `responseFormat`, `inputs`, `outputs`, `demonstrations`, and
  `promptingTechnique` can also be set. See the [Data Model](/prompt-management/data-model) page for
...
<Warning>
  **System Message Conflict**: You cannot set both a `prompt` (system message) and `messages` array
  with a system role in the same operation. Choose one approach to avoid errors.
</Warning>

<Note>
  The two SDKs do not take the same fields. `model`, `temperature` and `maxTokens` go through the
  TypeScript SDK and the REST API; the Python SDK's `create` and `update` do not accept them yet, so
  set them from the UI or over REST. See the [REST reference](/api-reference/prompts/overview).
</Note>
...
<Warning>
  **System Message Conflict**: Same rule applies - you cannot set both a `prompt` and `messages`
  array with a system role in the same operation.
</Warning>

<Note>You must include at least one field to update the prompt.</Note>
...
<Warning>
  **Permanent Deletion**: Deleting a prompt removes ALL versions permanently. This action cannot be
  undone.
</Warning>
...
<Warning>
  **Critical**: You cannot set both a `prompt` field and a `messages` array containing a system role
  in the same operation. This will throw an error.
</Warning>
...
<Tip>
  Combine these settings with [Workflows](/workflows/overview), where you can evaluate different
  configurations on the same dataset and compare the results.
</Tip>

### Workflows Integration

A workflow uses these prompt settings in its LLM nodes, so you can:

- **Compare versions**: Evaluate different prompt versions, models, and configurations on one dataset
- **Measure results**: Read the pass rate, cost and duration of each run
- **Save what works**: Save the prompt from the workflow as a new version

<Card
  title="Use prompts in workflows"
  icon="diagram-project"
  href="/prompt-management/features/advanced/workflows"
>
  Pick a prompt in an LLM node, evaluate it on a dataset and save the result as a version.
</Card>
