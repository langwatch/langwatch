...
curl -H "X-Auth-Token: $LANGWATCH_API_KEY" \
  "https://app.langwatch.ai/api/v1/prompts/pizza-prompt?tag=production"

# Using shorthand syntax
curl -H "X-Auth-Token: $LANGWATCH_API_KEY" \
  "https://app.langwatch.ai/api/v1/prompts/pizza-prompt:production"
```
...
<Warning>
  You cannot use both shorthand syntax and the `?tag=` query parameter at the same time. If both are
  provided, the API returns a 422 error.
</Warning>
...
curl -X POST -H "X-Auth-Token: $LANGWATCH_API_KEY" \
  ...
  -d '{"name": "canary"}' \
  "https://app.langwatch.ai/api/v1/prompts/tags"
```
...
| --------------------- | ---------------------------------- | ---------------------------- |
| List all tags         | `GET /api/v1/prompts/tags`            | `platform_list_prompt_tags`  |
| Create tag            | `POST /api/v1/prompts/tags`           | `platform_create_prompt_tag` |
| Rename tag            | `PUT /api/v1/prompts/tags/{tag}`      | `platform_rename_prompt_tag` |
| Delete tag            | `DELETE /api/v1/prompts/tags/{tag}`   | `platform_delete_prompt_tag` |
| Assign tag to version | `PUT /api/v1/prompts/{id}/tags/{tag}` | `platform_assign_prompt_tag` |

<Note>
  Custom tag names must not be purely numeric and cannot be "latest". The `latest` tag is protected
  and cannot be renamed or deleted. `production` and `staging` are default seeded tags that can be
  renamed or deleted like any custom tag.
</Note>
...
<Warning>
  The TypeScript SDK does not currently support `tags.rename()`. Use the REST API or Python SDK for
  renaming tags.
</Warning>
...
| `POST`   | `/api/v1/prompts/tags`          | Create a new tag (`{ "name": "canary" }`)            |
| `PUT`    | `/api/v1/prompts/tags/:tag`     | Rename a tag (`{ "name": "new-name" }`)              |
| `DELETE` | `/api/v1/prompts/tags/:tag`     | Delete a tag (cascades removal from all versions)    |
| `PUT`    | `/api/v1/prompts/:id/tags/:tag` | Assign a tag to a version (`{ "versionId": "..." }`) |

## CLI

CLI tag management is not available yet, see [issue #3090](https://github.com/langwatch/langwatch/issues/3090). Use the REST API, an SDK, or the MCP tools to list, create, rename, delete, and assign tags.
