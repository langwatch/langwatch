...
const prompt = await langwatch.prompts.get("pizza-prompt", { label: "production" });

// Fetch the staging version
const staging = await langwatch.prompts.get("pizza-prompt", { label: "staging" });
```

<Note>
  The TypeScript SDK uses `label` (not `tag`) in the options object. This maps to the same tag concept on the server.
</Note>
