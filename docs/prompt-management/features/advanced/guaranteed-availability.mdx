...
When you use the Prompts CLI to manage dependencies, prompts are **materialized locally** as standard YAML files. The LangWatch SDKs automatically detect and use these materialized prompts when available, and fall back to the API when they are not.
...
<Tabs>
  ...
  <Tab title="TypeScript SDK">
    ...
    import { LangWatch, FetchPolicy } from "langwatch";
    ...
    // The SDK reads the materialized file first and only calls the API when it is missing
    ...
    const response = await openai.chat.completions.create({
      model: prompt.model.replace("openai/", ""),
      messages: compiledPrompt.messages,
    });

    console.log(response.choices[0].message.content);

    // Air-gapped mode: never call the API, fail when the materialized file is missing
    const offlinePrompt = await langwatch.prompts.get("customer-support-bot", {
      fetchPolicy: FetchPolicy.MATERIALIZED_ONLY,
    });
    ```

    **Behavior:**
    1. SDK looks the handle up in `prompts.json` and resolves the materialized file through `prompts-lock.json`
    2. If the file is there, loads the prompt from disk (no network call)
    3. If not, fetches from the LangWatch API
    4. Throws error if both local file and API are unavailable
...
The application runs entirely offline, using only materialized prompts. No LangWatch API access required.
{/\*

## Advanced Fetch Policies (Future Feature)

<Note>
  **Coming Soon**: Advanced fetch policies will provide fine-grained control over when prompts are
  fetched vs. using materialized versions.
</Note>
