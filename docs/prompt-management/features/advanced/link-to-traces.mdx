...
After linking prompts and traces, the prompt information appears in the trace's metadata.
...
// AI SDK 7 emits no spans until an integration is registered. Do this once,
// at startup, before any generateText call.
...
async function customerSupportGeneration() {
  return tracer.withActiveSpan("customer-support-generation", async () => {
    ...
    // Use with AI SDK (native instrumentation support). A compiled prompt
    // carries your system turn inside `messages`, so AI SDK 7 needs
    // allowSystemInMessages to accept it.
