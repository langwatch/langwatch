...
This creates the following structure to manage your prompts:

```bash
├── prompts # directory to create your prompts
│   └── .materialized # where remote fetched prompts are stored
...
Your newly created or fetched prompt is a yaml file in the `prompts/` directory, and looks like this:
...
Once you've created your prompts, you can use them in your application code. The LangWatch SDK fetches and compiles prompts with dynamic variables.
...
The SDK loads prompts dynamically at runtime, and fetches both kinds from LangWatch:
...
<Check>
  The compiled prompt contains fully rendered messages ready to send to your LLM provider, with all
  variables replaced and the correct model configuration applied.
</Check>
