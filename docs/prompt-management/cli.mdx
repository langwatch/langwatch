...
`langwatch prompt create` writes the current flagship model. Prefer modern
defaults — the latest OpenAI generation is `openai/gpt-5.5`. Pick an older
or cheaper model (for example `gpt-4o-mini`) when you intentionally optimize
for cost or latency; the platform applies the same current default when a
prompt is synced without an explicit model.
...
omits it on purpose. Sync stays a pure pass-through: a `pull` writes back
exactly what the platform stored, no value invented or stripped.
