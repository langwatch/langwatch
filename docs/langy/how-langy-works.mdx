...
Langy is a coding-agent harness. A Go manager pod runs one sandboxed [opencode](https://github.com/anomalyco/opencode) worker per conversation, and that worker does the actual reading, testing, and code editing. The worker's only interface to LangWatch is the `langwatch` CLI: it shells out to CLI commands, not to an MCP server or an internal data API. Model calls leave through the LangWatch AI gateway, and code changes leave only as bot-authored GitHub pull requests. Every turn a worker takes is itself a traced run in your LangWatch project.
