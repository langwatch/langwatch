...
description: "Each Langy conversation has one worker process, isolated by Unix identity by default: it reads LangWatch through the langwatch CLI, calls models through the LangWatch AI Gateway, and ships code only as pull requests."
...
Langy runs as a separate service, `langyagent`, next to the LangWatch app. The app dispatches each chat turn to it; `langyagent` keeps one worker process per conversation and reaps it when the conversation goes idle. The manager drives each worker over stdio pipes established when the worker starts, so a worker has no network control listener. Access to its control descriptors depends on the [worker isolation posture](/langy/security/sandbox#the-worker-to-worker-boundary).
...
<Info>
  **Also check:** [Security and the sandbox](/langy/security/sandbox), [Self-hosting overview](/self-hosting/langy/overview)
</Info>
...
graph LR
  ...
  worker["Langy worker<br/>(one per conversation, isolated)"]
...
| `langyagent` | The agent service. Spawns one worker per conversation and drives it over stdio. Single replica; scale vertically. |
| Langy worker | The coding agent itself, holding that conversation's credentials, repo clone, and skills. A distinct Unix identity separates it from siblings by default; `workerIsolation: none` disables that protection. Reaped after ~10 minutes idle, home wiped. |
...
<Steps>
  ...
  <Step title="Your CI runs the tests it wrote">
    The Scenario tests Langy wrote run in your CI on that PR. The PR card returns with the result, so you see the diff and the check status together.
  </Step>
