...
description: "Each Langy conversation is one isolated worker process: it reads LangWatch through the langwatch CLI, calls models through the LangWatch AI Gateway, and ships code only as pull requests."
...
Langy runs as a separate service, `langyagent`, next to the LangWatch app. The app dispatches each chat turn to it; `langyagent` keeps one worker process per conversation and reaps it when the conversation goes idle. The manager drives each worker over the stdio pipes it spawned it with, so a worker opens no port and nothing outside the pod can address it.
...
| Langy worker | The coding agent itself, holding that conversation's credentials, repo clone, and skills. Walled off from its siblings by its own Unix identity. Reaped after ~10 minutes idle, home wiped. |
