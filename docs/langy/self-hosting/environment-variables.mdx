...
These are the environment variables that configure Langy. The agent-side variables (`LANGY_*`, `OPENCODE_*`) are set on the `langyagent` pod through the `charts/langyagent` ConfigMap and Secret; the control-plane variables (`GITHUB_LANGY_*`, `CREDENTIALS_SECRET`) are set on the LangWatch app. Defaults shown are the values the chart or the Go service uses out of the box.

<Info>
**Pairs with:** [Enable Langy via Helm](/langy/self-hosting/helm), [Register the GitHub App](/langy/self-hosting/github-app), [Networking and egress](/langy/self-hosting/networking-and-egress).
</Info>

<Note>
Many `LANGY_*` names in the `packages/langy` and frontend code are exported TypeScript constants, Zod discriminators, and event-type identifiers, not deploy knobs. The variables below are the runtime configuration. `release_langy_enabled` is a feature flag (set in `/ops/feature-flags` or PostHog), not an env var. The GitHub PR rate limit `LANGY_GITHUB_PRS_PER_DAY` is 20, hardcoded, not configurable.
</Note>
...
|---|---|---|---|
| `LANGY_INTERNAL_SECRET` | Shared bearer secret for app-to-agent auth (and the per-frame HMAC base). Held in the `langwatch-langyagent-auth` Secret, injected into both the agent pod and the app. Create it out-of-band. | Yes | none |
...
See [Networking and egress](/langy/self-hosting/networking-and-egress) for how these fit together and the honest limit on enforcement.
...
The mirror lane copies each turn's trace into a LangWatch project you designate, so whoever runs the install can watch Langy work. It is dormant unless both the endpoint and the key are set, and it is off by default for a self-hoster.

| Variable | Description | Required | Default |
|---|---|---|---|
| `LANGY_MIRROR_TRACE_ENDPOINT` | Base URL of the operator ops project for the mirror lane. Set together with `_KEY`. | No | unset (dormant) |
| `LANGY_MIRROR_TRACE_KEY` | The mirror project's API key (a normal project key). Set together with `_ENDPOINT`. | No | unset (dormant) |
...
Set these on the control plane to enable bot-authored pull requests. If `GITHUB_LANGY_PRIVATE_KEY` is unset, the GitHub feature is silently off. See [Register the Langy GitHub App](/langy/self-hosting/github-app) for the registration walkthrough.
