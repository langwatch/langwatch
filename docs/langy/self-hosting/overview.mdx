...
description: Requirements and architecture for running Langy on your own Kubernetes cluster, including the separate agent pod, the gVisor requirement, and the enablement flag.
...
Langy runs as a separate Kubernetes Deployment, the `langyagent` pod, that ships in the LangWatch umbrella Helm chart but is **off by default**. To run Langy self-hosted you enable that pod, satisfy its one hard infrastructure requirement (a `gvisor` RuntimeClass), and flip the `release_langy_enabled` feature flag for the users who should see it. The GitHub App and outbound network egress are both optional and off until you configure them.
...
| User exposure | Gated by the `release_langy_enabled` feature flag, separate from the runtime toggle |
...
<CardGroup cols={2}>
  <Card title="Enable Langy via Helm" icon="ship" href="/langy/self-hosting/helm">
    Create the shared secret, opt the agent in with `chartManaged=true`, and flip the feature flag.
  </Card>
