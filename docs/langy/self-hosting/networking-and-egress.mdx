...
The Langy agent pod has **no outbound HTTPS by default**: `networkPolicy.allowExternalHttps` is `false`, so a fresh install cannot reach GitHub, npm, or any external host. To let Langy clone and push (open PRs), you enable egress and the pod's traffic then passes through an L7 egress adapter that require-TLS, allow-lists destinations, and enforces a GitHub FQDN floor. LLM traffic never uses this path; it goes to the in-cluster [AI Gateway](/ai-gateway/self-hosting/helm).
...
See [Environment variables](/langy/self-hosting/environment-variables#egress) for the full table.
