...
gVisor is a hard requirement, not a recommendation. The Helm chart for the agent (`charts/langyagent`) has a render-time `fail` guard: with the agent enabled and no `gvisor` RuntimeClass set, the chart refuses to render. The cluster must define a `gvisor` RuntimeClass before the pod can schedule.

On a cluster with no sandboxed runtime, opting out takes an explicit pair, `runtimeClassName: ""` plus `acceptUnsandboxedRuntime: true`; the chart refuses to render on the first value alone. You then lose the pod-to-host boundary and keep only the in-pod walls below.
