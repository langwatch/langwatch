...
On LangWatch Cloud the pod always runs under gVisor. A self-hosted umbrella-chart install runs on the node's normal runtime by default, so it installs on any cluster, and states in its install notes that the pod-to-host sandbox is absent; you then keep only the in-pod walls below. Harden it with `langyagent.runtimeClassName: "gvisor"` once the cluster defines the RuntimeClass.

The agent's own chart (`charts/langyagent`) keeps a render-time `fail` guard: managed, with no runtime set and the unsandboxed acceptance withdrawn (`acceptUnsandboxedRuntime: false`), it refuses to render rather than dropping the sandbox silently.
