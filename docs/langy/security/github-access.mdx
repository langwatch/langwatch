...
Langy contributes to your code in exactly one way: **bot-authored pull requests through a GitHub App**. There is no per-user OAuth and no stored access token. Langy never pushes to a branch you have not reviewed and never merges its own PR. A human reviews and merges.
...
Langy can open pull requests only on the repositories the app is installed on. Installing the app on a repository is what grants access; there is no separate grant. A single LangWatch organization can install the app on several GitHub accounts, and each installation is listed separately.
