...
The worker streams its spans through the trace relay, authenticated with the per-session key and a fail-closed resource-attribute allow-list. A platform-owned `langy.turn` root span is recorded, and the AI gateway records the `gen_ai` spans, including the full prompt and completion, for the LLM calls.
