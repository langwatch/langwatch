...
Every Langy turn is an agent run, and it is traced like any other agent run in your project. The worker streams its spans through the trace relay (authenticated with the per-session key and a fail-closed resource-attribute allow-list), a platform-owned `langy.turn` root span is recorded, and the AI gateway records the `gen_ai` spans (including the full prompt and completion) for the LLM calls. You watch Langy work with the same tools you use to watch your own agents, and the cost of a Langy turn shows up as cost in your project.
...
- **`git` / `gh` / `npm` traffic** reaches the internet only when `allowExternalHttps` is on and the destination passes the L7 egress adapter. The operator floor is `github.com`, `api.github.com`, and `codeload.github.com`; a project can add to the allow-list.
...
<Note>
On a self-hosted install the mirror lane is **off** unless you set both variables, and you choose the tier. On LangWatch SaaS the default is the `content` tier, with specific organizations restricted. If you are self-hosting and do not want any operator copy of Langy's traces, leave `LANGY_MIRROR_TRACE_ENDPOINT` and `LANGY_MIRROR_TRACE_KEY` unset.
</Note>

<Info>
**Pairs with:** [How Langy is isolated](/langy/security/sandbox), [What Langy can do in your GitHub](/langy/security/github-access), [Models and bring your own key](/langy/models).
</Info>
...
- [Self-hosting: environment variables](/langy/self-hosting/environment-variables)
