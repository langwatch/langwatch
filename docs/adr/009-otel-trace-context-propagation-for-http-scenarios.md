...
For SDK-based (local) runs, `JudgeSpanCollector` collects spans in-memory because the agent-under-test runs in the same process. For HTTP targets, the user's spans are generated in a separate service, exported via their own OTEL pipeline, and eventually land in LangWatch's Elasticsearch. The judge is blind.
...
│      ↓             │           │      ↓             │
│ Judge (no spans!)  │           │ Spans → ES         │
└────────────────────┘           └────────────────────┘
```

The core challenge is timing: spans arrive in ES asynchronously. The user's OTEL SDK batches spans (typically 5s intervals), exports them to LangWatch's collector, and ES indexes them. By the time the judge runs, spans may or may not have arrived.

## What we considered

**Option A: In-process span collection via OTEL exporter.**
Route the user's spans directly to the child process via a custom OTEL exporter endpoint. Avoids ES entirely. Rejected because it requires SDK changes, a new network endpoint per child process, and breaks the existing architecture where the child process is a short-lived fire-and-forget process.

**Option B: W3C Trace Context + query ES at judge time (chosen).**
Propagate `traceparent` headers so the user's spans link to our trace. Query ES for those spans before the judge evaluates. Simple, no SDK changes for header propagation, uses existing ES infrastructure.
...
We will propagate W3C trace context headers in HTTP adapter requests and query ES for linked spans before judge evaluation.
...
The trace ID is captured explicitly during HTTP adapter calls (when `injectTraceContextHeaders` is invoked) and stored on the adapter instance. After the conversation completes, the trace ID is read from the adapter and passed to `EsBackedJudgeAgent.setTraceId()` before judge evaluation. This avoids relying on ambient OTEL context at judge time, which would be fragile since the judge may run in a different span context than the adapter.

### Span collection from ES

After the conversation completes and before judge evaluation, the system queries ES for spans matching the propagated trace ID. Because spans arrive asynchronously:
...
- If the ES query fails, a synthetic error span (`langwatch.span_collection.error`) is created with the failure reason, so the judge sees the infrastructure problem in its digest.

### Judge integration

The `@langwatch/scenario` SDK's `judgeAgent({ spanCollector })` already accepts a `JudgeSpanCollector` instance. We create an ES-backed implementation that pre-populates spans from the ES query. The `JudgeSpanDigestFormatter` renders these spans into the judge's prompt as `<opentelemetry_traces>`.
...
ES query failures are different: they represent infrastructure problems. The synthetic error span ensures this is visible in the judge's digest and reasoning.
...
- Uses existing ES query infrastructure and `JudgeSpanCollector` interface
...
- Reconstructing `ReadableSpan` objects from ES documents requires an adapter layer
