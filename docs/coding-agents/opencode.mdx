...
LangWatch mints one personal key per device and never replaces another device's key, so setting up a second machine does not stop this one. A machine cloned from another keeps the parent's key until it mints its own. One workspace holds up to ten live personal keys per tool: past that, a new device retires the key that has gone unused the longest. If that machine comes back, it mints a fresh key on its next session and asks you to restart OpenCode.
