---
title: OpenCode Monitoring with OpenTelemetry
sidebarTitle: OpenCode
description: Monitor opencode with OpenTelemetry. Send sessions, cost and traces to LangWatch with the LangWatch command-line tool.
keywords:
  - opencode monitoring
  - opencode usage tracking
  - opencode telemetry
  - opencode opentelemetry
  - opencode cost
---
...
<Note>
  ...
  your own providers, LangWatch never creates one for you.
</Note>

To route the model calls themselves through the gateway with a service key, see [Route through the LangWatch AI Gateway](#route-through-the-langwatch-ai-gateway) below.
...
<Note>
  The shell function needs zsh, bash or fish. On Windows, start OpenCode with `langwatch opencode`
  each time instead.
</Note>
...
| `langwatch instrument opencode --project <id-or-slug>` | A team project. See [Team projects](/coding-agents/team-projects).                                            |
| `langwatch instrument opencode --key <ingest-key>`     | The project that owns the key, with no login. See [Headless machines and CI](/coding-agents/headless-and-ci). |
...
- **[Sessions](/coding-agents/sessions)**: one row per OpenCode session, with the context it carried and how long it worked.
...
OpenCode's telemetry carries the arguments of each tool call and the result it returned, which is what makes a trace readable. To keep content out of storage, set the content categories to **Drop** in the [data privacy policy](/platform/data-privacy): the strip happens at ingestion, before storage, and tokens, cost, model and timing survive. Who can open a stored session's content is covered on [Privacy](/coding-agents/privacy).

## Route through the LangWatch AI Gateway

Everything above is telemetry: opencode keeps your own provider logins. Choosing **Using an API key** on the first wrapper run instead routes the model calls through the LangWatch AI Gateway with a virtual key, which adds budgets and model policies on top of the same telemetry.

The wrapper starts opencode with `OPENAI_BASE_URL` and `ANTHROPIC_BASE_URL` set to the gateway URL followed by `/v1`, and `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN` set to your virtual key. opencode appends `/chat/completions` and `/messages` to the base URL without `/v1`, so the base URL must end in `/v1`. It detects the Anthropic provider from `ANTHROPIC_API_KEY` only, which is why that variable is set as well. To route a plain `opencode` run with a service virtual key, set the same variables:

```bash
export OPENAI_BASE_URL="https://gateway.langwatch.ai/v1"
export OPENAI_API_KEY="vk-lw-..."
export ANTHROPIC_BASE_URL="https://gateway.langwatch.ai/v1"
export ANTHROPIC_API_KEY="vk-lw-..."
export ANTHROPIC_AUTH_TOKEN="vk-lw-..."
opencode
```

Name the model in opencode's own `provider/model` form, for example `--model anthropic/claude-sonnet-4-6` or `--model openai/gpt-5-mini`.
...
<Accordion title="The command reports that it could not write the shell start file">
  The shell is not zsh, bash or fish. Start OpenCode with `langwatch opencode` each time, which
  needs no start file.
</Accordion>

<Accordion title="The OpenTelemetry flag stays off">
  You set `experimental.openTelemetry` to `false` yourself at some point. LangWatch never overrides
  that choice. Set it to `true` in `~/.config/opencode/opencode.jsonc`, or remove the line and run
  `langwatch instrument opencode` again.
</Accordion>
