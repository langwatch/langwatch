...
An organization that must not store conversational content sets input, output and system instructions to **Drop** in the [data privacy policy](/platform/data-privacy). LangWatch strips dropped content at ingestion, before storage, on every ingestion path, coding-agent telemetry included.
...
**Also check:** [Data privacy policy](/platform/data-privacy) for the full policy reference: scopes, dispositions, audiences and PII redaction.
