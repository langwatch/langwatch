...
Long context is not free even when it fits the window. Every cache rebuild re-bills the whole context at write rates, compactions burn turns, and cost per turn climbs as a session swells.

Between "fresh session" and "the window is full" there is a size your sessions should stay under. It is different per team, per model and per working style, so measure your own sessions to find it.

This investigation runs as a skill for your coding agent. It reads your sessions from LangWatch, computes the economics, and writes a report with your number.
...
- **Compaction rate**: how often the agent had to compress its conversation to keep going.
...
The report ties each recommendation to your own evidence: start a fresh session instead of pushing a long one further; offload research to sub-agents, whose context stays out of the main session; watch whether compactions arrive too late to save the session that needed them.
