...
"Provider X is 80% cheaper" compares price cards, not workloads. A coding agent's input is mostly cache reads billed at a fraction of fresh input, so a provider with a lower sticker price and no cache discount can cost more on your actual traffic. A real comparison prices your own month of usage under each candidate, and that is what this investigation does.
...
- **No cache**: for candidates without caching, your cache reads rebill as fresh input. This row is where most cheap-provider claims die.
...
Repricing prices the same usage; a different model changes the usage. The report states window differences and obvious capability constraints and leaves the quality judgment to you. It is a cost analysis, not a migration verdict.
