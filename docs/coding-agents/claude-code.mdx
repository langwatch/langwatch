...
To wire a single repository instead of the whole machine, put the block in `.claude/settings.json` in the repository and keep the key and the content flags in a gitignored `.claude/settings.local.json`. See [Per-repository telemetry](/coding-agents/claude-code-usage#per-repo-telemetry-with-plain-claude-no-wrapper). To apply the block to every machine of a fleet through managed settings, see [Headless machines and CI](/coding-agents/headless-and-ci#roll-out-to-a-managed-fleet).
