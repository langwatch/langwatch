...
| `OTEL_LOG_ASSISTANT_RESPONSES` | The assistant's response text, on a light event of its own |
| `OTEL_LOG_RAW_API_BODIES` | The full request and response JSON of every model call — opt-in, for debugging |
...
`langwatch claude` and `langwatch instrument claude` set `OTEL_LOG_ASSISTANT_RESPONSES`, the light event added in Claude Code 2.1.193 that carries just the assistant's reply text. They do **not** set `OTEL_LOG_RAW_API_BODIES`: that flag makes Claude Code serialise the entire request (the whole conversation) and the whole response to JSON on every model call, which is avoidable work that grows with the session, and the light event already gives LangWatch the response text. Add `OTEL_LOG_RAW_API_BODIES=1` yourself if you need the full request and response body for debugging; LangWatch still reads it when present.
...
The wrapper starts Claude Code with `ANTHROPIC_BASE_URL` set to the gateway URL and `ANTHROPIC_AUTH_TOKEN` set to your virtual key, and removes `ANTHROPIC_API_KEY` from the environment, because Claude Code warns when both are set. Claude Code appends `/v1/messages` to the base URL itself, so set the base URL with no path. To route a plain `claude` run with a service virtual key, set the same two variables:
...
A bare `claude-*` model name routes to your Anthropic credential. Write `provider/model`, for example `bedrock/us.anthropic.claude-haiku-4-5-20251001`, to reach the model through another provider, or map a name on the virtual key. See [Model naming](/ai-gateway/model-naming) and [Model aliases](/ai-gateway/model-aliases).
...
<AccordionGroup>
  ...
  <Accordion title="The trace has no response">
    Response text is off. `langwatch claude` and `langwatch instrument claude` set `OTEL_LOG_ASSISTANT_RESPONSES=1` for you, so this only happens on a hand-written configuration or a session that was open before you re-ran the wrapper (restart Claude Code so it re-reads `~/.claude/settings.json`). Claude Code reports the assistant's response text on its lighter `assistant_response` event (needs `OTEL_LOG_ASSISTANT_RESPONSES=1`, and carries only the response text) or, if you opt in, on its `api_response_body` event (needs `OTEL_LOG_RAW_API_BODIES=1`, and carries the response JSON; the request JSON arrives on the separate `api_request_body` event). Set `OTEL_LOG_ASSISTANT_RESPONSES=1` for just the response text.

    When `OTEL_LOG_ASSISTANT_RESPONSES` is unset, Claude Code falls back to the value of `OTEL_LOG_USER_PROMPTS`, so prompt logging alone can still emit the response text. The response is redacted when the flag is set to `0`, or when it is unset and prompt logging is off. Don't rely on the fallback: set it explicitly.
...
Use this when you cannot install the LangWatch command-line tool. The block below mirrors the variables `langwatch instrument claude` writes for you. `OTEL_LOG_RAW_API_BODIES` is included commented out: the wrapper does not set it, but you can turn it on to also export the full request and response JSON for debugging.
...
# export OTEL_LOG_RAW_API_BODIES=1   # opt-in: full request/response JSON, for debugging
...
<Warning>
  The `OTEL_LOG_*` flags put your session content in LangWatch. Read [Content and privacy](#content-and-privacy) before you use them where the content matters.
</Warning>
