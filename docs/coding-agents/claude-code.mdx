...
<AccordionGroup>
  ...
  <Accordion title="The trace has no response">
    `OTEL_LOG_ASSISTANT_RESPONSES` is missing. `langwatch claude` and `langwatch instrument claude` set it for you, so this only happens on a hand-written configuration or a session that was open before you re-ran the wrapper (restart Claude Code so it re-reads `~/.claude/settings.json`). Claude Code reports the assistant's response text on its lighter `assistant_response` event (needs `OTEL_LOG_ASSISTANT_RESPONSES=1`, and carries only the response text) or, if you set it yourself, on its `api_response_body` event (needs `OTEL_LOG_RAW_API_BODIES=1`, and carries the full request and response JSON). Set `OTEL_LOG_ASSISTANT_RESPONSES=1` for just the response text.
