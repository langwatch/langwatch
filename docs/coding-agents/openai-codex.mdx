...
The general model, both reporting channels and their limits, is covered on [Repository and PR attribution](/coding-agents/repository-and-pr-attribution). The Codex specifics live here, because Codex records its directory once, when the session starts: a `cd` inside the session does not change it, so the declared channel carries more weight than on other agents.
