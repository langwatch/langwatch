...
- **A span your app sent us.** The individual steps that make up a request to your agent or LLM application, or to a coding assistant you track. Logs and metrics arrive alongside them and are not metered.
...
Spans dominate the count for most customers, but testing produces billable events: an experiment over 100 rows with 2 evaluators is roughly 300 events. There are no separate charges per feature on top of this, and everything you create (prompts, evaluators, datasets, dashboards) is free.
...
<AccordionGroup>
  ...
  <Accordion title="What counts as a billable event?">
    Billable events are the spans your app or a tracked coding assistant sends, evaluation results, the steps of an experiment run, and the steps of a scenario simulation. Logs and metrics ride along with those spans and are not metered. Pricing is based on the total number of ingested events per billing period. [The full list is here](/pricing/billable-events), including how many events a sweep or a simulation produces, what is never billed, and how retries are deduplicated.
  </Accordion>
