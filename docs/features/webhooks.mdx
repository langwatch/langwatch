...
- **One batch is one message, and we cap it at 256 KiB** including the attributes, which is the size a standard queue accepts without extended-payload storage. A batch over the cap fails terminally and says so; lower `max_batch_size` so each delivery carries fewer events.
