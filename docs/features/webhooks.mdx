...
1. Fix the receiver.
2. Re-enable: `POST /endpoints.update {"id": "<endpoint id>", "status": "active"}`.
...
`POST /endpoints.getHealth {"id": "<endpoint id>"}`:
