...
- **Standard queues only.** A `.fifo` queue is refused at save time. Delivery is at-least-once with dedup on the envelope `id`, which is what a standard queue provides; ordering was never promised, and FIFO caps at 300 messages a second.
...
- **One batch is one message, capped at 256 KiB** including the attributes. A batch over the cap fails terminally and says so; lower `max_batch_size` so each delivery carries fewer events.
