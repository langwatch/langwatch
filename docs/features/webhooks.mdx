...
<Note>
Webhook endpoints are an **Enterprise** feature (the `webhookEndpointsEnabled` plan flag). Without it the API answers `402` with `error.code = "enterprise_plan_required"` and the settings page shows an upgrade state. Self-hosted enterprise licenses include it.
</Note>
...
Manage endpoints under **AI Gateway > Webhooks** (`/settings/gateway/webhooks`) or over the HTTP API with an organization API key:

Every operation is a `POST` to a named operation, and every argument travels in
the JSON body — including the endpoint id.

```
POST /api/webhooks/endpoints.create             webhookEndpoints:manage
POST /api/webhooks/endpoints.list               webhookEndpoints:view
POST /api/webhooks/endpoints.get                webhookEndpoints:view
POST /api/webhooks/endpoints.update             webhookEndpoints:manage
POST /api/webhooks/endpoints.archive            webhookEndpoints:manage
POST /api/webhooks/endpoints.rollSecret         webhookEndpoints:manage
POST /api/webhooks/endpoints.test               webhookEndpoints:manage
POST /api/webhooks/endpoints.listDeliveries     webhookEndpoints:view
POST /api/webhooks/endpoints.getHealth          webhookEndpoints:view
POST /api/webhooks/eventTypes.list              webhookEndpoints:view
POST /api/webhooks/events.list                  webhookEndpoints:view
POST /api/webhooks/events.get                   webhookEndpoints:view
```

Pin a version by putting it in the path — `/api/webhooks/2026-08-07/endpoints.list`
— or call `/latest/` for the newest, or the bare path above, which is the one
the reference documents.

Create an endpoint subscribed to the billing stream:

```bash
curl -sS -X POST https://app.langwatch.ai/api/webhooks/endpoints.create \
...
`POST /api/webhooks/eventTypes.list` returns the live catalog. Types are grouped by family (the first dotted segment), which is also what the settings UI renders as checkbox groups and what the `gateway.*` wildcard matches.
...
1. Fix the receiver.
2. Re-enable: `POST /api/webhooks/endpoints.update {"id": "<endpoint id>", "status": "active"}`.
...
`POST /api/webhooks/endpoints.getHealth {"id": "<endpoint id>"}`:
...
`POST /api/webhooks/endpoints.listDeliveries {"id": "<endpoint id>"}` lists recent attempts, newest first: the attempt number, envelope count, outcome, **your endpoint's HTTP status**, latency, and the error text when transport failed. Your receiver's responses are recorded, so debugging a rejecting endpoint starts here rather than in your own logs.
...
`POST /api/webhooks/events.list`, with `from`, `to` and optionally `type`, `cursor` and `limit` in the body, is the organization's emitted-events log: cursor-paged, newest first, filterable by type. Webhooks are push over this log, never the only copy of it, so a consumer that missed deliveries can list what was emitted and recover.
...
`POST /api/webhooks/events.get`, with `{ "id": "..." }`, reads one event back by the `id` its envelope carried. A `404` means the log cannot answer for that id, and deliberately does not say which reason applies.
