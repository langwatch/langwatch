...
In the LangWatch dashboard, use the custom chart builder to plot any data LangWatch collects or generates, and to set how it is displayed. You then use our REST API to fetch the graph data.
...
ENDPOINT="https://app.langwatch.ai/api/v1/analytics"
...
<Frame>
  <img
    className="block"
    src="/images/screenshot-show-json.png"
    alt="Custom graph in the LangWatch dashboard"
  />
</Frame>

Within this modal, you'll find the JSON payload required for the precise custom analytics
data. Copy this payload and paste it into the body of your REST POST request.
