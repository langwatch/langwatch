...
A conversation past that ceiling is still judged. It is cut to fit, the row
comes back marked truncated, and the cut keeps the opening. If what you are
asking about is how a conversation *ended*, write the statement yourself with
`conversation_bounded`, which keeps both ends and writes a marker naming how
many turns it dropped from the middle:
