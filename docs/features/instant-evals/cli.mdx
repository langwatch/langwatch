...
| `--criteria <text>` | For a yes or no question: what counts as yes, then what counts as no. Write it twice |
...
| `--threshold <probability>` | Where a yes or no question's probability becomes a pass |
...
`span.attribute.<key>`), evaluation fields (`eval`, `evaluator*`, `guardrail`),
