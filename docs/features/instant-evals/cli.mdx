...
The row count is exact. The price is not: it comes from the first fifty rows
the statement returns, so a selection whose rows vary a lot in length costs
more or less than the estimate said. Read it as the order of magnitude, and
read the finished run's own token count for what it actually cost.
...
Stops the run before its next page. You keep every judgement it had already
written, and you can read them the same way. Cancel a run that has already
finished and you get back a 409.
