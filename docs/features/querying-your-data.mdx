...
datasets. It returns columns, not traces.
...
datasets and columns, the filter fields and syntax, worked examples in each, and
the decision table above. Every example in it is checked against the validator
and the translator on every build, so an example it publishes runs.
...
`--format csv` writes a header row with RFC 4180 escaping. `-o` writes to a file.

```bash
langwatch query --sql-file export.sql --format jsonl -o train.jsonl
```
...
```bash
langwatch query --sql-file export.sql --page-by keyset --format jsonl -o train.jsonl
```
