...
description: A comparison run sends every scenario to two or more targets in one run, so you read the pass rate, the cost and the latency of each side by side.
---
...
Secret parameters stay at the run level and are shared by every target.
...
On the REST API the same values go in `runParameters` on each target, and `parameters` at the run level holds what every target shares. See [Run from CI](/agent-testing/run-from-ci#compare-two-agents-or-one-agent-on-two-settings) for the request body.
