...
keywords: langwatch, agent testing, agent simulations, local development, tunnel, langwatch agent dev, cloudflare tunnel, development loop
description: Run platform test suites against the agent on your machine, edit the code, and run them again, with no deployment in between.
---
...
If the agent is not registered yet, [Connect your agent](/agent-testing/connect-your-agent) covers the registration.

## The loop

1. Start the agent process on your machine, then start the tunnel: `langwatch agent dev --port 8010`.
2. Run the test suite, from **Agent Testing > Scenarios** or with `langwatch test-suite run <test-suite-id> --target http:<agent-id> --wait`.
3. Read the results, change your code, and run the test suite again. The tunnel keeps forwarding to the port, so a restarted or hot-reloaded server needs no new tunnel.
...
A tunnel URL is public, and a local agent process usually has real model provider keys. `langwatch agent dev` therefore creates a per-session secret, puts a proxy in front of your local port that rejects any request without the `X-LangWatch-Dev-Secret` header, and adds that header to the agent's configuration for the session. Only the platform's scenario calls send the secret. On exit, the command removes both the proxy and the header row.
...
The agents list and the run target selector show a **local tunnel** badge on the repointed agent, so the team sees that the agent currently points at a developer machine.
...
Use `--tunnel-url` with your own tunnel when you hit these limits.
