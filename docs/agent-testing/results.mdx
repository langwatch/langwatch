...
**Group by** decides what a row is:
...
Press a run plan. The sidebar lists its runs as **Run #n** with the note, the time and the pass rate; **Load More...** reads older ones. The header of the selected run shows **Pass**, **Completed**, **Avg Agent Latency**, **Avg Agent Cost**, **Total Duration** and **Total Cost**.
...
The **Table** view has one row per scenario execution: **Result**, **Scenario**, **Evaluators** and **Time · cost**. The row menu offers **Open the conversation**, **Rerun this scenario** and **Edit scenario**. The **Grid** view shows the same executions as cards, one colour per result.
...
|---|---|---|
| `Passed` | Every criterion passed. | |
| `Failed` | At least one criterion failed. | Read **JUDGE REASONING**, then the transcript. |
