...
Run a test suite against it by name:

```bash
langwatch test-suite run 'Smoke' --target connected:support-agent --wait
```

`connected:<name>` runs the agent in `development`. When no process is connected there but one other environment is online, the run uses that one. When several are online, or none, the run is refused with `agent_environment_unresolved` and names them: pick one as `connected:<name>@<environment>`. `connected:<agent-id>` is the same target, and `langwatch agent list --format json` prints the id.
...
A connected agent column in the experiments workbench has an optional **attachment** input beside **input**. Map a dataset `image` or `file` column to it. See [Images and files in datasets](/datasets/dataset-images).

The last user message then carries content parts instead of plain text. It holds the text of the turn, plus an `image_url` part for a picture, an `input_audio` part for a recording, or a `file` part with the file name and the file data in base64.
...
|---|---|
| `concurrency` | How many calls one instance takes at the same time. Default 10. When every instance is full, the platform waits and retries within the call's deadline. |
...
<Tabs>
  <Tab title="Python">
    ...
    |---|---|---|
    | `name` | required | The agent's name, and the `connected:<name>` target. |
    ...
    | `concurrency` | `10` | Calls this instance takes at the same time. |
