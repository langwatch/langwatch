...
description: An evaluator attached to a test suite runs after every scenario run, reads the conversation, the scenario's fields or the trace, and reports beside the judge's verdict. A required pass or fail evaluator fails the scenario when it fails, and a score-only evaluator never gates.
---
...
For example, on a text-to-SQL agent the suite declares `golden_sql` and `table_schema`, and **SQL Query Equivalence** reads `output` from `tool_calls.run_sql.input`, `expected_output` from `fields.golden_sql` and `expected_contexts` from `fields.table_schema`.
...
The drawer on that attachment: one mapping per input under **Variables**, and the **Required to pass** switch under them. The steps to set this up are on [Grade runs with evaluators](/agent-testing/grade-runs-with-evaluators).
...
| Status | Meaning |
|---|---|
| `passed`, `failed` | The evaluator's verdict. A failed required evaluator fails the scenario, and the verdict line names it. A required input the trace never delivered, no such tool call or no retrieved contexts, is recorded as `failed` with the reason naming it. |
...
Also check: [Grade runs with evaluators](/agent-testing/grade-runs-with-evaluators), [Test suites](/agent-testing/test-suites), [Scenarios](/agent-testing/scenarios), [Results](/agent-testing/results), [Linking your traces](/agent-testing/linking-your-traces).
