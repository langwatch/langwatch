...
An input named `expected_output`, `expected_contexts` or another `expected_*` reads the field whose name matches, so `expected_output` matches `golden_sql` and `expected_contexts` matches `table_schema`. A suite with one field maps an expected input to it. A tool call or the spans are never inferred: set them by hand. `spans` hands a code or LLM evaluator the whole run, every span of every trace as JSON, when one tool call at a time is not enough.
