...
Mappings are inferred when an evaluator is attached: an input named `input`, `question` or `user_input` reads the first user message; `output`, `response` or `answer` the last agent message; `transcript`, `conversation` or `messages` the transcript; `contexts` or `retrieved_contexts` the trace's contexts; an input named `expected_output`, `expected_contexts` or another `expected_*` reads the field whose name matches (`expected_output` matches `golden_sql`, `expected_contexts` matches `table_schema`). A suite with one field maps an expected input to it. A tool call is never inferred: set it by hand.
