...
<img
  src="/images/agent-testing/scenario-parameters-form.png"
...
The API accepts the same declarations: `POST /api/v1/scenarios` and `PATCH /api/v1/scenarios/{id}` take a `parameters` array of `{ name, description, defaultValue, secret }` objects.
...
<img
  src="/images/agent-testing/scenario-parameters-body-template.png"
...
<Note>
  A parameter reaches a downstream node only if the entry node has an edge carrying it. Adding a
  parameter to the run does not add the edge. Open the workflow and connect the new entry field to
...
<img
  src="/images/agent-testing/scenario-parameters-run-dialog.png"
  alt="The test suite run confirmation dialog with a Parameters block, the fixture field prefilled from the scenario's default value"
  width="100%"
/>
...
<Warning>
  A secret is hidden in the record of the run, not in the conversation. When the agent under test
  repeats the value in its own answer, that answer is conversation content and the run stores it.
</Warning>
...
<img
  src="/images/agent-testing/scenario-parameters-run-drawer.png"
