...
Next to `params`, the same fields render `{{ messages }}`, `{{ input }}` and `{{ threadId }}` from the conversation, and `{{ traceId }}` and `{{ traceparent }}` from the turn's trace context. See [Linking your traces](/agent-testing/remote-traces) for what the trace variables carry.
...
See [Run from CI](/agent-testing/run-suites-from-ci) for the full request and response, and for the poll that waits on the batch.
...
<CardGroup cols={2}>
  ...
  <Card title="Simulations getting started" icon="rocket" href="/agent-testing/getting-started">
  ...
  <Card title="Run from CI" icon="code" href="/agent-testing/run-suites-from-ci">
