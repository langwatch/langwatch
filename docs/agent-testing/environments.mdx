...
<Note>
  An HTTP agent that runs on your machine is a different case: the platform calls it, so it needs a reachable URL. `langwatch agent tunnel` covers that, see [Other ways to connect](/agent-testing/other-ways-to-connect).
</Note>
