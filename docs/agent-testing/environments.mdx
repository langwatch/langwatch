...
<Note>
  An HTTP agent that runs on your machine is a different case: the platform calls it, so it needs a reachable URL. `langwatch agent dev` covers that, see [Other ways to connect](/agent-testing/other-ways-to-connect).
</Note>
...
Quote the target, because the shell reads `?` and `&` itself. A value after `?` is a [run parameter](/agent-testing/scenario-parameters) for that target alone.
