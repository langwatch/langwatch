...
- The ElevenLabs key needs `convai_read`, `convai_write` and conversation read — a key missing any of these fails silently at a different step.
...
- LangWatch never sends prompt overrides. Enabling overrides on the agent changes nothing here — the agent's own ElevenLabs prompt is what gets tested.
