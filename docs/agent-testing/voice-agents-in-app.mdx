...
**Phone number targets** dial the agent over the phone. A scenario run places an outbound call to the target number through your Twilio account and the simulated caller speaks to whoever answers. The Phone number option appears in the agent drawer once your project has a Twilio provider in **Settings > Model Providers** (account SID, auth token and from-number); until then the drawer points you there. Phone targets have no **Talk to it** button, because a phone call has no browser leg. See [issue #8014](https://github.com/langwatch/langwatch/issues/8014).
