...
The Phone number option appears in the agent drawer once your project has a Twilio provider in **Settings > Model Providers** (account SID, auth token and from-number); until then the drawer points you there. A phone target supports **Run a scenario** only. **Talk to it** and **Call it myself** are not available for it, because a phone call has no browser leg. See [issue #8014](https://github.com/langwatch/langwatch/issues/8014).

On a self-hosted deployment, set `VOICE_PUBLIC_BASE_URL` only when the voice worker has its own hostname; when it is unset the app's own public base host is used. Set `VOICE_WS_PORT` to pin the port the phone transport listens on, so your public origin can route to it; it defaults to an ephemeral port.
