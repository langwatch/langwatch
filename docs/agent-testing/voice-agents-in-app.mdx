...
| `VOICE_RUNS_MAX_CONCURRENT` | 2 | Per pod, per project. More voice runs queue instead of failing. Text runs are unaffected. On a multi-pod deployment the effective ceiling for a project is this number times the pod count, since each pod tracks its own count. |
