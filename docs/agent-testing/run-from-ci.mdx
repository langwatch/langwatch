...
A connected agent is `connected:<name>`, which runs it in `development` or in the one other environment it is online in. `connected:<name>@<environment>` names the environment.

`--wait` polls until the batch is complete, for up to 45 minutes, and exits non-zero when a run failed, which is what fails the job. `--wait 90` raises the limit to ninety minutes. The test suite is named by its id or by its name.
...
<Note>
  The `/api/v1/suites` family still answers and is deprecated. Move CI jobs to `/api/v1/test-suites`
  and `/api/v1/run-plans`.
</Note>
...
A run goes against every target you name, so naming two targets runs each scenario against both and stores the results under one batch, one column per target on the results page. A target can also have its own parameter values, so the same agent named twice with different values is a comparison of that agent on two settings. [Compare agents](/agent-testing/compare-agents) covers how the platform shows them.
...
`GET /api/v1/simulation-runs/batches/{batchRunId}` answers with the counts of one batch:

```bash
curl "https://app.langwatch.ai/api/v1/simulation-runs/batches/batch_xyz789" \
  -H "X-Auth-Token: ${LANGWATCH_API_KEY}"
```
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  body=$(curl -s -w '\n%{http_code}' --connect-timeout 10 --max-time 30 \
    "https://app.langwatch.ai/api/v1/simulation-runs/batches/${BATCH_RUN_ID}" \
    -H "X-Auth-Token: ${LANGWATCH_API_KEY}")
