...
<Note>
  The `/api/suites` family still answers and is deprecated. Move CI jobs to `/api/v1/test-suites`
...
| `runningCount` | Runs that are queued, in progress, or waiting on their evaluators.                                                  |
| `settledCount` | Runs that are none of those: passed, failed, errored, cancelled or stalled, with every attached evaluator recorded. |
