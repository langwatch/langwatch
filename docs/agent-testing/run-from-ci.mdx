...
`--wait` polls until the batch is complete, for up to ten minutes, and exits non-zero when a run failed, which is what fails the job. A batch that needs longer, such as a suite with repeats, is polled with the batch endpoint below. The test suite is named by its id or by its name.
