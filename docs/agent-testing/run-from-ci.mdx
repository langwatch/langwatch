...
keywords: langwatch, agent testing, agent simulations, ci, test suite, run plan, batch, batchRunId, isComplete, poll, wait
...
Add `--format json` (or `-o json`) to get one final document on stdout instead of the progress lines. It holds `outcome`, `tallies` and the per-run `results`, so a job step can read the verdict without parsing prose.
...
A run goes against every target you name, so naming two targets runs each scenario against both and files the results under one batch, one column per target on the results page. A target may also carry the parameter values it alone runs with, so the same agent named twice with different values is a comparison of that agent on two settings. [Compare agents](/agent-testing/compare-agents) covers how the platform shows them.
...
The suffix after the question mark is a query string. Both halves are percent-decoded, so a value that holds `?` or `&` must be written as `%3F` or `%26`. A value is read as the type it looks like: `true` and `false` become booleans, a plain number becomes a number, everything else stays text.
...
`runParameters` are merged over the run-level `parameters`, and the target wins. Use `parameters` for what every target shares, such as a fixture id or a tenant, and `runParameters` for what tells the targets apart.
...
The request has its own time limits, so a read that hangs cannot hold the job open. The deadline is read between requests, so the job stops within one request and one sleep of it, which is 40 seconds with the values above.

`langwatch test-suite run ... --wait` does the same poll, and its exit code follows the results.

## Give the batch a note

`--note` keeps one short line with the batch: why it was run, or what changed. Every run in the batch keeps it, and the platform shows it beside the run.
