...
One input and one expected output cannot test an agent. An agent calls tools, keeps state and takes several turns to reach an outcome. Agent testing checks the whole conversation, including what the agent did internally.

You write a scenario: the situation the user is in, and the criteria the agent must meet. For example, a customer asks for a refund on an order from last year; the agent must look up the order and must not promise the refund.

LangWatch plays the scenario against your agent and shows a pass or fail for each criterion, with the judge's reasoning and the full conversation.
