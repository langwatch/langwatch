...
Write a scenario: the situation the user is in, and the criteria the agent must meet. For example, a customer asks for a refund on an order from last year; the agent must look up the order and must not promise the refund. LangWatch plays the scenario against your agent with a simulated user, and a judge gives a pass or fail for each criterion with its reasoning and the full conversation.
