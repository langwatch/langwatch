...
<CardGroup cols={2}>
  ...
  <Card title="Agent Testing" description="Test agent behavior in multi-turn conversations with a simulated user." icon="masks-theater" href="/agent-testing/overview" />
  ...
  <Card title="AI Gateway" description="Governed, observable, sub-millisecond-overhead OpenAI- and Anthropic-compatible endpoint for every LLM call in your org. Personal IDE keys, budgets, routing policies, anomaly detection." icon="route" href="/ai-gateway/overview" />
  <Card title="AI Governance" description="One control plane above the gateway: per-user workspaces, ingestion sources, anomaly rules, OCSF/SIEM export, compliance architecture. The admin home for rolling LangWatch out company-wide." icon="shield-halved" href="/ai-governance/overview" />
</CardGroup>
