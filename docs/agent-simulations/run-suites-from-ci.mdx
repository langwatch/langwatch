...
```bash
langwatch suite run suite_abc123 --note "$(git log -1 --pretty=%s)" --wait
```

The note is free text, up to 200 characters. `langwatch simulation-run list` and `langwatch simulation-run get` show it back.
