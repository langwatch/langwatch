...
A run goes against every target you name, so naming two targets runs each scenario twice and files both under one batch. That is how you compare a candidate against what is in production.

A target may also carry the parameter values it alone runs with. Name the same agent twice with different values and you compare that agent on two models in the same run.
...
The results page shows one column per target, each with its own pass rate, duration and cost, so you read the two side by side rather than as one fused number.
