...
```bash
langwatch suite run "Checkout regression" --target http:agent_abc123 --wait
```

`--wait` polls until the batch is complete and exits non-zero when a run failed, which is what fails the job. The suite is named by its id or by its name.

## From the REST API

`POST /api/v1/test-suites/{id}/run` schedules one run for each scenario of the suite, against each target you name:
...
The body takes `targets` and, when you want them, `name`, `repeatCount`, `simulatorModel`, `judgeModel`, `parameters`, `note` and `idempotencyKey`. Without `name`, the run goes under the run plan named after the suite and the target, so every run of the same suite against the same agent joins one history. Repeat the same `idempotencyKey` to make a retried job join the first run instead of starting a second one.
...
To run a configuration of your own instead of a whole suite, `POST /api/v1/run-plans/run` takes a `config` object with the scope, the targets, the repeat count and the two models. `GET /api/v1/run-plans` lists the plans, and `POST /api/v1/run-plans/{id}/run` runs one again with the configuration it holds.
...
`langwatch.run_plans.run(...)` starts a plan of your own, with `scope`, `targets`, `repeat_count`, `simulator_model` and `judge_model`.
...
`langwatch suite run ... --wait` does the same poll, and its exit code follows the results.
...
```bash
langwatch suite run suite_abc123 --target http:agent_abc123 \
