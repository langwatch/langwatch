...
| `--tunnel-url <url>` | Use a tunnel you already run, for example a named Cloudflare tunnel or ngrok, and skip provisioning one. |
...
`--no-auth` turns the proxy off, for a server that already authenticates every request. The agent's own configured authentication headers pass through the tunnel unchanged either way.
