...
<Note>
  **Also check:** [Testing agents behind
  authentication](/agent-simulations/authenticated-agents) lists exactly which fields
  resolve secret references, and covers OAuth2 client-credentials exchange with a code
  agent.
</Note>
...
<Note>
  **Also check:** [Linking your traces](/agent-simulations/remote-traces) explains what
  the judge sees, how long it waits for traces, and what happens when they do not arrive.
</Note>
...
<Frame>
  <img
    className="block"
    src="/images/agent-simulations/register-http-agent.png"
...
<Frame>
  <img
    className="block"
    src="/images/agent-simulations/agent-test-request.png"
...
<Frame>
  <img
    ...
    alt="The run detail with the conversation transcript and a View trace link on the turn"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="The run results with the verdict and the judge reasoning citing a tool call span from the trace"
  />
</Frame>
...
<Frame>
  <img
    className="block"
    src="/images/agent-simulations/run-trace.png"
    alt="The trace waterfall with the simulation spans and the agent's own spans in one trace"
  />
</Frame>
