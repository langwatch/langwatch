...
Then create a scenario, file it into a test suite, and run the suite against the agent:
...
`--folder` files the scenario into a test suite that exists, by name or by id. The run goes under the run plan named after the suite and the target, so the next run of the same pair joins the same history.
