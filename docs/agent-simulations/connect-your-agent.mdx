...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    The agent entry point is Node only. It is a separate export, `langwatch/agent`.
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    Declare the parameters in a definition map. `params` in the handler is typed from it:
    ...
    export const supportAgent = connectAgent(
      ...
      async ({ messages, params }) => {
        // params: { model: "gpt-5" | "gpt-5-mini"; plan: string; maxTools: number }
        return await runMyAgent(messages, { model: params.model });
      },
    );
    ```

    `parameters` also takes a JSON Schema object, so a schema library you already use works:
    ...
    connectAgent({ name: "support-agent", parameters: schema["~standard"].jsonSchema }, handler);
    ```

    Any library that implements Standard Schema exposes its JSON Schema at `schema["~standard"].jsonSchema`, which is what the SDK reads. Pass the schema object, not the library's own validator instance, so the SDK does not depend on the library.
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Python">
    ...
    `langwatch.agent.serve()` blocks the current thread while the connection runs. `langwatch.Param(description=..., options=[...], type=..., secret=...)` annotates one parameter. `langwatch.AgentReply(output, session=...)` returns a reply with a session value.
  </Tab>
  <Tab title="TypeScript">
    ...
    | `environment` | resolved, see above | Which environment row this process registers under. |
    | `description` | none | One line about the agent, shown on the agents page. |
    | `parameters` | none | A definition map, or a JSON Schema object. |
    | `enabled` | `true`, `false` under `CI` | Whether this process connects at all. |
...
| Symptom | Cause | Fix |
|---|---|---|
| The row reads **Offline** | The process stopped, or its outbound connection is blocked by a proxy or a firewall. | Restart the process. Allow outbound WSS to your LangWatch endpoint, or use an [HTTP agent](/agent-simulations/other-ways-to-connect) on a network that blocks WebSockets. |
...
| Trace-dependent criteria come back inconclusive | The agent reports its traces to a different project, or reports none. | Point the agent's tracing at the project that runs the scenarios. |
