...
<Steps>
  ...
  <Step title="Run it">
    Run your usual test command (`pytest` or `vitest`). With `LANGWATCH_API_KEY` set, the run streams to the [Simulations dashboard](/agent-simulations/overview) with full audio playback and per-segment transcripts, and writes a `recordings/<scenario>/full.wav` to listen back.
