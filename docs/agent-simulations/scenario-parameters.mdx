...
A secret parameter is a run parameter whose value the platform never stores and never shows in clear. Use one for an API key, a bearer token, or a password that changes per run.
...
```bash
langwatch suite run <suite-id> --param api_token=tok-live-1
```

The run records the name `api_token` and no value. The run detail drawer lists the name with a mask, and the CSV export leaves the parameter out.
