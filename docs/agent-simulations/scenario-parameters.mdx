...
A secret parameter is a run parameter whose value the platform encrypts before it stores anything, and never shows in clear. Use one for an API key, a bearer token, or a password that changes per run.
...
```bash
langwatch suite run <suite-id> --param api_token="$TENANT_API_TOKEN"
```
...
The run records the name `api_token`. The value is encrypted on its way to the target under test, and no plain copy of it is kept: it is not in the stored run, the runs API, the CSV export or the audit log. The run detail drawer lists the name with a mask.
