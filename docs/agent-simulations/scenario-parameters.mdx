...
<img
  src="/images/simulations/scenario-parameters-form.png"
  alt="The scenario editor with the parameters dialog open over it, declaring a fixture parameter with a description and a default value, referenced as params.fixture in the situation and criteria"
  width="100%"
/>
...
<img
  src="/images/simulations/scenario-parameters-body-template.png"
  alt="The HTTP agent body template editor referencing params.fixture, with the available variables hint listing params.NAME"
  width="100%"
/>
...
<Note>
  A parameter reaches a downstream node only if the entry node has an edge carrying it.
  Adding a parameter to the run does not add the edge. Open the workflow and connect the
  new entry field to the node that reads it.
</Note>
...
<img
  src="/images/simulations/scenario-parameters-run-dialog.png"
  alt="The suite run confirmation dialog with a Parameters block, the fixture field prefilled from the scenario's default value"
  width="100%"
/>
...
<Warning>
  A secret is hidden in the record of the run, not in the conversation. When the agent
  under test repeats the value in its own answer, that answer is conversation content and
  the run stores it.
</Warning>
...
<img
  src="/images/simulations/scenario-parameters-run-drawer.png"
  alt="The simulation run detail drawer with the Parameters section showing the resolved value the run used, next to the rendered criteria and the judge's verdict"
  width="100%"
/>
...
<CardGroup cols={2}>
  <Card
    ...
    icon="key"
    href="/agent-simulations/authenticated-agents"
  >
    Reference project secrets from an HTTP target or a code agent
  </Card>
  <Card
    title="Simulations getting started"
    icon="rocket"
