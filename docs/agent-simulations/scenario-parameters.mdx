...
<Warning>
  Do not put credentials in parameters. The run records every value, and every user who can open the run can read it. Put an API key in a project secret and read it as `{{ secrets.NAME }}`. See [Testing agents behind authentication](/agent-simulations/authenticated-agents).
</Warning>
