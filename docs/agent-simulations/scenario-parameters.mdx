...
A [connected agent](/agent-simulations/connect-your-agent) declares its parameters in the code, from the function that runs it. The platform reads them when the agent connects, so the declaration is the code and no second copy needs to be kept in step by hand.
...
An agent declaration carries the same fields a scenario declaration does, plus two the code makes available:

- **`type`**, one of `string`, `number` or `boolean`. It comes from the annotation in Python and from `type` in the TypeScript definition map. The value reaches the run with that type, so a string parameter keeps `007` as text instead of turning it into the number 7.
- **`options`**, a closed list of allowed values, up to 50. It comes from `Literal` or an `Enum` in Python and from `options` in TypeScript.
...
The function receives the values the run supplied, with the type it declared. A parameter the run did not supply arrives as its default, so the function signature is complete on its own and a plain call from a unit test needs no parameters at all.
...
<img src="/images/simulations/scenario-parameters-run-dialog.png" alt="The test suite run confirmation dialog with a Parameters block, the fixture field prefilled from the scenario's default value" width="100%" />
