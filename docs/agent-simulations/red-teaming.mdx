...
Need maximum adaptability against a hardened agent? [**GOAT**](https://langwatch.ai/scenario/advanced/red-teaming/goat) is a drop-in alternative that picks a technique *per turn* instead of following fixed phases (based on Meta's ICML 2025 paper).
...
<Steps>
  ...
  <Step title="Run it">
    Run it alongside your test suite. We recommend **50 turns** for thorough coverage — agents that hold early often break under sustained pressure. With `LANGWATCH_API_KEY` set, every adversarial turn shows up in the [Simulations dashboard](/agent-simulations/overview), and `scenario redteam-report` opens the findings dashboard with severity and prioritized fixes.
  </Step>
</Steps>
