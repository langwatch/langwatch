...
An agent that has not connected for 30 days is archived. A reconnect from the same name, environment and scope brings the same row back with its history.
