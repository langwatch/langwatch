...
Use the project's own secret store as the shared store. The platform reads every
project secret from the database when it prepares each row, so a row can read a
session that an earlier row wrote. The REST secrets API accepts a project API
key, which is how the agent writes it back.

The value is encrypted at rest and no API returns it, so this is the correct
place for a session token. A dataset row is the alternative and it is stored in
clear and is readable in the UI, so it suits a fixture and not a credential.

Add two more project secrets so the agent can write:
...
The agent then reads the session, uses it when it is fresh, and logs in again
when it is not. The full example, executed continuously against the real
code-agent runtime, is
...
REFRESH_MARGIN_SECONDS = 60            # refresh this long before the end


class Code:
    def __call__(self, message: str):
        session = get_session()  # the one line each row calls
        ...
...
def read_cached_session():
    ...
    if time.time() - entry["issued_at"] >= SESSION_TTL_SECONDS - REFRESH_MARGIN_SECONDS:
        return None
    return entry["session"]
```

Set `SESSION_TTL_SECONDS` to what the target system promises, and keep
`REFRESH_MARGIN_SECONDS` larger than your slowest row. A session that is older
than the window minus the margin counts as stale, so the agent refreshes it
before it expires instead of after a call fails.

### What this pattern does not do

The `secrets` namespace a row reads is a snapshot taken when that row starts. A
row cannot read it again later, and it cannot see a write made by a row that runs
beside it. Three results follow:

- The first parallel wave performs one login per row in the wave. The rows after
  the wave reuse the session. No lock prevents this, because a row that waits
  still holds the old snapshot. Keep the login idempotent on the target system,
  or lower the run's parallelism when the target rate-limits the login.
- A row that starts with a fresh session keeps it for the whole row. Give the
  margin enough room for the longest call the row makes.
- A failed write does not fail the row. The row already holds a working session,
  and the only cost is that the next row logs in again. The example reports it
  and continues.

<Note>
  On a version without the secrets REST API, keep the same shape and put the
  session in a dataset row instead. A dataset row is stored in clear and is
  readable by anyone with project access, so use it only for a short-lived
  token, never for a password.
</Note>
...
| The first few rows log in, the rest do not | Expected. Rows that start together share one empty snapshot, so each one logs in. |
