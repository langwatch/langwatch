...
Secret names must be `UPPER_SNAKE_CASE` (`^[A-Z][A-Z0-9_]*$`). Values are encrypted at rest, and the UI never shows one. `GET /api/secrets/by-name/{name}/value` is the only route that returns a value, and it needs an API key that can manage secrets; every other secrets route answers with names and timestamps. See [reusing one login across the rows of a run](#reusing-one-login-across-the-rows-of-a-run).
...
- A failed write does not fail the row. The row already holds a working session,
  ...
  what the store did, in its own fixed words, and nothing from the exception
...
| `ACME_SESSION is not stored yet, or could not be read, this row logs in` | The line does not tell the two causes apart, so check both. Nothing is stored yet, which is expected on the first row and after a failed write, or the read did not get through: `LANGWATCH_API_KEY` must be able to manage secrets and `LANGWATCH_ENDPOINT` must point at your instance. |
