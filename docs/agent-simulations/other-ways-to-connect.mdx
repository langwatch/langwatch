...
[Connecting from code](/agent-simulations/connect-your-agent) is the recommended path: the process that already runs your agent connects itself, and there is no URL, no request template and no credential in the agent configuration. Use one of the paths below when that is not possible.
...
<Note>
  **Also check:** [Testing agents behind authentication](/agent-simulations/authenticated-agents) lists exactly which fields resolve secret references, and covers an OAuth2 client-credentials exchange.
</Note>
...
| `{{ traceId }}`, `{{ traceparent }}` | The turn's trace identifiers, see [Linking your traces](/agent-simulations/remote-traces) |
...
<Frame>
<img className="block" src="/images/agent-simulations/register-http-agent.png" alt="The HTTP agent editor with the endpoint URL, request body template and output path" />
</Frame>
...
<Frame>
<img className="block" src="/images/agent-simulations/agent-test-request.png" alt="The Test tab showing a 200 response from the agent endpoint and the extracted reply" />
</Frame>
...
`sessionPath` is optional; leave it out when the endpoint keeps nothing between turns.
...
It fits a target that is glue and no more than glue: a token exchange in front of an API, or a call into a service that has no repository of its own. It does not run your product's code, its dependencies or its secrets, so it is not the way to test a real agent. Use [connecting from code](/agent-simulations/connect-your-agent) for that.
...
[Testing agents behind authentication](/agent-simulations/authenticated-agents) carries the worked OAuth2 client-credentials example, the secret rules and the runtime's limits.
