...
<Tip>
  **Quick setup?** [Copy the scenarios prompt](/skills/code-prompts#add-scenario-tests)
  into your coding agent to add simulation tests automatically.
</Tip>
...
Spread `input.propagation_headers` (Python) or `input.propagationHeaders` (TypeScript) onto your adapter's outgoing HTTP request, so the remote service joins each turn's trace. [Linking your traces](/agent-simulations/remote-traces) covers the mechanism, the wait behavior, and how to write trace-aware criteria.
