---
title: Remote Traces
sidebarTitle: Remote traces
keywords: langwatch, agent simulations, remote traces, traceparent, trace propagation, judge, tool calls, opentelemetry
...
Adoption is one change on the agent's side, and no change at all when the server runs OpenTelemetry HTTP auto-instrumentation. The snippets are in [Connect your agent](/agent-simulations/connect-your-agent#4-adopt-the-trace-context).
...
An inconclusive behavior criterion is the signal to check propagation and the project key; the [common failures](/agent-simulations/connect-your-agent#common-failures) table covers the causes.
...
<CardGroup cols={2}>
  ...
  <Card title="Local development" icon="laptop-code" href="/agent-simulations/local-development">
