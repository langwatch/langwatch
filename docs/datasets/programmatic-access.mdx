---
title: "Datasets: programmatic access from the SDKs, the CLI and the MCP server"
...
| `datasetId` | ID of the parent dataset (TypeScript only) |
| `projectId` | ID of the project (TypeScript only) |
| `createdAt` | Timestamp of creation (TypeScript only) |
| `updatedAt` | Timestamp of last update (TypeScript only) |

In Python, `get_dataset` returns entries with `id` and `entry` only. The records returned by `list_records` and `create_records` carry all six fields in both SDKs.
