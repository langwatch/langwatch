...
<Note>
  If you are using a **service API key** (e.g. for CI/CD or multi-project setups), you
  must also set the `LANGWATCH_PROJECT_ID` environment variable (or pass
  `project_id`/`projectId` to the SDK) so the SDK knows which project to access. You can
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ```typescript await langwatch.datasets.delete("my-dataset"); ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Python">
    ```python deleted_count = langwatch.dataset.delete_records( "my-dataset",
    record_ids=["rec-1", "rec-2"], ) print(f"Deleted {deleted_count} records") ```
  </Tab>
  <Tab title="TypeScript">
    ```typescript const result = await langwatch.datasets.deleteRecords("my-dataset",
    ["rec-1", "rec-2"]); console.log(`Deleted ${result.deletedCount} records`); ```
  </Tab>
</Tabs>
