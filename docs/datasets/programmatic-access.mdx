...
<Note>
  ...
  the `LANGWATCH_PROJECT_ID` environment variable (or pass `project_id`/`projectId` to the SDK) so
  the SDK knows which project to access. You can find the project ID in your project settings.
</Note>
