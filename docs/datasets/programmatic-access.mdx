...
description: Manage datasets from LangWatch using the SDK for offline evaluations and automated workflows.
---
...
Upload a CSV, JSON, or JSONL file to an existing dataset.
...
<Tabs>
  <Tab title="Python">
```python
result = langwatch.dataset.create_dataset_from_file(
    "From CSV",
    file_path="feedback.csv",
)
print(f"Dataset: {result.dataset.name} ({result.dataset.slug})")
