...
<Note>
  ...
  the `LANGWATCH_PROJECT_ID` environment variable so the SDK knows which project to access. You can
  find the project ID in your project settings.
</Note>
...
<Tabs>
  <Tab title="Python">```python langwatch.dataset.delete_dataset("my-dataset") ```</Tab>
  <Tab title="TypeScript">```typescript await langwatch.datasets.delete("my-dataset"); ```</Tab>
</Tabs>
...
<Tabs>
  <Tab title="Python">
    ```python deleted_count = langwatch.dataset.delete_records( "my-dataset", record_ids=["rec-1",
    "rec-2"], ) print(f"Deleted {deleted_count} records") ```
  </Tab>
  <Tab title="TypeScript">
    ```typescript const result = await langwatch.datasets.deleteRecords("my-dataset", ["rec-1",
    "rec-2"]); console.log(`Deleted ${result.deletedCount} records`); ```
  </Tab>
</Tabs>
...
for index, row in evaluation.loop(df.iterrows()): # Run your LLM/agent
