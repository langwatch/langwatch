To capture a RAG, start a RAG span inside the trace, giving it the input query being used:
...
<Note>
  On LangChain.js, RAG spans are captured automatically by the LangWatch callback when
  using LangChain Retrievers, with `source` as the documentId.
</Note>
