<Note>
  On short-live environments like Lambdas or Serverless Functions, be sure to call <br /> `await
  trace.sendSpans();` to wait for all pending requests to be sent before the runtime is destroyed.
</Note>
...
To capture the case where your code throws an exception, wrap the code in a `try/catch` block. In the `catch`, update or end the span with the exception:
