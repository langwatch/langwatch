<Note>
  On short-live environments like Lambdas or Serverless Functions, be sure to call <br />{" "}
  `await trace.sendSpans();` to wait for all pending requests to be sent before the
