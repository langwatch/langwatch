...
Codex models (your own ChatGPT subscription, model prefix `openai_codex/`) speak the Responses API, and the codex backend accepts a fixed set of fields: anything outside it fails the whole request upstream with `400 Unsupported parameter`. The gateway builds the outgoing body itself, so every field follows the same policy as above. The gateway also pins three invariants the backend requires: the bare model name, `stream` on, and `store` off. Dropped fields are signaled on the `X-LangWatch-Params-Dropped` header and the gateway span; the response body is the backend's own SSE stream, so `extra_fields.params_dropped` is not injected into it. `drop_tuning_params` works on this lane too.
