...
On the OpenAI-compatible lane the gateway forwards your `POST /v1/chat/completions` body as written. Two fields are exceptions: it removes the gateway's own `drop_tuning_params`, and the organization's cache rules can strip or add a top-level `cache_control`, see [Cache control](/ai-gateway/cache-control). Whatever the provider accepts, you get.
