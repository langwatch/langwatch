...
description: Gateway-specific settings (fallback priority, provider config overrides) now live on each ModelProvider's Advanced tab.
...
4. Fill in the gateway-only fields (fallback priority, providerConfig JSON overrides). The provider credential itself (API key, region, endpoint) stays on the **Basic** tab and is reused by both the gateway and the rest of LangWatch.
