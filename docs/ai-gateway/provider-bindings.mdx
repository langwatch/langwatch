...
description: Gateway-specific settings (Fallback priority, Provider config (JSON)) now live on each ModelProvider's Advanced (Gateway) section.
...
| `rate_limit.rpm`, `.tpm`, `.rpd` | The same inputs are on the **ModelProvider → Advanced (Gateway)** tab. The gateway does not read them. See [Rate limits](/ai-gateway/rate-limits) for the limits it does enforce |
| `fallback_priority_global` | Same field, on the **Advanced (Gateway)** tab. Default-policy ordering uses this then `createdAt` |
...
4. Fill in the gateway-only fields, **Fallback priority** and **Provider config (JSON)**. The provider credential itself (API key, region, endpoint) stays on the **Basic** tab and is reused by both the gateway and the rest of LangWatch.
...
The **Advanced (Gateway)** tab also surfaces health status, circuit-breaker state, and the last `disabledAt` timestamp. These are populated by the gateway; the operator can't write them directly.
...
- [RBAC](/ai-gateway/rbac): `modelProviders:manage` gates the **Advanced (Gateway)** tab; the old `gatewayProviders:*` resource is retired.
