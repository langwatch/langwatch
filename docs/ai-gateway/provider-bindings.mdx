...
The section has an input for each of the three, and the value you save is stored on the row. The gateway does not enforce any of them: the config bundle it reads carries no rate limit on a provider, and its limiter counts requests per virtual key. Set a ceiling on the key instead. See [Rate limits](/ai-gateway/rate-limits).
