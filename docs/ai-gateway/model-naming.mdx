...
Concretely: on every request the resolver inspects the raw `model` string once. If it contains a `/`, the prefix is peeled off and checked against the VK's bound providers; the bare segment then identifies the upstream model. If there's no `/`, the bare string is looked up in the VK's alias map (if present) and then against the VK's bound-provider models. The outgoing request body's `model` field is rewritten to the bare form before dispatch so Bifrost's raw-body passthrough never ships `openai/gpt-5-mini` to OpenAI (that was the bug in rchaves's iter 107 `#63` dogfood — the upstream rejects a prefixed name with `invalid model ID`).
