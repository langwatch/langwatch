...
The gateway resolves the `model` field in **a single map lookup on the hot path**. There is no try-the-bare-form-then-fall-back-to-the-prefixed-form behaviour — that would cost multiple hash lookups and an error branch per request, and the hot path has a sub-millisecond budget.

Instead, when a VK's config is loaded (via the long-poll `/changes` feed from the control plane), the gateway pre-builds a `map[string] → providerRoute` per VK that contains **both the bare and the prefixed key** for every routable model:

| Map key | Resolves to |
|---|---|
| `gpt-5-mini` | openai route + `gpt-5-mini` |
| `openai/gpt-5-mini` | openai route + `gpt-5-mini` |
| `claude-haiku-4-5-20251001` | anthropic route + `claude-haiku-4-5-20251001` |
| `anthropic/claude-haiku-4-5-20251001` | anthropic route + `claude-haiku-4-5-20251001` |
| `coding-small` | *(alias → openai route + `gpt-5-mini`)* |

At request time, the raw `model` string is looked up once. If there's a hit, the route is used. If there's a miss, the request returns `400 bad_request` with an enriched error (below).
