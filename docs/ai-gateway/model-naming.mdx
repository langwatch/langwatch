...
<CardGroup cols={2}>
  ...
  <Card title="Routing handle" icon="key">
    `"model": "europe/claude-sonnet-5"`. Goes to that exact provider, when the key can
    reach it.
  </Card>
  <Card title="Alias" icon="arrow-right-arrow-left">
    `"model": "coding-small"`. A per-key redirect. See [Model
    Aliases](/ai-gateway/model-aliases).
  </Card>
</CardGroup>
...
<Steps>
  <Step title="One provider on the key">Use the model name. Nothing else is needed.</Step>
  ...
  <Step title="A stable name you want to repoint later">
    Use an alias. Edit the key once and no caller changes. See [Model
    Aliases](/ai-gateway/model-aliases).
  </Step>
  <Step title="Failover between providers">
    Name a model that more than one bound provider serves. The gateway carries the same
    model name to the next provider in the chain. See [Fallback
