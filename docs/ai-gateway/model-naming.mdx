...
<Steps>
  ...
  <Step title="Cross-provider fallback (primary dies, secondary takes over)">
    Name a model the fallback providers also serve, for example an Anthropic model with both an Anthropic and a Bedrock credential on the VK. One request resolves to one model, and the gateway carries that same model name to the next credential in the chain. See [Fallback Chains](/ai-gateway/providers/fallback-chains).
  </Step>
</Steps>
