...
See [Model Aliases](/ai-gateway/model-aliases) for the full rules, including how an alias is judged against `models_allowed` and the `GET /v1/models` discovery endpoint.
...
<Steps>
  ...
  <Step title="Cross-provider fallback (primary dies, secondary takes over)">
    Name a model that the fallback providers also serve, for example an Anthropic model with an Anthropic credential and a Bedrock credential on the VK. One request resolves to one model. The gateway carries that same model name to the next credential in the chain. See [Fallback Chains](/ai-gateway/providers/fallback-chains).
  </Step>
</Steps>

## Why not just pick one convention?

Gateways in the ecosystem (litellm, OpenRouter, Portkey) use `provider/model`. Provider SDK codebases use the bare name. Requiring either one would create friction for the other population.

Accepting both, with a single O(1) resolution step, not a retry loop, lets you write whichever is natural for your code.
