...
<Info>Aliases are one of three ways to reference a model. See [Model Naming](/ai-gateway/model-naming) for when to use bare (`gpt-5-mini`) vs prefixed (`openai/gpt-5-mini`) vs an alias.</Info>
...
2. Otherwise the gateway looks up `<name>` in `model_aliases`. If there is an entry, the gateway checks the mapped `provider/model` against `models_allowed`. If the check passes, the gateway dispatches it.
...
Aliases do not go around `models_allowed`. The allowlist judges the model that the alias resolves to, never the name that the caller typed. It accepts the two spellings of that model: an entry of `gpt-5-mini` and an entry of `openai/gpt-5-mini` match the same resolved model. Write the form that your team reads.

If `models_allowed: ["gpt-5-mini", "claude-haiku-4-5-20251001"]` and an alias maps to `openai/o3`, the gateway refuses a request that uses the alias with `model_not_allowed`. The refusal names `o3`, the resolved model, and it names the alias. The operator can see which side to correct.

This matters for security: do not grant a model through an alias that you would not grant directly.

[Policy rules](/ai-gateway/policy-rules) judge the resolved model in the same way. An alias that points to a denied model is blocked.
...
See [API: `GET /v1/models`](/ai-gateway/api/models) for the full response shape.
...
- When the alias becomes another layer of indirection the team has to look up. Keep alias names close to the provider-native names if the redirect is simple.
