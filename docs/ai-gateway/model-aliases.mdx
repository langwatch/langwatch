...
2. Otherwise, `<name>` is looked up in `model_aliases`. If found, the mapped `provider/model` is checked against `models_allowed` and, when it passes, dispatched.
...
Aliases do not bypass `models_allowed`. The allowlist judges the model the alias resolves to, never the name the caller typed, and it accepts either spelling of that model: an allowlist entry of `gpt-5-mini` and one of `openai/gpt-5-mini` both match the same resolved model, so you write whichever form your team reads more naturally.

If `models_allowed: ["gpt-5-mini", "claude-haiku-4-5-20251001"]` and an alias maps to `openai/o3`, a request using the alias is refused with `model_not_allowed`. The rejection names `o3`, the model the alias resolved to, alongside the alias the key gave it, so the operator can see which side to fix.

This matters for security: don't grant a model via aliases that you wouldn't grant explicitly.

[Policy rules](/ai-gateway/policy-rules) judge the resolved model the same way, so an alias pointing at a denied model is blocked.
