...
- **Region availability.** Anthropic-on-Vertex is not in every GCP region, and the regions differ per model version; `us-central1`, `us-east5`, `europe-west1`, and `asia-southeast1` are the common ones. Check the model card for the version you pin. A 404 with "publisher not found" means wrong region.
