...
OpenAI needs only an API key to bind to the gateway. Any valid OpenAI API key in your LangWatch Model Providers table can be referenced by a virtual key and consumed via `/v1/chat/completions`, `/v1/embeddings`, `/v1/responses`, `/v1/images/generations`, `/v1/images/edits`, `/v1/audio/transcriptions`, and `/v1/audio/speech`.
...
| `POST /v1/images/generations` | `POST /v1/images/generations` | `gpt-image-2`, `gpt-image-1` and `gpt-image-1-mini`. See [Images](/ai-gateway/api/images). |
| `POST /v1/images/edits` | `POST /v1/images/edits` | Multipart edit on the same models. |
...
OpenAI automatically caches prompt prefixes ≥ 1024 tokens. Support varies by model within the GPT-4, GPT-5, and o-series families. There's no `cache_control` block to preserve, OpenAI handles it upstream. The gateway forwards requests untouched in `mode=respect`, so cache hits are observed naturally.
...
- **Rate-limit responses (429)**: a 429 moves to the next credential in the VK's [fallback chain](/ai-gateway/providers/fallback-chains). When the chain is used up, or the VK has no chain, the gateway sends OpenAI's `Retry-After` header to the client as is.
