...
- **Rate-limit responses (429)**: a 429 always walks the VK's [fallback chain](/ai-gateway/providers/fallback-chains) to the next credential. When the chain is exhausted or the VK has none, OpenAI's `Retry-After` header is surfaced to the client verbatim.
