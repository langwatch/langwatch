...
Google AI Studio serves Gemini through the "plain" Google API, distinct from Vertex. It uses API-key auth and needs no GCP project setup.
...
Gemini has an implicit context cache for prompts > 32k tokens. Support varies by model. The gateway forwards requests untouched; caching behaviour is entirely upstream-managed.
...
- **Rate limits.** AI Studio's free tier has aggressive rate limits; the paid tier requires a Google account with billing attached. A 429 triggers fallback if configured.
