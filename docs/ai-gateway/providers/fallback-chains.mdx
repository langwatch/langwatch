...
<Info>
  Per-attempt nested spans (`langwatch.fallback.attempt`, `.reason` attrs) are a v1.1
  observability follow-up. In v1, the counter + header give you the aggregate picture;
  per-attempt reasoning lives in the gateway log line for that request-id.
</Info>
