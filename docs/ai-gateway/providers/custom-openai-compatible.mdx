...
- **Groq, Together, Fireworks**: chat + embeddings (depends on the model).
...
Gateway default `cache.mode = respect` forwards any cache-control headers or body fields the upstream understands. OpenAI-compatible servers rarely expose a prompt-caching API, so `X-LangWatch-Cache` reads as `miss` unless the upstream implements one.
