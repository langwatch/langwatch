...
<Info>
  The following attributes are **not yet emitted** (tracked for v1.1): `langwatch.policy.blocked`,
  `langwatch.budget.breached_scope`, `.warnings`, `langwatch.stream.*`, `langwatch.client.name`,
  `langwatch.cache.outcome`, `.forced_injected`. Operators looking for these signals should use the
  Prometheus counters documented below. Request-id correlation via the
  `X-LangWatch-Gateway-Request-Id` header lets operators join metric spikes back to individual
  traces.
</Info>
...
| `gateway_realtime_mints_total`                | counter | `vendor`, `outcome`       | Realtime voice session mints. `outcome` is `minted`, `session_limit`, `registry_unavailable`, or `provider_error`. One mint is one billable voice call, so this is the call rate                                                                                                                                                                                                                                                                  |
| `gateway_realtime_session_limit_blocks_total` | counter | `virtual_key_id`          | Mints refused because the key already held its maximum open voice sessions. A rising count on one key means its cap is too low for its traffic, or that its sessions are not being closed                                                                                                                                                                                                                                                         |
| `gateway_realtime_registry_errors_total`      | counter | `operation`               | Failed calls to the control plane's voice-session record, by `reserve`, `correlate`, `release` or `usage`. A `reserve` failure refuses the mint outright; a `correlate` failure costs the session its exact join key to the vendor's post-call report, so it falls back to a time-window match                                                                                                                                                    |
| `gateway_client_rejects_total`                | counter | `code`, `vk_id`           | Requests **the gateway itself** rejected as the caller's fault after authentication, by gateway error code (`missing_model`, `bad_request`, `model_not_allowed`, `budget_exceeded`, `payload_too_large`, …) and by the virtual key that sent them. These are logged at `info`, so this counter is the only signal that will move when one client loops on a malformed body. Alert on a sustained per-key rate rather than on any single rejection |
...
- **Provider rejections.** An upstream 4xx (an OpenAI 429, an Anthropic 402) is the caller's fault too, but it is not the gateway rejecting the client, and counting it would turn a bad hour at a provider into an alert that reads as client misbehaviour. Those live on `gateway_provider_attempts_total{outcome="rate_limit"}` and the upstream status on `gateway_http_requests_total`.
...
```promql
sum by (code) (rate(gateway_client_rejects_total{vk_id="<full-vk-id>"}[15m]))
```
