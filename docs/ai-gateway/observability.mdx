...
Source of truth: `services/gateway/internal/otel/attrs.go`. The gateway emits these attributes on every span (via the `*chi.Context` helper and the dispatch handlers):
...
| `gen_ai.usage.cache_read.input_tokens` / `gen_ai.usage.cache_creation.input_tokens` | Cache-read + cache-creation counters (OTel GenAI semconv — Lane A iter 42 dropped proprietary `langwatch.usage.cache_*_tokens` in favour of these). |
...
- `langwatch.guardrail.verdict` — declared as the canonical aggregate verdict; **not yet wired at the dispatcher hot path in v1** (see [Guardrails → Observability](/ai-gateway/guardrails#observability) for the caveat).

<Info>The following attributes were originally drafted but are **not emitted in v1** (tracked for v1.1): `langwatch.fallback.attempt` / `.reason`, `langwatch.policy.blocked`, `langwatch.budget.breached_scope` / `.warnings`, `langwatch.stream.*`, `langwatch.client.name`, `langwatch.cache.outcome` / `.forced_injected`. Operators looking for per-feature signals in v1 should use the Prometheus counters documented below — those are the authoritative v1 observability surface. Request-id correlation via the `X-LangWatch-Request-Id` header lets operators join metric spikes back to individual traces.</Info>
