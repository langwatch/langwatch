...
| `gateway_client_rejects_total` | counter | `code`, `vk_id` | Requests rejected as the caller's fault after authentication, by gateway error code (`bad_request`, `model_not_allowed`, `budget_exceeded`, …) and by the virtual key that sent them. These are logged at `info`, so this counter is the only signal that will move when one client loops on a malformed body. Alert on a sustained per-key rate rather than on any single rejection. **Authentication failures are not counted here** — a missing or unresolvable key is rejected before the request has a key to attribute it to, so watch `gateway_http_requests_total{status="401"}` for those |
