...
3. The OTel exporter reads `langwatch.project_id` on each span and routes the OTLP export to that project's ingestion endpoint with the project's collector token (cached from the control-plane).
4. LangWatch's existing ingest pipeline stores the trace under the project.
