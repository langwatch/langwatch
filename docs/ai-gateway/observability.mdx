...
4. The gateway ships all traces to a single endpoint (`GATEWAY_OTEL_DEFAULT_ENDPOINT`, default `https://app.langwatch.ai/otel/v1/traces`).
...
On self-hosted deployments, set `GATEWAY_OTEL_DEFAULT_ENDPOINT` to your in-cluster OTel collector (or LangWatch ingestion endpoint for hybrid setups). Attribution works the same way: per-project span attributes are filed at ingest.
...
| `gen_ai.usage.input_tokens` / `gen_ai.usage.output_tokens` | Regular input/output tokens (OTel GenAI semconv). |
| `gen_ai.usage.cache_read.input_tokens` / `gen_ai.usage.cache_creation.input_tokens` | Provider-native cache counters (OTel GenAI semconv — iter 42 dropped proprietary `langwatch.usage.*` names in favour of these). |
| `langwatch.reasoning_tokens` | Reasoning tokens (o-series, Claude extended thinking). |
| `langwatch.cache.outcome` | `hit` / `miss` / `bypass` / `force`. |
