...
Two things this counter deliberately excludes, because both would make a per-key alert on it fire for keys doing nothing wrong:
...
```promql
topk(5, sum by (vk_id, code) (rate(gateway_client_rejects_total{code!=""}[15m])))
sum by (code) (rate(gateway_client_rejects_total{vk_id="vk_Hkj6EJ..."}[15m]))
```
