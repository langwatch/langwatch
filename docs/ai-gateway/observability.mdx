...
<Info>
  ...
  `langwatch.policy.blocked`, `langwatch.budget.breached_scope`, `.warnings`,
  `langwatch.stream.*`, `langwatch.client.name`, `langwatch.cache.outcome`,
  `.forced_injected`. Operators looking for these signals should use the Prometheus
  counters documented below. Request-id correlation via the
  `X-LangWatch-Gateway-Request-Id` header lets operators join metric spikes back to
  individual traces.
</Info>
