...
<Note>
  The billing surface (webhook delivery and the pull APIs) is an **Enterprise**
  capability, gated by the same plan flag as webhook endpoints. Reads require
  `gatewaySpend:view`; replay requires `gatewaySpend:manage`. Both are organization-tier
  permissions carried by an organization API key.
</Note>
