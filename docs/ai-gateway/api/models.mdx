...
<Note>
  The list is deliberately permissive, it covers everything the VK could plausibly route, not a
  strict enumeration. `models_allowed` globs and provider shortcuts may not expand to real upstream
  models until you send a real request. The gateway's per-request model resolution remains the
  source of truth for "will this actually route?"
</Note>
