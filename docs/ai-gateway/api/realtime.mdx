...
<Note>
  The gateway does not carry the voice socket, so it cannot end a call that is already
  running. A voice budget admits at session start and reconciles when the call ends.
  Overshoot for voice is bounded by the length of a call, not by the size of one request.
</Note>
