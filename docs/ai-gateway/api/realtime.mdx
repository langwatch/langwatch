...
<Note>
The reconciler exists because of how ElevenLabs treats webhook delivery.
ElevenLabs does not retry a failed delivery, by design for HIPAA, and it
disables a webhook after 10 consecutive failures. One rejected delivery would
therefore lose that call's billing data for good, and ten in a row would stop
delivery for every session on that webhook. The reconciler asks the vendor for
the same numbers on its own schedule, so a lost delivery costs a couple of
