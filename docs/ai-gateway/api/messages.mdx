...
Beyond plain pass-through, the gateway preserves tool-call streaming deltas byte-for-byte, which agentic CLIs depend on, and forwards `cache_control` blocks identically, which keeps the 90% Anthropic cache discount.

## Request

```
POST /v1/messages
x-api-key: vk-lw-<ULID>     # or Authorization: Bearer vk-lw-...
...
```json
{
  ...
  "messages": [{ "role": "user", "content": "What's the first paragraph?" }]
}
```
...
Subject to VK `policy_rules.tools`, a blocked tool name returns `403 tool_not_allowed` before the request leaves the gateway. See [Policy Rules](/ai-gateway/policy-rules).
