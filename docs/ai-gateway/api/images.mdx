...
Models: `openai/gpt-image-2`, `openai/gpt-image-1`, `openai/gpt-image-1-mini`, `openai/dall-e-3`. Configure the OpenAI key once in **Settings → Model Providers**; every virtual key routed to that provider can then call its image models.
...
Body matches OpenAI's [image generation schema](https://platform.openai.com/docs/api-reference/images/create).
...
`response_format` is forwarded only when you send it. The gpt-image family rejects the field and dall-e-3 needs it, so the gateway never sets a value of its own.
...
Form matches OpenAI's [image edit schema](https://platform.openai.com/docs/api-reference/images/createEdit): one or more `image[]` parts, a `prompt` part, and `model`. A single file may also be sent under `image`. The optional parts are `mask`, `n`, `size`, `quality`, `background`, `input_fidelity`, `output_format`, `output_compression`, `response_format` and `user`; anything else is dropped rather than sent to the provider.
