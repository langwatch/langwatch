...
- `trace_project_id` is decided once, when the key is written, and stored on it. Changing what the key is scoped to therefore never moves where its traces and spend land; send `trace_project_id` on the update to move it, validated the same way create validates it. A key written before this was stored can carry `null`, which means its traces are dropped and no budget counts its spend: give it a `trace_project_id`.
