  ...
  "data": [
    {
      ...
      "trace_project_source": "project_scope",
...
- An org- or team-scoped key also needs a place for its traces and spend to land, and has to say where: pass `trace_project_id` (requires `virtualKeys:manage` on that project; it is a destination, not a scope, and grants no access to the key). Without it, and without exactly one project scope to take it from, creation refuses with `gateway_trace_project_ambiguous`. The same refusal covers a key scoped to several projects at once. An organization whose only project is the hidden governance one is exempt, because there is nothing else to name; one with no governance project either refuses with `trace_project_required`.
- `trace_project_source` on every key says which rule put its traces where they went: `explicit`, `project_scope`, or `governance_fallback`. Keys created before this rule can carry `governance_fallback`, and their spend is attributed to the governance project rather than to any project you would think to budget. Give them a `trace_project_id` to move them.
