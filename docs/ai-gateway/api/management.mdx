...
Returns every non-archived budget in your organization across all seven scope types (`organization`, `team`, `project`, `virtual_key`, `principal`, `group`, `attributed_user`), with live `spent_usd` from the spend ledger. `scope_type` is an optional comma-separated filter.
