...
```
GET /end-users/:end_user_id/spend
GET /end-users/:end_user_id/spend?anchor_virtual_key_id=vk_...
```

Current spend and the applicable per-end-user cap for one external end-user id, resolved against the organization's attributed-user templates (`gatewayBudgets:view`). The pair a rebilling platform polls at period close. Response rows: `{budget_id, anchor_id, window, on_breach, limit_usd, spent_usd, period_started_at}`; empty when no template applies. See [Billing & spend events](/ai-gateway/billing-events#per-end-user-spend).
...
`error.code` carries the machine code your integration can branch on (`validation_error`, `trace_project_required`, `routing_policy_required`, `scope_org_mismatch`, `group_budget_requires_clickhouse`, `spend_source_unavailable`, ...). See [API: Errors](/ai-gateway/api/errors) for the full type enum.
