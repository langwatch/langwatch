...
Every write emits a row in `GatewayAuditLog` — visible under **/settings/audit-log** (filter by resource type `virtualKey` / `gatewayBudget` / `gatewayProvider`). Writes via API tokens without an associated user (service tokens) are attributed to `actor = "svc_<projectId>"`. See [Audit log](/ai-gateway/audit) for the REST export path + the v1.1 unification roadmap.
