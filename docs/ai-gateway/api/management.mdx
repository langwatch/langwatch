...
The four creates (`/virtual-keys`, `/budgets`, `/cache-rules`, and `/api/webhooks/endpoints.create`) accept an `Idempotency-Key` header, so a client that never learned the outcome can retry without creating a second resource:
...
Two sibling HTTP families complete the platform and are documented on their own pages, because they authenticate with an **organization API key** and organization-exclusive permissions rather than the credential used above:

- `/api/webhooks/*`: endpoint management, secret roll, test fire, delivery log, health, and the emitted-events log. RPC-named — every operation is a POST to a dotted name such as `endpoints.rollSecret`, with its arguments in the body. See [Webhooks](/features/webhooks).
