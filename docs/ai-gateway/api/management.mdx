...
- `scopes` defaults to the calling project. Team- and org-scoped keys need a scoped API key holding `virtualKeys:manage` at each requested scope; legacy project keys can only mint keys for their own project. An org- or team-scoped key also requires the organization's governance project so its traces and spend land somewhere a budget can see (`trace_project_required` otherwise).
- `budget` creates a VIRTUAL_KEY-scoped budget atomically with the key: the key can never exist without the cap you asked for. Windows: `DAY`, `WEEK`, `MONTH`.
...
`error.code` carries the machine code your integration can branch on (`validation_error`, `trace_project_required`, `routing_policy_required`, `scope_org_mismatch`, `group_budget_requires_clickhouse`, `provider_not_in_organization`, `spend_source_unavailable`, ...). See [API: Errors](/ai-gateway/api/errors) for the full type enum.
