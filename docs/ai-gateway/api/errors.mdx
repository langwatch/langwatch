...
```json
{
  "error": {
    ...
    "tips": [
      "Contact your LangWatch admin to raise the project spending limit",
...
<Note>
  A non-streaming call that runs long writes a keep-alive byte, which commits status `200` before
  the outcome is known. The response then carries `X-LangWatch-Heartbeat-Active: true`, and the body
  still holds an `error` key when the call failed. Read the body rather than the status on those
  responses.
</Note>
