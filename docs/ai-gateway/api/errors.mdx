...
<Note>
  ...
  administration. Creating or editing a budget over the control plane is never refused for
  being over cap.
</Note>
...
<Note>
  Clients must ignore unknown fields. Fields are added to this envelope over time and none
  of the additions above changes an existing one.
</Note>
...
<Note>
  ...
  `guardrail_blocked`), clients keying off `type` will already have a usable
  classification. The `code` is the granular discriminator if you need it (for example, a
  retry policy that distinguishes "upstream flaked, retry with a different VK" from
  "guardrail policy said no, don't retry").
</Note>
