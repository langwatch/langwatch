...
| `meta` | no | Structured detail the sentence deliberately leaves out, so a client never has to scrape prose. Keys are `lower_snake_case` (for example `budget_id`, `budget_scope`, `scope_kind`). Omitted entirely when there is nothing to add. |
...
Validation failures put the offending fields under `meta`: `meta.target` names the part of the request that failed (`json`, `query`, `param`), `meta.fields` lists the offending paths, and `meta.reasons` carries one `{ code, message, meta }` entry per violation with `meta.field`, `meta.type` and, where the schema knows them, `meta.expected` and `meta.received`.
