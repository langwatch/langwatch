...
<Note>
  `402 budget_exceeded` is a data-plane refusal because a budget stops **traffic**, not
  administration. Creating or editing a budget over the control plane is never refused for being
  over cap.
</Note>
...
<Note>
  Clients must ignore unknown fields. Fields are added to this envelope over time and none of the
  additions above changes an existing one.
</Note>
...
| `virtual_key_expired`            | `403` | The VK carries an expiration date and that date has passed. The key material is intact and its status is still `active`, so extending `expires_at` puts it back in service without minting a new secret.                                                                                                                                                                                                                                                                                                                                                                                    |
...
These are _soft_ signals on **successful** responses:
...
<Note>
  The `type` on a streaming terminal frame always reflects the category (`provider_error`,
  `guardrail_blocked`), clients keying off `type` will already have a usable classification. The
  `code` is the granular discriminator if you need it (for example, a retry policy that
  distinguishes "upstream flaked, retry with a different VK" from "guardrail policy said no, don't
  retry").
</Note>
