...
Upstream provider rate limits are opaque to the VK owner and always trigger fallback: an upstream 429 walks the VK's chain to the next credential. A `Retry-After` from upstream is surfaced to the client when no fallback remains.
