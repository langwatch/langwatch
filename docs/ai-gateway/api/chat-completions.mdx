...
A response header `X-LangWatch-Gateway-Request-Id` is emitted on the first frame. Mid-stream failures close the connection with a terminal `event: error` frame (see [Errors](/ai-gateway/api/errors#streaming-errors)).
...
Errors from an authenticated request carry `X-LangWatch-Gateway-Request-Id`. Use it for support tickets and to jump straight to the trace in the LangWatch UI.
...
print("Request id:", resp.response.headers["X-LangWatch-Gateway-Request-Id"])
```
...
Upstream provider rate limits are opaque to the VK owner. An upstream 429 is fallback-eligible: the gateway moves to the next credential in the VK's chain when there is one and the attempt budget allows it. A `Retry-After` from upstream is surfaced to the client when no fallback remains.
