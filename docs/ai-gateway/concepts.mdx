...
- `group` (each member of a group, individually)
- `attributed_user` (each end user seen on a key or project, individually)

Windows: `minute`, `hour`, `day`, `week`, `month`, `total`, `manual`. The cycling windows align to the calendar unless you anchor them to your own start instant. See [Budgets](/ai-gateway/budgets#windows).
...
Spend is debited after the response completes, using the token counts the provider reported, from the spend commands the gateway emits for every request. Redeliveries of the same `gateway_request_id` collapse, so a retry never double-bills.
...
- **RBAC**: the principal must have permission to _use_ the VK at all.
...
Ordered list of **ModelProvider** ids (not models) the VK can dispatch through. Carried on the routing policy bound to the VK (or, when the VK pins no policy, on the org's default policy). The gateway decides which failures move to the next provider. It reads the real upstream result, and the rule is the same for every key. Triggered by:

- `5xx` from upstream.
- `timeout` from upstream.
- `429 rate_limit_exceeded` from upstream.
- `network_error` (connection reset, DNS, TLS).
- Upstream `404`. In a chain with more than one provider, this usually means that this provider does not serve that model.
- `circuit_breaker`, gateway-internal tripped after N consecutive failures.

**Does not** trigger fallback:

- Upstream `400 / 401 / 403`. These stop the request. A `400` points to the request itself. A `401` or `403` usually points to the provider credential. A different provider would hide the fault that you must repair.
- A caller that disconnected, or a caller deadline that ended. This tells nothing about the provider.
...
By default the gateway forwards Anthropic `cache_control` blocks, OpenAI cached-prompt markers, and Gemini implicit caches **byte-identically**. Prompt caching saves up to 90% on input tokens, and a gateway that reformats the payload loses that saving.
...
Each block emits an `AuditLog` row (gateway shape) + the `X-LangWatch-Gateway-Request-Id` response header → trace correlation.

## Traces (per-tenant observability)

Every request emits a LangWatch trace. The gateway is a shared multi-tenant service, and it routes each trace to the **owning tenant's project**: tenant A never sees tenant B's traffic.
...
<Info>
  Previously-documented per-feature attrs (`langwatch.fallback.attempt`, `.reason`,
  `langwatch.policy.blocked`, `langwatch.budget.breached_scope`, `langwatch.guardrail.post_flag`,
  `.verdict`) are **not emitted in v1**. For per-feature signals today, use the Prometheus counters
  + correlate via `X-LangWatch-Gateway-Request-Id`. See [Observability → Per-feature
  attributes](/ai-gateway/observability#per-feature-attributes-when-applicable) for the v1 ↔ v1.1
  breakdown.
</Info>

## Request id

Once a request is authenticated, its response carries `X-LangWatch-Gateway-Request-Id: req_<30 hex characters>`. Use it to:
...
Once these concepts are in your head the rest of the docs are detail views on the same primitives: [Virtual Keys](/ai-gateway/virtual-keys), [Budgets](/ai-gateway/budgets), [Providers](/ai-gateway/providers/overview), and [CLI Integrations](/ai-gateway/cli/overview).

If you rebill this traffic to your own customers, two more read next: [Billing and spend events](/ai-gateway/billing-events) for the per-request record you reconcile against, and [Webhooks](/features/webhooks) for having it pushed to you instead of polling.
