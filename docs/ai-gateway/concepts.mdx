...
Spend is debited after the response completes, using the token counts the provider reported, via the budget-debit projection, the ledger records at most one debit per `gateway_request_id` so gateway retries never double-bill.
