...
Ordered list of **ModelProvider** ids (not models) the VK may dispatch through. Carried on the routing policy bound to the VK (or, when the VK pins no policy, on the org's default policy). Which failures walk the chain is decided by the gateway from the real upstream outcome, the same way for every key. Triggered by:

- `5xx` from upstream.
- `timeout` from upstream.
- `429 rate_limit_exceeded` from upstream.
- `network_error` (connection reset, DNS, TLS).
- Upstream `404`, which in a multi-provider chain usually means this provider does not serve that model.
- `circuit_breaker`, gateway-internal tripped after N consecutive failures.

**Does not** trigger fallback:

- Upstream `400 / 401 / 403`. These are terminal: a `400` is the request, and a `401` or `403` is usually the provider credential. Switching credentials would hide the thing that has to be fixed.
- A caller that disconnected or ran out its own deadline, which says nothing about the provider.
