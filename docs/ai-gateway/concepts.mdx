...
**Show-once secret:** the full key is displayed exactly once at creation and stored argon2id-hashed. Only a short prefix (`lw_vk_live_01HZX9`) remains visible in the UI. Rotation replaces the secret (old secret valid 24h grace); revocation propagates to gateway caches within 60 seconds.
