...
One ModelProvider can back many bindings (e.g. the same OpenAI credential under `primary` + `canary` slots, each with different rate limits). Bindings live at the project scope; the underlying ModelProvider may live at org / team / project scope (see [Provider Bindings → Scope & access](/ai-gateway/provider-bindings#scope--access)).
...
Each block emits a `GatewayAuditLog` row + the `X-LangWatch-Request-Id` response header → trace correlation. (A dedicated `langwatch.policy.blocked` span attribute is tracked as a v1.1 observability follow-up; see [Observability → Per-feature attributes](/ai-gateway/observability#per-feature-attributes-when-applicable) for the full v1 vs v1.1 matrix.)
...
Attributes always present (source of truth: `services/gateway/internal/otel/attrs.go`):
