...
- Requests against any VK **never seen by this pod** (cold) still fail — the gateway has no bundle to fall back to. Mitigation: enable `GATEWAY_CACHE_BOOTSTRAP_ALL_KEYS=true` so every pod warms its L1 with all active VKs at startup. Pair with Redis L2 (`GATEWAY_REDIS_URL`) so HPA-scaled pods inherit the warm set.
...
|---|---|---|---|
| LangWatch control plane | None for cached VKs | ~15 min JWT exp + `LW_GATEWAY_AUTH_CACHE_HARD_GRACE` (default 30 min) of stale-while-error; bootstrap cache + Redis L2 extend coverage to cold-for-pod VKs | Outbox re-enqueues debits safely; they flush on recovery. New VK creation is blocked until control plane returns. See [Recipe 8](#recipe-8--control-plane-outage--stale-while-error) for the operator runbook |
