...
The gateway does not send debits. Cost is captured as attributes on the OTel span the gateway emits; the control plane's `gatewayBudgetDebits` projection reads finalised spans and writes the ClickHouse ledger rows. Budget enforcement on the gateway is a purely local precheck against the budget snapshot baked into the cached bundle, so spend only moves when the whole chain runs:

```
gateway request -> OTel span (cost attrs) -> collector -> langwatch-workers
  -> budget-debit projection -> ClickHouse ledger -> bundle snapshot
  -> gateway precheck
```

Because the ledger is a projection over the append-only event log, a debit
that never landed is re-derived by replaying that window — the ledger is
repairable rather than a one-shot write.

**Diagnose, in order:**

| Check | How | If it fails |
|---|---|---|
| Spans leaving the gateway | `sum(rate(gateway_http_requests_total[5m]))` against your collector's received-spans counter | OTLP export is broken; see [Observability](/ai-gateway/observability) |
...
| Ledger advancing | Query `gateway_budget_ledger_events` in ClickHouse for the scope | The projection is stalled or the span carried no cost attrs |
...
|---|---|---|---|
| LangWatch control plane | None for cached VKs | ~15 min JWT exp + `LW_GATEWAY_AUTH_CACHE_HARD_GRACE` (default 6 h) of stale-while-error; bootstrap cache + Redis L2 extend coverage to cold-for-pod VKs | Spend accounting is unaffected: cost rides the OTel span and the ledger catches up once the control plane returns. New VK creation is blocked until control plane returns. See [Recipe 8](#recipe-8--control-plane-outage--stale-while-error) for the operator runbook |
