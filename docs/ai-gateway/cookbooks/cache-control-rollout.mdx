...
The forwarded request body now has `cache_control: {type: "ephemeral"}` on `system[-1]` (or `messages[-1].content[-1]` if the last user message has array content). Anthropic serves the cached prefix on the next matching request at 10% input-token cost.
...
Your eval suite uses dedicated VKs prefixed `vk-lw-*` and tags every request with `X-Langwatch-Suite: evals`. These requests should hit provider caches as rarely as possible, because fresh completions keep cache effects out of the eval scores.
...
```bash
curl -X PATCH https://app.langwatch.ai/api/gateway/v1/cache-rules/<rule_id> \
  -H "Authorization: Bearer $LANGWATCH_PAT" \
  -H "Content-Type: application/json" \
  -d '{ "action": { "mode": "force", "ttl": 600, "salt": "2026Q2-template-rollout" } }'
```

`action` is replaced wholesale, so send the mode and TTL you want to keep alongside the new salt.
...
- **`model` matchers fire on both endpoints.** A rule like `matchers.model = "claude-haiku-*"` matches on `/v1/chat/completions` and `/v1/messages` alike.
