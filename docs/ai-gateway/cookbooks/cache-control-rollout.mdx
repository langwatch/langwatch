...
- **`model` matcher doesn't fire on `/v1/messages` in v1.** Model parsing happens below cache-override in the gateway pipeline for the Anthropic-native endpoint. Use `/v1/chat/completions` or move the model check out of `matchers.model` into a `--match-tag` convention for v1. Full fix tracked as a low-pri iteration lift.
