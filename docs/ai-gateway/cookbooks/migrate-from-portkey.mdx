...
Run `/gateway/usage` spend + `/gateway/audit` mutation history in parallel to your existing Portkey analytics during the rollout — the LangWatch audit log will show you every VK / budget / provider mutation, which is especially useful in the cutover period when your ops team is still adjusting governance.
