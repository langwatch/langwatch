...
A virtual key, or the upstream provider credential behind it, can leak: pushed to a public repo, left on an ex-employee's laptop, mirrored into a vendor log by CI. This cookbook walks the end-to-end path: the gateway surfaces below contain a leaked virtual key without breaking every legitimate caller at the same time. A leaked provider credential is rotated in the provider console instead, see [What NOT to do](#what-not-to-do).
...
langwatch virtual-keys list --format json \
  | jq --arg p "$PREFIX" '.data[] | select(.display_prefix | startswith($p))'
```
...
2. Click **Audit history** in the header. This is a deep-link into `/settings/audit-log?targetKind=virtual_key&targetId=<vk_id>` pre-filtered to this VK only. You'll see every CREATE, UPDATE, ROTATE event with actor + timestamp + changed-fields diff summary, alongside any related platform actions on the same resource.
...
curl -sS -X POST "https://app.langwatch.ai/api/gateway/v1/virtual-keys/$VK_ID/rotate" \
  -H "Authorization: Bearer $LANGWATCH_API_KEY"
# Response: { "vk_id": "...", "secret": "vk-lw-...", "rotated_at": "..." }
```
...
<Info>
  Dedicated **grace-window observability** (`gateway_vk_grace_window_hit_total` Prometheus counter +
  `langwatch.vk.grace_window=true` span attr) is a **v1.1 follow-up**. For v1, teams measure rollout
  coverage by inspecting the LangWatch trace stream for each caller and cross-referencing against
  `gateway_request_id`s known to have been minted after `rotated_at`. File an issue if you hit this
  and need the dedicated metric sooner.
</Info>
...
# Revocation is audit-logged with the actor. To record why, disable the key
# with a reason first, then revoke:
langwatch virtual-keys disable "$VK_ID" --reason "secret leaked in commit SHA $LEAK_SHA"
```
...
Cache invalidation propagates to every gateway pod within ~60 seconds via the `/api/internal/gateway/changes` long-poll. If you need instant invalidation everywhere for a regulatory-grade incident, restart the gateway deployment, the next request to hit a cold pod pulls fresh state before serving.
...
Pipe the result through `psql -A -F$'\t'` and gzip it into `incident-<vk_id>-audit.tsv.gz`. The shape carries the actor (user or `svc_<projectId>`), action code, `before`, `after` JSON, and `metadata.requestId` for cross-referencing the trace stream. See [Audit log → Querying programmatically](/ai-gateway/audit#querying-programmatically) for the full schema.
