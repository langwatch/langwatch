...
<Info>
  Dedicated **grace-window observability** (`gateway_vk_grace_window_hit_total` Prometheus
  counter + `langwatch.vk.grace_window=true` span attr) is a **v1.1 follow-up**. For v1,
  teams measure rollout coverage by inspecting the LangWatch trace stream for each caller
  and cross-referencing against `gateway_request_id`s known to have been minted after
  `rotated_at`. File an issue if you hit this and need the dedicated metric sooner.
</Info>
