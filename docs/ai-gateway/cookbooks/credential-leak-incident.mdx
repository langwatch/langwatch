...
A virtual key, or the upstream provider credential behind it, can leak: pushed to a public repo, left on an ex-employee's laptop, mirrored into a vendor log by CI. The gateway surfaces below contain the leak without breaking every legitimate caller at the same time.
