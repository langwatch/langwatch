...
`metadata` is written `key:value` and splits on the first colon, so a value may contain colons of its own. A key may not, and both halves must be non-empty. Repeating a key widens that key: `metadata=tier:gold&metadata=tier:silver` matches either.
