...
`metadata` is written `key:value` and splits on the first colon, so a value may contain one. Repeating a key widens that key: `metadata=tier:gold&metadata=tier:silver` matches either.
