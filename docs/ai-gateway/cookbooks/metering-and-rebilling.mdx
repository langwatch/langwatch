...
The buckets are disjoint. An image generation reports `output_tokens` 0 and its render under `output_image_tokens`, so summing `output_tokens` alone undercounts image traffic. Cost is the sum across all buckets, so reconcile every one of them.
