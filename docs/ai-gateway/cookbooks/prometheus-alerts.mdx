    ...
    rules:
      ...
      - alert: SpendSpoolDrops
        ...
        annotations:
          ...
          description: |
            Budget debits are written from the spend records this pod spools,
            so a dropped record is spend that is neither billed nor counted
            against a budget. reason=intake means the pod produced faster than the
            spool could write; reason=overflow means the drainer is not
            shipping fast enough and the oldest segments were deleted to stay
            inside the size bound. Check the control plane's spend-command
            ingest is reachable and returning 2xx
      ...
      # control-plane warehouse than from Prometheus, since attribution
      # happens control-plane side when the spend commands are folded into
      # debits. See /gateway/usage UI for the visual equivalent.
  ...
  - interval: 1m
    ...
    alert_rule_test:
      - eval_time: 10m
        ...
        exp_alerts:
          - exp_labels:
              { credential_id: cred_openai_primary, severity: page, team: ai-platform }
