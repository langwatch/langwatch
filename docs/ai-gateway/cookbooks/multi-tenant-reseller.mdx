...
If you need SIEM export: the Postgres `GatewayAuditLog` table is queryable (not ClickHouse — gateway writes are PG-authoritative per finding #8 architecture). Set up a daily export to your SIEM's ingestion pipeline; see [Audit log](/ai-gateway/audit) for the REST export path (`GET /api/gateway/v1/audit`).
