...
If you need SIEM export: the Postgres `AuditLog` table is queryable for gateway-shape rows (filter on `targetKind IN ('virtual_key', 'budget', 'provider_binding', 'cache_rule')`). Set up a daily export to your SIEM's ingestion pipeline; see [Audit log](/ai-gateway/audit) for the REST export path (`GET /api/gateway/v1/audit`).
