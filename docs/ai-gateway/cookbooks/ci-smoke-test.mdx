...
- A ModelProvider scoped at ORG/TEAM/PROJECT is healthy (reaches actual upstream) via its Advanced (Gateway) settings.
- VK creation + secret reveal flow works at the requested scope.
- VK resolves end-to-end against the gateway data plane.
- Response carries `X-LangWatch-Trace-Id`, `X-LangWatch-Span-Id`, `X-LangWatch-Gateway-Request-Id`, and `traceparent`.
...
- At least one healthy ModelProvider configured under **Settings → Model Providers** at ORG, TEAM, or PROJECT scope. Set the fallback priority in its Advanced (Gateway) section if you want CI to exercise the fallback order. The RPM, TPM and RPD inputs on the same row are stored and not enforced, so CI cannot exercise them; to test a rate limit, set RPM or RPD on the virtual key.
...
# 1. Mint short-lived VK at the org scope (uses the org's default RoutingPolicy)
echo "Creating CI VK at ORG:$CI_ORG_SLUG..."
CREATE=$(langwatch virtual-keys create \
  --name "ci-smoke-$TIMESTAMP" \
  --env test \
  --scope "ORG:$CI_ORG_SLUG" \
  --format json)
...
[ -z "$REQUEST_ID" ] && { echo "FAIL: missing X-LangWatch-Gateway-Request-Id"; exit 1; }
...
| `curl ... 403 model_not_allowed` | No ModelProvider visible from `ORG:$CI_ORG_SLUG` exposes `gpt-5-mini` | Replace the model in the script with one your eligible ModelProviders actually support, or add an OpenAI ModelProvider at the org scope |
| `FAIL: trace propagation` | Per-tenant OTel middleware is off | See [Observability → trace-id handshake](/ai-gateway/observability#trace-id-propagation--concrete-handshake) |
