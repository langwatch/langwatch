...
```json
{
  ...
  "panels": [
    ...
    },
    {
      ...
      "description": "Emitted for the request and response directions when the control-plane guardrail call fails and the gateway fails open. Sustained > 0 means the evaluator is missing its SLO. The stream_chunk path also fails open but swallows the error internally, so it is not counted here; use the langwatch.guardrail.stream_chunk_fail_open trace attribute for that.",
      "datasource": "$datasource",
