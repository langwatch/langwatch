...
Prompt caching cuts input-token costs by up to 90% on hot prefixes, and a gateway that reformats or reorders the payload silently invalidates the cache. The gateway preserves provider-native caching byte-for-byte by default, and lets operators compose overrides without breaking the primitives underneath.

## The control surface: four layers (v1)

1. **Gateway default**: `respect` (byte-for-byte passthrough). Never changes the payload.
...
Latency budget: rules are resolved **once per VK bundle refresh** and baked into the bundle as a pre-sorted array. At request time, the gateway walks the sorted array until a match fires, for the expected low single-digit rule count per VK, this is sub-microsecond. Hot-path total overhead stays at ~700 ns.

Editing a rule triggers that refresh rather than waiting for one. Every create, edit and delete appends a change event in the same transaction as the write; the gateway's `/changes` long-poll returns it on the next cycle and drops the organization's cached bundles, so the following request rebuilds against the new rules.
...
- Rules are resolved once per VK bundle refresh, not once per request, and baked into the bundle as a pre-sorted array. A request only walks that array to find its match, so evaluation costs no upstream call.
- Rule hits exposed in observability via `langwatch.cache.rule_id` span attribute + `gateway_cache_rule_hits_total{rule_id, mode_applied}` counter.
