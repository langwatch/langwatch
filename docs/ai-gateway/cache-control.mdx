---
title: Cache Control
description: Provider-agnostic cache control for every gateway request — passthrough, per-VK defaults, per-request headers, and v1.1 rule-based overrides.
...
2. **Per-VK default** — `respect` / `disable` today, `force` / `ttl=NNN` in v1.1. Configurable in the VK drawer under "Cache control".
...
4. **Per-request header** — `X-LangWatch-Cache: respect|disable|force|ttl=NNN` on an individual request. Highest precedence when set.
...
| Provider | Native scheme | `respect` behavior | `disable` behavior | `force` (v1.1) | Notes |
|---|---|---|---|---|---|
| **Anthropic** (direct + Bedrock + Vertex) | Explicit `cache_control: {type: "ephemeral"\|"persistent"}` on content/system/tool blocks | Bytes forwarded unchanged; provider honours markers (5 min / 1 h TTL) | Recursive JSON-walk drops every `cache_control` key at any nesting depth before dispatch | Gateway injects `cache_control: ephemeral` on large stable prefixes (system + last user turn) if not present | `cache_read_input_tokens` and `cache_creation_input_tokens` separately reported in the trace |
| **OpenAI** (direct + Azure) | Automatic — prefixes ≥1024 tokens cached, no client flag, 50% discount on cached tokens | Bytes forwarded unchanged (nothing to preserve — provider decides) | No-op on wire; gateway can optionally set a salt via a request-id suffix to break the cache key (future) | No-op — caching is out of the client's control; rule may still force a cache-salt variation for A/B | `usage.prompt_tokens_details.cached_tokens` reported in response |
| **Gemini** (Vertex + direct) | Explicit `cachedContent` resource — client POSTs to `/v1/cachedContents` then passes `cachedContent: <id>` on generation | Bytes forwarded unchanged; client holds the cache ID | Strips `cachedContent` field from the request → forces cold generation | **Deferred to v1.2** — force-creating a cache ID requires a second upstream call which breaks the zero-hop passthrough framing | TTL minimum 1 min, max 10 min for context caches |
...
| **Bedrock Claude** | Inherits Anthropic's `cache_control` shape via converse API | Same as Anthropic | Same as Anthropic | Same as Anthropic (v1.1) | Bedrock region matters — not all regions support Anthropic cache yet |
| **Vertex Claude** | Inherits Anthropic's shape | Same as Anthropic | Same as Anthropic | Same as Anthropic (v1.1) | — |

The common shape: `respect` is always safe; `disable` always strips cache-related fields (where applicable); `force` is provider-specific injection (v1.1).
...
- Anthropic (incl. Bedrock Claude + Vertex Claude): gateway injects `cache_control: {type: "ephemeral"}` on the system message and the last user turn if not already present. Shipped in v1.
- Gemini: deferred to v1.1 — forcing a cache ID requires a separate `/v1/cachedContents` POST which breaks the zero-hop passthrough framing; needs its own spec round.
- OpenAI / Azure: no-op on wire (their cache is automatic); the mode can still be set by a rule to log intention for observability.

v1 ships Anthropic `force` with TTL-maps-to-ephemeral-or-persistent; Gemini `force` returns `400 cache_override_not_implemented` with a pointer to this page.

### `ttl=NNN`

Same as `force` but carries an explicit TTL in seconds. Provider mapping:
- Anthropic: chooses `ephemeral` (5 min) or `persistent` (1 h) based on the TTL range; anything above 3600 clamps to `persistent`.
- Gemini: sets the `ttl` field on `cachedContent` (deferred to v1.2).

Currently returns `400 cache_override_not_implemented`.

## Response headers

Every successful response carries:

- `X-LangWatch-Cache-Mode` — the mode that was applied (`respect` / `disable` / `forced`). Useful for tracing and A/B analysis: a client that sets `X-LangWatch-Cache: disable` should see `X-LangWatch-Cache-Mode: disable` on the response, independent of whether the provider reported a cache outcome.
