...
The response header `X-LangWatch-Cache-Mode` reports the outcome: `respect`, `disable` or `force` when a rule fired, `auto` when the provider default fired. The header is absent when nothing changed the body.
