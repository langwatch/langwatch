...
| Mode | Behaviour |
|---|---|
| `respect` | Forward cache markers byte-identically. Provider handles caching. **(default)** |
| `force` | Gateway injects `cache_control: ephemeral` on large stable prefixes (system message, tool definitions, first N messages) even if the client didn't. Useful for clients that don't know about caching. Gateway never *removes* client-supplied markers in this mode. |
| `disable` | Gateway strips every cache marker and forces a cold call. Use for benchmarks and A/B cache-impact tests. Gateway's own semantic cache is also bypassed. |
| `ttl=3600` | Same as `force` but with an explicit TTL hint. Provider may or may not honour it depending on the caching model. |

Override per-request via header: `X-LangWatch-Cache: force`, `X-LangWatch-Cache: disable`, `X-LangWatch-Cache: ttl=3600`.

Malformed header → `400 cache_override_invalid` error envelope.

## What's reported back

Every response carries `X-LangWatch-Cache: <outcome>`:

- `hit` — provider reported serving from cache (large portion of input from cache).
- `miss` — no cache hit.
- `bypass` — mode was `disable`.
- `forced` — mode was `force` and the gateway injected markers that weren't in the incoming request.
...
- You're fronting a legacy OpenAI-SDK codebase that doesn't know about Anthropic's cache_control blocks, but you've flipped the VK's primary to Anthropic. The gateway inserts caching on the stable prefix automatically, so cost drops dramatically with no code change.
- You have a long tool-definition section that's identical across requests.

**Use `disable` when:**

- You're benchmarking "cold cost" before deciding whether a caching tier is worth enabling.
...
- You want explicit control over cache retention (e.g. "cache this prompt for 1 hour, no more").
