...
This page covers the standard setup and how to propagate trace ids so you don't double-count cost.
...
| `X-LangWatch-Gateway-Request-Id` | `req_` + 30 hex characters, use in support tickets |
...
const requestId = resp.response.headers.get("X-LangWatch-Gateway-Request-Id");
```
