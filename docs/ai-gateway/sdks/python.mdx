...
This page covers the standard setup and how to propagate trace ids through the gateway so you don't double-count cost.
...
| `X-LangWatch-Gateway-Request-Id` | `req_` + 30 hex characters, gateway-scoped. Use this in support tickets |
...
Grab the `X-LangWatch-Gateway-Request-Id` for support tickets or log correlation:
...
request_id = resp.http_response.headers["X-LangWatch-Gateway-Request-Id"]
...
client = anthropic.Anthropic(
    ...
    api_key="vk-lw-...",   # SDK accepts VK here; gateway also accepts via x-api-key
)
