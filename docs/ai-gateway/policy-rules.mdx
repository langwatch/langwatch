...
- **Models**: regex-matched against the resolved model id (distinct from the glob-based `models_allowed` allowlist, see [Models dimension vs models_allowed](#models-dimension-vs-models_allowed)).

Each has a `deny` list and an optional `allow` list.

<Note>**Enforcement point:** policy-rule checks run immediately after authentication and rate limiting, **before** guardrails, budget reconciliation, or the upstream provider. The tools, MCP, and URL dimensions judge the request body as sent; the models dimension runs a step later, at model resolution, so it can judge the model the request actually resolves to. A blocked request never incurs a provider token cost, a guardrail evaluator run, or a budget debit. This is the cheapest possible reject path in the dispatcher.</Note>
...
The `models` dimension is RE2-matched against the **resolved** model id, the one the request will actually be billed and served under, not the string the caller typed. Alias resolution runs first, then the rules judge its result. This is policy-by-regex (e.g. "block anything matching `^gpt-4(-turbo)?$`"), distinct from the static `models_allowed` glob allowlist on the VK.

Two consequences follow directly, and both are the point of judging the resolved id:

- An alias pointing at a denied model **is** blocked. Naming `gpt-4` in an alias is not a way around a rule that denies `^gpt-4(-turbo)?$`.
- A rule denying a name that resolves elsewhere does **not** block. If `gpt-4` is an alias for `azure/my-eastus-deployment`, a deny on `^gpt-4$` never fires, because nothing denied ever runs.

Rules match either spelling of the resolved model, the bare id and the provider-qualified one, so `^gpt-4.*` and `^openai/gpt-4.*` both reach `openai/gpt-4o`. Write whichever your team reads more naturally instead of maintaining both.

The other three dimensions are properties of the request body as sent, so tools, MCP servers, and URLs are judged exactly as the client wrote them.
...
They compose: a request passes only if both checks allow it. `models_allowed` is applied first, during model resolution, and the regex rules judge what resolution settled on. Both read the resolved model in either spelling, so neither can be routed around with an alias. Use `models_allowed` for 90% of cases and reach for `policy_rules.models` when regex semantics are actually needed.
...
| **Models** | RE2 regex policy on the resolved model, after any alias is applied (distinct from the static `models_allowed` glob on the top of the VK form). | Same allow semantics. |
...
- `X-LangWatch-Gateway-Request-Id` response header for joining the 403 back to the trace stream.
