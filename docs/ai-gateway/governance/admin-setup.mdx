...
`docs/integration/install.mdx` for full prerequisites and self-hosted
...
#    /me/usage      — see Budget caveat below
#    /governance    — admin's governance home with live activity
```
...
<Note>
  **Budget caveat for `/me/usage`**: spend is computed from the
  ...
  This matches production behavior — admins attach budgets in step 4
  ...
  seeded user (or to the personal team) before firing completions.

  **Product follow-up**: separating the usage-display query from the
