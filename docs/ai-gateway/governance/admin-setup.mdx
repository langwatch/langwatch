...
<Note>**Governance is currently behind a preview flag.** The `/governance` UI surface (and the **Govern** main-menu entry) require the `release_ui_ai_governance_enabled` flag enabled for your org.
...
Once the flag is on, fresh admins can reach `/governance` even with no project created yet — governance is org-scoped, not project-scoped.</Note>
