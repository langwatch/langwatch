...
The repo ships dogfood helpers that mint a personal virtual key and
fire a real completion through your local Go gateway, so you can see
real spend land in `/me/usage` and the governance dashboard before you
roll out to your team.

```bash
# 1. Bring up the stack (postgres + redis + clickhouse + app + workers)
...
- [`langwatch` CLI](/integration/cli): the unified CLI lives in the
  TypeScript SDK, and the device-flow login wraps every coding
