...
LangWatch governance events (per-user spend, per-source activity, anomaly alerts) can be pulled into your security team's SIEM alongside your other audit feeds.
...
| **AWS Security Hub** | Custom integration via the `BatchImportFindings` API; your cron worker maps OCSF events to Security Hub findings |
...
- ⏳ **Heavyweight SIEM push infrastructure** (per-org SIEM push UI, DLQ + replay, managed Splunk/Datadog HEC integrations). The OCSF read API and the cursor-pull cron pattern cover the common case; rich push integrations land per-customer when sales pipeline justifies them
...
- ⏳ **Cryptographic signing of exported rows**: deferred to the tamper-evidence follow-up; see [Compliance architecture / Tamper-evidence](/ai-gateway/governance/compliance-architecture#tamper-evidence-boundaries)
...
| `microsoft_365_audit` | Management Activity API `UserId` (a UPN; `UserKey` is a PUID and is carried separately) | Management Activity API `Operation` | Management Activity API `AgentId` |
...
- [Governance data retention](/ai-gateway/governance/retention): OCSF events are kept as long as the spans they were derived from
