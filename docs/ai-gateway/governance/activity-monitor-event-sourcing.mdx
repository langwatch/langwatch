...
```text
┌────────────────────────────────────────────────────────────────┐
│  /api/v1/ingest/otel/:sourceId  /api/v1/ingest/webhook/:sourceId      │
...
- New command: `RecordActivityEventCommand` wired into the
  pipeline.
- Refactor `/api/v1/ingest/otel/:sourceId` and
  `/api/v1/ingest/webhook/:sourceId` to call the command instead of
  writing CH directly.
