...
```
enterprise/modules/governance/server/src/services/    # Personal workspace, VK, RoutingPolicy services
enterprise/modules/governance/server/src/adapters/    # Ingestion pullers + activity-source adapters
enterprise/modules/governance/server/src/transport/api-rest/governance-ingest.api.ts    # IngestionSource receivers
enterprise/modules/governance/server/src/transport/api-trpc/personal-virtual-key.api.ts
enterprise/modules/governance/server/src/transport/api-trpc/routing-policy.api.ts
enterprise/modules/governance/server/src/transport/api-trpc/personal-dashboard.api.ts   # personalContext + personalUsage + personalBudget
enterprise/modules/governance/web/src/screens/governance/    # Admin routing policies, ingestion sources, activity monitor
modules/gateway/server/src/                   # AI Gateway control plane: virtual keys, budgets
modules/gateway/web/src/                      # Gateway screens; budget-exceeded 402 banner
services/aigateway/                                     # AI Gateway data plane (Go, Bifrost-embedded)
modules/auth/server/src/transport/api-rest/auth-cli-device-flow.api.ts   # Device-flow + access tokens
modules/api-key/web/src/screens/api-key/cli-auth.screen.tsx   # Device-flow approval UX
modules/user/web/src/screens/personal-workspace/    # /me overview, sessions, configure
