...
Closed SaaS (Cowork / Copilot / Workato / OpenAI / Claude / S3)
         ...
         │      ▶  /api/v1/ingest/<source-type>/<source-id>
...
- 🚧 Generic OTel passthrough receiver (`/api/v1/ingest/otel/<sourceId>`)
- 🚧 Generic webhook receiver (`/api/v1/ingest/webhook/<sourceId>`)
...
```
packages/enterprise/features/governance/server/src/services/    # Personal workspace, VK, RoutingPolicy services
packages/enterprise/features/governance/server/src/adapters/    # Ingestion pullers + activity-source adapters
packages/enterprise/features/governance/server/src/transport/api-rest/governance-ingest.api.ts    # IngestionSource receivers
packages/enterprise/features/governance/server/src/transport/api-trpc/personal-virtual-key.api.ts
packages/enterprise/features/governance/server/src/transport/api-trpc/routing-policy.api.ts
packages/enterprise/features/governance/server/src/transport/api-trpc/personal-dashboard.api.ts   # personalContext + personalUsage + personalBudget
packages/enterprise/features/governance/web/src/screens/governance/    # Admin routing policies, ingestion sources, activity monitor
packages/features/gateway/server/src/                   # AI Gateway control plane: virtual keys, budgets
packages/features/gateway/web/src/                      # Gateway screens; budget-exceeded 402 banner
services/aigateway/                                     # AI Gateway data plane (Go, Bifrost-embedded)
packages/features/auth/server/src/transport/api-rest/auth-cli-device-flow.api.ts   # Device-flow + access tokens
packages/features/api-key/web/src/screens/api-key/cli-auth.screen.tsx   # Device-flow approval UX
packages/features/user/web/src/screens/personal-workspace/    # /me overview, sessions, configure
apps/ui/src/model/ui-route-table.ts                     # Where /me, /settings and /cli/auth are mounted
