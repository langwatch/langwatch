...
plane, its own admin console, its own audit trail.
...
| `ModelProvider` | Upstream LLM API key (basic creds) + Gateway-only fields (RPM/TPM/RPD, fallback priority, providerConfig) in the **Advanced (Gateway)** section | Scoped to ORGANIZATION/TEAM/PROJECT; one record per credential, no separate gateway binding |
