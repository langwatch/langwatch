...
- **Microsoft Purview / Copilot Studio** — pull every 1-5 min via
  the Office 365 Management Activity API.
- **Claude Compliance API** — pull. Plus Cowork **OTel push**
  (Tier 4) when the admin enables it centrally.
- **OpenAI Compliance Platform** — pull JSONL files at
  minutes-level latency.
- **Workato Activity Audit Log** — push to a webhook the customer
  configures, or stream-to-S3.
- **Gemini for Workspace / Vertex** — pull via Cloud Logging, or
  push via Pub/Sub sink.
...
- 🚧 IngestionSource backend foundation (Tier 3 + 4 entry point).
- 🚧 Admin oversight dashboard (cross-tier rollup, mocked data first).
