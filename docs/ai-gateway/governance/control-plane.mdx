...
- 🚧 OTel passthrough + generic-webhook receivers (Tier 3 + 4
  ingest endpoints).
- 🚧 IngestionSource setup forms (per-source-type, in lane B).
- 🚧 IngestionSourceService + repository scaffold so the admin
  oversight dashboard's mocked queries can swap to real data.
