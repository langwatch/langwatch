...
|---------|---------|----------|----------------|
| `/governance` (forthcoming top-level home) | Daily-use **org governance home** — spend, ingestion-source health, anomalies, per-user breakdown. Read-mostly. | Org admins whose org has activated governance (personal VKs, IngestionSources, RoutingPolicies, or gateway usage exists) | Top-level route, org-scoped. Replaces the read-mostly content currently temporarily under `/settings/governance`. |
...
| # | Step | Shot |
|---|------|------|
| 1 | Admin oversight `/settings/governance` (mocked v0) | [open](https://i.img402.dev/0hqnvut00t.png) |
| 2 | `/settings/governance/ingestion-sources` — empty state | [open](https://i.img402.dev/wx4v7m7a8g.png) |
| 3 | Composer open — `sourceType` picker + dynamic per-type form | [open](https://i.img402.dev/ycld8afavo.png) |
| 4 | Composer filled — Generic OTel example | [open](https://i.img402.dev/yixqegurj3.png) |
| 5 | One-time secret reveal modal — `ingestSecret` + OTLP URL + Copy | [open](https://i.img402.dev/w4fl31ukrc.png) |
| 6 | List polled, flipped to **Active**, last event 13 s ago | [open](https://i.img402.dev/supafck5au.png) |
