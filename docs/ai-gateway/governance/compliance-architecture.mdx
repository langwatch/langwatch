...
- `ORGANIZATION_ADMIN` — full set (all `view` + `manage` actions across all 5 governance resources)
- `ORGANIZATION_MEMBER` — none (chrome hides Govern by default)
- `ORGANIZATION_EXTERNAL` — none (lite-tier billing scope; future derivation reads from billing state)
