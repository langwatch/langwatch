...
### 3. RBAC via project membership

Two roles at org scope:

- **Org admin** — full read/write on IngestionSource, AnomalyRule, retention config, OCSF export
- **Auditor** (read-only) — read access across all governance projections + OCSF export, no write

Both roles map onto project-membership of the hidden Governance Project. Members of customer-visible projects do NOT inherit governance read access (governance is an org-wide concern, not a per-project one). Enforced at every tRPC procedure + every REST adapter.
