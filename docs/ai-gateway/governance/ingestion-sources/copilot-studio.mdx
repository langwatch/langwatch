...
- **Purview event → ActivityEvent mapping.** Purview's audit JSON has `Operation`, `UserId`, `RecordType`, `ResultStatus` etc. The mapping into Actor / Action / Target / `eventType` is pending and will live in a `purview.ts` normaliser alongside `otel.ts`.
