...
- **JSONL → ActivityEvent mapping.** OpenAI Enterprise Compliance JSONL has a documented schema (event type, user ID, model, prompt + completion tokens, content classification flags). Mapping into Actor / Action / Target / cost / token fields is pending and will live in an `openai-compliance.ts` normaliser.
- **Cost reconciliation.** OpenAI's compliance feed includes per-event token counts but not USD cost. The cost-enrichment pipeline already handles this for `gateway_activity_events` once the puller lands — no extra work needed on the receiver side.
