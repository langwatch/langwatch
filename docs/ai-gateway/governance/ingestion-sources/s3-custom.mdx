...
- **Persistence:** envelope-only. One ActivityEvent with `eventType: agent.action`, the verbatim payload in `rawPayload`, and `actor` / `target` / `costUsd` / token counts unset (parser pending).
