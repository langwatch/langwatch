...
<Note>
  On SaaS, governance rollout is managed per organization: if you don't see these surfaces, or
  `langwatch login --device` is refused with a governance error, ask your admin.
</Note>
...
<Note>
  The single-binary installers (`curl ... | sh`, Homebrew tap, PowerShell `iex`) are on the roadmap
  but not yet published. Until then, `npm install -g langwatch` is the canonical path on every OS
...
4. The CLI fetched your AI tools, inheritable providers, and budget from `/api/v1/auth/cli/bootstrap` and rendered the ceremony you see above.

Tokens persisted at `~/.langwatch/config.json` (mode 0600).

<Note>
  You did **not** pick a model provider during setup. Your admin already wired up Anthropic, OpenAI,
  Gemini, etc. at the organization level. You just inherit them.
</Note>
...
<Info>
  **Verified end-to-end in CI.** `langwatch login` + each of the five wrappers above (`claude`,
  `codex`, `cursor`, `gemini`, `opencode`) is exercised in the `governance-wrapper.e2e.test.ts`
  Node-only e2e suite (fake control-plane, fake gateway, mocked tool binaries) covering login config
  write, env-var injection, routing with bearer-VK, budget pre-flight (402/200/5xx), and exit-code
  propagation. 16 scenarios, 3-second runtime, runs on every PR. See [CLI debug → e2e
  coverage](/ai-governance/cli-debug#cli-wrapper-e2e-coverage) for the assertion shape per tool.
</Info>
