...
`fail_open` is emitted for the `request` and `response` directions only, when the control-plane guardrail call fails and the gateway lets the request through. The `stream_chunk` path also fails open, but it swallows the error internally and records nothing on this counter, so a `direction="stream_chunk", verdict="fail_open"` series will never appear. Use the `langwatch.guardrail.stream_chunk_fail_open` span attribute for that case.
