...
For VKs where an unavailable guardrail should _not_ block (e.g. best-effort redaction on low-stakes traffic), set `guardrails.response_fail_open: true` on the VK config. The gateway then passes the response through with a warning log. (No dedicated fail_open span attribute is emitted in v1; the response's error class plus the guardrail upstream's own span is how operators trace this path today.)
...
Latency budget: **≤50 ms per chunk**. If the evaluator doesn't respond in time, or the upstream guardrail errors, the gateway **fails open**: the chunk passes through with an OTel warning (the WARN log carries the reason). This is an explicit contract decision: blocking the user's stream on a slow policy service is worse than occasional pass-through. Note that this path swallows the error internally, so no `fail_open` verdict is counted for it; the WARN log and the `langwatch.guardrail.stream_chunk_fail_open` span attribute are the signals for slow or flaky evaluators on stream chunks.
...
- Pre: blocks return `403 guardrail_blocked` with zero cost recorded on the span.
...
**Dispatch.** The gateway fans out the guardrail calls in a direction in parallel, bounded by `MAX_GUARDRAIL_CONCURRENCY` (default 8). All calls start at the same time; the dispatcher waits for the slowest verdict, with the early-exit rule below.
...
`fail_open` is emitted on all three directions, whenever the control-plane guardrail call fails and the gateway lets the traffic through unchecked. The `request` and `response` directions do so only when the virtual key opts into fail-open; the default is fail closed and the request is refused instead. The `stream_chunk` direction always fails open by design, so a slow policy service never stalls a stream a user is already reading, which means a `direction="stream_chunk", verdict="fail_open"` series is the one to watch: it is the only signal that chunk-level policy stopped being enforced. The `langwatch.guardrail.stream_chunk_fail_open` span attribute carries the reason for an individual bypass, which the counter deliberately does not, since the reason is unbounded text.

<Info>
  `gateway_http_requests_total{(status = "403")}` is a useful cross-check: it flips on every block
  verdict regardless of direction, so a divergence between it and
  `sum(rate(gateway_guardrail_verdicts_total{(verdict = "block")}[5m]))` points at a direction that
  is blocking without being counted.
</Info>
