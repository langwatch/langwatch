...
<CardGroup cols={2}>
  <Card title="I'm a developer" icon="laptop-code" href="/ai-gateway/governance/personal-keys">
  ...
  <Card title="I'm an org admin" icon="shield-halved" href="/ai-gateway/governance/admin-setup">
...
The rest of this page uses a **service virtual key**, the flow for non-personal traffic (server-to-server calls from your applications, evaluators, prompt playground, etc.).

<Note>
  You'll need a LangWatch account, an **organisation admin** or **project admin** role, and at least
  one model provider (OpenAI, Anthropic, Azure OpenAI, Bedrock, Vertex, or Gemini) configured at
  **Settings → Model Providers**. If you haven't set one up yet, start there, the AI Gateway reuses
  those credentials.
</Note>

<CardGroup cols={2}>
  <Card title="Python, TypeScript SDK" icon="code" href="/ai-gateway/sdks/python">
    Drop-in for the OpenAI, Anthropic SDKs, swap `base_url` and `api_key`.
  </Card>
  <Card title="Use it from your coding assistant" icon="terminal" href="/ai-gateway/cli/overview">
    Wire Claude Code, Codex, OpenCode, Cursor, or Aider through the gateway.
  </Card>
  <Card title="Core concepts" icon="compass" href="/ai-gateway/concepts">
    Virtual keys, budgets, ModelProvider gateway settings, routing policies, cache rules, fallback
    chains.
  </Card>
  <Card title="API reference" icon="book" href="/ai-gateway/api/chat-completions">
...
- [Caching passthrough](/ai-gateway/caching-passthrough): the rule that keeps your 90% Anthropic cache discount.
- [CLI Integrations](/ai-gateway/cli/overview): ship the gateway to every developer's Claude Code, Codex, Cursor.
- [Budgets](/ai-gateway/budgets): cap spend per organization, team, project, key, person, or end user.
- [Billing and spend events](/ai-gateway/billing-events): the per-request record, for reconciliation and rebilling.
