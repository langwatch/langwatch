...
Clients read LLM streams as Server-Sent Events: coding CLIs parse tool-call deltas incrementally, UX layers render token-by-token, and any gateway that reformats the stream breaks something. The LangWatch AI Gateway is explicit about what it may and may not mutate.

## The streaming contract

### Before the first chunk: mutations allowed

- Gateway may inject response headers (`X-LangWatch-Gateway-Request-Id`, `X-LangWatch-Provider`, `X-LangWatch-Model`).
...
- No mid-stream provider switch, even if the primary dies after chunk N, the gateway does NOT fall back to a secondary provider silently. Instead it terminates the stream.
...
- **Budget ≤50 ms per chunk.** Slow or errored guardrails **fail open**: the chunk passes through and `langwatch.guardrail.stream_chunk_fail_open=<reason>` is set on the trace. Blocking the user's stream on a slow policy service is worse than occasional pass-through. Note that this path swallows the error internally, so it does **not** emit `gateway_guardrail_verdicts_total{verdict="fail_open"}`; the trace attribute is the signal for degraded policy services on stream chunks. Only the `request` and `response` directions count fail-open verdicts.
...
- `X-LangWatch-Gateway-Request-Id` on the first frame.
...
<Info>Per-stream telemetry attrs (`langwatch.stream.chunk_count`, `.first_chunk_ms`, `.total_ms`, `.terminated_at_chunk`) are a v1.1 observability follow-up. In v1, the single-span-per-request model + log-line correlation is the operator signal.</Info>
...
- **`success_no_usage` warning is now rare.** With the auto-injection above, OpenAI streams should always report usage. If the provider still returns no usage frame, the gateway appends one SSE comment line after the final chunk: `: provider_did_not_report_usage_on_stream`. Comment lines are ignored by every spec-compliant client, so they never break parsing; they stay visible to `curl -N`. The trace span also carries `span.status = success_no_usage`. This means either (a) the caller opted out of usage explicitly via `include_usage: false`, or (b) the provider returned a stream without a final usage frame.

### Setting `stream_options.include_usage` in your client

Auto-injection means you don't have to. To keep the call explicit and portable across gateways, the form is identical to direct-OpenAI usage:
...
```promql
sum(rate(gateway_streaming_usage_missing_total[5m])) > 0
```

After the gateway-side auto-injection landed, this should sit at zero in steady state for OpenAI, Azure streaming traffic. If it spikes, capture the request body via `LW_LOG_MESSAGE_BODY=1` (see [Self-hosting → Config → Debug knobs](/ai-gateway/self-hosting/config#debug-development-env-knobs)) and check whether the caller is opting out of usage explicitly.
