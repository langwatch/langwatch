...
If this fires, a client is streaming without `include_usage` and you're effectively not charging those requests. The gateway's warning header and `span.status = success_no_usage` make this observable — wire a Grafana alert on that span status too.
