...
<Info>
  Per-stream telemetry attrs (`langwatch.stream.chunk_count`, `.first_chunk_ms`, `.total_ms`,
  `.terminated_at_chunk`) are a v1.1 observability follow-up. In v1, the single-span-per-request
  model + log-line correlation is the operator signal.
</Info>
