...
gateway:
  ...
  env:
    ...
    GATEWAY_CACHE_BOOTSTRAP_ALL_KEYS: "false"  # reserved; no-op today, see Config docs

  # Admin / pprof listener — loopback-bound by default.
  ...
  # an existingAuthSecretName to gate the listener with a bearer token.
  admin:
  ...
  # Server timeouts close slowloris-style attacks. WriteTimeout
  ...
  # Per-direction guardrail budgets
  guardrails:
...
<Note>The chart ships defaults for every new field (admin / budget / guardrails) — `helm upgrade` from an earlier chart version works without overrides.</Note>
...
- Use Redis (`GATEWAY_REDIS_URL`) as L2 cache so a replica joining a running cluster warms up from shared cache rather than hammering the control-plane. This is the primary mechanism for keeping replicas serving while the control plane is briefly offline — see [Auth-cache stale-while-error](/ai-gateway/self-hosting/config#auth-cache) for the soft/hard-grace knobs.
- `GATEWAY_CACHE_BOOTSTRAP_ALL_KEYS=true` is a planned v1.1 enhancement to pre-warm L1 with every active VK on startup; the flag is wired in chart values but currently has no Go-side implementation, so setting it has no effect today. Use Redis L2 + the auth-cache stale-while-error grace window for cold-start coverage.
...
- **Startup (`/startupz`)** — similar to readiness but lenient on bootstrap duration for large VK populations. Optionally runs a one-shot network connectivity probe (see below) against a configured host list; if any host fails to resolve or dial, `/startupz` never flips green and k8s recycles the pod.
