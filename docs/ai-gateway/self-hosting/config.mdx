...
| `SERVER_GRACEFUL_SECONDS`        | `60`          | Drain window after `SIGTERM`. `/readyz` flips to 503 immediately; in-flight requests get this many seconds to complete before the listener closes. Must be zero or positive: it is time spent waiting, so a negative value is refused at startup. The chart sets it from `shutdown.timeoutSeconds`. |
| `SERVER_DRAIN_DELAY_SECONDS`     | `3`           | Pause between flipping `/readyz` to draining and closing the listener, so the load balancer stops routing here before the door shuts. Zero or positive, same as above. The chart sets it from `shutdown.preDrainWaitSeconds`. |
