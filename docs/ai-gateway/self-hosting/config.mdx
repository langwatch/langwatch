...
The config refresh is conditional: the gateway sends back the version token the control plane stamped on the config it holds, and a key nobody changed is confirmed with a 304 instead of materialized and sent again. Lowering the TTL therefore costs the control plane a revision lookup per key per interval, not a bundle.
