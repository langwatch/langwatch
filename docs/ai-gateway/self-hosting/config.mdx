...
| `SERVER_GRACEFUL_SECONDS`       | `900`               | Graceful-stop window, which begins once `SERVER_DRAIN_DELAY_SECONDS` has elapsed. `/readyz` flips to 503 immediately; the listener then stops accepting, and in-flight work gets this many seconds to finish after it begins shutting down. The window covers request completion, response flushing, and draining the spend spool to the ingest endpoint. The default covers the 14-minute upstream provider timeout, which is the longest a streaming response can legitimately run, plus a minute of margin. It is a ceiling and not a sleep, so an idle gateway still exits immediately. Must be zero or positive: it is time spent waiting, so a negative value is refused at startup. The chart sets it from `shutdown.timeoutSeconds`. |
| `SERVER_DRAIN_DELAY_SECONDS`    | `3`                 | Pause between flipping `/readyz` to draining and closing the listener, so the load balancer stops routing here before the door shuts. Charged on top of `SERVER_GRACEFUL_SECONDS` rather than out of it, so size `terminationGracePeriodSeconds` above the sum of the two. Zero or positive, same as above. The chart sets it from `shutdown.preDrainWaitSeconds`.                                                                                                                                                                                                                                                                                                                                                                           |
...
The gateway is on the hot path of every LLM request, so a brief control-plane outage must not translate into mass authentication rejection. When a cached entry crosses its JWT exp AND the refresh fails for transport reasons (network/timeout/5xx/parse error), the entry's soft expiry is extended by `LW_GATEWAY_AUTH_CACHE_SOFT_BUMP_SECONDS` and the cached bundle continues to serve, up to a hard cap of `(JWT exp + LW_GATEWAY_AUTH_CACHE_HARD_GRACE_SECONDS)`. Auth-class rejections (401/403/404) evict immediately, no grace for known-bad credentials. Setting `LW_GATEWAY_AUTH_CACHE_HARD_GRACE_SECONDS` to a negative value disables stale-while-error entirely (legacy hard-fail behaviour at JWT exp); `0` means unset and takes the default.

The names without the `LW_GATEWAY_AUTH_CACHE_` prefix, and the duration-string forms they replaced (`LW_GATEWAY_AUTH_CACHE_HARD_GRACE=6h`), are not read. The gateway refuses to start when it finds one still set, naming the variable to use instead, rather than booting on the default and leaving the old setting looking applied.

All three are a plain count of seconds, not a duration string: `300`, not `5m`.

| Var                                        | Default      | Purpose                                                                                                                                            |
| ------------------------------------------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LW_GATEWAY_AUTH_CACHE_SOFT_BUMP_SECONDS`  | `300` (5m)   | Per-failure soft-expiry extension during a control-plane outage.                                                                                   |
| `LW_GATEWAY_AUTH_CACHE_HARD_GRACE_SECONDS` | `21600` (6h) | Outage cap measured from JWT exp. A negative value disables stale-while-error.                                                                     |
| `LW_GATEWAY_AUTH_CACHE_CONFIG_TTL_SECONDS` | `60`         | How stale a cached key's config (credentials, base URLs, routing) can get before a background revalidation. A negative value disables the refresh. |

The config refresh is conditional whenever the gateway has a version token to offer: it sends back the token the control plane stamped on the config it holds, and a key that did not change is confirmed with a 304 instead of materialized and sent again. A key that did change still answers 200 with the whole bundle, which is the point of the refresh, so the saving is on the keys that were not edited. Lowering the TTL therefore costs one revision lookup per key per interval, and a full bundle only for the keys edited in it. The exception is a config the gateway holds no token for, which is what a response that arrived without an `ETag` leaves behind: that one is refreshed unconditionally and costs a full bundle every interval, so a proxy that strips `ETag` gives the saving up entirely.
