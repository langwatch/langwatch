...
The config refresh is conditional whenever the gateway has a version token to offer: it sends back the token the control plane stamped on the config it holds, and a key nobody changed is confirmed with a 304 instead of materialized and sent again. Lowering the TTL then costs the control plane a revision lookup per key per interval rather than a bundle. A cached config with no token, which is what a response that arrived without an `ETag` leaves behind, is refreshed unconditionally and does cost a full bundle, so a proxy that strips `ETag` gives up the saving.
