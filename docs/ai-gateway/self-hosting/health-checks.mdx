...
3. **Drain window**: existing in-flight requests continue for up to `shutdown.timeoutSeconds` (default 15). SSE streams continue until the upstream provider closes them or the request finishes naturally.
4. **Process exits 0**.

Match your `terminationGracePeriodSeconds` to `preDrainWaitSeconds + timeoutSeconds` plus a few seconds of slack:

```yaml
terminationGracePeriodSeconds: 30  # chart default: 5 + 15 + 10 slack
```

`preDrainWaitSeconds` exists precisely to give the LB time to notice `/readyz=503` before the listener stops accepting — if you still see a fraction of in-flight requests hit an already-draining replica during rolling deploys, bump `shutdown.preDrainWaitSeconds` (and `terminationGracePeriodSeconds` to match) rather than reaching for a manual `preStop` sleep.
