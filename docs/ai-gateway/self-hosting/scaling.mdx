...
On boot the gateway runs a one-shot auth-cache warm-up against the control plane (`/api/internal/gateway/keys/changes`) before flipping `/startupz` to 200. For tenants with 50K+ virtual keys this takes 2–3 seconds; the chart's `probes.startup.failureThreshold: 30 × periodSeconds: 2 = 60s` window gives plenty of room. The pod is excluded from the load balancer until `/startupz` passes — no traffic hits a half-warmed cache.

The cache is in-process and per-replica. Replicas warm independently; there is no cross-replica L2 cache in the current gateway. (See "Future tunables" for the values.yaml entries that anticipate this.)
