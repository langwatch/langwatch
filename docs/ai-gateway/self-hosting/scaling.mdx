...
The chart ships an HPA with CPU-based scaling plus an opt-in Prometheus custom metric (`infra/charts/gateway/values.yaml: autoscaling.*`):
...
Chart default (`infra/charts/gateway/values.yaml: podDisruptionBudget`):
...
Chart defaults (`infra/charts/gateway/values.yaml: resources`):
