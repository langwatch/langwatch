...
4. **The streaming response was misclassified as non-streaming.** Was the cause for codex 0.122+ on `/v1/responses` until iter-110 — `stream:true` lived past the gateway's 32 KiB peek window and `PeekStream(peek)` returned false. Fixed by widening the peek window for `/v1/responses` to 256 KiB.
