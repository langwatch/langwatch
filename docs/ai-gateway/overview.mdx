...
<Info>**~11 µs of gateway-side overhead at 5k RPS sustained.** Written in Go with chi + in-process JWT verification. The hot path sits in front of every LLM call, it has to be close to free.</Info>

## Try it in 30 seconds

Swap your OpenAI `base_url` + `api_key` and you're done. Don't have a virtual key yet? Two paths:

- **Developer using a coding CLI** (Claude Code, Codex, Cursor, Gemini): run `langwatch login --device` and a personal VK is auto-issued, see [Personal IDE keys](/ai-gateway/governance/personal-keys).
- **Service, app traffic** (server-to-server): mint one in the LangWatch app under **AI Gateway → Virtual Keys**: see the [Quickstart](/ai-gateway/quickstart).
...
See [Quickstart](/ai-gateway/quickstart) for the full five-minute walkthrough, minting a VK, wiring a provider, calling the gateway, inspecting the trace.

Prefer to self-host? See [Self-Hosting → Helm](/ai-gateway/self-hosting/helm). The same image and config shape run in both modes.

## Why an AI Gateway?

The simplest setup is "one app, one provider API key, one language SDK." That stops working the moment a second team, a second app, or a second model provider shows up:
...
- **OpenAI-compatible endpoint** at `/v1/chat/completions`, `/v1/embeddings`, `/v1/models`, etc. Drop in `OPENAI_BASE_URL=https://gateway.langwatch.ai/v1` and `OPENAI_API_KEY=vk-lw-…` and every existing OpenAI-SDK codebase works unchanged.
...
- **Virtual keys**: LangWatch-issued credentials (`vk-lw-<ulid>`) with project/team/org scope. Show-once secrets, peppered-HMAC-SHA256 hashed, rotatable, revokable within 60 seconds.
- **Hierarchical budgets**: enforce spend limits at organisation, team, project, virtual-key, principal, group, or per-end-user scope, with periods from a minute to a whole grant, calendar-aligned or anchored to your own billing date, and soft-warn or hard-block semantics.
...
- **Fallback, when the key asks for it**: per-virtual-key routing is an explicit choice. New keys default to no fallback (a provider failure surfaces to the caller); a key set to fall back walks its eligible chain (e.g. OpenAI → Anthropic → Bedrock) on 5xx, timeout, 429, network error, 404, or circuit-breaker-open. Terminal 4xx client errors like 400 and 401 never trigger fallback.
...
5. **[Budgets](/ai-gateway/budgets)** and **[Billing and spend events](/ai-gateway/billing-events)**: cap spend per scope, and pull the per-request record to rebill your own customers.
6. **[Webhooks](/features/webhooks)**: get budget breaches and the spend stream pushed to your systems instead of polling.
7. **[AI Governance](/ai-governance/overview)**: once the gateway is in place, layer on org-wide controls: anomaly detection, OCSF/SIEM export, per-user workspaces, IngestionSources for non-gateway telemetry.
