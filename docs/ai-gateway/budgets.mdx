...
Gateway operations are debited **after** the response completes, using the provider-reported token counts. Every debit carries a `gateway_request_id` (ULID) and is **idempotent** by that id, so a gateway retry or a redelivered batch never double-bills.
