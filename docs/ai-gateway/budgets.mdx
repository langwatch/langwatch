...
<Note>
The ClickHouse fold runs alongside the primary ledger today to validate the path. The pre-request budget check reads the primary ledger. The CH totals view becomes the authoritative read in a follow-up, at which point the primary-ledger write path retires. Customer-visible behaviour is unchanged — a $1 budget still blocks at $1 of spend during and after the cutover.
</Note>
