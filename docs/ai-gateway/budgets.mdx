...
<Note>
  The ClickHouse ledger is the primary read wherever it is enabled; the Postgres `spentUsd` column
  is the fallback for self-hosted installs without it. A $1 budget blocks at $1 of spend on either.
</Note>
