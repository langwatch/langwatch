...
The **/gateway/budgets** list shows every budget with a utilization bar + hard cap + remaining. Clicking a row opens **/gateway/budgets/[id]** (Lane B iter 18.2, `f85319cc7`) — a detail page mirroring the VK detail page's layout:
...
- **Resolved scope target**: if `scope.kind = VIRTUAL_KEY`, links to the VK detail page. Similar resolution for `ORGANIZATION` / `TEAM` / `PROJECT` / `PRINCIPAL` — each scope kind surfaces the human name of the target rather than just an id.
- **Recent 20 debits**: each row shows timestamp, amount, originating VK (deep-linked to the VK detail page), and the `gateway_request_id` for trace search.

Paired with **/gateway/usage**, which shows a byDay sparkline between the stat tiles and "Top virtual keys" (Lane B iter 18.1, `3cb7a401a`). The sparkline is gated on ≥ 2 data buckets so the 24 h preset doesn't render a single-point chart.
