...
<Note>
  **One audit log, two write shapes.** Gateway-shape rows carry `targetKind`, `targetId`,
  `before`, `after`. Platform-shape rows carry `args`, `metadata`, `error`. The viewer
  renders both with the same row template, computes `source` from the presence of
  `targetKind`, and offers a target-kind filter for narrowing to gateway events only.
</Note>
