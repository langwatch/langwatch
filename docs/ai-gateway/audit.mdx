...
Action codes for gateway mutations (the `GatewayAuditAction` string-literal union in `packages/features/gateway/server/src/ports/gateway-audit.port.ts`):
...
- [AI Governance → Compliance Architecture](/ai-governance/compliance-architecture): how audit events get OCSF-mapped and exported to your SIEM (Splunk, Datadog, Sentinel, Elastic, Sumo).
...
- [Virtual Keys → Rotation](/ai-gateway/virtual-keys#rotation): the operation behind the `gateway.virtual_key.rotated` action code.
