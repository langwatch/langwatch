...
Server-side, the secret is stored `argon2id(secret + pepper)`. A short prefix is stored alongside for grep/lookup.
...
| Guardrails | Attach existing LangWatch evaluators as `pre` / `post` / `stream_chunk` hooks. |
