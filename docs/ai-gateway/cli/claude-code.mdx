...
The LangWatch AI Gateway's `/v1/messages` endpoint is Claude-Code-compatible: tool-call deltas are forwarded byte-for-byte after the first chunk (see the [streaming contract](/ai-gateway/streaming)), `cache_control` blocks are passed through untouched (see [caching passthrough](/ai-gateway/caching-passthrough); Claude Code uses prompt caching aggressively, so it depends on this), and the `/v1/models` endpoint returns Anthropic-model shapes so Claude Code's model picker works.

## Quick path: `langwatch claude` wrapper (recommended for personal IDE keys)

If you've authenticated via `langwatch login --device` ([Personal IDE keys](/ai-gateway/governance/personal-keys)), the simplest invocation is the wrapper. It exec's Claude Code with the right env vars already pointing at your gateway:

```bash
langwatch claude              # interactive
langwatch claude "fix tests"  # non-interactive prompt
```

All Claude Code flags pass through unchanged, `langwatch claude` is byte-for-byte equivalent to the bare `claude` invocation, just with `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN` already set. No env vars to maintain.

## Manual setup: service VK, shared workstation

If you have a **service-virtual-key** for non-personal traffic, or you want every plain `claude` invocation in your shell to route through the gateway without the wrapper:

```bash
export ANTHROPIC_BASE_URL="https://gateway.langwatch.ai"
export ANTHROPIC_AUTH_TOKEN="vk-lw-01HZX..."
claude
```
...
```bash
# .envrc
export ANTHROPIC_AUTH_TOKEN="vk-lw-<project-specific-vk>"
```
...
- Run `claude --print "say hi"` and look at the exit status + the `X-LangWatch-Gateway-Request-Id` header. If the request fails, the gateway emits a diagnostic error envelope (not a provider-opaque one).
...
- `fallback.max_attempts: 2`, one attempt per credential in the chain.
