...
```bash
export LANGWATCH_VK="vk-lw-01HZX..."
opencode
```
...
- **Streaming hangs mid-response**: check `X-LangWatch-Gateway-Request-Id` against the LangWatch trace; gateway emits a terminal `event: error` on mid-stream failures (it won't silently switch providers). See [Streaming](/ai-gateway/streaming).
