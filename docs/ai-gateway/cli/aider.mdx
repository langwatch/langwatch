...
For Anthropic Claude models, Aider uses `litellm` under the hood, and it routes `anthropic/claude-*` model names through its Anthropic handler, which uses `ANTHROPIC_BASE_URL`:

```bash
export ANTHROPIC_API_KEY="vk-lw-01HZX..."
export ANTHROPIC_BASE_URL="https://gateway.langwatch.ai"

aider --model anthropic/claude-sonnet-4-6
```

This hits the gateway's `/v1/messages` endpoint, which is Anthropic-native and preserves `cache_control` blocks byte-for-byte, which Aider's repo-map caching depends on (see below).
