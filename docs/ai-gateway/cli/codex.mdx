...
<Note>Pin to a Bifrost-registered model name. Codex's default `gpt-5.4` (and aliases like `gpt-5-mini` without a date suffix) currently resolve to "provider is required" via the gateway because Bifrost's model registry only matches concrete names. `gpt-4o-mini` works today; `gpt-5` family pins like `gpt-5-2025-08-07` work as Bifrost adds them.</Note>
