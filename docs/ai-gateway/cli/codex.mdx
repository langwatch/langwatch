...
- VK `models_allowed: ["gpt-5-mini", "gpt-5", "gpt-4o", "claude-haiku-4-5-20251001"]`.
  ...
  A chain that spans two providers only fails over for models both of them
  serve. The gateway skips a credential that cannot serve the resolved model
  rather than dialing it, so a request for `gpt-5-mini` never reaches the
  Anthropic slot: that model has one attempt, and an OpenAI outage surfaces to
  Codex. Give a model failover by mapping its name on the routing policy, or by
  pointing a [model tier](/ai-gateway/model-aliases) at each provider's own
  equivalent and having Codex ask for the tier.
