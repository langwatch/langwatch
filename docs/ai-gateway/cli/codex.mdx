...
The Codex CLI speaks the OpenAI Chat Completions and Responses APIs. Both are available from the LangWatch AI Gateway, and Codex works with either once you set the two env vars.

## Quick path: `langwatch codex` wrapper (recommended for personal IDE keys)

If you've authenticated via `langwatch login --device` ([Personal IDE keys](/ai-gateway/governance/personal-keys)), the simplest invocation is the wrapper. It exec's Codex with the right env vars already pointing at your gateway:

```bash
langwatch codex "explain this repo"
```

All Codex flags, subcommands pass through unchanged, `langwatch codex` is byte-for-byte equivalent to bare `codex`, just with `OPENAI_BASE_URL` and `OPENAI_API_KEY` already set.

## Manual setup: service VK, shared workstation

If you have a **service-virtual-key** for non-personal traffic, or you want every plain `codex` invocation to route through the gateway without the wrapper:

```bash
export OPENAI_BASE_URL="https://gateway.langwatch.ai/v1"
export OPENAI_API_KEY="vk-lw-01HZX..."
codex exec "explain this repo"
```

Every Codex call hits the gateway, which applies your VK's policies (budget, guardrails, blocked patterns, fallback) and emits a LangWatch trace.
...
<Note>
  Pin to a Bifrost-registered model name. Codex's default `gpt-5.4` (and aliases like `gpt-5-mini`
  without a date suffix) currently resolve to "provider is required" via the gateway because
  Bifrost's model registry only matches concrete names. `gpt-4o-mini` works today; `gpt-5` family
  pins like `gpt-5-2025-08-07` work as Bifrost adds them.
</Note>
...
- **Anthropic, Bedrock Claude, Vertex Claude** → Claude Code via `/v1/messages` (see [Claude Code](./claude-code))
- **Google Gemini, Vertex Gemini** → gemini-cli via `/v1beta/models/…:generateContent` (see [Gemini CLI](./gemini-cli))
...
OPENAI_API_KEY="vk-lw-..." \
OPENAI_EXTRA_HEADERS='{"X-LangWatch-Cache":"disable"}' \
codex exec "benchmark"
```

<Note>
  OpenAI CLI, SDK support for custom headers varies; the header-injection method above is
  pseudo-code for illustration. Practical override: add `cache.mode: disable` to a dedicated
  "cold-benchmark" VK.
</Note>
...
- VK fallback chain: `openai-primary → anthropic-fallback`.
- VK `models_allowed: ["gpt-5-mini", "gpt-5", "gpt-4o", "claude-haiku-4-5-20251001"]`.

  A chain across two providers only moves over for models that the two
  providers serve. The gateway does not call a credential that cannot serve
  the resolved model. So a request for `gpt-5-mini` never reaches the
  Anthropic slot: that model gets one attempt, and an OpenAI outage shows in
  Codex. To give a model a backup, map its name on the routing policy, or
  point a [model tier](/ai-gateway/model-aliases) to a model from each
  provider and let Codex ask for the tier.
