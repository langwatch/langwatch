...
<Note>
  Pin to a Bifrost-registered model name. Codex's default `gpt-5.4` (and aliases like
  `gpt-5-mini` without a date suffix) currently resolve to "provider is required" via the
  gateway because Bifrost's model registry only matches concrete names. `gpt-4o-mini`
...
<Note>
  ...
  pseudo-code for illustration. Practical override: add `cache.mode: disable` to a
  dedicated "cold-benchmark" VK.
</Note>
