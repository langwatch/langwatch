...
- VK `models_allowed: ["gpt-5-mini", "gpt-5", "gpt-4o", "claude-haiku-4-5-20251001"]`.
  ...
  A chain that spans two providers carries the resolved model name to the next
  one. Anthropic does not serve `gpt-5-mini`, so a failover for that model
  answers 404, which is itself fallback-eligible and moves the request along
  the chain. To make a model reachable on both providers, map its name on the
  routing policy, or point a [model tier](/ai-gateway/model-aliases) at each
  provider's own equivalent and have Codex ask for the tier.
