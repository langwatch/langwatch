...
export GEMINI_API_KEY="vk-lw-01HZX..."
...
<Note>
  Gemini CLI sends the VK secret as `x-goog-api-key` (its native auth header). The gateway's auth
  middleware accepts that header, an `Authorization: Bearer …` header, OR an `x-api-key` header,
  whichever your client sends. All three paths resolve to the same VK.
</Note>
...
export GEMINI_API_KEY="vk-lw-<vertex-bound-vk>"
```
