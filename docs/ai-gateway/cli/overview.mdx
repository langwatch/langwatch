...
The AI Gateway puts coding CLIs behind one governed endpoint. Each engineer keeps their preferred CLI, and the organization controls cost, guardrails, and visibility.
...
**Also check:** [Coding Agents](/coding-agents/overview) for the telemetry side of the same tools: sessions, cost, terminal replay and pull request attribution on your own plan, with no gateway routing.
...
| [opencode](/ai-gateway/cli/opencode) | `langwatch opencode` | `/v1/chat/completions` (or `/v1/messages`) | Per-provider config in `opencode.json` | The wrapper injects both the Anthropic and the OpenAI env pairs. Pin opencode `1.13.x`, `1.14.x` has a known regression with custom providers. |
...
The error message names both accepted auth headers, `Authorization: Bearer vk-lw-...` (OpenAI SDK, Claude Code, Cursor, Aider) AND `x-api-key` (Anthropic SDK). Either works against either endpoint.
