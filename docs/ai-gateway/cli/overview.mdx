...
|---|---|---|---|
| [Claude Code](/ai-gateway/cli/claude-code) | `/v1/messages` | `ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN` | Tool-call deltas preserved byte-for-byte. Pin to dated model name (e.g. `claude-haiku-4-5-20251001`). |
| [Codex CLI](/ai-gateway/cli/codex) | `/v1/responses` | `~/.codex/config.toml` `model_providers`, `wire_api = "responses"` | Codex 0.122+ requires `wire_api = "responses"` (chat dropped). Pin model to a Bifrost-registered name. |
| [Gemini CLI](/ai-gateway/cli/gemini-cli) | `/v1beta/models/{model}:generateContent` | `GOOGLE_GEMINI_BASE_URL`, `GEMINI_API_KEY` | Native Gemini API passthrough. Works with all Gemini models supported by your VK's Vertex/Gemini credential. |
| [opencode](/ai-gateway/cli/opencode) | `/v1/chat/completions` (or `/v1/messages`) | Per-provider config in `opencode.json` | Pin opencode `1.13.x` — `1.14.x` has a known regression with custom providers. |
