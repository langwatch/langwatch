...
<Tip>
  **Debugging a CLI sign-in or fold pipeline?** See [AI Governance → CLI
  Debug](/ai-governance/cli-debug): `langwatch governance status`, `langwatch ingest tail`, the
  `langwatch login` error catalog (incl. `409 no_default_routing_policy`), and OCSF probe.
</Tip>
