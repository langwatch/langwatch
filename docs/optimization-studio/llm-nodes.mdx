---
title: LLM Nodes
description: Use LLM Nodes in Optimization Studio to invoke LLMs from workflows and run controlled evaluations for agent testing.
---
...
You can quickly test an LLM node by using the "Run with manual input" option, which allows you to input test queries and see immediate results. The system shows you both the cost and duration of each execution.
...
You can connect LLM nodes to datasets. Through the entry node, you can:
...
A workflow can contain several LLM nodes. You can build one by:
...
You can also set up custom LLM providers, on top of the default ones, by:
...
Apart from the main providers (OpenAI, Anthropic, Google, Groq, etc) the system also supports any OpenAI-compatible APIs, for example on your own self-hosted Llama.

The LLM Node serves as the foundation for more advanced features like image processing, evaluation, and automatic optimization, which are covered in the next tutorials.
