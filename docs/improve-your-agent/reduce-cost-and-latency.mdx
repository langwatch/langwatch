...
Declare the model in the signature of your [connected agent](/agent-testing/connect-your-agent), where a `Literal` becomes a closed option list in the run dialog:
...
For example, on the six scenarios of the [example agent in the SDK repository](https://github.com/langwatch/langwatch/tree/main/sdks/python/examples/agent-optimization) with three repeats, gpt-5-mini answered a conversation in 28 seconds against 54 seconds for gpt-5 and cost $0.036 against $0.046, but it failed the scenario where the customer gives a wrong order number twice out of three times, because it never asked the question that leads to the right order.
...
- **Context size.** You pay for the system prompt, the tool list and the history on every turn. The input tokens of the first and the last call of a conversation, on the trace, show how much of the bill is context; ask Langy or your coding agent to shorten the prompt or trim the history, and compare.
...
```text
Use the langwatch CLI to run the last test suite I ran against my agent once per value of its model
