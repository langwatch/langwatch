...
2. **A more detailed error message**, which says which parameter was rejected and which values it accepts, so the model can fix the call instead of guessing again.
...
For example, the same tool before and after the first three changes on the list, which put the accepted values in the description, return a more detailed error message and validate at the boundary:
