...
With a scenario suite as the metric, every metric call is one simulated conversation, so an optimizer that plans hundreds of calls costs hundreds of conversations. The table counts those calls on the six-scenario example of [Optimize with DSPy](/improve-your-agent/optimize-with-dspy), next to what each algorithm changes, so you can pick one by what you need changed and what you can afford.
