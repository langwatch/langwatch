...
On this page we show which algorithm to pick for improving your agent, from the guided loop to the DSPy optimizers, when the metric is a scenario suite.

The metric is what sets the cost: every metric call is one simulated conversation, so an optimizer that plans hundreds of calls costs hundreds of conversations. The table counts those calls on the six-scenario example of [Optimize with DSPy](/improve-your-agent/optimize-with-dspy).
...
| GEPA | Rewrites the instructions from text feedback, keeps a Pareto front of candidates | A metric that returns a score and feedback text | Set `max_metric_calls`: 48 took 51 runs, 31 minutes and $0.64 on the example | Yes | The judge's reasoning is the feedback, which is the scenario case |
...
- **GEPA when the metric returns text feedback.** The scenario judge writes why a candidate failed, and GEPA's reflection reads that text and proposes the next candidate from it, without you reviewing each step.
- **MIPROv2 when the metric is a number.** Use MIPROv2 with an evaluator that returns a score and no reasoning, or with a dataset of expected outputs; LangWatch tracks every trial.
...
`langwatch.dspy.init(experiment=..., optimizer=optimizer)` records every trial of a tracked optimizer in **Experiments**, with the score, the instructions of each predictor and the examples it chose. The tracked optimizers are BootstrapFewShot, BootstrapFewShotWithRandomSearch, COPRO, MIPROv2 and GEPA. For any other optimizer, the [custom optimizer](/dspy-visualization/custom-optimizer) page shows how to log a step yourself.
