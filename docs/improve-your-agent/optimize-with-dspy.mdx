...
description: Scores every candidate on full multi-turn conversations with your agent, running the agent improvement loop automatically with DSPy algorithms.
---

Writing a metric for an agent is hard, because a reply has no single expected output to compare against.

Your scenario suite is already that metric: it runs the whole conversation with a simulated user and judges it on the criteria you wrote, so the optimizer runs the [improvement loop](/improve-your-agent/overview) one candidate after another, against the same suite you use as the quality gate.
