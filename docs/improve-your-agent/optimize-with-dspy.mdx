...
description: Use your scenario suite as the metric of a DSPy optimizer, so it scores every candidate on full multi-turn conversations with your agent, on the cases that matter to you, instead of on a metric you have to invent, and runs the improvement loop on its own.
---

Writing a metric for an agent is the hard part of using an optimizer, because a reply has no single expected output to compare against, and a score on one reply says little about a whole conversation.

Your scenario suite is already that metric: it runs the full conversation with a simulated user and judges it on the criteria you wrote, so the optimizer improves the agent against the same suite you use as the quality gate in the [improvement loop](/improve-your-agent/overview), one candidate after another, without you writing each hypothesis.

To run a DSPy optimizer you give it a program and a metric. For an agent, the program is a `dspy.ReAct` module, and because the instructions of that module contain every tool description, an optimizer that rewrites the instructions rewrites the tool descriptions too.

The metric runs one scenario against the candidate program and scores the verdict, with a penalty for every step over the budget, where a step is one reply or one tool call. GEPA also reads the judge's reasoning as text feedback and writes the next candidate from it.
...
Build the agent as a `dspy.ReAct` over the tools, and write a scenario adapter that passes the conversation to it as `dspy.History` plus the last user message and returns the answer:
...
Write one `dspy.Example` per scenario, with the situation, the criteria and a step budget, where a rejected tool call that the agent retries counts as an extra step.
...
The optimizer calls the program on every example and then the metric on what the program returned. So wrap the whole scenario in the program: it runs the conversation with the candidate agent inside it and returns the verdict, the steps and the judge's reasoning, and the metric turns those into a score:
...
The metric scores a failed scenario zero and a passed scenario one minus half a point per budget of extra steps, so the pass gate comes first and the step budget is the objective under it. When a tool rejects its arguments during the conversation, the example adds one line to the feedback that says so, which is what GEPA needs to rewrite that tool's description.
...
Neither optimizer saw the tool code, but both saw the rejected calls in the traces, and both wrote the accepted values into the `react` instructions. MIPROv2, from the proposer's candidates:
...
That is the same tool description fix as in [Fix tool calls](/improve-your-agent/fix-tool-calls), found by search. On both optimizer runs the damaged blender return went from failed to passed and from 5 steps to 4; the hand-written fix on that page, which also validates the arguments and rewrites the error message, got the same return down to 3.
...
- The optimizer changes text, meaning the instructions and the tool descriptions. Changing a tool's schema, adding a missing tool or capping the harness is still a code change: see [Fix tool calls](/improve-your-agent/fix-tool-calls).
- Scenarios are not deterministic: the simulated user words the situation differently on every run, so a candidate can pass once and fail once. Evaluate the baseline and the final program twice before you compare them, and use `--repeat` on the platform for the same reason.
