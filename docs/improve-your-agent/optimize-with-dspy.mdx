...
The optimizer never saw the tool code; it only saw the rejected calls in the traces, and from those it wrote this into the `react` instructions:
...
That is the same tool description fix as in [Fix tool calls](/improve-your-agent/fix-tool-calls), found by search instead of by reading the trace yourself. The damaged blender return went from 5 steps to 4 and from failed to passed. Two scenarios did not follow: the one where the customer asks for a recipe failed before and after, because the agent answers the recipe instead of saying it only helps with orders and no candidate instruction changed that, and the angry customer passed before and failed after. The best trial's 83% also did not repeat on the final pass, which is the non-determinism described in the Limits below.
