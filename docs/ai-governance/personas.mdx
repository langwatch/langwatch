...
| **Project-only user** (LLMOps customer, no personal-workspace activity) | `TeamUser` membership exists with `Team.isPersonal=false` AND no traces in `Project.isPersonal` | `/<project>/traces`, the existing LLM Ops Trace Explorer | Existing LLMOps customer story stays unchanged. First paint is the Trace Explorer, not /me. |
