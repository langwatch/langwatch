...
- **A personal CLI key follows its session**: a personal key the CLI mints stores the session's login key as `parentApiKeyId`. Logout, a devices-tab revoke, a re-login from the same device and session expiry all revoke that login key, and every key parented to it goes with it under cause `session`, `expired` or `offboarded`. A mint from a revoked session answers 401.
...
- the mint route returns a one-time `ik-lw-…` token and thereafter exposes only the prefix
- a mint from a CLI session stores that session's login key as `parentApiKeyId`, and a mint from a session whose login key was revoked answers 401
