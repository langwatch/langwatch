...
- **A CLI key follows its session**: a key the CLI mints stores the session's login key as `parentApiKeyId`. Logout, a devices-tab revoke, a re-login from the same device and session expiry all revoke that login key, and every key parented to it goes with it under cause `session`, `expired` or `offboarded`. A mint from a revoked session answers 401. `install` and the MCP mint accept only source types a published template names; a tool the CLI wraps answers `ingestion_key_source_not_allowed`.
