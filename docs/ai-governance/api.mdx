...
**Cross-surface audit-uniformity regression** — `langwatch/ee/governance/services/__tests__/auditSurface.crossSurface.integration.test.ts`. Invokes `createOrgTemplate` via tRPC, Hono, and MCP service-direct in one test (Lane B-8 at `d96cd4300`); asserts the three audit rows have **identical** payload shape — same `action`, same `targetKind`, same `organizationId`, same metadata-key-set — with only `metadata.surface` varying (`"trpc"` / `"hono"` / `"mcp"`). Slug regex format identical, no default-fallback leakage. CLI is a placeholder anchor in this file; lands as a 4th case once Lane-B's scaffold ships. Pairs with the MCP audit-uniform regression above to pin the umbrella spec's `@audit-uniform` invariant exhaustively.
