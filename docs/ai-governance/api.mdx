...
- **A CLI key follows its session**: a key the CLI mints stores the session's login key as `parentApiKeyId`. Logout, a revoke from the devices tab, a re-login from the same device and session expiry all revoke the login key, and every key parented to it is revoked with cause `session` (or `expired`). A mint from a session whose login key is already revoked answers 401. `install` and the MCP mint have no session and accept only source types a published template names; a tool the CLI wraps answers `ingestion_key_source_not_allowed` there.
