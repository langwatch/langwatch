...
Governance routes register through the same `describeRoute` + `hono-openapi` pipeline as the rest of the public REST API. The canonical OpenAPI spec lives at `langwatch/src/app/api/openapiLangWatch.json`, and the generated client layers regenerate from it.
