...
The wire shape — request bodies, response envelopes, status-code mapping, and surface attribution — is locked by an integration test that runs against real Postgres + the real Hono pipeline (no service mocks):

`langwatch/src/app/api/governance/__tests__/governance-rest-api.integration.test.ts`

14 scenarios (as of Sergey's Lane B-4 commit at `1839d9f54`) cover ingestion-templates' full verb set, the 401 / 400 / 403 / 404 envelopes, and a wire-level audit-uniform assertion that `metadata.surface === "hono"` for state-changing calls. Downstream lanes (CLI, MCP) mirror this contract verbatim — if any lane drifts the snake_case body/response shape, RBAC ceiling, or status-code mapping, this test catches it.
...
Use this file as the authoritative reference when implementing a CLI command or MCP tool against a governance route — the request/response shape that lives there is exactly what the route accepts and returns.
