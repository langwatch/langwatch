...
<Note>**TBD-IMPL — surface attribution threading**: the current ingestion-templates routes at `0bb951160` emit audit rows from the service layer (correct), but the per-surface `surface` field is threaded through later (per Sergey's commit body: "surface attribution... is threaded through later once tRPC + CLI + MCP are all online"). Until that lands, all rows currently emit without an explicit surface tag — they'll backfill to the four-value enum once the threading PR lands.</Note>
