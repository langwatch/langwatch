...
![Claude Code tile expanded, setup command + walkthrough](/images/ai-governance/portal/tile-claude-expanded.png)
...
│   2. Ask whether to use your own subscription or an API key         │
│   3. exec `claude` with the right env vars pre-injected            │
│                                                                     │
│  No wrapper: wire it once with                                     │
...
3. The CLI asks how to run the tool. Pick **Using an API key** for the
   gateway, or your own subscription to send telemetry only. The CLI
   remembers the answer.
4. On the gateway answer, the CLI receives a personal virtual key,
   persists it at `~/.langwatch/config.json` (mode 0600), and exec's the
   underlying tool (Claude Code in this case). On the subscription
   answer, no virtual key is issued.
...
returns a `409 no_default_routing_policy` error and points to
...
![Anthropic tile expanded, VK creation form before issuance](/images/ai-governance/portal/tile-anthropic-form.png)
...
![Anthropic tile issued, masked secret + reveal/copy + base URL](/images/ai-governance/portal/tile-anthropic-issued.png)
...
`/gateway/routing-policies`. Once a provider is bound,
...
an internal RAG system, or anything that is not a one-developer experiment,
...
![Copilot Studio tile expanded, admin markdown body + external CTA link](/images/ai-governance/portal/tile-copilot-studio.png)
...
`/governance/inventory?tab=catalog`.
