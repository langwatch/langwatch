...
Org admins curate the AI Tools Portal from
**Governance → Inventory → Catalog**
(`/governance/inventory?tab=catalog`). Anything you publish here lands on
...
<Info>
  **Pairs with:** [Portal overview](/ai-governance/personal-portal/overview) for the
  user-side tile grid, and [End-user tile
  interactions](/ai-governance/personal-portal/end-user) for what each tile expands into.
</Info>

<Note>
  **Permission required**: `aiTools:manage` (granted to ADMIN by default; delegable via
  the `CustomRolePermissions` JSON column).
</Note>
