...
<Info>
  **Pairs with:** [No-spy mode](/ai-governance/no-spy-mode), the organization-wide posture built
  from this policy for enterprises that must not store employee conversational content, and
  [Compliance architecture](/ai-governance/compliance-architecture) for how privacy fits the broader
  audit story.
</Info>
...
export, or breach can surface it. The cost is that no one can debug
with it, and it cannot be undone for data that already arrived
dropped.

Choose Restrict when the question is _who may read this_. Choose Drop
when the requirement is _this must not exist in storage_.
...
| `hr-helpdesk`, assigned to the HR department | Captured | Restricted to the `HR` group | The department rule sets output; no rule sets input.                                                                                        |
...
the surrounding text intact. How detection decides what is a
credential is covered on
[Secrets redaction](/coding-agents/secrets-redaction).
...
<Warning>
  With secrets redaction off, any credential pasted into a prompt or returned by a tool is stored in
  the clear. Turn it off only when you are certain no real credentials transit the projects in
  scope.
</Warning>
...
| ----------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------- |
| **Disabled**            | None                                                                   | No scanning at all                                         |
...
<Note>
  Credit card numbers are validated before redaction, so a random 16-digit order id in your traces
  is left intact.
</Note>
...
Privacy is default-on: secrets and essential PII are scrubbed before
