...
<Info>
  **Pairs with:** [No-spy mode](/ai-governance/no-spy-mode), the organization-wide posture
  built from this policy for enterprises that must not store employee conversational
  content, and [Compliance architecture](/ai-governance/compliance-architecture) for how
  privacy fits the broader audit story.
</Info>
...
<Warning>
  With secrets redaction off, any credential pasted into a prompt or returned by a tool is
  stored in the clear. Turn it off only when you are certain no real credentials transit
...
<Note>
  Credit card numbers are validated before redaction, so a random 16-digit order id in
  your traces is left intact.
</Note>
