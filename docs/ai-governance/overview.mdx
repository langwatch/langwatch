...
The sign-in flow starts in the terminal, moves through the browser for one-click approval, comes back to the CLI ready to wrap any supported tool.

<Frame caption="Running a wrapped tool before signing in is a hard fail with one clear next step. The wrapper exits 1, prints 'Not logged in. Run `langwatch login --device` first.', spawns no child process, leaves no half-broken state.">
...
<Frame caption="If the user hits a budget cap, the wrapper surfaces it inline, names the cap, the spend so far, and the `langwatch request-increase` command to ping the admin. No silent failures.">
