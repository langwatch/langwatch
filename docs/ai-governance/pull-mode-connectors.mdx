...
<Info>
  **Pairs with:** [Ingestion sources](/ai-governance/ingestion-sources/index) (the broader
  substrate that pull-mode lands events into) and [CLI
  debugging](/ai-governance/cli-debug) (`langwatch ingest tail` shows pulled events
  alongside pushed ones).
</Info>

<Warning>
  **Available on Enterprise plans.** Pull-mode connectors ship as part of the Enterprise
  multi-source governance ingestion stack. Apache 2.0 deployments use single-source
  `otel_generic` push only. See [Open-core
  licensing](/ai-governance/overview#open-core-licensing).
</Warning>
