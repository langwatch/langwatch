...
<Info>**Pairs with:** [AI Gateway → Audit](/ai-gateway/audit). The gateway emits the spans; governance projections derive OCSF-mapped events from them for SIEM export, and can re-derive them from the event log at any time.</Info>
...
```
   ...
   /api/otel/v1/traces     /api/ingest/otel/:sourceId    /api/ingest/webhook/:sourceId
     ...
     │  (projection)   │ │    _events      │ │  Log detail pane   │
     │                 │ │  (projection)   │ │  Existing UI       │
     └─────────────────┘ └─────────────────┘ └────────────────────┘
              │                  │
              ▼                  ▼
     ┌───────────────────┐ ┌──────────────────────┐
     │ Anomaly detection │ │ OCSF read API        │
