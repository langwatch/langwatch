...
<Info>
  **Pairs with:** [Ingestion Sources index](/ai-governance/ingestion-sources/index). Webhook
  envelope shape is shared with [`s3_custom`](/ai-governance/ingestion-sources/s3-custom) callback
  mode.
</Info>

Workato pushes job-completed webhooks. The receiver is wired and persists each delivery as a single envelope event; the parser that turns Workato's recipe, job, connection events into precise Actor, Action, Target fields is the next step on the [adapter roadmap](/ai-governance/ingestion-sources/index#adapter-roadmap).

## Currently implemented

- **Receiver:** `POST /api/v1/ingest/webhook/:sourceId`.
- **Auth:** `Authorization: Bearer lw_is_<secret>`.
- **Body:** any JSON payload (Workato's exact webhook envelope).
- **Persistence:** one governance event per delivery, with `eventType: agent.action`, `actor`/`target` empty, and the verbatim payload in `rawPayload`. The CLI tail shows one row per Workato webhook hit and the per-source detail page lets you expand the row to see the raw envelope.
...
| Field         | Value                                                    |
| ------------- | -------------------------------------------------------- |
| Webhook URL   | `https://<your-langwatch>/api/v1/ingest/webhook/<sourceId>` |
...
So the CLI tail row looks like:
