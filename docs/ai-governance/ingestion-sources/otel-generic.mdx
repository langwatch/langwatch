...
<Info>
  **Pairs with:** [LangWatch OpenTelemetry guide](/integration/opentelemetry/guide). Same
  wire shape; this page uses the governance ingest endpoint instead of a project-scoped
  one, so events get `langwatch.origin.kind = "ingestion_source"` instead of `app`.
</Info>
...
<Note>
  **Not sending your own app's traces here.** This endpoint is for *third-party* OTel
  feeds your org wants in the governance audit trail. For your own application's LLM
  traces, use [`/api/otel/v1/traces`](/integration/quick-start) with your project API key,
  see [Choosing the right OTel endpoint](/observability/trace-vs-activity-ingestion) for
  the full comparison.
</Note>
