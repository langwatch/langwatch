...
<Info>
  **Pairs with:** [LangWatch OpenTelemetry guide](/integration/opentelemetry/guide). Same wire
  shape; this page uses the governance ingest endpoint instead of a project-scoped one, so events
  get `langwatch.origin.kind = "ingestion_source"` instead of `app`.
</Info>
...
<Note>
  **Not sending your own app's traces here.** This endpoint is for *third-party* OTel feeds your org
  wants in the governance audit trail. For your own application's LLM traces, use
  [`/api/otel/v1/traces`](/integration/quick-start) with your project API key, see [Choosing the
  right OTel endpoint](/observability/trace-vs-activity-ingestion) for the full comparison.
</Note>

## Currently implemented

- **Receiver:** `POST /api/v1/ingest/otel/:sourceId` (also accepts `claude_cowork` sources, see [Anthropic Cowork](/ai-gateway/governance/ingestion-sources/claude-cowork)).
...
| Field         | Value                                                                  |
| ------------- | ---------------------------------------------------------------------- |
| OTLP URL      | `https://<your-langwatch>/api/v1/ingest/otel/<sourceId>`                  |
...
exporters:
  otlphttp/langwatch:
    endpoint: https://<your-langwatch>/api/v1/ingest/otel/<sourceId>
    headers:
