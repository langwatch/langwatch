...
4. _(Optional)_ **API permissions** → Microsoft Graph → Application permissions → `Directory.Read.All` → Grant admin consent. This lets LangWatch show department and job title beside each conversation. Skip it and conversations still arrive, without the org context.
...
<Note>
  Service Reader is the tightest stock role that grants read access to both `bot` and
  `conversationtranscripts` at organisation-wide reach. The similarly named Bot Transcript Viewer
  only reads records the account created itself, which for a service account is none. See the
  [security roles
  reference](https://learn.microsoft.com/en-us/power-platform/admin/security-roles-privileges) for
  details.
</Note>
