...
<Info>
  **Pairs with:** [AI Gateway → Anthropic provider](/ai-gateway/providers/anthropic). Direct
  Anthropic API traffic flows through the gateway data plane; Cowork tool-use telemetry is a
  separate OTLP feed routed through this ingestion source.
</Info>
...
- **Receiver:** `POST /api/v1/ingest/otel/:sourceId`, same endpoint as [`otel_generic`](/ai-gateway/governance/ingestion-sources/otel-generic).
