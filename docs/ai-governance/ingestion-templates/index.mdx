...
write-only, ingest-only role: it can create traces and nothing else, so it's safe to
spray into an agent's environment. Full-access keys carry the `sk-lw-…` prefix instead,
so you can tell the two apart at a glance.
...
in that flow. The only credential LangWatch holds is the ingestion key (`ik-lw-…`) we
