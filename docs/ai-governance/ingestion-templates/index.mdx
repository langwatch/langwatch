...
- **User side:** a binding token (`ik-lw-…`) scoped to your personal project. You paste
...
- **Binding token**: `ik-lw-…` issued one-time, with a Show/Hide toggle. Copy it
  while it's visible; after you dismiss the drawer it's masked to its first 9
  characters.
...
When your tool emits an OTLP payload with `Authorization: Bearer ik-lw-<token>`, the
receiver:
...
- **Token = scope.** The `ik-lw-*` binding token authoritatively determines which
...
in that flow. The only credential LangWatch holds is the binding token (`ik-lw-…`) we
