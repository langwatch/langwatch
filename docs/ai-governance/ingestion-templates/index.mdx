...
- `langwatch.user.id`, `langwatch.project.id`, `langwatch.template.id`,
  `langwatch.ingest_key_id`, `langwatch.source = "claude_code"`, all
...
6. **Post-auth re-stamps** the closed `protectedTemplateAttributeKeys` (and, for
   ...
   receiver-authoritative, including `langwatch.ingest_key_id` and
...
- `langwatch.ingest_key_id`, receiver-stamped provenance (the ingestion key
