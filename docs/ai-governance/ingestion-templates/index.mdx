...
tokens, and model populated automatically. An ingestion key is one
[API key](/platform/api-keys) row with an `ik-lw-…` prefix, scoped to one project with a
write-only, ingest-only role: it can create traces and nothing else. That is a much
smaller blast radius than a full project key (`sk-lw-…`), which can read and change your
project data, so the prefix tells you which one you are holding at a glance. An ingestion
key is still a secret: the traces it writes carry your prompts and your tool content, and
anyone who holds the key can write into your project. Protect it the same way you protect
...
canonical traces from the gateway response.
...
- **Admin/platform side:** a curated catalog row (slug, source identifier, credential
  ...
  OttlEditor at `/governance/inventory?tab=catalog → Ingestion Templates → Clone to
...
- **User side:** an ingestion key (`ik-lw-…`) scoped to your personal project with a
...
The credential determines the scope, and payload claims about attribution are advisory.
The receiver attribution path is identical to gateway-VK ingest; the source-shape OTTL is
org-customizable for admins who need it.

---

## Worked example: connect Claude Code on Anthropic 20x

By the end of this flow you have a Claude Code trace visible at `/me/traces` with
`gen_ai.usage.*`, `gen_ai.response.model`, and `langwatch.cost.usd` populated.
...
**Connect Claude Code, auto-shaped**, with the subcopy _"Traces normalized into
gen_ai.\* canonical. Cost/tokens/model populated automatically by the receiver."_
...
- **Ingestion key**: `ik-lw-…` issued one-time, with a Show/Hide toggle. Copy it
...
- **Snippet preview**: pre-wired environment variables for Claude Code:
  ...
  export OTEL_EXPORTER_OTLP_HEADERS="Authorization=Bearer ik-lw-<your-key>"
  ```
...
<Note>
  The key is shown one-time. If you lose it, **rotate** the ingestion key from the tile to mint a
  new one. The previous key is revoked immediately when you rotate (hard-cut v1).
</Note>
...
- `langwatch.user.id`, `langwatch.project.id`, `langwatch.template.id`,
  `langwatch.api_key.id`, `langwatch.source = "claude_code"`, all
  receiver-stamped post-auth (authoritative attribution + template provenance), with
...
- `ottlRules`, an optional OTTL transform applied to the span before the receiver's
  ...
  OTTL via the OttlEditor** in `/governance/inventory?tab=catalog → Ingestion
...
When your tool emits an OTLP payload with `Authorization: Bearer ik-lw-<key>`, the
receiver:

1. **Prefix-discriminates** the bearer, `ik-lw-*` → ApiKey verification path.
...
Two primitives stop a template from raising its own reach or attributing payloads to
another principal: the ingest-only ceiling at step 3 and the principal-field guard at
step 6. Key verification at step 2 and the tenant binding at step 4 still apply.
...
- `langwatch.api_key.id`, receiver-written provenance (the id of the API key
  that landed the trace, never the key material). Rewritten from the
  authenticated identity on every authenticated OTLP request, so a payload
  cannot set it. Redaction exempts this exact name from the secret-name
  deny-list on the strength of that guarantee, and still scrubs real key
  material under it by shape.
...
- **Credential = scope.** The `ik-lw-*` ingestion key authoritatively determines which
...
template via the OttlEditor at `/governance/inventory?tab=catalog → Ingestion
...
A tile flips to green-checked only after the template is verified end to end on a real
install: an admin publishes it via `/governance/inventory?tab=catalog`, a user installs it
via `/me`, the upstream tool emits a real trace, and the resulting attribution holds up
under cross-user isolation and forge attempts against both protected lists.
...
in that flow. For this path the credential LangWatch holds is the ingestion key (`ik-lw-…`)
we mint for you, which is scoped to your personal project and write-only. Device approval
mints no virtual key. If you later send traffic through the AI Gateway, that first call
issues a personal virtual key (`vk-lw-…`) as well.
