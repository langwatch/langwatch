...
**where to send the OTLP** and **which template the user is binding** so the trace gets
the right `langwatch.source` provenance and lands in the right personal project.

That's an Ingestion Template:

- **Admin/platform side:** a curated catalog row (slug, source identifier, credential
  schema, optional `ottlRules`). v1 platform-published rows ship empty `ottlRules` —
  Claude Code, Cursor, and Claude cowork all emit OpenTelemetry `gen_ai.*` canonical
  spans natively, so no transform is needed. The OTTL slot is reserved for a future
  admin-authored OTTL surface.
- **User side:** a binding token (`lwub_…`) scoped to your personal project. You paste
  that token into your tool's OTLP exporter; the tool ships, LangWatch attributes.

Credential is scope. Payload is advisory. The receiver attribution path is identical to
gateway-VK ingest — only the source-shape parsing (a future v2 surface) differs.
...
> **Before you start:** you need a LangWatch account. If you're brand new, sign in at
> [`/auth/signin`](/auth/signin) — first-time sign-in completes onboarding and lands you on
> your **personal workspace** (`/me`). On self-hosted instances the same path works against
> your own deployment's host.
...
- `langwatch.user.id`, `langwatch.project.id`, `langwatch.template.id`,
  ...
  receiver-stamped post-auth (authoritative attribution + template provenance).
...
- `ottlRules` — an optional OTTL transform applied to the span before the receiver's
  authoritative re-stamp pass. **v1 platform templates ship empty `ottlRules`** because
  Claude Code, Cursor, and Claude cowork all emit OpenTelemetry `gen_ai.*` canonical
  spans natively — there is nothing to reshape. The slot is reserved for a future
  iteration when admin OTTL authoring lands and admins can curate org-specific
  transforms.
- `organizationId` — `NULL` for platform-published templates (visible everywhere); set
  ...
  templates lands in a future iteration alongside `ottlRules` editing.
...
4. **Sets `tenantId`** to `binding.personalProjectId` — token-as-scope. This is
   authoritative.
5. **Applies `template.ottlRules`** under a snapshot+restore principal-field guard.
   In v1 platform templates ship empty rules so this step is a no-op; the guard
   infrastructure exists to wrap admin-authored OTTL when it lands.
6. **Post-auth re-stamps** the 19 closed `protectedTemplateAttributeKeys` as receiver-
   authoritative. If a future template's OTTL tried to write any of those, the original
   value is restored and an audit row `gateway.template_ottl_protected_field_attempt`
   fires. v1 templates have no OTTL, so the re-stamp is purely a write of the
   receiver-known attribution + provenance keys.
...
These would become legitimately writable by template OTTL when admin-authored OTTL
lands in a future iteration — admins authoring OTTL can reshape upstream
provider-specific attrs into the canonical `gen_ai.*` keys. Locking them would defeat
that future feature. **v1 templates ship empty `ottlRules`** so the protection list
is essentially dormant in v1; it activates the moment any non-empty OTTL runs through
the guard.

See [Security model recap](#security-model-recap) below for the v1-vs-future trust
...
### v1 trust boundary: empty templates, upstream-emitted canonical

v1 ships **only platform-team-authored templates with empty `ottlRules`**. Cost / tokens /
model values come directly from the upstream tool's OTel exporter (Claude Code, Cursor,
and Claude cowork all emit `gen_ai.*` canonical natively), and the receiver derives
`langwatch.cost.usd` from those upstream-emitted values via `canonicalCostExtractor`.

That means v1 cost / tokens / model integrity rides on **upstream-tool correctness**, not
on any LangWatch-side transform. We have no template OTTL writing those fields in v1; we
trust Claude Code, Cursor, and Claude cowork to populate `gen_ai.usage.*` correctly per
the OTel semantic-conventions spec.
...
`scripts/dogfood/emit-otlp.sh`, including forge-attempt regressions against the 19-key
protected list — and (b) a real-user track — admin publishes via
...
When admin OTTL authoring lands in a future iteration, the trust boundary expands: an
admin-authored OTTL transform that miscomputes cost / tokens / model would affect every
trace through that template. The 19-key protected list stays as-is for platform-authored
OTTL; we may layer a **tier of trust** that adds cost / tokens / model to the protected
set for org-authored OTTL specifically. Cross-org cost integrity is structurally protected
by the attribution-keys re-stamp regardless of the tier.
