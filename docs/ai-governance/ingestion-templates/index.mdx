...
- `langwatch.user.id`, `langwatch.project.id`, `langwatch.template.id`,
  `langwatch.reserved.ingest_key_id`, `langwatch.source = "claude_code"`, all
...
6. **Post-auth re-stamps** the closed `protectedTemplateAttributeKeys` (and, for
   ...
   receiver-authoritative, including `langwatch.reserved.ingest_key_id` and
...
- `langwatch.reserved.ingest_key_id`, receiver-stamped provenance (the ingestion key
  that landed the trace). Lives under the reserved namespace rather than
  `langwatch.api_key.id` so the redaction pipeline's secret-name deny-list never has
  to special-case it.
