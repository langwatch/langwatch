...
- **Admin/platform side:** a curated catalog row (slug, source identifier, credential
  ...
  OttlEditor at `/governance/tool-catalog → Ingestion Templates → Clone to
...
- `ottlRules`, an optional OTTL transform applied to the span before the receiver's
  ...
  OTTL via the OttlEditor** in `/governance/tool-catalog → Ingestion
...
template via the OttlEditor at `/governance/tool-catalog → Ingestion
...
install: an admin publishes it via `/governance/tool-catalog`, a user installs it
