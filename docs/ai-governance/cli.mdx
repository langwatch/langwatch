...
<Info>
  **Pairs with:** [Governance REST API](/ai-governance/api) (the CLI is a thin shell over it; both
  call into the same service-layer functions) and [Governance MCP server](/ai-governance/mcp) (same
  surface for agent invocation). Distinct from [`langwatch ingest …`](/ai-governance/cli-debug):
  that namespace is for debugging ingestion sources; `langwatch governance …` is for managing every
  governance resource.
</Info>
...
# from the CLI. Mirrors the public REST surface at /api/v1/governance/*, every
...
The actual help-text strings + command tree land in `sdks/typescript/src/cli/commands/governance/` and `sdks/typescript/src/cli/index.ts` (Lane-B at `ed51b0ea1`).
...
The first preview ships with the two governance resources wired through Hono REST + service-layer + repository (Ask B-1 through B-7). Additional resources fold into the same `langwatch governance <resource> <verb>` namespace as the umbrella spec's coverage extends.
...
| `ingestion-keys`      | `list` `mint` `rotate` `revoke` (project-scoped ingest-only ApiKeys, `ik-lw-` token returned once at mint) | `organization:view`                               |

Resources expected to fold into the same namespace as their REST surfaces ship: `anomaly-rules`, `audit-log`, `gateway-budgets`, `ingestion-sources`, `members`, `invites`, `role-bindings`, `ai-tool-entries`, `sessions`, `virtual-keys`. The CLI is built so adding a new resource is a `commands/governance/<resource>.ts` file plus a `governanceCmd.command(...)` wiring in `sdks/typescript/src/cli/index.ts`.

## Audit emission

Every state-changing CLI invocation emits an audit row with `AuditLog.metadata.surface = "cli"`. The CLI is a REST shell: it calls `/api/governance/*` and sends the `X-LangWatch-Surface: cli` request header.

The Hono backend's `resolveSurfaceFromRequest` helper reads the header, validates it against the `GovernanceCallSurface` enum, which rejects in-process spoofing of `trpc` and `mcp`, and threads the value into the service-layer `surface` parameter added at `fc6d54100`. The payload shape is identical to a Hono REST audit row apart from the surface field.

The header-validating Hono helper + the 7 mutation call-sites switching from hardcoded `'hono'` to `resolveSurfaceFromRequest(req)` ship in Lane-B's CLI scaffold at `ed51b0ea1`. The `X-LangWatch-Surface: cli` constant lives at `sdks/typescript/src/cli/utils/governance/surface.ts`.
...
The end state is that an agent can fully configure governance for a fresh org by chaining CLI commands. The v1 preview ships the IngestionTemplate authoring + ingestion-key mint slice; budgets, anomaly-rules, audit-log fold in as their CLI surfaces ship.
...
#    (POST /api/v1/governance/ingestion-templates/clone, auto-derives org from the apiKey's project)
...
- [Access Control (RBAC)](/platform/rbac): RBAC scopes the CLI enforces
