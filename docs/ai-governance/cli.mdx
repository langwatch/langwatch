...
The governance CLI ships in v1 behind a preview flag — `LANGWATCH_GOVERNANCE_PREVIEW=1`. Without the flag set, the `governance` subcommand is hidden from `--help` and returns "unknown command" if invoked. This lets the surface evolve through early customer adoption without locking a contract that's still being shaped. Once the corresponding `release_ui_ai_governance_enabled` PostHog flag flips for an org, the CLI gate flips alongside.

```bash
LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance --help
```
...
LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance ingestion-templates --help
# Verbs:
...
```bash
LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance ingestion-templates list
# Default: human-readable table

LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance ingestion-templates list --json
# One JSON object per record on stdout, suitable for piping through jq:
LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance ingestion-templates list --json | jq '.[] | select(.source_type=="claude_code")'
```
...
```bash
LANGWATCH_GOVERNANCE_PREVIEW=1 langwatch governance audit-log query --filter 'surface=cli' --since '24h ago'
```
