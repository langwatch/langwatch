...
<Info>
  **Pairs with:** [Overview](/ai-governance/overview) for the bigger picture (license tiers, two
  personas, mermaid architecture) and [Control plane & integration
...
1. Navigate to **Governance → Inventory → Sources tab** (`/governance/inventory?tab=sources`).
...
The [AI Tools Portal](/ai-governance/personal-portal/overview) is the card grid every developer sees on `/me`. A fresh org has an empty catalog; the [admin editor](/ai-governance/personal-portal/admin-catalog) at `/governance/inventory?tab=catalog` shows you a one-click **Import starter pack** CTA on first visit:

1. Navigate to **Governance → Inventory → Catalog tab** (`/governance/inventory?tab=catalog`).
