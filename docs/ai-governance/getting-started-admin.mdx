...
You are an org admin on a fresh LangWatch deployment. By the end of this page you will have:
...
If you are a developer trying to _use_ the org as a developer (not configure it), jump to the [developer walkthrough](/ai-governance/getting-started-developer) instead.

<Info>
  **Pairs with:** [Overview](/ai-governance/overview) for the bigger picture (license
  tiers, two personas, mermaid architecture) and [Control plane & integration
  tiers](/ai-governance/control-plane) for the navigation map across surfaces.
</Info>
...
Ask your admin to publish a default routing policy at Gateway → Routing Policies.
```

Publish at least one policy _before_ opening CLI sign-in to your team:

1. Navigate to **Gateway → Routing Policies** (`/gateway/routing-policies`).
2. Click **+ New routing policy**.
3. Pick a recommended starter shape: "Anthropic primary, OpenAI fallback, prompt caching on" works on day one.
...
1. Navigate to **Governance → Ingestion Sources** (`/governance/ingestion-sources`).
2. Click **+ New source**, pick **Generic OTLP (`otel_generic`)**, and fill the form.
...
The source flips to `active` on the first received event. The full per-platform setup (Cowork, Workato, S3, Copilot Studio, OpenAI/Anthropic compliance APIs) lives in [Ingestion Sources](/ai-governance/ingestion-sources/index): these source types require an Enterprise plan.
...
The [AI Tools Portal](/ai-governance/personal-portal/overview) is the card grid every developer sees on `/me`. A fresh org has an empty catalog; the [admin editor](/ai-governance/personal-portal/admin-catalog) at `/governance/tool-catalog` shows you a one-click **Import starter pack** CTA on first visit:

1. Navigate to **Governance → Tool Catalog** (`/governance/tool-catalog`).
...
2. Click **+ Add members**.
3. Type their email, pick a role (`Member` by default, `Admin` if they should help configure the org), and pick which teams they belong to.
...
1. Navigate to **Gateway → Budgets** (`/gateway/budgets`).
2. Click **+ New budget**.
...
Budgets enforce _at_ the limit. [Anomaly rules](/ai-governance/anomaly-rules) detect _before_ it, the patterns that precede a breach (spend spike, geo-mismatch, off-hours usage). The detector evaluates `spend_spike` rules live today; other rule types are preview-only (the [coverage table](/ai-governance/anomaly-rules#rule-type-coverage) marks which is which).
...
| `/me` shows "your IT team is still setting things up" empty state         | Step 4 skipped                                                                                                                      | Open `/governance/tool-catalog` → **Import starter pack**.                                                                                                                             |
