...
<Info>**Pairs with:** [AI Gateway → CLI Integrations](/coding-agents/overview). The `langwatch` CLI is shared between Gateway and Governance, `login` and `logout` are gateway-side, `ingest` and `governance status` subcommands are governance-side, both behind the same device-flow OAuth.</Info>

A small set of read-only commands for SREs, governance admins, and on-call engineers to answer "is data flowing?" without opening the web UI. They reuse the same backend the [admin oversight dashboard](/ai-governance/control-plane) and the [per-source detail page](/ai-governance/ingestion-sources/index) already render, so what you see in the terminal and what you see in the browser are guaranteed identical (byte-for-byte on `--json`).
...
These commands are **not** for end-user developers, they don't proxy completions, debit budgets, or mutate state. For coding-CLI integration, see [Coding CLI Integrations](/coding-agents/overview).
...
The CLI does not fall back to a non-default policy; selection is explicit. See [Getting started as an admin](/ai-governance/getting-started-admin) for the rest of the org-bootstrap checklist.
