...
<Info>
  **Pairs with:** [AI Gateway → CLI Integrations](/ai-gateway/cli/overview). The
  `langwatch` CLI is shared between Gateway and Governance, `login` and `logout` are
  gateway-side, `ingest` and `governance status` subcommands are governance-side, both
  behind the same device-flow OAuth.
</Info>
