...
`langwatch login` opens a browser sign-in. Approve it once and the CLI works with your personal account from then on: every command resolves your personal project by itself, with no API key and no env vars. `langwatch whoami` shows who you are logged in as.
