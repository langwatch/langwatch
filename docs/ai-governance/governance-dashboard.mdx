...
<Info>
  **Pairs with:** [Control plane & integration tiers](/ai-governance/control-plane) for the surface
  map, [Anomaly rules](/ai-governance/anomaly-rules) for what populates the anomalies strip, and
  [Departments](/ai-governance/departments) for how the **Spend by department** card attributes
  spend across the org.
</Info>

<Note>
  **Visibility**: the `/governance` entry only appears in the main sidebar once the org's
  setup-state OR-of-flags trips (see [When the entry shows up](#when-the-entry-shows-up) below). A
  brand-new org sees the **Governance** admin pages instead until they have data flowing.
</Note>
