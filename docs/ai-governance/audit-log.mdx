...
<Info>
  **Pairs with:** [Compliance architecture](/ai-governance/compliance-architecture) (the OCSF v1.1
  export consumes the same event stream), [CLI debug](/ai-governance/cli-debug) (`langwatch
  governance status` mirrors the same setup-state signals the audit log records on first
  activation), and [Ingestion sources](/ai-governance/ingestion-sources) (S3 audit log feeds from
  external SaaS platforms, Workato, Copilot Studio, Anthropic Compliance, land here as OTel-shaped
  events through the same pipeline, so a single audit log query covers in-platform mutations *and*
  third-party-tool activity in one place).
</Info>

<Note>
  **Permission required**: the audit log surface is gated to `organization:manage` (admins). Members
  on the org see an "Access Restricted" panel when they navigate here directly. The underlying tRPC
  procedure is also gated server-side, so direct API calls return UNAUTHORIZED for non-admins.
</Note>
...
A single user action often produces both: creating a Gateway Budget through the admin UI writes a `gatewayBudgets.create` Platform row _and_ a `gateway.budget.created` Gateway row, both stamped with the same `targetId` so you can correlate them in the same view.
...
Audit log retention follows your organization's [retention policy](/platform/data-retention), scoped per organization, team, and project. Configuring it requires an Enterprise plan; other plans use the platform default window. Organizations under SOC 2, ISO 27001, or HIPAA obligations usually set a multi-year period here.

Retention applies to the underlying ClickHouse partition; the Postgres `AuditLog` mirror table is kept for fast UI lookups against the most recent year. The full model is described in [Compliance architecture](/ai-governance/compliance-architecture).
