...
Commit `.claude/settings.json` with the non-secret flags and keep the bearer token **plus the `CLAUDE_CODE_ENABLE_TELEMETRY` enable flag** in a gitignored `.claude/settings.local.json`. Telemetry is **off by default**: the enable flag lives only in the local file, so a fresh clone with just the committed `settings.json` (and no `settings.local.json`) never attempts to export, whatever the committed flags say. That is what makes it safe to commit the content flags, including `OTEL_LOG_USER_PROMPTS` (prompt text) and `OTEL_LOG_RAW_API_BODIES` (the full request and response JSON of every model call). None of that content ships until the repo owner opts in locally by adding the enable flag. `settings.local.json`'s `env` merges per-key with `settings.json`'s `env`, so the local enable flag combines with the committed flags rather than replacing them.
