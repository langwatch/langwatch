...
<Warning>Those four flags are what put your session content in LangWatch: prompt text, the input and output of every tool call, including file contents, and the model request and response bodies of every turn. Committing any of them turns it on for everyone who clones the repo, which is why the recipe keeps all four in each person's own gitignored file. The enable flag can arrive from any settings scope Claude Code reads, not only from that file: a managed settings file, a shell export, or a contributor's own `~/.claude/settings.json` can each turn telemetry on, and a content flag sitting in the committed file rides along with it.</Warning>
