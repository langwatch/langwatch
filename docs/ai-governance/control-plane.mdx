...
| `/governance/ingestion-sources`, `/governance/anomaly-rules`, `/governance/routing-policies` | Admin-authoring (CRUD), create, rotate, archive sources, write rules, publish routing policies. | Org admins with `organization:manage` | Settings sub-routes, deliberately separate from daily-use surfaces. |
