...
<Info>**Pairs with:** [AI Gateway → Overview](/ai-gateway/overview). The gateway is the data plane (request path); this page is the control plane (configuration + the projections derived from those spans) that sits next to it in the same compose stack and Helm chart.</Info>
