...
<Info>**Pairs with:** [AI Gateway → Overview](/ai-gateway/overview). The gateway is the data plane (request path); this page is the control plane (configuration + fold processing) that sits next to it in the same compose stack and Helm chart.</Info>
...
- **`microsoft_365_audit`**, pull every 15 min from the Office 365
  Management Activity API. `parserConfig`:
  `{ credentials: { tenantId, clientId, clientSecret } }`, the secret
  encrypted at rest rather than hashed — the puller has to present the
  real value to Microsoft. Replaces `copilot_studio`, which polled an
  Entra directory-change feed that returned no Copilot interactions.
