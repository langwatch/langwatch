...
[Coding CLI Integrations](/coding-agents/overview). The CLI's job
...
$ langwatch               app.langwatch.ai           api.anthropic.com
  login --device  ◀──SSO──▶  /api/v1/auth/cli/*
  ↓ writes ~/.langwatch
...
- **One credential covers every tool the dev uses.** Same VK for
  ...
  [CLI overview](/coding-agents/overview).
...
- **In-house agents using the LangWatch SDKs**: the existing
  `langwatch` SDK setup, with no governance-specific step on top.
...
- ✅ **`IngestionSourceService` + REST receivers + tRPC admin
  router.** `POST /api/v1/ingest/otel/:sourceId` (Tier 4 entry
  point) and `POST /api/v1/ingest/webhook/:sourceId` (Tier 3 push
...
- For per-tool CLI config (env vars, dialect notes):
  [Coding CLI Integrations](/coding-agents/overview).
