...
<Info>
  **Pairs with:** [AI Gateway → Budgets](/ai-gateway/budgets). Budgets enforce hard spend limits;
  anomaly rules detect patterns *before* a budget breach (spend-spike, geo-mismatch, off-hours).
</Info>

<Note>
  **Available on Enterprise plans.** Non-enterprise organizations see an upgrade card on
  `/governance/anomaly-rules` instead of the composer. See [Open-core
  licensing](/ai-governance/open-core-licensing) for the full Apache 2.0, Enterprise split.
</Note>
