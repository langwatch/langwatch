...
For the agent-end-to-end gate (real Claude Code agent provisions governance via MCP only, no UI clicks) the runbook lives at `dev/docs/runbooks/governance-dogfood-mcp-claude-code.md`. Six steps: `claude mcp add langwatch http://localhost:5560/mcp` → read-tool walkthrough (admin_list + get with project apiKey) → write-tool walkthrough (clone + update_ottl_rules + archive after OAuth → 3 audit rows with `metadata.surface = "mcp"`) → binding install (1 audit row + one-time `lwub_…` token) → fail-closed `AUTH_REQUIRED:` negative case → checklist for pasting evidence back.
