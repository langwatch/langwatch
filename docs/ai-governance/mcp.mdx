...
The Path B mechanics (per `fc6d54100` + Sergey's MCP guidance):

- **RBAC check happens at the MCP tool layer**, BEFORE the service call — the tool mirrors `checkOrganizationPermission` from `langwatch/src/server/api/rbac.ts` against the apiKey's scopes and returns `FORBIDDEN` early if the required permission is missing. Services trust the surface — they do not gate on the MCP-resolved scope a second time. This split is intentional: the MCP tool owns the RBAC contract for its surface; the service owns the multitenancy + business-logic contract.
- **`callerUserId` flows from the authenticated user** — for `UserIngestionBindingService.{install, uninstall, rotateToken}` (which need a real human caller), the MCP handler's authenticated user id is passed through as `callerUserId`. For `IngestionTemplateService` authoring methods (which take `callerUserId` only for the audit trail), same pattern.
