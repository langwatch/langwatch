...
LangWatch prompts use [Liquid](https://liquidjs.com/) as their template language. If you're coming from Jinja2, you'll find Liquid familiar, with filters, strict compilation modes, and safe rendering built in.
...
| **Workflows**         | Node inputs defined in the workflow              |
...
<Note>
  **Truncate behavior differs between SDKs.** In the TypeScript SDK (LiquidJS), `truncate: N`
  outputs N characters of content then appends `"..."`. In the Python SDK (python-liquid),
  `truncate: N` means N total characters including the `"..."` suffix. Keep this in mind if you rely
  on exact output length.
</Note>
