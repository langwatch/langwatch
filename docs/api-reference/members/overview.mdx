...
| -------- | ------------------------------------------- | --------------------- | ------------------------------------------------------------- |
| `GET`    | `/api/v1/organization/members`                 | `organization:view`   | List members with their role and status                       |
| `GET`    | `/api/v1/organization/members/{userId}`        | `organization:view`   | Read one member and the teams they reach                      |
| `PATCH`  | `/api/v1/organization/members/{userId}`        | `organization:manage` | Change the organization role, or disable and re-enable access |
| `DELETE` | `/api/v1/organization/members/{userId}`        | `organization:manage` | Remove a member from the organization and its teams           |
| `GET`    | `/api/v1/organization/members/{userId}/access` | `organization:manage` | Read everything the member can reach and where it comes from  |
...
curl "https://app.langwatch.ai/api/v1/organization/members?includeDisabled=true" \
  -H "Authorization: Bearer sk-lw-..."

# Promote someone
curl -X PATCH https://app.langwatch.ai/api/v1/organization/members/user_abc123 \
  -H "Authorization: Bearer sk-lw-..." \
