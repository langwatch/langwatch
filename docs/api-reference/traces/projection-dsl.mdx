...
<Info>
  Annotations are manual reviews captured in the LangWatch UI (thumbs, scores, comments).
  They live alongside SDK-emitted `events` and are joined into the same response — no
  separate annotations call needed.
</Info>
...
<Note>
  ...
  evaluations, metadata updates, spans, and annotation **creates** and **deletes**. **One
  gap today:** an **in-place edit** to an existing annotation (e.g. changing a score from
  3 to 5) writes Postgres only and does not advance the trace's ClickHouse `updated_at`,
  so `dateField: "updated"` does not pick it up until the trace changes again. New and
  removed annotations *are* caught. See
