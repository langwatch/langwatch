...
description: "Declare exactly which trace fields to return from POST /api/v1/traces/search — including nested events, annotations, and evaluations — so a daily ETL becomes one paginated loop instead of a per-trace fan-out."
---

## Why

By default `POST /api/v1/traces/search` returns the full trace shape. For an ETL
...
```bash
curl -X POST https://app.langwatch.ai/api/v1/traces/search \
...
```jsonc
{
  ...
  "evaluations": [{ "name": "Faithfulness", "score": 0.91 }],
  "events": [{ "type": "thumbs_up_down", "metrics": { "vote": 1 } }],
  "annotations": [{ "is_thumbs_up": true, "scores": { "quality": { "value": "5" } } }],
}
```
...
<Info>
  Annotations are manual reviews captured in the LangWatch UI (thumbs, scores, comments). They live
  alongside SDK-emitted `events` and are joined into the same response — no separate annotations
  call needed.
</Info>
...
at-most-once within a single scroll**. A trace modified again _while you are
paging_ moves past the cursor and is not re-delivered in the current scroll —
...
<Note>
  The `updated` watermark advances on every ClickHouse-side change to a trace — evaluations,
  metadata updates, spans, and annotation **creates** and **deletes**. **One gap today:** an
  **in-place edit** to an existing annotation (e.g. changing a score from 3 to 5) writes Postgres
  only and does not advance the trace's ClickHouse `updated_at`, so `dateField: "updated"` does not
  pick it up until the trace changes again. New and removed annotations *are* caught. See
  [#4736](https://github.com/langwatch/langwatch/issues/4736) for the cross-store watermark
  follow-up.
</Note>
