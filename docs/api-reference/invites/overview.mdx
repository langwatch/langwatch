...
| -------- | -------------------------------- | ------------------------------------------------ |
| `GET`    | `/api/v1/organization/invites`      | List pending invites with their acceptance links |
| `POST`   | `/api/v1/organization/invites`      | Create up to 50 invites in one batch             |
| `DELETE` | `/api/v1/organization/invites/{id}` | Revoke a pending invite                          |
...
```bash
curl -X POST https://app.langwatch.ai/api/v1/organization/invites \
...
```bash
curl https://app.langwatch.ai/api/v1/organization/invites \
  -H "Authorization: Bearer sk-lw-..."

curl -X DELETE https://app.langwatch.ai/api/v1/organization/invites/<invite_id> \
  -H "Authorization: Bearer sk-lw-..."
```
