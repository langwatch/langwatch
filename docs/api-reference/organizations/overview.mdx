...
```
instance key  ->  POST /api/v1/organizations  ->  organization + admin API key
                                                     |
                                                     v
...
| ------ | ------------------------- | ------------------------------------------------------ |
| `GET`  | `/api/v1/organizations`      | List the organizations on this instance                |
| `POST` | `/api/v1/organizations`      | Create an organization and its bootstrap admin API key |
| `GET`  | `/api/v1/organizations/{id}` | Read one organization                                  |
...
```bash
curl -X POST https://langwatch.acme.internal/api/v1/organizations \
...
`GET /api/v1/organizations` and `GET /api/v1/organizations/{id}` return the organizations on the instance, which is what lets a provisioning run check whether it has work to do before it does any. An id that does not exist answers 404.
