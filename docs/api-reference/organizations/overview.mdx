...
<Note>
  **Self-hosted only.** These endpoints exist on a self-hosted LangWatch deployment that
  has `LANGWATCH_INSTANCE_ADMIN_API_KEY` configured. Without that variable, and on
  LangWatch Cloud, they answer 404: the family is absent rather than forbidden. Creating
  organizations on LangWatch Cloud is not part of this API.
</Note>
