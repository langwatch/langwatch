...
- `POST /api/v1/evaluations/{evaluator}/evaluate` - Run an evaluation using a specific evaluator
...
<Warning>
  **Important for Analytics:** When calling evaluators from code (Real-Time Evaluations), always
  provide a descriptive `name` parameter to distinguish between different evaluation checks in
  Analytics.
</Warning>
...
```
POST /api/v1/evaluations/{evaluator_path}/evaluate
```
