...
Reading your own keys needs `organization:view`. Creating, updating and revoking need `organization:manage`, and so does reading or listing keys that belong to somebody else.
...
A key you do not own is readable only with real organization adminness plus `organization:manage`. Without both, the id answers 404 rather than 403, because a 403 would confirm the key exists.

## Updating

`PATCH` is partial at the field level: send `name` alone and only the name changes. `bindings`, when you send them, replace the key's bindings outright, so what you send is what the key has afterwards.
