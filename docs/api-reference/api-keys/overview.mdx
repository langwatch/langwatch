---
title: "Overview"
description: "Create and manage API keys programmatically. Supports personal keys (user-scoped) and service keys (bound to organization, team or project scopes, for automation)."
---
...
- **Service keys**: no user association, bound to the organization, one or more teams, or one or more projects, with a role on each. Use them for CI/CD, scaffolding tools, and service-to-service integrations
...
<Info>
  Service keys with neither `projectIds` nor `bindings` get org-wide ADMIN access. Always scope to
  specific projects when possible.
</Info>

`projectIds` is shorthand rather than a separate mechanism: each id becomes one ADMIN binding on that project, added to any `bindings` you sent rather than replacing them. Reach for `bindings` directly when you want a role other than ADMIN, or a team or organization scope. Service keys only, and a personal key that sends it is refused.
...
- `bindings`, the roles the key holds and where, in the same `role` / `scopeType` / `scopeId` shape [role bindings](/api-reference/role-bindings/overview) use. `scopeType` is `ORGANIZATION`, `TEAM` or `PROJECT`.
