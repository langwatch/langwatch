...
| `GET`    | `/api/v1/api-keys/{id}` | Read one key and the access it carries     |
| `PATCH`  | `/api/v1/api-keys/{id}` | Update a key's name, description or access |
| `DELETE` | `/api/v1/api-keys/{id}` | Revoke an API key                          |

## Reading a key back

`GET /api/v1/api-keys/{id}` and `PATCH /api/v1/api-keys/{id}` return the same body, and it contains every field a write accepts. `bindings` comes back in exactly the shape `POST` and `PATCH` take, so comparing what you asked for against what the key has is a comparison rather than a translation. The secret itself is never returned: it exists only in the create response.
...
```bash
curl -X PATCH https://app.langwatch.ai/api/v1/api-keys/<api_key_id> \
...
<Info>
  Service keys without `projectIds` get org-wide ADMIN access. Always scope to specific projects
  when possible.
</Info>
