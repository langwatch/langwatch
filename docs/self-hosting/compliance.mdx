...
- **Art. 19 log retention (≥6 months)**: **Enterprise** extended retention policies, with windows set in years. Art. 19 — not Art. 12 — governs how long the automatically generated logs are kept: at least six months, unless other Union or national law provides otherwise. The Apache 2.0 default 49d window sits below that floor, so a deployment in scope of Art. 19 raises the extended-retention window to at least six months.

Most enterprise-deploying-an-LLM-agent uses engage Art. 12 (the logging
capability) and Art. 19 (retention of those logs, at least six months —
which the Enterprise extended-retention windows satisfy). Art. 18 is a
separate duty: the provider keeps the system's *technical documentation*
available to authorities for ten years. That documentation set is not the
operational log data LangWatch stores, so Art. 18 does not drive the
