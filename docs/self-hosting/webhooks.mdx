...
A gateway request that was admitted but never confirmed is settled after a grace window and delivered as `gateway.request.settled` with unknown cost (see [settled semantics](/ai-gateway/billing-events#event-lifecycle)). The default grace is **30 minutes**. Tune it with:
