...
<Warning>
  Leave this unset on any deployment serving more than one organization. With it on, anyone who can
  create a webhook endpoint can name **any queue this deployment's role is allowed to write to**,
  including another tenant's, and the platform writes to it. It is safe only on a single-tenant
  install whose queues are its own, and in local development.
</Warning>
