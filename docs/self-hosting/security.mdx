...
- **The Langy assistant** (`langyagent.chartManaged`) runs its manager as root with `CHOWN`, `DAC_OVERRIDE`, `FOWNER`, `SETUID` and `SETGID` by default. The manager needs those capabilities to provision and launch each worker under a distinct UID. Removing them makes provisioning fail; it does not select shared identity. The overlay disables the assistant.

  To run it non-root, explicitly set `langyagent.workerIsolation: none` with `langyagent.acceptWorkerIsolationDisabled: true`. That defaults to UID 1000 with `drop: [ALL]` and `RuntimeDefault` seccomp. Set `langyagent.podSecurityContext.runAsUser` to an allowed nonzero UID when required by cluster policy; a nonzero `langyagent.containerSecurityContext.runAsUser` overrides the pod UID.

  Read [the accepted exposure](/self-hosting/langy/setup#hardening-clusters-that-refuse-root): workers share credentials, files and control-descriptor access, and can read manager secrets. Forcing `runAsNonRoot` or a nonzero UID while retaining `per-uid` is rejected during Helm rendering.
...
With Prometheus off, app metrics reporting (`app.telemetry.metrics.enabled`) is off by default too, so the app does not expose a `/metrics` endpoint and Prometheus does not scrape it; the overlay pins it off to keep the two coupled. Anonymous usage analytics (`app.telemetry.usage.enabled`) is a separate setting, on by default; set it to `false` for an air-gapped or no-egress install.
