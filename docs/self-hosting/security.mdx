...
<Tip>
  For inter-service encryption, deploy a service mesh like Istio or Linkerd. This adds mTLS between
  all pods without application changes.
</Tip>
...
With Prometheus off, app metrics reporting (`app.telemetry.metrics.enabled`) is off by default too, so no component exposes or scrapes a `/metrics` endpoint; the overlay pins it off to keep the two coupled. Anonymous usage analytics (`app.telemetry.usage.enabled`) is a separate setting, on by default; set it to `false` for an air-gapped or no-egress install.
