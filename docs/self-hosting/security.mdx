...
- **Blocked requests answer 404 or 503**, depending on your controller
  (ingress-nginx serves its default-backend 404; Traefik and the AWS load
  balancer controller answer 503). All fail closed. Expect a low background rate
  in your ingress error metrics from internet scanners — that is the block
  working, not the app faulting.
