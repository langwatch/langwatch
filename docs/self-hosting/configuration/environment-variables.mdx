...
| `app.dataplane.legacyAzureRead` | Keeps the `AZURE_BLOB_*` settings emitted after moving writes back to S3, so objects already written to Azure stay readable. The write toggle is not emitted, so new writes go to S3 |
