...
The dataplane S3 bucket carries **two** surfaces:

- **Dataset storage** — opt-in via `DATASET_OBJECT_STORAGE_ENABLED`. Persists rows uploaded through the dataset UI.
- **Scenario media externalization** — automatic for every applicable scenario event. The `/api/scenario-events` ingest path extracts inline `audio` / `image` / `video` / `document` bytes from incoming events, content-addresses them under `{projectId}/{sha256}`, and rewrites the event payload to reference the stored object by id. The bytes are served back via `GET /api/files/:id`.

When `S3_BUCKET_NAME` is set, both surfaces use that bucket. When it's not set, scenario media falls back to the local filesystem at `LANGWATCH_LOCAL_STORAGE_PATH` (dev-only — see warning below).

| Variable | Description | Default |
|----------|-------------|---------|
| `S3_BUCKET_NAME` | Dataplane bucket shared by datasets + stored-objects | — |
| `DATASET_OBJECT_STORAGE_ENABLED` | Enable S3 for dataset uploads (separate from scenario media) | `false` |
