...
| `AZURE_BLOB_ACCOUNT_KEY` | Azure Storage account key (SharedKey auth) | — |
...
Deployments whose only egress is a corporate HTTP proxy can set the standard `HTTPS_PROXY` (or `HTTP_PROXY`) and `NO_PROXY` variables. The `ses` and `resend` gateways route their API calls through it; `NO_PROXY` entries are honoured, including leading-dot domain suffixes and `*`.

The `smtp` gateway ignores these variables on purpose, because an SMTP relay is normally an internal host that must be reached directly. `sendgrid` is not proxy-aware either: its client offers no seam to attach a proxy agent, so behind a proxy use `ses`, `resend`, or point `smtp` at SendGrid's own SMTP relay.

<Note>
The AWS SDK does not read `HTTPS_PROXY` by itself, so before this was supported a proxied deployment failed with a `read ETIMEDOUT` from SES. If you are on an older version and cannot upgrade, `smtp` against an internal relay avoids the proxy entirely.
</Note>
...
| `app.dataplane.providers.azureBlob.accountKey.value` | `AZURE_BLOB_ACCOUNT_KEY` |
