...
Deployments whose only egress is a corporate HTTP proxy can set the standard `HTTPS_PROXY` (or `HTTP_PROXY`) and `NO_PROXY` variables. The `ses`, `resend` and `sendgrid` gateways route their API calls through it; `NO_PROXY` entries are honoured, including leading-dot domain suffixes and `*`.
...
The `smtp` gateway deliberately ignores these variables, because an SMTP relay is usually an internal host that must be reached directly. Point `SMTP_HOST` at the relay itself.
