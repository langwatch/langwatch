...
<Warning>
When `STORED_OBJECTS_BACKEND=azure` is set, all three of `AZURE_BLOB_ACCOUNT_NAME`, `AZURE_BLOB_ACCOUNT_KEY`, and `AZURE_BLOB_CONTAINER` are required together. An incomplete config fails loud at write time, naming the missing variable — it never silently falls back to S3 or the local filesystem. Authentication is SharedKey only; workload identity and per-project Azure BYOC are tracked separately (issues #6087, #6088).
</Warning>
