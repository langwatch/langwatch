...
<Note>
See [Langy environment variables](/langy/self-hosting/environment-variables) for the full `LANGY_*` and `GITHUB_LANGY_*` reference, and [Enable Langy via Helm](/langy/self-hosting/helm) for the `langyagent.chartManaged` toggle and the `LANGY_INTERNAL_SECRET` secret.
</Note>
