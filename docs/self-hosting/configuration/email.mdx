...
A gateway that is named but unusable fails at send time with a message naming the setting to fill in. When a different gateway is fully configured, the message names that one too, which is what an install sees when it names one gateway and supplies another's settings:
