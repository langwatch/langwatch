...
<Note>
  `EMAIL_PROVIDER` decides which gateway runs, and no other gateway is tried if it fails.
  Leave it unset and the gateway is inferred from whichever credentials are present,
  `USE_AWS_SES` first and then `SENDGRID_API_KEY`. `smtp` and `resend` are only ever used
  when named.
</Note>
...
<Note>
  A SES account still in the sandbox can only send to verified addresses, which includes
  the address alerts are delivered to. Request production access before relying on it.
</Note>
