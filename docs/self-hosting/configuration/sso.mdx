...
Complete the [identity and access migrations](/self-hosting/upgrade#complete-identity-and-access-migrations-first)
on the preparatory release before installing the identity and access update.
Check the persisted migration results in **Ops > Migrations**; a successful
startup does not prove that existing SSO connections or user accounts finished
migrating. Keep the current provider configuration and credentials throughout
this step.
