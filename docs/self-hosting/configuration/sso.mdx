...
<Note>
  A `NEXTAUTH_PROVIDER` value LangWatch does not recognise, or one whose client
  credentials are missing, starts the deployment in email mode and logs which provider it
  could not mount. That keeps a licensed deployment signable-in while you correct the
  value, rather than pointing the sign-in page at an identity provider that was never
  wired up.
</Note>
...
<Note>
  The provider is `azure-ad` while its credentials sit under `providers.azureAd`. The
  first is the value LangWatch selects on, the second is the chart's key for the
  `AZURE_AD_*` variables.
</Note>
...
<Note>
  If OneLogin sits behind another identity provider, for example a Cognito user pool that
  federates to it, point LangWatch at whichever one it talks to directly and let that one
  handle the hop. LangWatch only needs one OIDC issuer.
</Note>
...
<Note>
  Users keep their existing data, projects, and permissions after the SSO migration. The
  migration only changes their authentication method.
</Note>
