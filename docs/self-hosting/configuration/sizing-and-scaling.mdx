...
<Note>
  ClickHouse auto-tunes internal parameters (memory limits, thread pools, merge settings) based on
  the CPU and memory you allocate. You only need to set `clickhouse.cpu` and `clickhouse.memory`.
</Note>
...
Every figure below is `requests`/`limits` as the chart renders it, and the totals are the sum of requests across all replicas. They include the AI Gateway and Langy, which run by default and are a real part of what your cluster has to fit.
...
`examples/values-local.yaml` is the same sizing without Prometheus (~2.6 CPU, ~6.6 Gi), bundled with NodePort access and images pinned to a locally built `local` tag. That last part makes it Kind-only: see [Local Kubernetes](/self-hosting/deployment/kubernetes-local).
...
- 10-20 GB is sufficient for the size profiles on this page

### Redis

- Minimal storage: job queue, cache and sessions
...
<Tip>
  For ClickHouse, prioritize memory over CPU. ClickHouse benefits from large memory for caching and
  merge operations.
</Tip>
