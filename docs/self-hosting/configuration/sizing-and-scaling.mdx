...
<Note>
  ClickHouse auto-tunes internal parameters (memory limits, thread pools, merge settings)
  based on the CPU and memory you allocate. You only need to set `clickhouse.cpu` and
  `clickhouse.memory`.
</Note>
...
<Tip>
  For ClickHouse, prioritize memory over CPU. ClickHouse benefits from large memory for
  caching and merge operations.
</Tip>
