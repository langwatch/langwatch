...
<Tip>
  This is a **zero-downtime migration**. Elasticsearch and ClickHouse run side-by-side
  during the transition. New data flows to ClickHouse immediately, and you migrate
  historical data at your own pace.
</Tip>
...
<Note>
  The Helm chart configures `CLICKHOUSE_URL` automatically from the ClickHouse values, no
  manual env var needed for either managed or external ClickHouse.
</Note>
...
<Note>
  Keep your `ELASTICSEARCH_NODE_URL` configured during this phase. LangWatch v3 can still
  read from Elasticsearch for data that hasn't been migrated yet.
</Note>
...
<Warning>
  Point these at your actual Elasticsearch and ClickHouse instances. If they're running in
  Docker or Kubernetes, you may need to set up port forwarding or use the internal network
  addresses.
</Warning>
...
<Warning>
  **This step is critical.** If you skip it, ClickHouse may immediately expire or offload
  historical data as it arrives during migration.
</Warning>
...
<Tip>
  We recommend setting TTL values to a **multiple of 7** (e.g., 7, 14, 28, 49) to align
  with ClickHouse partition boundaries for more efficient data management.
</Tip>
...
<Tip>
  For large history systems, we recommend running one target at a time instead of `all`,
  so you can apply the tuning profiles below for each target individually.
</Tip>
