...
Add the ClickHouse service to your `infra/compose.yml`:
...
Add `CLICKHOUSE_URL` to your app and workers environment in `infra/compose.yml`:
...
2. **Remove the Elasticsearch service** from your `infra/compose.yml` or Helm values
...
```bash
# After removing the elasticsearch service from infra/compose.yml
docker compose up -d
```
