...
<Warning>
  **Drain is irreversible.** Drained jobs cannot be recovered. Always prefer moving to DLQ
  first if there's any chance the jobs can be reprocessed later.
</Warning>
...
<Tip>
  **Use canary redrives after deploying a fix.** If the fix works for the canary batch,
  replay the rest. If it doesn't, the canary groups return to the DLQ and you haven't made
  the problem worse.
</Tip>
