...
spec:
  ...
  source:
    ...
    chart: langwatch
    targetRevision: 3.3.0  # use the latest published chart version, see https://github.com/langwatch/langwatch/releases
    helm:
...
The pre-upgrade preflight Job validates that every required key is present on both Secrets before pods roll, so a missing or misspelled key fails the sync with a clear list of what is missing rather than crashlooping pods opaquely.
