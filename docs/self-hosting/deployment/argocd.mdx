...
- Chart template comment: `infra/charts/langwatch/templates/app/secrets.yaml` documents the per-key lookup-or-rand shape inline.
- Chart values: `infra/charts/langwatch/values.yaml` sections `secrets`, `gateway.secrets`, `autogen`, `preflight`.
- Helper that enforces app+gateway Secret-name consistency: `infra/charts/langwatch/templates/_helpers.tpl` defines `langwatch.appSecretName` and `langwatch.validateSecrets`.
