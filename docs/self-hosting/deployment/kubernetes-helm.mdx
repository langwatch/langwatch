...
The ingress controller routes a host to only one install, so each install also needs its own ingress hosts: set `ingress.hosts` and `gateway.ingress.host` per install, or disable an ingress that install does not need.
