...
3. **Worker pods must be running.** Budget enforcement reads from a ClickHouse rollup that the trace-processing pipeline's budget-debit projection writes into. If you deploy with `workers.enabled=false`, budgets stop accumulating spend and breach enforcement silently degrades. The default `workers.enabled=true` is correct for production.
