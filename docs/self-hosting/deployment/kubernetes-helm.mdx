...
<Warning>
  `autogen.enabled=true` generates random secrets on each install. This is fine for
  testing but not for production, secrets change on reinstall and invalidate sessions. See
...
<Note>
  Replicated ClickHouse requires an odd number of replicas (3, 5, 7) for Keeper consensus.
  Use 3 replicas unless you have a specific reason for more.
</Note>
