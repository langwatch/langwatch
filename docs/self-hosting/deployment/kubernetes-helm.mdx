...
Create the app Secret and the gateway-auth Secret:
...
The gateway-auth Secret is mounted by both `langwatch-app` and the AI gateway pod, so the two values must stay byte-for-byte identical. Pre-creating it once and pointing the chart at it (next step) keeps both deployments stable across upgrades and avoids re-rolling pods on `helm upgrade`.
...
1. **Shared secrets must exist before install** (or use `autogen.enabled=true` and let the chart materialise them). The gateway and the LangWatch app both mount `LW_GATEWAY_INTERNAL_SECRET` + `LW_GATEWAY_JWT_SECRET` from `gateway.secrets.existingSecretName` (default `langwatch-gateway-auth`), same byte-for-byte values. The app additionally reads `virtualKeyPepper` from its own `secrets.existingSecret` Secret (default `langwatch-app-secrets`). The [Production Deployment](#production-deployment) section above shows the two `kubectl create secret generic` commands. With `autogen.enabled=false` and one of those Secrets missing, the preflight Job aborts the install with a clear list of missing keys; under `autogen.enabled=true` the chart materialises both Secrets on first install via lookup-or-rand.
