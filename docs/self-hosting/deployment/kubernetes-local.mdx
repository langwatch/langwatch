...
cd langwatch/infra/charts/langwatch
make example-up
```
...
kind create cluster --name lw-local \
  --config infra/charts/lib/kind-config.yaml \
  --wait 60s
```
...
- `platform/app/langwatch_nlp:local`, NLP service
...
Ensure the Kind cluster was created with the port mapping config (`infra/charts/lib/kind-config.yaml`). Recreate the cluster if needed: `make clean && make example-up`.
