...
| **Port**       | 5560                                                                                                                                                            |
| **Base**       | `node:24-alpine`                                                                                                                                                |
| **Entrypoint** | Runs pending Postgres/ClickHouse/LangWatchQL migrations, then starts the API process (`pnpm start` in `apps/api`).                                              |
| **Workers**    | In Kubernetes, Workers run as a separate Deployment using the same image, started from `apps/worker` instead of `apps/api`. There is no in-process worker mode. |

**What it does:**

- Serves the LangWatch web UI — a prebuilt Vite/React single-page app, served as static assets by the API process (no separate UI server)
...
<Note>
  LangEvals calls external LLM providers (OpenAI, Azure OpenAI, Google) to run model-based
  evaluations. Ensure your evaluator pods have network access to these providers, or configure your
  own provider credentials via the Helm chart.
</Note>
...
<Tip>
  Pin to a specific version tag in production to prevent unexpected changes during upgrades. Update
  deliberately using the [Upgrade Guide](/self-hosting/upgrade).
</Tip>
