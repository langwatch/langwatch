...
<Warning>
The Docker Compose setup is being updated for LangWatch v3 with ClickHouse. The current compose file uses OpenSearch (v2). For production and v3, use the [Kubernetes Helm chart](/self-hosting/deployment/kubernetes-helm).
</Warning>
...
| `opensearch` | `langwatch/opensearch-lite:latest` | 9200 | Trace storage |
...
- **ClickHouse** — uses OpenSearch for trace storage (v3 Helm chart uses ClickHouse)
