...
On a pod pinned to gVisor — the hardened posture, not the default — the user-space kernel implements no netfilter, so `iptables` and `nftables` fail in every mode. Any egress control based on iptables (REDIRECT, OWNER-match DROP, NAT) is off the table there. The L7 adapter below is what makes egress control work identically whichever runtime the pod lands on, which is why it is the mechanism rather than a gVisor workaround.
