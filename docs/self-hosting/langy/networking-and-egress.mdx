...
On a pod pinned to gVisor (an optional hardening setting), the default Netstack mode does not expose the host kernel's netfilter, so egress controls based on `iptables` or `nftables` features such as REDIRECT, OWNER-match DROP, and NAT are unavailable inside the sandbox. gVisor's host-networking mode uses the host network stack and its firewall rules, but gives up part of Netstack's isolation. The L7 adapter avoids Netstack's firewall limitations and works consistently whichever runtime the pod uses.
