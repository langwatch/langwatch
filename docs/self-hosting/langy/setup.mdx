...
credentials of the person who asked. Workers use stdio pipes instead of an HTTP
control listener, and their config files contain environment variable names
rather than secret values. Access to live credentials and control descriptors
still depends on Unix identity.

Per-worker identity is on by default and is the protection a hardened cluster
may force you to give up:
...
Network restrictions are also on by default:

- **Network.** A default-deny NetworkPolicy. Workers reach the control plane and
  the gateway, with other destinations blocked by default. Egress to the internet is off until you turn it
...
app.kubernetes.io/name=langyagent` lists no pods, and the reason sits on the
Deployment.
...
chart supports `per-uid` with a manager running as UID 0 and five capabilities:
`CHOWN`, `DAC_OVERRIDE`, `FOWNER`, `SETUID` and `SETGID`. That configuration
provisions worker files with `chown` and applies each worker identity with
`setuid`/`setgid`, so it does not satisfy those admission policies.

Forcing `runAsNonRoot` or a nonzero `runAsUser` while retaining `per-uid`
is rejected during Helm rendering. Dropping required capabilities does not
select shared identity: it makes worker provisioning fail. Helm also merges
capability maps, so adding `drop: ["ALL"]` alone does not remove the chart's
`add` list.
...
With no identity override, the pod defaults to UID 1000 with `runAsNonRoot: true`,
`capabilities.drop: ["ALL"]`, no `add`, and `seccompProfile: RuntimeDefault`.
This satisfies PSA `restricted` without requiring an exemption, a RuntimeClass or a
separate namespace. Any configured `runtimeClassName` is retained.

For clusters requiring a different UID, set
`langyagent.podSecurityContext.runAsUser` to an allowed nonzero UID. The
container inherits that UID unless `langyagent.containerSecurityContext.runAsUser`
sets a nonzero override. Omitting the acknowledgement fails the render with a message
naming the trade, so nobody reaches this posture by leaving a field blank.

<Warning>
**What `workerIsolation: none` costs.** Shared identity removes isolation
...
The pod logs a warning at boot identifying the shared-identity posture, so the posture is
...
The agent pod and every reference to it are removed on the next upgrade. Other
components retain their configuration.
