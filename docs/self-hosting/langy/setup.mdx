...
## Hardening with a sandboxed runtime

Langy's workers run shell written by a language model. The default install runs
the pod on the node's normal runtime so the chart installs on any cluster, and
the install notes state that the pod-to-host sandbox is absent.

What you keep by default: each conversation's worker runs under its own UID with
...
all colleagues carries a very different risk here than one exposed to untrusted
users, and that judgement is yours to make.

For production, pin the pod to [gVisor](https://gvisor.dev/docs/user_guide/install/).
Managed offerings like GKE Sandbox provide the RuntimeClass; on self-managed
clusters install the gVisor operator or a RuntimeClass manifest first:
...
To make the chart refuse to render unless a sandboxed runtime is configured,
also set `acceptUnsandboxedRuntime: false`.
