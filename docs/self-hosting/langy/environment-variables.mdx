...
The agent-side variables (`LANGY_*`) are set on the `langyagent` pod through the `charts/langyagent` ConfigMap and Secret. The control-plane variables (`OPENCODE_AGENT_URL`, `GITHUB_LANGY_*`, `CREDENTIALS_SECRET`) are set on the LangWatch app. Defaults shown are the values the chart or the Go service uses out of the box.

`OPENCODE_AGENT_URL` is the app-to-agent base URL and keeps its legacy name; it has nothing to do with the [opencode CLI](/coding-agents/opencode) that LangWatch instruments.
