...
The agent-side variables (`LANGY_*` apart from the app URL below) are set on the `langyagent` pod through the `charts/langyagent` ConfigMap and Secret. The control-plane variables (`LANGY_AGENT_URL`, `GITHUB_LANGY_*`, `CREDENTIALS_SECRET`) are set on the LangWatch app. Defaults shown are the values the chart or the Go service uses out of the box.
...
| `ENVIRONMENT` | Deployment environment. Read for telemetry and log labeling only; it holds no security boundary. | No | `production` |
...
| `LANGY_READINESS_TIMEOUT_MS` | How long to wait for a freshly spawned worker's ready handshake. The Go default is 15000; the chart raises it to 60000 for sandboxed-runtime cold boot. | No | `60000` (chart) |
| `LANGY_WORKSPACE_ROOT` | Writable workspace root (an `emptyDir` mount). | Yes | `/workspace` |
| `LANGY_WORKER_ISOLATION` | Per-worker identity posture: `per-uid` (each worker gets its own uid) or `none` (all workers share the pod's user). Set through `langyagent.workerIsolation`, which also requires `acceptWorkerIsolationDisabled` for `none`. Any other value refuses to boot. See [the sandbox page](/langy/security/sandbox). | No | `per-uid` |
...
| `CREDENTIALS_SECRET` | Signs the GitHub install state round-trip and encrypts provider keys at rest. Already required in production, so GitHub uses the existing secret. | Yes (prod) | none |
