...
The API key must be one issued to a user (Settings → API keys), that user must have Langy access, and the key must carry the `langy:create` permission. The turn runs as that user, so create a dedicated user and key for the monitor rather than reusing a person's. The project's own key from the project settings page is not owned by anyone and is refused with `403`. `X-Project-Id` names the project the turn runs in; a key scoped to exactly one project may omit it, any other key without it is refused with `401`. The [Langy API surface flag](/self-hosting/langy/setup#staged-rollout) must be on for that project; while it is off, the endpoint answers `404` like any unmounted path.
...
Every response the probe answers itself carries `Cache-Control: no-store`. The one exception is the `404` while the surface flag is off, which is byte-identical to an unmounted path, headers included, so the flag reveals nothing. The endpoint sits beside the other subsystem probes (`/api/health/collector`, `/api/health/scenarios`, and so on) and answers refusals in the same shape they do.
