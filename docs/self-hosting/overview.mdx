...
That fits a small cluster at roughly 2.6 CPU and 6.7 Gi of requests, and generates its own secrets without manual preparation. Auto-generated secrets change on every reinstall, so swap them for real ones before you rely on the deployment: see [Kubernetes (Helm)](/self-hosting/deployment/kubernetes-helm) for full setup instructions.
