...
graph LR
    ...
    subgraph Workers["Event Sourcing Pipeline"]
        Pipeline["Event Sourcing Pipeline"]
    end

    App -->|"enqueue"| Redis["Redis"]
    Redis -->|"consume"| Workers
    Workers -->|"events & projections"| CH["ClickHouse"]
    Workers -->|"SSE updates"| App
    App -->|"queries"| CH
    App -->|"control plane"| PG["PostgreSQL"]
    CH -->|"cold storage"| S3["S3"]
    Workers -->|"evals"| Evals["LangEvals"]
    Workers -->|"NLP"| NLP["LangWatch NLP"]
    Evals -->|"LLM calls"| LLM["External LLMs"]
    NLP -->|"LLM calls"| LLM
    Workers -->|"alerts"| Notify["Slack / Email / Webhooks"]
```
