...
<Note>
  If Worker queue depth grows in Redis, it means processing is falling behind ingestion.
  The fix is to add more Worker replicas, each one is stateless and consumes jobs
  independently.
</Note>
...
<Note>
  LangEvals and NLP make outbound calls to external LLM providers (OpenAI, Azure, etc.).
  Ensure these pods have network egress to the relevant endpoints.
</Note>
