...
```mermaid
graph TD
    LLMApp["Your LLM App<br/><i>LangWatch SDK</i>"]

    subgraph Platform["Your Infrastructure"]

        LLMApp -->|"OTel / REST"| App

        subgraph Pipeline["Event Sourcing Pipeline"]
            SI["Span Ingestion"] --- TS["Trace Summarisation"]
            TS --- EP["Experiment Processing"]
            TS --- UB["UI Update Broadcasting"]
            TS --- GT["Generate Topics"]
            TS --- LMP["LLM Metric Processing"]
            TS --- LCE["LLM Cost Enrichment"]
            TS --- AP["Annotation Processing"]
            TS --- EE["Evaluation Execution"]
            TS --- EMB["Embedding Extraction"]
            TS --- ATS["Automation / Triggers"]
            TS --- PII["PII Redaction"]
        end

        Redis["Redis<br/><i>BullMQ</i>"]
        App["LangWatch App<br/><i>:5560</i>"]
        PG["PostgreSQL"]
        CH["ClickHouse"]
        S3["S3"]
        NLP["LangWatch NLP<br/><i>:5561</i>"]
        Evals["LangEvals<br/><i>:5562</i>"]

        App -->|"enqueue jobs"| Redis
        Redis -->|"consume jobs"| Pipeline
        Pipeline -->|"SSE updates"| App
        Pipeline -->|"events & projections"| CH
        Pipeline -->|"evaluation requests"| Evals
        Pipeline -->|"NLP tasks"| NLP
        App -->|"queries"| CH
        App -->|"control plane"| PG
        CH -->|"cold data + datasets"| S3
        Pipeline -->|"alerts"| Notifications["Slack, Email, Webhooks"]
    end

    ExtLLM["External LLMs<br/><i>OpenAI, Azure, etc.</i>"]
    Evals -->|"LLM evals"| ExtLLM
    NLP -->|"workflow runs"| ExtLLM
```
