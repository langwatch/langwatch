...
Redis is required for more than one App replica. Instance presence and call dispatch live in Redis; without it the registry is in memory, correct for one replica only, and the App refuses a connection when `LANGWATCH_APP_REPLICAS` is above 1. That variable defaults to `1`, and Helm scales the App with `app.replicaCount` on its own, so a Helm deployment must set `LANGWATCH_APP_REPLICAS` to the same value as `app.replicaCount`. Without Redis, a mismatch fails in one of two ways. If `app.replicaCount` is above `LANGWATCH_APP_REPLICAS`, each pod runs its own in-memory registry, and an agent connected to one pod reads as Offline on the others. If `LANGWATCH_APP_REPLICAS` is above `app.replicaCount`, the App refuses every agent connection, also when only one pod runs.
