...
A self-hosted LangWatch deployment runs the app, the workers, PostgreSQL, Redis, ClickHouse with S3 storage, and the NLP and evaluation services, connected by an event-sourcing pipeline.
...
The API process (`apps/api`, built on Hono), the single external entry point
for all traffic. It serves REST, tRPC and SSE, and also serves the prebuilt
browser bundle (a Vite/React single-page app, `apps/ui`) as static assets —
one container is the whole interactive tier:
...
- **Authentication**: email, Google, GitHub, GitLab, Azure AD, Auth0, Okta
...
Same `langwatch/langwatch` image, started from `apps/worker` instead of
`apps/api` (there is no in-process worker mode — the app and the workers are
always separate processes). Consumes jobs from a BullMQ queue in Redis and
runs the event sourcing pipeline (see below).
...
- **Stored objects**: externalized byte content (audio/image/video/document) content-addressed under `{projectId}/{sha256}` and served back via `GET /api/v1/files/:id`.
...
<Note>
  If Worker queue depth grows in Redis, it means processing is falling behind ingestion. The fix is
  to add more Worker replicas, each one is stateless and consumes jobs independently.
</Note>
...
<Note>
  LangEvals and NLP make outbound calls to external LLM providers (OpenAI, Azure, etc.). Ensure
  these pods have network egress to the relevant endpoints.
</Note>

### Connected agents

A [connected agent](/agent-testing/connect-your-agent) is the customer's own process holding an outbound WebSocket to the App. Three requirements:

- **WebSocket upgrade on `/api/v1/agents/connect`.** The App serves it on its normal HTTP listener, next to `/api/trpc-ws`. An ingress that already passes `/api/trpc-ws` needs no change; one that terminates or rewrites upgrades must pass this path through.
- **A read timeout above 15 seconds.** The App pings every 15 seconds to keep the connection alive. A shorter idle timeout closes the connection between pings, and the agent flaps between Online and Offline. The `access-ingress.yaml` overlay sets `proxy-read-timeout: 120`.
- **Outbound WSS from the customer's side.** The process that runs the agent opens `wss://` to your App's public hostname. The customer's side opens no listening port and needs no inbound rule.
- **HTTP long polling for a network that blocks WebSockets.** The same frames also travel over `POST /api/v1/agents/connect/register`, `GET /api/v1/agents/connect/poll` and `POST /api/v1/agents/connect/frames`. A poll is held open for up to 25 seconds, so the ingress read timeout must be above that; the `proxy-read-timeout: 120` above covers it. The SDK switches to this path on its own when a proxy answers the upgrade with an HTTP status, or from the start with `LANGWATCH_AGENT_TRANSPORT=http`.

Redis is required for more than one App replica. Instance presence and call dispatch live in Redis; without it the registry is in memory, correct for one replica only, and the App refuses a connection when `LANGWATCH_APP_REPLICAS` is above 1.

That variable defaults to `1`, and Helm scales the App with `app.replicaCount` on its own, so a Helm deployment must set `LANGWATCH_APP_REPLICAS` to the same value as `app.replicaCount`.

Without Redis, a mismatch fails in one of two ways. If `app.replicaCount` is above `LANGWATCH_APP_REPLICAS`, each pod runs its own in-memory registry, and an agent connected to one pod reads as Offline on the others. If `LANGWATCH_APP_REPLICAS` is above `app.replicaCount`, the App refuses every agent connection, also when only one pod runs.
