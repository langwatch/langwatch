...
A license is not permission to run the software. It buys the Enterprise security surface and the support relationship.
...
The Apache 2.0 floor and the Enterprise line are drawn per directory in the repository. See [Open-core licensing](/ai-governance/open-core-licensing) for the file-level map.
...
This is also the recovery path for a deployment whose only administrators sign in through SSO: it needs no working login to take effect.
</Step>
