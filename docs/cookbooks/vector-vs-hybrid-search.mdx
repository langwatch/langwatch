...
keywords:
  [vector search, hybrid search, semantic search, lexical search, information retrieval, AI search]
---
...
We'll use LanceDB to store our product data and embeddings. LanceDB provides both vector and hybrid search capabilities within one single API.
...
LanceDB combines vector search with full-text search in a single query. Let's see how this works with a practical example:
...
3. Organizes the results in a DataFrame for comparison
