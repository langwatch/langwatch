...
description: Embedding model tuning for better retrieval in RAG systems, covering model selection, dimensionality, and domain-specific tuning.
keywords:
  [
    RAG embeddings,
    optimize embeddings,
...
Semantic similarity is not the same as task relevance. That’s why fine-tuning your embedding model, even on a small number of labeled pairs, can make a noticeable difference. I’ve often seen teams improve their recall by 10-15% by fine-tuning their embedding models with just a couple hundred examples.
...
The key to running quick experiments is to parametrize the retrieval pipeline. This lets you swap different retrieval methods as your RAG system evolves. Let's start by defining the metrics we want to track.
...
Now we can set up our parametrized retrieval pipeline. I'll vary the number of retrieved documents to see how it affects recall and MRR. Note that you can vary other parameters (like the embedding models or rerankers) as well with this parametrized pipeline.
...
We’ll be using sentence-transformers, it plays nicely with Hugging Face, and has plenty of community examples if you get stuck. Let's first transform our data in the format that sentence-transformer expects it.
