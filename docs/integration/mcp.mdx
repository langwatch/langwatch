...
- **Remote (HTTP/SSE)**: For web-based assistants (ChatGPT, Claude Chat). Hosted at `https://mcp.langwatch.ai`. Supports both Bearer token auth (ChatGPT) and OAuth client_credentials (Claude Chat) — each user brings their own key.
