...
<Tab title="Cursor">
  1. Open Cursor Settings 2. Navigate to the **Tools and MCP** section in the sidebar 3.
  Add the LangWatch MCP server. Under the `mcpServers` object, add an entry named
  `langwatch` with these fields: - `command`: `npx` - `args`: `-y`, then the package name
  `@langwatch/mcp-server` on a separate token - `env.LANGWATCH_API_KEY`: your LangWatch
  API key
</Tab>
...
<Info>
  Organization management tools require an API key with org-level permissions. If the
  configured key lacks organization permissions, these tools will return a clear error
  message.
</Info>
...
<Info>
  Your AI assistant will automatically choose the right tools based on your request. You
  don't need to call these tools manually.
</Info>
