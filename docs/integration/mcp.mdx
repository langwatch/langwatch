...
See the [API Keys guide](/platform/api-keys#create-a-personal-key) for step-by-step instructions.
</Step>
...
_Requires a paid Claude plan._
...
```bash
claude mcp add langwatch --env LANGWATCH_API_KEY=your-api-key-here -- npx -y @langwatch/mcp-server
```
...
<Tab title="Cursor">
  1. Open Cursor Settings 2. Navigate to the **Tools and MCP** section in the sidebar 3. Add the
  LangWatch MCP server. Under the `mcpServers` object, add an entry named `langwatch` with these
  fields: - `command`: `npx` - `args`: `-y`, then the package name `@langwatch/mcp-server` on a
  separate token - `env.LANGWATCH_API_KEY`: your LangWatch API key
</Tab>
...
_Requires a Plus or Team plan._
...
Ask your AI assistant to write scenario tests for your agents:
...
Ask your AI assistant to add LangWatch tracking to your existing code:
...
An AI agent can use the LangWatch MCP to instrument itself while it is being built, which lets it track and debug its own behavior.
...
| Tool | Description |
|------|-------------|
| `platform_list_prompts` | List all prompts in the project |
| `platform_get_prompt` | Get a prompt with messages, model config, and version history |
| `platform_create_prompt` | Create a new prompt with messages and model configuration |
| `platform_update_prompt` | Update a prompt or create a new version |

### Evaluations (requires API key)

| Tool | Description |
|------|-------------|
| `platform_list_evaluators` | List the saved evaluators in the project |
| `platform_get_evaluator` | Get a saved evaluator with its type, settings and input mappings |
| `platform_create_evaluator` | Create a saved evaluator |
| `platform_update_evaluator` | Update a saved evaluator's settings or mappings |
| `platform_delete_evaluator` | Delete a saved evaluator |
| `platform_list_monitors` | List the online evaluations |
| `platform_get_monitor` | Get an online evaluation with its evaluator, preconditions and sampling |
| `platform_create_monitor` | Create an online evaluation |
| `platform_update_monitor` | Update, enable or disable an online evaluation |
| `platform_delete_monitor` | Delete an online evaluation |
| `platform_experiment_list` | List the experiments in the project |
| `platform_run_experiment` | Start an experiment run |
| `platform_experiment_status` | Get the progress of a run |
| `platform_experiment_list_runs` | List the runs of an experiment |
| `platform_experiment_results` | Get the per-row results of a run |

Dataset tools are listed on the [datasets page](/datasets/programmatic-access#mcp-tools-ai-coding-agents). Annotation tools are `platform_list_annotations`, `platform_get_annotation`, `platform_create_annotation` and `platform_delete_annotation`.
...
<Info>
  Organization management tools require an API key with org-level permissions. If the configured key
  lacks organization permissions, these tools will return a clear error message.
</Info>
...
<Info>
  Your AI assistant will automatically choose the right tools based on your request. You don't need
  to call these tools manually.
</Info>
