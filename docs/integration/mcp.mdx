...
4. In **Advanced settings**, set **OAuth Client ID** to `langwatch` and **OAuth Client Secret** to your LangWatch API key (get one at [app.langwatch.ai/authorize](https://app.langwatch.ai/authorize))
...
- **Remote (HTTP/SSE)**: For web-based assistants (ChatGPT, Claude Chat). Hosted at `https://app.langwatch.ai`. Supports Bearer token auth (ChatGPT via `/sse`) and OAuth client_credentials (Claude Chat via `/mcp`) — each user brings their own key.
