...
langwatch scenario create "refund flow" \
  ...
  --folder "Checkout"                         # file it into a test suite
langwatch scenario run <scenario-id> --target http:<agent-id>
```

A **test suite** is a folder of scenarios: a name and the scenarios you file under it. Every project has a `Default` suite, so no scenario is loose.

```bash
langwatch suite list
langwatch suite create "Checkout"
langwatch suite get <id|name>
langwatch suite rename <id|name> "Checkout regression"
langwatch suite archive <id|name>
langwatch suite run <id|name> --target http:<agent-id>   # every scenario × target
```
...
```bash
langwatch run-plan run --target http:<agent-id> --suite "Checkout" \
...
| Flag | Effect |
|---|---|
| `--target <type:id>` | What to run against, repeatable. `http:`, `code:` and `workflow:` take an agent id, `prompt:` takes a prompt id. |
| `--all`, `--suite`, `--label`, `--scenario` | The scope. Exactly one kind per run; `--suite`, `--label` and `--scenario` repeat. |
...
`langwatch suite run` and `langwatch scenario run` are shorter forms of the same thing: they run a plan named after the suite or the scenario and the target, and take `--name`, `--repeat`, `--param`, `--note` and `--wait` the same way.
...
```bash
langwatch suite run <suite-id> --target http:<agent-id> --param account_tier=platinum --param region=eu-central
...
- `run-plan run`, `suite run` and `scenario run` supply the parameters that the scenarios declare. A name that no scenario in the run declares is rejected before anything is scheduled. See [Scenario run parameters](/agent-simulations/scenario-parameters).
