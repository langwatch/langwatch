...
Team management is off by default too. To create teams and add or remove their members from the CLI, log in with `langwatch login --device --manage-teams`. The authorize screen then includes team management, which needs the organization as the key's scope and is granted only if you can manage teams in the organization. A team command refused on a login without it names this command.
