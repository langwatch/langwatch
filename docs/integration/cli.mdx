...
Ask Langy for a code change in the LangWatch panel first. Its request appears in the terminal, and you approve it there. The command then prints what Langy does, one line per call:
...
Paths outside the folder, `sudo` and files that may hold secrets are refused whatever you answer. A grant lives with this run of the command: stop it and start it again, and Langy asks once more.
