...
`langwatch agent dev` (alias `langwatch agent tunnel`) fronts a local port with a public tunnel, points a registered HTTP agent's URL at it, and restores the previous URL on Ctrl-C.

```bash
langwatch agent dev --port 8010
langwatch agent dev --port 8010 --agent <id or name>
```
...
| `--wait`                                         | Block until the batch is complete, and exit non-zero when a run failed.                                                                                                                                                                          |
