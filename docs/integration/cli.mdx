...
| Flag | Effect |
|---|---|
| `--target <type:id>` | What to run against, repeatable. `http:`, `code:` and `workflow:` take an agent id, `prompt:` takes a prompt id. A `?key=value` suffix gives that target its own parameter values. |
