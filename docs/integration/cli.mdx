...
Paths outside the folder and `sudo` are refused whatever you answer. A file that may hold secrets always asks, whether Langy reads it directly or through a shell command, and a command runs with the paths, the shell and the language of your machine rather than with every variable your terminal holds. A grant lives with this run of the command: stop it and start it again, and Langy asks once more.
