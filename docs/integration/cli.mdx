...
Paths outside the folder and `sudo` are refused whatever you answer. A file that may hold secrets always asks, whether Langy reads it directly or through a shell command: `.env` and `.envrc`, keys and keystores, `.netrc`, `.npmrc`, `.pgpass`, `.git-credentials`, anything named for a token or a secret, `.git/config`, and anything under `.ssh/`, `.aws/` or `.docker/`. A committed `.env.example` still reads without a question. A grant lives with this run of the command: stop it and start it again, and Langy asks once more.
...
Everything else your terminal holds stays behind, and a name that ends in `_KEY`, `_TOKEN`, `_SECRET` or `_PASSWORD` never passes even when it belongs to one of those families. Your project reads its own `.env` file itself, so nothing in the project depends on what the command line hands over.
