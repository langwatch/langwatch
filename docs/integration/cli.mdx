...
Every run command takes a repeatable `--param key=value`. The values are the constants a run shares: a fixture id, a tenant, a plan, a region.
...
A value is read as the type it looks like. Exactly `true` or `false` is a boolean, and a number that renders back to exactly what you typed is a number. Everything else stays text, which is what keeps `007`, `1.50` and a long account number as the identifiers they are. Repeating a name keeps the last value.

Where the values land depends on the command:

- `suite run` and `scenario run` supply the parameters the scenarios declare. A name no scenario in the run declares is rejected before anything is scheduled. See [Scenario run parameters](/agent-simulations/scenario-parameters).
- `experiment run` merges them into every dataset row, the same as the `parameters` field of the [experiments run API](/evaluations/experiments/ci-cd#choosing-what-to-evaluate).
- `workflow run` sends them as entry inputs, merged over `--input`, so a `--param` pair wins where both name the same key.
...
The CLI is human-first (tables, colour, spinners in a terminal) and agent-perfect: driven by an AI coding assistant it switches to machine output automatically. Everything an agent needs to know is also built into the CLI itself, and `langwatch help agent-mode` prints the condensed version of this section.
...
Non-TTY invocations (agents, pipes, CI) are served by the background daemon described below, and the output is identical, just fast. `LANGWATCH_NO_DAEMON=1` opts out per invocation.
