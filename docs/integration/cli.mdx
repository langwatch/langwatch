...
A **connected** agent registers itself: the process that runs the agent opens the connection and reports its name, its environment and its parameters. The CLI creates no row for it. See [Connect your agent](/agent-simulations/connect-your-agent).
...
A connected agent needs none of this: it reaches out from your machine, so a local process is already a target. See [Other ways to connect](/agent-simulations/other-ways-to-connect) for when each one applies.
...
- `run-plan run`, `test-suite run` and `scenario run` supply the parameters that the scenarios declare. A name that no scenario in the run declares is rejected before anything is scheduled. See [Scenario run parameters](/agent-simulations/scenario-parameters).
