...
<Warning>
  The macOS binaries are **not yet codesigned or notarized**. Gatekeeper will refuse to
  run them with *"langwatch cannot be opened because the developer cannot be verified"*.
</Warning>
...
<Note>
  ...
  `SHA256SUMS`. Signing and notarization are tracked as follow-up work; until then, `npm
  install -g langwatch` avoids Gatekeeper entirely and is the smoother path on macOS.
</Note>
...
<Tip>
  If you're inside an assistant with no shell (for example, a chat-only environment), the
  same content is available over plain HTTP, append `.md` to any documentation path, e.g.
  `https://langwatch.ai/docs/integration/python/guide.md`. Indexes:
  [docs](https://langwatch.ai/docs/llms.txt),
  [scenarios](https://langwatch.ai/scenario/llms.txt).
</Tip>
...
<Note>
  ...
  model-provider`, and cache rules over [REST](/ai-gateway/api/management#cache-rules) or
  the dashboard.
</Note>
