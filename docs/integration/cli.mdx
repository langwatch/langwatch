...
description: The `langwatch` CLI is a single tool that drives LangWatch from the terminal, for you and for your coding assistant. Instrument code, version prompts, run scenarios, inspect traces, query analytics, and more.
---

import AgentReportNotice from "/snippets/agent-report-notice.mdx";

The CLI is designed to be driven by a coding assistant, and it reaches everything in the LangWatch app except cache rules, which you manage over [REST](/ai-gateway/api/management#cache-rules) or in the dashboard.

Every subcommand supports `--help`, and every command accepts the output-contract flags (`-o json`, `--json <fields>`, `--jq <expr>`, `--limit <n>`; see [Agent usage](#agent-usage)). Run `langwatch --help` to see the full command tree.
...
<Warning>
  The macOS binaries are **not yet codesigned or notarized**. Gatekeeper will refuse to run them
  with *"langwatch cannot be opened because the developer cannot be verified"*.
</Warning>
...
Then run the binary as normal. (If `xattr` reports _"No such xattr"_, the attribute was never set, so no action is needed.)

<Note>
  Only clear quarantine on a binary whose checksum you have verified against the release's
  `SHA256SUMS`. Signing and notarization are tracked as follow-up work; until then, `npm install -g
  langwatch` avoids Gatekeeper entirely and is the smoother path on macOS.
</Note>
...
2. **How do you want to use it?**: three options:
   - **AI tools, agentic flows**: `claude`, `codex`, `copilot`, `code`, `cursor`, `gemini`, `opencode`. Mints an OAuth-style device session in `~/.langwatch/config.json` (user-scoped) so `langwatch claude` etc. wrap any tool through your gateway.
...
- **Permissions**: everything you can do day to day (traces, datasets, prompts, evaluations, the AI Gateway, model providers). Management access stays off by default: the key cannot manage teams, or manage the organization's settings, members and roles. It can never delete the organization.

To manage those from the CLI, log in with `langwatch login --device --management`. The authorize screen then shows a management access request listing what it adds. It needs the organization as the key's scope, and the key gets only the management permissions you hold in the organization; if you hold none, the approval is refused. Any command refused on a login key for one of these permissions names this re-login command.
...
| User-global | `~/.langwatch/config.json` (mode `0600`) | Device session (`access_token` + `refresh_token`), control-plane URL, gateway URL, default org, used by `langwatch claude/codex/copilot/code/cursor/gemini/opencode`, `langwatch instrument`, `langwatch whoami` |
...
A device login's key is only sent to the address the login was made against. A project's `.env` can set `LANGWATCH_ENDPOINT`, so the CLI never pairs your login with an address a folder names.

When `LANGWATCH_ENDPOINT` names another address than your login's, a command uses the API key given for that address (`LANGWATCH_API_KEY` or `--api-key`). With no key it stops and names both addresses. Run `langwatch login --device` with the variable set to sign in there, or unset it to use the login you have.
...
<Tip>
  If you're inside an assistant with no shell (for example, a chat-only environment), the same
  content is available over plain HTTP, append `.md` to any documentation path, e.g.
  `https://langwatch.ai/docs/integration/python/guide.md`. Indexes:
  [docs](https://langwatch.ai/docs/llms.txt), [scenarios](https://langwatch.ai/scenario/llms.txt).
</Tip>
...
langwatch agent list --format json
langwatch agent get <id>
```

| Column        | Value                                                                         |
| ------------- | ----------------------------------------------------------------------------- |
| `Name`        | The agent's name                                                              |
| `Environment` | Which environment the row belongs to, for a connected agent                   |
| `Status`      | `Online` with the instance count, or `Offline` with the time it was last seen |
| `Type`        | `connected`, `http`, `code` or `workflow`                                     |
| `ID`          | The agent id, which every `--target` accepts                                  |
| `Owner`       | The person or the machine a personal development agent belongs to             |
| `Updated`     | When the agent was last changed                                               |

`--format json` adds the parameters the agent declares and the instances that serve it.

A **connected** agent registers itself: the process that runs the agent opens the connection and reports its name, its environment and its parameters. The CLI creates no row for it. See [Connect your agent](/agent-testing/connect-your-agent).
...
langwatch agent update <id> --config '{"url": "https://staging.example.com/v2/chat"}'
langwatch agent run <id> --input '{"message": "where is my order?"}'
langwatch agent delete <id>
```
...
langwatch agent tunnel --port 8010 --agent <id or name>
```

| Flag                   | Effect                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------- |
| `--port <port>`        | Tunnel `localhost:<port>`.                                                                   |
| `--url <url>`          | Tunnel a full local URL, path included.                                                      |
| `--agent <id or name>` | Which registered HTTP agent to repoint. A picker opens when omitted.                         |
| `--tunnel-url <url>`   | Use a tunnel you already run and skip provisioning one. Also skips the session secret proxy. |
| `--no-update-url`      | Print the tunnel URL and leave the agent configuration as it is.                             |
| `--no-auth`            | Skip the session secret proxy, for a server that authenticates every request itself.         |

The command is a live session with no machine-readable output, so it refuses `-o json`.

A connected agent needs none of this: it reaches out from your machine, so a local process is already a target. See [Other ways to connect](/agent-testing/other-ways-to-connect) for when each one applies.
...
```bash
langwatch langy --share-control
```

Ask Langy for a code change in the LangWatch panel first. Its request appears in the terminal:

```
╭─ Langy wants to work in acme-support ───────────────────────────────────────╮
│                                                                             │
│   Instrument traces with LangWatch                                          │
│   Project ACME Support, asked 2 minutes ago.                                │
│   /Users/dev/acme-support                                                   │
│                                                                             │
│   Do you want to share this folder?                                         │
│ ❯ 1. Share this folder with Langy                                           │
│   2. Cancel this request                                                    │
│                                                                             │
│   Enter or a number to answer · ↑↓ to choose · Esc to leave the request     │
│   open                                                                      │
╰─────────────────────────────────────────────────────────────────────────────╯
```

When two conversations asked for the same folder, you pick one first. After you share it, the command prints what Langy does, one line per call:
...
Reading files, running read-only commands and ordinary git work need no answer. Everything else stops and asks:

```
╭─ Langy wants to run in acme-support ────────────────────────────────────────╮
│                                                                             │
│   uv run pytest                                                             │
│   This runs checks. Stops after 5 minutes if it has not finished.           │
│                                                                             │
│   Do you want to allow this?                                                │
│ ❯ 1. Yes, allow "uv run" for this session                                   │
│   2. Yes, this time only                                                    │
│   3. No, and tell Langy what to do instead                                  │
│                                                                             │
│   The session grant covers every command that starts with "uv run".         │
│                                                                             │
│   Enter or a number to answer · ↑↓ to choose · Esc to deny · or answer on   │
│   the card in LangWatch                                                     │
╰─────────────────────────────────────────────────────────────────────────────╯
```

A number answers at once. The arrow keys move the highlight and Enter confirms it, and Escape denies with no reason. Choosing the third option reads one line of text and sends it to Langy as the reason, so you can say what to do instead.

The same question also appears as a card in the LangWatch panel, and the first answer wins. When the card is answered first, the selector closes and the line under the call says the answer came from the card.

With the output piped or redirected there is no selector, because there is no terminal to draw it on. The command prints the question and the card in the panel is the only way to answer it.

A session grant is the program and its first argument, so `uv run` allows the tests and not `uv pip install`, and `.venv/bin/python -c` allows a one-line program and not every python command. The line under the options says what it covers.
...
- the shell and the locale: `PATH`, `HOME`, `SHELL`, `USER`, `TMPDIR`, `TERM`, `LANG`, and the `LC_*` and `XDG_*` families
- where the toolchains live: `GOPATH`, `GOROOT`, `NVM_DIR`, `PYENV_ROOT`, `VIRTUAL_ENV`, `CONDA_PREFIX`, the `HOMEBREW_*` paths, and every `*_HOME` such as `JAVA_HOME`, `PNPM_HOME` or `CARGO_HOME`
- how it reaches the network and what it trusts: `HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`, `ALL_PROXY`, `SSL_CERT_FILE`, `SSL_CERT_DIR`, `NODE_EXTRA_CA_CERTS`, `REQUESTS_CA_BUNDLE` and `CURL_CA_BUNDLE`

Everything else your terminal holds stays behind, and a name that ends in `_KEY`, `_TOKEN`, `_SECRET` or `_PASSWORD` never passes even when it belongs to one of those families. Your project reads its own `.env` file itself, so it keeps working with the smaller environment.
...
langwatch scenario create "refund flow" \
  --situation "User asks for a refund on a recent order" \
  --criteria "Agent verifies identity,Agent processes the refund" \
  --test-suite "Checkout"                     # file it into a test suite
langwatch scenario run <scenario-id> --target http:<agent-id>
```

A **test suite** groups scenarios: a name and the scenarios you file under it. Every project has a `Default` test suite, so no scenario is loose.

```bash
langwatch test-suite list
langwatch test-suite create "Checkout"
langwatch test-suite get <id|name>
langwatch test-suite rename <id|name> "Checkout regression"
langwatch test-suite archive <id|name>
langwatch test-suite run <id|name> --target http:<agent-id>   # every scenario × target
```

A **run plan** is what you run, and its name is its identity. A run under the name of a plan that exists replaces that plan's configuration and joins its history; a run under a new name creates the plan. The configuration is the scope, the targets, the repeat count and the two simulation models:

```bash
langwatch run-plan run --target http:<agent-id> --test-suite "Checkout" \
  --name "Checkout nightly" --repeat 3 --note "after the pricing change"
langwatch run-plan run --target http:<agent-id> --target prompt:<prompt-id> --all
langwatch run-plan run --target http:<agent-id> --label critical --label support
langwatch run-plan run --target http:<agent-id> --scenario <scenario-id>

langwatch run-plan list                       # the plans of the project
langwatch run-plan get <id>                   # the configuration the next run uses
langwatch run-plan archive <id>
```

| Flag                                             | Effect                                                                                                                                                                                                                                           |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--target <type:id>`                             | What to run against, repeatable. `connected:`, `http:`, `code:` and `workflow:` take an agent id, `prompt:` takes a prompt id. `connected:` also takes `<name>@<environment>`. A `?key=value` suffix gives that target its own parameter values. |
| `--all`, `--test-suite`, `--label`, `--scenario` | The scope. Exactly one kind per run; `--test-suite`, `--label` and `--scenario` repeat.                                                                                                                                                          |
| `--name <text>`                                  | The plan to run under. Left out, the platform names the plan itself.                                                                                                                                                                             |
| `--repeat <n>`                                   | How many times to run each scenario, 1 to 5.                                                                                                                                                                                                     |
| `--simulator-model`, `--judge-model`             | The model that plays the user, and the model that judges.                                                                                                                                                                                        |
| `--param key=value`                              | A value for a parameter the scenarios declare, repeatable.                                                                                                                                                                                       |
| `--note <text>`                                  | One line about this run, up to 200 characters. It belongs to the run, not to the plan.                                                                                                                                                           |
| `--idempotency-key <key>`                        | Repeat it to make a retry join the first run instead of starting a second one.                                                                                                                                                                   |
| `--wait [minutes]`                               | Block until the batch is complete, for up to 45 minutes or the number of minutes given, and exit non-zero when a run failed.                                                                                                                      |

A target is written `<type>:<referenceId>`. A **connected** agent takes a second form, `connected:<name>@<environment>`, which reads better in a CI job than an id and survives a project rebuild:

```bash
langwatch run-plan run --test-suite "Checkout" \
  --target connected:support-agent@production \
  --target connected:support-agent@staging --wait
```

Every connected target must be online when the run starts. A run naming an offline agent is refused with `agent_offline` before any scenario is scheduled. A personal development agent is refused with `agent_owner_only` for anyone but its owner, and a project API key names nobody, so it can never target one.
...
The `/api/v1/suites` endpoints the older CLI versions called still answer and are deprecated. The commands above use `/api/v1/run-plans` and `/api/v1/test-suites`.
...
langwatch run-plan run --test-suite "Checkout" \
  ...
  --name "Checkout model comparison" --wait

langwatch run-plan run --all \
  --target http:agent_candidate \
  --target http:agent_production
```

Quote the value: the shell reads `?` and `&` itself. Both halves of a pair are percent-decoded, so a reference id or a value that holds `?` or `&` must be written as `%3F` or `%26`. A value is read as the type it looks like, the same rule `--param` uses.
...
```bash
langwatch test-suite run "Checkout" \
  --param account_tier=platinum \
  --target 'http:agent_abc123?model=gpt-5' \
  --target 'http:agent_abc123?model=gpt-5-mini'
```

Both targets run with `account_tier=platinum`, and each one with its own `model`.
...
```bash
langwatch test-suite run <test-suite-id> --target http:<agent-id> --param account_tier=platinum --param region=eu-central
...
- `run-plan run`, `test-suite run` and `scenario run` supply the parameters that the scenarios declare. A name that no scenario in the run declares is rejected before anything is scheduled. See [Scenario run parameters](/agent-testing/run-parameters).
- `experiment run` merges the values into every dataset row. See [Running experiments in CI/CD](/evaluations/experiments/ci-cd#choosing-what-to-evaluate).
...
For the full scenario testing guide, see the [Scenarios documentation](/agent-testing/scenarios).
...
Agents use this to debug their own instrumentation: after running your code once, ask the assistant to run `langwatch trace search --limit 5` and verify traces are flowing. If no trace appears, the instrumentation is wrong, no need to read logs.
...
- **`agent`**, agent definitions used by scenarios and monitors (see [Manage agents](#manage-agents))
...
# Move a project to another team (project and team by id, slug or name)
langwatch projects move checkout --team Payments
...
A move runs the same checks as the project settings screen: the team must be an active team of the same organization, and a project cannot cross the personal workspace boundary. Gateway keys that send traces to the project switch to the new team's budgets right away.
...
langwatch teams create --name "Payments" --copy-members-from team_01HZ...
...
langwatch teams members add team_01HZ... user_01HZX... user_01HZY... --role MEMBER
langwatch teams members remove team_01HZ... user_01HZX...
```

`--copy-members-from` adds every member of the source team with the role they hold there. A member holding only a custom role is listed as not copied; bind it with `langwatch role-bindings create`. `teams members add` takes several users; one that cannot be added does not stop the rest, and the command fails naming it.
...
`langwatch doctor` runs the same checks as **Settings, Checkup** on the install the CLI is pointed at, and prints one line per row with `PASS`, `FAIL` or `NOT CHECKED`, the fix under a fail, and then the exact usage report the install sends to LangWatch.

```bash
export LANGWATCH_ENDPOINT=https://langwatch.acme.internal
export LANGWATCH_API_KEY=...        # a project API key of the install

langwatch doctor                    # the free checks and the usage report
langwatch doctor --run              # also reach the LangWatch hosts, write to storage, verify SMTP, test a model provider, run the canaries
langwatch doctor --run --scenario-run-plan-id smoke-suite
langwatch doctor --format json      # the rows and the report as one document
```

The checks live on the install, behind `GET /api/checkup` and `POST /api/checkup/run`; the command asks and prints, so the page and the terminal cannot disagree. `--run` opens connections and can spend money on a judge or a model, which is why it is a flag. See [Data and telemetry](/self-hosting/data-and-telemetry) for what the report carries.
...
# Create for someone else: the secret is not printed, the app shows it to them once
langwatch virtual-keys create --name prod-key --reveal-once
...
`--reveal-once` withholds the secret from the output and returns a `reveal_id` and the key's prefix instead. The secret is stored encrypted for 24 hours and can be read exactly once through the app, which is how Langy shows a key without ever printing it.
...
A budget counts spend from the moment it is created: spend earlier in the current window is not counted against it. The budget commands take a project key or an organization key; an organization key needs no project.
...
<Note>
  Provider credentials and cache rules have no CLI group. Manage providers with `langwatch
  model-provider`, and cache rules over [REST](/ai-gateway/api/management#cache-rules) or the
  dashboard.
</Note>
...
`trace export -o` is an output _file_, the coding-assistant wrappers pass flags
...
| `--jq <expr>`           | Project the result before printing: dot paths, `.items[]`, `.items[].field`, `.items[0]` (negative indexes count from the end), and `length` with or without a `\|` in front |
| `--limit <n>`           | Keep at most n rows of the result, in the machine formats. A command that pages with a `--limit` of its own keeps that one                                                   |

```bash
langwatch trace search --jq '.traces[].traceId'
langwatch prompt list --jq length          # how many, in one call
langwatch prompt list --limit 5 -o json    # the first five rows only
...
Non-interactive invocations (an agent piping output, CI, any call whose stdin/stdout/stderr is not a TTY) are served by a warm background daemon instead of paying node's cold start on every call. Interactive (terminal) invocations always run in-process and are unaffected.

So are the commands that mutate auth or take over stdio (`login`, `logout`, `config`, `open`, `instrument`, `report`, the `claude`/`codex`/`copilot`/`code`/`cursor`/`gemini`/`opencode` wrappers, `daemon` itself) and the long-running flags (`--follow`, `--watch`, `--wait`). Windows is excluded too: the daemon is not supported there, so on Windows every invocation runs in-process.
