...
`--scope` takes `type:id` pairs (`org`, `team`, or `project`) and repeats for several; it defaults to the calling project. `--providers-allowed` is a comma-separated list of ModelProvider ids. The atomic `--budget-*` flags cap the key itself and accept `day`, `week`, or `month`. `--reveal-once` withholds the secret from the output and returns a `reveal_id` and the key's prefix instead: the secret is stored encrypted for 24 hours and can be read exactly once through the app, which is how Langy shows a key without ever printing it.
