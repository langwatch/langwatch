...
description: LangWatch receives traces from an existing OpenTelemetry setup in Python and TypeScript.
keywords: OpenTelemetry, OTel, auto-instrumentation, LangWatch, Python, TypeScript, observability, tracing
---

The LangWatch SDKs are built entirely on top of the [OpenTelemetry (OTel)](https://opentelemetry.io/) standard. They integrate with existing OTel setups and interoperate with the wider OTel ecosystem across both Python and TypeScript environments.
...
- **Reuse Existing Setup:** LangWatch integrates with your existing OTel `TracerProvider` and instrumentation.

**LangWatch also works with the standard OpenTelemetry auto-instrumentation libraries.** You can combine LangWatch's LLM-specific observability with insights from other parts of your application stack.

## Use the OpenTelemetry Ecosystem: Auto-Instrumentation

The OpenTelemetry foundation gives LangWatch **automatic compatibility with the ecosystem of OpenTelemetry auto-instrumentation libraries.**
...
const handle = setupObservability({
  ...
  spanProcessors: [
    ...
    new BatchSpanProcessor(
      new JaegerExporter({
        endpoint: "http://localhost:14268/api/v1/traces",
      }),
    ),
...
const handle = setupObservability({
  ...
  serviceName: "my-service",
  spanProcessors: [new ConsoleSpanExporter()],
  debug: {
    consoleTracing: true,
    consoleLogging: true,
    logLevel: "debug",
