...
<ParamField path="max_string_length" type="Optional[int]" default="None">
  Optional per-field byte cap for string values in inputs, outputs, and metadata. Defaults to `None` (no client-side truncation), so full content is sent to the backend, which owns sizing. Set to an integer (≥ 100) to truncate longer strings client-side before transmission.
</ParamField>

<Warning>
  **Upgrade note (behavior change):** earlier SDK versions truncated string values at 5,000 bytes by default. Now that `max_string_length` defaults to `None`, full input/output is transmitted, so projects **without** server-side blob offload enabled send larger network payloads than before. If you need to bound client-side payload size, set `max_string_length` to an integer.
</Warning>
...
<ResponseField name="started_at" type="Optional[int]">
  Timestamp when the span started (milliseconds since epoch). Falls back to the span's actual start time.
</ResponseField>
<ResponseField name="first_token_at" type="Optional[int]">
  For LLM operations, timestamp when the first token was received (milliseconds since epoch). Powers the Time to First Token (TTFT) metric, see [Tracking Time to First Token](/integration/python/tutorials/tracking-time-to-first-token).
</ResponseField>
<ResponseField name="finished_at" type="Optional[int]">
  Timestamp when the span finished (milliseconds since epoch). Falls back to the span's actual end time.
</ResponseField>
