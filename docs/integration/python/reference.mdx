...
A dictionary defining custom timestamps for a span, in **milliseconds** since epoch.

<ResponseField name="started_at" type="int" required>
  Timestamp when the span started (milliseconds since epoch).
</ResponseField>
...
<ResponseField name="finished_at" type="int" required>
  Timestamp when the span finished (milliseconds since epoch).
</ResponseField>
