...
LangWatch integrates with a range of Python libraries and frameworks. These integrations instrument your code automatically and capture data from your LLM applications with minimal setup.
...
<AccordionGroup>
  <Accordion title="How do I track LLM costs and token usage?">
    LangWatch captures cost and token data automatically. If you're missing costs or token counts, our [cost tracking tutorial](/integration/python/tutorials/tracking-llm-costs) covers troubleshooting steps, model cost configuration, and manual token tracking setup.
  </Accordion>
