...
sidebarTitle: Python
description: The LangWatch Python SDK attaches custom metadata and attributes to traces and spans.
icon: python
...
This tutorial walks you through capturing both types using the Python SDK.
...
Span attributes provide context for a specific operation or unit of work _within_ a trace. They are useful for details that are relevant only to that particular step. Examples include:
