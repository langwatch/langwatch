...
<Warning>
  Assigning to the dict directly, e.g.
  `langwatch.get_current_span().timestamps["first_token_at"] = ...`, does **not** work.
  The timestamps are only exported to the underlying OpenTelemetry span when they go
