...
description: LangWatch logs custom evaluations, triggers managed evaluations, and runs guardrails.
...
- [`LangWatchSpan.add_evaluation()`](/integration/python/reference#langwatchspan-object-methods)
- [`LangWatchTrace.add_evaluation()`](/integration/python/reference#langwatchtrace-object-methods)

## 2. Server-Side Managed Evaluations (`evaluate` & `async_evaluate`)

LangWatch allows you to trigger evaluations that are performed by the LangWatch backend. These can be [built-in evaluators](/evaluations/evaluators/list) (e.g., for faithfulness, relevance) or [custom evaluators you define](/evaluations/evaluators/custom-scoring) in your LangWatch project settings.
...
- [`LangWatchSpan.evaluate()`](/integration/python/reference#langwatchspan-object-methods) and [`LangWatchSpan.async_evaluate()`](/integration/python/reference#langwatchspan-object-methods)
- [`LangWatchTrace.evaluate()`](/integration/python/reference#langwatchtrace-object-methods) and [`LangWatchTrace.async_evaluate()`](/integration/python/reference#langwatchtrace-object-methods)
...
<Tip>
  **Understanding the `data` Parameter:**
  ...
  For the `data` parameter specifically: `BasicEvaluateData` gives `input`, `output` and `contexts` a standardized structure, which is what many built-in evaluators expect. But `data` can be **any dictionary**, so you can pass arbitrary structures tailored to the custom server-side evaluators you define.

  `BasicEvaluateData` with fields like `expected_output` is useful when [evaluating if the LLM is generating the right answers](/evaluations/experiments/ui/answer-correctness) against a set of expected outputs. Where a golden answer is not available, use an open-ended evaluation instead, such as [LLM-as-a-judge](/evaluations/experiments/ui/llm-as-a-judge).
</Tip>
...
```
`as_guardrail=True` changes what happens when the *evaluation process itself* fails, for example when the evaluator service is down. The result carries `status="error"` and `passed` defaults to `True`, so a broken guardrail does not break your application. That is a "pass by default on error" stance, which is the right one when the guardrail is advisory rather than a hard gate.

For safety-focused real-time evaluations such as PII detection or prompt injection monitors, see [Setting up Real-Time Evaluations](/evaluations/online-evaluation/setup-monitors).
