...
A callback attached only to the model call inside one node produces LLM spans and nothing else. The tool nodes and the plain function nodes never appear, so nothing in the trace shows which step the graph actually ran.
