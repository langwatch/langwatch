...
does not touch the global one, so each SDK operates independently.
...
<Note>
The Vercel AI SDK's `@ai-sdk/otel` integration sends spans to the global
provider by default, but it accepts a custom tracer. Point it at the isolated
provider instead of falling back to manual spans:
...
registerTelemetry(
  new OpenTelemetry({ tracer: lwProvider.getTracer("gen_ai") }),
);
```

This keeps the AI SDK's standard GenAI attributes and routes its spans to the
dedicated provider. `lwProvider.getTracer("gen_ai")` only reconfigures
`@ai-sdk/otel` — it does not retarget other auto-instrumentations. Anything
else that emits through the global OTel API needs its own provider
configuration, or manual spans via `lwProvider.getTracer()` as shown above.
</Note>
...
receives **all** spans, so use the `LangWatchTraceExporter` filter options to
