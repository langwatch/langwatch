...
<Note>
Auto-instrumentations that emit spans through the global OTel API send spans to
the global provider, not the dedicated one. The Vercel AI SDK's `@ai-sdk/otel`
integration takes a `tracer` option, so point it at the isolated provider:
...
Without the `tracer` option the integration defaults to
`trace.getTracer("gen_ai")` on the global provider. For instrumentations that
expose no such option, use `lwProvider.getTracer()` directly to create LLM spans
on the isolated provider.
</Note>
