...
<Note>
  The `@ai-sdk/openai` and `ai` packages are only required for the example in this guide.
  You can skip this step if you're only looking to install the LangWatch SDK.
</Note>
...
<Note>
  ...
  multi-project setups). Project API keys obtained via `npx langwatch login` or from the
  project settings page already have the project context built in.
</Note>
...
<Tip>
  ...
  Conventions](/integration/typescript/tutorials/semantic-conventions) - standardized
  naming guidelines for attributes and metadata.
</Tip>
...
<Note>
  ...
  documentation](/integration/typescript/integrations). Each integration includes
...
<Note>
  ...
  Conventions](/integration/typescript/tutorials/semantic-conventions) guide. For advanced
  span management techniques, check out [Manual
...
<Warning>
  Earlier SDK versions called `process.exit(0)` as soon as LangWatch's own flush finished,
  which ended the process out from under any other `SIGINT` / `SIGTERM` listener. If your
  application relied on that to terminate (because it registers a signal handler that
  never exits the process itself), either exit at the end of your own handler
  (recommended) or set `advanced.UNSAFE_exitProcessAfterAutoShutdown: true` to restore the
...
<Tip>
  Always call `shutdown()` before your application exits to prevent data loss. The method
  is safe to call multiple times.
</Tip>

<Warning>
  If you don't call `shutdown()`, some traces may be lost when your application terminates
  abruptly.
</Warning>
...
<Accordion title="High memory usage">
  - Use batch processing: `processorType: 'batch'` - Implement graceful shutdown -
...
<Tip>
  Start simple and add complexity gradually. You can always add more detailed tracing
