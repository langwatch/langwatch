...
- Your **LangWatch API key** from [Settings → API Keys](https://app.langwatch.ai/settings/api-keys) (see [API Keys guide](/platform/api-keys))
...
<Note>
  The `@ai-sdk/openai` and `ai` packages are only required for the example in this guide. You can
  skip this step if you're only looking to install the LangWatch SDK.
</Note>
...
<Note>
  ...
  multi-project setups). Project API keys obtained via `npx langwatch login` or from the project
  settings page already have the project context built in.
</Note>
...
<Check>
  **What you'll see**: A trace named "ask-ai" with input/output data, timing, and status.
</Check>
...
- **Trace**: The entire `askAI` function execution
...
<Note>For detailed explanations of all concepts, see our [Concepts Guide](/concepts).</Note>

<Tip>
  For consistent observability across your application, learn about [Semantic
  Conventions](/integration/typescript/tutorials/semantic-conventions) - standardized naming
  guidelines for attributes and metadata.
</Tip>

## Integrations

LangWatch offers integrations with many popular TypeScript libraries and frameworks. These integrations provide automatic instrumentation, capturing relevant data from your LLM applications with minimal setup.
...
<Note>
For detailed integration guides, see our [integration documentation](/integration/overview). Each integration includes framework-specific examples and best practices.
</Note>
...
<Note>
  For consistent attribute naming and TypeScript autocomplete support, see our [Semantic
  Conventions](/integration/typescript/tutorials/semantic-conventions) guide. For advanced span
  management techniques, check out [Manual
...
By default, LangWatch flushes pending traces when your application receives `SIGINT` or `SIGTERM`, or when the event loop drains:
...
**LangWatch never terminates your process.** Node runs every listener registered for a
signal, so if your application has its own shutdown logic (draining a queue, finishing
in-flight database writes, closing connections), LangWatch flushes its traces and then
stands aside. Your handler decides when the process ends, and it is never cut off
part-way through.

The one exception keeps one-shot scripts working. Node skips a signal's default action
while any listener is registered, so if LangWatch's handler turns out to be the _only_
listener for that signal, it removes itself once the flush is done and re-raises the
signal. The process then ends on the signal exactly as it would have if LangWatch were
not installed.

<Warning>
  Earlier SDK versions called `process.exit(0)` as soon as LangWatch's own flush finished, which
  ended the process out from under any other `SIGINT` / `SIGTERM` listener. If your application
  relied on that to terminate (because it registers a signal handler that never exits the process
  itself), either exit at the end of your own handler (recommended) or set
  `advanced.UNSAFE_exitProcessAfterAutoShutdown: true` to restore the old behaviour while you
  migrate.
</Warning>
...
process.on("SIGINT", async () => {
  ...
  await Promise.race([handle.shutdown(), new Promise((resolve) => setTimeout(resolve, 5000))]);
  process.exit(1);
});
```
...
<Tip>
  Always call `shutdown()` before your application exits to prevent data loss. The method is safe to
  call multiple times.
</Tip>
...
<Accordion title="High memory usage">
  - Use batch processing: `processorType: 'batch'` - Implement graceful shutdown - Consider reducing
  data capture in production
</Accordion>
...
- **[Framework Integrations](/integration/overview)** - Specific guides for OpenAI, LangChain, Azure, and more

<Tip>
  Start simple and add complexity gradually. You can always add more detailed tracing later as your
  application grows!
</Tip>
