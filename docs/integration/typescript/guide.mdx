...
npm install @ai-sdk/openai ai @ai-sdk/otel
```

<Note>
The `@ai-sdk/openai`, `ai` and `@ai-sdk/otel` packages are only required for the example in this guide. You can skip this step if you're only looking to install the LangWatch SDK. On AI SDK 7, `@ai-sdk/otel` is what makes the SDK emit spans at all.
</Note>
...
async function chatWithAI(userMessage: string) {
  return await tracer.withActiveSpan("chat-with-ai", async (span) => {
    // Make the LLM call
    // Auto-captures LLM data once registerTelemetry has run at startup
    const response = await generateText({
      model: openai("gpt-5-mini"),
      prompt: userMessage,
    });
...
while any listener is registered, so if LangWatch's handler turns out to be the *only*
...
<Warning>
  Earlier SDK versions called `process.exit(0)` as soon as LangWatch's own flush
  finished, which ended the process out from under any other `SIGINT` / `SIGTERM`
  listener. If your application relied on that to terminate (because it registers a
  signal handler that never exits the process itself), either exit at the end of your own
  handler (recommended) or set `advanced.UNSAFE_exitProcessAfterAutoShutdown: true` to
