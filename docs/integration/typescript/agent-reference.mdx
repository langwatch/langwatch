...
The entry point is Node only. `connectAgent` returns the handler, so `supportAgent({ messages })` calls it directly and a unit test needs no connection. The returned function also exposes `.disconnect()`, which closes the connection and drops the agent from the presence list.
...
| `parameters` | definition map or JSON Schema | none | What the agent accepts from a run. |
...
**A definition map.** `params` is typed from it, with no schema library involved:
...
**A Standard Schema's JSON Schema.** Any library that implements Standard Schema exposes it at `schema["~standard"].jsonSchema`:
...
Pass the schema object, never the library's validator instance. The SDK does not import a schema library, which is what keeps it out of your bundle.
