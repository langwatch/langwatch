...
A trace filter decides which spans the LangWatch exporter sends. Use filters to keep AI and business spans while dropping framework chatter.
...
setupObservability({
  ...
  langwatch: "disabled",
  spanProcessors: [
    new BatchSpanProcessor(new LangWatchTraceExporter({ filters: [{ preset: "vercelAIOnly" }] })),
  ],
});
```
...
<Tip>
  Use simple matchers (`equals`, `startsWith`) where possible; regex matches more patterns but is
