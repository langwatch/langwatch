...
| Source | Signal |
|--------|--------|
| Vercel AI SDK 7+ | `gen_ai.client.operation.time_to_first_chunk` span attribute (seconds) |
| Vercel AI SDK 6 and earlier | `ai.response.msToFirstChunk` span attribute (milliseconds) |
...
<Note>
  If you stream through the Vercel AI SDK, no per-call instrumentation is needed. On AI SDK 7, register the integration once at startup with `registerTelemetry(new OpenTelemetry())` from `@ai-sdk/otel` — without it the SDK emits no spans at all. Every streamed call then reports TTFT on its own.
</Note>
