...
<Note>
  If you stream through the Vercel AI SDK with `experimental_telemetry` enabled, TTFT is captured
  automatically from `ai.response.msToFirstChunk`, nothing to do.
</Note>
  ...
  return await tracer.withActiveSpan("LLM call", async (span) => {
    ...
    for await (const chunk of stream) {
      if (firstTokenAt === null) {
        ...
        span.setAttribute("gen_ai.server.time_to_first_token", firstTokenAt - startedAt);
      }
...
<Frame>
  <img src="/images/ttft/ttft-enable-column.png" alt="Columns picker with the TTFT column" />
</Frame>
...
<Frame>
  <img src="/images/ttft/ttft-drawer.png" alt="Trace drawer header showing the TTFT pill" />
</Frame>
