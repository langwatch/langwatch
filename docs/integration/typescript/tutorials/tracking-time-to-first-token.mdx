...
<Note>
  If you stream through the Vercel AI SDK, TTFT is captured automatically, nothing to do. On AI SDK 7 this requires `registerTelemetry(new OpenTelemetry())` from `@ai-sdk/otel` at startup — without it the SDK emits no spans at all.
</Note>
