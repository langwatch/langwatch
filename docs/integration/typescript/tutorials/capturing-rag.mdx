...
async function multiSourceRAG(query: string): Promise<string> {
  return await tracer.withActiveSpan("Multi-Source RAG", async (span) => {
    ...
    const allContexts = [...vectorStoreContexts, ...databaseContexts, ...apiContexts];
...
```typescript
async function ragRetrievalWithErrorHandling(query: string): Promise<LangWatchSpanRAGContext[]> {
  return await tracer.withActiveSpan("RAG Retrieval with Error Handling", async (span) => {
    span.setType("rag");
...
<Tip>
  For production RAG applications, combine manual RAG spans with [Semantic
