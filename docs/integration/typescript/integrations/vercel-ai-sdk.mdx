...
<Note>
  On AI SDK 6 and earlier there is no `registerTelemetry`. Drop `registerTelemetry`
  from the `ai` import, drop the `@ai-sdk/otel` import and the
  `registerTelemetry(new OpenTelemetry())` call, and set
  `experimental_telemetry: { isEnabled: true }` on each `generateText` and
  `streamText` call instead. AI SDK 7 emits no spans at all until an
  integration is registered, so a v6
