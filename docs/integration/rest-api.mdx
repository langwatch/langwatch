...
If your preferred programming language or platform is not directly supported by the existing LangWatch libraries, you can use the REST API with `curl` to send trace data from any system that can make HTTP requests.
...
curl -X POST "$LANGWATCH_ENDPOINT/api/v1/collector" \
  ...
  "spans": [
    {
      ...
      "timestamps": {
        ...
        "first_token_at": $((($(date +%s) * 1000) + 300)),
...
All `timestamps` are unix epoch **milliseconds**. The optional `first_token_at` records when the first streamed token reached the user and powers the Time to First Token (TTFT) metric, see [Tracking Time to First Token](/integration/python/tutorials/tracking-time-to-first-token). Token counts and cost are derived from `metrics` and the model pricing registry, see [Tracking LLM Costs and Tokens](/integration/python/tutorials/tracking-llm-costs).

For the type reference of how a `span` should look like, check out our [types definitions](https://github.com/langwatch/langwatch/blob/main/sdks/python/src/langwatch/types.py).

It's optional but highly recommended to pass the `user_id` on the metadata if you want to use user-specific analytics and the `thread_id` to group related traces together. Read more about those and other concepts [here](../concepts).

3.  Execute the `curl` command. If successful, LangWatch will process your trace data.

This method of integration sends traces from any system capable of making HTTP requests, including a less common programming language or a custom-built platform.
