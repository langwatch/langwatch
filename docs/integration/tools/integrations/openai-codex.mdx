...
Codex tells the processes it starts nothing about itself, so the command reads which session is running from the session transcripts on the machine. One running session is found on its own. When two codex sessions are working at the same moment, nothing on the machine says which one ran the command, so it declares nothing and prints one line asking you to name the session: pass `--agent codex --session-id <id>`. A session that only ended a few minutes ago, after a codex restart, does not make the machine ambiguous, because the running session wrote to its transcript far more recently.
