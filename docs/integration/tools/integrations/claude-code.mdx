...
LangWatch joins a session to a pull request through the repository and branch the session reports. Two channels report it, and together they cover every way a session moves:

- **Automatic**: the session reports the directory it is working in, read at the start of the session and again after every turn. A native switch with the EnterWorktree tool moves it; a `cd` inside the Bash tool does not, because the session's own directory stays where it was launched.
- **Declared by the agent**: the agent runs `langwatch ingest context` from inside a checkout, and the session is attributed to that repository and branch from that point on. This covers the case the automatic channel cannot see: work done through the Bash tool in a directory the session never natively entered.

The setup installs the second channel for you. The LangWatch plugin (or, without plugin support, the hooks `langwatch instrument claude` writes) tells every session to run the command when it switches repository, branch or worktree, so a session declares its own moves without you asking it to. You can also just tell a session "declare your working context" at any time.

Work done outside any checkout, for example reviewing with `gh` commands from a home directory, attaches to no repository and no pull request, and its token cost cannot be attributed. The per pull request rollup is available over the API as [Get pull request coding agent usage](/api-reference/coding-agents/get-pull-request-coding-agent-usage).

One limit to know: a session keeps the first repository it reported. A later declaration for a different branch of the same repository is attributed in full, and the session's pull requests reflect every branch it declared. A declaration for a different repository keeps that session's row on the first repository.
