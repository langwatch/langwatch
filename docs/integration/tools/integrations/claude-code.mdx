...
#    Claude Code 2.1.x emits its cost-bearing api_request and user_prompt
#    events as OTLP LOG records (not spans), and emits periodic usage
#    metrics. LangWatch's receiver synthesizes a trace per Claude Code
#    session from the logs and lifts cost / tokens / model onto it.
export OTEL_TRACES_EXPORTER=otlp
export OTEL_LOGS_EXPORTER=otlp
export OTEL_METRICS_EXPORTER=otlp

# 3. Required so user_prompt events ship the prompt body — the
#    receiver reads it as langwatch.input on the synthesized trace.
export OTEL_LOG_USER_PROMPTS=1

# 4. Set OTLP protocol and endpoint
...
<AccordionGroup>
  ...
  <Accordion title="Trace shows but output/response is empty">
    Anthropic does NOT emit the assistant's response text in OTel as of Claude Code 2.1.x. Only token counts ship on the wire. LangWatch renders the input + cost + token breakdown but the output text stays empty until anthropic resumes emitting the response.
  </Accordion>
