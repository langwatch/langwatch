...
[OpenRouter](https://openrouter.ai) provides a unified API to access LLMs from many providers. LangWatch traces calls made through OpenRouter's OpenAI-compatible endpoint by reusing the [OpenAI](/integration/go/integrations/open-ai) `otelopenai` middleware.
...
<Info>Set `LANGWATCH_API_KEY` and `OPENROUTER_API_KEY` environment variables before running.</Info>
