...
sidebarTitle: Groq
description: Instrument Groq API calls in Go using LangWatch for LLM observability, cost tracking, and agent evaluation insights.
keywords: go, golang, groq, instrumentation, langwatch, openai-compatible
---

LangWatch traces calls to the Groq API. Groq provides an OpenAI-compatible endpoint, so you reuse the [OpenAI](/integration/go/integrations/open-ai) `otelopenai` middleware: point the client at Groq's base URL and label the spans with `WithGenAIProvider`.
...
<Info>Set `LANGWATCH_API_KEY` and `GROQ_API_KEY` environment variables before running.</Info>
