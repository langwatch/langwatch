...
go get github.com/langwatch/langwatch/sdks/go/instrumentation/azureopenai
...
<Info>
  Set `LANGWATCH_API_KEY`, `AZURE_OPENAI_API_KEY`, and `AZURE_OPENAI_ENDPOINT` environment variables
  before running.
</Info>
...
<Warning>
  The `defer tp.Shutdown(ctx)` call is essential. Without it, traces buffered in memory will be lost
