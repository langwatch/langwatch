...
<Tip>
  ...
  Guide](/integration/go/guide). For practical examples of creating traces and spans, see the [Core
  Concepts section](/integration/go/guide#core-concepts) in the guide.
</Tip>
...
<ParamField path="SetType(spanType SpanType)" type="function">
  Sets the span type for categorization in LangWatch. This enables specialized UI treatment and
  analytics. **Example:** ```go span.SetType(langwatch.SpanTypeLLM) // For LLM calls
  ...
  <Tip>
    Using span types is optional but highly recommended as it enables LangWatch to provide more
    tailored insights and visualizations.
  </Tip>
</ParamField>
...
<ParamField path="SetThreadID(threadID string)" type="function">
  ...
  **Example:** ```go span.SetThreadID("conversation-123") span.SetThreadID("user-session-abc-def")
  ```
  <Note>
    All spans within the same trace will share the same thread ID, allowing you to group related
    interactions together.
  </Note>
</ParamField>

<ParamField path="SetUserID(userID string)" type="function">
  Assigns a user ID to the trace for user-centric analytics and filtering. **Example:** ```go
  span.SetUserID("user-abc-123") span.SetUserID("customer-xyz-789") ```
</ParamField>
...
<ParamField path="RecordInputString(input string)" type="function">
  ...
  **Example:** ```go span.RecordInputString("What is the weather like today?")
...
<ParamField path="RecordOutputString(output string)" type="function">
  ...
  **Example:** ```go span.RecordOutputString("The capital of France is Paris.")
...
<ParamField path="SetRequestModel(model string)" type="function">
  Sets the model identifier used for a request (e.g., an LLM call). This is the model you requested
  to use. **Example:** ```go span.SetRequestModel("gpt-5-mini")
  span.SetRequestModel("claude-sonnet-5") span.SetRequestModel("llama-3.1-8b") ```
</ParamField>

<ParamField path="SetResponseModel(model string)" type="function">
  Sets the model identifier reported in a response. This is the actual model that processed your
  request. **Example:** ```go // Providers answer with the dated build they actually served, so this
  is // normally a longer identifier than the one you requested above.
  ...
  <Note>
    ...
    Setting both is what lets you see, later, which build produced a given output.
  </Note>
</ParamField>
...
<Tip>
  For step-by-step instructions on setting up OpenAI instrumentation, see the [OpenAI integration
  guide](/integration/go/integrations/open-ai).
</Tip>
...
<ParamField path="WithCaptureInput()" type="function">
  Records the full input payload as a span attribute. This captures the complete request sent to the
  LLM. **Example:** ```go otelopenai.Middleware("my-app", otelopenai.WithCaptureInput()) ```
  <Warning>
    Enabling input capture may include sensitive data in your traces. Ensure this aligns with your
    data privacy requirements.
  </Warning>
</ParamField>

<ParamField path="WithCaptureOutput()" type="function">
  Records the full response payload as a span attribute. For streams, this is the final accumulated
  response. **Example:** ```go otelopenai.Middleware("my-app", otelopenai.WithCaptureOutput()) ```
  <Tip>
...
<ParamField path="WithGenAISystem(system string)" type="function">
  Sets the `gen_ai.system` attribute. Useful for identifying providers like `"anthropic"` or
  `"azure"`. Defaults to `"openai"`. **Example:** ```go // For Anthropic Claude
  otelopenai.Middleware("my-app", otelopenai.WithGenAISystem("anthropic")) // For Azure OpenAI
...
<ParamField path="WithTracerProvider(provider trace.TracerProvider)" type="function">
  Specifies the `trace.TracerProvider` to use. Defaults to the global provider. **Example:** ```go
  customProvider := sdktrace.NewTracerProvider(...) otelopenai.Middleware("my-app",
...
<Note>
  Multiple filters use AND semantics (span must pass all). Within a `Criteria`, matchers use OR
  semantics (span matches if any matcher matches).
</Note>
