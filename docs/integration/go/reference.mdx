...
<ParamField path="SetRequestModel(model string)" type="function">
  ...
  **Example:**
  ...
  span.SetRequestModel("claude-sonnet-5")
...
<ParamField path="SetResponseModel(model string)" type="function">
  ...
  **Example:**
  ```go
  // Providers answer with the dated build they actually served, so this is
  // normally a longer identifier than the one you requested above.
  span.SetResponseModel("gpt-4o-mini-2024-07-18")
  ```
  
  <Note>
  The response model may differ from the request model, especially with OpenAI's model updates. Setting both is what lets you see, later, which build produced a given output.
  </Note>
</ParamField>
