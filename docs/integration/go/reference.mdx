...
<ParamField path="SetGenAIRequestParams(params GenAIRequestParams)" type="function">
  Sets `gen_ai.request.*` — temperature, top_p, max_tokens, stop, reasoning_effort, …
</ParamField>
...
| `…/instrumentation/gopenai` | `github.com/sashabaranov/go-openai` | `http.RoundTripper` | — |
...
<ParamField path="WithDataCapture(mode langwatch.DataCaptureMode)" type="function">
  Controls whether request (input) and response (output) **content** is recorded at the source. **Defaults to `langwatch.DataCaptureAll`** — input and output are captured by default.
  ...
  **Example:**
  ...
  <Note>
  This replaces the previous opt-in `WithCaptureInput()` / `WithCaptureOutput()` options, which have been **removed**.
  </Note>
</ParamField>
...
<ParamField path="WithGenAIProvider(provider attribute.KeyValue)" type="function">
  ...
  **Example:**
  ...
  <Note>
  `gen_ai.system` was removed from the GenAI semantic conventions in favour of `gen_ai.provider.name`. `WithGenAISystem` remains only as a deprecated alias for `WithGenAIProvider`.
  </Note>
</ParamField>
