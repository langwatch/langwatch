...
1. **First response:** Explore the codebase silently (read files, check prompts, search traces, check git log). Then summarize what you found and ask what you still need. Where `langy-card` blocks render, each question is a `choices` block and they arrive one at a time — a numbered prose interview is unanswerable there, and the absolute rules against prose questions win over this step's wording. Elsewhere, 2-3 targeted questions:
...
Based on discovery, present a structured plan, and ask for confirmation in the surface's answerable format — where `langy-card` blocks render, a `choices` block rather than a prose "shall I proceed?", which is a question the reader cannot click.
