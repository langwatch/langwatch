...
- **Empty input/output**: The most common issue. Check if `autotrack_openai_calls(client)` (Python) or the telemetry integration (TypeScript/Vercel AI) is configured. On AI SDK 7 that means installing `@ai-sdk/otel` (`npm install @ai-sdk/otel`) and calling `registerTelemetry(new OpenTelemetry())` from it at startup; on AI SDK 6 and earlier, `experimental_telemetry: { isEnabled: true }` per call.
...
| Issue | Cause | Fix |
|-------|-------|-----|
| All traces show `<empty>` input/output | Missing autotrack or telemetry config | Add `autotrack_openai_calls(client)`, or `npm install @ai-sdk/otel` plus `registerTelemetry(new OpenTelemetry())` at startup on AI SDK 7 (`experimental_telemetry: { isEnabled: true }` on AI SDK 6 and earlier) |
