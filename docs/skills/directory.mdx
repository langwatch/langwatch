...
- **Empty input/output**: The most common issue. Check if `autotrack_openai_calls(client)` (Python) or the telemetry integration (TypeScript/Vercel AI) is configured. On AI SDK 7 that means `registerTelemetry(new OpenTelemetry())` from `@ai-sdk/otel` at startup; on AI SDK 6 and earlier, `experimental_telemetry: { isEnabled: true }` per call.
...
| Issue | Cause | Fix |
|-------|-------|-----|
| All traces show `<empty>` input/output | Missing autotrack or telemetry config | Add `autotrack_openai_calls(client)`, or `registerTelemetry(new OpenTelemetry())` on AI SDK 7 (`experimental_telemetry: { isEnabled: true }` on AI SDK 6 and earlier) |
