...
6. `langwatch suite list --format json` to see what test suites exist, and `langwatch run-plan list --format json` to see what run plans they are run under
...
2. Run any tests / re-query analytics to verify (`langwatch trace search`, `langwatch suite run <id|name> --target <type:id> --wait`, etc.)
