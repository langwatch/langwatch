...
6. In **Say who it lets in**, choose the arrival policy for this connection:
   ...
   people who match the verified domains. Save the choice.
7. Select **Turn it on** when the setup checks are complete.
...
To end a person's sessions and organization
access, deactivate or delete that person through SCIM before removing the
connection, or use the directory offboarding controls. Those operations revoke the person's browser sessions
and CLI tokens, remove organization membership and role bindings, and keep
their LangWatch data. Reactivating through a later directory push does not
restore access that was assigned by hand; the new push must assert membership
and mapped access again.
