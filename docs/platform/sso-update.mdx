...
| --- | --- |
| Moves across at their next sign-in | No action needed. The new connection recognizes them by their address. If your previous provider is their only way in, finishing confirms the address it signed them in with, so they keep a way in. |
| Has to sign in through the new connection once | Ask them to sign in. Your previous provider is their only way in, and it signed them in with a different address from the one on their account, so finishing waits until they have another way in. |
...
| No deactivated member can only sign in through the previous provider, under an address finishing cannot confirm | Remove those members from the organization in **Settings > Directory**. |
