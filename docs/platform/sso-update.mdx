...
- If directory sync (SCIM) provisions your people today, nothing changes for
  it. It moves to the new connection when you finish. See [Directory
  sync](#directory-sync).
...
account by their email address. That works before and after you finish, so
most members never hold the update up. The **Members moved across** row counts
who has signed in through the new connection, and how many more will be moved
across at their next sign-in.

Under **Not moved across yet**, each remaining member is listed with what
moving them still needs:

| Beside a member | What to do |
| --- | --- |
| Moves across at their next sign-in | Nothing. The new connection recognizes them by their address. |
| Has to sign in through the new connection once | Ask them to sign in. Your previous provider is their only way in, so finishing waits until they have another. |
| Cannot be matched: their address is not confirmed | Ask them to confirm their email address in their account settings, or remove them from the organization if they have left. |
| Cannot be matched: another account has the same address | Contact support. |
| Cannot be matched: their address is not on a domain you proved | Prove that domain on the new connection, or remove them from the organization if they have left. |
...
| Every member has moved across, or will at their next sign-in | Deal with the members listed under **Not moved across yet**, as above. Members who have left can be removed in **Settings > Directory**. |
| No deactivated member can only sign in through the previous provider | Remove those members from the organization in **Settings > Directory**. |
