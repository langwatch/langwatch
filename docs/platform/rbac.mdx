...
<Note>
  **Custom roles are available on Enterprise plans.** The built-in organization, team and
  project roles below work on every plan. Creating custom roles and assigning them needs
  an Enterprise plan, and a self-hosted deployment needs an Enterprise license. Calls to
  the [Roles API](/api-reference/roles/overview) without one are refused with HTTP 402 and
...
<Info>
  For group-based access control and identity provider integration, see [SCIM
  Groups](/platform/scim-groups).
</Info>
...
<Tip>
  Use group bindings (especially SCIM groups from your IdP) to manage access at scale,
  assigning a binding to a group is equivalent to assigning it to every member of that
  group. See [SCIM Groups](/platform/scim-groups).
</Tip>
