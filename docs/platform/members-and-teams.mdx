...
Every member has one organization role. Access to projects comes from a team role: a person on a team holds that role on every project in the team, and a project-level binding adds access on one project without removing the team role.
