...
<Note>
  Configuring data retention requires an **Enterprise plan**. On other plans, every project uses the
  platform default window. See the [pricing page](/pricing) for how extended retention is billed.
</Note>
...
<Note>
  When you retain data longer than your plan's included window, the additional storage is billed at
  **€3 per extra GB**. You only pay for what you store.
</Note>
...
<Steps>
  ...
  <Step title="Choose a scope">
    Pick the organization, a team, or a single project. A more specific scope overrides the broader
    defaults.
  </Step>
...
<Warning>
  **Deletion is permanent.** Once data is removed it cannot be recovered. If you need to keep data
  beyond your retention window, export it before it expires.
</Warning>
