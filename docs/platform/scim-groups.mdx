...
<Info>
  Groups are an **Enterprise** feature, whether they come from SCIM or are created by hand. Set up
  [SCIM Provisioning](/platform/scim) for users first.
</Info>
...
<Info>
  The name and the members of a SCIM group belong to the identity provider. The REST API and the
  settings page refuse to change them.
</Info>
