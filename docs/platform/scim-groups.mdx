...
<Info>
  SCIM Groups are an **Enterprise** feature. See [SCIM Provisioning](/platform/scim) for
  setting up user provisioning first.
</Info>
...
<Tip>
  You can add multiple bindings to a single group, for example, _Member on the whole
  organization_ plus _Admin on Team Platform_.
</Tip>
...
<Info>
  Use `PATCH active=false` for temporary suspension (e.g. leave of absence). Use DELETE
  for permanent offboarding. The distinction matters because DELETE removes all role
  bindings and group memberships, re-provisioning the user starts from scratch.
</Info>
...
<Note>
  Manual groups are not synced with any identity provider. Membership must be managed from
  within LangWatch.
</Note>
...
<Info>
  SCIM-managed groups cannot have their members or names modified via the REST API. Those
  operations are reserved for the identity provider.
</Info>
