...
<Note>
  **Available on Enterprise plans.** SCIM provisioning needs an Enterprise plan, and a self-hosted deployment needs an Enterprise license. The plan is checked on every call, not only when a token is created, so if it lapses, tokens that already exist stop verifying and your identity provider's requests are refused. Minting a token through the [SCIM Tokens API](/api-reference/scim-tokens/overview) without an Enterprise plan is refused with HTTP 402 and the error code `enterprise_plan_required`. To enable it, reach out to [enterprise@langwatch.ai](mailto:enterprise@langwatch.ai).
</Note>
...
- SSO configured for your organization (recommended, see [SSO Setup](/self-hosting/configuration/sso))
...
- They lose access to **this organization**; their account is deactivated only if this was their last active organization
...
- Their membership is **disabled**: they lose access to this organization
- Their org membership row and role bindings are **preserved**
- Their account is deactivated only if this was their last active organization
- Re-activating (`active: true`) immediately restores access

<Info>
  Suspending and deleting act on the user's membership of **this** organization. A person who belongs to more than one organization keeps their account and their access everywhere else; only when their last active membership ends is the account itself deactivated.

  The `active` attribute we return follows the same rule: it reports whether the user is active in the organization your SCIM token is scoped to, not whether their LangWatch account exists. A user suspended here reads back as `active: false` even though they are still working in another organization.
</Info>

Re-activating restores access, but it does **not** restore usage-attribution links —
the records matching this person to their provider logins in cost reports were closed
when they were offboarded, and an administrator relinks them.
