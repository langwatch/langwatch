...
- **Organization management**: Organization, Members and Invites, Teams, Access Groups, Custom Roles, Role Bindings and SCIM Provisioning are the provisioning surface: CLI + REST + UI, no SDK or MCP. Everything except Teams is available on Enterprise plans and answers `402 enterprise_plan_required` below one. `langwatch organizations create|list|get` provisions organizations on a **self-hosted** instance and authenticates with the instance administrator credential (`LANGWATCH_INSTANCE_ADMIN_API_KEY`), not an organization API key; on LangWatch Cloud those paths do not exist. They are listed under Organization because they address the same resource.
