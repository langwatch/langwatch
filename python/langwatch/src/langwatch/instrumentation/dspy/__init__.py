...
from langwatch.observability.tracing import trace
from langwatch.observability.span import span
...
class LangWatchDSPy:
            ...
            with span(
                ...
                if len(self.history) > 0:
                    ...
                    if len(outputs) == 1:
                        ...
                        span_.update_attributes({"output": outputs[0]})
                    else:
                        response["outputs"] = outputs
                        span_.update_attributes({"output": outputs})
                    if "prompt" in entry:
                        response["prompt"] = entry["prompt"]
                        span_.update_attributes({"input": entry["prompt"]})
                    if "messages" in entry:
                        response["messages"] = entry["messages"]
                        span_.update_attributes({"input": entry["messages"]})
                    ...
                    if "usage" in entry:
                        if "prompt_tokens" in entry["usage"]:
                            ...
                            span_.update_attributes({"prompt_tokens": entry["usage"]["prompt_tokens"]})
                        if "completion_tokens" in entry["usage"]:
                            llm_call["completion_tokens"] = entry["usage"]["completion_tokens"]
                            span_.update_attributes({"completion_tokens": entry["usage"]["completion_tokens"]})

                    if "cost" in entry:
                        llm_call["cost"] = entry["cost"]
                        span_.update_attributes({"cost": entry["cost"]})
    ...
    def tracer(self):
        return trace(
            name=f"dspy_experiment_{self.experiment_slug}",
            type="workflow",
            metadata={
...
class LangWatchTrackedBootstrapFewShot(BootstrapFewShot):
    ...
    def _log_step(self, round_idx: int, predictors: List[DSPyPredictor] = []):
        ...
        with span(
            name=f"bootstrap_round_{round_idx}",
            type="component",
            metrics={"bootstrapped_demos": bootstrapped_demos_count}
        ):
...
class LangWatchTrackedBootstrapFewShotWithRandomSearch(
    ...
    def _patch_evaluate(self):
        ...
        def patched_evaluate_call(self, program: dspy.Module, *args, **kwargs):
            ...
            with span(
                name=f"random_search_step_{step}",
                type="component",
                metrics={"score": score}
            ):
                langwatch_dspy.log_step(
