...
const skills = listPublishedSkills()
  .map(({ slug, src, isRecipe }) => {
    ...
    return {
      ...
      featureFlag: frontmatter["feature-flag"],
    };
  })
  // A flag-gated skill still needs its compiled body (Langy resolves the
  // flag live, per caller) but this bundle ships inside the CLI binary,
  // installed by anyone — there is no per-caller flag to resolve there, so
  // "gated" can only mean "not in this static bundle yet". Same reasoning as
  ...
  .map(({ featureFlag: _featureFlag, ...skill }) => skill);
