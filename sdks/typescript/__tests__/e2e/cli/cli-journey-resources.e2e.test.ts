...
describe("given the built CLI working a seeded project", () => {
  ...
  describe("when a dataset is created, filled and deleted from the terminal", () => {
    ...
    it(
      "changes the platform's own state at each step",
      async () => {
  ...
  describe("when a dataset is created without a name", () => {
    ...
    it(
      "exits non-zero and says which argument is missing",
      () => {
  ...
  describe("when an evaluator is created from the catalogue", () => {
    ...
    it(
      "creates it and lists it by name",
      async () => {
        ...
        const type =
          catalogue.find((each) => each.slug === "langevals/exact_match")?.slug ??
          catalogue[0]?.slug;
  ...
  describe("when the evaluator type is one the catalogue does not hold", () => {
    ...
    it(
      "refuses it naming the option, before any request",
      () => {
  ...
  describe("when a scenario is created from the terminal", () => {
    ...
    it(
      "creates it and the platform holds it",
      async () => {
  ...
  describe("when a test suite is created and run from the terminal", () => {
    ...
    it(
      ...
      async () => {
        ...
        const run = workspace.cli.run(
          `test-suite run ${suite.id} --target http:${agent.id} -o json`,
        );
  ...
  describe("when simulation runs are listed from the terminal", () => {
    ...
    it(
      "exits zero and prints the runs it found",
      () => {
  ...
  describe("when agents are listed and read from the terminal", () => {
    ...
    it(
      "names the agent the project holds and prints it by id",
      async () => {
        const agent = await langwatch.agents.create({
  ...
  describe("when the organization is read from the terminal", () => {
    ...
    it(
      "exits zero and names the organization",
      () => {
        ...
        const answered =
          answer.ok === false
            ? { refusal: answer.error?.code }
            : { named: Boolean(answer.name ?? answer.id) };
        expect([{ refusal: "enterprise_plan_required" }, { named: true }]).toContainEqual(answered);
      },
      CLI_TIMEOUT_MS,
    );
  });
