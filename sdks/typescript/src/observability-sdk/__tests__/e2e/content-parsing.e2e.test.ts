...
describe("Content Parsing E2E", () => {
  ...
  it(
    "parses and extract chat messages correctly",
    async () => {
      ...
      await tracer.withActiveSpan("chat-conversation", async (span) => {
        ...
        // Add message events
        logger.emitGenAISystemMessageEvent({
          content: chatInput.messages[0]?.content ?? "",
        });
        logger.emitGenAIUserMessageEvent({
          content: chatInput.messages[1]?.content ?? "",
        });
  ...
  it(
    ...
    async () => {
      ...
      const ragContexts = [
        {
          content: "Solar energy reduces carbon emissions.",
          document_id: "doc_solar_001",
          chunk_id: "chunk_1",
        },
        {
          content: "Wind power is cost-effective.",
          document_id: "doc_wind_003",
          chunk_id: "chunk_2",
        },
      ];
