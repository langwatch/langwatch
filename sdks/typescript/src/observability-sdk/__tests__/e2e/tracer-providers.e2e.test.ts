...
describe("Tracer Provider Configuration E2E", () => {
  ...
  it(
    "works with standard tracer provider",
    async () => {
  ...
  it(
    "handles multiple tracer instances",
    async () => {
  ...
  it(
    "handles nested spans with context",
    async () => {
      ...
      const parentSpan = trace.spans?.find(
        (s) => s.name === "parent-context-test",
      );
      const childSpan = trace.spans?.find(
        (s) => s.name === "child-context-test",
      );
