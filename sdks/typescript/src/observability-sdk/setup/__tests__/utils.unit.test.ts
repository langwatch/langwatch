...
describe("utils.ts", () => {
  describe("isConcreteProvider", () => {
    ...
    it("returns false for object with _activeSpanProcessor but no array registry", () => {
      expect(utils.isConcreteProvider({ _activeSpanProcessor: {} })).toBe(
        false,
      );
      expect(
        utils.isConcreteProvider({
          _activeSpanProcessor: { _spanProcessors: "not-array" },
        }),
      ).toBe(false);
    });
    ...
    it("returns true for OTel 1.x shape: _registeredSpanProcessors array", () => {
      expect(utils.isConcreteProvider({ _registeredSpanProcessors: [] })).toBe(
        true,
      );
    });
    it("returns true for alternate shape: activeSpanProcessor._spanProcessors array", () => {
      expect(
        utils.isConcreteProvider({
          activeSpanProcessor: { _spanProcessors: [] },
        }),
      ).toBe(true);
    });
  });
  ...
  describe("createMergedResource", () => {
    it("merges attributes, serviceName, and givenResource", () => {
      ...
      const merged = utils.createMergedResource(
        attributes,
        serviceName,
        givenResource,
      );
