...
describe("setupObservability Integration - Configuration Options", () => {
  it("reflects apiKey and endpoint in the exporter", async () => {
    ...
    const exportSpy = vi.fn((spans, resultCallback) =>
  ...
  it("uses serviceName and attributes in resource", async () => {
    ...
    expect(
      getInternalTracerConfig(provider).resource.attributes["service.name"],
    ).toBe("test-service");
    expect(
      getInternalTracerConfig(provider).resource.attributes[
        "deployment.environment"
      ],
    ).toBe("test");
    expect(
      getInternalTracerConfig(provider).resource.attributes["service.version"],
    ).toBe("1.0.0");
  ...
  it("uses custom Resource if provided", async () => {
    ...
    const provider: any = getConcreteProvider(trace.getTracerProvider());
    expect(
      getInternalTracerConfig(provider).resource.attributes["custom.resource"],
    ).toBe("yes");
    await handle.shutdown();
  });
  ...
  it("uses spanLimits if provided", async () => {
    ...
    const provider: any = getConcreteProvider(trace.getTracerProvider());
    expect(
      getInternalTracerConfig(provider).spanLimits?.attributeCountLimit,
    ).toBe(1);
    await handle.shutdown();
  });
  ...
  it("uses sampler if provided", async () => {
    ...
    const handle = setupObservability(options);
    const provider: any = getConcreteProvider(trace.getTracerProvider());
    expect(getInternalTracerConfig(provider).sampler.toString()).toBe(
      "customSampler",
    );
    await handle.shutdown();
  });
  ...
  it("uses idGenerator if provided", async () => {
    ...
    const handle = setupObservability(options);
    const provider: any = getConcreteProvider(trace.getTracerProvider());
    expect(getInternalTracerConfig(provider).idGenerator.generateSpanId()).toBe(
      "spanid",
    );
    expect(
      getInternalTracerConfig(provider).idGenerator.generateTraceId(),
    ).toBe("traceid");
    await handle.shutdown();
  });
  ...
  describe("data capture configuration", () => {
    it('sets "none" mode in observability config', async () => {
      ...
      const { shouldCaptureInput, shouldCaptureOutput } = await import(
        "../../../../config.js"
      );
