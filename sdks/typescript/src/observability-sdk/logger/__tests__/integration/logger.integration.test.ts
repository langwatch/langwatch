...
describe("given logger observability wired to a real OpenTelemetry SDK", () => {
  ...
  describe("log record creation and data flow", () => {
    it("creates log records with proper LangWatch attributes through real OpenTelemetry", async () => {
      ...
      expect(exportedLogRecord.attributes?.["langwatch.service"]).toBe(
        "test-service",
      );
      expect(exportedLogRecord.attributes?.["langwatch.environment"]).toBe(
        "test",
      );
      expect(exportedLogRecord.attributes?.["langwatch.operation"]).toBe(
        "test-operation",
      );
      expect(exportedLogRecord.attributes?.["langwatch.user_id"]).toBe(
        "user-123",
      );
      expect(exportedLogRecord.attributes?.["custom.attribute"]).toBe(
        "custom-value",
      );
      expect(exportedLogRecord.attributes?.["performance.latency_ms"]).toBe(
        150,
      );
    ...
    it("handles different severity levels correctly", async () => {
      ...
      exportedLogRecords.forEach((record, index) => {
        expect(record.attributes?.level).toBe(
          ["debug", "info", "warn", "error"][index],
        );
      });
    });
    ...
    it("handles log records without attributes", async () => {
      ...
      expect(exportedLogRecord.body).toBe(
        "Simple log message without attributes",
      );
      expect(exportedLogRecord.severityText).toBe("INFO");
      expect(exportedLogRecord.severityNumber).toBe(9);
  ...
  describe("data capture integration", () => {
    ...
    it("preserves other log record properties when output capture is disabled", async () => {
      ...
      const logger = getLangWatchLogger(
        "data-capture-properties-test-logger-9",
      );
  ...
  describe("GenAI-specific logging", () => {
    it("handles GenAI-specific attributes correctly", async () => {
      ...
      expect(exportedLogRecord.attributes?.["gen_ai.request.model"]).toBe(
        "gpt-4",
      );
      expect(exportedLogRecord.attributes?.["gen_ai.request.temperature"]).toBe(
        0.7,
      );
      expect(exportedLogRecord.attributes?.["gen_ai.request.max_tokens"]).toBe(
        150,
      );
      expect(
        exportedLogRecord.attributes?.["gen_ai.response.finish_reason"],
      ).toBe("stop");
      expect(exportedLogRecord.attributes?.["gen_ai.usage.prompt_tokens"]).toBe(
        15,
      );
      expect(
        exportedLogRecord.attributes?.["gen_ai.usage.completion_tokens"],
      ).toBe(25);
      expect(exportedLogRecord.attributes?.["gen_ai.usage.total_tokens"]).toBe(
        40,
      );
      expect(exportedLogRecord.attributes?.["langwatch.service"]).toBe(
        "genai-service",
    ...
    it("handles GenAI error scenarios", async () => {
      ...
      expect(exportedLogRecord.attributes?.["gen_ai.error.retry_after"]).toBe(
        60,
      );
    });
  });
  ...
  describe("attribute and metadata validation", () => {
    it("validates and sanitize attribute values", async () => {
      ...
      expect(exportedLogRecord.attributes?.["string.attr"]).toBe(
        "valid string",
      );
    ...
    it("handles complex attribute type coercion", async () => {
      ...
      // Verify valid attributes are present
      expect(exportedLogRecord.attributes?.["valid.string"]).toBe(
        "normal string",
      );
  ...
  describe("custom logger provider integration", () => {
    it("works with custom logger providers", async () => {
      ...
      const customProvider = new LoggerProvider({
        processors: [
          new SimpleLogRecordProcessor({ exporter: customExporter }),
        ],
      });
