  ...
  describe("data validation", () => {
    it("handles various LLMResult formats", () => {
      ...
      const testCases = [
        ...
        {
          generations: [
            [{ text: "text" }, { message: { content: "msg", type: "ai" } }],
          ],
        },
...
describe("Helper Functions", () => {
  describe("className", () => {
    ...
    it("extracts class name from lc_namespace array", () => {
      const serialized = {
        lc_namespace: ["langchain", "chains", "LLMChain"],
        lc: 1,
        type: "not_implemented",
      } as any;
    ...
    describe("LangGraph node naming", () => {
      ...
      it("prioritizes router over node when both are present", () => {
        const result = deriveNameAndType({
          runType: "chain",
          serialized: {
            id: ["Branch<analyze,generate_answer>"],
            lc: 1,
            type: "not_implemented",
          } as any,
          metadata: {
    ...
    describe("Router vs Node prioritization", () => {
      ...
      it("handles complex routing scenarios", () => {
        const result = deriveNameAndType({
          runType: "chain",
          serialized: {
            id: ["Branch<validate,process,retry>"],
            lc: 1,
            type: "not_implemented",
          } as any,
          metadata: {
    ...
    describe("Router naming", () => {
      it("names routers with path and decision", () => {
        const result = deriveNameAndType({
          runType: "chain",
          serialized: {
            id: ["Branch<analyze,search,analyze>"],
            lc: 1,
            type: "not_implemented",
          } as any,
          metadata: {
  ...
  describe("buildLangChainMetadataAttributes", () => {
    it("filters out LangGraph metadata keys", () => {
      ...
      expect(
        result["langwatch.langchain.run.metadata.langgraph_node"],
      ).toBeUndefined();
      expect(
        result["langwatch.langchain.run.metadata.langgraph_step"],
      ).toBeUndefined();
      expect(
        result["langwatch.langchain.run.metadata.thread_id"],
      ).toBeUndefined();

      // Should contain other keys
      expect(result["langwatch.langchain.run.metadata.custom_key"]).toBe(
        "custom_value",
      );
