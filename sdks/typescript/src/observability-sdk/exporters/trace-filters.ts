 ...
 * ```
 */
export function isHttpRequestSpan({ name }: ReadableSpan): boolean {
  const spanName = name ?? "";
  const isHttpVerbMatch = /^(GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)(?: |$)/.test(spanName);
  return isHttpVerbMatch;
}
