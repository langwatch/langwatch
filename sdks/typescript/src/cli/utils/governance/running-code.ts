...
const isRuntime = (value: string): boolean => /^(?:node|bun)(?:\.exe)?$/.test(basename(value));
const isLauncher = (value: string): boolean => /^(?:langwatch|lw)(?:\.exe)?$/.test(basename(value));
const isEntrypoint = (value: string): boolean =>
  isLauncher(value) || /\/(?:langwatch|sdks\/typescript)\/dist\/cli\/index\.js$/.test(value);
