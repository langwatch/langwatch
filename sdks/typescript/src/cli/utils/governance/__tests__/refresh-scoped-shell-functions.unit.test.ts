...
describe("refreshScopedShellFunctions", () => {
  const staleGeminiVars = buildOtelEnvBlock("gemini", STALE_ENDPOINT, STALE_TOKEN);
  const currentGeminiVars = buildOtelEnvBlock("gemini", CURRENT_ENDPOINT, CURRENT_TOKEN);
  ...
  describe("given a stale gemini function in ~/.zshrc and ~/.bashrc", () => {
    ...
    describe("when refreshed with the current run's values", () => {
      ...
      it("preserves user-authored rc lines outside the markers", () => {
        ...
        expect((rc.match(/# >>> langwatch gemini begin >>>/g) ?? []).length).toBe(1);
      });
    });
  });
