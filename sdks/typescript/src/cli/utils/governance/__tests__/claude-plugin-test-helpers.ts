...
export interface ClaudePluginHarness {
  ...
  seedMarketplace: (args?: { repo?: string; publishedVersion?: string }) => void;
...
function configIo() {
  return {
    readConfig: (): Record<string, unknown> =>
      JSON.parse(fs.readFileSync(process.env.LANGWATCH_CLI_CONFIG!, "utf8")) as Record<
        string,
        unknown
      >,
    writeConfig: (extra: Record<string, unknown> = {}): void => {
...
export function installClaudePluginHarness({
  ...
  const settingsPath = (): string => path.join(state.home, ".claude", "settings.json");
