...
describe("resolveWrapperMode", () => {
  ...
  describe("when copilot resolves to ingestion mode", () => {
    ...
    it.each([
      ...
      "off",
      "  false  ",
    ])("treats the case-insensitive/falsey opt-out %j as off (never silently forces capture on)", async (value) => {
      const { resolveWrapperMode } = await import("../wrapper-mode.js");
      mintCopilot();
    ...
    it("respects an explicit content-capture opt-out with a loud tokens-only notice", async () => {
      ...
      try {
        ...
        if (prev === undefined)
          ...
          process.env.OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT = prev;
      }
    });
  });
  ...
  describe("when the mode was forced by the path-selection UX (silent default)", () => {
    ...
    it.each([
      "claude",
      "codex",
    ])("does not pin tool_mode for %s either when a forcedMode was passed (cross-tool regression)", async (tool) => {
      const { resolveWrapperMode } = await import("../wrapper-mode.js");
      (cliApi.mintIngestionKey as ReturnType<typeof vi.fn>).mockResolvedValue({
        token: "sk-lw-tok",
        prefix: "sk-lw-tok",
