 ...
 */
export function normalizeSessionName(raw: string | null | undefined): string | null {
...
export function buildSessionContextLogPayload({
  ...
  if (context.branch) {
    attributes.push(attribute({ key: "vcs.ref.head.name", value: context.branch }));
  }
  if (context.worktree) {
    attributes.push(attribute({ key: "vcs.worktree.name", value: context.worktree }));
  }
  if (title) {
...
function attribute({ key, value }: { key: string; value: string }): OtlpKeyValue {
