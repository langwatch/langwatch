/**
 * Help and version runs of a wrapped tool start no session: no config, login, key or wiring.
 * Recognised from args alone and handed to the tool with the calling shell's environment.
 */
import { spawn } from "node:child_process";

import { parseProjectScopeFlags, parseToolModeFlag } from "./wrapper-path-choice";
...
export function infoRunToolArgs(args: string[]): string[] {
  ...
  const stripped = parseToolModeFlag(parseProjectScopeFlags(beforeBoundary).args, {}).args;
 ...
 */
export function aliasShellFor(env: NodeJS.ProcessEnv = process.env): string | null {
  const shellName = (env.SHELL ?? "").split("/").pop() ?? "";
  return process.platform !== "win32" && (shellName === "zsh" || shellName === "bash")
    ? env.SHELL!
    : null;
}
