  ...
  describe("given a config file that already has sections", () => {
    describe("when the block is written", () => {
      ...
      it("leaves the user's existing settings intact", () => {
        fs.writeFileSync(
          configPath,
          ['model = "gpt-5.6-sol"', "", "[otel]", 'environment = "mine"', ""].join("\n"),
        );
  ...
  describe("given no config file yet", () => {
    describe("when the block is written", () => {
      it("creates the file holding just the block", () => {
        ...
        expect(tableOwning(fs.readFileSync(configPath, "utf8"), "notify")).toBeNull();
      });
    });
  });
  ...
  describe("given the user already set their own notify program", () => {
    describe("when the block is written", () => {
      ...
      it("chains their program so it still runs after every turn", () => {
        ...
        expect(fs.readFileSync(configPath, "utf8")).toContain("terminal-notifier");
      });
  ...
  describe("given capture already moved the user's own notify aside", () => {
    describe("when capture is enabled again", () => {
      ...
      it("still chains their program exactly once", () => {
        ...
        expect(result.action).toBe("unchanged");
        expect(content.match(/langwatch displaced notify begin/g)).toHaveLength(1);
        expect(content.match(/langwatch codex notify begin/g)).toHaveLength(1);
      });

      /** @scenario "Enabling capture again keeps the user's own program running" */
      it("gives their notify back live when capture is then turned off", () => {
        const original = 'notify = ["/usr/bin/terminal-notifier", "-title", "Codex"]';
        fs.writeFileSync(
          configPath,
