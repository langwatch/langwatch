  ...
  try {
    ...
    return {
      ...
      table: () => {
        ...
        console.log(`  ${chalk.gray("Author:")}    ${detail.authorLabel ?? chalk.gray("—")}`);
        console.log(`  ${chalk.gray("Saved:")}     ${new Date(detail.createdAt).toISOString()}`);
        if (detail.changedFields.length > 0) {
          console.log(`  ${chalk.gray("Changed:")}   ${detail.changedFields.join(", ")}`);
        }
