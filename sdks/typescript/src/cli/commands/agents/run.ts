...
type Agent = Awaited<ReturnType<AgentsApiService["get"]>>;

const parseInputFlag = (raw: string | undefined): Record<string, unknown> => {
  if (!raw) return {};
  try {
...
export const runAgentCommand = async (
  ...
  const input = parseInputFlag(options.input);

  if (agent.type === "connected") return runConnectedAgent({ service, agent, input, options });
  if (agent.type === "http") return runHttpAgent({ agent, input });
  return runWorkflowAgent({ agent, input });
};
