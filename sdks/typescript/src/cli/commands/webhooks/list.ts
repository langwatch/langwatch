...
export const listWebhooksCommand = async (): Promise<CommandResult | void> => {
  ...
  try {
    ...
    return {
      data: endpoints,
      table: () => printEndpoints(endpoints),
    };
  } catch (error) {
...
type WebhookEndpoints = Awaited<ReturnType<WebhooksApiService["list"]>>;

function printEndpoints(endpoints: WebhookEndpoints): void {
  ...
  console.log();
  formatTable({
    data: endpoints.map(endpointRow),
...
/** A queue endpoint has no URL, so the address is whichever one the endpoint delivers to. */
function endpointRow(e: WebhookEndpoints[number]) {
  const address = e.sqs?.queue_url ?? e.url ?? "";
  return {
    ID: e.id,
    Destination: e.destination_kind === "sqs" ? "Amazon SQS" : "HTTPS",
    Address: address.length > 45 ? `${address.slice(0, 42)}...` : address,
    Status:
      e.status === "active"
        ? chalk.green("active")
