...
describe("sessionEventsCommand()", () => {
  ...
  it("passes kinds through and stops at the requested limit", async () => {
    fetchMock.mockResolvedValueOnce(page([modelCall(1720000000000, "r1")], "cursor-1"));
  ...
  it("exits with an error for a non-numeric --limit", async () => {
    await expect(sessionEventsCommand("session-abc", { limit: "abc" })).rejects.toThrow(
      ProcessExitError,
    );
  ...
  it("sends ISO and epoch-ms bounds as epoch ms", async () => {
    fetchMock.mockResolvedValueOnce(page([modelCall(1720000000000, "r1")], null));
