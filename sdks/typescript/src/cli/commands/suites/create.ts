...
export const createSuiteCommand = async (
  ...
  if (!options.scenarios) {
    console.error(
      chalk.red(
        "Error: --scenarios is required (comma-separated scenario IDs)",
      ),
    );
    process.exit(1);
  }
  ...
  const repeatCount = options.repeatCount
    ? parseInt(options.repeatCount, 10)
    : 1;
  ...
  try {
    ...
    return {
      ...
      table: () => {
        ...
        console.log(
          `  ${chalk.gray("Scenarios:")} ${suite.scenarioIds.length}`,
        );
        ...
        console.log(
          chalk.gray(
            `Run it with: ${chalk.cyan(`langwatch suite run ${suite.id}`)}`,
          ),
        );
      },
    };
