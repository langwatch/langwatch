...
describe("listSuitesCommand()", () => {
  ...
  describe("when the API call fails", () => {
    it("exits with code 1", async () => {
      mockGetAll.mockRejectedValue(new SuitesApiError("Network error", "GET /api/suites"));
...
describe("getSuiteCommand()", () => {
  ...
  describe("when suite is not found", () => {
    it("exits with code 1", async () => {
      mockGet.mockRejectedValue(new SuitesApiError("Not found", "GET /api/suites/nonexistent"));
...
describe("createSuiteCommand()", () => {
  ...
  describe("when the plan is given a scope", () => {
    ...
    it("resolves a test suite named by name into its id", async () => {
      mockGetAll.mockResolvedValue([{ id: "suite_folder_1", name: "Refunds", kind: "folder" }]);
      mockCreate.mockResolvedValue(makeSuite());
  ...
  describe("when targets are missing", () => {
    it("exits with code 1", async () => {
      await expect(createSuiteCommand("Test Suite", { scenarios: "scenario_1" })).rejects.toThrow(
        ProcessExitError,
      );
    });
  });
...
describe("runSuiteCommand()", () => {
  ...
  describe("when run has skipped archived references", () => {
    it("shows warning about skipped items", async () => {
      mockRun.mockResolvedValue(
        makeRunResult({
          skippedArchived: { scenarios: ["old_scenario"], targets: ["old_agent"] },
        }),
      );
  ...
  describe("when API call fails", () => {
    it("exits with code 1", async () => {
      ...
      await expect(runSuiteCommand({ id: "nonexistent", options: {} })).rejects.toThrow(
        ProcessExitError,
      );
    });
  });
  ...
  describe("when the id names a test suite folder", () => {
    ...
    it("reports the job count and the batch run ID", async () => {
      mockRun.mockResolvedValue(makeRunResult({ batchRunId: "batch_folder", jobCount: 4 }));
    ...
    it("reports the platform's refusal when the folder has no target", async () => {
      ...
      await expect(runSuiteCommand({ id: "folder_abc", options: {} })).rejects.toThrow(
        ProcessExitError,
      );
    });
  });
