...
function parseTargets(targetStrings: string[]): SuiteTarget[] {
  return targetStrings.map((t) => {
    ...
    if (colonIndex === -1) {
      console.error(
        chalk.red(
          `Error: Invalid target format "${t}". Use <type>:<referenceId>`,
        ),
      );
      process.exit(1);
    }
...
export const updateSuiteCommand = async (
  ...
  try {
    ...
    if (options.scenarios)
      updateData.scenarioIds = options.scenarios
        .split(",")
        .map((s) => s.trim());
    if (options.targets && options.targets.length > 0)
      updateData.targets = parseTargets(options.targets);
    if (options.repeatCount)
      updateData.repeatCount = parseInt(options.repeatCount, 10);
    if (options.labels)
