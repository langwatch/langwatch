...
> (`github.com/langwatch/langwatch/sdks/go`), which emits OpenTelemetry spans. Use
...
```bash
go get github.com/langwatch/langwatch/sdks/go/client
```
...
This is a **separate Go module** (`sdks/go/client`) with its own `go.mod` and a
