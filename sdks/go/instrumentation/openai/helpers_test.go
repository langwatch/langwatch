...
// Convenience semconv key aliases used across the test files.
var (
...
// genAIInputKey/genAIOutputKey are the OTel gen_ai message attribute keys, under
// which LLM chat messages are recorded (a raw JSON array, no typedValue
// envelope). genAISystemKey is the system-instructions key (a plain string).
var (
