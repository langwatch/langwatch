...
//
// semconv v1.41.0 has no constant for token counting, so "count_tokens" is
// spelled out; every other mapped value comes from a typed constant.
func operationName(action string) string {
	...
	case "":
		return ""
	default:
		// gen_ai.operation.name values are snake_case in semconv, while Gemini's
		// URL verbs are camelCase. An unmapped verb (predict, streamRawPredict, …)
		// is normalised so it still groups with the Python and TypeScript spans
		// for the same operation instead of landing verbatim.
		return snakeCase(action)
	}
}

// snakeCase converts a camelCase Gemini action verb to the snake_case form
// gen_ai.operation.name uses ("streamRawPredict" -> "stream_raw_predict"). Runs
// of capitals are kept together ("countTokensV2" -> "count_tokens_v2").
func snakeCase(s string) string {
	...
	for i, r := range runes {
		...
		// A boundary is a lower/digit -> upper transition, or the last upper of a
		// run followed by a lower ("RawPredict" -> "raw_predict", not "r_a_w…").
