...
class CreateApiKeyBody:
    """
    Attributes:
        name (str): Human-readable name for this token
        bindings (list[CreateApiKeyBodyBindingsItem]): Role bindings that define what this token can access
        description (str | Unset): Optional description
        expires_at (datetime.datetime | Unset): Optional expiration date (ISO 8601)
        key_type (CreateApiKeyBodyKeyType | Unset): personal = tied to a user. service = not tied to any user, for
            automation. Default: CreateApiKeyBodyKeyType.PERSONAL.
        project_ids (list[str] | Unset): For service keys with restricted scope: list of project IDs to grant ADMIN
            access to. Omit for full org access.
    """

    name: str
    bindings: list[CreateApiKeyBodyBindingsItem]
