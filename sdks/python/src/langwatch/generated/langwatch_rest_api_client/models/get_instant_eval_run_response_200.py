...
from ..models.get_instant_eval_run_response_200_status import GetInstantEvalRunResponse200Status

if TYPE_CHECKING:
    from ..models.get_instant_eval_run_response_200_matched_by_question import (
        GetInstantEvalRunResponse200MatchedByQuestion,
    )
    from ..models.get_instant_eval_run_response_200_parameters import GetInstantEvalRunResponse200Parameters
    from ..models.get_instant_eval_run_response_200_questions_item import GetInstantEvalRunResponse200QuestionsItem


T = TypeVar("T", bound="GetInstantEvalRunResponse200")


@_attrs_define
class GetInstantEvalRunResponse200:
    """
    Attributes:
        id (str): The run id.
        name (None | str): What the run was called, if anything.
        sql (str): The statement, exactly as submitted.
        parameters (GetInstantEvalRunResponse200Parameters): The values the statement's parameters were filled with.
        questions (list[GetInstantEvalRunResponse200QuestionsItem]): One entry per eval function the statement projects,
            derived from it when the run was accepted.
        limit (int): Rows this run may judge.
        status (GetInstantEvalRunResponse200Status): Where the run is in its life.
        ...
        matched_by_question (GetInstantEvalRunResponse200MatchedByQuestion): Judgements that matched, per question.
    ...
    name: None | str
    sql: str
    parameters: GetInstantEvalRunResponse200Parameters
    questions: list[GetInstantEvalRunResponse200QuestionsItem]
    limit: int
    status: GetInstantEvalRunResponse200Status
    total: int | None
    progress: int
    matched: int
    matched_by_question: GetInstantEvalRunResponse200MatchedByQuestion
    failed: int
    skipped: int
    tokens: int
    cost_usd: float
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        parameters = GetInstantEvalRunResponse200Parameters.from_dict(d.pop("parameters"))

        questions = []
        _questions = d.pop("questions")
        for questions_item_data in _questions:
            questions_item = GetInstantEvalRunResponse200QuestionsItem.from_dict(questions_item_data)

            questions.append(questions_item)

        limit = d.pop("limit")

        status = GetInstantEvalRunResponse200Status(d.pop("status"))
        ...
        matched_by_question = GetInstantEvalRunResponse200MatchedByQuestion.from_dict(d.pop("matchedByQuestion"))
        ...
        get_instant_eval_run_response_200 = cls(
            id=id,
            name=name,
            sql=sql,
            parameters=parameters,
        ...
        get_instant_eval_run_response_200.additional_properties = d
        return get_instant_eval_run_response_200
