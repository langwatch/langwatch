...
| Direct ClickHouse writes on event-sourced aggregates | NEVER write directly to ClickHouse (INSERT, ALTER UPDATE, ALTER DELETE, or `ch.command`) from service/repository/route code for event-sourced aggregates (traces, experiment runs, evaluations, scenarios). Writes MUST go through command dispatch (`app.X.commandName(...)`) → event → fold/map projection. The projection owns the CH write. Direct writes bypass the event log, corrupt the source of truth, and cannot be replayed. Test fixtures inserting rows directly into CH are fine — the rule applies to production code paths |
