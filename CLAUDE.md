...
| Putting a `role: "system"` message in an AI SDK `messages` array | AI SDK 7 throws `InvalidPromptError`; v6 only warned, and **the typechecker cannot see it** — the call compiles and dies on the first request. Pass the system prompt as top-level `instructions`. `allowSystemInMessages: true` is only for transcripts that legitimately carry system turns you did not author (the playground, a scripted scenario), never for unvalidated client input. When you hoist, re-check any `slice`/index arithmetic that assumed the system message was element 0. See `dev/docs/best_practices/ai-sdk.md` |
| Assuming `experimental_telemetry: { isEnabled: true }` still sends us traces | It sends **nothing** on AI SDK 7. OpenTelemetry moved to `@ai-sdk/otel` and emits no spans until `registerTelemetry(new OpenTelemetry())` runs once at startup — silently, with no warning. The scope also renamed `ai` → `gen_ai`, so scope-matching filters must accept both. Every customer-facing snippet (docs, SDK examples, onboarding codegen) must show the registration and list `@ai-sdk/otel` |
| Treating a green `pnpm typecheck` as proof AI SDK code works | v7 kept deprecated aliases for nearly everything it renamed, so broken calls compile clean. The breakages that matter — system-in-messages, dead telemetry, renamed span scope, all-step `usage` aggregation — are invisible to tsc. Exercise the code path |
| Constructing an AI SDK provider inline (`createOpenAI(...)`) in a route or service | Resolve models through `getVercelAIModel({ projectId, featureKey })` (`src/server/modelProviders/utils.ts`) or `getCodexVercelAIModel`. They apply the project's configured provider, keys and defaults, and carry the `featureKey` cost attribution an inline provider skips |
