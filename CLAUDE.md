...
| Importing a component into server code to reuse a constant it happens to export | **Enforced:** no *value*-import chain from server code may reach a browser-only package (React, Chakra, Ark, Emotion, react-router, lucide-react, browser OTel) — `src/server/__tests__/frontend-boundary.unit.test.ts` walks the real graph transitively and fails the build. One such import pulled 2,020 modules / 212 MB RSS into every backend process. `src/server/mailer/**` is the one exception, since react-email renders templates server-side. **Also enforced, at author time:** the ast-grep rule `no-client-imports-in-server` (`dev/lint/ast-grep/rules/`) refuses the direct import via `make lint-rules` and the CI `ast-grep` job, including a value-import of any `**/components/**` file even a framework-free one — it invites exactly that chain later. The two are not redundant: a linter reads one file and sees only the direct hop, the test walks the graph and catches the chain. The three imports that predate it (`server/datasets/upload-utils.ts`, `server/app-layer/langy/streaming/langyTurnRelay.ts`, `server/scenarios/execution/data-prefetcher.ts`) carry a `// ast-grep-ignore: no-client-imports-in-server-ts` marking them as debt; don't add more. Move the shared value into a framework-free module both sides import (`import type` is always fine — types are erased, and the rule exempts it, which is precisely what Biome's `noRestrictedImports` cannot express — measured, 3 of its 5 hits on this tree were type-only) |
