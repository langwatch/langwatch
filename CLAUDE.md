...
**Typecheck takes a machine-wide slot.** A tsgo run peaks around 3 to 4 GiB and
saturates every core, so the `typecheck` scripts go through
`dev/scripts/typecheck-queue.mjs`: it counts the runs live across every
worktree, terminal and agent on the machine, and a run past the limit waits its
turn instead of piling on. With a slot free it prints nothing and is otherwise
transparent (same stdio, same exit code). Queued, it says so on stderr, which is
what tells you a slow run was waiting rather than hung. `TYPECHECK_SLOTS=N`
overrides the limit and `TYPECHECK_SLOTS=0` turns the queue off; unset, the
limit comes from the machine (one per 6 GiB of RAM, capped at one per 4 cores)
and CI does not queue at all. `node dev/scripts/typecheck-queue.mjs --explain`
shows the limit and who currently holds a slot. See
`specs/setup/typecheck-slots.feature`.
