...
| Running `pnpm typecheck` and assuming the TypeScript is checked | `tsconfig.tsgo.json` excludes `**/*.test.ts`, `**/*.test.tsx` and `**/__tests__/**`, so `pnpm typecheck` never looks at a test file. Use `pnpm typecheck:all`, which is what CI runs, or a change confined to a test file will typecheck clean locally and fail CI. It is one program over app plus tests (`tsconfig.tsgo.all.json`), not the two projects run back to back: every test file imports the app, so two programs checked the app's 17k files twice, for 35.1M type instantiations against 18.9M for the single program |
