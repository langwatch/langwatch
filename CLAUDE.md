...
**Two processes vs one (workers).** By default `pnpm dev` runs the app and the background workers as a **single Node process**: the app boots with the `"all"` process role and hosts the worker stack in-process via `startWorkers()`, saving the RAM of a second full copy of the server module graph. This is the default for both plain `pnpm dev` and `pnpm dev:haven` (changed by the memory-footprint work — previously plain `pnpm dev` ran two processes; see `specs/setup/memory-footprint.feature`). Opt back into a standalone `workers` lane with `pnpm dev:workers` (plain) or `pnpm dev:workers:haven` (haven) — i.e. `WORKERS_IN_PROCESS=0`; `pnpm dev:single` / `pnpm dev:single:haven` remain as the explicit single-process forms, now equivalent to the defaults. Workers keep their `langwatch:workers` logger name, so their lines stay identifiable without a lane of their own. This is dev-only — production still runs web and worker as separate deployments and `NODE_ENV=production` ignores the flag. See `dev/docs/adr/004-docker-dev-environment.md` (Amendment: In-process workers) and `specs/setup/in-process-workers-dev.feature`. Whether a role runs the worker stack is `roleRunsWorkers(role)` (`src/server/app-layer/config.ts`) — use it, never compare `processRole === "worker"` directly.
...
| Importing a component into server code to reuse a constant it happens to export | Server code must never *value*-import `~/components/**`, a `**/components/**` file, or a browser-only package (React, Chakra, Ark, Emotion, react-router). One such import pulled 2,020 modules / 212 MB RSS into every backend process. Move the shared value into a framework-free module both sides import (`import type` is fine — types are erased). Enforced by `src/server/__tests__/frontend-boundary.unit.test.ts`; `src/server/mailer/**` is the one exception, since react-email renders templates server-side |
