...
| Writing a `HandledError` message as customer copy, or expecting the client to render it | `message` is server copy for logs and never crosses the wire (#5984) — the wire message is the `code`. Write it for whoever reads the trace; put customer-facing copy in the client presentation registry, keyed by code |
| Adding a new error code without customer-facing copy | The client presentation registry is exhaustive over every code, including the generated Go ones — a missing entry fails `pnpm typecheck`. Write the title and description when you add the code, not later |
