...
| Importing `Avatar` from `@chakra-ui/react` | Import it from `~/components/ui/avatar`. Chakra derives initials with `charAt(0)`, which returns half of any emoji outside the basic plane and paints a replacement box — customers do name projects and suites with a leading emoji. The wrapper re-exports every other part unchanged, so the import is the whole difference, and `components/ui/__tests__/avatarImportGuard.unit.test.ts` enforces it. More generally, taking the first character of customer-typed text goes through `~/utils/firstGrapheme`, never `charAt(0)` or `slice(0, 1)` |
