...
| Opening or mounting a second drawer from inside an open drawer | Drawers are URL-routed singletons: one mounts at a time, a swap unmounts the current drawer and wipes its draft state, and `useState`/`useDisclosure`-mounting a drawer component is the same banned pattern. Build the sub-flow inline in the current drawer. See `dev/docs/best_practices/drawers.md` |
