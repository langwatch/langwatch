...
Tests follow the same rule. `pnpm test` never needed a container, and
...
- `dev/docs/ARCHITECTURE.md` - THE architecture record: modules, processes, supply, transports, eventing, browser
...
| Running `npx vitest` / `npm exec vitest` directly                                                           | Always go through a package's own scripts: `pnpm --filter <package> test <path>`, and `test:integration` only in the packages that declare one. The root `test:component` and `test:integration` scripts are stubs that exit 1. The RAM guardrails CLAUDE.md used to promise (`vmForks`, `isolate: false`, a per-worker memory cap) lived in the deleted monolith's config and no per-package config restores them yet; CI caps worker count with `VITEST_MAX_WORKERS`, local runs do not, so scope runs narrowly                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Writing a new method that returns `T \| null`                                                               | **New code only:** `find*` returns an **array** (empty, never null); `get*`/`getBy*` returns exactly **one or throws**. `T \| null` is not a shape we write any more. The ~1,217 existing nullable `find*` methods are deliberately left as they are - converting one changes behaviour at every caller that branches on absence - so the tree carries both conventions on purpose. Name it `get<WhatYouGetBack>` with the key in the parameters (`getOrganizationId({ teamId })`); `getBy<Key>` only when the key distinguishes the method and the entity itself comes back (`getById`, `getByEmail`). The full verb vocabulary - which reads, writes and derivations may be named what, and why `resolve*`/`read*` are decided per call site while `infer`/`classify`/`detect`/`pick`/`describe`/`map` answer `undefined` legitimately - is [ADR-146](dev/docs/adr/146-method-verb-vocabulary.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Taking `database: object` in an adapter's `.create(` and casting `as PrismaClient` at the repository seam   | The composition root already holds a typed `PrismaClient`. Pass it through: adapter takes `prisma: PrismaClient`, hands it to the repository (also typed), no cast at any layer. Only `repositories/prisma/**` and `adapters/postgres.*.adapter.ts` may name `PrismaClient` — that is what `prisma-containment` allows, and `typed-prisma-seam` rejects any new file that reintroduces the old untyped shape. Full shape in `dev/docs/ARCHITECTURE.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
