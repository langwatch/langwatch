...
import { RecordCanonicalLogCommand } from "../log-processing.adapter.ts";
import { createLogProcessingPipeline } from "../log-processing.adapter.ts";
import {
  ...
  describe("given several queued records from one shard", () => {
    describe("when the coalesced batch is processed", () => {
      ...
      it("appends them as one insert holding every record in dispatch order", async () => {
        ...
        expect(events.map((event: CanonicalLogRecordReceivedEvent) => event.aggregateId)).toEqual(
          payloads.map((payload) => payload.recordId),
        );
        expect(
          events.every(
            (event: CanonicalLogRecordReceivedEvent) =>
              event.type === CANONICAL_LOG_RECORD_RECEIVED_EVENT_TYPE,
          ),
        ).toBe(true);
      ...
      it("keeps each record's idempotency key so a retry cannot duplicate it", async () => {
        ...
        expect(
          events.map((event: CanonicalLogRecordReceivedEvent) => event.idempotencyKey),
        ).toEqual(payloads.map((payload) => `${TENANT_ID}:${payload.recordId}`));
      });
    });
  });
});
