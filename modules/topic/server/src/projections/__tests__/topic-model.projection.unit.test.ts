...
describe("TopicModelFoldProjection", () => {
  ...
  describe("when a duplicate seed folds after the model already has topics", () => {
    // Regression: nothing upstream enforces the `seed:v1` idempotency key (no queue dedup on
    // recordTopics; ClickHouse inserts cannot be unique), so a boot seed racing the write-path
