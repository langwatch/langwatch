...
interface EmailSlice {
  members: string[];
  subject: { value: string; usingDefault: boolean };
  body: { value: string; usingDefault: boolean };
}

const EMPTY_TEMPLATES: TemplateDraft = {
...
const emailClient = {
  ...
  initialSlice: (): EmailSlice => ({
    members: [],
    subject: { value: "", usingDefault: true },
    body: { value: "", usingDefault: true },
  }),
  isComplete: (slice: unknown) => (slice as EmailSlice).members.length > 0,
  summary: (slice: unknown, identity: SummaryIdentity) =>
    `${identity.name} → email to ${(slice as EmailSlice).members.length} recipient`,
  fromTriggerRow: (_row: SavedTriggerRow): EmailSlice => emailClient.initialSlice(),
  toActionParams: (slice: unknown): EmailActionParams => ({
    members: (slice as EmailSlice).members,
  }),
  testFireTarget: () => ({ webhook: null }),
  templatesFromSlice: (slice: unknown): TemplateDraft => {
    const email = slice as EmailSlice;
    return {
      ...EMPTY_TEMPLATES,
      emailSubjectTemplate: email.subject.usingDefault ? null : email.subject.value,
      emailBodyTemplate: email.body.usingDefault ? null : email.body.value,
    };
  },
  ConfigForm: null,
};
...
const slackClient = {
  ...
  initialSlice: () => ({}),
  isComplete: () => true,
  summary: (_slice: unknown, identity: SummaryIdentity) => identity.name,
  fromTriggerRow: () => ({}),
  toActionParams: () => ({}),
  testFireTarget: () => ({ webhook: null }),
  templatesFromSlice: () => EMPTY_TEMPLATES,
  ConfigForm: null,
};
...
const CLIENT_PROVIDERS = createClientProviderRegistry(CLIENT_DEFINITIONS);

const model = createAutomationDraftModel(CLIENT_PROVIDERS);
const {
  INITIAL_DRAFT,
...
type AutomationDraft = WebAutomationDraft<typeof CLIENT_DEFINITIONS>;
...
describe("draftReducer", () => {
  describe("when SET_ACTION is dispatched", () => {
    it("changes the action but preserves every provider's slice", () => {
      ...
      expect((next.slices[TriggerAction.SEND_EMAIL] as { members: string[] }).members).toEqual([
        "a@acme.test",
      ]);
...
describe("SET_FILTER_QUERY", () => {
  ...
  it("is cleared when switching to an alert, which watches a metric", () => {
    ...
    expect(reducer(withQuery, { type: "SET_SOURCE", value: "customGraph" }).filterQuery).toBeNull();
  });
...
describe("notifyChannel + isNotifyAction", () => {
  it("returns the channel for notify providers and null otherwise", () => {
    ...
    expect(notifyChannel({ ...SAMPLE, action: TriggerAction.SEND_SLACK_MESSAGE })).toBe("slack");
