...
describe("filterBlockKit", () => {
  describe("when given a mix of allowed and disallowed blocks", () => {
    it("keeps allowlisted block types and drops the rest", () => {
      ...
      expect(blocks.map((b) => b.type)).toEqual(["header", "section", "divider", "markdown"]);
    });
  });
  ...
  describe("when a section text runs past Slack's character limit", () => {
    it("cuts it to the cap and marks it as truncated", () => {
      ...
      if (!textBlock || typeof textBlock !== "object") {
        throw new Error("expected a sanitized section text block");
      }
  ...
  describe("when a rich_text block carries mention elements", () => {
    it("drops broadcast/user/usergroup/channel inline elements (notification-abuse vector)", () => {
      ...
      if (!block || !Array.isArray(block.elements)) {
        throw new Error("expected rich-text elements");
      }
      const section = block.elements[0] as Record<string, unknown>;
  ...
  describe("when a rich_text block carries an unknown sub-block type", () => {
    it("keeps only allowlisted rich_text sub-block types", () => {
      ...
      const types = (block.elements as Record<string, unknown>[]).map((element) => element.type);
  ...
  describe("when a rich_text_list nests sections with mentions", () => {
    it("recursively sanitizes each list item's inline elements", () => {
      ...
      if (!block || !Array.isArray(block.elements)) {
        throw new Error("expected rich-text list");
      }
      const list = block.elements[0] as Record<string, unknown>;
      if (!list || !Array.isArray(list.elements)) {
        throw new Error("expected rich-text list item");
      }
      const item = list.elements[0] as Record<string, unknown>;
