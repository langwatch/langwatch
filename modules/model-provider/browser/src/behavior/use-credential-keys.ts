...
export type UseCredentialKeysActions = {
  setUseApiGateway: (use: boolean, onGatewayToggle?: (useGateway: boolean) => void) => void;
...
export function useCredentialKeys({
  ...
  const originalSchemaShape = useMemo<Record<string, unknown>>(() => {
    return providerDefinition?.keysSchema ? getSchemaShape(providerDefinition.keysSchema) : {};
  ...
  const displayKeys = useMemo(() => {
    return getDisplayKeysForProvider(provider.provider, useApiGateway, originalSchemaShape);
  ...
  const [customKeys, setCustomKeys] = useState<Record<string, string>>(() =>
    buildCustomKeyState({
      ...
      options: { providerEnabledWithEnvVars: provider.enabled },
    }),
  );
    ...
    (nextProvider: MaybeStoredModelProvider): boolean => {
      ...
      setCustomKeys(() =>
        buildCustomKeyState({
          displayKeyMap: nextDisplayKeys,
          storedKeys,
          options: { providerEnabledWithEnvVars: nextProvider.enabled },
        }),
      );
