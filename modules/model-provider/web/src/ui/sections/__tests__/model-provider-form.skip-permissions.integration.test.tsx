...
): ReturnType<typeof keyedRow> & { langySkipPermissionsModels: string[] | null } {
  return {
    ...keyedRow({
      providerKey: "openai",
      ...
      it("shows which line is invalid on the field, and saves nothing", async () => {
        mockMutateAsync.mockRejectedValue({
          error: {
            ...
            message: "Line 2 of the allowed models list is not a valid pattern.",
            meta: {
              line: 2,
              pattern: "^(unclosed",
              fieldErrors: {
        ...
        await waitFor(() =>
          expect(screen.getByText("Line 2 is not a valid pattern.")).toBeInTheDocument(),
        );
