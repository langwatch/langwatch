 ...
 *    builders (`langwatch-ql-access-model.service.ts`,
 *    `langwatch-ql-postgres-mapping.service.ts`,
 *    `langwatch-ql-view-provisioning.service.ts`) in the
 *    order the integration harness proves works: access model, bridge,
 *    views. Pure; `src/tasks/lwql-provision.task.ts` is the only caller with I/O.
 *
...
import { LWQL_POSTGRES_READER_ROLE } from "../../services/langwatch-ql-production-provisioning.service.ts";
...
export function selfHostedClickHouseProvisioningStatements({
  ...
  return [
    ...
    ...catalogShapes.postgresViews(LWQL_VIEW_CATALOG).map(
      (view) => `DROP TABLE IF EXISTS ${accessModel.qualified(names, view.sourceTable)}`,
    ),
