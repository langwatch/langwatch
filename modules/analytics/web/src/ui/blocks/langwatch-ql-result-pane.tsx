/**
 * A result has one fetched answer and several local views. Keep diagnostics
 * outside those views, preserve stale answers, and present failures through the
 * host error port rather than exposing transport messages.
 */
...
const DEFAULT_RESULT_VIEW: LangWatchQLResultView = "table";

export interface LangWatchQLResultPaneProps {
   ...
   */
  renderChartArea?: (view: LangWatchQLResultView, openSpecification: () => void) => ReactNode;
  /** App-owned error chrome; the package keeps the refusal structure portable. */
  renderError?: (error: unknown, fallbackTitle: string) => ReactNode;
}
...
function resultChip({
  ...
  state: LangWatchQLRequestState;
  failure: LangWatchQLFailure | undefined;
  isStale: boolean;
  ...
  if (state.outcome?.kind !== "result") return void 0;
  if (isStale) {
    return {
...
function EmptyState({ onInsertExample }: { onInsertExample?: () => void }) {
  return (
    <VStack flex="1" justify="center" align="center" padding={8} textAlign="center" gap={0}>
      <Text fontFamily="mono" fontSize="24px" color="fg.subtle" marginBottom={3} aria-hidden="true">
        SELECT …
      </Text>
...
function RunningState() {
  return (
    <VStack flex="1" justify="center" align="center" padding={8} gap={2} data-testid="lwql-loading">
 ...
 */
function tabIdFor(base: string, view: LangWatchQLResultView): string {
...
function ViewTabs({
  ...
  onViewChange: (view: LangWatchQLResultView) => void;
   ...
   */
  panelIdFor: (view: LangWatchQLResultView) => string | undefined;
}) {
  const entries: readonly { value: LangWatchQLResultView; label: string }[] = [
...
export function LangWatchQLResultPane({
  state,
  onRun,
  onInsertExample,
  renderChartArea,
  ...
  const [view, setView] = useState<LangWatchQLResultView>(DEFAULT_RESULT_VIEW);
  ...
  const hasChartPanel = renderChartArea !== void 0 && chartAreaVisited;
  ...
  const panelIdFor = (target: LangWatchQLResultView): string | undefined => {
    if (target === "table") return `${panelIdBase}-table`;
    return hasChartPanel ? `${panelIdBase}-chart-area` : void 0;
  };

  const { isStale, failure, result, chip } = readPaneModel(state);

  const changeView = (next: LangWatchQLResultView) => {
  ...
  if (state.outcome === null) {
    return <NoOutcomePane isInFlight={state.isInFlight} onInsertExample={onInsertExample} />;
  }
  ...
  return (
    ...
    >
      ...
      {state.outcome.kind === "error" && failure && (
        <FailureBlock error={state.outcome.error} failure={failure} renderError={renderError} />
      )}
      ...
      {result && (
        <ResultBody
          ...
          panelIdBase={panelIdBase}
          chartArea={
            renderChartArea && chartAreaVisited ? renderChartArea(view, openSpecification) : void 0
          }
        />
      )}
    </VStack>
...
function readPaneModel(state: LangWatchQLRequestState) {
  const isStale = isLangWatchQLResultStale(state);
  const failure =
    state.outcome?.kind === "error" ? readLangWatchQLFailure(state.outcome.error) : void 0;
  const result = state.outcome?.kind === "result" ? state.outcome.result : void 0;
  return {
    isStale,
    failure,
    result,
...
function ResultHeader({
  ...
  return (
    ...
    >
      ...
      <Box flex="1" />
      {isInFlight && (
        <HStack gap={2} color="fg.muted" data-testid="lwql-loading">
          <Spinner size="xs" />
...
function FailureBlock({
  ...
  renderError: ((error: unknown, fallbackTitle: string) => ReactNode) | undefined;
}) {
  return (
    <Stack gap={3} padding={5} overflowY="auto">
      {renderError ? (
        renderError(error, "Couldn't run the query")
      ) : (
      ...
      {failure.code === LWQL_UNPARSEABLE_CODE &&
  ...
  return (
    <>
      ...
      <Box
        ...
        display={view === "table" ? void 0 : "none"}
      >
      ...
      {chartArea !== void 0 && (
        <Box
          ...
          display={view === "table" ? "none" : void 0}
      ...
      <Box borderTopWidth="1px" borderColor="border" flexShrink={0}>
        <LangWatchQLDiagnostics diagnostics={result.diagnostics} />
        <LangWatchQLResultMeta statistics={result.statistics} />
      </Box>
    </>
  );
}
