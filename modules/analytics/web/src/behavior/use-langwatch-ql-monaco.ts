...
function hoverProvider({ readSchema }: { readSchema: ReadSchema }): languages.HoverProvider {
  return {
...
export function useLangWatchQLMonaco({
  ...
  const handleMount: OnMount = (instance, monaco) => {
    ...
    disposablesRef.current.push(
      ...
      monaco.languages.registerHoverProvider(LWQL_LANGUAGE, hoverProvider({ readSchema })),
    );
