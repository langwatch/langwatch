 ...
 * shell and each widget's own recharts source string. Split from widgets.ts so
 * each file stays under the source-line budget. @see ./analytics-v2-widgets.ts
 */
...
/** Shared guards: error message, loading, or the one empty-state on zero rows. */
...
/** Assemble one widget's source from its imports, row mapping, and chart element. */
function widgetCode({
  imports,
  rows,
  chart,
...
export const traceCountCode = widgetCode({
  imports: "ResponsiveContainer, LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip",
...
export const tokensCode = widgetCode({
  imports: "ResponsiveContainer, AreaChart, Area, CartesianGrid, XAxis, YAxis, Tooltip, Legend",
  rows: `  const rows = data.map(function (row) {
    return {
...
export const latencyCode = widgetCode({
  imports: "ResponsiveContainer, LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, Legend",
  rows: `  const rows = data.map(function (row) {
    return {
      bucket: String(row.bucket).slice(0, 10),
      p50: Number(row.p50),
