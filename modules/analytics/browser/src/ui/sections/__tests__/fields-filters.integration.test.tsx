...
describe("FieldsFilters", () => {
  ...
  describe("when rendering", () => {
    ...
    it("shows count badge when multiple values selected", () => {
      renderComponent({
        filters: { "metadata.labels": ["Production", "Staging"] } as Record<FilterField, string[]>,
      });
   ...
   * The Popover + virtualizer's Chakra v3 Checkbox fires `onChange` unreliably,
   * so the click handler lives on the `<HStack>` row container instead.
   */
