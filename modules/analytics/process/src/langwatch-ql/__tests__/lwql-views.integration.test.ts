...
const GATED_COLUMN_POSITIONS = (database: string) =>
  [
    ...
    ["window", `SELECT row_number() OVER (PARTITION BY CapturedInput) FROM ${database}.traces`],
    [
      "subquery",
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when the catalog is compared against the tables the migrations created", () => {
  /**
  ...
  it("declares the sort key and the engine its source table actually has", async () => {
    ...
    for (const view of resident) {
      ...
      ).toEqual(expect.arrayContaining(view.dedup.versionColumn ? [view.dedup.versionColumn] : []));
    }
  });
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when a source table holds two versions of one row", () => {
  ...
  it("returns one row for a record whose two versions carry two sort keys", async () => {
    ...
    expect(Number(deduped[0]!.Score), "the view returned the stale version").toBeCloseTo(
      EVALUATION_DEDUP_FIXTURE.latestScore,
    );
    expect(Number(deduped[0]!.DurationMs)).toBe(EVALUATION_DEDUP_FIXTURE.latestDurationMs);
  });
  ...
  it("returns exactly one row per record for every view", async () => {
    for (const view of LWQL_VIEW_CATALOG) {
      ...
      expect(Number(duplicated), `${view.name} returns more than one row for some key`).toBe(0);
    }
  });
});
 ...
 */
describe("given the LangWatchQL views provisioned over the shipped fact tables when a rollup table holds two partial rows for one bucket", () => {
  /**
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when the restricted identity names a column outside the catalog", () => {
  ...
  it("is refused by the database, while a catalog column reads", async () => {
    ...
    await expectClickHouseError(
      () =>
        selectRows(tenantA, `SELECT ${OFF_CATALOG_COLUMN} FROM ${facts}.trace_summaries LIMIT 1`),
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when captured content is reached through a LangWatchQL view", () => {
  ...
  it("strips every content key from the attribute maps while keeping the dimensions", async () => {
    ...
    for (const key of CONTENT_CATEGORIES.flatMap((category) => CONTENT_KEY_CATALOG[category])) {
      expect(
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when the LangWatchQL database's own definitions are audited", () => {
  ...
  it("covers every object the restricted identity can read, in every database", async () => {
    ...
    for (const row of coverage.filter((entry) =>
      ...
      expect(Number(row.has_policy), `${row.table} is a physical table with no row policy`).toBe(1);
    }
  });
...
describe("given the LangWatchQL views provisioned over the shipped fact tables when a caller's time predicate is applied through a view", () => {
  /**
  ...
  it("prunes partitions, and each dedup strategy's cost is recorded", async () => {
    ...
    const strategies = ["none", "in-tuple", "final"] satisfies LangWatchQLDedupStrategy[];
