...
describe("validateLangWatchQL given SQL the LangWatchQL API is meant to answer", () => {
  it.each([
    ...
    ["DISTINCT with LIMIT BY", "SELECT DISTINCT Model FROM traces ORDER BY Model LIMIT 1 BY Model"],
...
describe("validateLangWatchQL given a statement that is not a read query", () => {
...
describe("validateLangWatchQL given a SETTINGS clause", () => {
  it.each([
...
describe("validateLangWatchQL given a clause that redirects output", () => {
  it.each([
...
describe("validateLangWatchQL given a reference to server metadata", () => {
  it.each([
...
describe("validateLangWatchQL given a table outside the caller's catalog", () => {
  it.each([
...
describe("validateLangWatchQL given a table function", () => {
  it.each([
...
describe("validateLangWatchQL given a restricted field", () => {
  /**
...
/**
 * A column set does not become resolvable by being wrapped in a function or buried in a
 * clause, so the gate refuses it wherever it appears. The one exemption is a bare `count(*)`,
 * where the star names a row count and reveals no column.
 */
describe("validateLangWatchQL given a column set outside the projection", () => {
  const regexPositions: readonly [string, (matcher: string) => string][] = [
  ...
  const regexCases: [string, string][] = regexPositions.flatMap(([where, build]) =>
    regexMatchers.map((matcher): [string, string] => [`${matcher} in ${where}`, build(matcher)]),
  );
  ...
  const wildcardCases: [string, string][] = [
    ...
    ["tuple(* REPLACE (1 AS TraceId))", "SELECT tuple(* REPLACE (1 AS TraceId)) FROM traces"],
    ...
    ["COLUMNS('^Trace') APPLY (toString)", "SELECT COLUMNS('^Trace') APPLY (toString) FROM traces"],
    [
  ...
  it.each([
    ...
    ["count(*) OVER (PARTITION BY *)", "SELECT count(*) OVER (PARTITION BY *) FROM traces"],
  ])("refuses %s", (_case, sql) => {
    expect(codesOf(validate(sql))).toContain("WILDCARD_NOT_ALLOWED");
  });
  ...
  it.each([
    ...
    ["a table-qualified path through it", "SELECT traces.body.null FROM traces"],
  ])("refuses %s", (_case, sql) => {
  ...
  it.each([
    ["a subfield of the gated column", "SELECT TraceId, body.null FROM traces"],
    ["a table-qualified path through it", "SELECT traces.body.null FROM traces"],
    ["an alias-qualified path through it", "SELECT t.body.null FROM traces AS t"],
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    const violation = result.violations.find((entry) => entry.code === "GATED_COLUMN");
  ...
  it("accepts COLUMNS(TraceId) and refuses COLUMNS(body) by the named field", () => {
    ...
    expect(codesOf(validate("SELECT COLUMNS(body) FROM traces"))).toEqual(["GATED_COLUMN"]);
  });
});

describe("validateLangWatchQL given nesting past the configured depth", () => {
...
describe("validateLangWatchQL given a join shape the LangWatchQL schema does not define", () => {
...
describe("validateLangWatchQL given a violation whose refusal should name what exists", () => {
  ...
  it("lists the caller's allowed views on an unknown view", () => {
    ...
    expect(result.ok).toBe(false);
    if (result.ok) return;
    const violation = result.violations.find((entry) => entry.code === "TABLE_NOT_ALLOWED");
    expect(violation?.availableViews).toEqual(["analytics.spans", "analytics.traces"]);
  });

  /** @scenario "A TABLE_NOT_ALLOWED violation names the views that exist" */
  it("sorts and deduplicates availableViews regardless of the policy's own order", () => {
    const result = validate("SELECT id FROM billing.invoices", {
      allowedTables: ["analytics.traces", "analytics.spans", "analytics.traces"],
      gatedColumns: [],
      defaultDatabase: "analytics",
    });
  ...
  it.each([
    ...
    ["MULTIPLE_STATEMENTS", "SELECT TraceId FROM traces; SELECT TraceId FROM spans"],
    ["STATEMENT_NOT_ALLOWED", "INSERT INTO traces VALUES (1)"],
    ["SETTINGS_CLAUSE", "SELECT TraceId FROM traces SETTINGS max_threads = 1"],
    ...
    ["APP_FUNCTION_NAME_CASE", "SELECT THREAD_TRACES(ConversationId) AS ids FROM traces"],
    ["APP_FUNCTION_ALIAS_REQUIRED", "SELECT thread_traces(ConversationId) FROM traces"],
    ["APP_FUNCTION_ARGUMENT", "SELECT thread_traces() AS ids FROM traces"],
    ["APP_FUNCTION_GATED", "SELECT conversation(ConversationId) AS transcript FROM traces"],
  ] as [LangWatchQLViolationCode, string][])("names a hint for %s", (code, sql) => {
    const policy =
  ...
  it("covers every violation code with a hint case", () => {
    ...
    expect([...covered].toSorted()).toEqual([...LWQL_VIOLATION_CODES].toSorted());
  });
...
describe("validateLangWatchQL given a query that breaks several rules at once", () => {
 ...
 */
describe("validateLangWatchQL given an accepted query whose shape a diagnostic will read", () => {
...
describe("validateLangWatchQL given the row cap on what one request returns", () => {
  ...
  it("leaves a statement that pages with OFFSET alone", () => {
    const result = validate("SELECT TraceId FROM traces ORDER BY TraceId LIMIT 20 OFFSET 40");
    expect(result.ok && result.appendRowLimit).toBe(false);
  });
  ...
  it("refuses a UNION branch whose own LIMIT is over the cap", () => {
    const result = validate(
      "SELECT TraceId FROM traces LIMIT 5 " + "UNION ALL SELECT TraceId FROM spans LIMIT 99999",
    );
    expect(codesOf(result)).toEqual(["LIMIT_TOO_HIGH"]);
  });
  ...
  it("refuses a UNION where one branch names no LIMIT at all", () => {
    const result = validate(
      "SELECT TraceId FROM traces LIMIT 1 " + "UNION ALL SELECT TraceId FROM spans",
    );
  ...
  it("accepts a UNION where every branch names its own bounded LIMIT", () => {
    const result = validate(
      "SELECT TraceId FROM traces LIMIT 5 " + "UNION ALL SELECT TraceId FROM spans LIMIT 5",
    );
    expect(codesOf(result)).toEqual([]);
    expect(result.ok && result.appendRowLimit).toBe(false);
  });
...
/**
 * Gated means refused, not dropped: `annotations` carries `Comment` (gated `output` by the
 * derivation), so a caller without content access learns the column exists and why it was
 * refused, instead of silently getting fewer columns than it asked for.
 */
describe("validateLangWatchQL given the shipped catalog's content gates", () => {
