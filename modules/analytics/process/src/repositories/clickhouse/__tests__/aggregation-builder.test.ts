...
const storedSpansSelectLists = (sql: string): string[][] =>
  Array.from(sql.matchAll(/JOIN \(SELECT (.+?) FROM stored_spans/g)).map(([, columns]) =>
...
describe("aggregation-builder", () => {
  ...
  describe("buildTimeseriesQuery()", () => {
    ...
    it("does not inflate trace-level aggregations when mixed with evaluation metrics in simple path", () => {
      const input = {
        ...
        series: [
          ...
          {
            metric: "evaluations.evaluation_pass_rate" as AnalyticsSeries["metric"],
    ...
    // The query windows on trace OccurredAt, but evaluation_runs partitions on ScheduledAt
    // (UpdatedAt on older deployments) — an OccurredAt filter prunes nothing there, so an
    // unbounded JOIN subquery walks the tenant's ENTIRE evaluation history across every
    // partition, including S3-tiered cold ones. Measured in production at ~900 executions /
    // 6,400s of query time per 30min on a single worker pod before the bound existed.
    it("bounds the evaluation_runs JOIN subquery on both candidate partition columns", () => {
      ...
      const scheduledBounds =
        result.sql.split("ScheduledAt >= {previousStart:DateTime64(3)}").length - 1;
    ...
    it("handles metadata.user_id mixed with evaluation metrics correctly", () => {
      const input = {
        ...
        series: [
          {
            metric: "metadata.user_id" as AnalyticsSeries["metric"],
            aggregation: "cardinality" as const,
          },
          {
            metric: "evaluations.evaluation_pass_rate" as AnalyticsSeries["metric"],
    ...
    it("every ${ts}.<Column> reference in metric-translator is handled", async () => {
      ...
      const source = await fs.readFile(
        path.resolve(__dirname, "../clickhouse.metric-translator.mapper.ts"),
        "utf8",
      );
    ...
    it("does not JOIN stored_spans when metadata.span_type uses cardinality alongside trace-level metrics", () => {
      const input = {
        ...
        series: [
          {
            metric: "metadata.span_type" as AnalyticsSeries["metric"],
            key: "llm",
            aggregation: "cardinality" as const,
          },
          {
    ...
    it("adds JOINs when metrics require them", () => {
      const input = {
        ...
        series: [
          {
            metric: "evaluations.evaluation_score" as AnalyticsSeries["metric"],
            aggregation: "avg" as const,
          },
        ],
      };
    ...
    it("partitions model grouping by each span's own model", () => {
      ...
      expect(result.sql).toContain("if(empty(ts.Models), 'unknown', ts.Models[1])");
    });
    ...
    it("includes all date parameters", () => {
      ...
      expect(result.params.previousStart).toEqual(baseInput.previousPeriodStartDate);
    ...
    describe("when the query leaves out trace origins", () => {
      ...
      it("keeps the exclusion outside a negated filter selection", () => {
        ...
        expect(result.sql).toMatch(/AND \(NOT \(.*ts\.TopicId IN/s);
        expect(result.sql).not.toMatch(/NOT \([^)]*\{excludeOrigins:Array\(String\)\}/);
      });
    });
    ...
    it("builds date-bucketed pipeline query with evaluation groupBy", () => {
      const input = {
        ...
        series: [
          {
            metric: "evaluations.evaluation_runs" as AnalyticsSeries["metric"],
            aggregation: "cardinality" as const,
            pipeline: {
              field: "trace_id" as const,
              aggregation: "sum" as const,
    ...
    describe("when groupBy is evaluation field with cross-evaluator metrics", () => {
      ...
      it("excludes global EvaluatorId WHERE filter for evaluation_label groupBy", () => {
        ...
        expect(whereSection).not.toMatch(/AND\s+es\.EvaluatorId\s*=\s*\{groupByKey:String\}/);
      });
    ...
    describe("when evaluation pass rate metric is combined with labels groupBy", () => {
      ...
      it("rewrites es.Passed and es.Status to CTE column aliases", () => {
        ...
        const outerSelect = result.sql.split("FROM deduped_traces")[0]!.split(")\n    SELECT")[1]!;
  ...
  describe("buildDataForFilterQuery()", () => {
    ...
    it("builds query for topics.topics", () => {
      const result = buildDataForFilterQuery(projectId, "topics.topics", startDate, endDate);
    ...
    it("builds query for topics.subtopics", () => {
      const result = buildDataForFilterQuery(projectId, "topics.subtopics", startDate, endDate);

      expect(result.sql).toContain("ts.SubTopicId AS field");
    });

    it("builds query for metadata.user_id", () => {
      const result = buildDataForFilterQuery(projectId, "metadata.user_id", startDate, endDate);

      expect(result.sql).toContain("Attributes['langwatch.user_id']");
    });

    it("builds query for metadata.thread_id", () => {
      const result = buildDataForFilterQuery(projectId, "metadata.thread_id", startDate, endDate);

      expect(result.sql).toContain("Attributes['gen_ai.conversation.id']");
    });

    it("builds query for spans.model with JOIN", () => {
      const result = buildDataForFilterQuery(projectId, "spans.model", startDate, endDate);
    ...
    it("builds query for spans.type with JOIN", () => {
      const result = buildDataForFilterQuery(projectId, "spans.type", startDate, endDate);
    ...
    it("builds query for traces.error", () => {
      const result = buildDataForFilterQuery(projectId, "traces.error", startDate, endDate);
    ...
    it("returns empty result for unknown field", () => {
      const result = buildDataForFilterQuery(projectId, "unknown.field" as any, startDate, endDate);
  ...
  describe("buildTopDocumentsQuery()", () => {
    ...
    it("includes filters when provided", () => {
      ...
      const result = buildTopDocumentsQuery(projectId, startDate, endDate, filters);
  ...
  describe("buildFeedbacksQuery()", () => {
    ...
    it("prunes the stored_spans join to the StartTime partition window", () => {
      ...
      expect(result.sql).toContain("StartTime >= {startDate:DateTime64(3)} - INTERVAL 2 DAY");
      expect(result.sql).toContain("StartTime < {endDate:DateTime64(3)} + INTERVAL 2 DAY");
    });
    ...
    it("includes filters when provided with parameterized values", () => {
      ...
      const result = buildFeedbacksQuery(projectId, startDate, endDate, filters);
  ...
  describe("extractTraceAggregationColumn()", () => {
    ...
    it("handles dot-access columns wrapped in coalesce+sum", () => {
      expect(extractTraceAggregationColumn("coalesce(sum(ts.TotalCost), 0)")).toBe("ts.TotalCost");
    });
  });
  ...
  describe("when transforming composite metrics for dedup", () => {
    ...
    it("throws on a partially rewritable expression instead of emitting out-of-scope SQL", () => {
      expect(() =>
        transformMetricForDedup(`sum(ts.TotalCost + ts.SomeBrandNewColumn) AS t`, "t"),
