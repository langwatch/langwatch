/**
 * The Prisma schema parser turns a schema's text into the model/field facts the Postgres catalog is
 * derived from.
 */

import { describe, expect, it } from "vitest";

import { type PrismaModel, parsePrismaSchema } from "../lwql-prisma-schema.rules.ts";
...
describe("given an inline Prisma schema", () => {
  ...
  describe("when a field carries modifiers", () => {
    ...
    it("attaches /// docs to the field directly below them", () => {
      expect(field(project, "color")?.documentation).toBe("The owning colour choice.");
    });
  });

  describe("when a primary key is composite", () => {
    it("reads @@id as the field-name list", () => {
      expect(modelByName.get("Membership")?.primaryKey).toEqual(["teamId", "userId"]);
    });
  ...
  describe("when reading enums", () => {
    it("lists every value", () => {
      expect(manifest.enums).toEqual([{ name: "Color", values: ["RED", "GREEN", "BLUE"] }]);
    });
  });
});
