...
type CatalogColumn = (typeof LWQL_VIEW_CATALOG)[number]["columns"][number];

/** The source columns a column's expression names bare, which a projection alias could shadow. */
function unqualifiedSources(column: CatalogColumn): string[] {
  ...
  return column.sourceColumns.filter((source) => bare.includes(source));
}

/** The unit a column measures in, read off its name and published sentence, never its field. */
function measuredUnitOf(column: CatalogColumn): string | null {
  if (column.name.endsWith("Ms")) return "ms";
  if (/\bin USD\b/.test(column.description)) return "USD";
  return column.name.endsWith("TokenCount") ? "tokens" : null;
}

/** Every column name two views gate differently, with each view's gates. */
function gateConflictsByName(): string[] {
...
/** Everything one catalog entry promises a caller, checked against what it exposes. */
function expectViewShapeDeclared(view: (typeof LWQL_VIEW_CATALOG)[number]): void {
  ...
  for (const key of view.joinKeys) {
    expect(columnNames, `${view.name} advertises a join key it does not expose`).toContain(key);
  }
  ...
  const grouped =
    ...
    catalogShapes.grainColumns(view).length < view.dedup.keyColumns.length;
  for (const column of [
    ...(grouped ? [] : view.dedup.keyColumns),
    ...catalogShapes.grainColumns(view),
  ]) {
    expect(columnNames, `${view.name} declares a key column it does not expose`).toContain(column);
  }
  ...
  if (catalogShapes.isPostgresResident(view)) return;
  ...
  for (const column of [
    ...
    expect(sourceColumns, `${view.name} deduplicates on ${column} without granting it`).toContain(
      column,
    );
  }
}

describe("given the LangWatchQL view catalog when each entry's shape is checked", () => {
  ...
  it("declares a grain, join keys, a time column, and a dedup rule per view", () => {
    ...
    for (const view of LWQL_VIEW_CATALOG) expectViewShapeDeclared(view);
  });
  ...
  it("qualifies every source-column reference, so no projection alias can shadow one", () => {
    const unqualified = LWQL_VIEW_CATALOG.flatMap((view) =>
      view.columns.flatMap((column) =>
        unqualifiedSources(column).map((source) => `${view.name}.${column.name} names ${source}`),
      ),
    );
  ...
  it("declares a unit on every column that measures something", () => {
    ...
    for (const view of LWQL_VIEW_CATALOG) {
      for (const column of view.columns) {
        const expected = measuredUnitOf(column);
        if (expected === null) continue;
        checked += 1;
        expect(column.unit, `${view.name}.${column.name} measures ${expected}`).toBe(expected);
      }
    }
    expect(
      checked,
  ...
  it("gates a column name identically on every view that exposes it", () => {
    const conflicts = gateConflictsByName();
...
describe("given the LangWatchQL view catalog when a column reads a key out of an attribute map", () => {
  /**
  ...
  it("gates it exactly as the data-privacy policy classifies that key", () => {
    ...
    expect(checked, "no column reads a map key — this guard is inspecting nothing").toBeGreaterThan(
      0,
    );
  });
  ...
  it("keeps every content key out of the ungated columns", () => {
    for (const view of LWQL_VIEW_CATALOG) {
      for (const column of view.columns.filter(
        (candidate) => !catalogShapes.isContentGated(candidate),
      )) {
        const expression = expressionOf(column);
        for (const match of expression.matchAll(MAP_KEY_ACCESS)) {
          expect(
...
describe("given the LangWatchQL view catalog when the map filter is compared with the data-privacy catalog", () => {
  /** @scenario "The gated column set is derived from the data privacy policy, not hand-listed" */
  it("excludes every key the policy classifies as content, and its exploded form", () => {
    ...
    for (const key of expected) {
      ...
      expect(sql, `the filter does not exclude the exploded form of ${key}`).toContain(`'${key}.'`);
  ...
  it("filters the content keys out of every map column any dataset exposes", () => {
    let checked = 0;
    for (const view of LWQL_VIEW_CATALOG) {
      for (const column of view.columns.filter((candidate) => candidate.type.startsWith("Map("))) {
        checked += 1;
        expect(
          expressionOf(column),
    ...
    expect(checked, "no dataset exposes a map — this guard is inspecting nothing").toBeGreaterThan(
      0,
    );
  });
});
 ...
 */
describe("given the LangWatchQL view catalog when the analytics projections are inspected", () => {
 ...
 */
describe("given the LangWatchQL view catalog when a dataset's source aggregates rather than supersedes", () => {
  ...
  it("leaves a versioned dataset's version column required, so this is not a blanket exemption", () => {
    const versioned = LWQL_VIEW_CATALOG.filter(
      (view) => !view.dedup.aggregating && !catalogShapes.isPostgresResident(view),
    );
    expect(
      versioned.length,
 ...
 */
describe("given the LangWatchQL view catalog when a dataset's source sorts by a column its write path moves", () => {
  ...
  it("deduplicates by the record rather than by the engine's key", () => {
    ...
    expect([...catalogShapes.grainColumns(evaluationMetrics)]).toEqual([
      "TenantId",
      "EvaluationId",
    ]);
    expect(
      catalogShapes.grainColumns(evaluationMetrics).length,
  ...
  it("leaves the datasets whose sort keys hold still on the shipped default", () => {
    ...
    for (const view of pinned) {
      expect(
        catalogShapes.grainColumns(view).length,
 ...
 */
describe("given the LangWatchQL view catalog when a summed measure is declared", () => {
...
describe("given the LangWatchQL view catalog when a caller's permissions are turned into the validator's gated set", () => {
  /** @scenario "The gated column set is derived from the data privacy policy, not hand-listed" */
  ...
  it("separates the cost gate from the content gates", () => {
    ...
    expect(contentOnly).toEqual([...catalogShapes.contentGatedColumns(LWQL_VIEW_CATALOG)]);
...
describe("given the LangWatchQL view catalog when a dataset is gated as a whole", () => {
  ...
  it("withholds every column of the hidden dataset", () => {
    const withheld = catalogShapes.gatedColumns({
      protections: holding(false, true),
      views,
    });
  ...
  it("withholds none of them from a caller holding the dataset's permission", () => {
    expect(catalogShapes.gatedColumns({ protections: holding(true, true), views })).toEqual([]);
  });
});
