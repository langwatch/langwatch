/**
 * Worked LangWatchQL statements, one definition for every reader: the schema
 * endpoint's per-dataset example is mechanical and teaches nothing about the
 * questions people ask. See ADR-154.
...
/**
 * The database qualifier every published statement is written against — the
 * production spelling `DEFAULT_LWQL_DATABASE` carries, which a deployment
 * serving the views elsewhere rewrites out of the document it publishes.
 */
export const LWQL_EXAMPLE_DATABASE = "analytics";
 ...
 * predicate is what prunes partitions: without it the read touches every
 * partition the tenant has, including the cold ones on object storage.
 */
    ...
    parameters: [
      {
        ...
        type: "DateTime64(3)",
        description: "The previous page's last `OccurredAt`. Start from '1970-01-01 00:00:00.000'.",
      },
      {
        name: LWQL_KEYSET_AFTER_ID_PARAMETER,
        type: "String",
        description: "The previous page's last `TraceId`. Start from the empty string.",
      },
    ],
    gates: ["input", "output"],
    notes:
