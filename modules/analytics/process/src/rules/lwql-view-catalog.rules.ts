/**
 * `analytics.*` view catalog: one entry per LangWatchQL view, the whole grantable public surface.
 * Free-text/no-gate columns and event-sourcing bookkeeping are deliberately absent; each trace's
 * several view kinds project one event stream, not duplicate answers -- grain and dedup vary.
...
const TRACE_METRICS: LangWatchQLViewDefinition = {
  ...
  columns: [
    ...
    // `HasAnnotation`, `EarliestSpanStartMs` and `Attributes` are withheld: the first would answer
    // "annotated?" two ways against the authoritative `annotations` table, the second is fold
    // working state (migration 00061) not a trace fact, and the third's silent trim can't tell a
    // caller absent from evicted -- re-add once the trim marks what it dropped.
  ],
};
...
/**
 * LangWatchQL schema, ClickHouse facts then PostgreSQL entities/dimensions, in publish order --
 * one catalog since residence is a dataset property; only provisioning generators ask an
 * entry's residence. Generations/sessions stay unexposed: derived from spans/traces, not mapped.
 */
