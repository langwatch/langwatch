...
export function resetManagementApiTokenCache(): void {
  cachedToken = null;
}
 ...
 */
export async function getManagementApiToken(config: Auth0Config): Promise<string> {
  if (
    cachedToken &&
...
export async function changeAuth0Password(args: {
  ...
  const token = await getManagementApiToken(args.config);
