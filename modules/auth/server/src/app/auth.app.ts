 ...
 * neither reachable from the other. The public-environment procedure is gone
 * now (its one answer, the sign-in provider name, joined the shell config),
 * and every operation left is a process port's own.
 *
 * This door is unauthenticated by design, so the caller arrives as the
