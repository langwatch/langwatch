...
import { partsShownOnce, textWithoutSaidLines } from "../langy-shown-once.ts";
...
describe("given a turn that said its lines and then wrote them again", () => {
  it("keeps the said lines and empties the reply that repeated them", () => {
    ...
    expect(parts.map((part) => part.type)).toEqual(["tool-say", "tool-say", "text"]);
  ...
  it("reads a line that differs only in spacing as the same line", () => {
    expect(textWithoutSaidLines("  I said   this.  ", new Set(["I said this."]))).toBe("");
  });
  ...
  it("keeps a line that differs by a word", () => {
    ...
    expect(kept).toBe("I opened a pull request with the tracing change and the tests.");
  });
});
...
describe("given a proposal that was both said and asked on a card", () => {
  ...
  it("leaves a said line that no card asks", () => {
    ...
    expect(parts.map((part) => part.type)).toEqual(["tool-say", "tool-question", "text"]);
  });
});
