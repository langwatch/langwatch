...
import {
  ...
  renderSecretSnippet,
  secretSnippetCalls,
} from "../langy-secret-snippet-tool.ts";

const TEMPLATE =
  'export OPENAI_BASE_URL="https://gateway.acme.example/v1"\nexport OPENAI_API_KEY="{{secret}}"';

function call(id: string, input: Record<string, unknown>, state = "output-available") {
...
describe("secretSnippetCalls", () => {
  ...
  describe("given a call still streaming its input", () => {
    it("draws nothing until the input is complete", () => {
      expect(
        secretSnippetCalls([
          call("c1", { revealId: "rvl_abc", template: TEMPLATE }, "input-streaming"),
        ]),
      ).toEqual([]);
    });
  });
  ...
  describe("given other tool parts", () => {
    it("recognises only the secret_snippet tool", () => {
      ...
      expect(isSecretSnippetToolPart({ type: "tool-question", toolCallId: "q" })).toBe(false);
