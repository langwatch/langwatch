...
describe("shouldResumeAdoptedTurn", () => {
  ...
  it("never resumes the turn this tab's own send started", () => {
    expect(shouldResumeAdoptedTurn({ ...adopted, dispatchedTurnId: "turn-9" })).toBe(false);
  });

  it("resumes each adopted turn once", () => {
    expect(shouldResumeAdoptedTurn({ ...adopted, resumedTurnId: "turn-9" })).toBe(false);
    expect(shouldResumeAdoptedTurn({ ...adopted, resumedTurnId: "turn-8" })).toBe(true);
  });

  it("waits while no turn is in flight or the store tracks none", () => {
    expect(shouldResumeAdoptedTurn({ ...adopted, turnActive: false })).toBe(false);
    expect(shouldResumeAdoptedTurn({ ...adopted, activeTurnId: null })).toBe(false);
  });

  it("waits while the engine is already streaming", () => {
    expect(shouldResumeAdoptedTurn({ ...adopted, isStreaming: true })).toBe(false);
  });

  it("waits for a pending history load to own the engine first", () => {
    expect(shouldResumeAdoptedTurn({ ...adopted, isHistoryLoadPending: true })).toBe(false);
  });
  ...
  it("waits until the turn's own user message is the last one in the engine", () => {
    ...
    expect(shouldResumeAdoptedTurn({ ...adopted, lastEngineRole: "assistant" })).toBe(false);
    expect(shouldResumeAdoptedTurn({ ...adopted, lastEngineRole: null })).toBe(false);
  });
});
...
describe("shouldRefetchHistoryForAdoptedTurn", () => {
  ...
  it("waits while no turn is in flight or the store tracks none", () => {
    expect(shouldRefetchHistoryForAdoptedTurn({ ...foldAhead, turnActive: false })).toBe(false);
    expect(shouldRefetchHistoryForAdoptedTurn({ ...foldAhead, activeTurnId: null })).toBe(false);
  });
  ...
  it("stays quiet while no transcript has been read for the conversation", () => {
    ...
    // turn in it.
    expect(shouldRefetchHistoryForAdoptedTurn({ ...foldAhead, hasHistory: false })).toBe(false);
  });
