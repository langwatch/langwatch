...
function domain(overrides: Partial<LangyDomainError>): LangyDomainError {
  return { code: "unknown", httpStatus: 500, meta: {}, retryable: false, ...overrides };
}
...
describe("explainLangyError", () => {
    ...
    describe("when the failure is explained", () => {
      ...
      it("leaves the more specific codex card alone", () => {
        const presentation = explainLangyError(
          domain({
            code: "langy_agent_errored",
            reasons: [{ kind: "upstream_unauthorized" }, { kind: "codex_session_expired" }],
          }),
        );
  ...
  describe("given a genuinely unexpected failure", () => {
    it("keeps the calm generic copy and the trace id", () => {
      const presentation = explainLangyError(domain({ code: "unknown", traceId: "abc123" }));
...
describe("given the platform could not read the conversation back", () => {
  ...
  it("says the read failed in the shared words, with a retry, and never blames Langy", () => {
    ...
    expect(`${presentation.title} ${presentation.description}`).not.toContain("Langy");
  });
});
