...
/** The kinds `makeService` below wires — everything the ported test dispatches. */
const FAKE_DEFINITIONS: Record<string, LangyUiActionDefinition> = {
  "workbench.duplicateTarget": {
    payloadSchema: duplicateTargetSchema,
    requiredPermission: "experiments:update",
  },
  // Declares a budget far over the ceiling, so the total wait is the ceiling
  // itself and not this number — see the "server gives up" scenario below.
  "workbench.run": {
 ...
 */
function makeRedis(blpopBehavior: ("wait-empty" | (() => void))[] = []): {
  redis: UiActionRedis;
  store: FakeStore;
  blpopCalls: number[];
...
describe("LangyUiActionService", () => {
  ...
  describe("when no live tab is present for the project", () => {
    ...
    it("publishes anyway and answers from the backend after the claim window", async () => {
      ...
      const runnerCalls: { kind: string; experimentSlug?: string }[] = [];
