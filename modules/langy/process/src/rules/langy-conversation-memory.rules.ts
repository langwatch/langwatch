...
export function extractLangyConversationMemory({
  ...
  for (const message of messages) {
    ...
    turn += 1;
    for (const entry of message.parts.flatMap((part) => memoryEntriesOf({ part, turn }))) {
...
function memoryEntriesOf({
  ...
  if (isErrored(part)) return [];
  const digest = digestOf(part);
  if (!digest) return [];

  const ids = (digest.primaryId ? [digest.primaryId] : (digest.ids ?? []))
    .map((value) => sanitizeLangyPromptValue(value, MAX_MEMORY_ID_LENGTH))
...
function spokenMessages({
  ...
  return messages.flatMap((message) => {
    ...
    const text = sanitizeTranscriptText({
      value: extractLangyTextFromParts(message.parts),
      segmenter,
    });
...
export function renderLangyConversationTranscript({
  ...
  /** Splits a long message on grapheme boundaries; the caller constructs it. */
  segmenter: Intl.Segmenter;
}): string | null {
  const spoken = spokenMessages({ messages, segmenter });
  const last = spoken[spoken.length - 1];
  if (
    last &&
    last.role === "user" &&
    currentPrompt !== undefined &&
    last.text === sanitizeTranscriptText({ value: currentPrompt, segmenter })
  ) {
    spoken.pop();
  }
...
function sanitizeTranscriptText({
  ...
  const graphemes = Array.from(segmenter.segment(cleaned), ({ segment }) => segment);
