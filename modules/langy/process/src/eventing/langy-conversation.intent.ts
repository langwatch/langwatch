import type { Command, CommandHandler, IntentExecutor } from "@langwatch/eventing";
import {
  defineCommand,
  defineCommandSchema,
  EventUtils,
...
export const RequestLocalControlCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:local-request:${d.requestId}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
  }),
});
...
export const ConnectLocalWorkspaceCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:local-connect:${d.instanceId}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
  }),
});
...
export const DisconnectLocalWorkspaceCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:local-disconnect:${d.instanceId}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
  }),
});
...
export const ChangeLocalPolicyCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:local-policy:${d.occurredAt}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
  }),
});
...
export const StartUserWaitCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:wait-start:${d.waitId}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
    "payload.turn.id": d.turnId,
...
export const EndUserWaitCommand = defineCommand({
  ...
  aggregateId: (d) => d.conversationId,
  idempotencyKey: (d) => `${d.tenantId}:${d.conversationId}:wait-end:${d.waitId}`,
  spanAttributes: (d) => ({
    "payload.conversation.id": d.conversationId,
    "payload.turn.id": d.turnId,
