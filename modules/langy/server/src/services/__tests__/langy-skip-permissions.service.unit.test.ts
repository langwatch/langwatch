...
function row(overrides: Partial<SkipPermissionsProviderRow> = {}): SkipPermissionsProviderRow {
  return {
    id: "mp_anthropic",
    provider: "anthropic",
    routingHandle: null,
  ...
  describe('given a conversation running on "anthropic/claude-fable-5-1"', () => {
    describe("when the gate is asked whether the model may skip", () => {
      ...
      it("matches the bare model against the Anthropic list and answers yes", async () => {
        ...
        expect(providerRows.findAllAccessibleForProject).toHaveBeenCalledWith(PROJECT_ID);
      });
    });
  });
