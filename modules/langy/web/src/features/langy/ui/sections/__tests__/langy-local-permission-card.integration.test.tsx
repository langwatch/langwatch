...
describe("given a command the command line will not run on its own", () => {
  ...
  it("names the machine, the folder and the exact command, with the three answers", () => {
    ...
    expect(screen.getByText("Langy wants to run on rogerio-mbp in acme-app")).toBeDefined();
    expect(screen.getByText("pnpm typecheck")).toBeDefined();
    expect(screen.getByText("Runs the project's own type check before I commit")).toBeDefined();
...
describe("given a card that already settled", () => {
  ...
  it("names the pattern a session grant covered", () => {
    ...
    expect(screen.getByText('You allowed "pnpm *" for the session')).toBeDefined();
  });
  ...
  it("says nobody answered in time", () => {
    ...
    expect(screen.getByText("No answer in time, Langy continued without it")).toBeDefined();
    expect(screen.queryByText("Allow once")).toBeNull();
  });
  ...
  it("names the terminal, and the pattern the grant there covers", () => {
    ...
    expect(
      screen.getByText('Answered in the terminal: allowed "uv" for this session'),
    ).toBeDefined();
    expect(screen.queryByText("Allow once")).toBeNull();
  });
  ...
  it("names the terminal for an allow once and for a denial too", () => {
    ...
    expect(screen.getByText("Answered in the terminal: allowed this command once")).toBeDefined();
    ...
    expect(screen.getByText("Answered in the terminal: denied this command")).toBeDefined();
  });
...
describe("given a long command chain", () => {
  ...
  it("names every pattern the one session grant covers", () => {
    renderCard({
      card: {
        ...PENDING,
        command: "git fetch origin && git checkout -b langy/tracing origin/main",
        pattern: "git fetch",
        patterns: ["git fetch", "git checkout"],
      },
    });

    expect(screen.getByText('Allow "git fetch" and "git checkout" this session')).toBeDefined();
  ...
  it("says what the session grant covered once the card settles", () => {
    ...
    expect(
      screen.getByText('You allowed "git fetch", "git checkout" and "git push" for the session'),
    ).toBeDefined();
...
describe("given a command that runs under a time limit", () => {
  ...
  it("says after how long the command is stopped", () => {
    ...
    expect(screen.getByText("Stops after 5 minutes if it has not finished.")).toBeDefined();
  });
