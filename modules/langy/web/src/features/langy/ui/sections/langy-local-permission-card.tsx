...
function terminalAnswer({
  ...
  if (decision === "allow_pattern") {
    const named = langyPatternList(patterns);
    return named ? `allowed ${named} for this session` : "allowed this pattern for this session";
  }
  return "answered";
}
      ...
      <VStack align="stretch" gap={2}>
        ...
        {card.timeoutSeconds ? (
          <Text textStyle="2xs" color="fg.subtle">
            Stops after {langyTimeLimitText(card.timeoutSeconds)} if it has not finished.
          </Text>
        ) : null}
...
function PendingAnswers({
  ...
  const answer = api.langy.answerLocalPermission.useMutation({
    ...
    onError: (error) => {
      ...
      onFailure(describeError({ error, fallbackTitle: "Could not send your answer" }));
    },
  });
    ...
    return (
      <HStack gap={1.5}>
        <Box color="fg.muted" display="flex">
          {card.decision === "deny" ? <CircleSlash size={12} /> : <Check size={12} />}
        </Box>
        <Text textStyle="2xs" color="fg.muted">
          {langyDecisionLabel({
