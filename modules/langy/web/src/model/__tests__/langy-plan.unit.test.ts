...
describe("langyPlan", () => {
  ...
  describe("given tool calls interleaved with plan updates", () => {
    ...
    it("reads the steps and carries none of the work", () => {
      ...
      expect(plan.itemParts[0]!.map((p) => (p as { toolCallId: string }).toolCallId)).toEqual([
        "search",
      ]);
      expect(plan.itemParts[1]!.map((p) => (p as { toolCallId: string }).toolCallId)).toEqual([
        "read",
      ]);
    });
    ...
    it("does not attribute a preamble call to a later step", () => {
      const message = {
        parts: [tool("bash", "early"), todo([{ content: "Do the thing", status: "in_progress" }])],
      };
      const plan = langyPlan(message)!;
  ...
  describe("given malformed or partial todo input", () => {
    it("removes redundant machine statuses from customer-facing step text", () => {
      ...
      expect(
        cleanPlanContent("Create an evaluator (in_progress: blocked by insufficient permissions)"),
  ...
  describe("given a cancelled step", () => {
    it("keeps it in order but excludes it from the total", () => {
      ...
      const plan = langyPlan(message)!;
      expect(plan.items.map((i) => i.status)).toEqual(["completed", "cancelled", "in_progress"]);
      expect(plan.completedCount).toBe(1);
  ...
  describe("given the manager's typed plan override (the live turn)", () => {
    ...
    it("takes the override's items whatever the parts also ran", () => {
      const message = {
        parts: [todo([{ content: "Do the thing", status: "in_progress" }]), tool("bash", "c1")],
      };
      const plan = langyPlan(message, {
        overrideItems: [{ content: "Do the thing", status: "in_progress" }],
      })!;
      expect(plan.items).toEqual([{ content: "Do the thing", status: "in_progress" }]);
  ...
  describe("given the live snapshot is behind the turn's own plan updates", () => {
    ...
    it("keeps the override when the parts have finished no more than it has", () => {
      ...
      expect(plan.items.map((item) => item.content)).toEqual(["Capped step", "Second capped step"]);
    });
