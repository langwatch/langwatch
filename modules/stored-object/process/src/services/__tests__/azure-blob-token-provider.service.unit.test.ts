...
const { workloadGetToken, managedGetToken, cliGetToken, workloadCtorCalls, managedCtorCalls } =
  vi.hoisted(() => ({
...
/**
 * The identity travels on the credential now rather than through process
 * globals, so every case below varies it by building a credential rather than
 * by writing an environment variable.
 */
function workloadCredentials(
  identity: TokenModeCredentials["identity"] = {},
  rest: Partial<Omit<TokenModeCredentials, "mode" | "identity">> = {},
): TokenModeCredentials {
  return { mode: "workloadIdentity", accountName: "lwacct", identity, ...rest };
}

const workloadIdentityCredentials: TokenModeCredentials = workloadCredentials();
...
describe("getAzureBlobToken", () => {
  ...
  describe("given a storage endpoint in a sovereign cloud with a configured authority host and audience", () => {
    ...
    it("scopes to the public-cloud audience when no audience is configured, not the sovereign endpoint's default", async () => {
      ...
      const credentialsWithOnlyAuthority = workloadCredentials(
        {},
        { authorityHost: "https://login.microsoftonline.us" },
      );
  ...
  describe("given two projects resolve to the same Azure identity and audience", () => {
    ...
    it("reuses the same cached token across separately-constructed driver instances", async () => {
      ...
      const credentialsB = workloadCredentials({ ...shared });
  ...
  describe("given two projects resolve to different Azure identities", () => {
    ...
    it("acquires and uses a separate token for each — one identity's token is never presented to the other", async () => {
      ...
      const tokenA = await getAzureBlobToken(
        workloadCredentials({ tenantId: "tenant-a", clientId: "client-a" }),
      );
      const tokenB = await getAzureBlobToken(
        workloadCredentials({ tenantId: "tenant-b", clientId: "client-b" }),
      );
  ...
  describe("given workloadIdentity mode and the projected token has rotated on disk", () => {
    ...
    it("passes the CURRENT federated-token file path fresh on every exchange rather than caching content itself", async () => {
      const identity = { tenantId: "t", clientId: "c" };
      workloadGetToken.mockResolvedValueOnce(futureToken("token-1", 60 * 1000));

      await getAzureBlobToken(
        workloadCredentials({
          ...identity,
          federatedTokenFile: "/var/run/secrets/tokens/original",
        }),
      );

      // Simulate rotation: kubelet replaces the projected file at the same
      // path, but for a long-running worker the injected path itself could
      // also be re-pointed across a pod restart. Change it and force a refresh
      // (the token above is within the safety margin already). The cache key
      // does not carry the file path, so this is the same entry refreshing.
      workloadGetToken.mockResolvedValueOnce(futureToken("token-2"));

      await getAzureBlobToken(
        workloadCredentials({
          ...identity,
          federatedTokenFile: "/var/run/secrets/tokens/rotated",
        }),
      );
