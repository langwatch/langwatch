...
export function rulesToUI(rules: FeatureFlagRules): UIRule[] {
  // Empty input seeds the dialog with one organization-scoped rule, so
  // operators see the shape they are about to fill in rather than a blank.
...
export function uiToRules(rules: UIRule[]): FeatureFlagRules {
  return rules.map((rule) => {
    ...
    if (rule.scopeKind === "ORGANIZATION") {
      return { match: { ...rest, organizationId: target }, enabled: rule.enabled };
    }
