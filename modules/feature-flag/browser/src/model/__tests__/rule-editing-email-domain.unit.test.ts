 ...
 * types is stored lowercase without the `@`.
 * @see specs/ops/internal-feature-flags.feature
 */
import { describe, expect, it } from "vitest";

import { findUnfillableRule, parseEmailDomains, rulesToUI, uiToRules } from "../rule-editing.ts";

describe("given a stored rule enabling the flag for users at two domains", () => {
  const stored = [{ match: { emailDomain: ["acme.com", "acme.io"] }, enabled: true }];
  ...
  describe("when the operator types domains with an @, capitals and padding", () => {
    it("stores them lowercased and without the @", () => {
      ...
      expect(uiToRules([{ ...rule, target: " @Acme.com,ACME.io , " }])[0]?.match).toEqual({
        emailDomain: ["acme.com", "acme.io"],
      });
    });
  });
  ...
  describe("when the operator types one domain", () => {
    it("stores it as a single string", () => {
      ...
      expect(rulesToUI([{ match: { emailDomain: "acme.com" }, enabled: false }])[0]).toMatchObject({
        scopeKind: "EMAIL_DOMAIN",
        target: "acme.com",
      });
    });
  ...
  describe("when the field is blank or a domain still carries an @", () => {
    it("is reported as unfillable rather than saved", () => {
      ...
      expect(findUnfillableRule([{ ...rule, target: " , " }])).toBeDefined();
      expect(findUnfillableRule([{ ...rule, target: "qa@acme.com" }])).toBeDefined();
      expect(findUnfillableRule([{ ...rule, target: "acme.com" }])).toBeUndefined();
      expect(findUnfillableRule([{ ...rule, target: "@acme.com, acme.io" }])).toBeUndefined();
    });
  });
...
describe("parseEmailDomains", () => {
  it("splits on commas, lowercases, strips a leading @ and drops empties", () => {
    expect(parseEmailDomains("@Acme.com, acme.io,, ")).toEqual(["acme.com", "acme.io"]);
