/**
 * The email domain rule in the one-line targeting summary: an enabling
 * domain rule names its domains, a disabling one is not, and a domain rule
 * scoped to an organization speaks for that organization alone.
...
describe("given a rule enabling the flag for users at acme.com", () => {
  describe("when the summary is rendered", () => {
    ...
    it("names the domain", () => {
      const summary = summarizeTargeting([{ match: { emailDomain: "acme.com" }, enabled: true }]);
...
describe("given a rule enabling the flag for users at several domains", () => {
  describe("when the summary is rendered", () => {
    it("names every domain", () => {
      ...
      expect(targetingLabel(summary)).toBe("Enabled for users at acme.com, acme.io");
    });
  });
});
...
describe("given a domain rule that disables the flag", () => {
  describe("when the summary is rendered", () => {
    it("names no domain, because the rule switches no one on", () => {
      const summary = summarizeTargeting([{ match: { emailDomain: "acme.com" }, enabled: false }]);
