...
const { mockCanPublish, mockRouterPush } = vi.hoisted(() => ({
    ...
    api: {
      ...
      optimization: {
        getPublishedWorkflow: queryStub({
          version: "1.0.0",
          // A dsl the contract's own parser accepts: `parseStudioWorkflow`
          // validates what comes back from the server, so a fixture missing
          // `spec_version` is a fixture the product would never see.
          dsl: {
            spec_version: "1.4",
            name: "Test",
            icon: "🧩",
...
describe("given the studio Publish menu is rendered", () => {
  ...
  describe("when the active plan does NOT allow publishing", () => {
    ...
    it("does not redirect to plan management or fire subscription tracking when Publish is clicked", () => {
      ...
      // The `subscription_hook_click` half of this assertion is gone with
      // `trackEvent`: product analytics is the application's own browser
      // singleton and did not travel with the studio.
    });
