...
function identityRequest({
  ...
  return requestForMetric({
    metric: {
      ...
      description,
      gauge: {
        dataPoints: [{ timeUnixNano: "1700000000000000000", asDouble: value, attributes }],
      },
    },
...
describe("canonical metric identity", () => {
  ...
  describe("when the redaction policy rewrites nested values", () => {
    it("redacts nested AnyValue strings without flattening their types", async () => {
      const redactionService: MetricRedaction = {
        redactMetricAttributes: async (metric: { attributes: Record<string, string> }) => {
  ...
  describe("when the redaction policy is not idempotent", () => {
    it("isolates resource redaction across sibling points", async () => {
      ...
      expect(result.accepted[0]!.dataPoint.seriesId).toBe(result.accepted[1]!.dataPoint.seriesId);
      expect(result.accepted[0]!.dataPoint.resourceAttributesJson).toContain("api-redacted");
