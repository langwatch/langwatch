...
function makeRepository() {
  const calls = {
    ...
    getSeriesTotalsByPointAttribute: vi.fn(async () => []),
...
describe("canonical metric projection stores", () => {
  describe("when appending a single point", () => {
    it("stamps trace retention on raw, catalog, and rollup writes", async () => {
      ...
      await MetricDataPointAppendStore.create(repository, 49).append(dataPoint, context);
      await MetricSeriesCatalogAppendStore.create(repository, 49).append(dataPoint, context);
      await MetricTimeRollupAppendStore.create(repository, 49).append(dataPoint, context);
  ...
  describe("when replaying a chunk", () => {
    ...
    it("hands the whole chunk to the repository in a single call", async () => {
      ...
      await MetricDataPointAppendStore.create(repository, 49).bulkAppend(points, bulkContext);
      await MetricSeriesCatalogAppendStore.create(repository, 49).bulkAppend(points, bulkContext);
      await MetricTimeRollupAppendStore.create(repository, 49).bulkAppend(points, bulkContext);
      ...
      expect(repository.recomputeAffectedRollupsMany).toHaveBeenCalledExactlyOnceWith(bulkWrite);
  ...
  describe("when a chunk is empty", () => {
    it("does not touch the repository", async () => {
      ...
      await MetricDataPointAppendStore.create(repository, 49).bulkAppend(
        [],
