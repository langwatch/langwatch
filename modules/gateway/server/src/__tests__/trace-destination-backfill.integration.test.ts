 ...
 * Real Postgres: backfills the resolution once, covering deleted-project and cross-org edges.
...
/**
 * The migration is immutable shipped history (CLAUDE.md), so this test reads
 * it in place at `@langwatch/prisma-client` rather than owning a copy.
 */
...
async function seedLegacyKeys(tx: Prisma.TransactionClient, keys: LegacyKey[]): Promise<void> {
...
describe.skipIf(!databaseUrl)("the stored trace destination backfill against real PG", () => {
  beforeAll(async () => {
    ...
    for (const [id, teamId, kind, archivedAt, createdAt] of [
      ...
      [GOV_LIVE_ID, TEAM_ID, "internal_governance", null, new Date("2025-06-01T00:00:00Z")],
