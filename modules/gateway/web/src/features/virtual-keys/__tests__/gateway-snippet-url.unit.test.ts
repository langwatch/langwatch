...
describe("resolveSnippetGatewayBaseUrl", () => {
  describe("when this deployment exposes its gateway URL via publicEnv", () => {
    ...
    it("strips a trailing slash before appending /v1", () => {
      expect(resolveSnippetGatewayBaseUrl(undefined, "http://localhost:5563/")).toBe(
        "http://localhost:5563/v1",
      );
    });
  });
  ...
  describe("when publicEnv has not resolved yet", () => {
    it("falls back to the hosted SaaS URL", () => {
      expect(resolveSnippetGatewayBaseUrl(undefined, undefined)).toBe(HOSTED_GATEWAY_URL);
      expect(resolveSnippetGatewayBaseUrl(undefined, null)).toBe(HOSTED_GATEWAY_URL);
    });
  });
});
