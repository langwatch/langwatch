...
function renderDrawer(props: Partial<Parameters<typeof WebhookEndpointDrawer>[0]> = {}) {
  const onSave = vi.fn();
  // The drawer copies the generated external id and says so through the host's
  // toaster, so it needs one mounted above it even in the tests that never
  // reach for the clipboard.
...
describe("WebhookSecretDialog", () => {
  ...
  it("shows the secret with the shown-once warning while open", () => {
    renderWithGatewayHost(<WebhookSecretDialog secret="whsec_test123" onClose={vi.fn()} />, {
      host: fakeGatewayHost(),
    });
