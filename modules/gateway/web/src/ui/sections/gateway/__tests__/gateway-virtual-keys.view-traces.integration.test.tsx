...
/** One organization, one team, one project, read by an admin. */
  ...
  describe("given a key whose trace destination is live and reachable", () => {
    describe("when View traces is chosen from the row actions", () => {
      ...
      it("navigates to the destination's trace explorer filtered to the key", async () => {
        ...
        // The screen asks the host to navigate; the host records where.
        expect(host.recording.navigations).toHaveLength(1);
        const href = String(host.recording.navigations[0]);
