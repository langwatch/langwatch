    ...
    values: [
      {
        ...
        AnnotationIds: [],
        LastEventOccurredAt: args.occurredAt,
        TopicId: null,
        SubTopicId: null,
      },
    ],
...
describe.skipIf(!databaseUrl || !chUrl)("virtual key spend (real PG + real CH)", () => {
  beforeAll(async () => {
    ch = createTestClickHouseClient(chUrl!);

    await prisma.organization.create({
      data: { id: ORG_ID, name: `Spend Org ${suffix}`, slug: `spend-${suffix}` },
    });
  ...
  afterAll(async () => {
    ...
    // shared server should not accrue a suite's worth of rows per run.
    ...
    await prisma.virtualKey.deleteMany({
      where: {
        id: { in: [VK_UNBUDGETED_ID, VK_BUDGETED_ID, VK_ORG_SCOPED_ID, VK_TWO_MODELS_ID] },
      },
    });
    await prisma.project.deleteMany({ where: { id: { in: [PROJECT_ID, GOV_PROJECT_ID] } } });
  ...
  it("includes a request made moments ago in a rolling window", async () => {
    ...
    expect(summary.byVirtualKey.map((v) => v.virtualKeyId).toSorted()).toEqual(
      [VK_BUDGETED_ID, VK_UNBUDGETED_ID].toSorted(),
    );
  });
  ...
  describe("when one model is picked from the spend breakdown", () => {
    ...
    it("lists only that model's requests, and leaves every other slice whole", async () => {
      ...
      expect(unfiltered.recentDebits.map((d) => d.model).toSorted()).toEqual([
        "claude-sonnet-4",
        "gpt-5-mini",
      ]);
  ...
  it("includes the org-scoped key in the unfiltered org summary", async () => {
    ...
    expect(summary.byVirtualKey.map((v) => v.virtualKeyId).toSorted()).toEqual(
      [VK_BUDGETED_ID, VK_ORG_SCOPED_ID, VK_UNBUDGETED_ID].toSorted(),
    );
  });
});
