 ...
 * Real Postgres: PRINCIPAL wins the strictest cascade, incl. BLOCK when it alone is over-limit.
...
describe.skipIf(!databaseUrl)("GatewayService — PRINCIPAL cascade", () => {
  ...
  beforeAll(async () => {
    ...
    // The four collaborators below the budget repository — cache rules and the
    // guardrail catalogue — are constructed but never entered by a budget
