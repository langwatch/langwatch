...
async function loadVk(): Promise<VirtualKeyWithScopes> {
  const virtualKey = await PrismaGatewayVirtualKeyRepository.create(prisma).findByIdGlobal(VK_ID);
  if (!virtualKey) throw new Error(`virtual key ${VK_ID} was not created`);
