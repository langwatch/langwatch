...
describe("getGetPromptSnippets()", () => {
  describe("when no label is provided", () => {
    ...
    it("uses bare handle in the Go and curl snippets", () => {
      ...
      expect(go(snippets)).toContain('Prompts.Get(context.Background(), "support-triage", nil)');
  ...
  describe("when label is provided", () => {
    ...
    it("uses shorthand syntax in SDK snippets", () => {
      ...
      expect(python(snippets)).toContain('prompts.get("support-triage:production")');
      expect(typescript(snippets)).toContain("prompts.get('support-triage:production')");
    });
  ...
  describe("when the prompt declares variables", () => {
    ...
    it("derives a readable example from the variable's name", () => {
      ...
      expect(python(snippets)).toContain('question="How do I reset my password?"');
    });
  });
  ...
  describe("when the prompt declares no variables", () => {
    ...
    it("compiles with an empty argument list", () => {
      ...
      expect(typescript(snippets)).toContain("const compiled = prompt.compile();");
    });
  });
