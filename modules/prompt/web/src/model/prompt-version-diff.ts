/**
 * What changed between two versions of a prompt — the version history
 * lists newest-first, so a predecessor is simply the next list entry.
 * Deliberately free of React and server types, for direct reuse elsewhere.
 */
...
const formatFields = (fields: { identifier: string; type: string }[] | null | undefined) =>
  (fields ?? []).map((field) => `${field.identifier} (${field.type})`).join(", ");
...
function pairUpRun({
  ...
  const length = Math.max(previousEnd - previousStart, currentEnd - currentStart);
...
function compareMessages({
  ...
  alignMessages({ before, after }).forEach(({ previousIndex, currentIndex }, index) => {
    const previousLabel =
      (previousIndex === undefined ? undefined : beforeLabels[previousIndex]) ?? "Message";

    changes.push(
      ...compareMessagePair({
        index,
        label:
          (currentIndex === undefined ? undefined : afterLabels[currentIndex]) ?? previousLabel,
        previousLabel,
        previous: previousIndex === undefined ? undefined : before[previousIndex],
