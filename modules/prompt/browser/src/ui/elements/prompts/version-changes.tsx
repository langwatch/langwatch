...
import type { PromptVersionChange } from "../../../model/prompt-version-diff.ts";
  ...
  return (
    ...
    >
      ...
      {parts.map((part, index) => (
        <Box key={`${index}-${part.value}`} as="span" borderRadius="2px" {...partTone(part)}>
          {part.value}
        </Box>
      ))}
    </Box>
...
function ValueChange({ before, after }: { before: string; after: string }) {
  return (
    <HStack gap={2} fontSize="12px" flexWrap="wrap">
      ...
      {after && (
        <Text color="green.fg" background="green.subtle" borderRadius="2px" paddingX={1}>
          {after}
        </Text>
      )}
    </HStack>
