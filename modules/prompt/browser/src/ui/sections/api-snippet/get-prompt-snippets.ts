...
const STRING_EXAMPLES: [RegExp, string][] = [
  ...
  [/question|query|request|input|message|prompt|text|content/, "How do I reset my password?"],
  [/id$|^id|_id_/, "abc123"],
];
...
function pythonCompileCall(variables: PromptSnippetVariable[]): string {
  ...
  const args = variables
    .map((variable) => `    ${variable.identifier}=${toPythonLiteral(exampleValueFor(variable))},`)
    .join("\n");
  return `compiled = prompt.compile(\n${args}\n)`;
}
...
function typescriptCompileCall(variables: PromptSnippetVariable[]): string {
  ...
  const args = variables
    .map(
      (variable) => `  ${variable.identifier}: ${toTypeScriptLiteral(exampleValueFor(variable))},`,
    )
    .join("\n");
  return `const compiled = prompt.compile({\n${args}\n});`;
}
