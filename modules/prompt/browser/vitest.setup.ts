...
/**
 * The same shape other `*-web` packages state for themselves. `@testing-library/jest-dom`
 * registers the matchers suites moved from `platform/app` were written against, so assertions
 * travel unchanged rather than being rewritten into `.textContent` comparisons.
 */

/**
 * The rest are browser APIs jsdom doesn't ship that Chakra's overlays reach for. A missing
 * `ResizeObserver` surfaces as an unhandled rejection from an animation frame rather than a
 * failure, so a shard can fail with its own summary reading all green.
 */
