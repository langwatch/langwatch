...
import {
  ...
  type ChatMessage as StudioChatMessage,
  LATEST_SPEC_VERSION,
...
/**
 * Narrows a turn's role to the three the engine's chat-message parameter names.
 * Anything else is a role the model has no slot for, so it reads as the user's.
 */
function asStudioChatMessage(turn: PromptChatTurn): StudioChatMessage {
  const role = turn.role === "system" || turn.role === "assistant" ? turn.role : ("user" as const);
...
}): { messagesHistory: PromptChatTurn[]; inputs: Record<string, unknown> } {
  const formMsgs = (formValues.version.configData.messages ?? []).filter(
    (m) => m.role !== "system",
  );

  const lastLiveUserMsg = [...messages].reverse().find((m) => m.role === "user");
...
function buildNodeData({
  ...
  formValues: PromptConfigFormValues;
  messagesHistory: PromptChatTurn[];
  ...
  return {
    ...
    parameters: [
      ...
      { identifier: "instructions", type: "str", value: systemPrompt },
      {
        identifier: "messages",
        type: "chat_messages",
        value: messagesHistory.filter((m) => m.role !== "system").map(asStudioChatMessage),
      },
      {
