...
import { changeTargetsBrokeredPassword } from "../password-change-target.rules.ts";
  ...
  describe("given a deployment that brokers nothing", () => {
    it("never goes to the broker, whatever the person holds", () => {
      for (const provider of ["email", "google", "okta", "azure-ad"]) {
        expect(changeTargetsBrokeredPassword({ provider, holdsOwnPassword: false })).toBe(false);
        expect(changeTargetsBrokeredPassword({ provider, holdsOwnPassword: true })).toBe(false);
      }
    });
  });
