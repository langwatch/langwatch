...
interface Props {
  displayName: string;
  config: AiToolConfigOf<"coding_assistant">;
...
export function CodingAssistantTile({ displayName, config, iconAsset, iconKey }: Props) {
  ...
  return (
    <Box borderWidth="1px" borderColor="border.muted" borderRadius="md" padding={4} width="full">
          ...
          */}
          {!isSaas && (
            <>
              ...
              <CommandRow command={`langwatch login --endpoint ${appBaseUrl}`} />
            </>
          )}
      ...
      <Dialog.Root open={setupOpen} onOpenChange={(d) => setSetupOpen(d.open)} size="md" modal>
        <Dialog.Content>
          <Dialog.Header>
