...
export function CodingAgentUsageContent({ projectId }: { projectId: string }) {
  ...
  const produced = [
    totals.linesAdded > 0 || totals.linesRemoved > 0
      ? `${numeral(totals.linesAdded).format("0,0")} added / ${numeral(totals.linesRemoved).format(
          "0,0",
        )} removed`
      : null,
    totals.commits > 0
  ...
  return (
    <VStack align="stretch" gap={3}>
      <SimpleGrid columns={{ base: 2, md: 4 }} gap={3}>
        ...
        <Stat label="Tokens" value={numeral(totals.totalTokens).format("0,0")} />
        <Stat label="Active time" value={formatDurationSeconds(totals.activeTimeSec)} />
      </SimpleGrid>
      {produced.length > 0 && (
