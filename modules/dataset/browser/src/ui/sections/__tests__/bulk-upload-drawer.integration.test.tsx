...
// Each row polls dataset.getById while its file is preparing — resolve ready.
...
const render_ = (onUploaded = vi.fn()) =>
  renderWithDatasetHost(<BulkUploadDrawer open onClose={vi.fn()} onUploaded={onUploaded} />, {
...
describe("given the bulk upload drawer", () => {
  ...
  describe("when files are uploaded", () => {
    ...
    it("blocks upload while two columns share a name, and re-enables once fixed", async () => {
      ...
      expect(screen.getAllByText(/column names must be unique/i).length).toBeGreaterThan(0);
