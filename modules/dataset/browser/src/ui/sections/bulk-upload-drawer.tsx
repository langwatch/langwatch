...
/** Inline column list for one file: rename, retype, drag-reorder, exclude
 *  (down to the last column; adding is impossible, no data to back it).
 *  Each column's immutable `sourceHeader` lets the normalize job bind by
 *  header, not position, dropping headers absent here (`DatasetConfirmColumns`). */
function BulkColumnFields({
  ...
  const setBySource = (sourceHeader: string, patch: Partial<DatasetConfirmColumns[number]>) =>
    onChange(columnTypes.map((c) => (c.sourceHeader === sourceHeader ? { ...c, ...patch } : c)));
            ...
            <SortableColumnRow
              ...
              onRemove={
                columnTypes.length > 1
                  ? () => onChange(columnTypes.filter((c) => c.sourceHeader !== col.sourceHeader))
                  : undefined
              }
            />
          ))}
...
function ColumnDragOverlayRow({ col }: { col: DatasetConfirmColumns[number] }) {
  const typeLabel = COLUMN_TYPE_OPTIONS.find((o) => o.value === col.type)?.label ?? col.type;
  ...
  return (
    ...
    >
      ...
      <Input size="sm" value={col.name} readOnly tabIndex={-1} pointerEvents="none" bg="bg" />
      <HStack
        gap={2}
        width="44"
        height="8"
    ...
    >
      <HStack gap={3} width="full" align="center">
        <VStack align="start" gap={0} flex={1} minW={0}>
          <EditableName value={file.name} editable={file.status === "pending"} onCommit={onName} />
      ...
      {/* The confirm columns mount only while the row is open, so a collapsed
          row holds no focusable inputs (a11y) and stays out of the DOM. The
          platform drawer animated the transition through `motion`; this package
          does not carry an animation library, so the section appears and
          disappears outright. */}
