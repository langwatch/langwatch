...
} from "../dataset-attachment.rules.ts";
...
describe("dataset attachment upload policy", () => {
  ...
  describe("given a media type a browser can run", () => {
    describe("when the media type is checked", () => {
      it.each([
        "text/html",
    ...
    describe("when it arrives with parameters and in another case", () => {
      it("refuses it all the same", () => {
        expect(isRefusedAttachmentMediaType("TEXT/HTML; charset=utf-8")).toBe(true);
      });
    });
  });
  ...
  describe("given a media type the product accepts", () => {
    describe("when the media type is checked", () => {
      it.each([
        ...
        expect(() => assertDatasetAttachmentMediaTypeAllowed(mediaType)).not.toThrow();
      });
    });
  });

  describe("given a part that declares no media type", () => {
    describe("when the media type is normalized", () => {
      it("stores it as unnamed bytes", () => {
        expect(normalizeAttachmentMediaType("")).toBe("application/octet-stream");
        expect(normalizeAttachmentMediaType(undefined)).toBe("application/octet-stream");
      });
    });
  });

  describe("given a media type with the parameters a browser appends", () => {
    describe("when the media type is normalized", () => {
      it("drops the parameters", () => {
        expect(normalizeAttachmentMediaType("text/csv; charset=utf-8")).toBe("text/csv");
      });
    });
  });
});
