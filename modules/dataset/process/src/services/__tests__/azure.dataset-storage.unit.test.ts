 ...
 * `isMissingObjectError`), mirroring `@langwatch/stored-object-process`'s `ObjectNotFoundError`
...
class FixedAzureConfigResolver implements DatasetAzureConfigResolver {
  ...
  describe("when writing then reading chunks", () => {
    describe("given a dataset written to Azure Blob", () => {
      ...
      it("reads the same rows back in order", async () => {
        class InMemoryAzureConfigResolver implements DatasetAzureConfigResolver {
