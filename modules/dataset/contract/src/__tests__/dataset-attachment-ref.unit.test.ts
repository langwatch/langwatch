...
} from "../dataset-attachment-ref.ts";

describe("dataset attachment references", () => {
  describe("given a reference to a file stored in LangWatch", () => {
    it("recognises it", () => {
      expect(isDatasetAttachmentRef("/api/files/project-1/so_abc/report.pdf")).toBe(true);
    ...
    it("reads the project, the object and the file name out of it", () => {
      expect(parseDatasetAttachmentRef("/api/files/project-1/so_abc/report.pdf")).toEqual({
        projectId: "project-1",
        objectId: "so_abc",
        name: "report.pdf",
      });
    ...
    it("ignores a query string and a fragment", () => {
      expect(parseDatasetAttachmentRef("/api/files/p/so_abc/report.pdf?v=2#top")).toEqual({
        projectId: "p",
        objectId: "so_abc",
        name: "report.pdf",
      });
  ...
  describe("given a value that is not a stored reference", () => {
    it("refuses a relative step", () => {
      expect(isDatasetAttachmentRef("/api/files/p/../../etc/passwd")).toBe(false);
      expect(parseDatasetAttachmentRef("/api/files/p/../../etc/passwd")).toBeNull();
    });
  ...
  describe("given a cell value to label", () => {
    ...
    it("shows the file name of a stored file", () => {
      expect(attachmentDisplayName("/api/files/p/so_abc/quarter-report.pdf")).toBe(
        "quarter-report.pdf",
      );
    });

    it("decodes an escaped file name", () => {
      expect(attachmentDisplayName("/api/files/p/so_abc/quarter%20report.pdf")).toBe(
        "quarter report.pdf",
      );
    });

    it("shows the last part of an address on another site", () => {
      expect(attachmentDisplayName("https://example.com/files/a.png")).toBe("a.png");
    });
  ...
  describe("given a confirmed stored file", () => {
    describe("when the value a cell holds is built for it", () => {
      it("round-trips through the reference parser, name included", () => {
        const value = datasetAttachmentRefOf({
          projectId: "project-1",
          id: "so_abc",
          filename: "quarter one.pdf",
        });

        expect(value).toBe("/api/files/project-1/so_abc/quarter%20one.pdf");
        expect(parseDatasetAttachmentRef(value)).toEqual({
          projectId: "project-1",
          objectId: "so_abc",
          name: "quarter one.pdf",
        });
      });
    });
  });
