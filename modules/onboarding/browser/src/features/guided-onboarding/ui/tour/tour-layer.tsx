/**
 * The guided tour layer: a collaborator cursor and spotlight with a caption
 * per stop, advancing itself until it hands the screen to the Langy panel.
...
import {
  ...
  type TourHandoff,
  useGuidedTourStore,
} from "../../behavior/guided-tour-store.ts";
import { getTourActions } from "../../behavior/tour-registry.ts";
import {
  PANEL_SELECTOR,
  type Point,
  type Rect,
...
} from "../../model/tour-steps.ts";
import { TourCaption, TourCursor, TourSpotlight } from "./tour-overlay.tsx";
...
function cursorPoint(rect: DOMRect): Point {
  ...
  return {
    x: big ? rect.left + Math.min(rect.width * 0.5, 260) : rect.left + rect.width * 0.72,
    y: big ? rect.top + Math.min(rect.height * 0.4, 220) : rect.top + rect.height * 0.62,
  };
}

function autoPlacement(rect: DOMRect): "bottom" | "right" {
  return rect.width >= rect.height ? "bottom" : "right";
}

function captionPoint(rect: DOMRect, step: TourStep): Point {
  const placement = step.placement === "auto" ? autoPlacement(rect) : step.placement;
  let x = rect.right + 18;
...
/** Where a step's `before` and `onArrive` send the page: the declared host's own navigate. */
function useTourNavigate(): (to: string) => void {
  const host = useOnboardingHost();
  return useCallback((to: string) => host.navigate(to), [host]);
}

/**
 * The step timers. Every wait a step schedules (travel, caption, auto-advance,
 * the target poll) goes through `later`, so `clear` cancels the whole step at
 * once and no timer outlives the component.
 */
function useTourTimers(): { later: (fn: () => void, ms: number) => void; clear: () => void } {
 ...
 * and holds, then fades. Kept apart from the step timers: it outlives the
 * run that started it, so ending a step must not cancel it.
 */
function useTourHandoff({
  ...
  const run = useCallback(() => {
    ...
    setHandoff("lit");
    timers.current.push(window.setTimeout(() => setHandoff("fading"), TOUR_HANDOFF_HOLD_MS));
    timers.current.push(
      window.setTimeout(() => {
        setHandoff(null);
        setSpot(null);
 ...
 * the caption, and the flags the render reads to animate them. One hook
 * rather than seven pieces of state because the three always move together.
 */
...
/**
 * What a poll that has not found its target yet should do next. While an
 * earlier step's action is still answering the wait does not count down;
 * the action has its own ceiling so a promise that never settles cannot park it.
 */
...
/**
 * One step in flight: what it shows, what it waits for, and how it can move
 * the tour on. The phases below take it rather than close over scope.
 */
interface StepRun {
  step: TourStep;
  path: GuidedPath;
...
function arriveStep(run: StepRun, travelled: DOMRect): void {
  ...
  const arriveResult = run.step.onArrive?.({ ...run.ctx, actions: getTourActions() });
...
function landStep(run: StepRun, rect: DOMRect): void {
  run.emit("viewed", "tour_step", { path: run.path, step: run.stepIndex, target: run.step.target });
...
/**
 * The target is not there yet: the spotlight closes onto the cursor rather
 * than leave a lone hole, and reopens once the target is there. The step
 * gives up once its wait, or the page action's own ceiling, is spent.
 */
 ...
 * advance. Owns the page action a step is still waiting on, and how many
 * times the cursor has been placed (the first placement is instant).
 */
 ...
 * ends, the path's end action runs (`TOUR_END_ACTIONS`).
 */
function useTourRun({
  running,
  runId,
  ...
  steps: readonly TourStep[];
  goToStep: (index: number) => void;
}): { endTour: (status: TourEndStatus) => void; next: () => void; back: () => void } {
  ...
  const endTour = useCallback(
    (status: TourEndStatus) => {
      ...
      if (path && status === "completed") {
        emit("completed", "tour", {
          path,
          durationMs: nowInstant().epochMilliseconds - startedAt.current,
        });
      } else if (path) {
  ...
  useEffect(() => {
    ...
    paint.clearPointer();
    resetRun();
    startedAt.current = nowInstant().epochMilliseconds;
...
/**
 * The tour's behaviour in one hook: the store, what is painted, the step
 * engine and a run's lifecycle. `TourLayerInner` is markup only.
 */
function useTour() {
  ...
  return { ...paint, active, handoff, step, steps, stepIndex, cursorMoves, endTour, next, back };
}
...
function TourLayerInner() {
  ...
  return (
    <>
      {(active || handoff) && spot && <TourSpotlight spot={spot} spotZ={spotZ} handoff={handoff} />}
      {active && cursor && (
        <TourCursor
          cursor={cursor}
