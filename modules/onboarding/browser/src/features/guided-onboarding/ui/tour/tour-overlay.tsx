...
import { readMs, type TourStep } from "../../model/tour-steps.ts";

/** The circle-timer on the Next button. */
function TimerRing({ duration, playKey }: { duration: number; playKey: number }) {
  const r = 5.5;
  const c = 2 * Math.PI * r;
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        style={{ transition: armed ? `stroke-dashoffset ${duration}ms linear` : "none" }}
      />
    </svg>
  );
}
  ...
  return (
    <Box
      ...
      style={{
        ...
        transition: moves > 1 ? `transform ${TOUR_TRAVEL_MS}ms ${EASING}` : "none",
      }}
    >
      <svg
        width="20"
    ...
    >
      ...
      <HStack marginTop={3} justify="space-between">
        <chakra.button
          ...
          _hover={stepIndex > 0 ? { background: "bg.muted", color: "fg.muted" } : undefined}
        >
