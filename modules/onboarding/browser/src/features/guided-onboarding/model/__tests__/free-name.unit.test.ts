...
import { freeName } from "../free-name.ts";
...
describe("freeName", () => {
  describe("given no key carries the wanted name", () => {
    ...
    it("keeps the wanted name", () => {
      expect(freeName({ wanted: "production-app", taken: [] })).toBe("production-app");
      expect(freeName({ wanted: "production-app", taken: ["staging-app"] })).toBe("production-app");
    });
  });
  ...
  describe("given the wanted name is taken", () => {
    ...
    it("counts up from 2 past every taken suffix", () => {
      expect(freeName({ wanted: "production-app", taken: ["production-app"] })).toBe(
        "production-app-2",
      );
      expect(
        freeName({
      ...
      expect(
        freeName({ wanted: "production-app", taken: ["production-app", "production-app-3"] }),
