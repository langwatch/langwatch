...
import { isGuidedPathActive } from "../guided-path-active.ts";
...
describe("isGuidedPathActive", () => {
  describe("given the tour is running", () => {
    describe("when the state names no path yet", () => {
      ...
      it("is active", () => {
        expect(isGuidedPathActive({ state: { donePaths: [] }, tourRunning: true })).toBe(true);
      });
    });
  });
  ...
  describe("given the organization is not in the guided variant", () => {
    describe("when the state is absent", () => {
      it("is not active", () => {
        expect(isGuidedPathActive({ state: null, tourRunning: false })).toBe(false);
      });
    });
  });
});
