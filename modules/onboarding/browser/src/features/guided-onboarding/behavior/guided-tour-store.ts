...
interface GuidedTourState {
  ...
  start: (path: GuidedPath, options?: { onEnd?: (status: TourEndStatus) => void }) => void;
  /**
   * Runs a tour again from step 1, queueing nothing at the end: `path`'s, or
   * the last one run when none is named. The card names the path, so a
