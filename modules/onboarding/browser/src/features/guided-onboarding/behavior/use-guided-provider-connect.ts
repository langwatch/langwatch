 ...
 * shared credential form has saved a row, or records a skip. Pointing
 * Langy's own role at the model is not wired — see the handoff §10.
 */
...
export function useGuidedProviderConnect({
  ...
  const onSaved = useCallback(
    ...
    [refetch, recordProvider, organizationId, onConnected],
  );
