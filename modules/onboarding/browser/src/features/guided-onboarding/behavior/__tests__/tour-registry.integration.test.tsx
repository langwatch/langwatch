...
import { getTourActions, useRegisterTourActions, useTourRegistry } from "../tour-registry.ts";
...
describe("tour registry", () => {
  ...
  describe("when a page registers an action", () => {
    ...
    it("keeps another page's actions when one unmounts", () => {
      ...
      const unregisterGroups = useTourRegistry.getState().register({ expandGroup });
      const unregisterKeys = useTourRegistry.getState().register({ openVirtualKeyCreate: vi.fn() });
      unregisterKeys();
