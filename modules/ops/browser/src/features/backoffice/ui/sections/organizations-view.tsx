 ...
 * the license key are write-only, so an empty input means leave it alone. */
...
function changedIdentityFields({ form, organization }: FormDiff): Record<string, unknown> {
...
function changedBillingFields({ form, organization }: FormDiff): Record<string, unknown> {
  const data: Record<string, unknown> = {};
  const nextLimit = numOrNull(form.usageSpendingMaxLimit);
  if (nextLimit !== organization.usageSpendingMaxLimit) data.usageSpendingMaxLimit = nextLimit;
  ...
  if (form.pricingModel !== organization.pricingModel) data.pricingModel = form.pricingModel;
  return data;
}
 ...
 * echoes the stored value. An empty input means "leave the secret alone". */
function changedStorageFields({ form, organization }: FormDiff): Record<string, unknown> {
  const data: Record<string, unknown> = {};
  if (form.useCustomS3 !== !!organization.useCustomS3) data.useCustomS3 = form.useCustomS3;
...
type SetOrganizationField = <K extends keyof FormState>(key: K, value: FormState[K]) => void;
  ...
  return (
    <>
      ...
      <Field.Root>
        ...
        <Input value={form.promoCode} onChange={(e) => setField("promoCode", e.target.value)} />
      </Field.Root>
      <Field.Root>
...
function CustomS3Fields({ form, setField }: SectionProps) {
  return (
    <>
      ...
      <Text fontSize="xs" color="fg.muted">
        Credentials below are write-only. The server never reads them back. Leave blank to keep the
        stored value; type to replace.
      </Text>
      <ToggleRow
        label="Use custom S3"
