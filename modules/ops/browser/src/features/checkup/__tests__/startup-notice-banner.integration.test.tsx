...
vi.mock("../behavior/checkup-api.ts", () => ({
  checkupApi: {
    checkup: {
      dismissStartupNotice: { useMutation: () => ({ mutate, isPending: false }) },
    },
  },
}));

import { StartupNoticeBanner } from "../ui/sections/startup-notice-banner.tsx";
...
describe("StartupNoticeBanner", () => {
  describe("when an administrator sees the notice", () => {
    ...
    it("links the telemetry docs and the checkup page, and records the dismissal", () => {
      ...
      expect(screen.getByRole("link", { name: /open checkup/i })).toHaveAttribute(
        "href",
        "/settings/checkup",
      );
      ...
      expect(mutate).toHaveBeenCalledWith({ organizationId: "org_1", schemaVersion: 2 });
