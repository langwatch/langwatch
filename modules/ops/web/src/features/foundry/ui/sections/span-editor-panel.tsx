    ...
    <VStack align="stretch" gap={4} p={5} minW={0} overflow="hidden">
      ...
      <Flex gap={3} wrap="wrap">
        <Box flex="1" minW="140px">
          ...
          <Input
            size="sm"
            value={span.name}
            onChange={(e) => updateSpan(span.id, { name: e.target.value })}
          />
        </Box>
        <Box flex="1" minW="140px">
          <Text fontSize="xs" color="fg.subtle" mb={1}>
      ...
      <Flex gap={3} wrap="wrap">
        ...
        <Box flex="1" minW="100px">
          ...
          <Select.Root
            ...
            onValueChange={(e) =>
              updateSpan(span.id, {
                status: e.value[0] as "ok" | "error" | "unset",
              })
            }
          >
            <Select.Trigger>
      ...
      {span.status === "error" && (
        <Box rounded="lg" border="1px solid" borderColor="red.500/30" bg="red.500/5" p={3}>
          ...
          <Input
            size="sm"
            value={span.exception?.message ?? ""}
            onChange={(e) =>
              updateSpan(span.id, {
                exception: { ...span.exception, message: e.target.value },
              })
            }
          ...
          <Textarea
            size="sm"
            value={span.exception?.stackTrace ?? ""}
            onChange={(e) =>
              updateSpan(span.id, {
                exception: {
                  message: span.exception?.message ?? "",
                  stackTrace: e.target.value || undefined,
                },
              })
            }
      ...
      <Box>
        ...
        <AttributeEditor
          ...
          onChange={(attributes) => updateSpan(span.id, { attributes })}
        />
      </Box>
    </VStack>
  );
