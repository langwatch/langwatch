...
export default function BugReportsView() {
  ...
  const openReportId = typeof router.query.report === "string" ? router.query.report : null;
    ...
    <Table.Root size="sm" variant="line">
      ...
      <Table.Body>
        ...
        {reports?.map((report) => (
          ...
          >
            <Table.Cell whiteSpace="nowrap">{formatDateTime(report.createdAt)}</Table.Cell>
          ...
          {report.data && (
            <VStack align="stretch" gap={6}>
              <SimpleGrid columns={2} gap={3}>
                ...
                <Fact label="Kind">{kindLabel[report.data.kind] ?? report.data.kind}</Fact>
