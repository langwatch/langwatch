...
function createOptions(mutatingPiiRedact: (span: OtlpSpan) => void): RecordSpanCommandOptions {
  ...
  const costEnrichment: TraceSpanCostEnrichment = { enrich: async () => undefined };
  const tokenEstimation: TraceSpanTokenEstimation = { estimate: async () => undefined };
  const contentDrop: TraceSpanContentDrop = {
