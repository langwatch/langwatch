...
class CostEnrichmentFake implements TraceSpanCostEnrichment {
  readonly enrich = vi.fn(async (_span: OtlpSpan, _tenantId: string) => {});
}

class TokenEstimationFake implements TraceSpanTokenEstimation {
  readonly estimate = vi.fn(async (_span: OtlpSpan, _tenantId: string) => {});
}

class ContentDropFake implements TraceSpanContentDrop {
