 ...
 * the resource attributes and the instrumentation scope, so widen it here.
 */
function originSpan(
  overrides: Parameters<typeof createTestSpan>[0] & {
    resourceAttributes?: Record<string, unknown>;
    instrumentationScope?: { name: string; version: string | null };
  },
): NormalizedSpan {
  const { resourceAttributes, instrumentationScope, ...rest } = overrides;
  return {
    ...createTestSpan(rest),
...
describe("applySpanToSummary() langwatch.origin hoisting", () => {
  ...
  describe("when an evaluator workflow emits child spans on a customer's trace", () => {
    ...
    it("preserves the customer trace's origin once the root span has resolved it", () => {
      ...
      const evalChildSpan = originSpan({
        id: "eval-child-1",
        spanId: "eval-child-1",
        parentSpanId: "root-1",
        spanAttributes: {
