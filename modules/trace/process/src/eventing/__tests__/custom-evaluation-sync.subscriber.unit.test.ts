...
describe("customEvaluationSync subscriber", () => {
  ...
  beforeEach(() => {
    ...
    deps = {
      ...
      // Stands in for Evaluation's own slug rule, which this subscriber now
      // takes as an injected dependency rather than importing.
      deriveEvaluatorId: vi.fn(
        (name: string) => `customeval_${name.toLowerCase().replaceAll(" ", "_")}`,
      ),
    };
  });
  ...
  describe("when span has evaluation events", () => {
    ...
    it("derives the evaluator ID from the injected slug rule, verbatim", async () => {
      ...
      // The rule is handed the evaluation's NAME and its answer is used
      // unchanged: a subscriber that reformatted it would re-key every custom
      // evaluator the SDK reports.
      expect(deps.deriveEvaluatorId).toHaveBeenCalledWith("My Custom Eval");
      const call = vi.mocked(deps.reportEvaluation).mock.calls[0]![0];
      expect(call.evaluatorId).toBe("customeval_my_custom_eval");
    });
