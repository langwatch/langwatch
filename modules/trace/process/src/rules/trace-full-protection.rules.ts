...
function protectEvent(
  ...
  return {
    ...event,
    event_details: redactStringRecord(event.event_details, redactions),
  };
}
 ...
 */
export function applyTraceFullReadProtections(
  trace: TraceFullRecord,
  protections: TraceFullReadProtections,
): TraceFullRecord {
  const redactions = collectRedactions(trace, protections);
