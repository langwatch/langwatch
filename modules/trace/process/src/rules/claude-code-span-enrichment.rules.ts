...
import {
  ...
  type ClaudeSpanRef,
} from "./claude-code-message-index.rules.ts";
import {
  buildToolInput,
  buildToolOutput,
...
function indexToolLogsByUseId(toolLogs: ClaudeToolLog[]): Readonly<{
 ...
 */
function findReplyCandidates(
  log: ClaudeContentLog,
  traceCanonicalisation: TraceCanonicalisationService,
): Readonly<{ rank: number; text: string }>[] {
...
export function computeClaudeInteractionOutput({
  ...
  for (const log of logs) {
    ...
    if (!isInWindow(log, window)) {
      continue;
    }

    for (const candidate of findReplyCandidates(log, traceCanonicalisation)) {
