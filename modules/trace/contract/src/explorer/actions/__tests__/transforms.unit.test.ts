 ...
 * closed list. Spec: specs/traces-v2/explorer-actions.feature
 */
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

import { setFilter, toggleFacet } from "../transforms/query.ts";
import { expandRow, select } from "../transforms/rows.ts";
...
import { setGrouping, setLens, setPage, setPageSize, setSort } from "../transforms/view.ts";
import { explorerState, NOW } from "./explorer-fixtures.ts";

/** What the flat trace table can sort by — the page supplies this, not the contract. */
const FLAT_TABLE = { sortableColumnIds: ["time", "duration", "cost", "tokens"] };
...
describe("setFilter", () => {
  ...
  describe("when given a query that does not parse", () => {
    ...
    it("refuses with filter_invalid and leaves the given state as it was", () => {
      ...
      expect(refusalOf(() => setFilter({ state, payload: { query: "status:(" } }))).toBe(
        "filter_invalid",
      );
...
describe("toggleFacet", () => {
  describe("when the same value is clicked three times", () => {
    ...
    it("steps through included, excluded and neutral", () => {
      ...
      expect([first.state.queryText, second.state.queryText, third.state.queryText]).toEqual([
        "status:error",
        "NOT status:error",
        "",
      ]);
  ...
  describe("when a second value of the same field is clicked", () => {
    ...
    it("joins the two values with OR", () => {
      ...
      expect(next.state.queryText).toBe("(origin:application OR origin:evaluation)");
    });
  });
...
describe("setPageSize", () => {
  describe("when given a size the table does not offer", () => {
    ...
    it("refuses with page_size_invalid", () => {
      expect(
        refusalOf(() => setPageSize({ state: explorerState(), payload: { pageSize: 37 } })),
...
describe("select", () => {
  describe("when given two trace ids and a blank one", () => {
    ...
    it("holds the two trace ids", () => {
      ...
      expect(Array.from(next.state.selection.traceIds)).toEqual(["trace-a", "trace-b"]);
