...
/**
 * The query as the page would hold it: parsed, checked against the language's
 * own limits, and serialised back to its canonical text.
 */
function canonicalQuery(text: string): string {
  ...
  try {
    ...
    throw new ExplorerTransformError({
      code: "filter_invalid",
      message: error instanceof Error ? error.message : "The filter did not parse.",
      meta: { query: trimmed },
    });
  }
...
export const setFilter: ExplorerTransform<SetFilterPayload, { query: string }> = ({
  state,
  payload,
}) => {
  const { query, mode } = setFilterPayloadSchema.parse(payload);
  const next = canonicalQuery(
    mode === "add" ? combineQueries({ base: state.queryText, addition: query }) : query,
  );
  return {
...
/**
 * The query after one click on a sidebar value. A value steps from neutral to
 * included to excluded and back; a second value of the same field joins the
 * first with OR, and a value of another field narrows with AND.
 */
...
export const toggleFacet: ExplorerTransform<ToggleFacetPayload, { query: string }> = ({
  state,
  payload,
}) => {
  const { field, value, exclude } = toggleFacetPayloadSchema.parse(payload);
  const args = { queryText: state.queryText, field, value };
  const next = canonicalQuery(exclude ? excludedFacetQuery(args) : toggledFacetQuery(args));
  return {
    state: { ...state, queryText: next, page: 1 },
    result: { query: next },
  };
