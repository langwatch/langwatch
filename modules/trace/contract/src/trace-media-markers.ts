/**
 * FROZEN TWIN of `platform/app/src/shared/content-parts/media-markers.ts`.
 * The application keeps its copy while both graphs ingest; edit neither
 * without editing the other. The marker list is the cheap gate BOTH the
 * ingestion-side value walker and the fold-side media collector use to decide
 * whether a nested JSON string is worth parsing, so a marker present on one
 * side and absent on the other is media one graph surfaces and the other
 * never sees.
 */
/**
