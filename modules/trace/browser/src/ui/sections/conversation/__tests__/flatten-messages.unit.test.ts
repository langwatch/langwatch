...
describe("flattenMessages", () => {
  describe("given tool calls on the message", () => {
    ...
    it("reads both wire casings the same way", () => {
      const snake = flatten([{ id: "m1", role: "assistant", content: "", tool_calls: [call] }]);
      const camel = flatten([{ id: "m1", role: "assistant", content: "", toolCalls: [call] }]);
      ...
      const [part] = flatten([
        {
          ...
          role: "assistant",
          content: "",
          tool_calls: [{ id: "c", function: { name: "search", arguments: "not json" } }],
        },
      ]);
  ...
  describe("given a tool result in a later message", () => {
    ...
    it("keeps a result whose call is not in the transcript", () => {
      const parts = flatten([{ id: "m2", role: "tool", tool_call_id: "orphan", content: "done" }]);
  ...
  describe("given reasoning alongside a reply", () => {
    it.each([["reasoning_content"], ["thinking"]])("carries %s onto the text part", (field) => {
      const [part] = flatten([
        {
          id: "m1",
          role: "assistant",
  ...
  describe("given a reply still streaming", () => {
    it("appends it once, and not again after it is stored", () => {
      const streaming = [{ messageId: "m1", role: "assistant", content: "par" }];
      ...
      const stored = flattenMessages({
        messages: [message({ id: "m1", role: "assistant", content: "partial" })],
        streaming,
      });
