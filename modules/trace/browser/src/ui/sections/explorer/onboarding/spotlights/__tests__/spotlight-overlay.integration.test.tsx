  ...
  describe("isAnchorSettled()", () => {
    describe("given the anchor is on-screen and unchanged since the last frame", () => {
      it("reports it settled", () => {
        expect(
          isAnchorSettled({ next: rect(), previous: rect(), viewportWidth: VW, scrollX: 0 }),
        ).toBe(true);
      });
    });
    ...
    describe("given the anchor is still parked off-screen", () => {
      it("is never settled, even if unchanged (parked, not resting)", () => {
        ...
        expect(
          isAnchorSettled({ next: parked, previous: parked, viewportWidth: VW, scrollX: 0 }),
        ).toBe(false);
      });
    });
    ...
    describe("given there is no prior frame to compare", () => {
      it("is not settled yet", () => {
        expect(
          isAnchorSettled({ next: rect(), previous: null, viewportWidth: VW, scrollX: 0 }),
        ).toBe(false);
        expect(
          isAnchorSettled({ next: null, previous: rect(), viewportWidth: VW, scrollX: 0 }),
        ).toBe(false);
      });
    });
  });
