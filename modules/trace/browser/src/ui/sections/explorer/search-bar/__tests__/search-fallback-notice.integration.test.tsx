...
function noticeFor(
  notice: Partial<NonNullable<ReturnType<typeof useFilterStore.getState>["searchNotice"]>> = {},
) {
  useFilterStore.getState().applyQueryText('eval:"frustrated users"');
  useFilterStore.getState().recordSearchNotice({
    projectId: "project-1",
...
describe("given a phrase search that another route fell back to", () => {
  describe("when the strip renders", () => {
    ...
    it("names the phrase reading", () => {
      useFilterStore.getState().applyQueryText('"frustrated users"');
      useFilterStore.getState().recordSearchNotice({
        projectId: "project-1",
        query: useFilterStore.getState().queryText,
