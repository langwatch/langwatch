...
describe("searchSubmitProgress", () => {
  describe("given a sentence routed to an Instant Eval", () => {
    ...
    it("names the router, then the estimate, then the start", () => {
      expect(
        searchSubmitProgress({ isRouting: true, isEstimating: false, isStarting: false }),
      ).toBe("Searching");
      expect(
        searchSubmitProgress({ isRouting: false, isEstimating: true, isStarting: false }),
      ).toBe("Estimating the Instant Eval");
      expect(
        searchSubmitProgress({ isRouting: false, isEstimating: false, isStarting: true }),
  ...
  describe("when nothing is pending", () => {
    it("says nothing", () => {
      expect(
        searchSubmitProgress({ isRouting: false, isEstimating: false, isStarting: false }),
      ).toBeNull();
    });
  });
});
