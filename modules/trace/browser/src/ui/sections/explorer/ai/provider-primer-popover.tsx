...
const ProviderPrimerBody: React.FC<{ copy: ProviderPrimerCopy }> = ({ copy }) => (
...
/**
 * Shown where a feature needs a model provider and none is enabled: it says
 * why a provider is needed rather than disabling the affordance, and links
 * straight to the settings page in a new tab so the page stays put.
 * @see dev/docs/best_practices/inline-fix-links.md
 */
export const ProviderPrimerPopover: React.FC<ProviderPrimerPopoverProps> = (props) => {
  const { children, copy = ASK_AI_PRIMER_COPY } = props;
  const [ownOpen, setOwnOpen] = useState(false);
  const handleOwnOpenChange = useCallback((e: { open: boolean }) => setOwnOpen(e.open), []);
  ...
  return (
    <PopoverRoot
      ...
      onOpenChange={
        anchored ? (e: { open: boolean }) => anchored.onOpenChange(e.open) : handleOwnOpenChange
      }
