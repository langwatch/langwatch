/**
 * A determinate bar over the table while a run judges the query's rows: it
 * reads the run's counters, and Stop asks the run to cancel.
...
export function instantEvalProgressCopy({
  ...
  return phase === "stopping" ? `Stopping ${counters}` : `Reading the last verdicts ${counters}`;
}
...
export function InstantEvalProgressBar({
  judged,
  total,
  matched,
  question,
