...
describe("given a run with total 10,000, progress 3,200 and 412 matched", () => {
  describe("when the progress bar renders", () => {
    ...
    it("reads the counters with a Stop button, at 32 percent", () => {
      ...
      expect(screen.getByText("Judging 3,200 / 10,000 · 412 matched")).toBeInTheDocument();
      expect(instantEvalProgressPercent({ judged: 3_200, total: 10_000 })).toBe(32);
    ...
    it("says it is stopping, with Stop disabled, once the run was asked to stop", () => {
      ...
      expect(screen.getByText("Stopping 500 / 1,354 · 72 matched")).toBeInTheDocument();
      expect(screen.getByRole("button", { name: "Stop judging" })).toBeDisabled();
      expect(
        instantEvalProgressCopy({
          judged: 920,
          total: 1_354,
    ...
    it("says the total is still being counted before the run has one", () => {
      expect(instantEvalProgressCopy({ judged: 0, total: null, matched: 0 })).toBe(
        "Judging 0 / … · 0 matched",
      );
