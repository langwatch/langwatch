...
interface SortableSectionProps {
  ...
  id: string;
  /**
   * Renders the section. Receives the drag handle props the wrapper obtained from
   * `useSortable` so the SidebarSection inside can paint the GripVertical handle at the
   * header level.
   */
...
/**
 * Wraps a single facet section in a `useSortable` node so the operator can drag-reorder
 * the flat sidebar list. Replaces the previous group-level FacetGroupHeader sortable
 * now that group headings are gone from the sidebar.
 */
