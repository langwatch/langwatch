...
export function useFilterSidebarData() {
  ...
  const facetItems = useMemo(() => {
    ...
    for (const cat of categoricals) {
      const baseItems = buildFacetItems({ cat, isSynthetic: cat.synthetic ?? isSynthetic });
      // Surface values the user typed in the search bar that discover did not
      // return (rare value, custom label, paste from another query): without
      // this an active filter counts toward the badge with no row to clear it.
      // Synthesised rows carry no count, and are pinned to the top of the list
      // so an active value never hides below the show-more cut.
 ...
 * and order rules reach the rows, and `FACET_COLORS` being correct proves
