...
function flattenTypedParts({
  ...
  const parts = content
    .map((item, index) => decodeContentPart({ item, context: { ...base, index } }))
...
function partsForMessage({
  ...
  if (msg.role === "user" || msg.role === "assistant") {
    return [...toolCallParts({ msg, messageId }), ...flattenContent({ msg, messageId })];
  }
  if (msg.role === "tool") {
...
export function flattenMessages({
  ...
  const parts = messages.flatMap<DisplayPart>((msg, index) => {
    ...
    if (failure) {
      return [{ kind: "error", id: msg.id!, error: failure, traceId: msg.trace_id }];
    }
