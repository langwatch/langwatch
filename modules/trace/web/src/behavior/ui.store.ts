...
const persistedUiSchema = z.object({
  ...
  sidebarWidth: z.number().optional().nullable(),
});
...
function loadPersistedUI(): Persisted {
  ...
  try {
    ...
    const parsed = persistedUiSchema.safeParse(JSON.parse(stored));
    if (!parsed.success) {
      return DEFAULT_PERSISTED;
    }
    return {
      sidebarCollapsed:
        parsed.data.sidebarCollapsed !== void 0
          ? parsed.data.sidebarCollapsed
          : DEFAULT_PERSISTED.sidebarCollapsed,
      sidebarWidth:
        parsed.data.sidebarWidth !== void 0 &&
        parsed.data.sidebarWidth !== null &&
        parsed.data.sidebarWidth > 0
          ? parsed.data.sidebarWidth
