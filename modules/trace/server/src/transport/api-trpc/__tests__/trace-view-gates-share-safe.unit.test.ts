...
describe("sharedTrace share-safe gates", () => {
  describe("given a viewer without cost:view", () => {
    ...
    describe("when gating Sessions lens rows", () => {
      ...
      it("strips every session rollup cost", () => {
        const sessions = [
          { conversationId: "s-1", totalCost: 4.2, totalTokens: 100, traceCount: 7 },
          { conversationId: "s-2", totalCost: 0, totalTokens: 5, traceCount: 1 },
        ];
        const out = gateSessionCost({ sessions, protections: anonProtections });
  ...
  describe("given a viewer whose captured content is hidden", () => {
    ...
    describe("when gating a session with no coding-agent row", () => {
      it("passes it through untouched", () => {
        ...
        expect(gateSessionTitle({ sessions, protections: anonProtections })).toEqual(sessions);
      });
    });
  ...
  describe("given a member with cost:view", () => {
    ...
    it("leaves header spend untouched", () => {
      const header = { totalCost: 1.23, nonBilledCost: 0.4 } as TraceHeader;
      expect(gateHeaderCost({ header, protections: memberProtections }).totalCost).toBe(1.23);
    });
    ...
    it("leaves Sessions lens spend untouched", () => {
      ...
      expect(gateSessionCost({ sessions, protections: memberProtections })[0]?.totalCost).toBe(4.2);
    });
  });
  ...
  describe("given evaluator verdicts", () => {
    ...
    describe("when the viewer cannot read captured content", () => {
      ...
      it("keeps the verdict but strips inputs, details and error text", () => {
        const out = gateEvaluations({ evaluations, protections: anonProtections });
    ...
    describe("when the viewer may read captured content", () => {
      it("keeps details and the error message but still never a stacktrace or inputs", () => {
        const out = gateEvaluations({ evaluations, protections: memberProtections });
