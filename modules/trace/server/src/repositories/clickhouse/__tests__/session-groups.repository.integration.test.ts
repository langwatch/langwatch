    ...
    values: [
      {
        ...
        WireTraceId: "a1b2c3d4e5f60718a1b2c3d4e5f60718",
        WireSpanId: "1111aaaa2222bbbb",
        CorrelationTraceId: "",
        CorrelationSpanId: "",
        CorrelationSource: "none",
        TimeUnixNano: String(BigInt(timeMs) * 1_000_000n),
        ObservedTimeUnixNano: String(BigInt(timeMs) * 1_000_000n),
        ...
        AttributesJson: "{}",
        AttributesFlatJson: "{}",
        AttributeKeys: ["event.name", "session.id"],
        ...
        CanonicalPayload: "{}",
        OccurredAt: new Date(timeMs),
        AcceptedAt: new Date(timeMs),
        DedupVersion: String(18_446_744_073_709_551_615n - BigInt(timeMs)),
        _retention_days: 90,
        _size_bytes: body.length,
      },
    ],
    format: "JSONEachRow",
    clickhouse_settings: { async_insert: 0, wait_for_async_insert: 0 },
...
describe.skipIf(!clickHouseConfigured)("given two sessions with several traces each", () => {
  ...
  it("walks every session exactly once in descending last-activity order", async () => {
    ...
    for (let guard = 0; guard < 10; guard++) {
      ...
      cursor = { sortValue: last.lastActivityMs, conversationId: last.conversationId };
    }
