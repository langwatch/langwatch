...
describe("given plain free text", () => {
  describe("when extracting the content terms", () => {
    it("returns the term, including hash-prefixed identifiers", () => {
      expect(traceQueryRepository.extractFreeTextTerms("#6418")).toEqual(["#6418"]);
    });

    it("returns quoted phrases whole", () => {
      expect(traceQueryRepository.extractFreeTextTerms('"checkout failed"')).toEqual([
        "checkout failed",
      ]);
    });
  });
...
describe("given a mix of structured tags and free text", () => {
  describe("when extracting the content terms", () => {
    it("keeps only the free-text terms", () => {
      expect(traceQueryRepository.extractFreeTextTerms('service:api "checkout failed"')).toEqual([
        "checkout failed",
      ]);
    });

    it("skips negated terms", () => {
      expect(traceQueryRepository.extractFreeTextTerms("keep NOT dropped")).toEqual(["keep"]);
    });

    it("skips terms negated with the dash prefix", () => {
      expect(traceQueryRepository.extractFreeTextTerms("keep -dropped")).toEqual(["keep"]);
    });
    ...
    it("keeps a doubly negated term", () => {
      expect(traceQueryRepository.extractFreeTextTerms("NOT (NOT keep)")).toEqual(["keep"]);
    });
  });
});
...
describe("given a regex term", () => {
  describe("when extracting the content terms", () => {
    it("ignores it, a regex source is not a substring", () => {
      expect(traceQueryRepository.extractFreeTextTerms("/checkout.*failed/")).toEqual([]);
      expect(traceQueryRepository.extractFreeTextTerms("keep /checkout.*failed/")).toEqual([
        "keep",
      ]);
    });
  });
...
describe("given a query joined by OR", () => {
  describe("when extracting the content terms", () => {
    it("returns no terms so the content search is skipped", () => {
      expect(traceQueryRepository.extractFreeTextTerms("checkout OR refund")).toEqual([]);
    });

    it("returns no terms when the OR hides inside parentheses", () => {
      expect(
        traceQueryRepository.extractFreeTextTerms("service:api AND (checkout OR refund)"),
      ).toEqual([]);
    });
  });
});
...
describe("given a query joined only by AND", () => {
  describe("when extracting the content terms", () => {
    it("returns every free-text term", () => {
      expect(traceQueryRepository.extractFreeTextTerms("checkout AND refund")).toEqual([
        "checkout",
        "refund",
      ]);
    });
...
describe("given empty or unparsable input", () => {
  describe("when extracting the content terms", () => {
    it("returns no terms", () => {
      expect(traceQueryRepository.extractFreeTextTerms("")).toEqual([]);
      expect(traceQueryRepository.extractFreeTextTerms("   ")).toEqual([]);
    });
    ...
    it("returns no terms for a query the parser rejects", () => {
      expect(traceQueryRepository.extractFreeTextTerms('"unterminated')).toEqual([]);
    });
  });
});
...
describe("given a free-text term wider than a filter value may be", () => {
  describe("when extracting the content terms", () => {
    ...
    it("drops the content branch rather than scanning on it", () => {
      ...
      expect(traceQueryRepository.extractFreeTextTerms(`keep ${wide}`)).toEqual([]);
    });

    it("keeps a term sitting on the width limit", () => {
      const wide = "x".repeat(MAX_VALUE_LENGTH);

      expect(traceQueryRepository.extractFreeTextTerms(wide)).toEqual([wide]);
    });
  });
});
...
describe("given more free-text terms than the content search carries", () => {
  ...
  it("drops the content branch rather than answering a narrower query", () => {
    ...
    expect(traceQueryRepository.extractFreeTextTerms(nine)).toEqual([]);
  });
  ...
  it("keeps a query sitting on the cap", () => {
    ...
    expect(traceQueryRepository.extractFreeTextTerms(eight.join(" "))).toEqual(eight);
  });
});
