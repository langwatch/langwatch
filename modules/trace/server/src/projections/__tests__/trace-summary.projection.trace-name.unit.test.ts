...
import { ATTR_KEYS, type TraceNameChangedEvent } from "@langwatch/trace-contract";
import {
...
describe("applySpanToSummary() trace name extraction", () => {
  describe("when root span has a name", () => {
    ...
    it("populates traceName from root span", () => {
      const span = createTestSpan({ parentSpanId: null, name: "OrderProcessingAgent" });
  ...
  describe("when root span has an empty name", () => {
    ...
    it("defaults traceName to empty string", () => {
      const span = createTestSpan({ parentSpanId: null, name: "" });
  ...
  describe("when the user emits a TraceNameChanged event", () => {
    ...
    it("can be replayed multiple times with the latest value winning", () => {
      ...
      state = projection.apply(state, makeTraceNameChangedEvent({ newName: "First rename" }));
      state = projection.apply(state, makeTraceNameChangedEvent({ newName: "Second rename" }));
