...
import { createExtractorContext, createLogExtractorContext } from "../../__tests__/test-helpers.ts";
...
describe("CodexCanonicaliserService.applyLog", () => {
  ...
  describe("CodexCanonicaliserService.apply (span side)", () => {
    ...
    it("is a no-op for non-codex scopes (Path A gen_ai.* spans are GenAICanonicaliserService's lane)", () => {
      const ctx = createExtractorContext(
        {
          "gen_ai.request.model": "gpt-5.5",
...
describe("CodexCanonicaliserService.apply on the codex_exec scope (exec wire)", () => {
...
describe("CodexCanonicaliserService turn-span cache writes", () => {
...
describe("CodexCanonicaliserService.applyLog reasoning effort", () => {
