...
describe("TraceCanonicalisationService — take vs preserve semantics", () => {
  describe("when attributes are taken (consumed)", () => {
    it("removes gen_ai.prompt from output after mapping to gen_ai.input.messages", () => {
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "gen_ai.prompt": "Hello world" },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes gen_ai.completion from output after mapping to gen_ai.output.messages", () => {
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "gen_ai.completion": "The answer is 42" },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes llm.output_messages from output after mapping", () => {
      ...
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "llm.output_messages": messages },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes traceloop.entity.input from output after mapping", () => {
      ...
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "traceloop.entity.input": input },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes langwatch.thread.id from output after mapping to gen_ai.conversation.id", () => {
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "langwatch.thread.id": "thread-123" },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes langwatch.input from bag then re-sets in canonical output", () => {
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "langwatch.input": "some raw input" },
        events: [],
        span: stubSpan,
      });
    ...
    it("removes ai.prompt from output after mapping", () => {
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "ai.prompt": JSON.stringify("A vercel prompt") },
        events: [],
        span: makeStubSpan({
  ...
  describe("when langwatch.input is taken and re-set", () => {
    it("take removes from bag, setAttr re-adds to out, final has canonical form", () => {
      ...
      const result = canonicalisation.canonicalizeSpanAttributes({
        spanAttributes: { "langwatch.input": structuredInput },
        events: [],
        span: stubSpan,
      });
