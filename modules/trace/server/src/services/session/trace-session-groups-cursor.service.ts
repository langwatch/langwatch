/**
 * The Sessions lens page cursor: what it carries, how it travels, and how a page's last row turns
 * into the next. Its own module because it is the one part of the lens read that is a wire
 * contract rather than a query — clients hold these strings across requests.
 */
 ...
 * has drifted in either direction. A new sort column missing from here would make the schema
 * reject a cursor this very service minted, losing the caller their page mid-walk.
 */
...
/**
 * The decoded cursor. It carries the sort it was minted under because `sortValue` is meaningless
 * without it: the repository recomputes the keyset boundary from the current sort, so a cursor
 * from a cost sort compared against a timestamp aggregate would page through nonsense.
 */
...
export class TraceSessionGroupsCursorService {
  ...
  /**
   * The repository's keyset boundary for this request. A cursor minted under another sort points
