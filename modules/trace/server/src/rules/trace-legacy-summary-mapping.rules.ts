...
export function mapAttributesToMetadata(
  ...
  const parsedArrayKeys = assignArrayMetadata({
    metadata,
    attributes,
  });
...
function eventMetrics(rawMetrics: unknown): Record<string, number> {
...
function eventDetails(rawDetails: unknown): Record<string, string> {
...
type SpanEventResult = { kind: "event"; event: Event } | { kind: "none" };

/** One span's event result, empty when the span carries no typed `event` object. */
function readSpanEvent({
  span,
  projectId,
  traceId,
}: {
  ...
  return {
    ...
    event: {
      ...
      metrics: eventMetrics(eventRecord.metrics),
      event_details: eventDetails(eventRecord.details),
      trace_id: traceId,
      timestamps: {
...
export function extractEventsFromSpans({
  ...
  for (const span of spans) {
    const result = readSpanEvent({ span, projectId, traceId });
