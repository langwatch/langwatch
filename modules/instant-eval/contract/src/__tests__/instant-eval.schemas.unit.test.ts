...
describe("given a list requested with a cursor", () => {
  ...
  describe("when only one half is given", () => {
    ...
    it("refuses it and names the missing half", () => {
      ...
      expect(withoutId.error.issues.map((issue) => issue.path)).toEqual([["beforeId"]]);
      expect(withoutInstant.error.issues.map((issue) => issue.path)).toEqual([["before"]]);
...
describe("given a results page requested with no bounds", () => {
  ...
  describe("when it names the matches it wants", () => {
    it("reads every spelling of the flag the way it is written", () => {
      expect(instantEvalResultsQuerySchema.parse({ matched: "false" }).matched).toBe(false);
      expect(instantEvalResultsQuerySchema.parse({ matched: "no" }).matched).toBe(false);
      expect(instantEvalResultsQuerySchema.parse({ matched: "1" }).matched).toBe(true);
    });
  });
});
