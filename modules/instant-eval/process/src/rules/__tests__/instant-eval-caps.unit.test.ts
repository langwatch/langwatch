...
import { instantEvalRowCapFor, instantEvalRowLimitOrRefuse } from "../instant-eval-caps.rules.ts";
...
describe("given a project asking for a run", () => {
  describe("when no limit is requested", () => {
    ...
    it("takes the default cap on either plan", () => {
      expect(instantEvalRowLimitOrRefuse({ plan: free })).toBe(INSTANT_EVAL_DEFAULT_ROW_CAP);
      expect(instantEvalRowLimitOrRefuse({ plan: paid })).toBe(INSTANT_EVAL_DEFAULT_ROW_CAP);
    });
  });
    ...
    it("refuses the run and names the highest cap any plan offers", () => {
      const refusal = (plan: { isFree: boolean; name: string }) => {
        try {
          ...
          return error instanceof HandledError ? error : undefined;
        }
      };
  ...
  describe("when the requested limit is within the plan's ceiling", () => {
    it("gives the run the number it asked for", () => {
      expect(instantEvalRowLimitOrRefuse({ requested: 50_000, plan: paid })).toBe(50_000);
      expect(instantEvalRowLimitOrRefuse({ requested: 500, plan: free })).toBe(500);
    });
  });
});
