...
import { instantEvalRunQuestions } from "../instant-eval-run-questions.rules.ts";

const NOW = 1_758_000_000_000;

const calls: LangWatchQLJudgementCall[] = [
  {
    column: "annoyed",
    function: "eval",
    reads: "probability",
...
const booleanOnly = instantEvalRunQuestions([calls[0]!]);
...
function spanKeys(...pairs: [traceId: string, spanId: string][]): InstantEvalRowKey[] {
...
describe("given a page of judged rows", () => {
  describe("when it is mapped", () => {
    ...
    it("writes one row per trace and question", () => {
      ...
      expect(records.map((record) => [record.TraceId, record.QuestionId])).toEqual([
        ["t1", "annoyed"],
        ["t1", "intent"],
        ["t1", "helpfulness"],
        ["t2", "annoyed"],
    ...
    it("carries the run, the trace, the question and its verdict", () => {
      const { records } = mapInstantEvalPage({
        ...
        runId: "run-1",
        questions,
        rows: [{ TraceId: "t1", annoyed: 0.9, intent: "refund", helpfulness: 4.2 }],
        keys: keys("t1"),
        skipReason: "",
        now: NOW,
      });
  ...
  describe("when the judge answered nothing for a row", () => {
    ...
    it("names the stop on the rows it reached, not the page's own skip", () => {
      const { records } = mapInstantEvalPage({
        tenantId: "project-1",
        runId: "run-1",
        questions: booleanOnly,
        ...
        keys: keys("t1", "t2"),
        skipReason: "classifier_failed",
        unjudgedRows: { indexes: new Set([1]), reason: "cancelled" },
        now: NOW,
      });

      expect(records.map((record) => [record.Status, record.Error])).toEqual([
        ["judged", ""],
        ["skipped", "cancelled"],
      ]);
    });
...
describe("given a page's skip reasons", () => {
  describe("when one reason is chosen for the page", () => {
    it("takes the commonest", () => {
      expect(
        instantEvalSkipReason({ classifier_rate_limited: 3, classifier_input_too_large: 1 }),
      ).toBe("classifier_rate_limited");
    });

    it("prefers a failure over a decline on a tie", () => {
      expect(instantEvalSkipReason({ classifier_input_too_large: 2, classifier_failed: 2 })).toBe(
        "classifier_failed",
      );
    });
  ...
  describe("when the failure rate is measured", () => {
    ...
    it("puts a mostly failed page past the ceiling", () => {
      const rate = instantEvalPageFailureRate({
        counters: { rows: 10, matched: 0, matchedByQuestion: {}, failed: 21, skipped: 0 },
        questions: 3,
      });
    ...
    it("keeps a partly failed page under the ceiling", () => {
      const rate = instantEvalPageFailureRate({
        counters: { rows: 10, matched: 5, matchedByQuestion: {}, failed: 3, skipped: 0 },
        questions: 3,
      });
    ...
    it("counts a deliberate skip as no failure at all", () => {
      const rate = instantEvalPageFailureRate({
        counters: { rows: 10, matched: 0, matchedByQuestion: {}, failed: 0, skipped: 30 },
        questions: 3,
      });
    ...
    it("reports nothing for an empty page", () => {
      expect(
        instantEvalPageFailureRate({
          counters: { rows: 0, matched: 0, matchedByQuestion: {}, failed: 0, skipped: 0 },
          questions: 3,
        }),
      ).toBe(0);
    });
