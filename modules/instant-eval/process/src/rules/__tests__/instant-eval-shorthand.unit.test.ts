...
describe("given a target and some questions", () => {
  ...
  describe("when a window is named", () => {
    ...
    it("refuses a window that runs backwards", () => {
      expect(() =>
        expand({ start: "2026-09-02T00:00:00.000Z", end: "2026-09-01T00:00:00.000Z" }),
      ).toThrow(InstantEvalQueryInvalidError);
    });
  });
  ...
  describe("when the questions are long enough to crowd the classifier", () => {
    ...
    it("writes the budget the questions leave rather than the default", () => {
      ...
      const written = /llm_readable_trace\(TraceId, (\d+)\)/.exec(sql)?.[1];

      expect(Number(written)).toBeLessThan(INSTANT_EVAL_SHORTHAND_TEXT_BUDGET);
...
describe("given how questions are named", () => {
  describe("when no ids are given", () => {
    ...
    it("names them by position", () => {
      const { sql } = expand({ questions: [ask(), ask({ instructions: "The agent apologised" })] });
  ...
  describe("when two questions share an id", () => {
    ...
    it("refuses the second one", () => {
      expect(() => expand({ questions: [ask({ id: "annoyed" }), ask({ id: "annoyed" })] })).toThrow(
        /already the name of a column/,
      );
    });
  });
  ...
  describe("when the instructions carry a quote and a backslash", () => {
    ...
    it("escapes both", () => {
      const { sql } = expand({ questions: [ask({ instructions: "it's a path C:\\tmp" })] });
...
describe("given a question list read off the wire", () => {
