...
const connection = DB_URL
  ? PrismaConnectionService.create({
      ...
      logger: createLogger("langwatch:organization:test:membership-departments"),
    }).connect(PrismaConfigService.create().resolve({ databaseUrl: DB_URL, log: ["error"] }))
  : undefined;

describe.skipIf(!connection)(
  "PrismaOrganizationMembershipRepository dated department links",
  () => {
    ...
    it("seeds a dated link when the re-asserted pointer has none", async () => {
      await prisma().organizationUser.updateMany({
        where: { organizationId: ORG_ID, userId: SASHA },
        data: { departmentId: `standing-${ns}` },
      });
