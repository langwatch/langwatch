...
describe("the coding-agent read gates", () => {
  ...
  describe("given a viewer who may not read the project's captured content", () => {
    ...
    it("blanks every title and leaves the rest of the row intact", () => {
      const rows = gateSessionListTitles({
        rows: [sessionListRow()],
        canReadCapturedContent: false,
      });

      // Both sides of the conversation are required upstream of this gate: a
      // title paraphrases the prompt and the reply together, so a viewer
      // allowed one could read the other in it. The process resolves that and
      // hands the answer down as one decision.
