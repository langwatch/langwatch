...
const modelProviders = new TestModelProviderService((input: ModelCostEstimateInput): number => {
  ...
  const cacheWriteOneHour = Number(attrs["gen_ai.usage.cache_creation_1h.input_tokens"] ?? 0);
  return (
    (input.promptTokens ?? 0) * 0.000003 +
    (input.completionTokens ?? 0) * 0.000015 +
    cacheRead * 0.0000003 +
    (cacheWrite - cacheWriteOneHour) * 0.00000375 +
    cacheWriteOneHour * 0.000006
  );
});
...
describe("read-back losslessness (ADR-066)", () => {
  ...
  describe("when a further contribution folds onto the recovered state", () => {
    ...
    it("places a late-arriving step by when it started, not by when it turned up", () => {
      ...
      expect(withoutTimes.steps.map((step) => step.name)).toEqual(["Read", "Bash", "Grep"]);
    });
  });
});
...
describe("coding-agent session fold, per-agent gating", () => {
  describe("when a Cowork session folds from events only", () => {
    ...
    it("folds the tool run — name, count, step — from the tool_result event", () => {
      ...
      expect(state.steps).toEqual([{ name: "Bash", count: 1, startedAtMs: 2_000, failed: false }]);
...
describe("coding-agent session fold, codex", () => {
  ...
  describe("when a codex turn span contributes", () => {
    ...
    it("reads the input the canonicalisation settled on, without deriving it again", () => {
      const projection = makeProjection();
      const { "codex.turn.token_usage.non_cached_input_tokens": _omit, ...rest } = codexTurnFacts;
  ...
  describe("when a codex tool_result event contributes", () => {
    ...
    it("folds the tool run from the event and drops the sandbox outcome", () => {
      ...
      expect(state.steps).toEqual([{ name: "shell", count: 1, startedAtMs: 2_000, failed: false }]);
...
describe("usage by declared context", () => {
  ...
  describe("when model calls arrive stamped with different branches", () => {
    ...
    it("records each branch's own tokens and cost, and the counters stay the sum", () => {
      ...
      const split = state.usageByContext[contextUsageKey(stampOn("feat/split"))];
  ...
  describe("when a session declares more contexts than the record holds", () => {
    ...
    it("keeps the first contexts it saw and keeps counting every call", () => {
      ...
      expect(Object.keys(state.usageByContext)).toHaveLength(MAX_USAGE_CONTEXTS);
      expect(state.usageByContext[contextUsageKey(stampOn("feat/0"))]).toBeDefined();
      expect(state.usageByContext[contextUsageKey(stampOn(`feat/${calls - 1}`))]).toBeUndefined();
