...
const HANDLER_ANSWER_REASON =
  ...
  "including the two doors that stream server-sent events";
...
/** The handler context every door in this family runs on. */
type ExperimentV3Context = ServiceContext<EndpointVariables>;
...
export function createExperimentV3RestApp<TSession extends ExperimentV3RestSession>(options: {
  ...
  const readWorkbenchHandler = async (
    c: ExperimentV3Context,
  ...
  const saveWorkbenchHandler = async (
    c: ExperimentV3Context,
  ...
  return (
    service
      ...
      .registerRoute("post", "/execute", MANAGEMENT_API_VERSION, executeHandler, (b) =>
        policy(sessionAuth)(b)
          .withInput(executionRequestSchema)
          .withRawResponse(HANDLER_ANSWER_REASON, { contentType: "text/event-stream" })
          .withDocs({ hide: true }),
      )
      // ── POST /abort ────────────────────────────────────────────────────
      .registerRoute("post", "/abort", MANAGEMENT_API_VERSION, abortHandler, (b) =>
        policy(sessionAuth)(b)
          .withRawBody("text", { contentType: "application/json" })
          .withRawResponse(HANDLER_ANSWER_REASON, { contentType: "application/json" })
          .withDocs({ hide: true }),
      )
      // ── POST /:slug/run  (CI/CD execution) ─────────────────────────────
      .registerRoute("post", "/:slug/run", MANAGEMENT_API_VERSION, runHandler, (b) =>
        policy(apiKeyAuthRun)(b)
          .withParams(slugParamsSchema)
          // The body is read unparsed: an empty one is a full run, malformed
          // JSON is a 400 in this family's own words, and `runInputsBodySchema`
          // parses what is left.
          .withRawBody("text", { contentType: "application/json" })
          .withRawResponse(HANDLER_ANSWER_REASON, { contentType: "application/json" })
          .withDocs({
            ...
            responses: {
              ...
              400: {
                ...
                content: {
                  // These three refusals are hand-rolled in the handler and
                  // carry a sentence in `error` rather than a stable code, so
                  // this documents the shape as sent.
      ...
      .registerRoute("get", "/runs", MANAGEMENT_API_VERSION, listRunsHandler, (b) =>
        policy(apiKeyAuthRead)(b)
      ...
      .registerRoute("get", "/runs/:runId", MANAGEMENT_API_VERSION, runStatusHandler, (b) =>
        policy(apiKeyAuthRead)(b)
      ...
      .registerRoute(
        ...
        MANAGEMENT_API_VERSION,
        readWorkbenchHandler,
        (b) =>
          policy(apiKeyAuthExperimentsView)(b)
            .withParams(slugParamsSchema)
      ...
      .registerRoute(
        ...
        "/:slug/workbench-state",
        MANAGEMENT_API_VERSION,
        saveWorkbenchHandler,
        (b) =>
          policy(apiKeyAuthExperimentsUpdate)(b)
            .withParams(slugParamsSchema)
            ...
            .withDocs({
              ...
              responses: {
                ...
                200: {
                  ...
                  content: {
                    "application/json": { schema: resolver(saveWorkbenchStateResponseSchema) },
                  },
                },
              },
            }),
      ...
      // ── GET /:slug/versions ────────────────────────────────────────────
      .registerRoute("get", "/:slug/versions", MANAGEMENT_API_VERSION, listVersionsHandler, (b) =>
        policy(apiKeyAuthExperimentsView)(b)
          .withParams(slugParamsSchema)
          ...
          .withDocs({
            ...
            responses: {
              ...
              200: {
                ...
                content: {
                  "application/json": { schema: resolver(listWorkbenchVersionsResponseSchema) },
                },
              },
            },
          }),
...
export function createExperimentV3LegacyAliasRestApp(options: {
  ...
  const { service, policy } = options.security.createServiceVersionedApp({
    name: "evaluations-v3-alias",
    basePath: "/api/evaluations/v3",
    errorEnvelope: "legacy",
    bareMount: true,
  });

  const forward = (c: ExperimentV3Context) => {
  ...
  return service
    .registerAnyMethodRoute("/*", MANAGEMENT_API_VERSION, forward, (b) =>
      policy(
        publicEndpoint(
          "the alias rewrites the path and hands the request to the canonical route, " +
            "which authenticates it exactly as it would a direct one",
        ),
      )(b).withRawResponse("the canonical route's own answer, written through unchanged"),
    )
    .build();
}
