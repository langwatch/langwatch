...
import { toAttachmentContentPart } from "../attachment-parts.rules.ts";

const BYTES = Buffer.from("bytes");
const BASE64 = BYTES.toString("base64");

const dataUrl = ({ mediaType, name }: { mediaType: string; name?: string }): string =>
  name
...
describe("given a recording in a format a model names", () => {
  describe("when the part is built", () => {
    ...
    it.each([
      ...
      expect(toAttachmentContentPart(dataUrl({ mediaType }))).toEqual({
        type: "input_audio",
...
describe("given a recording in a format no model names", () => {
  describe("when the part is built", () => {
    ...
    it("sends it as a file part rather than calling it WAV", () => {
      ...
      expect(toAttachmentContentPart(value)).toEqual({
        type: "file",
...
describe("given a value that is not a base64 data URL", () => {
  describe("when the part is built", () => {
    it("yields nothing, because it carries no attachment", () => {
      expect(toAttachmentContentPart("https://example.com/shot.png")).toBeNull();
    });
  });
});
