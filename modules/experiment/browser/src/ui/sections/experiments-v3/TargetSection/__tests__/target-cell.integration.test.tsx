...
describe("TargetCellContent", () => {
  ...
  describe("when displaying output", () => {
    ...
    it("names the evaluator and the fix when no input resolved", () => {
      ...
      expect(
        screen.getByText(/Map the fields of Answer Correctness in the evaluator settings/),
      ).toBeInTheDocument();
      expect(screen.queryByText(/Check its configuration, then run again/)).not.toBeInTheDocument();
    });
