...
describe("transformBatchEvaluationData", () => {
  ...
  describe("given V2 format (legacy, no targets)", () => {
    ...
    it("preserves a malformed nested value for the existing display fallback", () => {
      const data: ExperimentRunWithItems = {
        experimentId: "exp-1",
        runId: "run-1",
        projectId: "proj-1",
        dataset: [
          {
            index: 0,
            entry: { question: "Hello" },
            predicted: { end: { response: "first" }, auxiliary: { value: 1 } },
          },
          {
            index: 1,
            entry: { question: "Again" },
            predicted: { end: "raw legacy value", auxiliary: { value: 2 } },
          },
        ],
        evaluations: [],
        timestamps: createTimestamps(),
      ...
      expect(result.rows[1]?.targets.end?.output).toBe("raw legacy value");
    });
