...
describe("PrismaAuthzReadRepository", () => {
  describe("when reading organization membership", () => {
    ...
    describe("when the membership has been disabled to free its seat", () => {
      it("reports the row as disabled rather than hiding it, so the denial can say so", async () => {
        ...
        expect(
          await PrismaAuthzReadRepository.create(prisma).findOrganizationMembership({
            userId: "alice",
            organizationId: "org-1",
          }),
  ...
  describe("when finding API key bindings", () => {
    it("reads the key's own bindings in this organization, with no membership gate", async () => {
      ...
      await PrismaAuthzReadRepository.create(prisma).findApiKeyBindings({
        apiKeyId: "key-1",
        organizationId: "org-1",
      });
  ...
  describe("when finding the API key owner", () => {
    describe("when the key belongs to a user", () => {
      it("returns the owning user id for a personal key", async () => {
        ...
        const owner = await PrismaAuthzReadRepository.create(prisma).findApiKeyOwner("key-1");
  ...
  describe("when finding share links", () => {
    it("filters by presented token AND the resource links, project-anchored", async () => {
      ...
      await PrismaAuthzReadRepository.create(prisma).findShareLinks({
        projectId: "proj-1",
        tokens: ["tok-1"],
        links: [
  ...
  describe("when finding the project lineage", () => {
    it("returns the owning team and organization, null for an unknown project", async () => {
      ...
      expect(await repository.findProjectLineage({ projectId: "proj-ghost" })).toBeNull();
    });
  });
});
