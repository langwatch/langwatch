...
import { AuthzGrantIdentityService } from "../authz-grant-identity.service.ts";
...
describe("grant identity", () => {
  ...
  describe("when the same fact is derived twice", () => {
    it("yields the same id — a KSUID with no random bits", () => {
      const a = grantIdentity.deriveGrantId(base);
      const b = grantIdentity.deriveGrantId({ ...base });
      expect(a).toBe(b);
  ...
  describe("when the deriving process's ksuid environment differs", () => {
    it("derives the same id, so two processes cannot disagree about one row", () => {
      ...
      try {
        setEnvironment("dev");
        const fromDev = grantIdentity.deriveGrantId(base);
        setEnvironment("staging");
        const fromStaging = grantIdentity.deriveGrantId(base);
  ...
  describe("when any part of the fact differs", () => {
    it("yields a different id per scope, principal, org, token, and business time", () => {
      const ids = [
        grantIdentity.deriveGrantId(base),
        grantIdentity.deriveGrantId({ ...base, organizationId: "org_other" }),
        grantIdentity.deriveGrantId({
          ...base,
        ...
        grantIdentity.deriveGrantId({ ...base, resourceToken: "tok_1" }),
        grantIdentity.deriveGrantId({ ...base, resourceToken: "tok_2" }),
        grantIdentity.deriveGrantId({ ...base, occurredAtMs: OCCURRED_AT + 60_000 }),
      ];
    ...
    it("ignores sub-second differences in business time", () => {
      ...
      expect(grantIdentity.deriveGrantId({ ...base, occurredAtMs: OCCURRED_AT + 500 })).toBe(
        grantIdentity.deriveGrantId(base),
      );
    });
  });
  ...
  describe("when two facts differ only in where one field ends and the next begins", () => {
    it("still yields different ids", () => {
      ...
      // other.
      const left = grantIdentity.deriveGrantId({
        ...base,
        principal: { type: "user", id: "a" },
        scope: { type: "TEAM", id: "bc" },
      });
      const right = grantIdentity.deriveGrantId({
        ...base,
