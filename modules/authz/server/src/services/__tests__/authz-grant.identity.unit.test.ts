...
describe("grant identity", () => {
  ...
  describe("when the same fact is derived twice", () => {
    it("yields the same id — a KSUID with no random bits", () => {
      const a = AuthzGrantIdentity.deriveGrantId(base);
      const b = AuthzGrantIdentity.deriveGrantId({ ...base });
      expect(a).toBe(b);
  ...
  describe("when the deriving process's ksuid environment differs", () => {
    it("derives the same id, so two processes cannot disagree about one row", () => {
      ...
      try {
        setEnvironment("dev");
        const fromDev = AuthzGrantIdentity.deriveGrantId(base);
        setEnvironment("staging");
        const fromStaging = AuthzGrantIdentity.deriveGrantId(base);
  ...
  describe("when any part of the fact differs", () => {
    it("yields a different id per scope, principal, org, token, and business time", () => {
      const ids = [
        ...
        AuthzGrantIdentity.deriveGrantId({ ...base, organizationId: "org_other" }),
        AuthzGrantIdentity.deriveGrantId({
          ...base,
        ...
        AuthzGrantIdentity.deriveGrantId({ ...base, resourceToken: "tok_1" }),
        AuthzGrantIdentity.deriveGrantId({ ...base, resourceToken: "tok_2" }),
        AuthzGrantIdentity.deriveGrantId({ ...base, occurredAtMs: OCCURRED_AT + 60_000 }),
      ];
  ...
  describe("when two facts differ only in where one field ends and the next begins", () => {
    it("still yields different ids", () => {
      ...
      // other.
      const left = AuthzGrantIdentity.deriveGrantId({
        ...base,
        principal: { type: "user", id: "a" },
        scope: { type: "TEAM", id: "bc" },
      });
      const right = AuthzGrantIdentity.deriveGrantId({
        ...base,
