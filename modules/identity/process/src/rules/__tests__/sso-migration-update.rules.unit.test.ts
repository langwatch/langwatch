...
import type { SsoMigrationScimStatus } from "@langwatch/identity-contract";
...
describe("scimStatusOf", () => {
  describe("given the previous connection's directory sync is pushing today", () => {
    ...
    it("asks for it to be repointed while nothing hosts the move across", () => {
      expect(scimStatusOf({ legacySyncs: true, replacementSyncState: null })).toBe(
        "needs-repointing",
      );
    });
    ...
    it("is ready once the replacement has a token, whether or not a push has landed yet", () => {
      ...
      expect(scimStatusOf({ legacySyncs: true, replacementSyncState: "SYNCING" })).toBe("ready");
    });
  });

  describe("given no directory sync pushes through the previous connection", () => {
    it("has nothing to move", () => {
      expect(scimStatusOf({ legacySyncs: false, replacementSyncState: null })).toBe(
        "not-applicable",
      );
      expect(scimStatusOf({ legacySyncs: false, replacementSyncState: "REVOKED" })).toBe(
        "not-applicable",
      );
    });

    it("is ready when the customer set the replacement's sync up themselves", () => {
      expect(scimStatusOf({ legacySyncs: false, replacementSyncState: "SYNCING" })).toBe("ready");
    });
  });
});
...
describe("migrationBlockers", () => {
  ...
  const codesFor = (scimStatus: SsoMigrationScimStatus) =>
    migrationBlockers({ ...ready, scimStatus }).map((blocker) => blocker.code);

  describe("given the previous connection's directory sync has not been repointed", () => {
    /** @scenario "Finishing waits for directory sync on the new connection while nothing moves it across" */
    it("refuses the finish until it is", () => {
      expect(codesFor("needs-repointing")).toEqual(["scim-needs-repointing"]);
    });
  });

  describe("given directory sync is on the replacement, or not in use", () => {
    it("does not hold the finish for it", () => {
      expect(codesFor("ready")).toEqual([]);
      expect(codesFor("not-applicable")).toEqual([]);
    });
  });
});
...
describe("strandedUserIdsOf", () => {
  const legacyIdentifierIds = new Set(["kim-legacy", "lee-legacy", "max-legacy"]);
  const identifier = (
    id: string,
    userId: string,
