...
import { IdentityEmailService } from "../identity-email.service.ts";
import {
...
function build(
  ...
  const repositories = liveRepositories(client);
  const emails = IdentityEmailService.create(
    repositories.heads,
    CachedIdentityLatchService.create({
      repository: repositories.latch,
      ttlMs: options.cacheTtlMs ?? IDENTITY_LATCH_CACHE_TTL_MS,
      maxUsers: options.cacheMaxUsers ?? IDENTITY_LATCH_CACHE_MAX_USERS,
      now: () => clock.now,
...
describe("the identity email read fork over Postgres", () => {
