...
export function reduceIdentity({
  ...
  switch (fact.type) {
    ...
    case LINK_REJECTED_EVENT_TYPE:
      // A proposal and its decision change no head: the identifier arrives
      // through the ordinary attach ceremony, so a fold here would be the
      // auto-link the proposal exists to refuse.
      return heads;
  }
}
