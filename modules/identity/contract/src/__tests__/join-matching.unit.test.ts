...
describe("given an address a public email provider issued", () => {
  describe("when organizations hold verified addresses on that domain", () => {
    ...
    it("covers the country variants of every provider it lists", () => {
      ...
      // where: an address on a missed variant reads as a work address, so a
      // stranger who verifies one is told an organization's name and size.
      // Pinned by brand, so adding a provider means adding its variants.
      const variantsOf: Record<string, string[]> = {
        ...
        yahoo: ["com", "co.uk", "co.jp", "de", "fr", "it", "es", "ca", "com.au", "com.br"],
