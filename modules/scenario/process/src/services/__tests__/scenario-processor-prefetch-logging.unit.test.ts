/**
 * Ported from
 * platform/app/src/server/scenarios/__tests__/scenario-processor-prefetch-logging.unit.test.ts.
 */
...
function logFor(reason: ScenarioModelParametersFailureReason | undefined) {
  ...
  ScenarioProcessorService.logPrefetchFailure({
    jobLogger,
    prefetchResult: {
      success: false,
...
describe("ScenarioProcessorService.logPrefetchFailure", () => {
  describe("when the customer's own configuration blocked the run", () => {
    ...
    it.each(["invalid_model_format", "provider_not_found", "missing_params"] as const)(
      "logs %s below error too",
      (reason) => {
  ...
  describe("when the failure is not one we recognise as the customer's", () => {
    ...
    it("keeps a reason added later at error level until someone classifies it", () => {
      const jobLogger = logFor("some_future_reason" as ScenarioModelParametersFailureReason);
