...
function rejectionAsksForReasoningOff(body: string): boolean {
  ...
  const parsed = providerErrorBodySchema.safeParse(raw);
...
function extractRetryEligibleRequestBody(
  ...
  const parsed = retryRequestBodySchema.safeParse(raw);
  if (!parsed.success) return undefined;
  const hasTools = parsed.data.tools !== undefined && parsed.data.tools.length > 0;
  if (!hasTools || parsed.data.reasoning_effort !== undefined) return undefined;
  return parsed.data;
}
