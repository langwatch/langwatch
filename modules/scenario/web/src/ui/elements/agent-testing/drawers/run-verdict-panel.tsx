...
function CriterionRow({ criterion, passed }: { criterion: string; passed: boolean }) {
  return (
    <HStack align="start" gap={2}>
      <Box marginTop="1px" flexShrink={0} color={passed ? "green.fg" : "red.fg"}>
    ...
    <VStack align="stretch" gap={2} data-testid={testId}>
      ...
      <VStack align="stretch" gap={2.5}>
        {criteria.map((criterion, at) => (
          <CriterionRow key={`${criterion}-${at}`} criterion={criterion} passed={passed} />
        ))}
      </VStack>
    </VStack>
  );
