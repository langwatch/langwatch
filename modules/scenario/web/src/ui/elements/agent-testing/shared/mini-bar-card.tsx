...
function barHeight({ value, scale }: { value: number | null; scale: number }): number {
...
function BarColumn({ bar, scale }: { bar: MiniBar; scale: number }) {
  return (
    <VStack flex={1} minWidth={0} maxWidth="28px" gap={1} align="center" justify="flex-end">
      <Text
        fontSize="10px"
        fontWeight="medium"
...
function BarArea({
  ...
  return (
    <HStack gap={3} height={`${BAR_AREA_HEIGHT}px`} alignItems="flex-end" marginTop={2}>
      {groups.map((group) => (
        <HStack
          key={group.key}
          flex={1}
