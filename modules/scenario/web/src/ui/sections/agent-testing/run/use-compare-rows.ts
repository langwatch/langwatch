...
function useCompareTransitions({
  ...
  const { target, setTarget, showParams, parameterLine, parameterRows } = fields;
  ...
  const leaveCompare = useCallback(
    (row: CompareRow) => {
      ...
      setParameterRows(secretRows.length > 0 ? [...rowsFromLine(line), ...secretRows] : null);
      setRowsRequested(secretRows.length > 0);
...
export function useCompareRows({
  ...
  const updateCompareRow = useCallback(
    (index: number, patch: Partial<CompareRow>) => {
      setCompareRows(compareRows.map((row, at) => (at === index ? { ...row, ...patch } : row)));
