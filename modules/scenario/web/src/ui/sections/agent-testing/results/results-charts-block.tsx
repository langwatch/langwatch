...
import { formatCost } from "@langwatch/suite-web/run-formatters";
...
function PassRateOverTimeChart({ buckets }: { buckets: SeriesBucket[] }) {
  return (
    ...
    >
      ...
      <HStack gap={1.5} height={`${CHART_HEIGHT}px`} alignItems="flex-end" marginTop={2}>
        {buckets.map((bucket) => (
          <VStack
            key={bucket.label}
            flex={1}
          ...
          >
            ...
            <Box
              ...
              background={bucket.isEmpty ? "border" : passRateColor(bucket.passRate)}
            />
          </VStack>
        ))}
      </HStack>
...
export function ResultsChartsBlock({ totals, buckets }: ResultsChartsBlockProps) {
  return (
    <Grid
      gap={3}
      alignItems="stretch"
    ...
    >
      <Stat label="Executions" value={String(totals.executions)} testId="results-stat-executions" />
      <Stat
        label="Pass rate"
