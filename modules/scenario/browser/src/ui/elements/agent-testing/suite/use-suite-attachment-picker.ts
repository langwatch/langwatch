...
function editAttachment({
  ...
  openEvaluatorEditor({
    ...
    onRemove: () => {
      update((draft) => ({
        ...draft,
        evaluators: draft.evaluators.filter((current) => current.id !== attachmentId),
      }));
      goBack();
    },
  });
...
function attachEvaluator({
  ...
  if (!draft) return;
  const navigation = origin === "list" ? { replaceCurrentInStack: true } : undefined;
  const already = draft.evaluators.find((attachment) => attachment.evaluatorId === evaluator.id);
  if (already) {
...
export function openEvaluatorPicker({
  ...
  attach: (params: { evaluator: AttachableEvaluator; origin?: "list" | "created" }) => void;
