...
function useSecretParameterFields({
  ...
  return {
    setSecretValue,
    hasMissingSecrets: isCollecting && (declaredWithoutValue || rowWithoutValue),
    /** Whether the block may fold back into the single line it started on. */
    canLeaveParameterRows: secretDefinitions.length === 0 && canCollapseRows(rows),
  };
}
...
function useRunDialogParameters({
  ...
  const { parameterDefinitions, declaredParametersOf, secretDefinitions } = useDeclaredParameters({
    scenarioIds,
    allScenarios,
    agents,
    targetAgentIds,
...
function useParameterRowActions({
  ...
  const editParameterLine = useCallback(
    ...
    [setParameterLine, setParameterLineTyped, setParameterRows, setParameterError],
  );
...
function buildCustomizeRunChips({
  ...
  if (!fields.showParams && !planFields.showCompare && hasParameterDefinitions) {
    chips.push({
      key: "params",
