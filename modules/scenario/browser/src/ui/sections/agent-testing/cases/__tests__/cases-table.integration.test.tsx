  ...
  describe("given the suite Refunds is open", () => {
    ...
    it("offers Edit suite between New scenario and Run suite, which opens the suite editor", async () => {
      ...
      const labels = Array.from(header?.querySelectorAll("button") ?? [])
        ...
        .filter((label) => !!label);
      expect(labels).toEqual(["New scenario", "Edit suite", "Open recent run", "Run suite"]);
    ...
    it("shows no chips row and counts the scenarios alone when nothing is declared", () => {
      ...
      expect(screen.queryByTestId("suite-declarations-row")).not.toBeInTheDocument();
      expect(screen.getByTestId("cases-panel-count-line")).toHaveTextContent("1 scenario");
    });
    ...
    it("lists the fields and the evaluators under the name, and opens the editor from a chip", async () => {
      ...
      expect(within(fields).getByTestId("suite-field-chip-golden_sql")).toHaveTextContent(
        "golden_sql",
      );
      expect(within(fields).getByTestId("suite-field-chip-attempts")).toHaveTextContent("attempts");
      ...
      expect(within(pill).getByTestId("evaluator-pill-required-att_1")).toBeInTheDocument();
      // expected_output reads nothing yet, so the pill carries the alert.
      expect(pill).toHaveAttribute("data-missing", "true");
      expect(within(pill).getByTestId("evaluator-pill-alert-att_1")).toBeInTheDocument();
    ...
    it("shows the Fields group alone when the suite declares no evaluator", () => {
      ...
      expect(screen.queryByTestId("suite-evaluators-group")).not.toBeInTheDocument();
      expect(screen.getByTestId("cases-panel-count-line")).toHaveTextContent(
        "1 scenario · 1 field",
      );
    });
