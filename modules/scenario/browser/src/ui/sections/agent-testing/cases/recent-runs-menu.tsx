...
function RecentRunRow({ run, onOpen }: { run: RecentRun; onOpen: (run: RecentRun) => void }) {
  const now = useNow();
    ...
    >
      <HStack gap={2} width="full">
        ...
        {run.isRunning ? (
          ...
          <Text fontSize="11px" fontWeight="semibold" color={passRateColor(run.passRate)}>
...
export function RecentRunsMenu(props: RecentRunsMenuProps) {
  ...
  return (
    <Menu.Root open={isOpen} onOpenChange={({ open }: { open: boolean }) => setIsOpen(open)}>
  ...
  return (
    ...
    >
      <Menu.TriggerItem value="open-recent-runs">
        <MenuActionLabel action="openRecentRuns">{OPEN_RECENT_RUNS_LABEL}</MenuActionLabel>
      </Menu.TriggerItem>
      <Menu.Content minWidth="240px" data-testid="recent-runs-submenu-list">
        <RecentRunsList period={period} scenarioIds={scenarioIds} hasRun isOpen={isOpen} />
      </Menu.Content>
    </Menu.Root>
  );
}
