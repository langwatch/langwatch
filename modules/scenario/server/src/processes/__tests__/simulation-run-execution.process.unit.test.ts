  ...
  buildProcessManager<SimulationProcessingEvent>({
    ...
    applier: simulationRunExecutionPM(
      ...
      TestSimulationService.create({
        ...
        recordEvaluations: () => Promise.reject(new Error("unused in evolve tests")),
      }),
    ),
  }).config,
...
describe("simulationRunExecution process (runtime-built definition)", () => {
  ...
  describe("when a run finishes owing its evaluator results", () => {
    ...
    it.each([
      ...
      ["an empty attachment list", { evaluators: { ...EVALUATORS, attachments: [] } }],
      [
        "its own evaluations",
        {
          results: {
    ...
    it("keeps the persisted view and state free of the attachment mappings", () => {
      ...
      expect(view.evaluators).toEqual([{ evaluatorId: "eval-1", required: true }]);
