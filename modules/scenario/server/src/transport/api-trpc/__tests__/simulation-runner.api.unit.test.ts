 ...
 *
 * Early validation on the `scenarios.run` transport.
 *
 * Every refusal here happens BEFORE anything is queued, which is what gives
 * the run dialog instant feedback instead of a job that fails halfway
 * through. The router is mounted on a bare tRPC root with an identity policy,
 * so what a failure looks like on the wire is the process's business: the
 * handled-error middleware maps a `HandledError` onto its HTTP status, and
 * that mapping is pinned where the middleware lives.
 *
...
// suite stays a unit test of the transport's own decisions.
...
describe("simulation runner run procedure", () => {
  const defaultInput = {
    projectId: "proj_123",
    scenarioId: "scen_123",
  ...
  describe("given all validation passes", () => {
    ...
    describe("when run is called without explicit setId", () => {
      ...
      it("records the models the prefetch resolved on the queued run", async () => {
        ...
        expect(queued.metadata.langwatch.resolvedSimulatorModel).toBe("openai/gpt-5-mini");
        expect(queued.metadata.langwatch.resolvedJudgeModel).toBe("openai/gpt-5");
      });
    ...
    describe("when the scenario declares parameters", () => {
      ...
      // The refusal is a HandledError carrying its own code and 422 status.
      // Mounted here on a bare root there is no handled-error middleware to
      // translate it, so the code is what this level can assert; the mapping
      // onto the wire belongs to the process that installs that middleware.
