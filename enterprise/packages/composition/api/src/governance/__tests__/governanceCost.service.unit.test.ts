...
): GovernanceCostProjectScope {
  return {
    findIdsByOrganization: vi.fn().mockResolvedValue(organizationProjectIds),
  } as unknown as GovernanceCostProjectScope;
}
...
function createService(deps: {
  ...
  ocsfEvents?: AppGovernanceOcsfEventsAdapter | undefined;
  gatewaySpend?: GovernanceGatewaySpendClickHouseRepository | undefined;
  projects?: GovernanceCostProjectScope;
}) {
...
describe("GovernanceCostService.summary", () => {
  ...
  describe("given metered spend recorded in the gateway ledger", () => {
    ...
    it("reads the ledger across every project tenant of the organization", async () => {
      ...
      expect(result.gateway.amountUsd).toBe(7);
      expect(projects.findIdsByOrganization).toHaveBeenCalledWith("org-1");
