...
/**
 * What one domain's chip says, in one place. Two surfaces read the same domain
 * state — the access panel and single sign-on setup — and they must not
 * disagree: a domain that says "Record missing" on one screen and "Proved" on
 * the other tells somebody their sign-in is fine while the evidence behind it
 * has gone.
 */
 ...
 * purpose — it is still routing, and a chip saying "Proved" would be
...
export function domainProofChipFor({
  ...
  if (proved && proofState === "WAVERING") {
    return {
      ...
      tone: "warning",
      title: graceEndsAtMs
        ? `We can't find your record right now. Nothing has changed yet — republish it before ${format(graceEndsAtMs, "d MMMM yyyy 'at' HH:mm")} and nothing will.`
  ...
  if (proved) {
    return {
      label: "Proved",
      tone: "good",
      title: "We found the record. This domain routes sign-ins to your identity provider.",
    };
  }
