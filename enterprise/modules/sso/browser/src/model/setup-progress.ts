...
/**
 * Where the journey actually is, as five answers rather than five ticks. A
 * screen that only knows "done" and "not done" shows four identical cards and
 * leaves the reader to work out which one is theirs and why the last refuses a
 * press. Three separable facts per step: is it finished, can it be done yet,
 * and is it the one to do now.
 */

/**
 * How one step is drawn: `done` stays finished, `current` is the one to do
 * now, `blocked` cannot be done yet and the note says what it waits for,
 * `todo` is later and unremarkable.
 */
...
export interface SetupProgress {
  ...
  /** Whether somebody has said who the connection admits. */
...
export function setupProgressFor({
  ...
  // step but the last is always available: a test sign-in names the connection
  // directly, and break glass is somebody an administrator names at will.
  const order: [keyof SetupProgress, boolean][] = [
    ["domain", domainProved],
  ...
  return {
    ...
    breakGlass: stateOf({ key: "breakGlass", done: breakGlassInPlace, current }),
...
/**
 * Turning it on is the only step with prerequisites, and the aggregate refuses
 * it until all four hold. Naming them turns a button that does nothing into a
...
function outstandingFor({
  ...
  return [
    ...
    // The only one that is a DECISION rather than a thing that had to happen.
