...
export function textOf(activity: Activity): string {
...
export function closeTurn(state: TurnAccumulator): void {
  if (state.open) {
 ...
 * conversations.
 */
export function applyMessage(params: { state: TurnAccumulator; message: ReadableMessage }): void {
...
export function toolNameOf(value: ToolCallValue): string {
  return (
 ...
 * so stands alone.
 */
export function callIdOf(params: { value: ToolCallValue; activityId: string }): string {
...
export function extractToolCallTrace(activity: Activity): ToolCallTrace | null {
  ...
  const value = (CopilotTranscriptGroupingService.asObject(activity.value) ?? {}) as ToolCallValue;
