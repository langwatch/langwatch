...
function turnSpan(params: {
  ...
  return {
    ...
    endTimeUnixNano: ConversationTraceAssemblyService.msToNano(Math.max(turn.endMs, turn.startMs)),
    attributes: attrs,
    status: { code: 1 },
  };
}
...
function toolSpan(params: {
  ...
  return {
    ...
    endTimeUnixNano: ConversationTraceAssemblyService.msToNano(Math.max(call.endMs, call.startMs)),
    attributes: attrs,
    status: { code: 1 },
  };
}
