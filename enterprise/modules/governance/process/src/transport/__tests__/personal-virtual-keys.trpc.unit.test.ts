...
function mount() {
  ...
  const app = createApiFixture<GovernanceRestApi>({
    listPersonalVirtualKeys: async (input, by) => {
      calls.push({ input, by });
      return [];
    },
    issuePersonalVirtualKey: async (input, by) => {
      calls.push({ input, by });
      return minted;
    },
    revokePersonalVirtualKey: async (input, by) => {
      calls.push({ input, by });
    },
  });
...
describe("the personalVirtualKeys tRPC namespace", () => {
  ...
  it("lists without an organization permission, leaving membership to the application", async () => {
    ...
    expect(calls).toEqual([
      { input: { organizationId: "org_1", targetUserId: "user_2" }, by: { id: "user_1" } },
    ]);
  });
