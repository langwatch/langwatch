...
function repositoryOver(rows: unknown[]) {
  ...
  return {
    client,
    repo: ClickHouseGatewaySpendRepository.create(async () => client as never),
  };
}
...
const READS = [
  ...
  [
    "by model",
    (repo: ClickHouseGatewaySpendRepository, input: ReadInput) => repo.sumWindowByModel(input),
  ],
  [
    "by virtual key",
...
describe("ClickHouseGatewaySpendRepository", () => {
  describe.each(READS)("when reading the ledger %s", (_label, read) => {
    ...
    it("applies the window to the collapsed request, not to its versions", async () => {
      ...
      expect(call.query_params.fromMs).toBe(Date.parse("2026-08-01T00:00:00.000Z"));
      expect(call.query_params).not.toHaveProperty("prefilterFromMs");
      // Inclusive of the last day: midnight AFTER it, exclusive.
      expect(call.query_params.toMs).toBe(Date.parse("2026-08-08T00:00:00.000Z"));
    });
