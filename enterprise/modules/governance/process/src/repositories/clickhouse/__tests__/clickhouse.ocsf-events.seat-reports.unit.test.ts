...
function repositoryOver(rows: unknown[]) {
  ...
  const repository = ClickHouseOcsfEventsRepository.create(async () => client as never);
  return { client, repository };
}

describe("ClickHouseOcsfEventsRepository.findLatestSeatReports", () => {
  describe("when a pool's newest report is read", () => {
    it("decodes the counts and the classifying facts off the payload", async () => {
      ...
      await expect(
        repository.findLatestSeatReports({ tenantId: "gov-1", actionName: SEAT_REPORT_ACTION }),
      ).resolves.toEqual([
        {
          sourceId: "is-1",
          skuPartNumber: "AGENT_SEAT_USL",
    ...
    it("scopes the read to the tenant and to seat reports alone", async () => {
      ...
      await repository.findLatestSeatReports({ tenantId: "gov-1", actionName: SEAT_REPORT_ACTION });
  ...
  describe("when one pool's payload cannot be read", () => {
    it("steps over it and keeps the rest of the licence list", async () => {
      ...
      expect(rows.map((row: { skuPartNumber: string }) => row.skuPartNumber)).toEqual([
        "AGENT_SEAT_USL",
      ]);
    });
  });
