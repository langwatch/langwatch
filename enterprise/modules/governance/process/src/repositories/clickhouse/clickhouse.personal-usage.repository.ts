...
/**
 * ClickHousePersonalUsageRepository — the queries behind the /me usage
 ...
 *
 * The public methods take the portable `PersonalUsageWindow` (`{startMs,
 * endMs}`) `PersonalUsageReader` declares, converting once to the Temporal
 * `Instant` window every query below is written against — the composition
 * package used to do this conversion in a separate wrapper adapter; folded
 * in here since the module now owns both halves.
 */
import type {
...
/** The window every query below is written against. */
type InternalWindow = {
...
function toInternalWindow(window: PersonalUsageWindow): InternalWindow {
  return {
    start: Temporal.Instant.fromEpochMilliseconds(window.startMs),
    end: Temporal.Instant.fromEpochMilliseconds(window.endMs),
  };
}
