...
describe("given an OpenAI Admin cost source", () => {
  ...
  describe("when the provider refuses the key part-way through a window", () => {
     ...
     * completion, leave the source looking healthy, and hide the one failure
     * an admin can actually act on. The sibling adapter for another provider
    ...
    it("fails the run rather than keeping what it read", async () => {
      fetchMock
        .mockResolvedValueOnce(jsonResponse(page({ nextPage: "page_2", hasMore: true })))
        .mockResolvedValueOnce(new Response("private upstream payload", { status: 401 }));
    ...
    it("does not quote the provider's reply back to the customer", async () => {
      fetchMock.mockResolvedValue(new Response("private upstream payload", { status: 403 }));
  ...
  describe("when the provider faults part-way through a window", () => {
     ...
     * rejection on every retry, so resuming AT the rejected page would park
     * the position on a page nothing will ever read past. The pages already
     * read still come back -- they are restated on the next run, which
     * re-asks for the same window -- but the position does not move onto the
     * rejection.
     */
  ...
  describe("when the provider reports a day's spend", () => {
    ...
    it("keeps every digit of a sub-cent figure through to the record", async () => {
      fetchMock.mockResolvedValue(
        jsonResponse(
          page({
            results: [costRow({ amount: { value: 0.0000001234, currency: "usd" } })],
          }),
        ),
      );
    ...
    it("sends the four dimensions the report is keyed on, daily, at the page ceiling", async () => {
      ...
      expect(url.searchParams.get("start_time")).toBe(String(Date.parse(CONFIG.startingAt) / 1000));
    });
  });
  ...
  describe("when the provider corrects a day it already reported", () => {
    ...
    it("starts the next run behind its own watermark", async () => {
      ...
      const secondRunStart = Number(requestedUrl(1).searchParams.get("start_time"));
  ...
  describe("when the provider refuses to group a window by API key", () => {
    ...
    it("re-reads the same window without that dimension", async () => {
      ...
      expect(requestedUrl(1).searchParams.getAll("group_by[]")).toEqual(["user_id"]);
      expect(result.events).toHaveLength(1);
    ...
    it("keeps asking without the key for the rest of the window it fell back on", async () => {
      ...
      fetchMock
        ...
        .mockResolvedValue(jsonResponse(page({ results: [costRow(belowFloorRow)] })));
      ...
      expect(requestedUrl(2).searchParams.getAll("group_by[]")).toEqual(["user_id"]);
  ...
  describe("when the key-grouping upgrade crosses a drain boundary", () => {
    ...
    it("skips the lookback so the keyed window does not overlap the user-only window", async () => {
      ...
      const run2 = await puller.runOnce({ ...RUN_OPTIONS, cursor: run1.cursor! }, CONFIG);
      ...
      expect(requestedUrl(2).searchParams.getAll("group_by[]")).toContain("api_key_id");
  ...
  describe("when a run cannot finish", () => {
    ...
    it("holds the cursor still and reports the failure", async () => {
      ...
      const result = await makePuller().runOnce({ ...RUN_OPTIONS, cursor: "prior-cursor" }, CONFIG);
    ...
    it("keeps the pages it already read when the deadline passes", async () => {
      fetchMock.mockResolvedValue(jsonResponse(page({ nextPage: "page_2", hasMore: true })));
    ...
    it("does not walk the window backwards when run after run runs out of time", async () => {
      ...
      expect(starts).toEqual([BUCKET_START_ISO, BUCKET_START_ISO, BUCKET_START_ISO]);
    });
    ...
    it("refuses a page claiming more data with no token to reach it", async () => {
      fetchMock.mockResolvedValue(jsonResponse(page({ hasMore: true, nextPage: null })));

      await expect(makePuller().runOnce(RUN_OPTIONS, CONFIG)).rejects.toThrow(
        /has_more with no next_page/,
      );
    });
    ...
    it("discards a page token minted under a different backfill start", async () => {
      ...
      await makePuller().runOnce({ ...RUN_OPTIONS, cursor: stale }, CONFIG);
  ...
  describe("when the row is not one the ledger can hold", () => {
    ...
    it("fails a run on a rejected key without writing the key into the reason", async () => {
      ...
      // A refused key leaves as a refusal rather than an error count: the
      // same answer arrives on every page, so there is no window to resume
      // and nothing to gain by keeping part of one.
      await expect(makePuller().runOnce(RUN_OPTIONS, CONFIG)).rejects.toMatchObject({
        retryable: false,
        // The reason is logged and shown on the source, so it is read by
        // people who were never given the credential -- and the provider's
        // own reply echoes a fragment of the key, so the body is drained and
        // never quoted. "sk-a" is the opening of both the real key and that
        // echo.
        message: expect.not.stringContaining(RUN_OPTIONS.credentials.token),
        customerMessage: expect.not.stringContaining(RUN_OPTIONS.credentials.token),
      });
    });

    it("fails a run with no admin key rather than reporting an empty organization", async () => {
      const result = await makePuller().runOnce({ cursor: null }, CONFIG);
