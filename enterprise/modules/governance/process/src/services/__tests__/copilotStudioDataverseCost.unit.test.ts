...
describe("the Azure cost read inside the Dataverse source", () => {
  ...
  describe("when the source names a subscription", () => {
    ...
    it("signs in for Azure Resource Manager, not for the environment", async () => {
      ...
      expect(scopes.some((body) => body.includes(encodeURIComponent(ENVIRONMENT_URL)))).toBe(true);
    });
  ...
  describe("when Microsoft asks the cost read to retry later", () => {
    ...
    it("records no cost, no error, and holds the window", async () => {
      costReplies = [{ status: 429, body: { error: "Too many requests. Please retry." } }];
  ...
  describe("when the stored position predates the cost read", () => {
    ...
    it("reads it as it always was and starts pricing from scratch", async () => {
      ...
      const transcriptCall = capturedCalls.find((c) => c.url.includes("/conversationtranscripts"));
      expect(decodeURIComponent(transcriptCall?.url ?? "")).toContain("2026-08-20T10:00:00.000Z");
      // Six is the AI subset of a forty-four row bill: the fixture's only
      // AI meter category is Foundry Models, and the other thirty-eight rows
      // are the subscription's networking and storage.
      expect(costEvents(result.events)).toHaveLength(6);
  ...
  describe("when reading the conversations fails after the cost was read", () => {
    ...
    it("resumes the transcript window from where the failed run started", async () => {
      ...
      const cursor = JSON.parse(String(result.cursor));
      expect(cursor.costPricedThroughDay).toBe(new Date().toISOString().slice(0, 10));
      expect(costEvents(result.events).length).toBeGreaterThan(0);
    });
  });
});
...
describe("the billing identity beside the bot's", () => {
  const signInCalls = () =>
    capturedCalls.filter((call) => call.url.includes("login.microsoftonline.com"));
  const armScope = encodeURIComponent("https://management.azure.com/.default");

  describe("when the source carries both identities", () => {
  ...
  describe("when the source carries only the bot's identity", () => {
