...
<<<<<<< HEAD:enterprise/modules/governance/server/src/services/databricks-genie-puller.service.ts
=======
  GENIE_FREE_USAGE_SKU_MARKER,
  mergeWarehouseCost,
>>>>>>> origin/main:platform/app/ee/governance/services/pullers/databricksGenie.puller.ts
...
<<<<<<< HEAD:enterprise/modules/governance/server/src/services/databricks-genie-puller.service.ts
   ...
   * the whole walk for no benefit, since the token outlives any single run.
   */
  private static async resolveWorkspaceToken(params: {
    credentials: Record<string, string> | undefined;
    workspaceUrl: string;
    signal?: AbortSignal;
    http: GovernanceHttpClient;
...
>>>>>>> origin/main:platform/app/ee/governance/services/pullers/databricksGenie.puller.ts
   ...
   * nothing.
   */
  private static warehouseCostParameters(chunk: {
    fromMs: number;
    toMs: number;
  }): { name: string; value: string; type: string }[] {
    return [
   ...
   * worst combination to leave in: too rare to notice, permanent when it lands.
   */
  private static unpricedFloor(chunk: { fromMs: number }): number {
    return chunk.fromMs - 1;
  }
   ...
   * from an outcome alone, so each question records how wide it was.
   */
  private static warehouseCostObserved({
    adapter,
    warehouseId,
    chunk,
  }: {
...
<<<<<<< HEAD:enterprise/modules/governance/server/src/services/databricks-genie-puller.service.ts
  ...
  private static toEpochMs(value: number): number {
    return value < EPOCH_SECONDS_CEILING ? value * 1000 : value;
  }

  private static startOfHourMs(ms: number): number {
    return Math.floor(ms / ONE_HOUR_MS) * ONE_HOUR_MS;
  }

  private static endOfHourMs(ms: number): number {
    return Math.ceil(ms / ONE_HOUR_MS) * ONE_HOUR_MS;
  }
   ...
   */
  private static earliest(a: number | null, b: number | null): number | null {
    if (a === null) return b;
    if (b === null) return a;
    return Math.min(a, b);
  }
   ...
   */
  private static genieIdentityFromScimUser(
   ...
   * and still lossless.
   */
  private static conversationWalkPlan({
    conversations,
    resumeConversationId,
  }: {
    conversations: PagedRead<z.infer<typeof conversationSchema>>;
   ...
   */
  private static withWarehouseCost({
    events,
    costByStatementId,
    costEnabled,
    watermarkMs,
  ...
  private static encode(cursor: GenieCursor): string {
