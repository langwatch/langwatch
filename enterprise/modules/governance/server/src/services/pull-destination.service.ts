...
/**
 * Where a pull is allowed to send a credential. This runs when a source is SAVED, which is
 * deliberately the only place it runs: the rejection has to reach whoever is making the change,
...
export class PullDestinationService {
  ...
  /**
   * Whether a URL is a Databricks workspace origin we may attach a token to. This is an egress
   * restriction, not a formatting check: the decrypted workspace token goes out as
   * `Authorization: Bearer` to whatever host this string names.
   */
  ...
  /**
   * The allowed suffixes as the customer reads them: "a, b or c". Built from the list the check
   * uses, so a new cloud cannot be added to the rule and left out of what the admin is told to
   * type instead.
   */
