...
export function assistantContentOf(
...
export function rootAttributesOf(
...
export function optionalRootAttributes(
  event: NormalizedPullEvent,
  frame: GenieMessageFrame,
): OtlpJsonAttr[] {
  const attachments = frame.payload.attachments ?? [];
...
export function queryAttachmentsOf(attachments: GenieAttachment[]): GenieAttachment[] {
  return attachments.filter((attachment) => typeof attachment.query?.query === "string");
}

export function queryStepSpan(
  ...
  if (typeof rowCount === "number") {
    stepAttrs.push(ConversationTraceAssemblyService.intAttr({ key: "row_count", value: rowCount }));
  }
