...
export function hasBatchGap(batches: number[]): boolean {
  if (batches.length < 2) {
    return false;
  }
 ...
 */
export function rememberBotFacts(params: { bot: BotFacts; extra: Record<string, unknown> }): void {
...
export function bucketRowsByConversation(
 ...
 */
export function byBatchThenArrival(a: IndexedRow, b: IndexedRow): number {
 ...
 */
export function isActivityObject(value: unknown): value is Activity {
  return !!value && typeof value === "object" && !Array.isArray(value);
}

/** Every activity these rows hold, in row order. */
export function activitiesOf(rows: IndexedRow[]): Activity[] {
 ...
 */
export function timeOrderActivities(activities: Activity[]): Activity[] {
  ...
  return sortable.map((item) => (item.ms === null ? item.activity : dated[nextDated++]!.activity));
}

/** True when the conversation happened while someone was building the agent. */
export function isDesignModeConversation(activities: Activity[]): boolean {
...
export function conversationGroupOf(params: {
  key: string;
 ...
 * showing.
 */
export function groupTranscriptRows(events: NormalizedPullEvent[]): ConversationGroup[] {
