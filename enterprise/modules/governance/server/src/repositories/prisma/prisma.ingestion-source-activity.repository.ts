...
/**
 * PrismaActivityMonitorRepository - read-side queries powering governance.
 * admin dashboard.
 *
...
export class PrismaActivityMonitorRepository implements ActivityMonitorRepository {
   ...
   */
  private async tryTracedEventWindowCounts(
  ...
  private async tryLoggedEventWindowCounts(
    args: WindowCountArgs,
  ): Promise<WindowCountRow | undefined> {
    const result = await args.ch.query({
      query: `
  ...
  private async tryPulledEventWindowCounts(
    args: WindowCountArgs,
  ): Promise<WindowCountRow | undefined> {
    const result = await args.ch.query({
      query: `
  ...
  async sourceHealthMetrics(input: {
    ...
    const windows = await Promise.all([
      this.tryTracedEventWindowCounts(args),
      this.tryLoggedEventWindowCounts(args),
      this.tryPulledEventWindowCounts(args),
    ]);
