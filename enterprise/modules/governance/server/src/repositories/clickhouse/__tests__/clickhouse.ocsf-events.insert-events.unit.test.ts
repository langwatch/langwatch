...
function rowsFor({ count, tenantId = "gov-1" }: { count: number; tenantId?: string }): GovernanceOcsfEvent[] {
  return Array.from({ length: count }, (_, index) => ({
    ...
    eventTime: Temporal.Instant.from("2026-08-30T00:00:00.000Z"),
    actorUserId: "",
    actorEmail: "",
    actorEnduserId: "",
    actionName: "invoke",
...
describe("ClickHouseOcsfEventsRepository.insertEvents", () => {
  ...
  describe("when a chunk after the first is rejected", () => {
    it("surfaces the rejection to the caller", async () => {
      ...
      await expect(repository.insertEvents(rowsFor({ count: 1201 }))).rejects.toThrow(
  ...
  describe("given rows of two tenants", () => {
    it("refuses the page rather than writing part of it", async () => {
      ...
      await expect(repository.insertEvents(mixed)).rejects.toThrow(/same tenant/);
      expect(insert).not.toHaveBeenCalled();
    });
  });
});
