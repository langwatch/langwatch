...
const adapter = {
  ...
  runOnce: (...args: Parameters<typeof runOnce>) => runOnce(...args),
};

async function runIngestionPull(input: {
  sourceId: string;
  cursor: string | null;
  pulledUsage?: {
    recordPulledUsage: (input: Record<string, unknown>) => Promise<void>;
  };
}) {
  const source = await findUnique(input.sourceId);
  ...
  return createWorkerService({
    ...
    usageEnabled: async (organizationId) => isEnabled(organizationId),
...
describe("the pull effect's pulled-usage emit seam", () => {
  ...
  it("keeps non-bill usage attached to the replacement source", async () => {
    ...
    expect(recordPulledUsage.mock.calls[0]![0].ingestionSourceId).toBe("src_replacement");
