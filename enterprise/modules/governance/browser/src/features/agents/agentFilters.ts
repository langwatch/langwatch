...
export function coerceSource(value: string | null): SourceFilter {
  if (AGENT_SOURCES.some((source) => source === value)) return value as AgentSource;
  return ALL_SOURCES;
}
...
function compareAgents(sort: AgentSort, a: GovernanceAgentRow, b: GovernanceAgentRow): number {
  if (sort === "lastActive") return ascending(a.lastActiveMinutesAgo, b.lastActiveMinutesAgo);
  if (sort === "requests") return descending(a.requests30d, b.requests30d);
  return descending(a.costUsd30d, b.costUsd30d);
}

function matchesFilters(row: GovernanceAgentRow, { source, ownership }: AgentFilters): boolean {
