...
const lineFor = (lines: ReturnType<typeof summarizeAgentFleet>["health"], key: string) =>
...
describe("summarizeAgentFleet", () => {
  describe("given the sample agent rows", () => {
    ...
    it("counts every row, spells the unit out, and captions the recent arrivals", () => {
      ...
      expect(summary.fleet.caption).toBe("+1 in the last 30 days · registered over time");
    });
    ...
    it("carries one row per health state, each with its own count and tone", () => {
      expect(summary.health.map((line) => line.key)).toEqual(["responding", "idle", "erroring"]);
      expect(summary.health.map((line) => line.label)).toEqual(["responding", "idle", "erroring"]);
      expect(summary.health.map((line) => line.tone)).toEqual(["good", "neutral", "bad"]);
    ...
    it("ranks the three biggest spenders, biggest share first", () => {
      ...
      expect(summary.topSpenders.map((agent) => agent.shareLabel)).toEqual(["38%", "24%", "18%"]);
    });
  });
  ...
  describe("when an agent has registered and never run", () => {
    ...
    it("counts it in the fleet and in none of the three health states", () => {
      ...
      const healthTotal = summary.health.reduce((total, line) => total + line.count, 0);
  ...
  describe("when one agent's spend was never measured", () => {
    ...
    it("excludes it from the ranking and from the total the shares divide", () => {
      expect(summary.topSpenders.map((agent) => agent.name)).toEqual(["measured-a", "measured-b"]);
      ...
      expect(summary.topSpenders.map((agent) => agent.shareLabel)).toEqual(["75%", "25%"]);
    });
  });
