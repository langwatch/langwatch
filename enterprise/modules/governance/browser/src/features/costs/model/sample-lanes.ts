 ...
 */
export function sampleCostSummary(periods: string[]): GovernanceCostSummary {
  const lastPeriod = periods[periods.length - 1] ?? "";
  const series: GovernanceCostDay[] = periods.map((day, index) => {
    ...
    return {
      ...
      billedByCurrency: [{ currencyCode: "USD", amount: billedUsd, previousAmount: null }],
  ...
  return {
    ...
    providers: [
      ...
      provider,
      amountUsd: series.reduce((sum, day) => sum + (day.billedUsd ?? 0), 0) * share,
      cellsWithoutAmount: 0,
      currenciesWithoutUsdAmount: [],
    })),
    gateway: {
