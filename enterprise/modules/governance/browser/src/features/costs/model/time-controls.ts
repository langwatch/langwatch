...
export const TIME_INTERVALS: readonly {
  value: TimeInterval;
  label: string;
}[] = [
...
export type TimeFrame = "last_3_months" | "last_12_months" | "year_to_date" | "last_2_years";
 ...
 */
export const TIME_FRAMES: readonly {
  value: TimeFrame;
  label: string;
...
export function coerceInterval({
  ...
  const widestThatFits = [...TIME_INTERVALS]
    .reverse()
    .find((option) => !isIntervalCoarserThanFrame({ interval: option.value, frame, now }));
...
export function timeFrameLabel(frame: TimeFrame): string {
  return TIME_FRAMES.find((option) => option.value === frame)?.label ?? "Last 12 months";
}

/** The label a chip shows for the current interval. */
export function timeIntervalLabel(interval: TimeInterval): string {
  return TIME_INTERVALS.find((option) => option.value === interval)?.label ?? "Quarter";
}
