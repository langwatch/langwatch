 ...
 * `GovernanceCostDay` per day). Month, quarter and year are therefore
 ...
 */
export function aggregateBuckets(buckets: DailyBucket[], interval: TimeInterval): DailyBucket[] {
  const byBucket = new Map<string, Map<string, { label: string; value: number }>>();
...
>(series: readonly T[], interval: TimeInterval): T[] {
  ...
  for (const day of series) {
    ...
    byBucket.set(start, {
      ...
      billedCellsWithoutAmount: held.billedCellsWithoutAmount + day.billedCellsWithoutAmount,
      gatewayCellsWithoutAmount: held.gatewayCellsWithoutAmount + day.gatewayCellsWithoutAmount,
      billedRevisedAt: laterOf(held.billedRevisedAt, day.billedRevisedAt),
      billedByCurrency: addCurrencyLines(held.billedByCurrency, day.billedByCurrency),
      billedProvisional: held.billedProvisional || day.billedProvisional,
    });
  }
  return [...byBucket.values()].toSorted((a, b) => a.day.localeCompare(b.day));
}
    ...
    byCode.set(
      ...
      standing
        ? {
            ...
            previousAmount: addOrWithhold(standing.previousAmount, line.previousAmount),
          }
        : line,
    );
  }
  return [...byCode.values()].toSorted((a, b) => a.currencyCode.localeCompare(b.currencyCode));
}
 ...
 */
export function aggregateSeatCounts(buckets: DailyBucket[], interval: TimeInterval): DailyBucket[] {
  const byBucket = new Map<string, DailyBucket>();
  // Ascending, so each write leaves the latest period of its bucket standing.
  for (const bucket of [...buckets].toSorted((a, b) => a.day.localeCompare(b.day))) {
