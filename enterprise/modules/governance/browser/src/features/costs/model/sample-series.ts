 ...
 */
export function sampleRanked(labels: readonly string[], topValue: number): RankRow[] {
...
export function sampleDaily(
  ...
  const randomByLabel = new Map(labels.map((label) => [label, seededRandom(hashLabel(label))]));
  return days.map((day) => ({
    day,
...
export function sampleForecast({
  ...
  const runRate = new Map(
    labels.map((label) => {
      const values = tail.map((bucket) => bucket.points.find((p) => p.key === label)?.value ?? 0);
...
export function sampleSeatPools(
  ...
  return pools.map((skuPartNumber, index) => {
    ...
    const bought =
      Math.round((base * (1 + SEATS_ADDED_PER_RENEWAL * Math.max(0, renewals))) / 10) * 10;
    ...
    const ceiling = ASSIGNED_AT_RENEWAL + (ASSIGNED_BY_YEAR_END - ASSIGNED_AT_RENEWAL) * eased;
    ...
    const seatsAssigned = Math.min(bought, Math.round(bought * ceiling * (0.94 + random() * 0.06)));
