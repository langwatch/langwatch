...
import { type GovernanceAzureBillingNote } from "@langwatch/enterprise-governance-contract";
 ...
 */
export function laneTrendPct(points: readonly { value: number | null }[]): number | null {
  const stated = points.filter((point): point is { value: number } => point.value !== null);
  if (stated.length < 4) return null;
  const middle = Math.floor(stated.length / 2);
  const mean = (from: number, to: number) =>
    stated.slice(from, to).reduce((total, point) => total + point.value, 0) / (to - from);
...
function laneDigits(magnitude: number): string {
  if (magnitude >= CENTS_STOP_MATTERING_AT) return GROUPED_USD_WHOLE.format(magnitude);
 ...
 */
export function meteredRequestsWithoutAmountNote(count: number | undefined): string | null {
...
function currencyDigits(magnitude: number): string {
  if (magnitude >= CENTS_STOP_MATTERING_AT) return GROUPED_WHOLE.format(magnitude);
 ...
 */
export function azureBillingNoteSentence(note: GovernanceAzureBillingNote): string {
  switch (note) {
...
export function restatementNote({
  ...
  if (provisionalDays > 0) {
    clauses.push(
      `${provisionalDays} ${days(provisionalDays)} can still change while the provider settles ${
