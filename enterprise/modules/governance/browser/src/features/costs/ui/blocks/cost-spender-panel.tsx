...
export function spenderDisplayRows(rows: SpenderRow[]): SpenderDisplayRow[] {
  const scale = rows.reduce((max, row) => Math.max(max, Math.abs(row.amountUsd ?? 0)), 0);
  return rows.map((row) => ({
    ...
    widthPct: row.amountUsd === null || scale === 0 ? 0 : (Math.abs(row.amountUsd) / scale) * 100,
  }));
}
...
function SpenderName({ row }: { row: SpenderDisplayRow }) {
  ...
  return (
    <HStack flex="0 0 62%" gap={2} minWidth={0}>
      ...
      {row.provider !== "" && (
        <Badge size="xs" variant="subtle" colorPalette="gray" title="provider" flexShrink={0}>
          {row.provider}
        </Badge>
      )}
      {row.agentId !== "" && (
        <Badge size="xs" variant="subtle" colorPalette="gray" title="agent" flexShrink={0}>
          {row.agentId}
        </Badge>
      )}
    </HStack>
  ...
  return (
    <Box flex="1" height="14px" borderRadius="sm" backgroundColor="bg.muted" overflow="hidden">
      <Box
        ...
        backgroundColor={row.notNamed ? "border.emphasized" : getHexColorForString(row.label)}
      />
    </Box>
  );
}
...
export function CostSpenderError({ onRetry }: { onRetry: () => void }) {
  return (
    <Alert.Root status="error" data-testid="cost-spenders-error">
      <Alert.Indicator />
      <Alert.Content>
        <Alert.Title>Provider-reported spend by user could not be loaded</Alert.Title>
        <Alert.Description>
          Something went wrong reading which users the provider named for this spend. The totals
