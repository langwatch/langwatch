...
export function CostFilterBar({
  ...
  return (
    <FilterChipRow>
      <FilterChip icon={<Building2 size={12} />} label="Department" value={departmentLabel}>
        <MenuItem value={ALL_DEPARTMENTS} onClick={() => onDepartmentChange(ALL_DEPARTMENTS, null)}>
          All departments
        </MenuItem>
        {departments.map((d) => (
          <MenuItem key={d.id} value={d.id} onClick={() => onDepartmentChange(d.id, d.name)}>
            {d.name}
          </MenuItem>
        ))}
      </FilterChip>
        ...
        {TIME_INTERVALS.map((option) => {
          ...
          return (
            <MenuItem
              ...
              value={option.value}
              disabled={tooWide}
              onClick={tooWide ? undefined : () => onIntervalChange(option.value)}
            >
              {option.label}
            </MenuItem>
          );
      ...
      <Text width="full" fontSize="xs" color="fg.muted" data-testid="cost-bucket-note">
        Department filters only the department breakdown. Every chart is bucketed by{" "}
        {timeIntervalLabel(interval).toLowerCase()}.
      </Text>
    </FilterChipRow>
  );
}
