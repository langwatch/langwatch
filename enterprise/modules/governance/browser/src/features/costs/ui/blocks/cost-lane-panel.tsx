...
import { CHART_SEAT_FILL } from "../../model/chart-theme.ts";
import {
...
import { SampleMark } from "../elements/sample-mark.tsx";
 ...
 */
function SeatLaneWithoutCounts({ status }: { status: "awaiting_data" | "read_failed" }) {
 ...
 */
function SeatPools({ pools }: { pools: GovernanceSeatPool[] }) {
  return (
...
function SeatPoolRow({ pool }: { pool: GovernanceSeatPool }) {
  ...
  const filled = pool.seatsBought > 0 ? pool.seatsAssigned / pool.seatsBought : null;
  return (
    <VStack align="stretch" gap={1} width="full">
      <HStack gap={2} width="full">
        ...
        <Text fontSize="sm" color="fg.muted" fontVariantNumeric="tabular-nums" flexShrink={0}>
