...
import {
  ...
  type GovernanceCostProviderDayRow,
...
import { CHART_SEAT_CONTRACT_FILL, CHART_SEAT_FILL } from "../../model/chart-theme.ts";
import {
  azureBillingNoteSentence,
  laneTrendPct,
  meteredRequestsWithoutAmountNote,
} from "../../model/cost-lane-format.ts";
import { isRefusedRead, summaryAsRead } from "../../model/cost-sample-mode.ts";
import {
  ALL_DEPARTMENTS,
  aggregateBuckets,
  aggregateLine,
...
import { sampleCostSummary, sampleSpenderRows } from "../../model/sample-lanes.ts";
import {
  type DailyBucket,
  type RankRow,
  recentMonths,
...
import {
  ...
  CostSpenderList,
  type SpenderRow,
} from "../blocks/cost-spender-panel.tsx";
import { CostPanelEmpty, costPanelEmpty } from "../elements/cost-panel-empty.tsx";
...
} from "./cost-provider-day-panel.tsx";
 ...
 */
function ReadCeilingNotice({ frame, showSample }: { frame: TimeFrame; showSample: boolean }) {
...
const SAMPLE_MODELS = ["gpt-5-mini", "gpt-5", "claude-sonnet-5", "claude-haiku-4-5"];
const SAMPLE_PEOPLE = [
  "ada@acme.test",
  "grace@acme.test",
  "alan@acme.test",
 ...
 */
function AgentSharePanel({ sample, showSample }: { sample: SampleSeries; showSample: boolean }) {
  return (
...
function AgentRankPanel({ sample, showSample }: { sample: SampleSeries; showSample: boolean }) {
  return (
 ...
 */
function useSampleSeries(periods: string[], interval: TimeInterval, department: string) {
  return useMemo(() => {
    const departments =
      department === ALL_DEPARTMENTS
        ? [...SAMPLE_DEPARTMENTS]
