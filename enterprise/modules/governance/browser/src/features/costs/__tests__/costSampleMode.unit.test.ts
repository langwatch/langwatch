...
import { type SummaryForSampleDecision, summaryAsRead } from "../model/cost-sample-mode.ts";
...
function summary(overrides: Partial<SummaryForSampleDecision> = {}): SummaryForSampleDecision {
  return {
...
describe("reading the headline summary as a real-data read", () => {
  ...
  describe("given the screen is structurally unavailable", () => {
    it("answers as measured and empty rather than waiting forever", () => {
      expect(summaryAsRead(summary({ unavailableReason: "no_cost_store" }))).toEqual({ length: 0 });
    });
  });
      ...
      const read = summaryAsRead(
        summary({
          billed: {
            ...
            currencyTotals: [{ currencyCode: "USD", amount: null, cellsWithoutAmount: 4 }],
          },
        }),
      );
      expect(read?.length).toBeGreaterThan(0);
      ...
      const read = summaryAsRead(
        summary({
          billed: {
            ...
            cellsWithoutAmount: 0,
            currenciesWithoutUsdAmount: [],
            currencyTotals: [{ currencyCode: "EUR", amount: 40, cellsWithoutAmount: 0 }],
          },
        }),
      );
      expect(read?.length).toBeGreaterThan(0);
