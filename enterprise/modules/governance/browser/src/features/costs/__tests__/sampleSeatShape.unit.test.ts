...
import { sampleSeatPools, sampleSeats } from "../model/sample-series.ts";
...
describe("the invented seat counts", () => {
  ...
  it("changes the bought count only at the renewal month, and never downward", () => {
    ...
    for (const [index, day] of MONTHS.slice(1).entries()) {
      expect(boughtOn(day)).toBeGreaterThanOrEqual(boughtOn(MONTHS[index] ?? ""));
    }
  });
  ...
  it("drops assignment at the renewal and climbs back across the year", () => {
    ...
    const acrossTheYear = ["2026-01-01", "2026-03-01", "2026-06-01", "2026-09-01", "2026-12-01"];
  ...
  it("gives the lane card and the chart the same counts for the same month", () => {
    ...
    const valueOf = (key: string) => lastBar?.points.find((point) => point.key === key)?.value ?? 0;
    ...
    const laneBought = lane.pools.reduce((sum, pool) => sum + pool.seatsBought, 0);
    const laneAssigned = lane.pools.reduce((sum, pool) => sum + pool.seatsAssigned, 0);
