...
import { recentMonths, sampleForecast } from "../model/sample-series.ts";
...
describe("windowDaysForFrame", () => {
  describe("when the frame reaches past what the reads answer", () => {
    it("asks for the ceiling rather than a window the read would refuse", () => {
      ...
      // a screen.
      expect(windowDaysForFrame({ frame: "last_2_years" })).toBe(READ_WINDOW_DAY_CEILING);
...
describe("aggregateSeatCounts", () => {
  describe("when three months inside one quarter each report a count", () => {
    ...
    it("reports the last month's count, not the three added together", () => {
      const months = ["2026-07-01", "2026-08-01", "2026-09-01"].map((day, index) => ({
        day,
        points: [
...
describe("aggregateLaneSeries", () => {
  describe("when every day in the period holds a figure", () => {
    it("sums them onto the period's own start", () => {
      const folded = aggregateLaneSeries([laneDay("2026-07-04"), laneDay("2026-08-09")], "quarter");
  ...
  describe("when one day in the period was restated", () => {
    ...
    it("marks the period restated and keeps the most recent date", () => {
      const folded = aggregateLaneSeries(
        [
          laneDay("2026-07-04", {
            ...
            billedByCurrency: [{ currencyCode: "USD", amount: 100, previousAmount: 90 }],
          }),
          laneDay("2026-08-09", {
            billedRevisedAt: 9_000,
            billedByCurrency: [{ currencyCode: "USD", amount: 80, previousAmount: 70 }],
      ...
      // the whole 160.
      expect(
        folded[0]?.billedByCurrency.find((line) => line.currencyCode === "USD")?.previousAmount,
      ).toBe(160);
    ...
    it("withholds the prior figure, because a partial one reads as the whole", () => {
      const folded = aggregateLaneSeries(
        [
          ...
          laneDay("2026-07-09", {
            billedByCurrency: [{ currencyCode: "USD", amount: 50, previousAmount: null }],
          }),
        ],
        "month",
      );
...
describe("given a forecast whose window is part served and part projected", () => {
  ...
  it.each(["month", "quarter", "year"] as const)(
    "names a bucket the chart draws, folded to %s",
    (interval) => {
  ...
  it("carries a steady run rate forward rather than re-rolling the noise", () => {
    const values = forecast.projected.map((bucket) => bucket.points[0]?.value ?? 0);
    const measuredValues = forecast.measured.map((bucket) => bucket.points[0]?.value ?? 0);
