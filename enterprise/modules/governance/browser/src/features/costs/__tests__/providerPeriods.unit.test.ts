...
const seriesIn = (bucket: { points: { key: string; value: number }[] }) =>
...
describe("the provider breakdown fold", () => {
  ...
  it("gives each period a separate figure per provider, adding to each provider's window", () => {
    ...
    const acrossWindow = (provider: string) =>
      folded.reduce((total, bucket) => total + (seriesIn(bucket)[provider] ?? 0), 0);
  ...
  it("folds the untotalled chart to one period per quarter as well", () => {
    ...
    expect(folded[0]!.points).toEqual([{ key: "total", label: "Spend", value: 131 }]);
  });
  ...
  it("folds days inside one quarter into a single period, provider by provider", () => {
    ...
    const folded = aggregateBuckets(providerDayBuckets(insideOneQuarter), "quarter");
  ...
  it("spans a period from its first billed day to its last, not from the calendar", () => {
    ...
    const q1 = periods.find(
      (period) => period.provider === "openai_admin" && period.period === "2026-01-01",
    );
  ...
  it("covers every day the period holds and no day outside it", () => {
    ...
    const covered = inWindow(q1.fromDay, q1.toDay).filter((row) => row.provider === "openai_admin");
    ...
    const q2 = periods.find(
      (period) => period.provider === "anthropic_admin" && period.period === "2026-04-01",
    )!;
  ...
  it("marks the period holding a withheld day as withheld, naming the provider", () => {
    ...
    expect(folded.map((bucket) => bucket.day)).toEqual(["2026-01-01", "2026-04-01"]);
  ...
  it("marks a period short when a day holds a dollar figure beside a bill in a currency with none", () => {
    ...
    expect(rowIsShort({ amountUsd: 50, currenciesWithoutUsdAmount: [] })).toBe(false);
    expect(rowIsShort({ amountUsd: null, currenciesWithoutUsdAmount: [] })).toBe(true);
  ...
  it("lists a provider short in two ways once, with its currencies", () => {
    ...
    expect(partialProviderNotes(costTotalBuckets(shortBothWays, "quarter"))).toEqual([
      "Anthropic (EUR, GBP)",
    ]);
  });
  ...
  it("counts a withheld day as no money rather than guessing at one", () => {
    ...
    const folded = aggregateBuckets(providerDayBuckets(withWithheld), "quarter");
