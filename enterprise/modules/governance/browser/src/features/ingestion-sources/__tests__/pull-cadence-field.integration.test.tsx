 ...
 */
const frequencyPicker = () => screen.getByRole("combobox", { name: "Frequency" });
...
describe("given the Cadence section of the composer", () => {
  describe("when a pull-mode source is being composed", () => {
    ...
    it("does not restate the picked schedule underneath the picker", () => {
      ...
      expect(screen.queryByText(/Leave as-is to use the recommended schedule/)).toBeNull();
    });
    ...
    it("puts the explanation behind an (i) beside the Cadence heading", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/How often we check this source for new activity/)).toBeTruthy();
      });
    });
