...
const timeOfDay = (input: { hour: number; minute: number }): string =>
...
function CadenceSummary({ effectiveCron }: { effectiveCron: string }) {
  ...
  return (
    <Text fontSize="xs" color="fg.muted" data-testid="cadence-summary">
      {summaryParts ? summarizePullCadence(summaryParts) : `Runs on the schedule ${effectiveCron}`}
    </Text>
  );
}
    ...
    <VStack align="stretch" gap={3}>
      <HStack justify="space-between" gap={2}>
        ...
        <HStack gap={0} alignItems="center">
          ...
          <FieldInfoTooltip description={CADENCE_EXPLANATION} testId="cadence-field-info" />
        </HStack>
        <HStack gap={2}>
