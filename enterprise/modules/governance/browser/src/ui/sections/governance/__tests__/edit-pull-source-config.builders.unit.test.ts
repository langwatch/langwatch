...
describe("buildAnthropicAdminPullConfig on the edit path", () => {
  ...
  it("still requires the secret on the create path", () => {
    ...
    expect(
      buildAnthropicAdminPullConfig(composer({ credentialsToken: "", report: "usage" })),
    ).toBeNull();
  });
});
...
describe("buildEditSubmission", () => {
  ...
  /**
   * `resolvePullConfig` names the offending field when a pull config will not
   * build. That is the behaviour under test's own reporting channel, not a
   * dependency of it, so the builder is handed a recorder and the refusal
   * tests read the toast off it.
   */
  function submit(overrides: Partial<Parameters<typeof buildEditSubmission>[0]> = {}) {
    const toaster = recordingGovernanceToaster();
    return {
      toaster,
      submission: buildEditSubmission({
  ...
  it("given a pull field the adapter cannot parse, refuses the save", () => {
    ...
    const { submission, toaster } = submit({
      parserConfig: {
        credentialsToken: "",
        report: "usage",
        startingAt: "not-a-date",
    ...
    // A refusal the admin is not told about is a Save button that does
    // nothing, so the toast is part of the behaviour, not decoration.
  ...
  it("sends the cadence for a pull source", () => {
    expect(submit({ pullSchedule: " 0 */2 * * * " }).submission?.pullSchedule).toBe("0 */2 * * *");
  });
  ...
  it("resolves a cleared cadence to the recommended schedule, never null", () => {
    ...
    // Saving null instead would disable the source — see the cadence suite,
    // which is what makes this more than a shape assertion.
    expect(submit({ pullSchedule: "   " }).submission?.pullSchedule).toBe(
  ...
  it("keeps the saved cadence and the rebuilt config agreeing on the default", () => {
    ...
    const { submission } = submit({ pullSchedule: "" });

    expect(submission?.pullSchedule).toBe(submission?.parserConfig.schedule);
  });
  ...
  it("omits the cadence key entirely for a push source", () => {
    ...
    expect(submission).not.toHaveProperty("pullSchedule");
  });
  ...
  it("trims the name and nulls an emptied description", () => {
    ...
    expect(submission?.name).toBe("Renamed");
    expect(submission?.description).toBeNull();
  });
  ...
  it("carries a freshly typed secret so the stored one is replaced", () => {
    ...
    expect(submission?.parserConfig.credentials).toEqual({
      token: "sk-ant-admin-new",
    });
  });
});
