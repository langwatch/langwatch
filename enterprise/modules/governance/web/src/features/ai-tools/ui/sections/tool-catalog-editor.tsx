...
export function ToolCatalogEditor({ organizationId, onAddTile, onEditTile }: Props) {
  ...
  const setEnabledMutation = api.aiTools.setEnabled.useMutation({
    ...
    onError: (err) => showErrorToast({ error: err, fallbackTitle: "Couldn't update tool" }),
  });
  ...
  const removeMutation = api.aiTools.remove.useMutation({
    onSuccess: () => {
      void utils.aiTools.adminList.invalidate({ organizationId });
      void utils.aiTools.list.invalidate({ organizationId });
      toaster.create({ title: "Tool deleted", type: "success" });
      setPendingDelete(null);
    },
    onError: (err) => showErrorToast({ error: err, fallbackTitle: "Couldn't delete tool" }),
  });
