 ...
 *
 * A WEB-SIDE COPY of `enterprise/packages/composition/api/src/governance/logic/
 * observedDepartments.ts`. That file is framework-free by its own admission but
 * lives in the api composition root, which a web package may not value-import
 * (frontend-boundary.unit.test.ts) — so this is the framework-free module both
 * sides import, ported here rather than reached across the boundary. The
 * server copy should eventually re-export this one; until then keep the two in
 * sync by hand.
 *
...
export interface PersonDepartmentFacts {
  ...
  /** ISO string when erased, `null` otherwise — the wire's own shape. */
