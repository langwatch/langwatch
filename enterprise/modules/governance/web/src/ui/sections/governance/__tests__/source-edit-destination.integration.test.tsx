 ...
 *
 * `trace-destination-field.integration.test.tsx` covers what the field says.
 ...
 * `buildEditSubmission` call behind it. A harness that re-implemented
...
describe("given a Genie source that already lands in Analytics", () => {
  describe("when the admin opens it for editing", () => {
    ...
    it("shows Analytics as the destination it is stored with, and says routed history stays", async () => {
      ...
      renderDrawer();
<<<<<<< HEAD:enterprise/modules/governance/web/src/ui/sections/governance/__tests__/source-edit-destination.integration.test.tsx
...
>>>>>>> origin/main:platform/app/ee/governance/dashboard/pages/__tests__/sourceEditDestination.integration.test.tsx
    });
  });
...
>>>>>>> origin/main:platform/app/ee/governance/dashboard/pages/__tests__/sourceEditDestination.integration.test.tsx
  ...
  describe("when they save without touching the destination", () => {
    ...
    it("sends no destination key, rather than echoing the stored one", async () => {
      ...
      // A save that went through raises nothing at the admin; a refusal the
      // builder toasted would show up here instead of as a silent no-op.
