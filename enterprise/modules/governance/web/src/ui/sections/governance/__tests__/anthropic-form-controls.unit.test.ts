...
describe("Anthropic composer controls", () => {
  describe("the report field", () => {
    ...
    it("opens on the cost report rather than on nothing", () => {
      ...
      expect(selectOptionsFor("report", {}).map((o) => o.value)).toContain("cost");
    });
  ...
  describe("the bucket width field", () => {
    ...
    it("says daily because the adapter's own default is daily", () => {
      ...
      // itself.
      const bucketWidth = anthropicAdminPullConfigSchema.shape.bucketWidth.parse(undefined);

      expect(bucketWidth).toBe("1d");
      expect(selectOptionsFor("bucketWidth", { report: "usage" })[0]?.label).toBe("1d — daily");
    });
    ...
    it("keeps offering the width an existing usage source is read at", () => {
      ...
      expect(selectOptionsFor("bucketWidth", reconciled).map((o) => o.value)).toEqual(["", "1h"]);
    ...
    it("keeps nothing on a cost source, whose width was never in effect", () => {
      ...
      expect(options.map((o) => o.value)).toEqual([""]);
      expect(
        buildAnthropicAdminPullConfig(composerWith({ report: "cost", bucketWidth: "1h" })),
      ).toBeNull();
    });
    ...
    it("keeps the width while the report field sits empty between picks", () => {
      ...
      expect(selectOptionsFor("bucketWidth", cleared).map((o) => o.value)).toEqual(["", "1h"]);
    ...
    it("still refuses to save from the state it is holding the width through", () => {
      ...
      expect(
        buildAnthropicAdminPullConfig(composerWith({ report: "", bucketWidth: "1h" })),
      ).toBeNull();
    });
    ...
    it("refuses a width no version of the form ever offered", () => {
      ...
      expect(
        buildAnthropicAdminPullConfig(composerWith({ report: "usage", bucketWidth: "2h" })),
      ).toBeNull();
    });
