...
const renderFields = (
  ...
  renderWithGovernanceHost(
    <Harness sourceType={sourceType} advancedExtras={advancedExtras} invalidKeys={invalidKeys} />,
...
describe("given the Genie source-specific fields", () => {
  describe("when the form first renders", () => {
    ...
    it("keeps the token, space IDs, warehouse ID, and bill-line switch hidden until Advanced expands", async () => {
      ...
      expect(screen.queryByText(/Also record Genie's own bill line/)).toBeNull();
      ...
      expect(screen.getByText(/Also record Genie's own bill line/)).toBeTruthy();
    });
  });
...
describe("given settings that belong in Advanced but are not parser fields", () => {
  ...
  describe("when the group is closed again", () => {
    ...
    it("gives back what was there, because the value never lived inside it", async () => {
      ...
      renderWithGovernanceHost(<Owner />, { host: fakeGovernanceHost() });

      const user = userEvent.setup();
      await user.click(screen.getByText("Advanced"));
      await user.type(await screen.findByLabelText("parent-held setting"), "0 * * * *");
      ...
      const reopened = await screen.findByLabelText<HTMLInputElement>("parent-held setting");
...
describe("given a source whose fields carry an explanation", () => {
  describe("when the form renders", () => {
    ...
    it("puts the explanation behind an (i) rather than under the input", () => {
      ...
      expect(screen.getByTestId("parser-field-info-environmentUrl")).toBeTruthy();
    });
    ...
    it("shows the explanation once the (i) is opened", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/From Power Platform admin centre/)).toBeTruthy();
      });
    });
    ...
    it("gives a field with nothing to explain no (i) at all", () => {
      ...
      expect(screen.queryByTestId("parser-field-info-credentialsTenantId")).toBeNull();
    });
  });
});
...
describe("given the Copilot Studio licence switch", () => {
  describe("when Advanced is expanded", () => {
    ...
    it("shows the switch already on, holding nothing", async () => {
      ...
      const toggle = (await screen.findByTestId("parser-switch-readSeats")) as HTMLInputElement;
...
describe("given the Copilot Studio create form", () => {
  describe("when the form first renders", () => {
    ...
    it("stands in labelled groups with the billing choice last", () => {
      ...
      const headings = screen
        ...
        .map((el) => el.textContent);
      expect(headings).toEqual(["Connection", "Cost", "Conversation access", "Billing"]);
    });
    ...
    it("keeps the prepaid declaration folded away under Advanced", async () => {
      ...
      expect(screen.queryByText("This Copilot runs on prepaid message packs")).toBeNull();
      await expandAdvanced();
      expect(screen.getByText("This Copilot runs on prepaid message packs")).toBeTruthy();
    });
    ...
    it("starts with the one-app switch on and no billing fields in sight", () => {
      ...
      expect(screen.queryByText(/Billing app registration client ID/)).toBeNull();
      expect(screen.queryByText(/Billing app registration client secret/)).toBeNull();
    });
  });
  ...
  describe("when the admin turns the one-app switch off", () => {
    ...
    it("reveals the billing credential fields", async () => {
      ...
      await user.click(screen.getByTestId("parser-switch-azureBillingUsesSameApp"));

      await waitFor(() => {
        expect(screen.getByText(/Billing app registration client ID/)).toBeTruthy();
      });
      expect(screen.getByText(/Billing app registration client secret/)).toBeTruthy();
    });
  });
});
...
describe("given the Copilot Studio directory switch", () => {
  describe("when the form first renders", () => {
    ...
    it("shows the switch already on, holding nothing", () => {
      ...
      const toggle = screen.getByTestId("parser-switch-readDirectory") as HTMLInputElement;
...
describe("given a save refused because a required field is empty", () => {
  describe("when the field is one the form is showing", () => {
    ...
    it("marks the control itself and says what to do about it", () => {
      ...
      expect(screen.getByTestId("parser-field-error-environmentUrl")).toBeTruthy();
      expect(
        screen.getByLabelText("Power Platform environment URL").getAttribute("aria-invalid"),
      ).toBe("true");
    });
    ...
    it("leaves every other field alone", () => {
      ...
      expect(screen.queryByTestId("parser-field-error-credentialsTenantId")).toBeNull();
      expect(
        screen.getByLabelText("Microsoft Entra tenant ID").getAttribute("aria-invalid"),
      ).not.toBe("true");
    });
  });
  ...
  describe("when the field is inside the Advanced group", () => {
    ...
    it("opens the group so the marked field is on screen", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("parser-field-error-credentialsToken")).toBeTruthy();
      });
    });
  });
  ...
  describe("when nothing was refused", () => {
    ...
    it("marks nothing, so an untouched form does not open red", () => {
      ...
      expect(screen.queryByTestId("parser-field-error-environmentUrl")).toBeNull();
    });
  });
});
