...
/**
 * Empty-state install affordance for LangWatch CLI; renders copy-pasteable snippet
 * with auto-detected endpoint for self-hosted.
 */
export function InstallCliCard({
  ...
  const { isSaas, appBaseUrl: baseHost } = useGovernanceHost().deployment();
  ...
  // `langwatch login` (the login flow persists it for later commands).
  // `npm install` itself never talks to LangWatch, so needs neither.
