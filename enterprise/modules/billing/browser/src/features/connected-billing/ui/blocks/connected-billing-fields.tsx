...
type SetField = <K extends keyof BillingForm>(change: { key: K; value: BillingForm[K] }) => void;

const CURRENCIES = ["USD", "EUR"] as const;
const BANK_TRANSFERS: readonly BankTransferChoice[] = ["", "us_bank_transfer", "eu_bank_transfer"];

const currencyOf = (value: string): BillingForm["seatCurrency"] =>
  CURRENCIES.find((currency) => currency === value) ?? "USD";

const bankTransferOf = (value: string): BankTransferChoice =>
  BANK_TRANSFERS.find((choice) => choice === value) ?? "";

/** The contract an operator onboards or renews on. */
export function ConnectedBillingFields({
  form,
  onChange,
  showPaymentFields,
}: {
      ...
      <SimpleGrid columns={2} gap={3} width="full">
        <Field.Root>
          ...
          <Input
            ...
            onChange={(event) => set({ key: "termStartsAt", value: event.target.value })}
          />
        </Field.Root>
        <Field.Root>
          ...
          <Input
            ...
            onChange={(event) => set({ key: "termEndsAt", value: event.target.value })}
          />
        </Field.Root>
        <Field.Root>
          ...
          <Input
            ...
            min={1}
            value={form.seats}
            onChange={(event) => set({ key: "seats", value: event.target.value })}
          />
        </Field.Root>
        <Field.Root>
          ...
          <HStack>
            ...
            <NativeSelect.Root width="28">
              <NativeSelect.Field
                ...
                onChange={(event) =>
                  set({ key: "seatCurrency", value: currencyOf(event.target.value) })
                }
              >
        ...
        <Field.Root>
          ...
          />
          <Field.HelperText>Must match the commit agreed on the license.</Field.HelperText>
        </Field.Root>
      </SimpleGrid>
    ...
    <SimpleGrid columns={2} gap={3} width="full">
      <Field.Root>
        ...
        <Input
          ...
          onChange={(event) => set({ key: "billingEmail", value: event.target.value })}
        />
      </Field.Root>
      <Field.Root>
        ...
        <HStack>
          <NativeSelect.Root>
            <NativeSelect.Field
              ...
              onChange={(event) =>
                set({ key: "bankTransferType", value: bankTransferOf(event.target.value) })
              }
            >
              <option value="">Wire to LangWatch, marked paid by finance</option>
              <option value="us_bank_transfer">United States, in dollars</option>
          ...
          <Input
            ...
            onChange={(event) => set({ key: "bankTransferCountry", value: event.target.value })}
          />
        </HStack>
      </Field.Root>
    </SimpleGrid>
