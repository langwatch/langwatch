 ...
 */
export type RoutingPolicyDatabase = Pick<PrismaClient, "routingPolicy" | "$transaction">;
...
export class PrismaRoutingPolicyRepository extends RoutingPolicyRepository {
  ...
  async findAll(
    ...
    const scopePredicates = input.selectableForScope
      ? ancestorScopePredicates({
          organizationId: input.organizationId,
          scope: input.selectableForScope,
  ...
  async count(input: { organizationId: string }): Promise<number> {
    return this.database.routingPolicy.count({ where: { organizationId: input.organizationId } });
  }
}

function ancestorScopePredicates({
  organizationId,
  scope,
  projectTeamId,
}: {
...
}): Prisma.RoutingPolicyScopeWhereInput[] {
  ...
  if (scope.scopeType === "PROJECT") {
    if (projectTeamId) predicates.push({ scopeType: "TEAM", scopeId: projectTeamId });
