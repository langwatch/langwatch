...
/** One source, as both summaries read it: only its status tone matters. */
 ...
 * A count alone answers half the question: "Provisioning 2" says nothing about
 * whether either is working, and the tab is closed, so the colour is the only
 * channel left before it is opened. Worst-wins, like the chip above.
 *
 * `undefined` while the read is in flight, so the badge draws nothing rather
 * than a zero it has not confirmed.
 */
export function directoryConnectionsBadge(sources: DirectorySyncSource[] | undefined): {
  ...
  if (sources.length === 0) {
    return { count: 0, tone: "neutral", title: "No identity provider is connected yet." };
  }
