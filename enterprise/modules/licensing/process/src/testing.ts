 ...
 * to `VALID_LICENSE_KEY` below (signed over them), so they are not the
 * catalogue's PRO plan and cannot be read from it.
 */
...
export { canonicalPemKey, mangledPemPastes } from "./fixtures/pem-pastes.fixture.ts";
export {
  TEST_PRIVATE_KEY,
  TEST_PUBLIC_KEY,
  WRONG_PRIVATE_KEY,
  WRONG_PUBLIC_KEY,
} from "./fixtures/license-keys.fixture.ts";
...
/**
 * Enterprise license whose term ended (expired 2020-01-01), genuinely
 * signed — proves a lapse keeps the seats and capabilities it sold rather
 * than dissolving to the open-source baseline (expired-license-enforcement.feature).
 */
...
/**
 * Forged license: claims a past end date it was never signed with, so the
 * signature fails. Comparing `expiresAt` instead would call it expired and
 * meter the seats it invents — the verdict follows the signature, not the date.
 */
