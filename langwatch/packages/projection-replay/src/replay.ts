...
/**
 * Process a batch: mark → pause → drain → cutoff → load → replay → unmark+unpause.
 */
async function replayBatch({
  ...
  // 2. PAUSE — prevent new dispatch for this projection
  emitPhase("pause");
  ...
  // 3. DRAIN — wait for active (in-flight) jobs only
  emitPhase("drain");
  ...
  if (withCutoffKeys.length === 0) {
    emitPhase("unmark");
    // Unpause — no events to replay for this batch
    await unpauseProjection({
      redis,
      pipelineName: projection.pipelineName,
      projectionName,
  ...
  // 6. REPLAY — coalesce all events into shared state per projection key,
  //    then store once per unique key (no per-aggregate grouping needed).
