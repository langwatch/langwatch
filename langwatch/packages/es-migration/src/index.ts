...
type MigrationTarget = "simulations" | "batch-evaluations" | "traces" | "trace-evaluations" | "all";
...
function parseArgs(): CliOptions {
  ...
  const validTargets: MigrationTarget[] = ["simulations", "batch-evaluations", "traces", "trace-evaluations", "all"];
...
function printUsage(): void {
  process.stderr.write(
    `Usage: es-migration [target] [options]\n` +
      ...
      `  traces             Migrate traces (spans → trace_summaries + stored_spans)\n` +
      `  trace-evaluations  Migrate trace evaluations → evaluation_runs\n` +
      ...
      `  CH_BATCH_SIZE            ClickHouse insert buffer size (default: 500)\n` +
...
async function main(): Promise<void> {
  ...
  const allTargets: Exclude<MigrationTarget, "all">[] = [
    "simulations", "batch-evaluations", "traces", "trace-evaluations",
  ];
      ...
      if (target === "simulations") {
        ...
        const service =
          target === "batch-evaluations"
            ...
            : app.simulationService; // traces/trace-evaluations use direct-write, service not used
