 ...
 *
 * A registry entry with `emitting: false` is a declared contract whose
...
const KNOWN_TYPES = new Set(WEBHOOK_EVENT_TYPES.map((t) => t.type));
const KNOWN_FAMILIES = new Set(WEBHOOK_EVENT_TYPES.map((t) => t.family));
