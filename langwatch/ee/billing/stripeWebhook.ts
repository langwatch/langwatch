...
/**
 * Processes an already-verified Stripe webhook event.
 *
 * The caller is responsible for signature verification
 * (`stripe.webhooks.constructEvent`) and for delivering the HTTP response.
 * Keeping this function transport-agnostic prevents the Fetch/Node request
 * mismatch that previously caused every webhook to be rejected (#446).
 */
      ...
      switch (event.type) {
        case "checkout.session.completed": {
          ...
          const selectedCurrency =
            selectedCurrencyRaw && VALID_CURRENCIES.has(selectedCurrencyRaw)
              ? selectedCurrencyRaw
              : null;
