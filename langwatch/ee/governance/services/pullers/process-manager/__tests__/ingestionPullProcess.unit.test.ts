...
import { INGESTION_PULL_EVENT_TYPES } from "~/server/event-sourcing/pipelines/ingestion-pull-processing";
...
function boot(at: number) {
  return ingestionPullProcessDefinition.evolve({
    previousState: ingestionPullProcessDefinition.initialState,
    input: { kind: "event", event: configured(at) },
  });
}
...
describe("ingestionPullProcessDefinition", () => {
  it("rejects invalid schedules before they enter process state", () => {
    expect(() =>
      ingestionPullProcessDefinition.evolve({
        previousState: ingestionPullProcessDefinition.initialState,
        input: {
          kind: "event",
          event: {
            ...configured(Date.parse("2026-07-17T10:00:00Z")),
            payload: {
              sourceId: "source-1",
              cron: "",
              cursor: null,
  ...
  it("runs one catch-up slot and schedules strictly after the handling time", () => {
    ...
    const result = ingestionPullProcessDefinition.evolve({
      previousState: state,
      input: {
        kind: "wake",
        scheduledFor: Date.parse("2026-07-17T10:15:00Z"),
        handledAt: Date.parse("2026-07-17T13:02:00Z"),
      },
    });
  ...
  it("does not overlap a healthy in-flight run", () => {
    ...
    const result = ingestionPullProcessDefinition.evolve({
      previousState: state,
      input: { kind: "wake", scheduledFor: 2_000, handledAt: 2_000 },
    });
    expect(result.intents).toEqual([]);
  ...
  it("advances the durable cursor only from a completion event", () => {
    ...
    const result = ingestionPullProcessDefinition.evolve({
      ...
      input: {
        kind: "event",
        event: {
          ...configured(Date.parse("2026-07-17T10:01:00Z")),
  ...
  it("clears its wake when disabled and late outcomes cannot re-enable it", () => {
    ...
    const disabled = ingestionPullProcessDefinition.evolve({
    ...
    const lateCompletion = ingestionPullProcessDefinition.evolve({
      previousState: disabled.state,
      input: {
        kind: "event",
        event: {
          ...configured(Date.parse("2026-07-17T10:02:00Z")),
          eventType: INGESTION_PULL_EVENT_TYPES.RUN_COMPLETED,
          payload: {
            sourceId: "source-1",
            cron: null,
