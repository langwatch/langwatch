 ...
 *     (Path B).
 *
 * A missing row resolves to the hardcoded Stage-A defaults below, so the policy
 * is purely additive: disabling a path is an explicit admin choice and a fresh
 * org behaves exactly as before this table existed. The CLI mirror of these
...
/**
 * Hardcoded Stage-A defaults. claude/codex/gemini/opencode allow both paths;
 * cursor is GUI-only so Path B (a terminal OTLP env) never reaches the agent
 * panel — it allows the gateway path only.
 */
