...
function buildStore({
  vk = { id: "vk-1", organizationId: "org-1", principalUserId: null },
  project = {
    id: "project-1",
    teamId: "team-1",
  ...
  const prisma = {
    virtualKey: { findUnique: vi.fn().mockResolvedValue(vk) },
  ...
  return { store, prisma, ledger, appendChangeEvent };
}
...
describe("GatewayBudgetDebitsAppendStore", () => {
  ...
  describe("given the budget lookup fails", () => {
    it("reports the failure rather than silently charging nothing", async () => {
      const store = new GatewayBudgetDebitsAppendStore({
        ...
        budgetRepository: {
          applicableForRequest: vi.fn().mockRejectedValue(new Error("PG down")),
        } as never,
        budgetCHRepository: new FakeLedger() as never,
        changeEvents: { append: vi.fn() } as never,
      });
  ...
  describe("given nothing can be charged", () => {
    ...
    it("refuses to charge a key belonging to another organization", async () => {
      const { store, ledger } = buildStore({
        vk: { id: "vk-1", organizationId: "org-other", principalUserId: null },
      });
  ...
  describe("given a key scoped to a principal", () => {
    it("resolves principal-scoped budgets from the key's owner", async () => {
      const applicableForRequest = vi.fn().mockResolvedValue([budget()]);
      const store = new GatewayBudgetDebitsAppendStore({
        prisma: {
          virtualKey: {
            findUnique: vi.fn().mockResolvedValue({
        ...
        budgetCHRepository: new FakeLedger() as never,
        changeEvents: { append: vi.fn().mockResolvedValue({ revision: 1n }) } as never,
      });
