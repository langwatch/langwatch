...
export default async function execute() {
  ...
  for (const project of projects) {
    ...
    // Create queries for different event types
    const createEventTypeQuery = (eventType: string) => ({
      size: 0,
      query: {
        bool: {
          should: [
    ...
    // Create multi-search body for all event types
    const msearchBody = [
      ...
      // Message snapshots
      { index: SCENARIO_EVENTS_INDEX.alias },
      createEventTypeQuery(ScenarioEventType.MESSAGE_SNAPSHOT),
      // Run started events
      { index: SCENARIO_EVENTS_INDEX.alias },
      createEventTypeQuery(ScenarioEventType.RUN_STARTED),
      // Run finished events
      { index: SCENARIO_EVENTS_INDEX.alias },
      createEventTypeQuery(ScenarioEventType.RUN_FINISHED),
    ];
