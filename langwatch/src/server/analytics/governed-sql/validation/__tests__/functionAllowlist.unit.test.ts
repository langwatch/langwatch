 ...
 * the functions a governed question needs are admitted — asserted by submitting
...
function validate(sql: string): GovernedSqlValidation {
...
function functionRefusalMessage(result: GovernedSqlValidation): string {
  if (result.ok) return "";
  return (
...
describe("the governed-SQL function allowlist", () => {
  describe("given the SQL the governed questions are written in", () => {
  ...
  describe("given a function that reaches outside the governed data", () => {
    it.each([
  ...
  describe("given an aggregate combinator", () => {
    ...
    /**
     * The suffixes that are absent. No governed view exposes an
