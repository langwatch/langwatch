...
/**
 * Invite procedures: creating, approving, accepting, and listing invites.
 *
 * Merged into the organization router in `root.ts` via `mergeRouters`, so every
 * procedure keeps its existing `organization.*` path. Split out of
 * organization.ts only to separate the two concerns at the file level.
 */
