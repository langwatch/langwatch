...
describe("RBAC Permission System", () => {
  describe("Permission Hierarchy Logic", () => {
    it("should allow direct permission match", () => {
      ...
      expect(hasPermissionWithHierarchy(permissions, "experiments:view")).toBe(true);
    ...
    it("should allow manage permissions to include view permissions", () => {
      ...
      expect(hasPermissionWithHierarchy(permissions, "experiments:view")).toBe(true);
      expect(hasPermissionWithHierarchy(permissions, "experiments:manage")).toBe(true);
    });
    ...
    it("should not allow view permissions to include manage permissions", () => {
      ...
      expect(hasPermissionWithHierarchy(permissions, "experiments:view")).toBe(true);
      expect(hasPermissionWithHierarchy(permissions, "experiments:manage")).toBe(false);
    });

    it("should not allow unrelated permissions", () => {
      const permissions = ["datasets:view"];
      
      expect(hasPermissionWithHierarchy(permissions, "experiments:view")).toBe(false);
      expect(hasPermissionWithHierarchy(permissions, "experiments:manage")).toBe(false);
    });

    it("should work with different resource types", () => {
      const permissions = ["analytics:manage", "guardrails:manage", "prompts:manage"];
      
      expect(hasPermissionWithHierarchy(permissions, "analytics:view")).toBe(true);
      expect(hasPermissionWithHierarchy(permissions, "guardrails:view")).toBe(true);
  ...
  describe("Team Role Permissions", () => {
    it("should allow ADMIN to access all experiment permissions", () => {
      expect(teamRoleHasPermission(TeamUserRole.ADMIN, "experiments:view")).toBe(true);
      expect(teamRoleHasPermission(TeamUserRole.ADMIN, "experiments:manage")).toBe(true);
    });

    it("should allow MEMBER to access all experiment permissions", () => {
      expect(teamRoleHasPermission(TeamUserRole.MEMBER, "experiments:view")).toBe(true);
      expect(teamRoleHasPermission(TeamUserRole.MEMBER, "experiments:manage")).toBe(true);
    });

    it("should allow VIEWER to access only experiment view permissions", () => {
      expect(teamRoleHasPermission(TeamUserRole.VIEWER, "experiments:view")).toBe(true);
      expect(teamRoleHasPermission(TeamUserRole.VIEWER, "experiments:manage")).toBe(false);
    });
    ...
    it("should allow MEMBER to access limited project permissions", () => {
      ...
      expect(teamRoleHasPermission(TeamUserRole.MEMBER, "project:create")).toBe(false);
  ...
  describe("Organization Role Permissions", () => {
    it("should allow ORGANIZATION_ADMIN to access organization permissions", () => {
      expect(organizationRoleHasPermission(OrganizationUserRole.ADMIN, "organization:view")).toBe(true);
      expect(organizationRoleHasPermission(OrganizationUserRole.ADMIN, "organization:manage")).toBe(true);
    });

    it("should allow ORGANIZATION_MEMBER to access organization view permissions", () => {
      expect(organizationRoleHasPermission(OrganizationUserRole.MEMBER, "organization:view")).toBe(true);
      expect(organizationRoleHasPermission(OrganizationUserRole.MEMBER, "organization:manage")).toBe(false);
    });

    it("should allow ORGANIZATION_EXTERNAL to access organization view permissions", () => {
      expect(organizationRoleHasPermission(OrganizationUserRole.EXTERNAL, "organization:view")).toBe(true);
      expect(organizationRoleHasPermission(OrganizationUserRole.EXTERNAL, "organization:manage")).toBe(false);
    });
  });
  ...
  describe("Custom Role Scenarios", () => {
    it("should simulate custom role with only manage permission", () => {
      ...
      expect(hasPermissionWithHierarchy(customPermissions, "experiments:view")).toBe(true);
      expect(hasPermissionWithHierarchy(customPermissions, "experiments:manage")).toBe(true);
    });
    ...
    it("should simulate custom role with only view permission", () => {
      ...
      expect(hasPermissionWithHierarchy(customPermissions, "experiments:view")).toBe(true);
      expect(hasPermissionWithHierarchy(customPermissions, "experiments:manage")).toBe(false);
    });

    it("should simulate custom role with mixed permissions", () => {
      // Simulate a custom role with mixed permissions
      const customPermissions = ["experiments:manage", "datasets:view", "analytics:manage"];
      ...
      expect(hasPermissionWithHierarchy(customPermissions, "datasets:manage")).toBe(false);
      expect(hasPermissionWithHierarchy(customPermissions, "analytics:view")).toBe(true);
      expect(hasPermissionWithHierarchy(customPermissions, "analytics:manage")).toBe(true);
    });
  });
  ...
  describe("Edge Cases", () => {
    it("should handle permissions with different action types", () => {
      ...
      expect(hasPermissionWithHierarchy(permissions, "messages:share")).toBe(true);
      expect(hasPermissionWithHierarchy(permissions, "messages:view")).toBe(false);
