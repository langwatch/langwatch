...
import { seedIngestionPullers } from "@ee/governance/services/pullers/ingestionPullScheduler";
...
export const start = async (
  ...
  return new Promise<Workers | undefined>((resolve, reject) => {
    ...
    // Pull-mode ingestion sources run on the event-sourcing global queue
    // (registered in pipelineRegistry). Seed one pull per active source;
    // each pull self-re-arms at its cron next-fire time. No BullMQ worker.
    void seedIngestionPullers();
