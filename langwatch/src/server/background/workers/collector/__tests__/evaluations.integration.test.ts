...
describe("scheduleEvaluations - thread idle timeout", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    // Clean up all monitors for this project before each test
  ...
  it("uses different timeout values correctly", async () => {
    ...
    testMonitorIds.push(monitor1.id);
    ...
    const monitor2 = await prisma.monitor.create({
      data: {
        ...
        checkType: "presidio/pii_detection", // Same type as monitor1
        executionMode: EvaluationExecutionMode.ON_MESSAGE,
        enabled: true,
        preconditions: [],
        parameters: {},
    ...
    testMonitorIds.push(monitor2.id);
