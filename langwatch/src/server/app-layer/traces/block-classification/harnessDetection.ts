 ...
 *   - Codex: scope `codex_cli_rs` (the Rust CLI native spans).
 *
 ...
 * (`codex_cli_rs`); they are inlined here to keep this a dependency-free pure
 * predicate. Gateway-path origin markers are not stamped today (README lists
...
export function detectCodingAgentHarness({
  ...
  if (scope === CODEX_RUST_SCOPE) return "codex";
