...
vi.mock("~/server/app-layer/triggers/graph-trigger-evaluation.service", () => ({
  evaluateGraphTrigger: evaluateGraphTriggerMock,
}));

vi.mock("~/server/app-layer/triggers/graph-trigger-heartbeat", () => ({
...
vi.mock("~/server/app-layer/triggers/webhook-delivery.service", () => ({
  WebhookDeliveryService: {
    create: vi.fn(() => ({ record: vi.fn().mockResolvedValue(undefined) })),
  },
}));

vi.mock("~/server/app-layer/triggers/dispatch/emailCaps", () => ({
