 ...
 *
 * Stream B (the raw-token fast path, ADR-071) gated on the asynchronous
 * conversation projection:
 *
 *     const conv = await conversations.getById({ id, projectId, userId });
 *     if (!conv) return 404;
 *
 * That projection lands after the event is queued. The browser subscribes to
 * `/fast` the instant it reads the `x-langy-turn-id` response header, which is
 * long before that. So on the FIRST turn of any conversation the projection did not
 ...
 *
 * NOT the worker handoff: it contains execution secrets and is an implementation
 * detail of process-outbox/liveness dispatch. This purpose-built record contains
 * only the turn actor's identity and ages out with the stream window.
 *
...
export class LangyTurnAccessStore {
   ...
   * fall back to the operational projection's visibility rule (which also covers
   * shared conversations and turns older than the window). It only ever confirms
   * the turn's own actor.
   */
  async isTurnActor({
    projectId,
    conversationId,
