 ...
 *   - the tRPC router (`api/routers/langy.ts`) spreads it into the turn-start
 *     input shape;
 ...
 * the turn-start input never declared the field, and a non-strict Zod object
 ...
 *
 * A schema both ends import cannot drift like that: if the router stops accepting
...
/**
 * The fields a turn carries beyond its messages. Spread into the turn-start
 * input shape. The array is capped: an unbounded context array is an unbounded
 * prompt.
 */
