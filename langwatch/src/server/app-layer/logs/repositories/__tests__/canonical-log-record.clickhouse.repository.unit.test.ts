...
import { CLAUDE_CODE_KIND_ATTR } from "~/server/app-layer/traces/claude-code-log-marking";
...
describe("CanonicalLogRecordClickHouseRepository", () => {
  ...
  describe("given a log record the Claude Code span fold consumes", () => {
    ...
    it("keeps a kind-marked claude row on the caller's retention", async () => {
      const insert = insertSpy();
      const repository = new CanonicalLogRecordClickHouseRepository(
        async () => ({ insert }) as never,
      );
      const marked = record();
      marked.attributeKeys = [...marked.attributeKeys, CLAUDE_CODE_KIND_ATTR];

      await repository.ensureLogRecord(marked, 49);

      // These rows once expired after a day, because a reactor copied them
      // into spans. That converter is gone: the marked rows ARE the Terminal
      // transcript's content, so shortening their retention would delete the
      // conversation while its trace is still live.
