...
export interface ProjectionHeuristicResult extends Partial<TraceProjectionData> {
  computedMetadata?: Record<string, string>;
}

export function detectInputOutput(spans: readonly SpanEvent[]): ProjectionHeuristicResult {
...
export function computeTimingMetrics(spans: readonly SpanEvent[]): ProjectionHeuristicResult {
  if (spans.length === 0) {
    return { totalDurationMs: 0, timeToFirstTokenMs: null, timeToLastTokenMs: null };
  }

  const startTimes = spans.map(span => span.data.startTimeUnixMs);
  const endTimes = spans.map(span => span.data.endTimeUnixMs);
  ...
  return {
    ...
    timeToFirstTokenMs: firstTokenEvent ? Math.max(0, firstTokenEvent.timeUnixMs - earliestStart) : null,
    timeToLastTokenMs: lastTokenEvent ? Math.max(0, lastTokenEvent.timeUnixMs - earliestStart) : null,
  };
}

export function deriveStatusFlags(spans: readonly SpanEvent[]): ProjectionHeuristicResult {
  const hasError = spans.some(span => span.data.status.code === 2);
  const hasOk = spans.some(span => span.data.status.code === 1);
...
export function deriveModelInfo(spans: readonly SpanEvent[]): ProjectionHeuristicResult {
...
export function aggregateTokenMetrics(spans: readonly SpanEvent[]): ProjectionHeuristicResult {
...
export function deriveTokensPerSecond(
  ...
  if (
    ...
    metrics.totalCompletionTokenCount === undefined ||
...
function findFirstAttribute(spans: readonly SpanEvent[], keys: readonly string[]): string | null {
  for (const span of spans) {
    for (const key of keys) {
      const value = span.data.attributes?.[key];
...
function collectUniqueAttributeValues(spans: readonly SpanEvent[], keys: readonly string[]): string[] {
...
function sumAttributes(spans: readonly SpanEvent[], keys: readonly string[]): number | undefined {
  let total: number | undefined;
  for (const span of spans) {
    for (const key of keys) {
      const raw = span.data.attributes?.[key];
...
function findEventByRegex(spans: readonly SpanEvent[], regex: RegExp) {
  for (const span of spans) {
    const event = span.data.events.find(e => regex.test(e.name));
    if (event) {
      return event;
    }
  }
