...
describe("EventStream - Edge Cases", () => {
  describe("when events have identical timestamps", () => {
    it("preserves original order for tied timestamps", () => {
      const events: Event[] = [
        { aggregateId: "1", timestamp: 1000, type: "A", data: { order: 1 } },
        { aggregateId: "1", timestamp: 1000, type: "B", data: { order: 2 } },
        { aggregateId: "1", timestamp: 1000, type: "C", data: { order: 3 } },
      ];
      ...
      expect(orderedEvents[0]!.type).toBe("A");
      expect(orderedEvents[1]!.type).toBe("B");
      expect(orderedEvents[2]!.type).toBe("C");
    });

    it("does not lose events with identical timestamps", () => {
      const events: Event[] = [
        { aggregateId: "1", timestamp: 1000, type: "A", data: {} },
        { aggregateId: "1", timestamp: 1000, type: "B", data: {} },
        { aggregateId: "1", timestamp: 1000, type: "C", data: {} },
      ];
  ...
  describe("when aggregateId is numeric", () => {
    it("handles MAX_SAFE_INTEGER", () => {
      ...
      const events: Event<number>[] = [
        { aggregateId: maxInt, timestamp: 1000, type: "A", data: {} },
      ];
    ...
    it("handles negative numbers", () => {
      const events: Event<number>[] = [
        { aggregateId: -42, timestamp: 1000, type: "A", data: {} },
      ];
    ...
    it("handles zero", () => {
      const events: Event<number>[] = [
        { aggregateId: 0, timestamp: 1000, type: "A", data: {} },
      ];
  ...
  describe("when aggregateId is an object", () => {
    it("converts to [object Object] without custom toString", () => {
      ...
      const events: Event<typeof objId>[] = [
        { aggregateId: objId, timestamp: 1000, type: "A", data: {} },
      ];
  ...
  describe("when custom comparator is provided", () => {
    it("applies custom ordering", () => {
      const events: Event[] = [
        { aggregateId: "1", timestamp: 1000, type: "A", data: { priority: 3 } },
        { aggregateId: "1", timestamp: 2000, type: "B", data: { priority: 1 } },
        { aggregateId: "1", timestamp: 3000, type: "C", data: { priority: 2 } },
      ];
      ...
      expect(ordered[0]!.type).toBe("B"); // priority 1
      expect(ordered[1]!.type).toBe("C"); // priority 2
      expect(ordered[2]!.type).toBe("A"); // priority 3
    });
