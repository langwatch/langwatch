  ...
  implements SpanIngestionWriteRepository
{
  tracer = getLangWatchTracer(
    "langwatch.span-ingestion.write.repository.clickhouse",
  );
  ...
  private transformSpanData(jobData: SpanIngestionWriteJob): ClickHouseSpan {
    const { tenantId, spanData } = jobData;
