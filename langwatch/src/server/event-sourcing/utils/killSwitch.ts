...
/**
 * Resolves an opt-IN flag for one tenant, defaulting to OFF.
 *
 * The mirror of `isComponentDisabled`: that one guards a capability that is on
 * until an operator stops it, so every uncertainty (no flag service, a lookup
 * error) resolves to "keep working". This one guards a capability that must
 * stay dark until an operator confirms it is safe to enable, so the same
 * uncertainties resolve the other way — to `false`.
 *
 * Uses `projectId` for the same reason the kill switch does: rule matching runs
 * against `{ projectId, organizationId }`, and `distinctId` is only the
 * percentage-rollout bucketing key. In this platform the event-sourcing tenant
...
export async function isTenantFeatureEnabled({
  ...
  try {
    ...
    if (logger) {
      logger.warn(
        { flagKey, tenantId, error },
        "Error checking feature flag, defaulting to disabled",
      );
    }
    return false;
  }
