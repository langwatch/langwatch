...
import { experimentRunItemAppendStore } from "../repositories/experimentRunItemAppendStore";
...
/**
 * MapProjection definition for experiment run result storage.
 *
 * Extracts the pure mapping logic from ExperimentRunResultStorageHandler.
 * Maps TargetResultEvent and EvaluatorResultEvent to ClickHouse records for storage
 * in the experiment_run_items table.
 */
export const experimentRunResultStorageMapProjection: MapProjectionDefinition<
  ClickHouseExperimentRunResultRecord,
  ExperimentRunResultEvent
> = {
  ...
  map(event: ExperimentRunResultEvent): ClickHouseExperimentRunResultRecord | null {
