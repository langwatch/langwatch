...
> & {
  ...
  const repository = new ExperimentRunStateRepositoryClickHouse(
    async () => clickHouseClient,
  );
  const experimentRunStateFoldStore =
    createExperimentRunStateFoldStore(repository);
  const experimentRunItemAppendStore = createExperimentRunItemAppendStore(
    async () => clickHouseClient,
  );
...
async function waitForExperimentRunState(
  ...
  const projection = await pipeline.service
    .getProjectionByName("experimentRunState", compositeKey, { tenantId })
    .catch(() => null);
  const data = projection?.data as ExperimentRunStateData | undefined;
...
describe.skip("Experiment Run Processing Pipeline", () => {
