...
> & {
  ...
  const pipeline = eventSourcing
    ...
    .withProjection("evaluationState", EvaluationStateProjectionHandler as any, {
    ...
    describe("Multi-Evaluation Tests", () => {
      it("processes multiple evaluations concurrently without interference", async () => {
        ...
        const [projection1, projection2, projection3] = await Promise.all([
          pipeline.service.getProjectionByName("evaluationState", evaluation1Id, {
            tenantId,
          }),
          pipeline.service.getProjectionByName("evaluationState", evaluation2Id, {
            tenantId,
          }),
          pipeline.service.getProjectionByName("evaluationState", evaluation3Id, {
            tenantId,
          }),
        ]) as [EvaluationState | null, EvaluationState | null, EvaluationState | null];
