...
> & {
  ...
  const evalRunStore = new EvaluationRunStore(
    new EvaluationRunService(
      new EvaluationRunClickHouseRepository(async () => clickHouseClient),
    ).repository,
  );
...
describe.skip("Evaluation Processing Pipeline - Integration Tests", () => {
  ...
  describe("Full Lifecycle Tests", () => {
    it("processes complete evaluation lifecycle: start -> complete (processed)", async () => {
      ...
      await waitForEvaluationRun(pipeline, evaluationId, tenantId, "processed");
  ...
  describe("Multi-Evaluation Tests", () => {
    it("processes multiple evaluations concurrently without interference", async () => {
      ...
      await Promise.all([
        waitForEvaluationRun(pipeline, evaluation1Id, tenantId, "in_progress"),
        waitForEvaluationRun(pipeline, evaluation2Id, tenantId, "in_progress"),
        waitForEvaluationRun(pipeline, evaluation3Id, tenantId, "in_progress"),
      ]);
      ...
      ])) as [EvaluationRun | null, EvaluationRun | null, EvaluationRun | null];
