...
import { pureSpanDataSchema } from "./commands";
import {
 ...
 * Contains pure span data (user input only) for proper event sourcing.
 * Computed fields (id, aggregateId, tenantId) are added during event processing.
 */
export const spanReceivedEventDataSchema = z.object({
  spanData: pureSpanDataSchema,
  collectedAtUnixMs: z.number(),
}) satisfies z.ZodType<{
  spanData: z.infer<typeof pureSpanDataSchema>;
  collectedAtUnixMs: number;
}>;
