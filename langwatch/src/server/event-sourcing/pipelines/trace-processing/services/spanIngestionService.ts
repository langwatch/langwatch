 ...
 *
 * This service sends spans to the trace-processing pipeline, which:
 * 1. Computes trace summaries (via TraceSummaryProjection)
 * 2. Stores individual spans to stored_spans (via SpanStorageEventHandler)
 *
 * @example
 * ```typescript
    ...
    return await this.tracer.withActiveSpan(
      ...
      async (span) => {
        ...
        for (const record of records) {
          ...
          // Send to trace-processing pipeline
          // This triggers both trace summary computation and span storage
