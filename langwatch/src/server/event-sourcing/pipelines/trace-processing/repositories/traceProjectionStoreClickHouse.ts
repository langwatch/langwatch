...
export class TraceProjectionStoreClickHouse implements ProjectionStore {
  tracer = getLangWatchTracer(
    "langwatch.trace-processing.projection-store.clickhouse",
  );
  logger = createLogger(
    "langwatch:trace-processing:projection-store:clickhouse",
  );
      ...
      async () => {
        try {
          await this.clickHouseClient.insert({
            ...
            values: [this.buildInsertRecord(projection, context.tenantId)],
            format: "JSONEachRow",
          });
        } catch (error) {
          this.logger.error(
