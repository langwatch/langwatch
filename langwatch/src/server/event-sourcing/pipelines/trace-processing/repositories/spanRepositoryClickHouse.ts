...
export class SpanRepositoryClickHouse implements SpanRepository {
  ...
  private transformSpanData(data: StoreSpanData): ClickHouseSpan {
    ...
    // Use the id from spanData if available, otherwise generate deterministically as fallback.
    // Event handlers should always provide an ID, but we handle missing IDs
    // for robustness (e.g., during event replay or testing).
    // The fallback uses deterministic generation to ensure idempotency.
    const id =
      spanData.id ||
      IdUtils.generateDeterministicSpanRecordIdFromData(
        tenantId,
        spanData.traceId,
        spanData.spanId,
        spanData.startTimeUnixMs,
      );
  ...
  private transformClickHouseSpanToSpanData(
    ...
    const links = clickHouseSpan["Links.TraceId"].map((traceId, index) => ({
      ...
      attributes: clickHouseSpan["Links.Attributes"][index]
        ? OtelConversionUtils.filterUndefinedAttributes(
    ...
    return {
      ...
      resourceAttributes: clickHouseSpan.ResourceAttributes
        ? OtelConversionUtils.filterUndefinedAttributes(
            clickHouseSpan.ResourceAttributes,
          )
        : undefined,
