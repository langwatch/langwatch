 ...
 *
 * The same five minutes the hand-rolled deferral used
 * (`DEFERRED_CHECK_DELAY_MS` in `originGate.reactor.ts`), now expressed as a
 * durable deadline rather than a queue delay: it lives on the process
 * instance row, so a restart between arming and firing does not lose it.
 *
 * **Coupled to `evaluationTrigger`**: that reactor sizes its evaluation dedup
 * TTL at this window + 60s so a trace whose evaluation is triggered twice —
 * once by a late span, once by the resulting `origin_resolved` — dispatches
 * one evaluation. When the reactor file is deleted at rewire time, its
 * importer must point at THIS constant; shortening it without moving that TTL
 * re-opens the duplicate-evaluation window.
 */
 ...
 * the trace's own content is none of the process's business (ADR-069), and
