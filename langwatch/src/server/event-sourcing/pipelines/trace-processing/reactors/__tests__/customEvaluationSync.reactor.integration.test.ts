...
describe.skip("Custom Evaluation Sync Reactor - End-to-End Integration", () => {
  ...
  let sharedEventSourcing: ReturnType<
    typeof createTestPipelines
  >["eventSourcing"];
  ...
  function createTestPipelines() {
    ...
    if (!clickHouseClient || !redisConnection) {
      throw new Error("ClickHouse and Redis required for integration tests.");
    }
    ...
    const evalPipelineDef = definePipeline<EvaluationProcessingEvent>()
      ...
      .build();
    const registeredEvalPipeline = eventSourcing.register(evalPipelineDef);
    ...
    // Create the subscriber with zero delay for faster tests
    const subscriber = createCustomEvaluationSyncReactor({
      reportEvaluation: evalCommands.reportEvaluation,
    });
    const fastSpec = { ...subscriber.spec, delay: 0 };
    ...
    const tracePipelineDef = definePipeline<TraceProcessingEvent>()
      ...
      .withSubscriber(subscriber.name, fastSpec)
    ...
    const registeredTracePipeline = eventSourcing.register(tracePipelineDef);
