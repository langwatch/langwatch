...
import { generateDeterministicTraceSummaryId } from "../utils/id.utils";
  ...
  handle(
    ...
    return this.tracer.withActiveSpan(
      ...
      (span) => {
        ...
        span.addEvent("events.extract.start");

        // Extract and enrich span data from all SpanReceivedEvents
        span.addEvent("spans.enrich.start");
        const spans: SpanData[] = [];
        let createdAt: number | null = null;
        let lastUpdatedAt = Date.now();
        let firstSpanReceivedEvent: (typeof events)[number] | null = null;
        ...
        if (!firstSpanReceivedEvent || spans.length === 0) {
          const now = Date.now();
          this.logger.debug(
            {
              tenantId,
          ...
          span.addEvent("projection.empty");

          return {
            id: generate("tracesummary").toString(),
            aggregateId,
            tenantId,
            version: now,
            data: {
        ...
        // Use createdAt (first span timestamp) for K-sortability
        const traceSummaryId = generateDeterministicTraceSummaryId(
