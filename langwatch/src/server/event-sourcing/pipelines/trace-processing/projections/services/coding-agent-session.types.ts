 ...
 *
 * So: `traceId` reaches the spans and logs, `sessionId` reaches the other
 * sessions of the same run, and `finalRequestId` reaches the exact response body
 * that ended the session. Text is measured (`promptChars`, `responseChars`) but
 * never carried.
 *
