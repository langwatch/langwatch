...
- **Claim strikes** - `processWithRetries` records a per-group strike in Redis (`{queue}:gq:group:{groupId}:strikes`, 1h TTL) before decoding and clears it in a `finally` on every surviving path. Only a job that kills the process leaves a strike behind. Strikes never park by themselves: every group co-in-flight with the killer dies with the same uncleared strike (up to `globalConcurrency` innocents per crash, redelivered as the same cohort after the active TTL), so counting alone cannot tell the poison from its bystanders. Once strikes exceed `LANGWATCH_GQ_POISON_STRIKE_THRESHOLD` (default 3, `0` disables), the group becomes a **suspect** and is run in isolation instead.
- **Isolation** - a suspect runs SOLO in the process: fastq intake pauses, every other in-flight job settles (clearing its own strike), then an isolation marker (`{queue}:gq:group:{groupId}:isolating`, value = the staged job id, 1h TTL) is written - awaited - before any decode/handler work, and cleared when the run settles. A death during a marked solo run is attributable beyond doubt; the surviving marker parks the group on its next claim (`reason: died_in_isolation`) - but only when it names the job being claimed. A death mid-run redelivers that exact staged job, so the match holds; a marker naming a job that already completed queue-side (the process died between the complete and the marker clear, e.g. during blob release) mismatches the group's NEXT job and is discarded at claim time instead of falsely parking it. A healthy bystander that inherited strikes survives its solo run and heals. One isolation at a time per process: a second suspect re-stages with backoff (`/iw/` staged-id suffix, distinct from the `/r/` retry budget); if quiesce times out or the marker write fails, the suspect runs *unattributed* (no marker) so a death is never mis-attributed: a death keeps the strikes for the next isolation attempt, while survival clears them like any other survived claim (strikes count consecutive deaths).
- **Decode cap** - staged values whose serialized (or decompressed) size exceeds `MAX_BLOB_BYTES` park the group unparsed. These are legacy bare-JSON values or tampered envelopes that predate or bypass the encode cap; dropping them to replay would just re-materialize the same value. Unblocking an oversized park re-hits the cap and re-parks: recovery is drain or move-to-DLQ (the stored error says so).
- **Recovery** - parked groups appear in the ops blocked summary with the stored error. Every operator exit resets the guard state: unblock, drain, and move-to-DLQ all clear the strikes key and the isolation marker, so a group re-created under the same id gets a fresh run instead of insta-parking on stale state within the 1h TTL. Draining discards the staged copies (event replay can rebuild).
...
| `gqGroupsPoisonParkedTotal` | claim-side poison guard parks (`reason`: died_in_isolation, oversized_payload) — alert on spikes: each park is a tenant's group stalled until an operator acts |
| `gqPoisonIsolationRunsTotal` | suspect isolation runs (`outcome`: cleared, unattributed, deferred) |
