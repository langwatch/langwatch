...
- **Isolation** - a suspect runs SOLO in the process: fastq intake pauses, every other in-flight job settles (clearing its own strike), then an isolation marker (`{queue}:gq:group:{groupId}:isolating`, 1h TTL) is written - awaited - before any decode/handler work, and cleared when the run settles. A death during a marked solo run is attributable beyond doubt; the surviving marker parks the group on its next claim (`reason: died_in_isolation`). A healthy bystander that inherited strikes survives its solo run and heals. One isolation at a time per process: a second suspect re-stages with backoff (`/iw/` staged-id suffix, distinct from the `/r/` retry budget); if quiesce times out or the marker write fails, the suspect runs *unattributed* (no marker) so a death is never mis-attributed: a death keeps the strikes for the next isolation attempt, while survival clears them like any other survived claim (strikes count consecutive deaths).
