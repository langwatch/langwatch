...
- **Non-retryable** → drop to the fail-safe (complete the slot; recover via event replay for fold/map only — a reactor on the path is named + preserved, not recovered; see ADR-081).
...
1. **Classifies** the error via `categorizeError`:
   ...
   - `NonRetryable` → drop to the fail-safe; the slot is completed and the work recovers via event replay for fold/map only — replay never invokes reactors, so a reactor job is named + dead-lettered, not recovered (ADR-081)
