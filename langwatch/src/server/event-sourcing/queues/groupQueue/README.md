...
- **Leases expire unless touched.** Stage, decode, the active-job heartbeat, and retry/transfer renew a holder's 3-day deadline. A crashed job stops renewing and ages out; the blob's 4-day backstop provides the lazy-reclaim interval. Release is idempotent and never eagerly deletes shared data.
