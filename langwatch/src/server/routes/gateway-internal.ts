 ...
 * `X-LangWatch-Gateway-Signature` header. Never expose publicly — the Helm
 * chart enforces this by default, hard-blocking `/api/internal` at the ingress
 * (`ingress.blockedPaths` in `charts/langwatch/values.yaml`); in-cluster callers
 * reach the app via the internal Service, not the ingress.
 *
