  ...
  return () => ({ op, next }) => {
    ...
    return observable((observer) => {
      ...
      const connect = () => {
        ...
        logger.info({ path: endpointUrl.pathname, input: op.input }, "Initiating SSE connection");
          ...
          try {
            ...
            if (isObject(parsed) && typeof parsed.type === "string") {
              if (parsed.type === "connected") {
                logger.debug({ path: endpointUrl.pathname }, "SSE connection acknowledged");
                return;
              }
              ...
              if (parsed.type === "error") {
                const msg = typeof parsed.message === "string" ? parsed.message : "SSE Error";
                logger.error({ path: endpointUrl.pathname, error: msg }, "SSE error message received");
            ...
            logger.debug({ path: endpointUrl.pathname, dataType: typeof parsed }, "SSE data message received");
        ...
        es.onerror = () => {
          ...
          logger.warn(
            { readyState: es?.readyState, attempt: reconnectAttempts + 1, maxReconnectAttempts },
            "SSE error"
          );
          ...
          logger.info({ attempt: reconnectAttempts, delay, path: endpointUrl.pathname }, "Scheduling SSE reconnection");
