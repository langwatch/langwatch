...
describe("Langy via HTTP wrapper", () => {
  ...
  it("returns analytics for a user-specified time range", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("how much did I spend last week?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("explains trace failures in plain English", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("find traces that failed recently and tell me why"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("lists scenarios", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("what scenario tests do I have?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("lists monitors", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("what monitors do I have set up?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("audits the setup and reports the biggest gap", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("audit my setup, what's the biggest gap?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("creates a dataset with rows (Layer 2: rows actually exist)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`create a dataset "${datasetName}" with 3 rows: France->Paris, Germany->Berlin, Japan->Tokyo`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
    ...
    const after = await listDatasets();
    const created = after.find((d) => !beforeIds.has(d.id));
    console.log(`Layer 2 dataset: ${created ? `${created.name} (records=${created.recordCount})` : "NOT FOUND"}`);
    expect(created).toBeTruthy();
  ...
  it("creates a scenario when asked (Layer 2: appears in API)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("create a basic customer-support scenario test for me"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  ...
  it("creates an evaluator (Layer 2: appears in API)", async () => {
    ...
    console.log(`Layer 2 evaluators delta: ${newOnes.length} new (names: ${newOnes.map((e) => e.name).join(", ")})`);
    expect(newOnes.length).toBeGreaterThan(0);
  });
  ...
  it("creates an agent (Layer 2: appears in API)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`create a customer-support agent called "${agentName}" with a basic helpful-assistant system prompt`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
    ...
    console.log(`Layer 2 agents delta: ${newOnes.length} new (names: ${newOnes.map((a) => a.name).join(", ")})`);
    expect(newOnes.length).toBeGreaterThan(0);
  });
  ...
  it("creates a monitor (Layer 2: appears in API)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`create a production monitor "${monitorName}" running hallucination evaluation on every trace`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
    ...
    console.log(`Layer 2 monitors delta: ${newOnes.length} new (names: ${newOnes.map((m) => m.name).join(", ")})`);
    expect(newOnes.length).toBeGreaterThan(0);
  });
  ...
  it("creates a prompt (Layer 2: appears in API)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`create a prompt with handle "${promptHandle}" containing "You are a helpful assistant"`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
    ...
    console.log(`Layer 2 prompts delta: ${newOnes.length} new (handles: ${newOnes.map((p) => p.handle ?? p.name).join(", ")})`);
    expect(newOnes.length).toBeGreaterThan(0);
  });
  ...
  it("creates a trigger (Layer 2: appears in API)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`create an alert trigger "${triggerName}" that fires when hallucination evaluation fails`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
  ...
  it("updates a prompt (Layer 2: content changed)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`update the prompt "${target.handle ?? target.name}" — change the system message to "You are a concise, expert assistant."`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
  ...
  it("runs a multi-step workflow: search then create dataset", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user(`look at last week's failed traces and create a dataset named "${datasetTag}" from them`),
        scenario.agent(),
        scenario.judge(),
      ],
    });
  ...
  it("returns a LangWatch URL when asked where to find prompts", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("where in LangWatch can I see my prompts?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("returns a deep link for datasets surface", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("where can I browse my datasets?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("returns a deep link for scenarios surface", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("where do I see my scenario test results?"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("multi-turn: create then update dataset (3 turns)", async () => {
    ...
    const datasets = await listDatasets();
    const created = datasets.find((d) => d.name === datasetName || d.name?.startsWith(datasetName));
    console.log(`Layer 2 multiturn dataset: ${created ? `${created.name} (rows=${created.recordCount})` : "NOT FOUND"}`);
    expect(created).toBeTruthy();
    expect(created!.recordCount).toBeGreaterThanOrEqual(3);
  });
  ...
  it("multi-turn: create scenario then run it (2 turns)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [
        scenario.user("create a simple greeting-bot scenario that checks the agent says hello"),
        scenario.agent(),
  ...
  it("declines an out-of-scope request without going off-topic", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("write me a haiku about Mondays"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("handles empty results gracefully (no hallucination)", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("show me traces from January 2030"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
  ...
  it("does not ask clarifying questions for an ambiguous but actionable request", async () => {
    ...
    const result = await scenario.run({
      ...
      script: [scenario.user("set up evaluations for me"), scenario.agent(), scenario.judge()],
    });
    if (!result.success) console.log("JUDGE REASONING:", result.reasoning);
    expect(result.success).toBe(true);
  });
