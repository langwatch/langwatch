...
function CustomDashboardsSection({ projectSlug }: { projectSlug: string }) {
    ...
    createDashboard.mutate(
      { projectId, name },
      {
        onSuccess: (newDashboard) => {
          ...
          void router.push(`/${projectSlug}/analytics/reports?dashboard=${newDashboard.id}`);
        },
        onError: () => {
          toaster.create({
  ...
  const handleFinishRename = () => {
    if (editingDashboardId && editingName.trim()) {
      renameDashboard.mutate(
        { projectId, dashboardId: editingDashboardId, name: editingName.trim() },
        {
          onSuccess: () => {
            void dashboardsQuery.refetch();
          },
