 ...
 *
 * Tests for CopyEvaluatorDialog – replicate evaluator to another project.
 */
...
// zag-js Select calls scrollTo on the listbox content; jsdom does not implement it
beforeAll(() => {
...
describe("CopyEvaluatorDialog", () => {
  ...
  it("renders dialog title and target project field", () => {
    render(<CopyEvaluatorDialog {...defaultProps} />, { wrapper: Wrapper });
    expect(screen.getByText("Replicate Evaluator")).toBeInTheDocument();
  ...
  it("calls copy mutation and onClose when project selected and Replicate clicked", async () => {
    ...
    await waitFor(() => {
      ...
      expect(copyMutateArgs?.evaluatorId).toBe("evaluator-1");
