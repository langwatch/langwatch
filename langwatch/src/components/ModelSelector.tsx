...
export const useModelSelectionOptions = (
  ...
  // tRPC `getAllForProject` now returns stored-only rows (no env-fed
  // phantom providers — `OPENAI_API_KEY` in the server env no longer
  // surfaces OpenAI in projects that haven't stored a provider row).
  // The Record-by-provider-key shape stays unchanged here so existing
  // callers don't move.
