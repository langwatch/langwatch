 ...
 *
 * Boundary tests for the side-effecting half of `handleDuplicateTarget`.
 *
...
describe("executeForkAgentDuplicate", () => {
  describe("given a workflow-type agent target", () => {
    ...
    describe("when the fork + publish + addTarget sequence succeeds", () => {
      it("runs agents.copy → workflow.publish → addTarget in order (#5879 scenario 2)", async () => {
        ...
        // Per-fn call count + argument shape (catches wrong tRPC inputs and
        // missed publishes — the regression modes named in the #5935 P2
        // review).
        ...
        // catches an accidental shallow fallback (where addTarget would run
        // before any mutation) or a swapped copy/publish order.
    ...
    describe("when workflow.publish throws after agents.copy succeeded", () => {
      beforeEach(() => {
      ...
      it("rolls back via agents.delete and does NOT add the column when workflow.publish throws", async () => {
        ...
        // No column added — the alternative would reintroduce #5879 (two
        // columns pointing at the same dbAgentId after a partial failure).
    ...
    describe("when agents.copy itself fails before any forked row exists", () => {
      beforeEach(() => {
  ...
  describe("given a code-type agent target (no workflow to publish)", () => {
    let stubs: MutationStubs;
    const addTarget = vi.fn();
    const openTargetEditor = vi.fn();
    ...
    describe("when the fork succeeds without workflow ids on the copy result", () => {
      // A code/HTTP/signature agent has no workflow to publish — `agents.copy`
      // returns no workflow ids, so `workflow.publish` must be skipped. This
      // is the regression path that would otherwise surface as "no committed
      // version" at run time (see #5879 / #5871).
  ...
  describe("given a prompt target", () => {
    ...
    describe("when the duplicate is requested", () => {
      // Prompt targets carry their own per-column draft
      // (`localPromptConfig`), so a spread-only duplicate is already
      // correct — forking is unnecessary and would actually break the prompt
      // editor's per-column editing model.
