...
export const SAMPLE_PREVIEW_TRACES: readonly TraceListItem[] = [
  ...
  makeTrace({
    ...
    totalCost: 0.0418,
    spanCount: 18,
    input: "Research the migration story from Cloud Run to Fly.io for our shape of workload (long-running websockets, ~50 RPS).",
    output:
...
export const ARRIVAL_PREVIEW_TRACES: readonly TraceListItem[] = [
  ...
  makeTrace({
    ...
    spanCount: 18,
    input:
      "Checkout conversion dropped ~18% yesterday morning. What happened?",
    output:
...
/** Trace IDs of the rows we want to call out as "newly arrived." */
export const ARRIVAL_PREVIEW_IDS: ReadonlySet<string> = new Set(
  ARRIVAL_PREVIEW_TRACES.map((t) => t.traceId),
);
 ...
 */
export const RICH_ARRIVAL_SPAN_TREE: SpanTreeNode[] = [
  {
 ...
 */
export const RICH_ARRIVAL_SPAN_DETAILS: SpanDetail[] = [
  {
    spanId: "span-root-agent",
    parentSpanId: null,
    name: "mastra.agent.run",
 ...
 */
export function buildRichArrivalHeader(): TraceHeader {
  return {
 ...
 */
export interface RichArrivalConversationContext {
  conversationId: string;
  total: number;
  turns: ConversationTurn[];
}

export function buildRichArrivalConversationContext(): RichArrivalConversationContext {
 ...
 */
export function buildRichArrivalEvaluations(): EvaluationRunData[] {
