...
function TargetAffordance({ targetId }: { targetId: string }) {
  ...
  const pick = useLangyContextTargetStore((s) => s.pick);
  const unpick = useLangyContextTargetStore((s) => s.unpick);
  ...
  const onClick = useCallback(() => {
    ...
    if (isAdded) {
      unpick(target.id);
      // Dismiss too: the chip showing might have been auto-derived from the
      // route rather than picked, and unpicking alone would leave it in the
      // composer, making the click look like it did nothing.
      langy.dismissChip(target.id);
    } else {
      pick(target);
      langy.restoreChip(target.id);
    }
  }, [target, isAdded, pick, unpick]);
