 ...
 *
 * This used to be a hand-written CSS file that redefined `--chakra-colors-*`
 * under `.langy-root`. It worked, but it meant the design tokens for a whole
 * surface lived outside the design system: invisible to `createSystem`, untyped,
 * unmergeable, and impossible to reference from a style prop.
 *
 * Now the same idea is expressed the way Chakra intends. Two custom CONDITIONS
 * are declared —
 *
 *     _langy      →  ".langy-root &"          (Langy, light)
 *     _langyDark  →  ".dark .langy-root &"    (Langy, dark)
 *
 * — and the palette is attached to the SAME semantic tokens the rest of the app
 * uses (`bg.surface`, `fg.muted`, `border`, `orange.*`). `mergeConfigs` folds
 * these condition keys in beside the app's existing `_light` / `_dark` values
 ...
 *
 * The specificity that makes it win is the same one the old CSS relied on, and
 * it now falls out of Chakra's own emission rather than being hand-maintained:
 * Chakra writes `.langy-root { … }` (0,1,0) after `:root, .light` (0,1,0 — later
 * wins), and `.dark .langy-root { … }` (0,2,0) over `.dark` (0,1,0 — higher
 * specificity wins). Verified against `system.getTokenCss()`.
 *
 * ── The palette itself ──────────────────────────────────────────────────────
 * Lifted from LangWatch's marketing site (`2lang-2watch`), and specifically from
 * the HOMEPAGE'S DARK SECTIONS (`SectionLangy`, `SectionEnterprise`) — the very
 * sections that present Langy — rather than from the wireframe mock-ups that
 * float inside its light sections. Those two are different languages, and taking
 * the wrong one is what made dark mode read as generic:
 *
 *   - TEXT IS PAPER AT ALPHA, NOT GREY. The site never writes a grey on ink; it
 *     writes white and turns it down (`text-paper/55`, `/35`). The warm ink-300
 *     greys that were here are a light-mode idea (ink on paper) applied to the
 *     wrong ground.
...
/** Warm paper + ink (light). Straight off the site's token block. */
const paper = {
  base: "#faf8f3",
  soft: "#efeee9",
  deep: "#e3e2dc",
...
/** The brand ramp. `300` is THE accent on dark; `500` is the action colour. */
const brand = {
  200: "#ffe2cc",
  300: "#ffb380",
  400: "#ff8a3d",
...
/** Shorthand for a token that only differs between Langy-light and Langy-dark. */
  ...
  theme: {
    semanticTokens: {
      colors: {
        ...
        bg: {
          surface: langy(paper.base, ink[900]),
          panel: langy(paper.soft, ink[950]),
          page: langy(paper.soft, ink[950]),
          subtle: langy("#f3f0e9", white(0.03)),
          muted: langy("#e9e5db", white(0.06)),
          emphasized: langy(paper.deep, white(0.1)),
        },

        // ── Text ────────────────────────────────────────────────────────────
        fg: {
          DEFAULT: langy(ink[800], "#ffffff"),
          muted: langy(ink[400], white(0.55)),
          subtle: langy(ink[300], white(0.35)),
        },
        ...
        border: {
          DEFAULT: langy("#dcd9cd", white(0.1)),
          muted: langy(ink[100], white(0.1)),
          emphasized: langy(ink[200], white(0.15)),
        },
        ...
        orange: {
          solid: langy(brand[500], brand[400]),
          fg: langy(brand[700], brand[300]),
          emphasized: langy(brand[300], brand300(0.3)),
          subtle: langy(brand[200], brand300(0.1)),
          muted: langy("#ffd3b3", brand300(0.16)),
        },

        // ── Brand purple ────────────────────────────────────────────────────
        // The categorical accent (agents, simulations, proposals). Pinned so
        // `purple.fg` inside Langy is the SITE's purple, not Chakra's default.
        purple: {
          solid: langy("#a855f7", "#a855f7"),
          fg: langy("#7e22ce", "#a855f7"),
          emphasized: langy("#c084fc", "rgba(168, 85, 247, 0.3)"),
          subtle: langy("#f3e8ff", "rgba(168, 85, 247, 0.1)"),
          muted: langy("#e9d5ff", "rgba(168, 85, 247, 0.16)"),
        },

        // ── Pass / fail ─────────────────────────────────────────────────────
        // Moss and rust are the site's own status accents. The `fg` variants are
        // lifted a little from the raw hexes so 11px text on ink stays legible —
        // the site uses them at larger sizes than Langy does.
        green: {
          fg: langy("#4a6b3a", "#7fa06a"),
          solid: langy("#5b7a4a", "#5b7a4a"),
        },
        red: {
          fg: langy("#9e4032", "#d6796a"),
          solid: langy("#b85240", "#b85240"),
        },
        ...
        langy: {
          ...
          barTrack: langy(paper.deep, white(0.1)),
      ...
      // no shadow at all; on paper because a hairline over cream is enough, and
      // a stack of four shadowed cards in a turn read as a deck of trading
