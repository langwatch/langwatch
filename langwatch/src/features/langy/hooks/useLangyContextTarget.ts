...
export function useLangyContextTarget(
  ...
  const isNear = useLangyContextTargetStore((state) =>
    isActive && id ? state.nearIds.has(id) : false,
  );
  ...
  const toggle = useCallback(() => {
    ...
    if (targets.activeChipIds.has(id)) {
      // Drop the pick, and dismiss the id as well — the chip showing might have
      // been auto-derived from the route rather than picked, and unpicking
      // alone would leave it sitting in the composer, making the click look
      // like it did nothing. Dismissal is exactly what the chip's own ✕ does.
      targets.unpick(id);
      langy.dismissChip(id);
    } else {
      targets.pick({ id, kind, label, ref: chipRef });
      // Undo any earlier dismissal, so re-adding a chip you removed works.
      langy.restoreChip(id);
    }
  ...
  const targetProps = useMemo<LangyContextTargetProps>(() => {
    ...
    return {
      ...
      "data-langy-target-state": visualState({ isAdded, isHovered, isNear }),
    };
  }, [isActive, id, isAdded, isHovered, isNear]);
