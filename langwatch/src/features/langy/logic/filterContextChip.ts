...
/**
 * Build the filter chip from the Trace Explorer's active filter query —
 * "filtered: <query>" — so the agent scopes "these traces" to what the user has
 * narrowed the table to. Pure so it can be unit-tested.
 *
 * The id embeds the query so a dismissed chip re-surfaces when the user edits
 * the filter to something different; an empty query yields no chip. Deliberately
 * keyed on the query text only, not the time range: the default 30-day window
 * isn't "filtering", and surfacing it would put a low-signal chip on the
 * composer for every visit.
 */
