 ...
 *      panel open; adding to context is an explicit act on an explicit button.
 *   3. Targets light up around the POINTER, not all at once.
 */
...
describe("useLangyContextTarget", () => {
  ...
  describe("given the Langy panel is open", () => {
    ...
    describe("when the pointer moves near the target", () => {
      it("lights it up faintly", () => {
        render(<HostRow onOpen={() => undefined} />);

        act(() =>
          targets().setProximity({
            nearIds: ["trace:abc123"],
            hoveredId: null,
          }),
        );
    ...
    describe("when the pointer moves onto the target", () => {
      it("lights it up firmly, so the affordance has something to sit on", () => {
        render(<HostRow onOpen={() => undefined} />);

        act(() =>
          targets().setProximity({
            nearIds: ["trace:abc123"],
            hoveredId: "trace:abc123",
          }),
        );
    ...
    describe("when the target has been absorbed into Langy", () => {
      it("reads as added, even with the pointer nowhere near it", () => {
        render(<HostRow onOpen={() => undefined} />);
