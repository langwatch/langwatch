...
secured.access(requires("organization:view")).get(
  "/",
  apiKeyServiceMiddleware,
  describeRoute({
    description:
      "List all API keys for the authenticated user in this organization",
  }),
  async (c) => {
...
secured.access(requires("organization:manage")).post(
  "/",
  apiKeyServiceMiddleware,
  describeRoute({
...
secured.access(requires("organization:manage")).delete(
  "/:id",
  apiKeyServiceMiddleware,
  describeRoute({
