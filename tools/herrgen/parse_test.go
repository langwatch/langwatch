...
func TestParseSkipsUnparseableSnippetsWithAWarning(t *testing.T) {
	...
	root := tree(t, map[string]string{
		"pkg/herr/herr.go": herrPackage,
		"modules/onboarding/web/src/ui/sections/observability/codegen/snippets/go/openai.snippet.go": `package main
...
func TestParseFailsOnAnUnparseableFileOutsideTheSnippets(t *testing.T) {
	...
	root := tree(t, map[string]string{
		"pkg/herr/herr.go": herrPackage,
		"modules/trace/server/src/background/probe.go": `package background
...
`,
	...
	if !strings.Contains(err.Error(), "modules/trace/server/src/background/probe.go") {
		t.Errorf("Parse() error = %q, want it to name the file", err)
	}
	if warnings.String() != "" {
