...
// ClickHouseMaxConcurrentQueries is the server's own admission cap, sized for
// a small shared VM. Rendered into the container's config AND exported to the
// app as CLICKHOUSE_SERVER_MAX_CONCURRENT_QUERIES — the app's pool sizing
// clamps its fallback to this budget, and both sides must read one constant
// or they drift apart silently.
const ClickHouseMaxConcurrentQueries = 32
...
func RenderClickHouseConfig(l ClickHouseLimits) string {
	return fmt.Sprintf(clickHouseConfigTemplate, l.MaxServerMemory, l.MarkCacheSize, ClickHouseMaxConcurrentQueries, renderLogConfig(l))
}
