  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      if [ -z "$image_exists" ]; then
        docker buildx build . --platform="linux/amd64" --push -t ${data.aws_ecr_repository.langwatch.repository_url}:${local.tag} --build-arg SENTRY_AUTH_TOKEN=$${SENTRY_AUTH_TOKEN}
        set +e
