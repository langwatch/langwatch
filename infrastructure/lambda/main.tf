  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      if [ -z "$image_exists" ]; then
        docker buildx build . --build-arg EVALUATOR=${local.evaluator_package} --platform="linux/amd64" --provenance=false $cache_from --cache-to type=inline --push -t ${data.aws_ecr_repository.lambda_repository.repository_url}:${local.tag}
        set +e
