  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      if [ -z "$image_exists" ]; then
        docker buildx build . -f Dockerfile.lambda --platform="linux/arm64" $cache_from --cache-to type=inline --provenance=false --push -t ${data.aws_ecr_repository.langwatch_nlp.repository_url}:${local.langwatch_nlp_tag}
        set +e
