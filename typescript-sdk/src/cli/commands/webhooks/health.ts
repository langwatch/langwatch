...
export const webhookHealthCommand = async (id: string): Promise<CommandResult | void> => {
  ...
  try {
    ...
    spinner.succeed(`Lag: ${lag}, status: ${health.status}${health.disabled_reason ? ` (${health.disabled_reason})` : ""}`);
    return {
      data: health,
      table: () => {
        console.log();
        ...
        console.log(`Success rate:  ${health.success_rate === null ? "-" : `${Math.round(health.success_rate * 100)}%`}`);
        console.log(`p95 latency:   ${health.p95_latency_ms === null ? "-" : `${health.p95_latency_ms}ms`}`);
