...
describe("warnIfGeminiOAuthSelected", () => {
  ...
  describe("when settings.json has selectedType=gemini-oauth", () => {
    /**
     * The trap rchaves hit + sergey's gemini Path A sweep surfaced:
     * gemini-cli silently prefers the OAuth cache even with API-key
     * env vars set. Warning gives users an actionable next step
     * instead of a silent gateway bypass.
     */
  ...
  describe("when settings.json has JSONC comments and trailing commas", () => {
    /**
     * gemini-cli tolerates JSONC. The wrapper does too, so a user
     * with a commented-up settings.json gets the same accurate
     * warning.
     */
