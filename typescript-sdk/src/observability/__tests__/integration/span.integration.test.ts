...
describe("Span Integration Tests", () => {
  ...
  describe("event recording integration", () => {
    it("should record GenAI events with proper format and attributes", async () => {
      const tracer = getLangWatchTracer("genai-events-test");

      await tracer.withActiveSpan("genai-conversation", async (span) => {
        span
          .setType("llm")
          ...
          .addGenAIToolMessageEvent(
            {
              role: "tool",
              content: "Code example provided successfully",
              id: "call_123",
            },
            "openai",
          )
      ...
      const systemEvent = span.events.find(
        (e) => e.name === semconv.LOG_EVNT_GEN_AI_SYSTEM_MESSAGE,
      );
      expect(systemEvent).toBeDefined();
      if (!systemEvent?.attributes) {
        throw new Error("Expected system event with attributes");
      }
      expect(systemEvent.attributes["gen_ai.system"]).toBe("openai");
      const systemBody = JSON.parse(
        systemEvent.attributes[
          semconv.ATTR_LANGWATCH_GEN_AI_LOG_EVENT_BODY
        ] as string,
      );
      expect(systemBody.role).toBe("system");
      expect(systemBody.content).toContain("helpful AI assistant");

      // Verify user message event
      const userEvent = span.events.find(
        (e) => e.name === semconv.LOG_EVNT_GEN_AI_USER_MESSAGE,
      );
      ...
      expect(userBody.content).toContain("async/await");

      // Verify assistant message event
      const assistantEvent = span.events.find(
        (e) => e.name === semconv.LOG_EVNT_GEN_AI_ASSISTANT_MESSAGE,
      );
      expect(assistantEvent).toBeDefined();
      if (!assistantEvent?.attributes) {
        throw new Error("Expected assistant event with attributes");
      }
      ...
      expect(assistantBody.content).toContain("syntax that makes it easier");

      // Verify tool message event
      const toolEvent = span.events.find(
        (e) => e.name === semconv.LOG_EVNT_GEN_AI_TOOL_MESSAGE,
      );
      expect(toolEvent).toBeDefined();
      if (!toolEvent?.attributes) {
        throw new Error("Expected tool event with attributes");
      }
      const toolBody = JSON.parse(
        toolEvent.attributes[
          semconv.ATTR_LANGWATCH_GEN_AI_LOG_EVENT_BODY
        ] as string,
      );
      expect(toolBody.role).toBe("tool");
      expect(toolBody.id).toBe("call_123");

      // Verify choice event
      const choiceEvent = span.events.find(
        (e) => e.name === semconv.LOG_EVNT_GEN_AI_CHOICE,
      );
      expect(choiceEvent).toBeDefined();
      if (!choiceEvent?.attributes) {
        throw new Error("Expected choice event with attributes");
      }
      const choiceBody = JSON.parse(
        choiceEvent.attributes[
          semconv.ATTR_LANGWATCH_GEN_AI_LOG_EVENT_BODY
        ] as string,
      );
      expect(choiceBody.finish_reason).toBe("stop");
      expect(choiceBody.index).toBe(0);
      expect(choiceBody.message.role).toBe("assistant");
    });

    it("should handle role defaults in events", async () => {
      const tracer = getLangWatchTracer("role-defaults-test");

      await tracer.withActiveSpan("role-defaults-span", async (span) => {
        span
          .setType("llm")
          // Not providing role - should default
          .addGenAISystemMessageEvent({
            content: "System message without explicit role",
          })
          .addGenAIUserMessageEvent({
            content: "User message without explicit role",
          })
          .addGenAIAssistantMessageEvent({
            content: "Assistant message without explicit role",
          })
          .addGenAIToolMessageEvent({
            content: "Tool message without explicit role",
            id: "default_call",
          })
          .addGenAIChoiceEvent({
            finish_reason: "stop",
            index: 0,
            message: {
              content: "Choice without explicit role",
            },
          });
      });
      ...
      // Check that all events have proper default roles
      span.events.forEach((event) => {
        if (!event.attributes) {
          throw new Error("Expected event to have attributes");
        }
        ...
        if (event.name === semconv.LOG_EVNT_GEN_AI_SYSTEM_MESSAGE) {
          expect(body.role).toBe("system");
        } else if (event.name === semconv.LOG_EVNT_GEN_AI_USER_MESSAGE) {
          expect(body.role).toBe("user");
        } else if (event.name === semconv.LOG_EVNT_GEN_AI_ASSISTANT_MESSAGE) {
          expect(body.role).toBe("assistant");
        } else if (event.name === semconv.LOG_EVNT_GEN_AI_TOOL_MESSAGE) {
          expect(body.role).toBe("tool");
        } else if (event.name === semconv.LOG_EVNT_GEN_AI_CHOICE) {
    ...
    it("should support fluent chaining", async () => {
      ...
      await tracer.withActiveSpan("fluent-span", async (span) => {
        const result = span
          ...
          .setInput("Start")
          .setAttribute("step", 2)
          .addGenAIUserMessageEvent({ role: "user", content: "Test" })
          .setAttribute("step", 3)
          .setOutput("End")
          .setAttribute("step", 4);
  ...
  describe("error handling and edge cases", () => {
    ...
    it("should handle empty and null event bodies gracefully", async () => {
      const tracer = getLangWatchTracer("empty-events-test");

      await tracer.withActiveSpan("empty-events-span", async (span) => {
        span
          .setType("llm")
          .addGenAIUserMessageEvent({
      ...
      if (userEvent?.attributes && systemEvent?.attributes) {
        const userBody = JSON.parse(
          userEvent.attributes[
            semconv.ATTR_LANGWATCH_GEN_AI_LOG_EVENT_BODY
          ] as string,
        ...
        expect(userBody.content).toBe("");
        expect(systemBody.content).toBe("Valid system message");
      }
    });

    it("should handle malformed GenAI event structures", async () => {
      const tracer = getLangWatchTracer("malformed-events-test");

      await tracer.withActiveSpan("malformed-events-span", async (span) => {
        span.setType("llm");

        try {
          // Test events with missing/invalid fields
          ...
          // Test choice event with minimal data
          span.addGenAIChoiceEvent({
            finish_reason: "stop",
            index: 0,
            message: {
              content: "Choice with minimal data",
            },
          });

          // Test tool message with required fields
          span.addGenAIToolMessageEvent({
            content: "Tool response",
            id: "tool_call_123",
          });
        } catch (error) {
          // Some malformed events might throw - this is acceptable
          span.addGenAISystemMessageEvent({
            content: "Fallback system message after error",
          });
        }
      });
      ...
      // All recorded events should have valid structure
      span.events.forEach((event) => {
        expect(event.name).toBeDefined();
        expect(event.attributes).toBeDefined();
        if (event.attributes) {
          expect(
            event.attributes[semconv.ATTR_LANGWATCH_GEN_AI_LOG_EVENT_BODY],
          ).toBeDefined();
