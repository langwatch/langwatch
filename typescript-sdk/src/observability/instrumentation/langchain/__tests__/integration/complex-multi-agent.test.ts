...
describe("LangChain Multi-Agent Integration Tests", () => {
  ...
  // Helper functions to create different agent types
  async function createResearchAgent() {
    const llm = new ChatOpenAI({ model: "gpt-4o-mini", temperature: 0.3 });
    const tools = [
      new DynamicTool({
        name: "web_search",
        description: "Search for recent information on AI trends",
        func: async (query: string) =>
          `Research results for: ${query} - Found 5 relevant articles about AI agent collaboration trends.`,
      }),
      new DynamicTool({
        name: "academic_search",
        description: "Search academic papers",
        func: async (query: string) =>
          `Academic papers on: ${query} - Found 3 recent papers on multi-agent systems.`,
      }),
    ];
  ...
  async function createAnalysisAgent() {
    ...
    const tools = [
      new DynamicTool({
        ...
        func: async (data: string) =>
          `Statistical analysis complete: Found significant trends in ${data}`,
      }),
      new DynamicTool({
        name: "pattern_recognition",
        description: "Identify patterns in research data",
        func: async (data: string) =>
          `Patterns identified: 3 key themes and 2 emerging trends in ${data}`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are an analysis expert. Process and analyze the provided research data.",
      ],
      ["human", "{input}\nContext: {context}"],
      ["placeholder", "{agent_scratchpad}"],
    ]);
  ...
  async function createReportAgent() {
    const llm = new ChatOpenAI({ model: "gpt-4o-mini", temperature: 0.7 });
    const tools = [
      new DynamicTool({
        name: "format_report",
        description: "Format analysis into professional report",
        func: async (content: string) =>
          `Formatted report: Executive Summary\n${content}\nConclusions and Recommendations`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are a report writer. Create clear, professional reports from analysis.",
      ],
      ["human", "{input}\nFormat: {format}"],
      ["placeholder", "{agent_scratchpad}"],
    ]);
  ...
  async function createSupervisorAgent() {
    ...
    const tools = [
      new DynamicTool({
        ...
        description: "Delegate data collection to specialized agent",
        func: async (task: string) =>
          `Data agent executed: ${task} - Collected market data from 10 sources`,
      }),
      new DynamicTool({
        name: "delegate_to_analysis_agent",
        description: "Delegate analysis to specialized agent",
        func: async (data: string) =>
          `Analysis agent processed: ${data} - Generated insights and trends`,
      }),
      new DynamicTool({
        name: "delegate_to_viz_agent",
        description: "Delegate visualization to specialized agent",
        func: async (analysis: string) =>
          `Visualization agent created: Charts and graphs for ${analysis}`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are a supervisor agent. Coordinate and delegate tasks to specialized agents.",
      ],
  ...
  async function createConversationalAgent() {
    ...
    const tools = [
      new DynamicTool({
        ...
        func: async (conv_id: string) =>
          `Context for ${conv_id}: Previous discussion about renewable energy project`,
      }),
      new DynamicTool({
        name: "knowledge_lookup",
        description: "Look up specific domain knowledge",
        func: async (topic: string) =>
          `Knowledge on ${topic}: Solar panels typically have 15-22% efficiency, costs vary by region`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are a helpful conversational assistant. Maintain context across the conversation.",
      ],
      [
        "human",
  ...
  async function createResilientAgent() {
    ...
    const tools = [
      new DynamicTool({
        ...
        description: "Primary data analysis tool (may fail)",
        func: async (source: string) => {
          if (source.includes("unreliable")) {
            throw new Error("Primary data source unavailable");
          }
          return `Primary analysis of ${source}`;
        },
      }),
      new DynamicTool({
        ...
        func: async (source: string) =>
          `Fallback analysis completed for ${source} - Used alternative data sources`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        "You are a resilient agent. Try primary methods first, then fallback to alternatives.",
      ],
      [
        "human",
  ...
  async function createSpecialistAgent(specialty: string) {
    const llm = new ChatOpenAI({ model: "gpt-4o-mini", temperature: 0.5 });
    const tools = [
      new DynamicTool({
        name: `${specialty}_analysis`,
        description: `Specialized ${specialty} analysis`,
        func: async (task: string) =>
          `${specialty} analysis of ${task}: Specialized insights from ${specialty} perspective`,
      }),
    ];
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      [
        "system",
        `You are a ${specialty} specialist. Provide expert analysis in your domain.`,
      ],
      [
        "human",
        "{input}\nSpecialist mode: {specialist_mode}\nParallel execution: {parallel_execution}",
      ],
      ["placeholder", "{agent_scratchpad}"],
    ]);
