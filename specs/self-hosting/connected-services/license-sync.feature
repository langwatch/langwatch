Feature: License sync
  ...
  @unit @unimplemented
  Scenario: The instance id is a random identifier created once
    When the install starts for the first time
    Then it creates a random instance id and keeps it
    And the same instance id is sent on every later sync
