...
Feature: Multi-aggregate pipeline
  ...
  Scenario: Declaring a pipeline with two aggregate types
    When I define a pipeline "authz" with aggregate types "authz_grant" and "authz_role"
  ...
  Scenario: A single-aggregate pipeline is unchanged
    ...
    And the command's queue group key is "org_1/command/recordSpan/trace:t1"
    And the command's dedup key is "org_1:trace:t1"
    And the command's kill-switch key is "es-trace-command-recordSpan-killswitch"
  ...
  Scenario: Appending an event of each aggregate to the shared pipeline
    Given a pipeline "authz" with aggregate types "authz_grant" and "authz_role"
  ...
  Scenario: A command's queue group key and kill-switch key use its bound aggregate
    ...
    Then the command's queue group key is "org_1/command/defineRole/authz_role:r1"
    And the command's dedup key is "org_1:authz_role:r1"
  ...
  Scenario: A store-miss re-fold pages the event's own aggregate type
    ...
    And stored "lw.authz.role.defined" events for "r1" and a "lw.authz.grant.attached" event for "r1"
    When a "lw.authz.role.defined" event for "r1" is delivered and the fold store has no row
    Then the paged history is read with aggregate type "authz_role" and id "r1"
    And the re-folded state holds only the role's events

  @unit
  Scenario: A fold projection with a custom key is still keyed by aggregate type on a multi-aggregate pipeline
    Given a pipeline "authz" with aggregate types "authz_grant" and "authz_role"
    And a fold projection "byOrg" whose custom key is the event's organization
    When a "lw.authz.grant.attached" event and a "lw.authz.role.defined" event for organization "org_1" are folded
    Then the fold store holds a row keyed "authz_grant:org_1" and a row keyed "authz_role:org_1"

  @unit
  Scenario: A fold store must honour the context key on a multi-aggregate pipeline
    Given a pipeline "authz" with aggregate types "authz_grant" and "authz_role"
    When I register a fold projection "ledger" whose store does not declare that it honours the context key
  ...
  Scenario: A replay rebuilds a multi-aggregate fold row under the key live dispatch used
    Given a pipeline "authz" with aggregate types "authz_grant" and "authz_role"
    And a fold projection "ledger" over both aggregates' events
    When a "lw.authz.grant.attached" event and a "lw.authz.role.defined" event for "x1" are replayed
    Then the fold store holds a row keyed "authz_grant:x1" and a row keyed "authz_role:x1"
  ...
  Scenario: A projection's kill-switch key on a multi-aggregate pipeline uses the pipeline name
    Given a registered pipeline "authz" with aggregate types "authz_grant" and "authz_role"
    And a fold projection "ledger"
    Then the projection's kill-switch key is "es-authz-projection-ledger-killswitch"
  ...
  Scenario: Ops introspection lists every aggregate type the pipeline owns
    ...
    And resolving the pipeline's process managers from aggregate type "authz_role" finds them
