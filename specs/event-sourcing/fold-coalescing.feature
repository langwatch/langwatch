Feature: Group-coalesced fold projections
  ...
  # Post-event work that reads trace-level data derived from stored_spans
  ...
  # re-reads, so no consumer is served stale data.

  @unit @coalescing @derivation
  Scenario: Repeated trace-level derivations within one fold version read stored spans once
    Given several consumers derive trace data for the same trace at one fold version
