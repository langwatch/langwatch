Feature: Predep detection and installation
  ...
  Scenario: All predeps already installed — phase auto-skips
    Given uv, postgres, redis-server, and clickhouse are on PATH or under "~/.platform/app/bin"
  ...
  Scenario: Confirming installs missing predeps in parallel into "~/.platform/app/bin"
    ...
    And uv is installed via "curl -LsSf https://astral.sh/uv/install.sh | sh" into "~/.platform/app/bin/uv"
    And postgres is downloaded from the EnterpriseDB binaries into "~/.platform/app/bin/postgres/"
    And redis-server is downloaded as a binary into "~/.platform/app/bin/redis-server"
    And clickhouse is downloaded via "curl https://clickhouse.com/" into "~/.platform/app/bin/clickhouse"
  ...
  Scenario: Installed binaries are placed under "~/.platform/app/bin" and prepended to PATH for the session
    When the predeps phase finishes
    Then "~/.platform/app/bin" exists
    And "~/.platform/app/bin/redis-server", "~/.platform/app/bin/clickhouse" are executable
    And the runner phase has "~/.platform/app/bin" prepended to PATH
  ...
  Scenario: AI Gateway monobinary is downloaded for the host platform
    ...
    When the predeps phase runs
    Then the gateway binary at "~/.platform/app/bin/aigateway" matches the host platform
