...
| specs/features/customer-io-nurturing-integration.feature | "First simulation run identifies user with has_simulations true" | KEEP | customerIoSimulationSync.subscriber.unit.test.ts covers first-sim identify |
| specs/features/customer-io-nurturing-integration.feature | "First simulation run fires first_simulation_ran event" | KEEP | first_simulation_ran tracked in simulation sync subscriber tests |
| specs/features/customer-io-nurturing-integration.feature | "First simulation fires immediately without debouncing" | KEEP | First-sim branch fires immediately in simulation sync subscriber |
| specs/features/customer-io-nurturing-integration.feature | "Subsequent simulation runs update org-wide count and timestamp with debouncing" | KEEP | Subsequent-sim branch in subscriber covers org-wide count + last_simulation_at |
| specs/features/customer-io-nurturing-integration.feature | "Simulation sync subscriber uses project-scoped dedup ID for debouncing" | KEEP | dedupId returns cio-sim-sync-{tenantId}; covered by subscriber unit test |
| specs/features/customer-io-nurturing-integration.feature | "Simulation tracking is independent of scenario template creation" | KEEP | Subscriber only fires on simulation pipeline finished events; scenario-create path not coupled |
...
| specs/features/suites/cancel-queued-running-jobs.feature | "Process manager broadcasts cancel to Redis on cancel_requested event" | DUPLICATE | covered by cancellation-event-sourcing.integration.test.ts (@scenario binding); broadcast now rides the simulation_run_execution PM outbox cancel intent |
| specs/features/suites/cancel-queued-running-jobs.feature | "Cancel-grace watchdog force-finishes when the broadcast is lost" | KEEP | simulationRunExecution PM cancel-grace wake (CANCEL_GRACE_MS=60s); no test exercises the lost-broadcast backstop |
...
| specs/features/suites/cancel-queued-running-jobs.feature | "Cancellation reaches a worker on a different pod" | KEEP | PM cancel intent publishes via Redis pub/sub; no integration test exercises cross-pod path |
| specs/features/suites/cancel-queued-running-jobs.feature | "Worker skips execution if cancel was already requested" | DUPLICATE | simulationRunExecution PM emits no execute intent once cancellation is recorded; covered by event-sourcing integration test |
| specs/features/suites/cancel-queued-running-jobs.feature | "Cancelling a queued run writes both cancel and finished events" | DUPLICATE | cancellation.ts dispatches cancel_requested; the PM writes the finished(CANCELLED) event; covered by cancellation.unit.test.ts |
...
| specs/features/suites/cancel-queued-running-jobs.feature | "Batch cancel across multiple workers terminates all active runs" | KEEP | code path exists via PM cancel intents; no integration test exercises multi-worker batch flow |
