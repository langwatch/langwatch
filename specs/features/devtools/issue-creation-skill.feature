Feature: Standardized GitHub issue creation via /issue skill
  ...
  Background:
    Given a SKILL.md file exists at .claude/skills/issue/SKILL.md
    And the skill is user-invocable via /issue
  ...
  Scenario: Confirms detected type before creating issue
    Given the user runs /issue "Refactor the auth module and add OAuth support"
  ...
  Scenario: Creates bug issue with template body sections
    Given the user runs /issue "Login page throws 500 error"
  ...
  Scenario: Creates feature request with template body sections
    Given the user runs /issue "Add CSV export for evaluation results"
  ...
  Scenario: Creates chore with template body sections
    Given the user runs /issue "Upgrade Prisma to v6"
  ...
  Scenario: Assigns issue to current GitHub user
    Given the user runs /issue "Fix pagination in traces view"
  ...
  Scenario: Adds issue to LangWatch Kanban project with default status
    Given the user runs /issue "Add dark mode support"
    And the user confirms the detected type
    When the skill creates the issue
  ...
  Scenario: Sets optional project fields when user specifies them
    Given the user runs /issue "Add dark mode support" with priority P1 and size M
    And the user confirms the detected type
    When the skill creates the issue
  ...
  Scenario: Sets Epic project field when user specifies an epic category
    Given the user runs /issue "Fix trace filtering" with epic "Traces UI/UX Extreme Makeover"
    And the user confirms the detected type
    When the skill creates the issue
  ...
  Scenario: Links issue as sub-issue of parent epic
    Given the user runs /issue "Fix trace date picker" with parent epic issue 500
    And the user confirms the detected type
    When the skill creates the issue
  ...
  Scenario: Skips sub-issue linking when no parent epic specified
    Given the user runs /issue "Update README" without specifying a parent epic
    And the user confirms the detected type
    When the skill creates the issue
  ...
  Scenario: Offers to launch implementation after creation
    Given the user runs /issue "Add webhook support"
    And the user confirms the detected type
