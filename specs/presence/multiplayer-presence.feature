Feature: Multiplayer presence within a project
  ...
  # Payload guardrails
  #
  # Peer cursors DO ship, as a separate anchor-scoped channel. What the
  # guardrails below pin is the boundary between the two: the durable presence
  # record never carries pointer data, and the cursor channel never writes to
  # it.
  # ---------------------------------------------------------------------------

  @unit
  Scenario: The recorded location carries only lens, route, and view
    Given Alice reports her location with a cursor and a text selection attached
    When the server records her session
    Then the recorded location contains only lens, route, and view
    And the cursor and selection fields are discarded

  @unit
  Scenario: A pointer coordinate smuggled inside route or view is rejected
    Given Alice reports a location whose route or view carries a pointer coordinate
    When the server validates the payload
    Then the update is rejected as invalid input
    And no session is recorded for her

  @unit
  Scenario: Cursor ticks are relayed to peers but never stored in presence
    Given Alice has an active presence session in project A
    When Alice's client sends a cursor tick
    Then peers receive the tick on the cursor channel
    And Alice's recorded presence session still carries no coordinates
