Feature: HttpPollingPullerAdapter (universal HTTP-polling adapter)
  ...
  Scenario: Happy-path single-page pull
    ...
    And the completion projection marks the IngestionSource as fully drained
  ...
  Scenario: Cursor extraction handles missing field
    ...
    And the process wake schedules the next pull per the cron schedule, not immediately
