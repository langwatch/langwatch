...
| specs/scenarios/scenario-failure-handler.feature | "Use pre-assigned scenarioRunId when no events exist in Elasticsearch" | UPDATE | Handler now requires a pre-assigned scenarioRunId (returns early if missing); scenario premise about "no events exist" is outdated |
