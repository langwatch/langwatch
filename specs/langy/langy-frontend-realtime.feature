Feature: Langy consumes the event-sourced backend with optimized fetches and lightweight signals
  ...
  #
  # PR2 moved Langy conversations onto the platform event-sourcing framework:
  # a slim `langy_conversations` fold projection (spine, no content) and the
  # existing `langy_messages` map projection (per-message rows). This spec pins
  # the frontend contract that mirrors Traces V2: never push row data over the
  # wire in real time — push a lightweight per-conversation signal, then let
  # React Query cancel()+invalidate() and refetch the slim projection. Heavy
  # message history is read on demand, separately from the slim list.
  #
  # Companion specs:
  ...
  @integration
  Scenario: The recent-chats list reads the slim conversation projection
