...
Feature: The langy-agent manager follows the house service conventions
  ...
  Scenario: Configuration is loaded from the documented environment variables
    ...
    Then it reads LANGY_INTERNAL_SECRET, LANGY_MAX_WORKERS, LANGY_WORKER_IDLE_MS,
      LANGY_READINESS_TIMEOUT_MS, PORT and SESSIONS_ROOT
  ...
  # ===========================================================================
  # Agent-definition assets (AGENTS.md + skills) travel in the binary
  # ===========================================================================
  # Backed by services/langyagent/internal/assets/assets_test.go and
  # skills/_tests/{native-skills,publish-sync}.test.ts. The full pipeline is
  # documented in skills/README.md and
  # services/langyagent/internal/assets/README.md.

  Scenario: The worker's rules doc and skills need no runtime mount
    Given the manager binary was built
    When a worker pool starts
    Then AGENTS.md and the skills tree come from inside the binary
    And no init container, template volume, or seed step is involved

  Scenario: Every worker's rules doc names that project's real URLs
    Given the embedded AGENTS.md carries an endpoint placeholder
    When a worker is provisioned for a project
    Then the worker's AGENTS.md contains the project's resolved LangWatch URL
    And the placeholder text never appears in the worker's copy

  Scenario: The production image carries the published skills plus the Langy-only ones
    Given the image build overlays the compiled public skills into the embed directory
    When a Langy conversation starts in production
    Then every skill in the public directory is available to the assistant
    And the Langy-only github skill is available alongside them

  Scenario: The Langy-only github skill never reaches the public skills repo
    Given the github skill assumes Langy's provisioned worker environment
    When the public skills repo is synced from the skills workspace
    Then the sync output contains no github skill
