Feature: Langy conversation titles are derived, then refined by a cheap model
  As a Langy user
  ...
  #     which a cheap-model reactor dispatches after a finalized turn,
  #   - a manual rename (`conversation_metadata_updated` carrying a title) is
  #     sticky and an auto title never overrides it.
  # The fold tracks WHERE the current title came from via `titleSource`
  # (derived | auto | user); the reactor and the fold both read it to decide
  # whether a regeneration is allowed.
  ...
  # ============================================================================
  # Auto-regeneration after a turn, on a cheap model
  # ============================================================================

  Scenario: The first finalized turn regenerates a concise title
    Given a conversation whose title is still the first-message placeholder
    When the agent's first turn is finalized
  ...
  Scenario: Regeneration is throttled, not run on every turn
    ...
    And only every few turns does a fresh regeneration run
  ...
  Scenario: Title generation never breaks the turn
    ...
    When a finalized turn would otherwise regenerate the title
  ...
  Scenario: A manual rename sticks even after prior auto titles
    ...
    And subsequent auto regeneration is skipped
  ...
  Scenario: A generated title appears in the sidebar without a refresh
    ...
    When a conversation's title is regenerated
