Feature: Data retention regression safety
  ...
  Scenario: Pinning a trace does not change retention
    Given a project has 30-day retention for traces
    ...
    And trace "abc123" follows the 30-day retention policy
  ...
  Scenario: Retroactive retention update applies uniformly across all retention-managed tables
    ...
    When the admin applies 90-day retention to existing trace data
  ...
  Scenario: Batched fold projections use the tenant retention policy
    Given a tenant has 30-day retention for traces
    And multiple trace events are processed through a batched fold projection
    When the folded state is stored
    Then the stored trace data uses 30-day retention

  @regression @unit
  Scenario: Proactive orphan sweep removes discovered orphan records
    Given PostgreSQL has records for a trace that no longer exists in ClickHouse
    When the proactive orphan sweep runs for the tenant
    Then the orphaned PostgreSQL records for that trace are removed
