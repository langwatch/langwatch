Feature: Policy rules — regex policy on tools, MCP servers, URLs, and models
  # Ref: docs/ai-gateway/policy-rules.mdx
  ...
  # ─────────────────────────────────────────────────────────────────────────
  # §1. Tools dimension — policy_rules.tools
  # ─────────────────────────────────────────────────────────────────────────

  Scenario: Tool call with a deny-matched name returns 403 tool_not_allowed
    Given VK config policy_rules.tools.deny = ["^shell\\..*", "^filesystem\\.write$"]
  ...
  Scenario: Anthropic-shape tool definitions match the same deny regex
    Given VK config policy_rules.tools.deny = ["^shell\\..*"]
  ...
  Scenario: Allow-list with non-matching tool returns 403
    Given VK config policy_rules.tools.allow = ["^safe\\..*"]
  ...
  Scenario: Empty policy_rules.tools passes every tool through
    Given VK config policy_rules.tools is unset
    When a request carries tools: [{function: {name: "shell.exec"}}]
    Then the request is dispatched to the upstream provider (no policy block)

  # ─────────────────────────────────────────────────────────────────────────
  # §2. MCP dimension — policy_rules.mcp
  # ─────────────────────────────────────────────────────────────────────────

  Scenario: MCP server name matched by deny regex returns 403
    Given VK config policy_rules.mcp.deny = ["^unauthorized-mcp$"]
  ...
  Scenario: URL allow-list — only whitelisted hosts pass
    Given VK config policy_rules.urls.allow = ["docs\\.langwatch\\.ai", "api\\.openai\\.com"]
    And no deny list is set
  ...
  # ─────────────────────────────────────────────────────────────────────────
  # §4. Models dimension — policy_rules.models
  # Distinct from models_allowed (static allowlist). Policy_rules.models
  # is regex-based for pattern-level policy (e.g. ban any preview model).
  # ─────────────────────────────────────────────────────────────────────────

  Scenario: Model deny regex returns 403 model_not_allowed
    Given VK config policy_rules.models.deny = [".*-preview$"]
  ...
  Scenario: Model allow regex — non-matching model returns 403
    Given VK config policy_rules.models.allow = ["^gpt-5-mini$", "^claude-haiku"]
    When a request routes to model "gpt-5"
    Then the response status is 403 model_not_allowed
    And error.message clarifies the model is outside the allow-regex

  Scenario: Policy_rules.models composes with models_allowed (both must pass)
    Given VK config models_allowed = ["gpt-5-mini", "claude-haiku-4-5-20251001"]
    And VK config policy_rules.models.deny = [".*-haiku.*"]
  ...
  Scenario: Deny wins over allow when both match
    Given VK config policy_rules.tools.deny = ["^shell\\."]
    And VK config policy_rules.tools.allow = [".*exec.*"]
    When a request carries tools: [{function: {name: "shell.exec"}}]
    Then the response status is 403 tool_not_allowed (deny wins)

  Scenario: Invalid regex fails closed (iter 14)
    Given VK config policy_rules.tools.deny = ["[unclosed-bracket"]
    When any request comes through that VK
    Then the response status is 503
    And error.code equals "policy_rules_invalid_regex"
  ...
  Scenario: RE2 incompatibility — backreferences refuse
    Given VK config policy_rules.tools.deny = ["(foo)\\1"]
    When the VK bundle is compiled
    Then the pattern compile fails with an RE2 syntax error
    And 503 policy_rules_invalid_regex is returned on every request through the VK
  ...
  Scenario: Ordering — policy_rules runs AFTER cache-override, BEFORE guardrails
    Given VK config has both cache.mode "disable" and policy_rules.tools.deny = ["^shell\\."]
  ...
  Scenario: Blocked request returns the proper gwerrors envelope
    Given a request is blocked by policy_rules.tools.deny
    Then the response is 403
    And error.type = "tool_not_allowed" (for tools) or "url_not_allowed" / "model_not_allowed" for the other dimensions
    And the WARN log line "policy_rules_compile_failed" fires when a compile error breaks policy (v1)
    # Per-dimension span attribute (langwatch.policy.violation) and a
    # dedicated gateway_policy_violations_total counter are tracked as
  ...
  # ─────────────────────────────────────────────────────────────────────────
  # §8. RBAC — gatewayVirtualKeys:update governs who authors policy_rules
  # ─────────────────────────────────────────────────────────────────────────

  Scenario: MEMBER with virtualKeys:update can edit policy_rules on their own VK
    Given a user with role MEMBER
    And the user is the principal owner of a VK
    When they PATCH the VK's config.policy_rules
    Then the update succeeds
    And an audit row is written

  Scenario: VIEWER cannot edit policy_rules
    Given a user with role VIEWER
    When they PATCH any VK's config.policy_rules
