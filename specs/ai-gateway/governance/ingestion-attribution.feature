Feature: Ingestion attribution invariant — credential is authoritative; payload is advisory
  ...
  | --- | --- | --- | --- |
  | Gateway VK (`langwatch claude/codex/cursor/gemini`) | `vk-lw-*` (project + owner-scoped) | `langwatch.virtual_key_id`, `langwatch.user.id` | `VK.projectId` (resolved at subscriber) |
  ...
  Implementation lives at:
    - enterprise/modules/governance/process/src/ (activity-monitor)                (subscriber + stamping)
    - enterprise/modules/governance/process/src/repositories/prisma/prisma.governance-setup-state.repository.ts (hidden Gov)
    - enterprise/modules/governance/process/src/transport/api-rest/                              (receivers)
  ...
  Scenario: OCSF auto-emission on threshold-crossed (deferred adoption)
    ...
    When future work needs to push the same signal to OCSF for SIEM
        ...
        audit feed)
    Then an OCSF subscriber adopting the polling evaluator's signal is
