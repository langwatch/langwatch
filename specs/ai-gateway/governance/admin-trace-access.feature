Feature: Admin trace access — bird's-eye drill-in with persistent 'viewing as' banner + audit-log
  ...
  Implementation lives at:
    ...
    - packages/features/ops/web/src/surfaces/impersonation/impersonation-banner.tsx (persistent banner)
    - packages/features/ops/server/src/services/impersonation.service.ts
                                                                       (server-side context)
    - packages/enterprise/features/governance/server/src/ (governance audit)                            (OCSF emission)
  ...
  # AdminViewingAsBanner exists in DashboardLayout ([gone] src/components)
  ...
  Scenario: Banner is scoped to project-anchored URLs only — does NOT leak onto admin-self surfaces
    ...
    When carol navigates AWAY from the impersonating context to any
      ...
      | /me/configure        | personal-self config              |
