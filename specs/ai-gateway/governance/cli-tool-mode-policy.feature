Feature: Per-tool path policy - an org admin enables or disables each langwatch <tool> path
  ...
  cursor) can route two ways:
    ...
    - Path B (direct OTLP ingestion): the tool exports telemetry straight to the
      personal OTLP endpoint with an ingestion key. Controlled by
      `allowOtelDirect`.
  ...
  Implementation lives under:
    - platform/app/ee/governance/services/platformToolPolicy.service.ts   (resolve + upsert)
    - platform/app/ee/governance/routers/platformToolPolicy.ts            (tRPC list/update)
    - platform/app/ee/governance/services/cliBootstrap.service.ts         (login payload)
  ...
  Rule: the CLI gates path selection on the resolved policy
    ...
    Scenario: OTLP direct disabled with a virtual key present forces the gateway path
      ...
      And it does not mint an ingestion key
