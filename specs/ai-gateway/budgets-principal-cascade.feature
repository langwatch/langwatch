Feature: AI Gateway — Per-user (PRINCIPAL) budgets in the strictest-wins cascade
  ...
  # ============================================================================
  # Department budgets ride the same cascade, one bucket per member
  # ============================================================================
  #
  # A department (GROUP) budget is the per-person tier expressed once: "people
  # in this department each get this much". It resolves through the same
  ...
  Scenario: A department budget resolves to the member's own bucket
    Given department "engineering" has a $50/month budget and alice is a member
    When alice's key is resolved
    Then the budget applies to alice
    And its spend accrues to alice alone, not to the department as a whole

  @bdd @phase-1b @principal-cascade @group
  Scenario: Two members of the same department do not share the allowance
    Given department "engineering" has a $50/month budget with alice and bob in it
  ...
  @bdd @phase-1b @principal-cascade @group
  Scenario: Joining a department picks up its budget on the next resolve
    Given department "engineering" has a budget and carol is not a member
    When carol joins the department
    And her key's configuration is resolved again
    Then the department budget now applies to carol

  @bdd @phase-1b @principal-cascade @group
  Scenario: Leaving a department drops its budget
    Given carol is in department "engineering" which has a budget
    When carol leaves
    And her key's configuration is resolved again
    Then the department budget no longer applies to her
    And the spend she already recorded stays in the ledger

  @bdd @phase-1b @principal-cascade @group
  Scenario: A key with no person behind it is out of reach of department budgets
    Given department "engineering" has a budget
    When a shared service key with no principal is resolved
    Then no department budget applies to it
