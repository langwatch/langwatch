Feature: Period selector remembers relative ranges as relative
  ...
  Scenario: Switching from absolute back to a relative quick selector clears absolute params
    ...
    Then the URL contains the relative period key
    And the URL does not contain "startDate" or "endDate"
  ...
  Scenario: An inverted date range in the URL is read the right way round
    Given the URL's startDate is later than its endDate
    When the page loads
    Then the resolved start is not after the resolved end
    And the span between them is not negative

  # Nothing renders the selector button, so what its label says in either mode
  # is unasserted.
