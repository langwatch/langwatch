Feature: Inviting a teammate through a drawer
  ...
  #
  # The two entry points that live inside the members *page* (the "Add members"
  # button and the inline email box) are parked: both are declared inline in
  # pages/settings/members.tsx, which no test renders. What the drawer does with
  # what they hand it is covered below.
  # ---------------------------------------------------------------------------
  ...
  Scenario: Typing an email inline opens the drawer carrying that email
    ...
    Then the invite-member drawer opens carrying what they typed

  @integration
  Scenario: The drawer opens pre-filled with an email typed elsewhere
    Given the invite-member drawer is opened carrying an email
    When the drawer renders
    Then the drawer's email field is pre-filled with that email
    And a drawer opened with nothing typed shows an empty email field
