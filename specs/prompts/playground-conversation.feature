Feature: Prompt Playground conversation
  ...
  Scenario: A turn is separated and carries its trace to the host
    Given a turn in the conversation carries a trace
    When the shared renderer draws the thread
    Then the turn separator is given the turn number and that trace
