Feature: The auth boundary is classes over identity services
  ...
  Scenario: better-auth never opens the database itself
    ...
    Then no unlisted source imports the app's database client or a Prisma client for its value
  ...
  Scenario: Prisma is spelled in the repository tier only
    ...
    Then a query appears only in a repository or adapter file or a named current-main SSO residual
  ...
  Scenario: A question about the data is asked in one place
    ...
    Then no unlisted source spells a case-insensitive email match
    And no unlisted source looks an organization up by its legacy SSO domain
