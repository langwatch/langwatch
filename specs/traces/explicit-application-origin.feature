Feature: Explicit application origin for race condition prevention
  ...
  # evaluation trigger process manager fires after 30s, sees empty origin, and the
  ...
  #   2. Empty origin = pending (not application) — the process manager and
  ...
  #      The process manager schedules a 5-min deferred check. On fire, re-read fold
  ...
  # --- Evaluation trigger process-manager guards ---
  ...
  Scenario: Evaluation trigger skips traces with empty origin and no SDK info
    ...
    When the evaluation trigger process manager fires at normal debounce
  ...
  Scenario: Evaluation trigger dispatches for any known origin (preconditions filter)
    ...
    When the evaluation trigger process manager fires at normal debounce
    Then evaluation commands are dispatched for matching monitors
    Because origin filtering is handled by precondition matchers, not the process manager

  # --- SDK version heuristic for old SDK backward compat ---
  # The fold projection infers origin = "application" when sdk.name is present
  # but no explicit origin or legacy markers exist. By the time the process
  # manager fires, the fold state already has langwatch.origin set.
  ...
  Scenario: Fold projection infers application origin for old SDK traces
    ...
    And when the evaluation trigger process manager fires at normal debounce
    Then evaluation commands are dispatched for matching monitors
  ...
  # The single process manager handles both phases — no separate deferred process manager.
  ...
  Scenario: Deferred check deduplicates per trace
    ...
    And each process-manager dispatch schedules a deferred check
