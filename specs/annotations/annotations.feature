Feature: Annotating traces
  ...
  # Every scenario is @unimplemented, which here means UNBOUND rather than
  # unbuilt. Most of this behaviour ships today and has tests; what does not
  # exist is the `@scenario` annotation tying each scenario to the test that
  # covers it. Tagging them @unit or @integration without doing that binding
  # would report coverage this file does not have, so they are parked until
  # someone walks the list and binds them one at a time.
  #
  # Related, and deliberately not merged into: annotations-list-selection.feature
  # covers the list's selection mechanics and annotation-queue-workflow.feature
  # covers moving through a queue. This file is the annotation domain itself.
