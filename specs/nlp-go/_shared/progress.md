...
- [x] Topic clustering migrated to langevals (ash) — workspace member at `services/langevals/evaluators/topic_clustering/`, the Topic server runner routes to langevals, lambda module + API Gateway routes in `langwatch-saas#460`. Tests skipped in CI per rchaves; manual exercise points at langevals on :5561.
