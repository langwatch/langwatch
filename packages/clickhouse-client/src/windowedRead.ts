 ...
 * narrowed its scan to two days either side of an approximate time. Generous on purpose, and the
...
/** Every way a windowed read can end, as `clickhouse_windowed_read_total` counts it. */
...
export interface WindowedReadMetrics {
 ...
 */
let windowedReadMetrics: WindowedReadMetrics | null = null;

/** Registers the process's counter. Called once, at composition. */
export function setWindowedReadMetrics(port: WindowedReadMetrics): void {
  windowedReadMetrics = port;
}

function incrementWindowedReadCount(table: string, outcome: WindowedReadOutcome): void {
