 ...
 * Mutual exclusion, proved the only way it can be: each contender is its own
 * node process contending for one lock file, exactly as two vitest workers do.
 */
...
const CONTENDER = join(import.meta.dirname, "fixtures", "schema-lock-contender.ts");
...
function startContender({ id, holdMs }: { id: string; holdMs: number }): Promise<unknown> {
  return run(process.execPath, [TSX_CLI, CONTENDER, lockPath, journalPath, id, String(holdMs)], {
    timeout: 60_000,
  });
}
...
describe("given several processes want the schema lock at once", () => {
  describe("when they all start together", () => {
    ...
    it("lets exactly one hold it at a time", async () => {
      const contenders = ["a", "b", "c", "d"].map((id) => startContender({ id, holdMs: 120 }));
