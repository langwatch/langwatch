 ...
 * The lock's own lifecycle: what one process does with the lock file, and the
 * recovery protocol's refusal to remove a lock it did not inspect.
 */
...
describe("given the process that held the lock is gone", () => {
  ...
  describe("when another suite is already recovering that same holder", () => {
    ...
    it("leaves the lock alone rather than racing the other recovery", async () => {
      ...
      await expect(lock.acquire()).rejects.toThrow(/waiting for the ClickHouse schema lock/);
