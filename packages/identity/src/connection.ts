...
/** Human-supplied attestation evidence is deliberately small enough to be
 * safe in the event log and useful in an operator review. */
...
/** One proof record at the portable JSON boundary. Fields introduced after
 * the original projection are optional so historical rows decode honestly as
 * unknown rather than being fabricated into stronger evidence. */
...
/**
 * The persisted cutover state for a direct connection replacing a
 * grandfathered Auth0 connection. Routing reads this state; timestamps are
 * evidence for people and audit logs, never an implicit routing decision.
 */
...
export const MIGRATION_ROUTE_SELECTED_EVENT_TYPE =
  "lw.identity.migration_route_selected" as const;
export const MIGRATION_FINALIZATION_STARTED_EVENT_TYPE =
  "lw.identity.migration_finalization_started" as const;
export const MIGRATION_FINALIZED_EVENT_TYPE =
  "lw.identity.migration_finalized" as const;
...
export const replacementConnectionRegisteredPayloadSchema =
  connectionRegisteredPayloadSchema.extend({
    replacesConnectionId: z.string().min(1),
    inheritedDomainVerifications: z
      .array(ssoDomainVerificationSchema)
      .default([]),
  });
...
export interface SsoConnectionState {
  ...
  /** The grandfathered connection this direct connection replaces. */
  replacesConnectionId: string | null;
  /** Explicit cutover state. Null for connections outside a migration pair. */
  ...
  switch (fact.type) {
    ...
    case REPLACEMENT_CONNECTION_REGISTERED_EVENT_TYPE:
      return {
        ...
        type: fact.data.type,
        state:
          fact.data.inheritedDomainVerifications.length > 0
            ? "VERIFIED"
            : "DRAFT",
        idpMetadata: fact.data.idp,
        arrivalPolicy: fact.data.arrivalPolicy,
        ...
        verifiedDomains: fact.data.inheritedDomainVerifications.map(
          (proof) => proof.domain,
        ),
    ...
    case MIGRATION_ROUTE_SELECTED_EVENT_TYPE:
      return {
        ...touched,
        migrationPhase:
          fact.data.route === "legacy" ? "GRACE_LEGACY" : "GRACE_DIRECT",
...
export function qualifySsoDomainOwnership({
  ...
  const proof = state.domainVerifications.find(
  ...
  if (proof.method === "dns-txt" || proof.method === "https-file") {
    const evidenceRef = proof.evidenceRef ?? proof.tokenHash;
    if (proof.verifiedAtMs <= 0 || !evidenceRef) {
      return { status: "UNKNOWN", reason: "incomplete" };
    }
    return {
      status: "QUALIFIED",
      proof: { ...proof, evidenceRef },
    };
  }

  if (proof.method === "operator-attested") {
    const evidenceRef = ssoAttestationEvidenceRefSchema.safeParse(
      proof.evidenceRef,
    );
    const note = ssoAttestationNoteSchema.safeParse(proof.note);
...
export function isConfiguredLegacySsoRoute(
  state: { source: SsoConnectionSource; providerId: string },
): boolean {
  return (
    state.source === "legacy-grandfathered" &&
    state.providerId.length > 0
  );
}

/** The selected route is persisted migration state, never a timestamp. */
export function selectedSsoMigrationRoute(
  state: { migrationPhase: SsoMigrationPhase | null },
): SsoMigrationRoute | null {
  switch (state.migrationPhase) {
    case "SETUP":
    case "GRACE_LEGACY":
      return "legacy";
    case "GRACE_DIRECT":
