...
/** The persisted migration and its operational proof as Settings renders it. */
...
/**
 * Operational reads for the migration pair. The aggregate owns route and
 * phase; this port supplies membership, authentication and SCIM evidence that
 * must not be folded into the connection merely so a settings page can count
 * it.
 */
...
export interface SelfServeSetupView {
  ...
  /** Present only while this organization has a grandfathered-to-direct pair. */
