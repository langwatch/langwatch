...
import type { TransactionAdapter } from "./context";
import { routeWrite, type WriteOperation } from "./routing";
 ...
 * transaction); a latched user's domain write in here would be an event
 * gap — logged so the flow that introduces one is seen. Today no
