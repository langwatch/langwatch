...
/*
 * Everything still required externally must be declared in this package's
...
const isInlined = (id) => {
  ...
  if (OPTIONAL_EXTERNALS.includes(basePackage(id))) return false;
...
for (const { name, entry } of ENTRIES) {
  const result = await build({
    ...
    outfile: path.join(OUT_DIR, `${name}.mjs`),
    absWorkingDir: APP,
...
if (undeclared.size > 0) {
  ...
  console.error(
    "  The bundle resolves external requires from node_modules at runtime; a --prod install only ships `dependencies`. Declare the packages above in this package's dependencies.",
  );
}
