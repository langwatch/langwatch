...
const BUNDLE_PATH = path.join(PACKAGE_ROOT, "dist", "server", "scenario-child-process.mjs");
...
describe("Pre-compiled Scenario Child Process", () => {
  describe("when the child process build step runs", () => {
    ...
    it("produces a single JavaScript file at dist/server/scenario-child-process.mjs", () => {
    ...
    it("keeps OpenTelemetry external so exactly one API instance exists", () => {
      ...
      // this must stay an external ESM import and no copy may be inlined.
      expect(content).toContain('from "@opentelemetry/api"');
    ...
    it("boots without MODULE_NOT_FOUND — every externalized require() resolves in a prod-shaped layout", () => {
      ...
      // Externalized dependencies appear as real ESM import STATEMENTS in the
      // bundle (each emitted on its own line), never as a bare `require("x")`
      // any more. Scanning whole-file text for `from "x"` also matches that
      // substring inside ordinary string literals, so only lines that
      // themselves start with the `import` keyword are considered.
