...
function openEntry(params: { runId: string; startedAt: number }): TopicClusteringRunHistoryEntry {
  return {
    runId: params.runId,
...
function withRun(
  ...
  const runs =
    index === -1
      ? [update(openEntry({ runId: params.runId, startedAt: params.occurredAt })), ...settled]
