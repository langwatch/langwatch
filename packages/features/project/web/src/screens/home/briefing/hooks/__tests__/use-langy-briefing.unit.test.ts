 ...
 * `currentPeriod` bucket; a key that drifts from the app-layer's reads 0.
...
describe("readGroupedSummaryMetric", () => {
  const series: SeriesInputType[] = [{ metric: "metadata.trace_id", aggregation: "cardinality" }];
  const key = buildSeriesName(series[0]!, 0);
...
/**
 * `buildAttentionInbox` turns current/prior facets into the attention inbox.
 * The assertions pin prioritization and the no-invented-cause contract.
 * Spec: specs/home/langy-briefing.feature
 */
describe("buildAttentionInbox", () => {
  const base: AttentionInboxSignals = {
    slug: "acme",
    currentErrorShapes: [],
    previousErrorShapes: [],
  ...
  describe("given a meaningful p50 regression", () => {
    ...
    it("compares periods, and omits small changes or a latency with no baseline", () => {
      const regressed = buildAttentionInbox({
        ...base,
        p50Latency: 2_000,
        previousP50Latency: 1_000,
      });
      const latency = byId(regressed, "latency-regression");
      ...
      expect(
        byId(
          buildAttentionInbox({ ...base, p50Latency: 1_200, previousP50Latency: 1_000 }),
          "latency-regression",
        ),
      ).toBeUndefined();
      expect(
        byId(
          buildAttentionInbox({ ...base, p50Latency: 8_000, previousP50Latency: undefined }),
          "latency-regression",
        ),
      ).toBeUndefined();
    });
  ...
  describe("when many supported signals fire at once", () => {
    ...
    it("orders changed errors, shared signals, and latency before repeated noise", () => {
      // A project with only totals to report contributes no attention row at
      // all: trace, token, user and cost counts live in the quiet overview.
      expect(buildAttentionInbox(base)).toEqual([]);
