/**
 * Ported from platform/app/src/components/home/__tests__/WelcomeHeader.test.ts
 * (origin/main); `getGreetingName`/`getTimeOfDay`/`getGreeting`'s own shape is
  ...
  describe("getGreeting", () => {
    describe("when name is provided", () => {
      ...
      it("returns personalized afternoon greeting", () => {
        expect(getGreeting({ timeOfDay: "afternoon", name: "Bob" })).toBe("Good afternoon, Bob");
      });

      it("returns personalized evening greeting", () => {
        expect(getGreeting({ timeOfDay: "evening", name: "Carol" })).toBe("Good evening, Carol");
      });
    });
    ...
    describe("when name is null", () => {
      ...
      it("returns anonymous afternoon greeting", () => {
        expect(getGreeting({ timeOfDay: "afternoon", name: null })).toBe("Good afternoon");
      });
