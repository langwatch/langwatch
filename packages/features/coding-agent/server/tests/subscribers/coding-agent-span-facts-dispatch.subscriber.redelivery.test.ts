...
import { CodingAgentTraceProcessingPort } from "../../src/ports/coding-agent-trace-processing.port";
import { createCodingAgentSpanFactsDispatchSubscriber } from "../../src/subscribers/coding-agent-span-facts-dispatch.subscriber";
...
function makeSubscriber(
  ...
  class TestTraceProcessing extends CodingAgentTraceProcessingPort {
    normalizeSpan(input: {
      tenantId: string;
      span: OtlpSpan;
  ...
  const subscriber = createCodingAgentSpanFactsDispatchSubscriber({
    ...
    traces: new TestTraceProcessing(),
  });
...
describe("codingAgentSpanFactsDispatch", () => {
  describe("when the same canonical span is redelivered", () => {
    it("resolves to one durable span contribution identity", async () => {
      const { subscriber, dispatched } = makeSubscriber();
      const event = rawSpanEvent({
        name: "claude_code.tool",
        spanId: "span-redelivery",
        attributes: { "session.id": "sess-redelivery", tool_name: "Bash" },
      });
      ...
      const durable = new Map(
        dispatched.map((contribution) => [
          `${contribution.tenantId}:${contribution.traceId}:${contribution.spanId}`,
          contribution,
        ]),
      );
  ...
  describe("when a span from an ordinary LLM trace passes by", () => {
    ...
    it("is ignored without decoding it", async () => {
      ...
      await subscriber.handle(rawSpanEvent({ name: "openai.chat", spanId: "s-1" }), context);
  ...
  describe("given the enqueue-time stage hook (ADR-069 R2)", () => {
    describe("when a matched coding-agent span is staged", () => {
      ...
      it("carries the small facts and none of the span's content", () => {
        ...
        const lifted = parseSpanFactsLiftedPayload(payload) as SpanFactsLiftedPayload | null;
        expect(lifted).not.toBeNull();
      ...
      it("keys identically whether staged as a lift or as a claim-check", () => {
        ...
        const liftedId = dedup.makeId(staged(subscriber.options?.enqueue?.stage?.(event)));
    ...
    describe("when the span cannot be lifted at the seam", () => {
      ...
      it("completes the staged fallback instead of retrying it into a blocked group", async () => {
        ...
        await expect(subscriber.handle(staged(payload), context)).resolves.toBeUndefined();
  ...
  describe("given the dedup key", () => {
    ...
    describe("when two spans in one trace carry no wire span id", () => {
      // An id-less span stages as a reference carrying an empty span id, so
      // this is a reachable production path, not a hypothetical. Keying it on
      // that empty span id would make both
  ...
  describe("given a claim-check staged job (ADR-069)", () => {
    ...
    describe("when the wire span carried no usable start", () => {
      ...
      it("stages the full event rather than a reference the read could not resolve", async () => {
        ...
        // The legacy reference producer kept this full event because its
        // zero start could not centre a reliable span-store lookup.
        await subscriber.handle(event, context);
    ...
    describe("when the reference carries a version this build does not know", () => {
      ...
      it("throws into the queue's retry instead of no-opping as another shape", async () => {
        ...
        const { subscriber, dispatched } = makeSubscriber(async () => normalizedFrom(event));
        ...
        expect((failure as ZodError).issues.map((issue) => issue.path.join("."))).toContain(
          "version",
        );
        expect(dispatched).toHaveLength(0);
      });
  ...
  describe("given a lifted-derivation staged job (ADR-069)", () => {
    describe("when the job carries the facts already lifted", () => {
      ...
      it("contributes them without reading the span store at all", async () => {
        ...
        await subscriber.handle(staged(liftedPayload({ spanId: "tool-lifted" })), context);
      ...
      it("succeeds even when the span never lands in the store", async () => {
        ...
        await subscriber.handle(staged(liftedPayload({ spanId: "tool-never-stored" })), context);
  ...
  describe("given a staged payload this build cannot read", () => {
    ...
    describe("when the type is span_received but the body carries no span", () => {
      ...
      it("throws instead of letting the name gate decline it", async () => {
        ...
        await expect(
          subscriber.handle({ ...envelope, data: {} } as unknown as TraceProcessingEvent, context),
