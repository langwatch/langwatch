...
import { EventingContributeLogFactsAdapter } from "../../src/adapters/eventing.contribute-log-facts.adapter";
import { EventingContributeMetricFactsAdapter } from "../../src/adapters/eventing.contribute-metric-facts.adapter";
import { EventingContributeSpanFactsAdapter } from "../../src/adapters/eventing.contribute-span-facts.adapter";
...
describe("EventingContributeSpanFactsAdapter", () => {
  describe("when a coding-agent span's facts are contributed", () => {
    it("emits one session-keyed span_facts_contributed event", async () => {
      const handler = EventingContributeSpanFactsAdapter.create();
...
describe("EventingContributeLogFactsAdapter", () => {
  describe("when a log with no correlation contributes", () => {
    ...
    it("carries the facts with a null trace id", async () => {
      const handler = EventingContributeLogFactsAdapter.create();
      const events = await handler.handle(
