...
function renderTable() {
  host = fakeCodingAgentActivityHost({
    permissions: permissionsRef.canManageOrganization ? ["organization:manage"] : [],
  });
...
describe("the personal Pull Requests table", () => {
  ...
  describe("given a session whose repository no installation covers", () => {
    ...
    it("offers an organization manager the option to link that repository", async () => {
      ...
      await user.click(screen.getByRole("button", { name: "Actions for feat/git-context" }));
  ...
  describe("given a mapped pull request whose status came from a snapshot", () => {
    ...
    it("draws a snapshot status back and says when it is from", async () => {
      ...
      expect(tooltip).toHaveTextContent(new Date("2026-07-01T10:00:00Z").toLocaleDateString());
    });
  });
  ...
  describe("given a listed pull request", () => {
    ...
    it("opens the detail for that pull request when the row is clicked", async () => {
      ...
      // The address is what opens the detail now: the table renders the drawer
      // inline from its own query key rather than through the application's
      // drawer registry, so the assertion is on what it wrote to the address.
      expect(host.recording.queries).toEqual([
        { next: { pullRequest: "github.com|acme/widgets|4218" }, replace: false },
      ]);
    });
    ...
    it("does not open the detail when the row's actions menu is used", async () => {
      ...
      await user.click(screen.getByRole("button", { name: "Actions for pull request 4218" }));
