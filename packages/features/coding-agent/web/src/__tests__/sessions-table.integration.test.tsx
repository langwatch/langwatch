 ...
 */
const tableElement = () => <SessionsTable projectId="proj-personal" projectSlug="acme-personal" />;

/** The surface the table is mounted in, rebuilt per render. */
let host: FakeCodingAgentActivityHost;

function renderTable() {
  host = fakeCodingAgentActivityHost();
  return render(tableElement(), { wrapper: codingAgentHostWrapper(host) });
}

/** Every address the table wrote, in order. */
const writtenQueries = () => host.recording.queries.map((write) => write.next);
...
describe("the personal Sessions table", () => {
  describe("given a personal workspace with coding-agent sessions", () => {
    ...
    it("gives each session a row carrying its context, compactions, rebuilds and waiting time", () => {
      ...
      expect(screen.getByRole("button", { name: "Open pull request #4218" })).toBeInTheDocument();
    });
    ...
    it("puts the tokens a compaction dropped behind the count", async () => {
      ...
      expect(await screen.findByText("Compacted from 180.0K to 42.0K tokens")).toBeInTheDocument();
      expect(await screen.findByText("Largest cache rebuild 90.0K tokens")).toBeInTheDocument();
    });

    it("reads a session that never compacted or rebuilt as having neither", () => {
      pinSessions([sessionRow({ compactions: 0, cacheRebuildCount: 0, pullRequests: [] })]);
  ...
  describe("given a session row that lists a pull request", () => {
    ...
    it("opens the pull request's detail and not the session replay", async () => {
      ...
      // The detail opens from the table's own query key now, rather than
      // through the application's drawer registry.
  ...
  describe("given a session that drove more pull requests than the row lists", () => {
    it("links the first few and puts the rest behind a hover", async () => {
      ...
      expect(await screen.findByText("#4222 Pull request 4222")).toBeInTheDocument();
    });
  });
  ...
  describe("given a listed page of sessions", () => {
    ...
    it("sorts from a heading, reverses on the second choice, and returns to the opening order on the third", async () => {
      ...
      expect(listedTitles()).toEqual(["Heavy session", "Middling session", "Light session"]);

      await user.click(screen.getByRole("button", { name: "Sort by Context" }));
      expect(headingFor("Context")).toHaveAttribute("aria-sort", "ascending");
      expect(listedTitles()).toEqual(["Light session", "Middling session", "Heavy session"]);
  ...
  describe("given a listed session", () => {
    ...
    it("reports a failed lookup rather than opening an empty replay", async () => {
      ...
      await waitFor(() => expect(host.recording.failures.length).toBe(1));
      expect(writtenQueries()).toEqual([]);
    });
