  ...
  >(
    ...
    return (
      createTrpcService({ root: trpc, procedures })
        ...
        .mutation("create", (p) =>
          p
            .withInput(createSuiteSchema)
            .withoutOutput("the created suite row, as the application composes it")
            .withPermission("scenarios:manage")
            .handle(async ({ ctx, input }) => ctx.app.suites.create(input)),
        )
        ...
        .query("getAll", (p) =>
          p
            ...
            .withoutOutput(
              "two row shapes in one list, both the application's; naming one here would refuse the other",
            )
        ...
        .query("getById", (p) =>
          p
            .withInput(projectSchema.extend({ id: z.string() }))
            .withoutOutput("a suite or a test suite, whichever the id named")
            .withPermission("scenarios:view")
            .handle(async ({ ctx, input }) => {
        ...
        .mutation("duplicate", (p) =>
          p
            .withInput(projectSchema.extend({ id: z.string() }))
            .withoutOutput("the copied suite row, as the application composes it")
            .withPermission("scenarios:manage")
            .handle(async ({ ctx, input }) => ctx.app.suites.duplicate(input)),
        )
        .mutation("archive", (p) =>
          p
            .withInput(projectSchema.extend({ id: z.string() }))
            .withoutOutput("the archived suite row, as the application composes it")
            .withPermission("scenarios:manage")
            .handle(async ({ ctx, input }) => ctx.app.suites.archive(input)),
        )
        .query("resolveArchivedNames", (p) =>
          p
            .withInput(
              z.object({
                projectId: z.string(),
            ...
            .withoutOutput("the application's own name lookup, keyed as it keys it")
            .withPermission("scenarios:view")
            .handle(async ({ ctx, input }) => ctx.app.suites.resolveArchivedNames(input)),
        )
        .mutation("run", (p) =>
          p
            .withInput(
            ...
            .withoutOutput("the scheduling receipt the application returns, spread as it stands")
            .withPermission("scenarios:manage")
            .handle(async ({ ctx, input }) => {
