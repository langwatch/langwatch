...
import { ClickHouseSuiteRunRepository } from "../src/repositories/clickhouse/clickhouse.suite-run.repository";
...
describe("ClickHouseSuiteRunRepository", () => {
  it("maps the complete suite run state shape", async () => {
    ...
    await expect(repository.getSuiteRunState({
      projectId: "project_1",
      batchRunId: "batch_1",
  ...
  it("reads and writes the same row shape used by the Eventing fold store", async () => {
    ...
    await expect(repository.getProjection("batch_1", { tenantId: createTenantId("project_1") }))
    ...
    await repository.storeProjection({
      ...
      version: "2026-08-25",
      data: stateRow,
    }, { tenantId: createTenantId("project_1"), metadata: { retentionPolicy: { scenarios: 14 } } });
  ...
  it("wraps ClickHouse resolution failures instead of silently succeeding", async () => {
    const repository = ClickHouseSuiteRunRepository.create({
      resolveClient: async () => { throw new Error("clickhouse unavailable"); },
      defaultRetentionDays: 30,
    });
    await expect(repository.getProjection("b", { tenantId: createTenantId("p") })).rejects.toBeInstanceOf(StoreError);
  });

  it("rejects a projection written for another tenant", async () => {
    const { repository } = setup([]);
    await expect(repository.storeProjection({
      id: "projection_2",
      aggregateId: "batch_1",
      tenantId: createTenantId("other"),
      version: "2026-08-25",
      data: stateRow,
    }, { tenantId: createTenantId("project_1") })).rejects.toBeInstanceOf(SecurityError);
  });
