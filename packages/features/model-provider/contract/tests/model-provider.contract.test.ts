...
describe("Model Provider contract", () => {
  it("requires an explicit tenant anchor", () => {
    expect(() => modelProviderWriteInputSchema.parse({ provider: "openai", enabled: true })).toThrow();
    expect(modelProviderWriteInputSchema.parse({ projectId: "project_1", provider: "openai", enabled: true }).projectId).toBe("project_1");
  });

  it("keeps scopes portable and strict", () => {
    expect(modelProviderScopeSchema.safeParse({ scopeType: "PROJECT", scopeId: "p1" }).success).toBe(true);
    expect(modelProviderScopeSchema.safeParse({ scopeType: "PROJECT", scopeId: "p1", prisma: true }).success).toBe(false);
  });
  ...
  it("bounds translation input at the contract boundary", () => {
    ...
    expect(translateInputSchema.safeParse({ projectId: "p1", text: "x".repeat(100_001) }).success).toBe(false);
  });

  it("owns the custom-model and Codex credential schemas", () => {
    expect(customModelEntrySchema.safeParse({
      modelId: "my-model",
    ...
    expect(codexTokenKeysSchema.safeParse({
