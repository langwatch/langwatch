...
export class PrismaModelDefaultRepository extends ModelDefaultRepository {
  ...
  static create(database: object): PrismaModelDefaultRepository {
    return new PrismaModelDefaultRepository(database as RootDatabase);
  }
  ...
  async getProjectContext(projectId: string): Promise<{
    ...
    return {
      ...
      organizationName: project.team.organization.name,
    };
  }
  async listOrganizationScopes(organizationId: string): Promise<{
  ...
  async save(
    input: Omit<ModelDefaultConfig, "createdAt"> & { createdAt?: Date },
  ): Promise<ModelDefaultConfig> {
    const organizationId = await this.organizationIdForScopes(input.scopes);
    return this.database.$transaction(async (database) =>
      this.saveInTransaction(database, input, organizationId),
    );
  }
    ...
    await this.database.$transaction(async (database) => {
      ...
      await this.saveInTransaction(
        database,
        {
          id:
            existing?.id ??
            `model_default_${Date.now()}_${Math.random().toString(36).slice(2)}`,
          config,
          scopes: [input.scope],
          authorId: input.authorId,
        },
  ...
  async tryResolve(input: {
    ...
    const priority: Record<ModelProviderScopeType, number> = {
      PROJECT: 3,
      TEAM: 2,
      ORGANIZATION: 1,
    };
    const matching = configs.flatMap((config) =>
      config.scopes.flatMap((scope) =>
        config.config[input.featureKey]
          ? [
              {
                value: config.config[input.featureKey]!,
                priority: priority[scope.scopeType],
              },
            ]
          : [],
      ),
  ...
  private async organizationForScope(scope: ModelDefaultScope): Promise<string> {
    ...
    if (scope.scopeType === "TEAM") {
      const row = await this.database.team.findUnique({
        where: { id: scope.scopeId },
  ...
  private async organizationIdForScopes(scopes: ModelDefaultScope[]): Promise<string> {
    if (scopes.length === 0)
      throw new Error("At least one model default scope is required");
    const organizations = await Promise.all(
      scopes.map((scope) => this.organizationForScope(scope)),
    );
    const organizationId = organizations[0]!;
    if (organizations.some((id) => id !== organizationId))
      throw new Error("Model default scopes must belong to one organization");
