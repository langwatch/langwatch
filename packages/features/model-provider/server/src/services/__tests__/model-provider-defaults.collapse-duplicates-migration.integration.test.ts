...
describe.skipIf(!DB_URL)("given configs written before the one-config-per-scope rule (real Postgres)", () => {
    ...
    it("keeps the config the resolver already picked and drops the rest", async () => {
      ...
      await expect(
        prisma.$transaction(async (tx) => {
          ...
          expect(await tx.modelDefaultConfig.findUnique({ where: { id: tiedLoserId } })).toBeNull();
