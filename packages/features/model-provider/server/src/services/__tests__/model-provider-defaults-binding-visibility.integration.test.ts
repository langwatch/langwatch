...
describe.skipIf(!DB_URL)("Default Models visibility for role-binding-only members (real Postgres)", () => {
  ...
  function bindingComputingAuthz(): AuthzService {
    return {
      getDecision: async (input: { userId: string; permission: string; scope: { tier: string; id: string } }) => {
  ...
  beforeAll(async () => {
    ...
    await prisma.roleBinding.createMany({
      data: [
        { organizationId, userId: member.id, role: "MEMBER", scopeType: "ORGANIZATION", scopeId: organizationId },
  ...
  describe("when the member's project access comes from a TEAM-scope binding", () => {
    ...
    it("lists the project-scoped configs in the Default Models snapshot", async () => {
      const service = ModelProviderDefaultsService.create({
        ...
        providers: PrismaModelProviderRepository.create(prisma, new IdentityModelProviderCredentialCodec()),
