...
describe.skipIf(!DB_URL)("Multi-instance provider rows and runtime row selection (real Postgres)", () => {
  ...
  beforeAll(async () => {
    ...
    const otherTeam = await prisma.team.create({
      data: { name: `Other Team ${ns}`, slug: `--team-b-${ns}`, organizationId: fixture.organizationId },
    });
  ...
  describe("given a write naming multiple scopes", () => {
    ...
    it("saves a single row with multiple ModelProviderScope entries", async () => {
      ...
      const storedScopes = (stored?.scopes ?? []).map((s) => `${s.scopeType}:${s.scopeId}`).sort();
  ...
  describe("when the collapse winner does not serve the resolved model", () => {
    ...
    it("prepares the call with the org row's credentials for the org-catalog model", async () => {
      const params = await execution.prepare({ model: "azure/gpt-5.4-mini", projectId: fixture.projectId });
  ...
  describe("when the resolved model is a registry model", () => {
    ...
    it("keeps the collapse winner (narrowest enabled row)", async () => {
      ...
      const params = await execution.prepare({ model: `groq/${registryModel}`, projectId: fixture.projectId });
    ...
    it("keeps the winner's credentials when a wider row lists the model in its custom catalog", async () => {
      ...
      try {
        ...
        await prisma.modelProvider.update({ where: { id: orgRow!.id }, data: { customModels: [] } });
      }
    });
  });
