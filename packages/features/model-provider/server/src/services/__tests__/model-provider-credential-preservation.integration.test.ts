...
describe.skipIf(!DB_URL)("ModelProviderCommandService credential preservation (real Postgres)", () => {
  ...
  describe("given an azure provider with stored credentials", () => {
    describe("when the payload carries only an extra header", () => {
      ...
      it("is refused and the stored credentials survive", async () => {
        ...
        const stored = await repository.tryFindById({ id: created.id, organizationId: fixture.organizationId });
