...
export class ModelProviderService extends ModelProviderServiceContract {
  ...
  async refreshCodexForGateway(input: {
    ...
    if (Number.isFinite(savedAtMs) && Date.now() - savedAtMs < 30_000) {
    ...
    const refreshed = await this.options.codexTokenRefresher.refresh({ tokens: keys });
