...
describe("StoredObjectsService", () => {
  it("stores internal bytes once in the single row store", async () => {
    ...
    await expect(
      store.find({ tenantId: "project_1", id: first.reference.id }),
  ...
  it("persists a pending upload and confirms it in the same row", async () => {
    ...
    await expect(
      store.find({ tenantId: "project_1", id: "so_aaaaaaaa" }),
  ...
  it("retries failed physical deletion from the same deleted row", async () => {
    ...
    await expect(
      store.find({ tenantId: "project_1", id: stored.reference.id }),
