...
export class PostgresStoredObjectStore extends StoredObjectStore {
  ...
  private fromRow(value: unknown): StoredObjectRecord {
    ...
    const provider = this.nullableString(row.storageProvider);
    const destinationId = this.nullableString(row.storageDestinationId);
    const relativeId = this.nullableString(row.storageProviderRelativeId);
    return {
      ...
      expiresAt: this.nullableDate(row.expiresAt),
      availableAt: this.nullableDate(row.availableAt),
      deletedAt: this.nullableDate(row.deletedAt),
      source: this.string(row.source, "source") as StoredObjectSource,
      legacyFingerprint: this.nullableString(row.legacyFingerprint),
