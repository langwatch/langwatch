...
Feature: Stored Objects service and API
  ...
  Scenario: Stored Objects lives in one feature package
    ...
    Then @langwatch/stored-objects-contract contains portable schemas, errors and RPC contracts
    And @langwatch/stored-objects-server contains the concrete store, service, migration and API registration
  ...
  Scenario: One Postgres row owns current state
    ...
    And StoredObjectsService is a concrete class
  ...
  Scenario: Stored Objects reuses the existing storage implementation
    Given the application composes StoredObjectsService
  ...
  Scenario: An authorized caller resolves and streams bytes
    ...
    Then metadata comes from Postgres
