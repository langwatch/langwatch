...
export class RoleService extends RoleServiceContract {
  ...
  async getForOrganization(input: {
    roleId: string;
  ...
  async updateForOrganization(input: {
    roleId: string;
    organizationId: string;
    changes: RoleUpdate;
    actor: LedgerActor;
  ...
  private async delete(input: {
    ...
    const role = await this.deps.repository.tryFindCustomByIdInOrganization({
      roleId: input.roleId,
      organizationId: input.organizationId,
    });
    if (!role) throw new RoleNotFoundError(input.roleId);
    ...
    if (!deleted) {
      ...
      const stillPresent = await this.deps.repository.tryFindCustomByIdInOrganization({
        roleId: input.roleId,
        organizationId: input.organizationId,
      });
  ...
  async validateAssignable(input: {
    roleIds: string[];
    organizationId: string;
  }): Promise<void> {
    if (input.roleIds.length === 0) return;
    const valid = await this.deps.repository.findAssignable(
      input.roleIds,
  ...
  async assertNoOrganizationExclusivePermissionsBelowOrganizationScope(input: {
    organizationId: string;
    customBindings: Array<{ customRoleId: string; scopeType: RoleBindingScopeType }>;
  }): Promise<void> {
    const bindings = input.customBindings.filter(
    ...
    for (const binding of bindings) {
      const permission = (permissions.get(binding.customRoleId) ?? []).find((candidate) =>
        this.deps.permission.isOrganizationExclusive(candidate),
      );
      if (permission) {
        throw this.deps.permission.organizationExclusiveScopeError({
          permission,
  ...
  removeExclusiveApiKeyRoles(input: {
    roleIds: string[];
    apiKeyId: string;
    organizationId: string;
    actor: LedgerActor;
