...
export interface GithubPullRequestMappingServiceDeps {
  ...
  /** Canonical project capability used for ownership and backfill discovery. */
  project: ProjectService;
  /** Canonical coding-agent capability used for session backfill. */
...
export class GithubPullRequestMappingService {
  static create(
    deps: GithubPullRequestMappingServiceDeps,
  ): GithubPullRequestMappingService {
    return new GithubPullRequestMappingService(deps);
  }

  private readonly deps: GithubPullRequestMappingServiceDeps;

  private constructor(deps: GithubPullRequestMappingServiceDeps) {
    this.deps = deps;
  }
  ...
  findForBranches(input: {
    ...
    return this.deps.repository.findAllByBranchKeys(input);
  }
  ...
  findAllByBranches(input: {
    ...
    return this.deps.repository.findAllByBranches(input);
  }
  ...
  pruneStaleBranchLinkage(): Promise<{ branchChecks: number }> {
    return this.deps.repository.deleteStaleBefore({
      before: new Date(nowMs(this.deps) - 7 * 24 * 60 * 60 * 1000),
    });
  }
  ...
  async requestBranchMapping(request: BranchMappingRequest): Promise<void> {
    if (!isMappableGithubHost(request.repositoryHost, this.deps.hostConfig)) return;
  ...
  async applyPullRequestEvent(event: GithubPullRequestEvent): Promise<boolean> {
    ...
    const scope = branchScopeFor({
      ...
      // about that one.
      repositoryHost: getGithubHost(this.deps.hostConfig),
  ...
  async runBackfillForOrganization({
    ...
    const projectIds = (
      await this.deps.project.listByOrganization({
        organizationId,
        page: 1,
        limit: BACKFILL_BRANCH_CAP,
      })
  ...
  private async readSessionsForBackfill({
    ...
    try {
      return await this.deps.codingAgent.listRecent({
        projectId,
        fromMs,
        toMs,
  ...
  private targetFromSession({
    ...
    return {
      organizationId,
      repositoryHost: normalizeGithubHost(session.repositoryHost, this.deps.hostConfig),
