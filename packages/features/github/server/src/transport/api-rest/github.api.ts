...
function setupError({
  ...
  return popupHtml(c, ports.github().popupErrorHtml(errorMessage), status);
}
...
async function handleInstall(c: Context, ports: GithubRestPorts): Promise<Response> {
  ...
  const session = await ports.session(c.req.raw);
  if (!session?.user) {
...
async function handleSetup(c: Context, ports: GithubRestPorts): Promise<Response> {
  ...
  const rejection = await rejectUnauthorizedSetup({ c, ports, state });
  ...
  await recordInstallAudit({ ports, state, installationId, accountLogin });
...
async function rejectUnauthorizedSetup({
  ...
  if (state.nonceRegistered) {
    const consumed = await ports.github().tryConsumeInstallNonce(state.nonce);
    if (consumed === false) {
      return setupError({
        c,
        ports,
  ...
  if (
    !(await ports.github().isOrganizationMember({
      userId: state.userId,
      organizationId: state.organizationId,
    }))
  ) {
  ...
  if (
    !(await ports.canManageOrganization({
      userId: state.userId,
      organizationId: state.organizationId,
    }))
  ) {
    return setupError({ c, ports, state, errorMessage: "Forbidden", status: 403 });
  }
...
async function handleWebhook(c: Context, ports: GithubRestPorts): Promise<Response> {
  const service = ports.github();
  const config = service.getAppConfig();
...
/** Builds the `/api/github` installation family over one process's ports. */
export function createGithubRestApp(options: {
  ...
  secured
    ...
    .get("/github/install", (c) => handleInstall(c, ports));

  secured
    .access(publicEndpoint(SETUP_PUBLIC_REASON))
    .get("/github/setup", (c) => handleSetup(c, ports));
