...
} from "../src/adapters/github-app-token.adapter";
...
describe("computeRepoScopeKey", () => {
  it("is stable and independent of repository id order", () => {
    const a = GithubAppTokenAdapter.computeRepoScopeKey({
      repositoryIds: ["1", "2", "3"],
    });
    const b = GithubAppTokenAdapter.computeRepoScopeKey({
      repositoryIds: ["3", "1", "2"],
    });
    expect(a).toBe(b);
  });
  ...
  it("differs between the full-installation scope and a single repo", () => {
    const all = GithubAppTokenAdapter.computeRepoScopeKey({});
    const one = GithubAppTokenAdapter.computeRepoScopeKey({ repositoryIds: ["42"] });
...
describe("mintInstallationToken", () => {
  describe("when scoped to a single repository", () => {
    it("POSTs repository_ids + minimal permissions and caches the token", async () => {
      ...
      const scope = GithubAppTokenAdapter.computeRepoScopeKey({ repositoryIds: ["42"] });
...
describe("configured", () => {
  it("is false without a private key, true with app id + key", () => {
    expect(GithubAppTokenAdapter.create("app", "", null).configured).toBe(false);
    expect(GithubAppTokenAdapter.create("app", privateKey, null).configured).toBe(true);
  });
});
