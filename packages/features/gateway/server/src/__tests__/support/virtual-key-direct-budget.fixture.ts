...
async function createVirtualKey(prisma: PrismaClient, id: string, name: string) {
...
async function seedKeysAndBudgets(prisma: PrismaClient) {
  await createVirtualKey(prisma, VK_DAILY_ID, "daily-key");
  await createVirtualKey(prisma, VK_STANDALONE_ID, "standalone-key");
  await createVirtualKey(prisma, VK_BOTH_ID, "both-key");
  await createVirtualKey(prisma, VK_INHERITED_ID, "inherited-key");
  await createVirtualKey(prisma, VK_NEIGHBOUR_USED_ID, "neighbour-used");
  await createVirtualKey(prisma, VK_NEIGHBOUR_UNUSED_ID, "neighbour-unused");

  await createBudget(prisma, {
    id: BUDGET_DAILY_ID,
