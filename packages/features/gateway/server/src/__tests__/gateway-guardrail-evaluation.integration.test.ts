...
class MonitorRowsFromDatabase extends MonitorService {
  async listEnabledGuardrailMonitors(
    ...
    const rows = await prisma.monitor.findMany({
      ...
      select: { id: true, evaluatorId: true, checkType: true, parameters: true },
    });
    return rows.map((row) => ({
      id: row.id,
...
describe.skipIf(!databaseUrl)("GatewayGuardrailEvaluationService against real PG", () => {
  beforeAll(async () => {
...
describe("wire direction mapping", () => {
  it("maps each wire direction onto its stored enum", () => {
    expect(guardrailWire.storedDirectionFor("request")).toBe("PRE");
    expect(guardrailWire.storedDirectionFor("response")).toBe("POST");
    expect(guardrailWire.storedDirectionFor("stream_chunk")).toBe("STREAM_CHUNK");
  });
});

describe("evaluation input", () => {
  it("scores the prompt on the request direction", () => {
    const data = guardrailWire.evaluationDataFor({
      direction: "request",
      content: { messages: [{ role: "user", content: "hi" }] },
    });
