...
function buildApp(): void {
  ...
  const ports = {
    ...
    spend: () => ({
      commands: {
        admitSpend: record(appended),
        confirmSpend: record(appendedConfirms),
        failSpend: { send: () => Promise.resolve() },
      },
      rating: new FlatRating(),
    }),
  } as unknown as GatewayInternalRestPorts;
  app = createGatewayInternalRestApp({ security: testRestSecurity(), ports });
}

function signedPost(path: string, payload: unknown): Request {
  const fullPath = `/api/internal/gateway${path}`;
  const body = JSON.stringify(payload);
  const timestamp = String(Math.floor(Date.now() / 1000));
  const signature = computeGatewaySignature(
    SECRET,
    buildGatewayCanonicalString({ method: "POST", path: fullPath, timestamp, body }),
  );
  return new Request(`http://api.test${fullPath}`, {
    method: "POST",
    body,
    headers: {
...
describe.skipIf(!databaseUrl)("spend-command ingest enrichment (real PG + internal route)", () => {
  beforeAll(async () => {
    buildApp();
    for (const tenant of [
      ...
      await prisma.organization.create({
        data: { id: tenant.orgId, name: `Ingest ${tenant.orgId}`, slug: tenant.orgId },
      });
  ...
  describe("given a batch of admissions the gateway drained", () => {
    ...
    it("joins the team and the principal onto every admission", async () => {
      await prisma.virtualKey.update({ where: { id: VK_ID }, data: { lastUsedAt: null } });
      ...
      // The key was in use, so admission is what advances the column: one
      // write for the whole batch, not one per record.
  ...
  describe("given the control plane cannot answer for every record", () => {
    ...
    it("degrades attribution for rows that no longer exist and still appends", async () => {
      ...
      // A key that no longer exists costs the principal and group budgets,
      // not the project's own debits.
      expect(appended[0]).toMatchObject({ team_id: TEAM_ID, principal_user_id: "" });
      // A project the control plane cannot resolve costs the team budgets.
      expect(appended[1]).toMatchObject({ team_id: "", principal_user_id: USER_ID });
      expect(appended[2]).toMatchObject({ team_id: TEAM_ID, principal_user_id: USER_ID });
    });

    it("appends an admission whose key belongs to another organization", async () => {
      // The record is already durable on the gateway's side and its outcome
      // is still coming, so a cross-tenant mismatch is logged and chased,
      // never turned into lost billing evidence.
      const { status, body } = await postRecords([admitRecord({ virtual_key_id: FOREIGN_VK_ID })]);
    ...
    it("fails the whole batch when the database cannot be read", async () => {
      vi.spyOn(store, "findVirtualKeysForAttribution").mockRejectedValue(
  ...
  describe("given an outcome the gateway drained after the request finished", () => {
    ...
    it("skips enrichment for a confirmation that names no key", async () => {
      ...
      expect(status).toBe(200);
      expect(appendedConfirms).toHaveLength(1);
      // Nothing to join against, so the seam does not touch the record at
      // all — these stay ABSENT rather than being stamped empty, and the
      // command schema's own defaults fill them when the pipeline parses it.
