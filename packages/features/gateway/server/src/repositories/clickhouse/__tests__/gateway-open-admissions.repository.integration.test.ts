...
describe.skipIf(!chUrl)("open admissions on the spend record (real ClickHouse)", () => {
  beforeAll(async () => {
    ...
    // leave those tests passing for a reason the query had no part in.
  ...
  describe("given a window of folded rows across every lifecycle status", () => {
    ...
    beforeAll(async () => {
      ...
      await fold(
        OPEN_OLDEST,
        foldState({ status: "admitted", occurredAtMs: BASE, updatedAt: BASE }),
      );
      ...
      open = (await finder.findOpenAdmissions(SWEEP)).filter((row) => row.tenantId === tenantId);
    }, 120_000);
  ...
  describe("given more open admissions than one sweep may settle", () => {
    ...
    beforeAll(async () => {
      ...
      const entries = Array.from({ length: MAX_OPEN_ADMISSIONS_PER_SWEEP + 1 }, (_, i) => ({
        tenantId,
        gatewayRequestId: i === MAX_OPEN_ADMISSIONS_PER_SWEEP ? capNewest : `req-cap-${i}-${run}`,
        state: foldState({
