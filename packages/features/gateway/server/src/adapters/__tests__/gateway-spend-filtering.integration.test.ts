...
describe.skipIf(!databaseUrl || !chUrl)("gateway spend filtering (real PG + real CH)", () => {
  beforeAll(async () => {
    ...
    await prisma.organization.create({
      data: { id: ORG_B_ID, name: `Filt Other Org ${suffix}`, slug: `filt-other-${suffix}` },
    });
    await prisma.team.create({
      data: {
        id: TEAM_C_ID,
  ...
  afterAll(async () => {
    ...
    await prisma.organization.deleteMany({ where: { id: { in: [ORG_ID, ORG_B_ID] } } });
  }, 120_000);
  ...
  describe("when nothing is filtered", () => {
    it("covers every seeded model", async () => {
      ...
      expect(await summariseBy(["model"])).toEqual(["claude-opus-5", "gemini-3-pro", "gpt-5-mini"]);
    });
  });
  ...
  describe("when the rollup is narrowed to the in-flight status", () => {
    it("refuses rather than answering the empty intersection", async () => {
      ...
      await expect(summariseBy(["model"], { filters: { status: "admitted" } })).rejects.toThrow(
        /admitted/,
      );
    });
  ...
  describe("when the caller filters to one team", () => {
    ...
    it("sums only that team's projects", async () => {
      scope.clearCache();
      const resolved = await scope.resolveSpendScope({
        organizationId: ORG_ID,
        teamIds: [TEAM_B_ID],
      });
      expect(resolved.tenantIds).toEqual([PROJECT_B_ID]);
  ...
  describe("when a filter matches nothing", () => {
    ...
    it("answers empty rather than the whole organization", async () => {
      ...
      const keys = await summariseBy(["model"], { filters: { virtualKeyIds: [] } });
      expect(keys).toEqual([]);
      ...
      expect(resolved.virtualKeyIds).toEqual([]);
    });
    ...
    it("keeps another organization's key out even though its spend is real", async () => {
      ...
      expect(await summariseBy(["model"], { filters: { virtualKeyIds: [VK_C_ID] } })).toEqual([]);
  ...
  describe("when the caller filters on a provider, an end user or a label", () => {
    it("narrows on each of them", async () => {
      ...
      expect(await summariseBy(["model"], { filters: { providerKeys: ["pk-google"] } })).toEqual([
        "gemini-3-pro",
      ]);
      expect(await summariseBy(["model"], { filters: { endUserIds: ["enduser-b"] } })).toEqual([
        "gemini-3-pro",
      ]);
  ...
  describe("when the caller filters on their own metadata", () => {
    ...
    it("widens a repeated key rather than matching nothing", async () => {
      const keys = await summariseBy(["model"], {
        filters: { metadata: [{ key: "customer_tier", values: ["gold", "silver"] }] },
      });
