...
import type { SettleSpendCommandData } from "./gateway-spend-commands.process";
 ...
 * number of instances.
 *
 * Lives here rather than beside the query it bounds so that the bound and the
 * sweep that reports on hitting it stay in one file. The repository imports it
 * from here, which is a one-way runtime edge.
 */
