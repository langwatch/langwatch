...
import { GatewayConfigMaterialiser } from "../gateway-config-materialisation.service";
...
const materialiser = () =>
  new GatewayConfigMaterialiser(prisma, new SuiteProjectService(), null, gateway, credentials);

async function bundleFor(keyId: string) {
  const vk = await createGatewayVirtualKeysPort(prisma).tryFindById({
    id: keyId,
