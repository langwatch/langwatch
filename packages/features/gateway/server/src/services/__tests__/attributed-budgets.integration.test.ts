...
import { attributedUserBucketScopeId } from "../../adapters/gateway-bucket-scope.adapter";
import { bucketPeriodFloorMs, budgetPeriodFloorMs } from "../../adapters/gateway-period.adapter";
...
function debitRow(
  over: Partial<BudgetDebitRow> & Pick<BudgetDebitRow, "budgetId" | "scopeId" | "gatewayRequestId">,
): BudgetDebitRow {
  return {
    tenantId: PROJECT_ID,
...
describe.skipIf(!databaseUrl || !chUrl)("attributed budgets and resets (real PG + real CH)", () => {
  beforeAll(async () => {
  ...
  afterAll(async () => {
    ...
    await prisma.gatewayBudgetBucketBoundary.deleteMany({ where: { organizationId: ORG_ID } });
  ...
  it("per-bucket reset isolates one end user", async () => {
    ...
    const templateBefore = await prisma.gatewayBudget.findUnique({ where: { id: template.id } });
    await service.reset({
      id: template.id,
      organizationId: ORG_ID,
      actorUserId: USER_ID,
    ...
    const templateAfter = await prisma.gatewayBudget.findUnique({ where: { id: template.id } });
  ...
  it("forgives the current period's spend without re-phasing the cycle", async () => {
    ...
    expect(await readAt({ budget: anchored, now: anchored.createdAt })).toBeCloseTo(42, 3);
