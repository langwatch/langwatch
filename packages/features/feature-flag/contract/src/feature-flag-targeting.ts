/**
 * The BROWSER-side vocabulary for a flag read's targeting scope.
 *
 * Server reads name their subject with the `FeatureFlagTarget` union in
 * `feature-flag-target.ts` (`kind: "project" | "organization" | "user" |
 * "anonymous" | "system"`), which expresses "no tenant" natively. This module
 * is the client's older, flatter spelling of the same idea, kept because
 * seventeen browser call sites are written in it and converting them is a
 * behaviour question rather than a rename. It moved here verbatim from
 * `platform/app/src/server/featureFlag/targeting.ts`, which the migration
 * deleted without landing a replacement.
 *
