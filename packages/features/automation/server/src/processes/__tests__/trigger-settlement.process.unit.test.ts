    ...
    describe("when another match is recorded", () => {
      ...
      it("keys the overflow entry per trigger per minute so a storm coalesces", () => {
        ...
        const fullPending = (offset: number) =>
          Object.fromEntries(
            Array.from({ length: MAX_PENDING_MATCHES }, (_, index) => pendingEntry(offset, index)),
          );
        ...
        expect(overflowKeyAt(minuteStart + 59_999, "trigger-1")).toBe("overflow:trigger-1:3");
        // The next minute gets its own row, so the storm stays visible.
        expect(overflowKeyAt(minuteStart + 60_000, "trigger-1")).toBe("overflow:trigger-1:4");
