...
export class WebhookProviderAdapter extends AutomationWebhookProviderPort {
  ...
  decryptHeaders(params: {
    ...
    return WebhookProviderAdapter.decryptHeaders(params, this.crypto);
  }
  ...
  persist(input: {
    ...
    return WebhookProviderAdapter.persistActionParams({ ...input, crypto: this.crypto });
  }

  redact(params: WebhookStoredActionParams): WebhookActionParams {
    return WebhookProviderAdapter.redactActionParams(params, this.crypto);
  }
  ...
  private static persistActionParams({
    ...
    const saved = existing ? WebhookProviderAdapter.decryptHeaders(existing, crypto) : {};
    ...
    return {
      ...
      ...(Object.keys(resolved).length > 0
        ...
        : {}),
      ...WebhookProviderAdapter.persistSigningSecret({ incoming, existing, crypto }),
    };
  }
  ...
  private static keepStoredSigningSecret(
    ...
    return {
      ...
      ...WebhookProviderAdapter.keepRotationWindow(existing),
    };
  }
  ...
  private static persistSigningSecret({
    ...
    if (submitted === WEBHOOK_HEADER_VALUE_KEPT) {
      return WebhookProviderAdapter.keepStoredSigningSecret(existing);
    }
    if (!submitted) return {};
    const current = existing?.signingSecretEncrypted
      ? crypto.decrypt(existing.signingSecretEncrypted)
      : null;
    if (current === submitted) return WebhookProviderAdapter.keepStoredSigningSecret(existing);
  ...
  private static redactActionParams(
    ...
    const names = Object.keys(WebhookProviderAdapter.decryptHeaders(params, crypto));
    const {
