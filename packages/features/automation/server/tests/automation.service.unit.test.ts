...
import { EmailSuppressionNameRepository } from "../src/repositories/email-suppression-name.repository";
import { TriggerFireHistoryRepository } from "../src/repositories/trigger-fire-history.repository";
import { TriggerRepository } from "../src/repositories/trigger.repository";
import type { ReportScheduleTarget } from "../src/repositories/trigger.repository";
import { UnsubscribeTokenVerifier } from "../src/ports/unsubscribe-token.port";
...
import { ScheduledJobStore } from "../src/ports/scheduled-jobs.port";
import type { ScheduledJobRecord } from "../src/ports/scheduled-jobs.port";
...
class EmptyCustomGraphs extends CustomGraphRepository {
	tryFindById(): Promise<null> { return Promise.resolve(null); }
	existsInProject(): Promise<boolean> { return Promise.resolve(false); }
...
class Fires extends TriggerFireHistoryRepository {
	...
	findAllRecentByTriggerId = vi.fn(
		async (_input: { projectId: string; triggerId: string; limit: number }) =>
			this.fires,
	);
...
describe("AutomationService trigger and fire-history lifecycle", () => {
	it("keeps reports out of trace and graph dispatch projections", async () => {
		...
		expect(
			(await service.getActiveTraceTriggersForProject("p")).map(
				(trigger) => trigger.id,
			),
		).toEqual(["trace"]);
		expect(
			(await service.getActiveGraphTriggersForProject("p")).map(
				(trigger) => trigger.id,
			),
		).toEqual(["graph"]);
	});
