...
import { SlackBlockKitTemplatePicker } from "../template-picker";
...
describe("SlackBlockKitTemplatePicker", () => {
  ...
  describe("given a webhook connection", () => {
    ...
    it("keeps a non-gated template selectable", () => {
      ...
      expect(screen.getByRole("button", { name: /use compact alert template/i })).toBeEnabled();
    });
  });
  ...
  describe("given a graph-alert draft", () => {
    it("shows only the graph-alert layouts", () => {
      ...
      expect(screen.getByRole("button", { name: /use one-liner template/i })).toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /use compact alert template/i }),
      ).not.toBeInTheDocument();
      expect(screen.queryByRole("button", { name: /use digest/i })).not.toBeInTheDocument();
    });
