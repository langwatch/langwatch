 ...
 *
 * A COPY of the same nine lines `@langwatch/gateway-web` and
 * `@langwatch/enterprise-governance-web` carry, for the same reason and with the
 * same gap: the reader belongs with the presentation registry it feeds, and
 * three families will converge on one when that registry moves out of
 * `platform/app`.
 *
...
export type AutomationHandledError = {
  code: string;
  httpStatus: number;
...
export function readHandledError(error: unknown): AutomationHandledError | null {
