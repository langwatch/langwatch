...
import digestTableSource from "./digest-table.liquid?raw";
import evalFailureDetailedSource from "./eval-failure-detailed.liquid?raw";
import evalFailureRichSource from "./eval-failure-rich.liquid?raw";
import graphAlertCompactSource from "./graph-alert-compact.liquid?raw";
import graphAlertDetailedSource from "./graph-alert-detailed.liquid?raw";
import graphAlertHistoryTableSource from "./graph-alert-history-table.liquid?raw";
...
import reportTableSource from "./report-table.liquid?raw";
...
export const SLACK_BLOCK_KIT_TEMPLATES: SlackBlockKitTemplateOption[] = [
  ...
  },
  {
    ...
    emoji: "🗂️",
    tagline: "A summary card with the evaluation and input, then the full input and output.",
    deliveryNote: "1 message per trace",
    cadenceFit: "immediate",
    kind: "trace",
  ...
  },
  {
    ...
    emoji: "🥧",
    tagline: "A pie chart of matches by evaluator, with the counts listed. No trace content.",
    deliveryNote: "all matches, 1 message",
    cadenceFit: "digest",
    kind: "trace",
  ...
  },
  {
    ...
    emoji: "✅",
    tagline: "A recovery banner when the metric is back within threshold, with a was to now value.",
    deliveryNote: "1 message per alert",
    cadenceFit: "immediate",
    kind: "graphAlert",
    gatedBlock: "alert",
  ...
  },
  {
    ...
    emoji: "🔇",
    tagline: "A no-data banner for heartbeat monitors, framing a silent metric as missing data.",
    deliveryNote: "1 message per alert",
    cadenceFit: "immediate",
    kind: "graphAlert",
    gatedBlock: "alert",
  ...
  },
  {
    ...
    emoji: "🗓️",
    tagline: "The metric's recent values as a Time and Value grid instead of a sparkline.",
    deliveryNote: "1 message per alert",
    cadenceFit: "immediate",
    kind: "graphAlert",
    gatedBlock: "data_table",
 ...
 */
export function reportSourceIsAutoLayout(reportSource: ReportTemplateSource | undefined): boolean {
...
export function templateOptionsFor({
  ...
  return SLACK_BLOCK_KIT_TEMPLATES.filter((opt) => {
    ...
    return reportSource !== undefined ? (opt.reportSources ?? []).includes(reportSource) : true;
  });
}
