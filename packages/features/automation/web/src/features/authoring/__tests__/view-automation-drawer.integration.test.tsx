  ...
  return render(
    <ChakraProvider value={defaultSystem}>
      <ViewAutomationDrawer
        onClose={mockCloseDrawer}
        onEdit={mockOpenDrawer}
  ...
  describe("given a graph alert with fire history", () => {
    ...
    describe("when the user clicks Edit", () => {
      it("hands the same automation over to the editor", async () => {
        ...
        // The drawer used to name the application's `automation` drawer and its
        // props; the screen owns that address now, so the panel says which
        // automation and the screen decides how it is opened.
        expect(mockOpenDrawer).toHaveBeenCalledWith("trigger_1");
      });
    });
  });
