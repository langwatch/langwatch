...
import { CadenceSection } from "../ui/sections/cadence-section-adapter";
  ...
  describe("given an alert draft", () => {
    ...
    describe("when the threshold is edited", () => {
      it("commits the parsed number to the draft", async () => {
        ...
        expect(useAutomationStore.getState().draft.graphAlert.threshold).toBe(250);
      });
    });
    ...
    describe("when the operator is changed", () => {
      it("commits it to the draft", async () => {
        ...
        expect(useAutomationStore.getState().draft.graphAlert.operator).toBe("lt");
      });
    });
  });
