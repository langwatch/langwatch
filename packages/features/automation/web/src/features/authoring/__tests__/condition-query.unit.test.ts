...
describe("serializeConditions", () => {
  describe("given a single membership condition", () => {
    it("emits field:value for is", () => {
      expect(serializeConditions([cond({ field: "status", operator: "is", value: "error" })])).toBe(
        "status:error",
      );
    });
  ...
  describe("given range conditions", () => {
    it("emits comparators", () => {
      expect(serializeConditions([cond({ field: "cost", operator: "gt", value: "0.1" })])).toBe(
        "cost:>0.1",
      );
      expect(serializeConditions([cond({ field: "cost", operator: "lte", value: "1" })])).toBe(
        "cost:<=1",
      );
    });
