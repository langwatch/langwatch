...
export interface AutomationDraft<C extends ProviderClients> {
...
export type DraftAction<C extends ProviderClients> =
  ...
  | { type: "SET_FILTERS"; value: AutomationFilters }
...
function createInitialDraft<C extends ProviderClients>(
...
function notifyChannel<C extends ProviderClients>(
  registry: ClientProviderRegistry<C>,
  draft: AutomationDraft<C>,
): "email" | "slack" | "webhook" | null {
  switch (draft.action) {
    case TriggerAction.SEND_EMAIL:
      return registry.SEND_EMAIL.client.channel;
    case TriggerAction.SEND_SLACK_MESSAGE:
      return registry.SEND_SLACK_MESSAGE.client.channel;
    case TriggerAction.SEND_WEBHOOK:
      return registry.SEND_WEBHOOK.client.channel;
    default:
      return null;
  }
}
...
function templatesFromDraft<C extends ProviderClients>(
  registry: ClientProviderRegistry<C>,
  draft: AutomationDraft<C>,
) {
  switch (draft.action) {
    case TriggerAction.SEND_EMAIL:
      return registry.SEND_EMAIL.client.templatesFromSlice(draft.slices.SEND_EMAIL);
    case TriggerAction.SEND_SLACK_MESSAGE:
      return registry.SEND_SLACK_MESSAGE.client.templatesFromSlice(draft.slices.SEND_SLACK_MESSAGE);
    case TriggerAction.SEND_WEBHOOK:
      return registry.SEND_WEBHOOK.client.templatesFromSlice(draft.slices.SEND_WEBHOOK);
    default:
...
function buildTestFirePayload<C extends ProviderClients>(
  registry: ClientProviderRegistry<C>,
  input: BuildTestFirePayloadInput<C>,
) {
  const {
    draft,
    projectId,
...
function actionParamsFromDraft<C extends ProviderClients>(
  registry: ClientProviderRegistry<C>,
  draft: AutomationDraft<C>,
) {
  switch (draft.action) {
    case TriggerAction.SEND_EMAIL:
      return registry.SEND_EMAIL.client.toActionParams(draft.slices.SEND_EMAIL);
    case TriggerAction.SEND_SLACK_MESSAGE:
      return registry.SEND_SLACK_MESSAGE.client.toActionParams(draft.slices.SEND_SLACK_MESSAGE);
    case TriggerAction.SEND_WEBHOOK:
      return registry.SEND_WEBHOOK.client.toActionParams(draft.slices.SEND_WEBHOOK);
    case TriggerAction.ADD_TO_DATASET:
      return registry.ADD_TO_DATASET.client.toActionParams(draft.slices.ADD_TO_DATASET);
    case TriggerAction.ADD_TO_ANNOTATION_QUEUE:
      return registry.ADD_TO_ANNOTATION_QUEUE.client.toActionParams(
        draft.slices.ADD_TO_ANNOTATION_QUEUE,
      );
    default:
      return {};
  }
...
export function filtersAreSet(filters: AutomationFilters): boolean {
...
export function conditionsAreSet<C extends ProviderClients>(draft: AutomationDraft<C>): boolean {
...
function configIsComplete<C extends ProviderClients>(
  ...
  switch (draft.action) {
    case TriggerAction.SEND_EMAIL:
      return registry.SEND_EMAIL.client.isComplete(draft.slices.SEND_EMAIL);
    case TriggerAction.SEND_SLACK_MESSAGE:
      return registry.SEND_SLACK_MESSAGE.client.isComplete(draft.slices.SEND_SLACK_MESSAGE);
    case TriggerAction.SEND_WEBHOOK:
      return registry.SEND_WEBHOOK.client.isComplete(draft.slices.SEND_WEBHOOK);
    case TriggerAction.ADD_TO_DATASET:
      return registry.ADD_TO_DATASET.client.isComplete(draft.slices.ADD_TO_DATASET);
    case TriggerAction.ADD_TO_ANNOTATION_QUEUE:
      return registry.ADD_TO_ANNOTATION_QUEUE.client.isComplete(
        draft.slices.ADD_TO_ANNOTATION_QUEUE,
      );
    default:
      return false;
...
function configurationSummary<C extends ProviderClients>(
  ...
  switch (draft.action) {
    case TriggerAction.SEND_EMAIL:
      return registry.SEND_EMAIL.client.summary(draft.slices.SEND_EMAIL, identity);
    case TriggerAction.SEND_SLACK_MESSAGE:
      return registry.SEND_SLACK_MESSAGE.client.summary(draft.slices.SEND_SLACK_MESSAGE, identity);
    case TriggerAction.SEND_WEBHOOK:
      return registry.SEND_WEBHOOK.client.summary(draft.slices.SEND_WEBHOOK, identity);
    case TriggerAction.ADD_TO_DATASET:
      return registry.ADD_TO_DATASET.client.summary(draft.slices.ADD_TO_DATASET, identity);
    case TriggerAction.ADD_TO_ANNOTATION_QUEUE:
      return registry.ADD_TO_ANNOTATION_QUEUE.client.summary(
        draft.slices.ADD_TO_ANNOTATION_QUEUE,
        identity,
      );
  }
...
export function createAutomationDraftModel<C extends ProviderClients>(
  registry: ClientProviderRegistry<C>,
) {
  return {
    INITIAL_DRAFT: createInitialDraft(registry),
    reducer: (state: AutomationDraft<C>, action: DraftAction<C>) => reduceDraft(state, action),
    notifyChannel: (draft: AutomationDraft<C>) => notifyChannel(registry, draft),
    templatesFromDraft: (draft: AutomationDraft<C>) => templatesFromDraft(registry, draft),
    buildTestFirePayload: (input: BuildTestFirePayloadInput<C>) =>
      buildTestFirePayload(registry, input),
    actionParamsFromDraft: (draft: AutomationDraft<C>) => actionParamsFromDraft(registry, draft),
    configIsComplete: (draft: AutomationDraft<C>) => configIsComplete(registry, draft),
    configurationSummary: (draft: AutomationDraft<C>) => configurationSummary(registry, draft),
    isNotifyAction,
    filtersAreSet,
    subjectIsSet,
    filterQueryIsSet,
