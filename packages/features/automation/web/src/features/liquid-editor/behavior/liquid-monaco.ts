...
export function setModelVariables(model: MonacoTextModel, variables: VariableInfo[]): void {
...
export function validateLiquidModel(
  ...
  const markers: editor.IMarkerData[] = detectUnknownVariables(model.getValue(), variables).map(
    (unknown) => {
...
export function setupLiquidJsonSchema(params: {
  ...
  const shadowModel = monaco.editor.createModel(initial.substituted, "json", shadowResource);
