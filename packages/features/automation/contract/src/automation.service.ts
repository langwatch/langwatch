...
export abstract class AutomationService {
	...
	abstract getActiveGraphTriggersForProject(
		projectId: string,
	): Promise<TriggerSummary[]>;
	...
	abstract getReportSchedules(input: {
		projectId: string;
	...
	abstract getSuppressions(input: {
		projectId: string;
	}): Promise<EmailSuppression[]>;
	abstract getAllEnriched(input: {
		projectId: string;
	}): Promise<Array<EmailSuppression & { triggerName: string | null }>>;
	abstract suppressEmail(
		input: SuppressEmailCommand,
	): Promise<EmailSuppression>;
	abstract removeSuppression(input: {
		id: string;
		projectId: string;
	}): Promise<void>;
