...
import { AnnotationSuggestionsChip } from "./annotation-suggestions-chip";
import type { AnnotationRow, AnnotationUser, AnnotationWithUser } from "./annotation-row";
...
function PeopleTooltip({ row }: { row: AnnotationRow }) {
  ...
  return (
    <VStack align="start" gap={0}>
      {row.createdByUser && (
        <Text marginBottom={2}>Queued by {row.createdByUser.name}</Text>
      )}
      {names.length > 0 && (
        <>
