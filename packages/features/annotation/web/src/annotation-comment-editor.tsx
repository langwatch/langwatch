...
export function AnnotationCommentEditor({
  ...
  const selectedReason = reasonScoreTypeId
    ? (scoreOptions[reasonScoreTypeId]?.reason ?? "")
    : "";
        ...
        <HStack width="full">
          ...
          {mode === "edit" && (
            <Menu.Root
              open={deleteMenuOpen}
              onOpenChange={(event) =>
                setDeleteMenuOpen(keepDeleteMenuOpen ? true : event.open)
              }
            >
...
function AnnotationCommentScoreBlock({
  ...
  const parsedDefaultValue = annotationScoreDefaultValueSchema.safeParse(
    scoreType.defaultValue,
  );
  const defaultRadioValue = parsedDefaultValue.success
    ? parsedDefaultValue.data.value
    : "";
...
export function AnnotationScoringDisabled({ children }: { children: ReactNode }) {
  return (
    <>
      <Separator />
      <Text>
        Scoring metrics are currently disabled. Enable them to add more data to your
        annotations.
      </Text>
      {children}
    </>
