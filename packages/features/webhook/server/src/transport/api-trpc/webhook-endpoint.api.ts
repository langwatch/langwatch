    ...
    return createTrpcService({
      ...
      .query("list", (p) =>
        p
          ...
          .withCustomPermission(entitled("webhookEndpoints:view"), VIEW)
          .handle(({ ctx, input }) =>
            ctx.app.webhooks.getAll({ organizationId: input.organizationId }),
          ),
      )
      .query("deliveries", (p) =>
        p
          ...
          .handle(async ({ ctx, input }) => {
            const page = await ctx.app.webhooks.getDeliveries({
