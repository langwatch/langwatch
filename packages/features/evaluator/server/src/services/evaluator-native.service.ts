...
export class EvaluatorNativeService {
  ...
  private countRedactedValues(
    ...
    const enabled = kind === "pii" ? EvaluatorNativeService.enabledPiiEntities(settings) : null;
    let hits = 0;
  ...
  private static evaluateApiKeysAndSecrets(data: Record<string, unknown>): SingleEvaluationResult {
  ...
  private static enabledPiiEntities(
    settings: Record<string, unknown> | undefined,
  ): Set<string> | null {
