...
import { EvaluatorRepository, type EvaluatorDatabase, type PersistEvaluatorInput } from "../evaluator.repository";
...
type EvaluatorRow = {
  id: string; projectId: string; name: string; slug: string; type: unknown;
  config: unknown; workflowId: string | null; copiedFromEvaluatorId: string | null;
...
function mapRow(row: EvaluatorRow): Evaluator {
  ...
  return evaluatorSchema.parse({
    ...
    type: row.type, config, workflowId: row.workflowId,
    copiedFromEvaluatorId: row.copiedFromEvaluatorId, archivedAt: row.archivedAt,
...
export class PrismaEvaluatorRepository extends EvaluatorRepository {
  ...
  private constructor(private readonly database: EvaluatorDatabase) { super(); }
  ...
  async tryFindByIdOnly(id: string): Promise<Evaluator | null> {
    const row = await this.database.evaluator.findFirst({ where: { id, archivedAt: null } });
    return row ? mapRow(row as unknown as EvaluatorRow) : null;
  }
  ...
  async tryFindByWorkflow(input: { workflowId: string; projectId: string }): Promise<Evaluator | null> {
    const row = await this.database.evaluator.findFirst({
      ...
      try {
        const row = await this.database.evaluator.create({
          data: {
            ...
            slug: requestedSlug, type: input.type,
        ...
        if (!(error instanceof Error) || !error.message.includes("Unique constraint") || !error.message.includes("slug")) throw error;
  ...
  async update(input: EvaluatorUpdateInput): Promise<Evaluator> {
    ...
    const row = await this.database.evaluator.update({
      where: { id: input.id, projectId: input.projectId }, data: data as never,
    });
    return mapRow(row as unknown as EvaluatorRow);
  }
  async archive(input: { id: string; projectId: string }): Promise<Evaluator> {
    const row = await this.database.evaluator.update({
      where: { id: input.id, projectId: input.projectId }, data: { archivedAt: new Date() },
    });
    return mapRow(row as unknown as EvaluatorRow);
  }
  async findCopies(input: { evaluatorId: string }): Promise<EvaluatorCopy[]> {
    const rows = await this.database.evaluator.findMany({
      ...
      select: {
        ...
        project: { select: { name: true, team: { select: { name: true, organization: { select: { name: true } } } } } },
      },
    });
    return (rows as unknown as Array<{ id: string; name: string; projectId: string; project: { name: string; team: { name: string; organization: { name: string } } } }>).map((row) => ({
  ...
  async updateNameAndConfig(input: { id: string; projectId: string; name: string; config: EvaluatorConfig }): Promise<void> {
    await this.database.evaluator.update({ where: { id: input.id, projectId: input.projectId }, data: { name: input.name, config: input.config as unknown as Prisma.InputJsonValue } });
  }
}
