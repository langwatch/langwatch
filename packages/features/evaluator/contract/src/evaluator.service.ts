import type {
  Evaluator, EvaluatorConfig, EvaluatorField, EvaluatorWithFields,
} from "./evaluator";
...
export type EvaluatorUpdateInput = {
  id: string;
  projectId: string;
  data: Partial<Pick<Evaluator, "name" | "type" | "workflowId">> & { config?: EvaluatorConfig };
};
export type EvaluatorCopy = { id: string; name: string; projectId: string; fullPath: string };
export type EvaluatorHistoryEntry = {
  id: string; action: string; createdAt: Date; args: unknown;
  user: { id: string; name: string | null; email: string | null } | null;
};
export type WorkflowEvaluatorFields = {
  workflowId: string; workflowName: string; workflowIcon?: string; fields: EvaluatorField[];
...
export abstract class EvaluatorService {
  ...
  abstract tryGetByIdWithFields(input: { id: string; projectId: string }): Promise<EvaluatorWithFields | null>;
  abstract getByIdWithFields(input: { id: string; projectId: string }): Promise<EvaluatorWithFields>;
  abstract tryGetBySlug(input: { slug: string; projectId: string }): Promise<Evaluator | null>;
  abstract tryGetByWorkflow(input: { workflowId: string; projectId: string }): Promise<Evaluator | null>;
  ...
  abstract getCopySource(input: { projectId: string; evaluatorId: string }): Promise<{ copy: Evaluator; source: Evaluator }>;
  abstract getHistory(input: { evaluatorId: string; projectId: string }): Promise<EvaluatorHistoryEntry[]>;
}
