...
export function EvaluatorCard({
  ...
  const Icon = evaluatorTypeIcons[evaluator.type as keyof typeof evaluatorTypeIcons] ?? CheckSquare;
  const typeLabel =
    evaluatorTypeLabels[evaluator.type as keyof typeof evaluatorTypeLabels] ?? evaluator.type;
  const evaluatorType = (evaluator.config as { evaluatorType?: string } | null)?.evaluatorType;
          ...
          <HStack width="full">
            ...
            <Menu.Root>
              <Menu.Trigger
                ...
                // The trigger's only child is an icon, so without this the row
                // actions have no accessible name at all: a screen reader
                // announces "button" and nothing else, and with a grid of them
                // there is no way to tell which card one belongs to. The online
                // evaluations table's trigger has always carried the same label.
                aria-label={`Actions for ${evaluator.name}`}
