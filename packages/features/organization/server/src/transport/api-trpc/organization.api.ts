...
/**
 * The authenticated principal, as the process's session carries it. The whole user travels
 * rather than the id alone because three collaborators — the plan provider, the seat guard
 * and the disable guard — identify the operator by more than their id.
 */
...
export class OrganizationTrpcApi {
  ...
  >(
    ...
    const { protected: procedure, policy, auditLogPolicy, validateOutput } = procedures;
    ...
    return (
      createTrpcService({
        ...
        .mutation("createAndAssign", (p) =>
          p
            .withInput(createAndAssignInputSchema)
            .withOutput(organizationCreatedSchema)
            .withPermission(BEFORE_MEMBERSHIP)
        ...
        /**
         * Disables or re-enables a membership so an organization can reconcile down to
         * the seats its license covers. See seat-reconciliation.feature.
         */
        .mutation("setMemberDisabled", (p) =>
          p
        ...
        .query("getAll", (p) =>
          p
            ...
            // FullyLoadedOrganization[]: Organization joined to Team, TeamUser,
            // CustomRole and the project feature's Project row, several fields
            // redacted per viewer. No existing schema for that aggregate; see
            // the file-level note.
            .withoutOutput(
              "FullyLoadedOrganization[] nests rows from four types with no contract schema yet (organization.rows.ts Team/TeamUser/CustomRole plus the project feature's own Project); deriving one is a follow-up",
            )
            .withPermission(BEFORE_MEMBERSHIP)
            .handle(async ({ ctx, input }) => {
              ...
              const userRoleBindings =
                orgIds.length > 0
                  ? await ports.listBindingsForSynthesis(ctx, { orgIds, userId })
                  : [];
              ...
              for (const organization of organizations) {
                ...
                if (organization.s3AccessKeyId) {
                  organization.s3AccessKeyId = ports.decryptStoredSecret(
        ...
        .query("getOrganizationWithMembersAndTheirTeams", (p) =>
          p
            ...
            // OrganizationWithMembersAndTheirTeams nests OrganizationUser +
            // User + TeamUser + Team rows with no contract schema yet; see the
            // file-level note.
            .withoutOutput(
              "OrganizationWithMembersAndTheirTeams nests OrganizationUser, User, TeamUser and Team rows with no contract schema yet; deriving one is a follow-up",
            )
        ...
        /**
         * Tightened from `organization:view` to manage — exposing one member's full record (role
         * assignments, team memberships) is an admin-surface read, not a peer-context read. No TS
         * callers currently depend on member-role access to this procedure.
         */
        .query("getMemberById", (p) =>
          p
            .withInput(organizationApiMemberScopeSchema)
            // OrganizationMemberWithUser nests OrganizationUser, User and
            // Team rows with no contract schema yet; see the file-level note.
            .withoutOutput(
              "OrganizationMemberWithUser nests OrganizationUser, User and Team rows with no contract schema yet; deriving one is a follow-up",
            )
            .withPermission(ORGANIZATION_MANAGE)
            .handle(async ({ input, ctx }) => {
        ...
        /**
         * Tightened from `organization:view` to manage — pending invites expose admin intent
         * (who's being added, with what role / to which teams). MEMBER reading this is a leak.
         * Both TS callers (settings/members, SubscriptionPage) are admin-only surfaces.
         */
        .query("getOrganizationPendingInvites", (p) =>
          p
        ...
        .mutation("updateTeamMemberRole", (p) =>
          p
            .withInput(updateTeamMemberRoleInputSchema)
            .withOutput(organizationWriteAckSchema)
            .withPermission(ORGANIZATION_MANAGE_VIA_TEAM)
            .handle(async ({ input, ctx }) => {
