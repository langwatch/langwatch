...
import { EvaluationAnalyticsRollupMapProjection } from "../projections/evaluation-analytics-rollup.projection.ts";

const TENANT = "proj-1";

const map = EvaluationAnalyticsRollupMapProjection.create({
  store: { append: async () => {} },
});
...
function makeCompleted(
  ...
  return evaluationCompletedEventSchema.parse({
    ...
    createdAt: Date.UTC(2026, 5, 20, 10, 17, 33),
