...
function buildDeps({
  ...
  return buildExecutionDeps({
    monitor,
    settingsRecoveryDisabled: isSettingsRecoveryDisabled,
    azureCredentials: {},
  });
}
...
async function executeWith(
  ...
  const executeForTrace = deps.evaluations.executeForTrace;
  expect(executeForTrace).toHaveBeenCalledTimes(1);
  const call = executeForTrace.mock.calls[0]?.[0];
  if (!call) throw new Error("executeForTrace was not called");
  return call;
}
