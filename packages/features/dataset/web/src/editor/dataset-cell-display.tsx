...
import { useDatasetTable } from "./dataset-table-context";
import {
  formatJsonCellValue,
  JSON_LIKE_TYPES,
  truncateCellValue,
...
export function DatasetCellDisplay({
  ...
  const { rowHeightMode, expandedCells, toggleCellExpanded, renderImage } =
    useDatasetTable();
  ...
  const displayValue = useMemo(() => {
    ...
    const formatted = isJsonType
      ? formatJsonCellValue(value)
  ...
  return (
    ...
    >
      <Box
        ...
        height="100%"
        maxHeight={
          isCompact ? `${COMPACT_MAX_HEIGHT}px` : isExpanded ? expandedMaxHeight : void 0
        }
