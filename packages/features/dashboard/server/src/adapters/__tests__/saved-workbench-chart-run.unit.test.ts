...
function serviceWithSavedChart() {
  ...
  const dashboard = PostgresDashboardAdapter.create({
    // The one row this path reads, as itself: the repository's database port
    // is the whole generated client, and a fake that satisfied its type would
    // be a thousand lines of methods this test never calls.
    database: { customGraph: { findFirst: async () => chart } } as unknown as Parameters<
