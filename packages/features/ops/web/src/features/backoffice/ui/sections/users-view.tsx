...
export default function UsersView() {
  ...
  // there's exactly one Dialog instance at a time — avoids the "mount a
  // Dialog per row" smell from the previous ImpersonateButton component.
                  ...
                  <Box width="full" height="full" display="flex" justifyContent="end">
                    <Menu.Root>
                      ...
                      <Menu.Content>
                        ...
                        {!user.deactivatedAt && (
                          <Menu.Item value="impersonate" onClick={() => setImpersonating(user)}>
                            <UserCheck size={16} />
                            Impersonate
                          </Menu.Item>
                        )}
...
export function ImpersonateDialog({
  ...
  const submit = async () => {
    ...
    try {
      ...
      // The application decides where an impersonated session lands, and it is
      // a navigation rather than a page load: `window.location` is one of the
      // browser reaches ADR-004 seals off from a screen closure.
      host.navigate("/");
    } catch (err) {
      showErrorToast({
        error: err,
...
function UserEditDrawer({ user, onClose }: { user: AdminUser | null; onClose: () => void }) {
  const showErrorToast = useShowErrorToast();
  const toaster = useOpsToaster();
