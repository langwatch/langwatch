...
import { BlobToolbar } from "../blocks/blob-toolbar";

import { BlobStoreBody } from "./blob-store-body";
import { DeletePayloadDialog } from "./delete-payload-dialog";
import { RunCleanupDialog } from "./run-cleanup-dialog";
import { useBlobListing } from "../../behavior/use-blob-listing";
import { useBlobStoreActions } from "../../behavior/use-blob-store-actions";
  ...
  return (
    <>
      <Text textStyle="sm" color="fg.muted" marginBottom={4}>
        Stored job payloads, what still references them, and what the next cleanup would do with
        each. Payload contents are never shown.
      </Text>
      ...
      {listing.rankedFromSample && (
        <Text textStyle="xs" color="fg.muted" marginBottom={3}>
          Ordered from a sample of {listing.sampled} payloads, not the whole store. Use storage
          order to walk everything.
        </Text>
      )}
