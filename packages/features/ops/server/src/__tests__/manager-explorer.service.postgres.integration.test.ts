...
describe.skipIf(!DB_URL)("process ops against a real Postgres", () => {
  let connection: PrismaConnection;
  let prisma: PrismaClient;

  const ns = `opstest.${randomUUID().slice(0, 8)}`;
  ...
  const ACTOR = `user_opstest_${randomUUID().slice(0, 6)}`;
  ...
  /** This block's rows only, narrowed on `nsDead`, not on `ns`, for the
   *  reason given above. */
