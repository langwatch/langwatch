...
import { BlobStoreRepository } from "../blob-store.repository";
import type { BlobDeleteResult } from "../blob-store.repository";
...
export class BlobStoreRedisRepository extends BlobStoreRepository {
  private readonly redis: IORedis | Cluster;
  private readonly sweeper: BlobSweeper;

  private constructor(redis: IORedis | Cluster) {
    super();
    this.redis = redis;
    this.sweeper = new BlobSweeper({ redis });
  }

  static create(redis: IORedis | Cluster): BlobStoreRedisRepository {
    return new BlobStoreRedisRepository(redis);
  }
