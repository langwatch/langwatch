...
import { ConnectYourAgentButton } from "../../ui/sections/connect-your-agent-button";
...
import { TraceIngestSection } from "../../ui/sections/trace-ingest-section";
...
export function PersonalOverviewScreen() {
  ...
  const bundledThisMonthUsd = Math.max(0, summary.spentThisMonthUsd - summary.billedThisMonthUsd);
  ...
  const maxDay = Math.max(
    ...spendByDay.map((d) => Math.max(showTheoretical ? d.usd : 0, showBilled ? d.billedUsd : 0)),
    0.01,
  );
  const maxTool = Math.max(
    ...spendByTool.map((t) => Math.max(showTheoretical ? t.usd : 0, showBilled ? t.billedUsd : 0)),
    0.01,
  );
...
export default PersonalOverviewScreen;
