...
describe("user avatar validation", () => {
  ...
  it("decodes an allowed image whose signature matches", () => {
    expect(
      avatars.parse(`data:image/png;base64,${PNG_BASE64}`).mediaType,
    ).toBe("image/png");
  });

  it("refuses active or mislabeled content", () => {
    expect(() =>
      avatars.parse("data:image/svg+xml;base64,PHN2Zz48L3N2Zz4="),
    ).toThrow(UserAvatarValidationError);
    expect(() =>
      avatars.parse(
