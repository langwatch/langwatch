...
const AUTH_SCOPE_WRITES = ["create", "update", "delete", "manage"].flatMap((action) =>
...
const CEILING_ESCAPING_GRAINS = [
  ...
  // so `virtualKeys:manage` is withheld as a single grain.
  "virtualKeys:manage",
...
describe("classifyForLangy", () => {
  ...
  describe("given a write on the auth scope", () => {
    describe("when it is classified", () => {
      ...
      it.each(AUTH_SCOPE_WRITES)("refuses %s, with the reason stated", (permission) => {
  ...
  describe("given an action the user-permission ceiling does not bound", () => {
    describe("when it is classified", () => {
      it.each(CEILING_ESCAPING_GRAINS)(
        "refuses %s on every family, trusted or not",
        (permission) => {
          expect(classifyForLangy(permission).disposition).toBe("excluded");
        },
      );
  ...
  describe("given a grain the policy allows on a family only an org-tier binding can grant", () => {
    describe("when it is classified", () => {
      it.each(["organization:view", "governance:manage", "activityMonitor:view", "aiTools:manage"])(
        "calls %s unreachable, not granted and not excluded",
        (permission) => {
          const verdict = classifyForLangy(permission);
