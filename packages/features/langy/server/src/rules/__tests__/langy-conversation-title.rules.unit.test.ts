...
import { normalizeLangyConversationTitle } from "../langy-conversation-title.rules";
...
describe("normalizeLangyConversationTitle", () => {
  ...
  describe("given raw text a user typed", () => {
    ...
    it("capitalises the first word and leaves the rest alone", () => {
      expect(normalizeLangyConversationTitle("instrument my traces with langwatch")).toBe(
        "Instrument my traces with langwatch",
      );
    });
  });
  ...
  describe("given a title longer than the character budget", () => {
    ...
    it("cuts it on a word boundary", () => {
      ...
      expect(title.length).toBeLessThanOrEqual(LANGY_TITLE_GENERATION.MAX_TITLE_CHARS);
      expect(title).toBe("Instrument the checkout service traces with LangWatch and");
    });
