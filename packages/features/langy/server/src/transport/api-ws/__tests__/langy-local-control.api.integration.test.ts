...
/** A project key: a real credential, and the wrong kind here. */
const projectApiKey = `sk-lw-${nanoid(48)}`;
/** The developer's own key: a real credential, and the wrong kind here. */
const personalToken = `sk-lw-${nanoid(48)}`;
...
/**
 * The session keys this suite mints and the directory that resolves them. A
 * stand-in for the API-key service, because approving a control request mints
 * through a port this process composes to a refusal.
 */
  ...
  async tryResolveToken({ token }: { token: string }): Promise<ResolvedApiKeyToken | null> {
    ...
    return {
      ...
      project: {
        ...
        slug: "local-control-project",
        teamId: "team_local_control",
        organizationId,
        isPersonal: false,
        ownerUserId: null,
      },
...
function testPorts(store: AgentStateStorePort) {
  const runtime = LangyLocalControlRuntimeAdapter.create({
    ...
    mintSessionKey: async ({ userId: owner }) => {
      ...
      mintedKeys.set(token, { apiKeyId, userId: owner });
      return { token, apiKeyId };
    },
    offlineWaitMs: 200,
    pollIntervalMs: 25,
    events: {
