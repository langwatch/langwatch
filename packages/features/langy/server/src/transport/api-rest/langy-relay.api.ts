...
/** Everything the relay reaches that Langy does not own. */
export type LangyRelayRestPorts = Readonly<{
  ...
  /** One counter per frame outcome, so the rates stay graphable fleet-wide. */
  metrics: Readonly<{
    frames(outcome: "applied" | "duplicate" | "rejected" | "terminal", count: number): void;
  }>;
}>;

/** Builds the `/api/internal/langy/relay` family over one process's ports. */
export function createLangyRelayRestApp(options: {
  ...
  secured.access(langyInternalPolicy()).post("/relay/frames", async (c) => {
    ...
    const relay = ports.langy().langyService.openRelayConnection();
    ...
    ports.metrics.frames("applied", tally.applied);
