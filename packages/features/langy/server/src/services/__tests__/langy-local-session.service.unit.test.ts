...
import {
  ...
  conversationUrl,
  disconnectMessage,
} from "../langy-local-session.service";
...
describe("conversationUrl", () => {
  describe("when the project the conversation belongs to is known", () => {
    ...
    it("points at that project's home page", () => {
      expect(conversationUrl("conv_1", "https://app.langwatch.ai", "acme-support")).toBe(
        "https://app.langwatch.ai/acme-support?langyConversation=conv_1",
      );
  ...
  describe("when the origin is missing or has no scheme", () => {
    it("returns the path rather than a guessed origin", () => {
      ...
      expect(conversationUrl("conv_1", "localhost:5570")).toBe("/?langyConversation=conv_1");
    });
  });
});
...
describe("conversationTitle", () => {
  describe("when Langy already named the conversation", () => {
    it("uses the name as it is", () => {
      expect(conversationTitle("Instrument tracing")).toBe("Instrument tracing");
    });
  });
  ...
  describe("when the conversation has no name yet", () => {
    ...
    it("cuts the first message back to a whole word", () => {
      const placeholder =
        "Please add a health endpoint to the support service and " + "write a test for it";
      const short = conversationTitle(placeholder);

      expect(short).toBe("Please add a health endpoint to the support service and\u2026");
    ...
    it("leaves no space or punctuation before the ellipsis", () => {
      expect(conversationTitle(`${"word ".repeat(20)}end`)).not.toMatch(/[\s.,;:!?-]\u2026$/);
    });
...
describe("disconnectMessage", () => {
  ...
  describe("when the folder has no name", () => {
    it("falls back to the path", () => {
      expect(disconnectMessage({ name: "", root: "/srv/checkout" }, "build-box")).toBe(
        "Local folder disconnected: /srv/checkout on build-box",
      );
    });
  });
});
