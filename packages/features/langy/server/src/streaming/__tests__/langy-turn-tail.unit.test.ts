...
import type { LangyStreamEntry, LangyStreamRead } from "../langy-token-buffer";
import type { TurnHealth } from "../langy-turn-settlement";
import { WEDGED_TURN_PATIENCE_MS } from "../langy-turn-settlement";
import { streamTurnEntries, type TurnTailBuffer } from "../langy-turn-tail";
...
const immediately = () => new Promise<boolean>((resolve) => setTimeout(() => resolve(true), 0));
...
describe("streamTurnEntries", () => {
  ...
  describe("when the turn keeps beating far longer than one manager request", () => {
    ...
    it("keeps delivering the agent's UI actions to the page that is watching", async () => {
      ...
      await vi.waitFor(() => expect(polls * pollMs).toBeGreaterThan(MANAGER_REQUEST_BUDGET_MS));
  ...
  describe("when the buffer already holds the whole turn", () => {
    it("replays it and ends without following the live edge", async () => {
      ...
      await done;
      expect(received).toEqual([{ type: "delta", text: "hi" }, { type: "end" }]);
