...
describe("settleWait", () => {
  describe("given a card this stream never carried", () => {
    ...
    it("records the answer anyway, because the record has not caught up", () => {
      useLangyLocalControlStore.getState().settleWait({ waitId: "wait-7", status: "answered" });

      expect(useLangyLocalControlStore.getState().waits["wait-7"]).toMatchObject({
        waitId: "wait-7",
        kind: "permission",
        status: "answered",
      });
  ...
  describe("given a question card", () => {
    it("keeps the kind the answer belonged to", () => {
      ...
      expect(useLangyLocalControlStore.getState().waits["wait-q"]?.kind).toBe("question");
    });
  });
});
...
describe("recordWait", () => {
  ...
  describe("given an entry for another conversation", () => {
    it("drops it, because nobody is reading that one", () => {
      useLangyLocalControlStore.getState().recordWait({ conversationId: "conv-2", wait: live() });
