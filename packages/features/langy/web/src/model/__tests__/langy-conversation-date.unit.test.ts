...
import { formatLangyConversationDate } from "../langy-conversation-date";
...
describe("formatLangyConversationDate", () => {
  ...
  it("includes the year only for older conversations", () => {
    expect(formatLangyConversationDate(Date.parse("2026-07-10T08:00:00.000Z"), NOW)).toMatch(
      /Jul 10/,
    );
    expect(formatLangyConversationDate(Date.parse("2025-12-10T08:00:00.000Z"), NOW)).toMatch(
      /2025/,
    );
  });
