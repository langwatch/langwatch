  ...
  describe("given the conversation's durable record alone", () => {
    ...
    describe("when the turn document has moved on", () => {
      it("keeps the settled state, whichever of the two holds it", () => {
        const cards = langyPermissionCards({
          record: [{ ...permissionWait({ waitId: "wait-1" }), toolCallId: "call-1" }] as never,
          toolCalls: [
            toolCall({
              wait: permissionWait({
                status: "answered",
...
describe("mergeLangyWaitStatus", () => {
  describe("given one side still pending", () => {
    it("takes the terminal state, whichever side reported it", () => {
      expect(mergeLangyWaitStatus({ durable: "pending", live: "expired" })).toBe("expired");
      expect(mergeLangyWaitStatus({ durable: "cancelled", live: "pending" })).toBe("cancelled");
    });
  });

  describe("given both sides pending", () => {
    it("stays pending", () => {
      expect(mergeLangyWaitStatus({ durable: "pending", live: "pending" })).toBe("pending");
...
describe("toolCallIdOfQuestionBlock", () => {
  describe("given a block id the question bridge minted", () => {
    it("reads the tool call back out of it", () => {
      ...
      expect(toolCallIdOfQuestionBlock("question:call:with:colons:2")).toBe("call:with:colons");
    });
  });
...
describe("routeLangyChoiceAnswer", () => {
  ...
  describe("given a question the tool is still waiting on", () => {
    ...
    it("returns the answer to the wait, not to the composer", () => {
      expect(routeLangyChoiceAnswer({ blockId: "question:call-q1:0", waits })).toEqual({
        kind: "wait",
        waitId: "wait-q1",
      });
    });
  ...
  describe("given a question whose wait already ended", () => {
    ...
    it("sends the answer as the next user message", () => {
      expect(routeLangyChoiceAnswer({ blockId: "question:call-q2:0", waits })).toEqual({
        kind: "message",
      });
    });
  });
