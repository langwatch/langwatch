...
function selectCommonLayout(contributors: Map<string, MetricRollupSourcePoint>): CommonLayout {
...
function collectContributors({
  ...
  return new Map<string, MetricRollupSourcePoint>([
    ...entries.map(({ point }) => [point.pointId, point] as [string, MetricRollupSourcePoint]),
    ...predecessors,
  ]);
}
