...
function mergedIndexSpan({
  ...
  for (const side of ["positive", "negative"] as const) {
    ...
    for (const point of points) {
      const counts = side === "positive" ? point.positiveBucketCounts : point.negativeBucketCounts;
      if (counts.length === 0) continue;
      const offset = (side === "positive" ? point.positiveOffset : point.negativeOffset) ?? 0;
...
function selectCommonLayout(contributors: Map<string, CanonicalMetricDataPoint>): CommonLayout {
  ...
  const threshold = commonZeroThreshold({
    ...
    bucketMaps: [...downscaled.values()].flatMap(({ positive, negative }) => [positive, negative]),
    scale,
  });
...
function differenceExponentialPoint({
  ...
  return {
    ...
    zeroCount,
    count,
    sum: previous.sum !== null && current.sum !== null ? current.sum - previous.sum : null,
  };
}
...
function collectContributors({
  ...
  return new Map<string, CanonicalMetricDataPoint>([
    ...entries.map(({ point }) => [point.pointId, point] as [string, CanonicalMetricDataPoint]),
    ...predecessors,
  ]);
}
