...
function validateExplicitHistogram(point: UnknownRecord): void {
  ...
  if (point.bucketCounts.length !== bounds.length + 1) {
    throw new Error("histogram bucketCounts must have exactly one more entry than explicitBounds");
  }
...
function validatePointShape({ point, kind }: { point: UnknownRecord; kind: MetricKind }): void {
