 ...
 * contract reads this package, so importing it back would close a cycle over
 * one suffix.
 */
function isSuiteSetId(setId: string): boolean {
  return setId.endsWith("__suite");
}
...
describe("agent test scenario", () => {
  describe("given the set id of a project's agent tests", () => {
    ...
    it("leaves the other internal sets alone", () => {
      ...
      expect(isAgentTestSetId("__internal__proj_1__on-platform-scenarios")).toBe(false);
