...
const CONNECTED_CALL_FAILURE = /Connected agent call failed \(([a-z_0-9]+)\):\s*([^\n]*)/;
...
> = {
  ...
  agent_call_timeout: {
    ...
    message: "The connected agent did not answer a turn before its call budget.",
  ...
  agent_disconnected: {
    ...
    message: "The connected agent instance disconnected while it was working on a turn.",
    hint: "Start the process again, then run again. The turn was not sent twice.",
  },
  agent_instance_lost: {
    code: ScenarioInfraErrorCode.AgentInstanceLost,
    message: "The instance this conversation was pinned to is no longer connected.",
  ...
  agent_busy: {
    ...
    message: "Every instance of the connected agent stayed busy for the whole retry budget.",
...
const SESSION_TOO_LARGE_SIZES = /a session of (\d+) bytes, above the limit of (\d+) bytes/;
...
function sessionTooLargeRule(): ClassificationRule {
  return {
    ...
    build: (text) => {
      ...
      const detail = sizes ? ` It is ${sizes[1]} bytes; the limit is ${sizes[2]} bytes.` : "";
      return {
        code: ScenarioInfraErrorCode.AgentPayloadTooLarge,
...
function modelEndpointUnreachableRule(): ClassificationRule {
  return {
    ...
    build: () => ({
      ...
      message: "The simulation could not reach the model endpoint, so no model answered.",
