...
import {
  ...
  STALL_THRESHOLD_MS,
} from "../src/processes/simulation-run-execution-evolution.process";
import { simulationRunExecutionPM } from "../src/processes/simulation-run-execution.process";
import {
  CANCEL_GRACE_MS,
...
import { TestSimulationService } from "../src/testing";

const PROJECT_ID = "project-1";
const RUN_ID = "run-1";
const TARGET = { type: "prompt" as const, referenceId: "prompt-1" };

class UnusedScenarioExecutionService extends ScenarioExecutionService {
  submit(_input: ScenarioExecutionJob): Promise<void> {
    return Promise.reject(new Error("unused in evolve tests"));
  }
  ...
  prefetch(_input: ScenarioExecutionPrefetchInput): Promise<ScenarioExecutionPrefetchResult> {
    throw new Error("prefetch unexpectedly called in evolve tests");
  }

  prepare(_input: ScenarioExecutionPrefetchInput): ScenarioExecutionPreparation {
    throw new Error("prepare unexpectedly called in evolve tests");
  }
...
const definition = buildProcessDefinition(
  buildProcessManager<SimulationProcessingEvent>({
    ...
    applier: simulationRunExecutionPM(
      new UnusedScenarioExecutionService(),
      TestSimulationService.create({
        finishRun: () => Promise.reject(new Error("unused in evolve tests")),
      }),
    ),
  }).config,
...
describe("simulationRunExecution process (runtime-built definition)", () => {
  ...
  describe("when the queued event records the run's parameters", () => {
    ...
    function executeIntentFor(metadata?: Record<string, unknown>) {
      ...
      const payload = evolution.intents[0]?.payload as Record<string, unknown> | undefined;
  ...
  describe("when the queued event declares a secret it carries no value for", () => {
    ...
    it.each([
      ...
      const finish = finishRunIntentSchema.parse(evolution.intents[0]?.payload);
      expect(finish.error).toContain("api_token");
    });
  ...
  describe("when the run reaches a recorded terminal state", () => {
    it.each([SIMULATION_RUN_EVENT_TYPES.FINISHED, SIMULATION_RUN_EVENT_TYPES.DELETED] as const)(
      "goes terminal and clears the wake on %s",
      (type) => {
        const evolution = evolveEvent(
          runningState(),
          makeEvent({
