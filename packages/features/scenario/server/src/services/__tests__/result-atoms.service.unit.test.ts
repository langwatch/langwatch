...
function makeScenarios({
  ...
  const plans: ScenarioPlanRecord[] = suites.map((suite) => ({
    id: suite.id,
    name: suite.name,
    slug: suite.slug,
    kind: "run_plan",
  ...
  return {
    ...
    findIdsByLabelsOrTestSuites: vi
      .fn()
      .mockImplementation(async ({ labels }: { labels?: string[] }) =>
        scenarios
          .filter((scenario) => (labels ?? []).some((label) => scenario.labels.includes(label)))
...
describe("getOverview", () => {
  describe("when grouping by run plan and a plan did not run in the window", () => {
    ...
    it("still lists the plan, reading as nothing in the period", async () => {
      ...
      const service = ResultAtomsService.create(
        makeRepo({ groups: [group({ GroupKey: getSuiteSetId(ranSuite.id) })] }),
        makeScenarios({ suites: [ranSuite, quietSuite] }),
      );
  ...
  describe("the cost of a group", () => {
    ...
    it("states its own coverage alongside the total", async () => {
      const service = ResultAtomsService.create(
        makeRepo({ groups: [group({ Atoms: "6", CostTotal: "1.25", CostUnknown: "2" })] }),
        makeScenarios(),
      );

      const overview = await service.getOverview({ filter, groupBy: "target" });

      expect(overview.groups[0]?.cost).toEqual({ totalUsd: 1.25, knownAtoms: 4, unknownAtoms: 2 });
    });
  });
  ...
  describe("the trend of a group", () => {
    ...
    it("reads oldest first", async () => {
      ...
      expect(overview.groups[0]?.trend.map((point) => point.key)).toEqual(["b1", "b2", "b3"]);
  ...
  describe("the pass-rate-over-time series", () => {
    ...
    it("returns every bucket in the window, marking the empty ones", async () => {
      ...
      const service = ResultAtomsService.create(
        makeRepo({ series: [{ Bucket: String(dayStart), Passed: "1", Settled: "2" }] }),
        makeScenarios(),
      );
      ...
      expect(overview.totals.series.every((one) => !one.isEmpty || one.passRate === null)).toBe(
        true,
      );
    });
  });
  ...
  describe("when the filter names a label", () => {
    ...
    it("resolves it to the scenarios that carry it", async () => {
      ...
      const service = ResultAtomsService.create(repo, scenarios);
      ...
      expect(repo.aggregateGroups).toHaveBeenCalledWith(
        expect.objectContaining({ filter: expect.objectContaining({ scenarioIds: ["scen-1"] }) }),
      );
    });
    ...
    describe("and also names a scenario the label does not cover", () => {
      ...
      it("intersects them rather than widening", async () => {
        ...
        expect(repo.aggregateGroups).toHaveBeenCalledWith(
          expect.objectContaining({ filter: expect.objectContaining({ scenarioIds: [] }) }),
        );
      });
    });
  });
  ...
  describe("when grouping by scenario", () => {
    ...
    it("names each group after its scenario and lists its labels", async () => {
      const service = ResultAtomsService.create(
        ...
        makeScenarios({
          scenarios: [{ id: "scen-1", name: "Refund flow", labels: ["checkout", "beta"] }],
        }),
      );

      const overview = await service.getOverview({ filter, groupBy: "scenario" });
    ...
    it("names a group the project holds no scenario for after the name its runs carried", async () => {
      const service = ResultAtomsService.create(
        makeRepo({ groups: [group({ GroupKey: "default-list-agents", Name: "List agents" })] }),
...
describe("getRunTargets", () => {
  describe("given a target a run from code named", () => {
    ...
    it("lists it under the name the run reported, pointing at no stored row", async () => {
      ...
      const targets = await service.getRunTargets({ projectId: "proj", startDate });
...
describe("the target parameters of an atom", () => {
  ...
  describe("given a target group whose target carried overrides", () => {
    ...
    it("carries them on the group, and on no other grouping", async () => {
      ...
      const byTarget = await ResultAtomsService.create(
        makeRepo({ groups: [variant] }),
        makeScenarios(),
      ).getOverview({ filter, groupBy: "target" });
      const byScenario = await ResultAtomsService.create(
        makeRepo({ groups: [variant] }),
        makeScenarios(),
      ).getOverview({ filter, groupBy: "scenario" });

      expect(byTarget.groups[0]?.targetParameters).toEqual({ model: "gpt-5-mini" });
...
describe("getAtoms", () => {
  describe("given an atom whose cost was never measured", () => {
    ...
    it("returns no number for it and names the source as unknown", async () => {
      ...
      const service = ResultAtomsService.create(makeRepo({ atoms: [row] }), makeScenarios());
...
describe("bucketSecondsFor", () => {
  describe("given a window a person reads in hours", () => {
    it("buckets by the hour", () => {
      expect(bucketSecondsFor({ startDate: now - 24 * 3600_000, endDate: now })).toBe(3600);
    });
  });

  describe("given a month", () => {
    it("buckets by the day", () => {
      expect(bucketSecondsFor({ startDate: now - 30 * 86400_000, endDate: now })).toBe(86400);
    });
  });

  describe("given a year", () => {
    it("buckets by the week", () => {
      expect(bucketSecondsFor({ startDate: now - 365 * 86400_000, endDate: now })).toBe(7 * 86400);
    });
  });
});
