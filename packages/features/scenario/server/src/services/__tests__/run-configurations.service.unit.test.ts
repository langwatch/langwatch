...
function row(overrides: Partial<RawRunConfigurationRow>): RawRunConfigurationRow {
...
describe("Feature: reading a configuration back off the runs", () => {
  describe("given a plan row holding the configuration of its last run", () => {
    ...
    it("keys the same as the client helper the dialog keys with", () => {
      ...
      // replaces built it.
      const fromPlanRow = configurationKey({
        config: {
          scope: { mode: "scenarios" },
          targets: sortSuiteTargets(targets),
          repeatCount: 2,
      ...
      const entry = toEntry({
        row: row({
          ...
          TargetParameters: [JSON.stringify({ seats: 12, region: "eu-central" }), ""],
          RepeatCount: "2",
          SimulatorModel: "openai/gpt-5-mini",
      ...
      expect(entry.configuration.targets).toEqual(expect.arrayContaining(targets));
  ...
  describe("given a plan whose prompt target carries scenario mappings", () => {
    ...
    it("reopens the target with those mappings", () => {
      const stored: SuiteTarget = {
        ...
        scenarioMappings: { question: { type: "value", value: "a refund request" } },
      };
  ...
  describe("given a run whose target the plan no longer holds", () => {
    it("still reopens the target the run named", () => {
      const target = toTarget({
        ...
        parameters: "",
        storedTargets: NO_PLAN_TARGETS,
      });

      expect(target).toEqual({ type: "workflow", referenceId: "removed-agent" });
    });
    ...
    it("drops a pair whose type is not one a run can name", () => {
      expect(
        toTarget({ pair: "carrier-pigeon:x", parameters: "", storedTargets: NO_PLAN_TARGETS }),
      ).toBeNull();
      expect(
        toTarget({ pair: "http:", parameters: "", storedTargets: NO_PLAN_TARGETS }),
      ).toBeNull();
    });
  });
  ...
  describe("given two entries of one configuration", () => {
    it("keeps the newest and orders newest first", () => {
      ...
      const older = toEntry({ row: row({ LastRunAtMs: String(LAST_RUN_MS - 1000) }), plan });
