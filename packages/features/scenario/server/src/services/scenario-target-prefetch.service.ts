...
async function tryGetAgent(
  agents: AgentService,
  projectId: string,
  agentId: string,
): Promise<Agent | null> {
  try {
    return await agents.getById({ projectId, id: agentId });
  } catch (error) {
...
async function tryGetLatestWorkflow(
...
async function getExecutionSecrets(
  secrets: SecretService,
  projectId: string,
  runSecretValues: Record<string, string>,
): Promise<Record<string, string>> {
  return {
    ...(await secrets.getValues({ projectId })),
    ...runSecretValues,
  };
}
...
export class ScenarioTargetPrefetchService {
  ...
  async tryFetch({
    ...
    if (target.type === "prompt") {
      return fetchPromptConfigData(projectId, target.referenceId, this.options.prompts);
    }
    if (target.type === "code") {
    ...
    if (target.type === "workflow") {
      return fetchWorkflowAgentData({
        ...
        workflowHydrator: this.options.workflowHydrator,
...
async function fetchPromptConfigData(
  ...
  const prompt = await prompts.tryGetPromptByIdOrHandle({
    projectId,
    idOrHandle: promptId,
  });
  if (!prompt) {
...
async function fetchHttpAgentData({
  ...
  const agent = await tryGetAgent(agents, projectId, agentId);
  if (agent?.type !== "http") {
    return null;
  }
  ...
  const secretValues = await getExecutionSecrets(secrets, projectId, runSecretValues);
...
async function fetchWorkflowAgentData({
  ...
  workflows: WorkflowService;
  workflowHydrator: ScenarioWorkflowHydratorService;
  workflowMappings: ScenarioWorkflowMappingService;
  secrets: SecretService;
  runSecretValues: Record<string, string>;
  legacyDefaultModel: string;
  ...
  const latest = await tryGetLatestWorkflow(workflows, projectId, workflowId);
  if (!latest) {
    return null;
  }

  const hydrateResult = await workflowHydrator.hydrate({
    dsl: latest.dsl,
    projectId,
    legacyDefaultModel,
  });
  ...
  const { inputs, outputs } = workflowHydrator.extractWorkflowIO(hydrateResult.dsl);
  ...
  workflowMappings.validate(data);
