...
describe("scenarioTabRegistry", () => {
  ...
  it("never leaks a registration across machines", async () => {
    const mine = `tab-${randomUUID()}`;
    const theirs = `tab-${randomUUID()}`;
  ...
  describe("parked handoffs", () => {
    ...
    it("hands a parked run to the next subscription", async () => {
      const tabKey = `tab-${randomUUID()}`;
      pendingKeys.push(ScenarioTabRegistryService.pendingKey(projectId, tabKey));
    ...
    it("expires a parked run so a tab opening much later does not jump to it", async () => {
      const tabKey = `tab-${randomUUID()}`;
      const key = ScenarioTabRegistryService.pendingKey(projectId, tabKey);
    ...
    it("keeps parked handoffs apart per machine", async () => {
      const mine = `tab-${randomUUID()}`;
      const theirs = `tab-${randomUUID()}`;
      pendingKeys.push(ScenarioTabRegistryService.pendingKey(projectId, mine));
