...
import { hasNoResults, shouldShowNoResponse } from "../model/scenario-run-status.utils";
...
describe("hasNoResults()", () => {
  ...
  describe("when the run has reached a terminal state with results", () => {
    it.each([ScenarioRunStatus.SUCCESS, ScenarioRunStatus.FAILED, ScenarioRunStatus.ERROR])(
      "returns false for %s",
      (status) => {
...
describe("shouldShowNoResponse()", () => {
  describe("when a finished run produced no messages and no error", () => {
    ...
    it.each([ScenarioRunStatus.SUCCESS, ScenarioRunStatus.FAILED, ScenarioRunStatus.ERROR])(
      "shows the no-response state for %s",
      (status) => {
        expect(
