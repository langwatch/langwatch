...
import { toAgentTestingAddress } from "../behavior/suites/use-suite-routing";
...
describe("toAgentTestingAddress", () => {
  ...
  describe("given the scenario library", () => {
    ...
    it("reads the v1 editor drawer as the case editor", () => {
      ...
      ).toBe("/acme/agent-testing?drawer.open=agentTestingCaseEditor&drawer.scenarioId=scenario_2");
    });
  });

  describe("given a run plan", () => {
    it("opens the plan", () => {
      expect(address(["run-plans", "nightly"])).toBe("/acme/agent-testing/results/nightly");
    });
  ...
  describe("given a legacy suites address", () => {
    it("opens the plan the query names", () => {
      expect(address(["suites"], { suite: "nightly" })).toBe("/acme/agent-testing/results/nightly");
    });
  ...
  describe("given a run set a code run writes into", () => {
    it("opens the set as a plan", () => {
      expect(address(["my-set"])).toBe("/acme/agent-testing/results/external:my-set");
    });
  ...
  describe("given one of the platform's own sets", () => {
    ...
    it("still opens the run the address names", () => {
      expect(address(["__internal__project_1__on-platform-scenarios", "batch_1", "run_1"])).toBe(
  ...
  describe("given query params that describe the view", () => {
    ...
    it("drops a query string that leaked into the segments", () => {
      expect(address(["my-set", "?period=7d"])).toBe("/acme/agent-testing/results/external:my-set");
    });
  });
});
