...
import { type BatchRun, ScenarioRunContent, targetKeyOfRun } from "@langwatch/suite-web";
...
type SelectedRunResultsProps = Pick<
  ...
  "plan" | "batches" | "selection" | "targets" | "cancel" | "viewMode" | "onRerunCase"
> & { batch: BatchRun };
...
function targetNameResolver({
  ...
  return (scenarioRun) => {
    ...
    return targets.find((target) => target.key === key)?.label ?? fallback(scenarioRun);
  };
}
...
export function RunPlanRunResults({
  ...
  if (batches.error) {
    return <RunsLoadError error={batches.error} onRetry={() => void batches.refetch()} />;
  }

  if (batches.isLoading) return <RunsLoadingSkeleton />;

  if (!selection.selectedBatch) {
    const EmptyColumn = selection.awaitedBatchRunId ? WaitingForFirstRun : NoRunInPeriod;
    return (
      <EmptyColumn
