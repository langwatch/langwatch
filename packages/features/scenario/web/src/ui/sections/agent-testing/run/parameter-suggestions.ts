...
export function errorOnLine({
  ...
  const holds = parseParameterLine(line).some(([name, raw]) => pairIsRefused({ name, raw, error }));
...
export function acceptParameterSuggestion({
  ...
  const replacement = state.mode === "field" ? `${row.value}=` : `${state.field}=${row.value}`;
 ...
 */
export function parameterPlaceholder(definitions: readonly DeclaredParameter[]): string {
  const first = definitions.find((definition) => definition.secret !== true);
  if (!first) return PARAMETER_LINE_PLACEHOLDER;
  const value = first.defaultValue !== undefined ? first.defaultValue : (first.options?.[0] ?? "");
