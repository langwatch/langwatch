...
export function batchTargetsOf({
  ...
  const identityOf = (target: { referenceId: string }) => targetIdentities.get(target.referenceId);
  ...
  const ownerNameOf = (target: { referenceId: string }) => identityOf(target)?.ownerName ?? null;
  ...
  const repeated = new Set(
    labelled.map((target) => target.referenceId).filter((id, at, ids) => ids.indexOf(id) !== at),
  );
...
function environmentLabelOf(identity: TargetIdentity | undefined): string | null {
...
export function useBatchTargets(scenarioRuns: ScenarioRunData[]): BatchTarget[] {
