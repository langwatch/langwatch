...
import { PICKER_UNFILED_GROUP_NAME } from "@langwatch/suite-web/suite-pickers";
...
function CaseChoices({
  ...
  const groups = [
    ...testSuites.map((testSuite) => ({
      id: testSuite.id,
      name: testSuite.name,
      scenarios: scenarios.filter((scenario) => scenario.testSuiteId === testSuite.id),
    })),
    {
      id: "__unfiled__",
  ...
  if (scope.mode === "labels") {
    return (
      <LabelChoices
        ...
        chosen={scope.labels}
        onToggle={(label) => onChange({ mode: "labels", labels: toggled(scope.labels, label) })}
      />
    );
  }
    ...
    <Box>
      ...
      >
        ...
        <Text paddingTop={1} fontSize="11px" color={FG_MUTED}>
          {count === 1 ? "1 scenario will run." : `${count} scenarios will run.`}
        </Text>
      </VStack>
    </Box>
  );
