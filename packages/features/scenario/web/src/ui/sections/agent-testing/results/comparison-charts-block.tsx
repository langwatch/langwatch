...
import { formatCost, formatLatency } from "@langwatch/suite-web";
import type { BatchRun, RunGroupSummary } from "@langwatch/suite-web";
import {
  MiniBarCard,
  type MiniBarGroup,
...
function targetGroups({
  ...
  return targets.map((target) => {
    ...
    const { value, text } = summary ? read(summary) : { value: null, text: "-" };
    return {
      key: target.key,
...
const averageLatencyOf = (summary: RunGroupSummary) => ({
  ...
  text: summary.averageAgentLatencyMs === null ? "-" : formatLatency(summary.averageAgentLatencyMs),
});
...
function passRateOverRuns({
  ...
  batchRuns,
  totalBatchCount,
}: Pick<ComparisonChartsBlockProps, "targets" | "batchRuns" | "totalBatchCount">): MiniBarGroup[] {
  return batchRuns
    .map((batch, index) => ({
      ...
      bars: targets.map((target) => {
        ...
        const { value, text } =
          summary.totalCount > 0 ? passRateOf(summary) : { value: null, text: "-" };
