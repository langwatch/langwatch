...
import { ScenarioGenerationError } from "../../../../model/scenario-generation";
...
describe("<AICreateModal/>", () => {
  ...
  // automatic per-test cleanup never registers.
  afterEach(cleanup);
  ...
  describe("given a handled configuration failure", () => {
    describe("when Configure is clicked", () => {
      it("links to settings/model-providers in a new tab", async () => {
        ...
        await waitFor(() => {
          const configureBtn = within(dialog).getByTestId("error-configure-model-provider-button");
