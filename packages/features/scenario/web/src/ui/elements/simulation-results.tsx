        ...
        <VStack alignItems="flex-start" gap={2} pl={2}>
          {criteria.map((criterion: string, idx: number) => (
            <HStack key={idx} align="flex-start" gap={3}>
              <Box w={2} h={2} bg={bulletColor} borderRadius="full" mt={2} flexShrink={0} />
  ...
  return (
    <Box p={6} width="100%" height="100%">
      ...
      <VStack alignItems="flex-start" gap={6} height="100%">
        ...
        <CriteriaSection title="Met Criteria" criteria={results.metCriteria} color="green" />

        <CriteriaSection title="Unmet Criteria" criteria={results.unmetCriteria} color="red" />
      </VStack>
    </Box>
  );
}
