    ...
    scenarios: {
      ...
      getResultAtoms: {
        useQuery: (input: Record<string, unknown>, options: { enabled: boolean }) => {
          ...
          return {
            data: options.enabled ? { atoms: atomState.atoms, hasMore: false } : undefined,
            isLoading: false,
          };
        },
      },
...
describe("the toolbar of the Results tab", () => {
  ...
  describe("when the top of the tab is read", () => {
    ...
    it("reads the filter row, then the charts, then the table", async () => {
      ...
      expect(
        filters.compareDocumentPosition(charts) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    ...
    it("holds the four filters, the period picker and the Charts toggle", () => {
      ...
      expect(within(row).getByRole("button", { name: /scenario/i })).toBeInTheDocument();
      expect(within(row).getByRole("button", { name: /label/i })).toBeInTheDocument();
      expect(within(row).getByRole("button", { name: /target/i })).toBeInTheDocument();
      expect(within(row).getByTestId("results-filter-status")).toBeInTheDocument();
      expect(within(row).getByTestId("results-period-picker")).toBeInTheDocument();
    ...
    it("keeps the stat strip and the chart closed until Charts is chosen", async () => {
      ...
      expect(screen.queryByTestId("agent-testing-results-charts")).not.toBeInTheDocument();
      expect(screen.queryByTestId("results-pass-rate-chart")).not.toBeInTheDocument();

      await user.click(screen.getByTestId("results-charts-toggle"));

      expect(screen.getByTestId("results-stat-pass-rate")).toHaveTextContent("75%");
  ...
  describe("when the Group by control is read", () => {
    ...
    it("draws four connected tabs in one enclosure, not a dropdown", () => {
      ...
      expect(screen.getByTestId("agent-testing-run-plans-table")).toBeInTheDocument();
    });
  });
  ...
  describe("when a filter is chosen", () => {
    ...
    it("narrows the read that both the table and the numbers come from", async () => {
      ...
      await user.click(screen.getByTestId("results-charts-toggle"));
      expect(screen.getByTestId("results-stat-executions")).toHaveTextContent("8");
      ...
      expect(screen.getByTestId("results-stat-executions")).toHaveTextContent("3");
      expect(
        within(screen.getByTestId("run-plan-row-checkout")).getByTestId("pass-rate-text"),
  ...
  describe("when another grouping is chosen", () => {
    ...
    it("lists a scenario once and opens it onto every run of it", async () => {
      ...
      expect(within(opened).getByTestId("results-run-line-exec_1")).toBeInTheDocument();
      expect(within(opened).getByTestId("results-run-line-exec_2")).toBeInTheDocument();
    ...
    it("reads a scenario that ran from code under its name, with every run of that name behind it", async () => {
      ...
      overviewState.byGroupBy.scenario = {
        totals: makeTotals(),
        groups: [makeGroup({ key: "default-list-agents", title: "list agents" })],
      };
      ...
      const row = await screen.findByTestId("results-group-row-default-list-agents");
      expect(row).toHaveTextContent("list agents");
      expect(row).not.toHaveTextContent("scenario_3I9");

      await user.click(row);
      const opened = await screen.findByTestId("results-group-expanded-default-list-agents");
      expect(within(opened).getByTestId("results-run-line-exec_a")).toBeInTheDocument();
      expect(within(opened).getByTestId("results-run-line-exec_b")).toBeInTheDocument();
    ...
    it("offers a scenario that ran from code in the Scenario filter, by its key", async () => {
      const user = userEvent.setup();
      codeScenarioState.scenarios = [{ key: "default-list-agents", name: "list agents" }];
      renderList();

      await user.click(screen.getByTestId("results-filter-scenario"));
      await user.click(await screen.findByRole("menuitemcheckbox", { name: /list agents/i }));

      expect(overviewState.lastInput?.scenarioIds).toEqual(["default-list-agents"]);
    });
    ...
    it("reads From code as the scope and default as the target of a set that runs from code", async () => {
      ...
      const row = screen.getByTestId("run-plan-row-external:default");
      expect(within(row).getByTestId("plan-scope")).toHaveTextContent("From code");
      expect(within(row).getByTestId("plan-targets")).toHaveTextContent("default");
      ...
      expect(within(target).getByTestId("from-code-badge")).toHaveTextContent("from code");
    ...
    it("reads a target a run from code named under that name, with the from code mark", async () => {
      ...
      const row = await screen.findByTestId("results-group-row-code:acmesupportagent");
      expect(row).toHaveTextContent("AcmeSupportAgent");
      expect(within(row).getByTestId("from-code-badge")).toHaveTextContent("from code");
    ...
    it("reads the agent a run from code named in the Targets column of its plan", () => {
      codeTargetState.targets = [{ key: "code:acmesupportagent", name: "AcmeSupportAgent" }];
      overviewState.byGroupBy.plan = {
        totals: makeTotals(),
        groups: [
          makeGroup({
      ...
      const row = screen.getByTestId("run-plan-row-external:default");
      expect(within(row).getByTestId("plan-targets")).toHaveTextContent("AcmeSupportAgent");
    });
    ...
    it("offers a target a run from code named in the Target filter, by its key", async () => {
      ...
      expect(overviewState.lastInput?.targetKeys).toEqual(["code:acmesupportagent"]);
    });
    ...
    it("names the agent a run reported on the run line behind its target row", async () => {
      ...
      renderList();
      await user.click(screen.getByRole("radio", { name: "Target" }));
      await user.click(await screen.findByTestId("results-group-row-code:acmesupportagent"));
    ...
    it("lists one row per target, each reading that target alone", async () => {
      ...
      expect(within(dev).getByTestId("pass-rate-text")).toHaveTextContent("25%");
      expect(within(prod).getByTestId("pass-rate-text")).toHaveTextContent("100%");
    });
    ...
    it("marks each target row with the kind of its agent, in no colour", async () => {
      ...
      const dev = await screen.findByTestId("results-group-target-mark-agent_dev");
    ...
    it("lists one row per scenario, target and run", async () => {
      ...
      expect(within(table).getByTestId("results-flat-row-exec_1")).toHaveTextContent(
        "Refund a paid order",
      );
      expect(within(table).getByTestId("results-flat-row-exec_2")).toHaveTextContent(
        "Track a parcel",
      );
      expect(within(table).getByTestId("results-flat-row-exec_2")).toHaveTextContent("prod-agent");
    });
  });
  ...
  describe("when a target ran with parameter overrides", () => {
    ...
    it("names each variant in the Targets cell of the plan", () => {
      ...
      expect(
        within(screen.getByTestId("run-plan-row-checkout")).getByTestId("plan-targets"),
