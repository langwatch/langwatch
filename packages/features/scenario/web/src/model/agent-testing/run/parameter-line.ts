...
export function formatParameterLine(definitions: readonly ScenarioParameterDefinition[]): string {
  return definitions
...
export function toParameterDefinitions({
  ...
  const known = new Map(existing.map((definition) => [definition.name, definition]));
