...
describe("plan-tier retention constants", () => {
  ...
  it("keeps the shorter paid preset below the enterprise custom floor", () => {
    ...
    expect(PAID_RETENTION_PRESET_DAYS[0]).toBeLessThan(ENTERPRISE_CUSTOM_MIN_RETENTION_DAYS);
  });
});
...
describe("platformDefaultRetentionDaysSchema", () => {
  ...
  describe("when the value is not a whole number of weeks", () => {
    it("rejects it, because retention is pruned by the weekly partition key", () => {
      const result = platformDefaultRetentionDaysSchema.safeParse(10);
      expect(result.success).toBe(false);
      expect(result.error?.issues[0]?.message).toContain("whole number of weeks");
    });
  });
