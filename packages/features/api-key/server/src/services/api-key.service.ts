...
function publicKey(row: StoredApiKey): ApiKey {
...
function isListedForMember(key: ApiKey, userId: string | null): boolean {
  return (
    !!userId &&
...
export class ApiKeyService extends ApiKeyCapability {
  ...
  async create(input: CreateApiKeyInput): Promise<{ token: string; apiKey: ApiKey }> {
    ...
    if (parsed.userId)
      await this.ensureCallerIsOrgMember({
        userId: parsed.userId,
        organizationId: parsed.organizationId,
      });
    for (const binding of bindings)
      await this.validateScope(binding, parsed.organizationId);
    await this.assertPersonalScopesOwnedBy({
      scopes: bindings,
      organizationId: parsed.organizationId,
    ...
    return {
      ...
      apiKey: publicKey(await this.repository.activate({ id: row.id })),
    };
  }
  ...
  async update(input: UpdateApiKeyInput): Promise<ApiKey> {
    ...
    const effectiveBindings = await this.writeBindings({
      apiKeyId: input.id,
      organizationId: input.organizationId,
      bindings: input.bindings,
      permissions,
  ...
  async list(input: { userId: string; organizationId: string }): Promise<ApiKey[]> {
    return (await this.repository.listForUser(input)).map(publicKey);
  }
  async listAll({ organizationId }: { organizationId: string }): Promise<ApiKey[]> {
    return (await this.repository.listForOrganization({ organizationId })).map(publicKey);
  }

  async revoke(input: RevokeApiKeyInput): Promise<ApiKey> {
    const existing = await this.getInOrganization(input.id, input.organizationId);
    if (SYSTEM_NAMES.has(existing.name)) throw new ApiKeyNotFoundError(input.id);
    if (
      !input.callerIsAdmin &&
      (existing.userId === null || existing.userId !== input.callerUserId)
    )
  ...
  async getByIdForCaller(input: {
    ...
    if (
      ...
      (!input.callerCanReadAnyKey && !isListedForMember(row, input.callerUserId))
    )
      throw new ApiKeyNotFoundError(input.id);
    return {
      ...publicKey(row),
  ...
  async listIngestionKeysForProject(input: {
    ...
    return (await this.repository.findIngestKeysForProject(input)).map(publicKey);
  }
  async validateCliSelection(input: {
    ...
    await this.assertSelectionWithinCeiling({
      userId: input.userId,
      organizationId: input.organizationId,
  ...
  async tryResolveDefaultCliSelection(input: {
    ...
    if (await this.isOrgAdmin(input))
      return {
  ...
  private async getInOrganization(
    ...
    const row = await this.repository.tryFindByIdInOrganization({ id, organizationId });
    if (!row) throw new ApiKeyNotFoundError(id);
    return row;
  }
  private async assertPersonalScopesOwnedBy(input: {
    scopes: ApiKeyScope[];
    organizationId: string;
  ...
  private async validateScope(
    ...
    if (!project || project.archivedAt || project.team.organizationId !== organizationId)
      throw new ApiKeyScopeViolationError(
  ...
  private async writeBindings(input: {
    apiKeyId: string;
    organizationId: string;
    bindings?: ApiKeyScope[];
    permissions?: string[];
  ...
  private async customRoles(
    ids: string[],
    organizationId: string,
  ): Promise<ApiKeyRoleSummary[]> {
