...
import { LangWatchQLVegaLiteChart } from "../../src/components/langwatch-ql-vega-lite-chart";
import { LangWatchQLVegaLoadBlockedError } from "../../src/visualization/no-network-vega-loader";
import type {
  LangWatchQLDataset,
...
} from "../../src/visualization/langwatch-vega-config";
...
const TOKENS: LangwatchVegaTokens = {
  ...
  gridColor: "#e5e7eb",
  domainColor: "#9ca3af",
  categoricalRange: ["#f97316", "#0ea5e9"],
};
const DARK_TOKENS: LangwatchVegaTokens = {
  ...
  gridColor: "#374151",
  domainColor: "#9ca3af",
  categoricalRange: ["#fb923c", "#38bdf8"],
};
const THEME_CONFIGS = {
  light: langwatchVegaConfig({ colorMode: "light", tokens: TOKENS }),
  dark: langwatchVegaConfig({ colorMode: "dark", tokens: DARK_TOKENS }),
} as const;
const PINNED_CONFIG = langwatchVegaPinnedConfig({ tokens: TOKENS });
...
vi.mock(
  "../../src/visualization/build-langwatch-ql-vega-spec",
  async (importOriginal) => {
    const original =
      await importOriginal<
        typeof import("../../src/visualization/build-langwatch-ql-vega-spec")
      >();
    return {
      ...original,
      buildLangWatchQLVegaSpec: (input: {
        ...
        pinnedConfig: Readonly<Record<string, unknown>>;
      }) => {
        if (build.throwWith !== null) throw build.throwWith;
        return original.buildLangWatchQLVegaSpec(input);
      },
    };
  },
);
...
const chartConfigSchema = z.object({
  axis: z.object({ labelColor: z.string() }),
  range: z.object({ category: z.array(z.string()) }),
  background: z.string(),
});

const chartConfig = (options: Record<string, unknown>) =>
  chartConfigSchema.parse(options.config);
...
describe("the LangWatchQL Vega-Lite chart", () => {
  describe("given a valid specification over the query result", () => {
    describe("when it renders", () => {
      ...
      it("embeds with no actions, an interpreter, and the repository's own loader", async () => {
        ...
        ).rejects.toBeInstanceOf(LangWatchQLVegaLoadBlockedError);
      });
  ...
  describe("given values a chart cannot place on an axis", () => {
    ...
    it("draws the chart and warns rather than turning them into zero", async () => {
      ...
      await waitFor(() => expect(vega.state.calls).toHaveLength(1));
      const warnings = await screen.findByTestId("lwql-chart-warnings");
  ...
  describe("given a chart that cannot be drawn", () => {
    ...
    it("refuses a result past the row ceiling, naming the limit it crossed", async () => {
      ...
      await screen.findByTestId("lwql-chart-failure");
      const panel = screen.getByTestId("lwql-chart-failure");
