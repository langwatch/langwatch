...
const MODEL_CATALOG_ROWS: LangWatchQLDataset = ["model-0", "model-1", "model-2", "model-3"].map(
  (model, index) => ({ model, vendor: `vendor-${index}` }),
);
...
function recordNetwork(): NetworkRecorder {
  ...
  XMLHttpRequest.prototype.open = function recordedOpen(this: XMLHttpRequest, ...args: unknown[]) {
