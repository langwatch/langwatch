...
import { createNoNetworkVegaLoader } from "../../src/visualization/no-network-vega-loader";
import {
  parseVegaLiteSpecText,
  validateVegaLiteSpec,
} from "../../src/visualization/validate-vega-lite-spec";
import { screenVegaExpression } from "../../src/visualization/vega-lite-expressions";
import { validateFieldReferences } from "../../src/visualization/vega-lite-fields";
import {
  ALLOWED_VEGA_LITE_TRANSFORMS,
  applyLangWatchQLVegaPolicy,
  LWQL_VEGA_LIMITS,
  LWQL_VEGA_RULES,
} from "../../src/visualization/vega-lite-policy";
import {
  getVegaLiteSchemaValidator,
  VEGA_LITE_SCHEMA_URL,
  validateAgainstVegaLiteSchema,
} from "../../src/visualization/vega-lite-schema";
import {
  collectViewNodes,
  measureSpecBytes,
} from "../../src/visualization/vega-lite-structure";
import {
  LWQL_VEGA_RULE_IDS,
...
const MODULE_DIR = fileURLToPath(new URL("../../src/visualization/", import.meta.url));
  ...
  describe("given the modules are imported outside a browser", () => {
    describe("when the policy runs", () => {
      ...
      it("evaluates no React, DOM, or browser-only Vega module", () => {
        ...
        const schemaSource =
          files.find(({ name }) => name === "vega-lite-schema.ts")?.source ?? "";
