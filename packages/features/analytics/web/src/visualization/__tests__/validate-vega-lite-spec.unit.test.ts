...
import { schemaInvalidEncodingType, unknownSchemaVersion } from "./fixtures/invalid/index";
import {
  LWQL_FIXTURE_COLUMNS,
  LWQL_FIXTURE_ROW_COUNTS,
} from "./fixtures/lwql-dataset-registry";
import { barOverQueryResult, lookupBetweenRegisteredDatasets } from "./fixtures/valid/index";
import {
  parseVegaLiteSpecText,
  validateVegaLiteSpec,
} from "../validate-vega-lite-spec";
  ...
  describe("given a value that is not a parsed specification object", () => {
    describe("when it is offered for validation", () => {
      ...
      it("refuses URLs, scalars, arrays, and unsupported schema versions without converting them", () => {
        ...
        expect(unknownSchemaVersion.$schema).toBe(
  ...
  describe("given several registered named datasets and their columns", () => {
    describe("when a spec reads more than one of them", () => {
      ...
      it("resolves each branch against the dataset that feeds it", () => {
        ...
        const crossedBranches = {
          ...layeredOverTwoDatasets,
          layer: layeredOverTwoDatasets.layer.map((branch, index) =>
            index === 1
              ? {
                  ...branch,
                  encoding: {
                    ...branch.encoding,
                    x: { ...branch.encoding.x, field: "latency" },
                  },
                }
              : branch,
          ),
