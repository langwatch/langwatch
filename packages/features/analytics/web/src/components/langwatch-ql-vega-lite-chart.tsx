...
import { useLangWatchQLChartModel } from "../hooks/use-langwatch-ql-chart-model";
import type { LangWatchQLVegaViewStatus } from "../hooks/use-langwatch-ql-vega-view";
import type {
...
export function LangWatchQLVegaLiteChart({
  ...
  const { containerRef, state, failures, warnings, isRefused } = useLangWatchQLChartModel(
    { spec, datasets, columnsByDataset, themeConfig, pinnedConfig, colorMode },
  );
...
function LangWatchQLChartWarnings({
  warnings,
}: {
