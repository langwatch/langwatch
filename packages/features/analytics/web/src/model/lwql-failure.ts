...
export function lwqlEditorMarkers(failure: LangWatchQLFailure): readonly LangWatchQLEditorMarker[] {
  return failure.violations
    .filter(
      (violation): violation is LangWatchQLViolationView & { at: LangWatchQLSourcePosition } =>
        violation.at !== void 0,
    )
    .map((violation) => ({
