...
export interface LangWatchQLResultTableProps {
...
export function LangWatchQLResultTable({ result }: LangWatchQLResultTableProps) {
  ...
  const data = useMemo(() => result.rows.map((row) => row), [result.rows]);
...
function columnDefs(columns: LangWatchQLQueryResult["columns"]): ColumnDef<LangWatchQLRow>[] {
  return columns.map((column, index) => ({
    ...
    header: column.name,
    cell: ({ row }) => (
      <LangWatchQLValueCell
        columnName={column.name}
        cell={readLangWatchQLCell({ row: row.original, column: column.name })}
      />
    ),
  }));
}
 ...
 */
function typesByColumnId(columns: LangWatchQLQueryResult["columns"]): Map<string, string> {
  return new Map(
...
function virtualRowWindow(virtualizer: Virtualizer<HTMLDivElement, Element>): VirtualRowWindow {
...
function ResultTableHeader({
  ...
  headers: readonly Header<LangWatchQLRow, unknown>[];
...
function ResultTableBody({
  ...
  rows: readonly Row<LangWatchQLRow>[];
  columnCount: number;
...
function DuplicateColumnWarning({ names }: { names: readonly string[] }) {
  return (
    <Alert.Root status="warning" data-testid="lwql-duplicate-columns">
      <Alert.Indicator />
      <Alert.Content>
