...
export interface LangWatchQLSchemaBrowserProps {
  ...
  error: unknown;
  /** Optional app-owned error chrome for the transport's handled error. */
...
function ColumnRow({
  ...
  column: LangWatchQLSchemaColumnModel;
  onInsert: (text: string) => void;
}) {
  ...
  return (
    <chakra.button
      ...
      data-testid={`lwql-schema-column-${column.name}`}
    >
...
function DatasetEntry({
  ...
  dataset: LangWatchQLSchemaDatasetModel;
  expanded: boolean;
  /**
  ...
  return (
    <Box width="full">
      <chakra.button
        ...
        _hover={canToggle ? { background: "bg.muted" } : void 0}
...
export function LangWatchQLSchemaBrowser({
  model,
  isLoading,
  error,
  renderError,
  ...
  const visible = useMemo(() => filterLangWatchQLSchemaModel({ model, search }), [model, search]);
  ...
  return (
    <VStack align="stretch" gap={2} width="full" padding={3} data-testid="lwql-schema-browser">
