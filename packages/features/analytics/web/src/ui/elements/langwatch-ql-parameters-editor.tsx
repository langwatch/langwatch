...
import { Box, Button, chakra, HStack, Input, Stack, Text, VStack } from "@chakra-ui/react";
...
function recordOf(rows: readonly ParameterRow[]): Record<string, LangWatchQLParameterValue> {
...
const SELECT_STYLE: Record<string, string | number> = {
  borderWidth: "1px",
  borderColor: "border",
  borderRadius: "6px",
  fontSize: "13px",
...
function parameterKind(value: string): ParameterKind {
  switch (value) {
    case "number":
    case "boolean":
    case "null":
...
function ParameterRowFields({
  ...
  rows: readonly ParameterRow[];
  onPatch: (changes: ParameterPatch) => void;
  onRemove: () => void;
}) {
    ...
    <Stack gap={1}>
      <HStack gap={2}>
        ...
        <chakra.select
          ...
          value={row.kind}
          {...SELECT_STYLE}
          onChange={(event) => onPatch({ kind: parameterKind(event.target.value) })}
        >
...
export function LangWatchQLParametersEditor({
  ...
  const isOpen = expanded || missingParameters.length > 0 || reservedParameters.length > 0;
  ...
  const update = useCallback(
    (next: readonly ParameterRow[]) => {
      ...
      onChange({
        ...
        sendable: next.every((row) => rowProblem({ row, rows: next }) === void 0),
      });
    },
    [onChange],
  );
  ...
  const patch = useCallback(
    (id: string, changes: ParameterPatch) => {
    ...
    <VStack align="stretch" gap={2} width="full" data-testid="lwql-parameters">
      ...
      {isOpen && (
        <Stack gap={2}>
          ...
          <Box>
            <Button size="xs" variant="outline" onClick={() => update([...rows, newRow()])}>
