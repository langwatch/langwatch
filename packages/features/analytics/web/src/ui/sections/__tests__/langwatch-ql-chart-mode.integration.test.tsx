...
vi.mock("@monaco-editor/react", () => ({
  default: (props: { value?: string; onChange?: (value: string | undefined) => void }) => (
    <textarea
...
const THEME_CONFIG = langwatchVegaConfig({ colorMode: "light", tokens: TOKENS });
  ...
  describe("given a successful LangWatchQL result", () => {
    ...
    describe("when the member edits the specification", () => {
      ...
      it("reports what is wrong as it is typed, and keeps reporting nothing when it is fixed", async () => {
        ...
        expect((await screen.findByTestId("vega-spec-editor-problems")).textContent).toContain(
          "not valid JSON",
        );
        ...
        await waitFor(() =>
          expect(screen.getByTestId("vega-spec-editor-problems").textContent).toContain("nowhere"),
        );
        ...
        await waitFor(() => expect(screen.queryByTestId("vega-spec-editor-problems")).toBeNull());
    ...
    describe("when the member reads the specification view's policy panel", () => {
      ...
      it("says where the specification stands and what the policy accepts", async () => {
        ...
        await waitFor(() =>
          expect(screen.getByTestId("vega-spec-policy-panel").textContent).toContain("Refused"),
        );
      });
    });
