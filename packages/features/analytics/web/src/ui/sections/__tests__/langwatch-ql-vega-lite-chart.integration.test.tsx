...
const chartConfig = (options: Record<string, unknown>) => chartConfigSchema.parse(options.config);
...
describe("the LangWatchQL Vega-Lite chart", () => {
  describe("given a valid specification over the query result", () => {
    describe("when it renders", () => {
      ...
      it("embeds with no actions, an interpreter, and the repository's own loader", async () => {
        ...
        await expect(
          (loader.load as (uri: string) => Promise<unknown>)("https://example.test/data.json"),
  ...
  describe("given values a chart cannot place on an axis", () => {
    ...
    it("draws the chart and warns rather than turning them into zero", async () => {
      ...
      expect(warnings.querySelector("[data-warning-code]")?.getAttribute("data-warning-code")).toBe(
  ...
  describe("given a chart that cannot be drawn", () => {
    ...
    it("names a failure raised while the specification is being built", async () => {
      ...
      await screen.findByTestId("lwql-chart-failure");
      expect(failureCode()).toBe("render-failure");
      expect(screen.getByTestId("lwql-chart-failure").textContent).toContain("Spec build gave out");
