  ...
  describe("given an authorized member with a live LangWatchQL schema", () => {
    ...
    describe("when a statement declares the granularity step", () => {
      ...
      it("shows the too-fine refusal when the chosen step overflows the budget", async () => {
        ...
        // THE REFUSAL'S OWN WORDS ARE `platform/app`'s REGISTRY'S and do not
        // travel; what the package can still prove is that the refusal REACHES
        // a reader as a named failure rather than being swallowed, and that the
        // code slug — which since #5984 is the wire message — is not what they
        // are shown.
        const alert = await screen.findByRole("alert");
        expect(alert).toHaveTextContent(/couldn't run/i);
        expect(alert).not.toHaveTextContent(/lwql_granularity_too_fine/);
      });
