...
export const LWQL_VEGA_RULES: readonly LangWatchQLVegaRule[] = LWQL_VEGA_RULE_IDS.map((id) => ({
  id,
  ...RULE_CATALOGUE[id],
}));
...
export function checkDatasetRowLimits(rowCountsByDataset: DatasetRowCounts): VegaValidationError[] {
  const errors: VegaValidationError[] = [];
  let total = 0;
 ...
 */
function refuseStringResourceProperties(objects: readonly JsonObjectNode[]): VegaValidationError[] {
  const errors: VegaValidationError[] = [];
    ...
    for (const key of EXPRESSION_BEARING_KEYS) {
      ...
      visitPredicate({
        ...
        visit: ({ value, path: valuePath }) => {
          if (typeof value === "string") found.push({ path: valuePath, expression: value });
        },
      });
    }
  }
...
function checkInteractiveParams(objects: readonly JsonObjectNode[]): VegaValidationError[] {
