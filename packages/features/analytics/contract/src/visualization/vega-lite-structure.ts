...
const COMPOSITION_ARRAY_KEYS: readonly string[] = ["layer", "hconcat", "vconcat", "concat"];

/** Keys whose presence makes `spec` a single child view template. */
const COMPOSITION_WRAPPER_KEYS: readonly string[] = ["facet", "repeat"];
...
export function joinPointer(parent: string, segment: string | number): string {
  ...
  return parent === JSON_POINTER_ROOT ? `${JSON_POINTER_ROOT}${escaped}` : `${parent}/${escaped}`;
}
...
export function repeatFieldsOf(repeat: unknown): Readonly<Record<string, readonly string[]>> {
...
export function countUnitViews(spec: unknown): number {
  ...
  if ("repeat" in spec) return total + repeatMultiplier(spec.repeat) * countUnitViews(spec.spec);
