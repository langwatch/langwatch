...
function within(column: string, day: string, days = 1): string {
  ...
  return (
    `${column} >= toDateTime64('${at(day)}', 3) ` + `AND ${column} < toDateTime64('${at(to)}', 3)`
  );
}
...
describe("given the /api/v1/query REST door and a seed with known answers", () => {
  ...
  beforeAll(async () => {
    ...
    await seedTenant({ admin: harness.admin, database: facts, tenantId: asking.id, factor: 1 });
    await seedTenant({
      admin: harness.admin,
      database: facts,
      tenantId: other.id,
    ...
    for (const [tenant, promptId] of [
      ...
      for (const statement of postgresTenantSeedStatements({
        tenantId: tenant.id,
        traceIds: annotatedTraceIds(tenant.id),
  ...
  describe("when a rolling window over trace metrics is asked for", () => {
    ...
    it("answers a one-hour rolling error rate over quarter-hour buckets", async () => {
      ...
      expect(result.rows.map((row: any) => row.rolling_error_rate)).toEqual([0, 0.1, 0.2, 0.3]);
      expect(result.rows.map((row: any) => Number(row.traces))).toEqual([5, 5, 5, 5]);
      expect(codes(result)).toEqual([]);
    });
  });
  ...
  describe("when cost is aggregated by model and prompt version", () => {
    ...
    it("answers the seeded spend for each model and prompt version of the asking project", async () => {
      ...
      expect(
        result.rows.map((row: any) => [row.model, Number(row.prompt_version), row.spend]),
      ).toEqual([
        [SECOND_MODEL, 1, 0.1],
        [PRIMARY_MODEL, 1, 0.2],
        [PRIMARY_MODEL, 2, 0.4],
      ...
      expect(new Set(result.rows.map((row: any) => row.project))).toEqual(new Set([asking.id]));
      expect(codes(result)).toEqual([]);
    });
  ...
  describe("when traces containing one operation before another are asked for", () => {
    ...
    it("returns only the trace whose retrieval preceded its generation", async () => {
      ...
      expect(result.rows.map((row: any) => row.TraceId)).toEqual([`${asking.id}-order-ab`]);
      expect(codes(result)).toEqual([]);
    });
  });
  ...
  describe("when the elapsed time between two events in a trace is asked for", () => {
    ...
    it("answers the seeded five seconds, and its negative for the reversed trace", async () => {
      ...
      expect(
        result.rows.map((row: any) => [row.TraceId, Number(row.retrieval_to_generation_seconds)]),
      ).toEqual([
  ...
  describe("when human annotations are compared against evaluator verdicts", () => {
    ...
    it("counts only the asking tenant's annotations", async () => {
      ...
      const result = await ask(`SELECT DISTINCT TenantId FROM ${database}.annotations`);
  ...
  describe("when a time-bucketed answer has an empty bucket inside its range", () => {
    ...
    it("says how many buckets are missing and where the gap is", async () => {
      ...
      expect(result.rows.map((row: any) => Number(row.traces))).toEqual([1, 1, 1]);
  ...
  describe("when the newest period of a comparison has not finished", () => {
    ...
    it("reports the comparison as unequal while that period is still filling", async () => {
      ...
      try {
        ...
        expect(diagnostic(result, "INCOMPLETE_COMPARISON_PERIOD").meta).toMatchObject({
