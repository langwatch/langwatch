...
describe("column-pruning", () => {
  ...
  describe("given a SELECT list is split into items", () => {
    it("keeps a reconstructed map as one item", () => {
      expect(selectListItems("TraceId, map('k', SpanAttributes['k']) AS SpanAttributes")).toEqual([
        "TraceId",
    ...
    it("still surfaces a whole-map projection in any position", () => {
      expect(selectListItems("SpanAttributes, TraceId, DurationMs")).toContain("SpanAttributes");
    });
  });
  ...
  describe("stored spans JOIN column pruning", () => {
    describe("when grouping by metadata.span_type", () => {
      ...
      it("projects a narrow SpanAttributes map instead of the whole column", () => {
        ...
        const storedSpansSelect = /SELECT\s+(?<list>[\s\S]*?)\s+FROM stored_spans/.exec(result.sql)
          ?.groups?.list;
    ...
    describe("when requesting sentiment.thumbs_up_down", () => {
      it("does not include wide span columns like Input and Output", () => {
        const result = buildTimeseriesQuery({
          ...
          series: [
            {
              metric: "sentiment.thumbs_up_down" as AnalyticsSeries["metric"],
              aggregation: "sum" as const,
            },
          ],
        });
