...
const codesOf = (diagnostics: readonly LangWatchQLDiagnostic[]) =>
  diagnostics.map((diagnostic) => diagnostic.code);

const find = (
  diagnostics: readonly LangWatchQLDiagnostic[],
  code: (typeof LWQL_DIAGNOSTIC_CODES)[number],
...
const BUCKET_COLUMNS: readonly LangWatchQLColumn[] = [
...
describe("given a LangWatchQL query that ran", () => {
  describe("when nothing about it is suspect", () => {
    ...
    it("documents the empty list as no known issue detected, never as proof", () => {
      ...
      for (const overclaim of ["guarantee", "correct", "verified", "proves"]) {
        expect(
          LWQL_CLEAN_DIAGNOSTICS_MEANING.toLowerCase(),
