...
export class LoggingAnalyticsTripwire extends AnalyticsTripwire {
  ...
  private constructor(
    private readonly enabled: (projectId: string) => Promise<boolean>,
  ) {
    super();
  }
  ...
  compare(input: {
    ...
    try {
      const divergences: Array<{ period: string; date: string; metric: string; routed: number | null; legacy: number | null }> = [];
      for (const period of ["current", "previous"] as const) {
        const routed = period === "current" ? input.routed.currentPeriod : input.routed.previousPeriod;
        const legacy = period === "current" ? input.legacy.currentPeriod : input.legacy.previousPeriod;
        ...
        for (const date of new Set([...routedByDate.keys(), ...legacyByDate.keys()])) {
          ...
          if (!routedBucket || !legacyBucket) {
            divergences.push({ period, date, metric: "*", routed: routedBucket ? 1 : null, legacy: legacyBucket ? 1 : null });
            continue;
          }
          ...
          for (const metric of new Set([...routedMetrics.keys(), ...legacyMetrics.keys()])) {
            ...
            if (routedValue === undefined || legacyValue === undefined) {
              divergences.push({ period, date, metric, routed: routedValue ?? null, legacy: legacyValue ?? null });
              continue;
            }
      ...
      if (divergences.length > 0) {
        logger.warn(
          { projectId: input.projectId, table: input.table, divergenceCount: divergences.length, divergences: divergences.slice(0, 10) },
          "ADR-034 tripwire: routed analytics result diverged from legacy",
        );
      }
    } catch (error) {
      logger.warn(
        { projectId: input.projectId, table: input.table, error: error instanceof Error ? error.message : String(error) },
