 ...
 * `platform/app/src/components/ui/ReplicateToProjectDialog.tsx`, narrowed to the
 * one entity it copies here. The platform component keeps its generic entity
 * label and its `extraContent` slot for the monitor and prompt callers it still
 * serves, and dies with them.
 *
 * WHAT DID NOT TRAVEL is the toast and the log line. Both were the application's
 * to make — a feature-web package may reach neither a toaster singleton nor a
 * logger — so the outcome is handed back to the caller, which is the screen, and
 * the screen tells the host. The refusal to select a project the reader cannot
 * create in is unchanged: the option renders greyed with "(no permission)" and
 * selecting it leaves the button disabled.
 */

export type AgentReplicateDialogProps = {
  open: boolean;
  agentName: string;
  projects: readonly AgentCopyTarget[];
  isLoading: boolean;
  onClose: () => void;
  onCopy: (targetProjectId: string) => Promise<void>;
};

export function AgentReplicateDialog({
  open,
  agentName,
  projects,
  isLoading,
        ...
        <Dialog.Body paddingBottom={6}>
          <VStack gap={4} align="start">
            <Text fontSize="sm" color="fg.muted">
              {`"${agentName}" will be copied into the project you choose.`}
            </Text>
            <Field.Root>
