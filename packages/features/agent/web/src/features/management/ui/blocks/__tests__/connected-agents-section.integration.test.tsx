...
describe("<ConnectedAgentsSection />", () => {
  ...
  describe("given the same name is registered in two environments", () => {
    ...
    it("draws one card for each and names its environment", () => {
      ...
      expect(screen.getByTestId("connected-agent-card-agent_1")).toBeInTheDocument();
      expect(screen.getByTestId("connected-agent-card-agent_2")).toBeInTheDocument();
    ...
    it("draws each environment in the colour of that environment", () => {
      ...
      const classOf = (environment: string) => screen.getByText(environment).className;
  ...
  describe("given three processes hold one agent", () => {
    ...
    it("reads Online in the header and names the instance count", () => {
      renderSection([agent({ instances: [instance(), instance(), instance()] })]);
      ...
      expect(screen.getByText("langwatch-python 1.2.3 · 3 instances")).toBeInTheDocument();
    });
  });
  ...
  describe("given one process holds the agent", () => {
    ...
    it("reads one instance in the singular", () => {
      ...
      expect(screen.getByTestId("connected-agent-status-online")).toHaveAttribute(
        "aria-label",
        "Online · 1 instance",
      );
    });
  ...
  describe("given no process holds the agent", () => {
    ...
    it("reads Offline in the header and when it was last seen", () => {
      ...
      renderSection([agent({ status: "offline", instances: [], lastSeenAt: twoHoursAgo })]);

      const presence = screen.getByTestId("connected-agent-status-offline");
      expect(presence).toHaveAttribute("aria-label", "Offline · last seen 2 hours ago");
  ...
  describe("given a development agent that belongs to a machine", () => {
    ...
    it("carries a chip with the machine name", () => {
      renderSection([agent({ environment: "development", hostLabel: "build-box" })]);
  ...
  describe("given the agent declares a parameter", () => {
    ...
    it("names the SDK and its version, and never the parameter", () => {
      renderSection([
        agent({
          parameters: [{ name: "model", type: "string", options: ["gpt-5", "gpt-5-mini"] }],
        }),
      ]);

      expect(screen.getByText("langwatch-python 1.2.3 · 1 instance")).toBeInTheDocument();
  ...
  describe("when the card is clicked", () => {
    ...
    it("opens the agent it stands for", async () => {
      ...
      expect(onOpen).toHaveBeenCalledWith(expect.objectContaining({ id: "agent_1" }));
    });
  });
