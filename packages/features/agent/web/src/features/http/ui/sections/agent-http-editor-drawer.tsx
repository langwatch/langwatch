...
const FIXED_VARIABLE_IDS = new Set(FIXED_VARIABLES.map((variable) => variable.identifier));
...
function getHttpConfig(agent: AgentWithFields | null | undefined): HttpAgentConfig | null {
  if (agent?.type !== "http") {
    return null;
  }
...
export function AgentHttpEditorDrawer(props: AgentHttpEditorDrawerProps) {
  ...
  const defaultScenarioMappings = props.defaultScenarioMappings ?? EMPTY_SCENARIO_MAPPINGS;
  ...
  const [scenarioMappings, setScenarioMappings] = useState<Record<string, AgentInputBinding>>({});
  ...
  const variables = useMemo(() => [...FIXED_VARIABLES, ...customVariables], [customVariables]);
  ...
  const isValid =
    name.trim().length > 0 && url.trim().length > 0 && hasScenarioInputMapping(scenarioMappings);
