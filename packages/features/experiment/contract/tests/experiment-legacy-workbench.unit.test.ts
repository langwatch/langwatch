...
import { isLegacyOnlineEvaluationWorkbenchState } from "../src/experiment-legacy-workbench";
...
describe("isLegacyOnlineEvaluationWorkbenchState", () => {
  it("identifies real-time wizard experiments as online evaluation backing data", () => {
    expect(isLegacyOnlineEvaluationWorkbenchState({ task: "real_time" })).toBe(true);
  });

  it("keeps offline experiment tasks in the experiments workflow", () => {
    expect(isLegacyOnlineEvaluationWorkbenchState({ task: "llm_app" })).toBe(false);
