...
import { connectedTargetFields } from "../connected-agent-target";
  ...
  describe("when the column is built", () => {
    ...
    it("edits each parameter as the type it declares", () => {
      const byName = new Map(
        connectedTargetFields(agent).inputs.map((input) => [input.identifier, input.type]),
      );
    ...
    it("leaves a secret out: a column mapping is not where a credential goes", () => {
      ...
      expect(inputs.some((input) => input.identifier === "api_token")).toBe(false);
    });

    it("writes the answer to one output", () => {
      expect(connectedTargetFields(agent).outputs).toEqual([{ identifier: "output", type: "str" }]);
    });
  });
