...
import { BatchEvaluationResultsTable } from "../../src/batch-results/batch-evaluation-results-table";
...
describe("BatchEvaluationResultsTable", () => {
  ...
  describe("Row Data", () => {
    ...
    it("resolves dataset images through the supplied renderer", () => {
      const data = createTestData({
        datasetColumns: [{ name: "image", hasImages: true }],
        rows: [
          {
            index: 0,
            datasetEntry: { image: "https://example.com/output.png" },
            targets: {
              "target-1": {
                targetId: "target-1",
                output: { response: "4" },
