...
} from "../../src/batch-results/leaderboard-trust-panel";
import { DEFAULT_WARN_THRESHOLD } from "../../src/batch-results/pairwise-leaderboard";
...
const leaderboardEntry = (variantId: string): BTLeaderboardEntry => ({
  variantId,
  wins: 30,
  losses: 30,
  matchups: 60,
...
describe("buildTrustChecks — reasons that must be the actual reason", () => {
  ...
  describe("given the judge shares a family with a variant that is not leading", () => {
    ...
    it("does not tell the reader to discount a lead it does not have", () => {
      ...
      const checks = build({
        leaderboard: leaderboard({
          entries: [leaderboardEntry("a"), leaderboardEntry("b")],
        }),
        judgeIndependence: independence({
          judgeModel: "openai/gpt-5",
          judgeFamily: "openai",
