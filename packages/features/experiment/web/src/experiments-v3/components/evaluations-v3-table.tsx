...
import { newTargetId } from "../actions/transforms/add-target";
import { DRAWER_WIDTH } from "../constants";
import { resolveTargetNameFromCache } from "../hooks/resolve-target-name";
import { useDatasetSync } from "../hooks/use-dataset-sync";
import { useEvaluationsV3Store } from "../hooks/use-evaluations-v3-store";
import { useExecuteEvaluation } from "../hooks/use-execute-evaluation";
import { useOpenEvaluatorEditor } from "../hooks/use-open-evaluator-editor";
import { scrollToTargetColumn, useOpenTargetEditor } from "../hooks/use-open-target-editor";
import { useDatasetSelectionLoader } from "../hooks/use-saved-dataset-loader";
import { useSyncWorkflowTargetFields } from "../hooks/use-sync-workflow-target-fields";
import type {
...
import { convertInlineToRowRecords } from "../utils/dataset-conversion";
import { isRowEmpty } from "../utils/empty-row-detection";
import { createEvaluatorEditorCallbacks } from "../utils/evaluator-editor-callbacks";
import { isCellInExecution } from "../utils/execution-scope";
import { convertFromUIMapping } from "../utils/field-mapping-converters";
import {
  buildInputsFromBodyTemplate,
  convertHttpComponentConfig,
} from "../utils/http-agent-utils";
import { evaluatorHasMissingMappings } from "../utils/mapping-validation";
import { toComparisonConfig } from "../utils/normalize-comparison";
import { createPromptEditorCallbacks } from "../utils/prompt-editor-callbacks";
import {
  type PromptOutputField,
  toTargetOutputFields,
} from "../utils/target-output-fields";
...
import { TargetSuperHeader } from "./target-super-header";
  ...
  Object.fromEntries([
    ...evaluators.map(
      (evaluator) =>
        [
          evaluator.id,
          results.evaluatorResults[target.id]?.[evaluator.id]?.[rowIndex] ?? null,
        ] as [string, unknown],
    ),
    ...(toComparisonConfig(target)
      ? [
          [
            target.id,
            results.evaluatorResults[target.id]?.[target.id]?.[rowIndex] ?? null,
          ] as [string, unknown],
        ]
      : []),
  ]);
  ...
  const createResizeHandler = useCallback(
    (columnId: string, columnType: string) => {
      return (event: React.MouseEvent | React.TouchEvent) => {
        ...
        const currentPct =
          columnSizing[columnId] ?? getDefaultPctForColumn(columnId, columnType);
      ...
      <AddOrEditDatasetDrawer
        datasetToSave={
          activeDataset
            ? {
                datasetId:
                  activeDataset.type === "saved" ? activeDataset.datasetId : undefined,
