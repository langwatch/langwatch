...
vi.mock("@langwatch/workflow-web/components/Markdown", () => ({
...
describe("ComparisonCell", () => {
  ...
  describe("given the comparison is still running", () => {
    it("shows a comparing indicator", () => {
      wrap(<ComparisonCell result={null} isLoading variantTargets={variantTargets} />);
  ...
  describe("given the judge call errored", () => {
    ...
    describe("given the judge call failed auth (403)", () => {
      ...
      it("uses the domain error to show a rich auth message", () => {
        ...
        expect(screen.getByText(/missing or invalid model api key/i)).toBeInTheDocument();
        ...
        expect(
          within(screen.getByTestId("comparison-error-details")).getByText(/^403/),
        ).toBeInTheDocument();
      });
    });
    ...
    describe("when the same auth failure arrives structurally and as raw text", () => {
      const readAuthCopy = (result: unknown) => {
        ...
        const copy = {
          headline: screen.getByText(MISSING_MODEL_API_KEY_EXPLANATION.headline).textContent,
          hint: screen.getByText(MISSING_MODEL_API_KEY_EXPLANATION.hint).textContent,
        };
        unmount();
        return copy;
      };
  ...
  describe("given a row-level run callback", () => {
    it("shows a per-row run action and calls it without selecting the cell", async () => {
      ...
      const onRun = vi.fn();
      wrap(<ComparisonCell result={null} variantTargets={variantTargets} onRun={onRun} />);

      await user.click(screen.getByRole("button", { name: /run comparison for this row/i }));
  ...
  describe("given a winning variant", () => {
    ...
    it("highlights the winner's column and scrolls to it when clicked", async () => {
      const user = userEvent.setup();
      wrap(
        <ComparisonCell result={{ label: "friendly-support" }} variantTargets={variantTargets} />,
      );

      await user.click(screen.getByTestId("comparison-winner"));

      expect(useEvaluationsV3Store.getState().ui.highlightedVariantTargetId).toBe("target-b");
  ...
  describe("given a legacy slot-letter verdict", () => {
    it("resolves the letter to the variant at that position", () => {
      wrap(<ComparisonCell result={{ label: "A" }} variantTargets={variantTargets} />);
      const winners = screen.getAllByTestId("comparison-winner");
      expect(winners).toHaveLength(1);
