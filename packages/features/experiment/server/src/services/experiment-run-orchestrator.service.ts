...
export class ExperimentRunOrchestratorService {
  ...
  static async *runOrchestrator(input: OrchestratorInput): AsyncGenerator<EvaluationV3Event> {
    ...
    const targetMetadata: ESBatchEvaluationTarget[] =
      ExperimentRunOrchestratorService.buildTargetMetadata({
    ...
    const processingPromise = (async () => {
      try {
          ...
          const cellPromise = (async () => {
            try {
              ...
              const cellEvents = connectedAgent
                ? ExperimentRunOrchestratorService.executeConnectedCell({
                    cell,
                    projectId,
                ...
                : runsAsWorkflow
                  ? ExperimentRunOrchestratorService.executeWorkflowCell({
                      cell,
                      projectId,
                  ...
                  : ExperimentRunOrchestratorService.executeCell(
                      cell,
                      projectId,
                      ports,
                      datasetColumns,
        ...
        if (!aborted && scopeCanProduceVariantOutputs) {
          const { cells: phase2Cells, skipReasons } =
            ExperimentRunOrchestratorService.generateComparisonCells({
              state,
              datasetRows,
              ...
              scopedRowIndices: ExperimentRunOrchestratorService.resolveScopedRowIndices({
                scope,
          ...
          if (phase2Cells.length > 0 && seedTargetOutputs) {
            const rowsThisRunOwns = new Set(
              ExperimentRunOrchestratorService.resolveScopedRowIndices({
                scope,
                rowCount: datasetRows.length,
              }),
            );
