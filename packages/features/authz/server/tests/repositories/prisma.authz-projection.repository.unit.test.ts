...
import { MIGRATION_OWNED_SOURCES } from "../../src/migrations/legacy-import.authz-grant.migration";
import type { GrantProjectionWrite } from "../../src/projections/authz-grant.projection";
import { PrismaAuthzProjectionRepository } from "../../src/repositories/prisma/prisma.authz-projection.repository";
...
function build() {
  ...
  return {
    prisma: mocks,
    executeRaw,
    repository: PrismaAuthzProjectionRepository.create(prisma),
  };
}
...
describe("PrismaAuthzProjectionRepository", () => {
  ...
  describe("given a migration-sourced grant fact", () => {
    ...
    // them.
    it.each(MIGRATION_OWNED_SOURCES)(
      "updates the compat binding in place and never creates one (%s)",
      async (source) => {
        const { repository, prisma } = build();
