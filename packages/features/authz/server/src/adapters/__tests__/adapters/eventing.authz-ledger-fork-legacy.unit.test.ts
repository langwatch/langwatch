...
describe("given an organization the genesis import has not reached", () => {
  describe("when a binding is attached rejecting duplicates", () => {
    ...
    it("writes the row itself and emits no command", async () => {
      const { writer, db, sent, epoch } = harness({ onLedger: false });
      ...
      expect(epoch.bump).toHaveBeenCalledWith({
        organizationId: ORG_ID,
      });
    });
  ...
  describe("when a RESOURCE grant is revoked", () => {
    /**
     * ShareRepository already sweeps the legacy ShareLink row. This adapter
     * makes an uncached cutover read so a rolled-back organization does not
     * append a new resource fact after returning to the legacy writer.
     */
    it("leaves the ledger untouched when the uncached gate answers legacy", async () => {
      const { writer, db, sent } = harness({ onLedger: false });
      ...
      expect(sent).toEqual([]);
      expect(db.grant.updateMany).not.toHaveBeenCalled();
      expect(db.auditLog.createMany).not.toHaveBeenCalled();
    });
  });
  ...
  describe("when the audit insert fails", () => {
    it("leaves the grant write successful, as the separate audit write always was", async () => {
      const { writer, db, epoch } = harness({ onLedger: false });
      ...
      expect(epoch.bump).toHaveBeenCalled();
    });
  });
