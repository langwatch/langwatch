 ...
 *
 * The same shape `@langwatch/gateway-web`, `@langwatch/agent-web` and
 * `@langwatch/model-provider-web` state for themselves, for the same reasons.
 ...
 * setup, so assertions travel unchanged rather than being rewritten into
 ...
 *
 * This file arrived with the RBAC screens. The two scope-picker suites that
 * predate them ran without it, so it adds capability and removes none.
 */
