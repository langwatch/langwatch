...
import type { PythonField, PythonProviderHandle } from "./python-provider.shared";
import { registerPythonProviders } from "./python-providers";
...
export interface WorkflowCodeEditorContractProps {
...
export type WorkflowCodeEditorModalHost = (input: {
  open: boolean;
  onRequestClose: () => void;
  children: ReactNode;
}) => ReactNode;

export function WorkflowCodeEditorModal({
  code,
  setCode,
  open,
  onClose,
  ...
  return (
    <Modal open={open} onRequestClose={onClose_}>
      <>
        ...
        >
          ...
          <Box flex={1} />
          {renderSecretControls?.({ onInsertSecret: handleInsertSecret })}
          <HStack
            as="button"
            onClick={onClose_}
...
export function WorkflowCodeEditor({
  ...
  return (
    <Suspense fallback={<div style={{ padding: "0 16px" }}>Loading editor...</div>}>
      <MonacoEditor
        height="100%"
