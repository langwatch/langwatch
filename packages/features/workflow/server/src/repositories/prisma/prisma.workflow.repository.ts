...
const mapVersion = (row: unknown, includeDsl = true): WorkflowVersion => {
  ...
  return workflowVersionSchema.parse({
    ...value,
    dsl: includeDsl
      ? workflowDslSchema.parse(value.dsl)
      : { name: "", version: value.version, nodes: [], edges: [] },
  });
};
...
export class PrismaWorkflowRepository extends WorkflowRepository {
  ...
  async findVersionHistory(input: {
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        ...(input.includeDsl ? { dsl: workflowDslSchema.parse(value.dsl) } : {}),
        parent: value.parent,
        author: value.author,
      };
    });
  ...
  async tryFindVersion(input: {
    ...
    const row = await this.database.workflowVersion.findFirst({
      where: { id: input.id, workflowId: input.workflowId, projectId: input.projectId },
    });
    return row ? mapVersion(row) : null;
  }
  ...
  async createWorkflow(input: PersistWorkflowInput): Promise<WorkflowWithVersion> {
    return {
      ...mapWorkflow(await this.database.workflow.create({ data: input })),
      currentVersion: null,
      latestVersion: null,
    };
  }
  ...
  async createVersion(input: PersistWorkflowVersionInput): Promise<WorkflowVersion> {
    return mapVersion(
      await this.database.workflowVersion.create({ data: { ...input, dsl: input.dsl } }),
    );
  }
  ...
  async updateAutoSavedVersion(
    ...
    return mapVersion(
      await this.database.workflowVersion.update({
        where: { id: input.id, projectId: input.projectId },
