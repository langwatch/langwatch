...
export class WorkflowNlpExecutionService {
  ...
  async execute(input: WorkflowExecutionInput): Promise<WorkflowExecutionResponse> {
    ...
    const event: StudioClientEvent = {
      ...
      payload: {
        trace_id: traceId,
        workflow: WorkflowNlpExecutionService.workflowPayload(workflow),
        inputs: [input.inputs],
        manual_execution_mode: false,
        do_not_trace: input.doNotTrace ?? controls.do_not_trace ?? false,
        ...(input.runEvaluations === undefined ? {} : { run_evaluations: input.runEvaluations }),
        origin,
      },
    };

    try {
      WorkflowNlpExecutionService.stripUnsupportedParams(providers, event.payload.workflow);
    } catch (error) {
      logger.warn(
  ...
  private static workflowPayload(state: StudioWorkflow) {
    return {
  ...
  private static customModelsForProvider(
  ...
  private static filterLlmConfig(providers: Record<string, ModelProviderSummary>, value: unknown) {
    ...
    const customModels = provider
      ? WorkflowNlpExecutionService.customModelsForProvider(providers, provider)
      : null;
  ...
  static stripUnsupportedParams(
    ...
    for (const node of workflow.nodes) {
      if (!WorkflowNlpExecutionService.isSignatureNode(node)) {
        continue;
      }

      const rootLlm = WorkflowNlpExecutionService.filterLlmConfig(providers, node.data.llm);
      if (rootLlm) {
      ...
      for (const parameter of node.data.parameters ?? []) {
        ...
        const value = WorkflowNlpExecutionService.filterLlmConfig(providers, parameter.value);
        if (value) {
  ...
  private static assertRequiredInputs(
    workflow: StudioWorkflow,
  ...
  private static assertRequiredModelKeys(
    workflow: StudioWorkflow,
    providers: Record<string, { provider: string; customKeys: Record<string, unknown> | null }>,
  ): void {
