...
const pinnedAttributeSchema = z.object({
  source: z.enum(["attribute", "resource"]),
  key: z.string(),
...
function readFromStorage(projectId: string): PinnedAttribute[] {
  ...
  try {
    ...
    return parsed.data.pins
      .map((pin) => pinnedAttributeSchema.safeParse(pin))
...
function writeToStorage(projectId: string, pins: PinnedAttribute[]): void {
  ...
  try {
    const payload = { version: STORAGE_VERSION, pins };
