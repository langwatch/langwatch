...
const storedShapeSchema = z.object({
  version: z.literal(1),
  explicitlyShown: z.unknown().optional(),
  explicitlyHidden: z.unknown().optional(),
});
...
function readFromStorage(projectId: string): {
  ...
  try {
    ...
    return {
      explicitlyShown: stringValues(parsed.data.explicitlyShown),
      explicitlyHidden: stringValues(parsed.data.explicitlyHidden),
    };
  } catch {
    return fallback;
  }
...
function stringValues(value: unknown): string[] {
  ...
  return values.data.flatMap((item) => {
    const stringValue = z.string().safeParse(item);
    return stringValue.success ? [stringValue.data] : [];
  });
}
