...
/**
 * Any overlay the application registers. Untyped here on purpose: the registry
...
/**
 * Reconciles the module-level stack when navigating from `openNow` to `drawer`: seeds
 * it from the address bar if empty, snapshots the leaving entry's current params, dedupes
 * `drawer` if it's already in the stack, then pushes it.
 */
function reconcileDrawerStackOnNavigate(router: Router, openNow: DrawerType): void {
  // An empty stack means the open overlay came from a deep link or outlived a
  // reload, so seed it from the address the browser is actually on and back
...
/** Opens `drawer`, updating the module-level navigation stack and the address bar. */
function performOpenDrawer({
  router,
  writeDrawer,
  drawer,
  props,
  ...
  if (options.resetStack || !openNow) {
    ...
    reconcileDrawerStackOnNavigate(router, openNow);
...
/** Clears the navigation stack and every `drawer.*` (and `span`) address param. */
function closeDrawerAddress(router: Router): void {
  drawerStack = [];
  const { path, query } = readAddress(router.asPath);
  for (const key of [...query.keys()]) {
...
export function useDrawer() {
  ...
  const writeDrawer = useCallback<WriteDrawer>(
    (drawer, params, options = {}) => writeDrawerAddress(router, drawer, params, options),
    [router],
  );
  ...
  const openDrawer = useCallback(
    (
      ...
      performOpenDrawer({ router, writeDrawer, drawer, props, options });
    },
    [router, writeDrawer],
  );

  const closeDrawer = useCallback(() => closeDrawerAddress(router), [router]);
