...
describe("DatasetSelector", () => {
  describe("given the project's datasets have not finished loading", () => {
    describe("when the picker renders", () => {
      ...
      it("announces that it is loading instead of showing an empty dropdown", () => {
        ...
        expect(screen.getByRole("status", { name: "Loading datasets" })).toBeInTheDocument();
  ...
  describe("given the project has no datasets", () => {
    ...
    /**
     * The editor the button leads to runs on a host a feature package may not
     * mount, so the application hands it in. A composition that hands in none
     * offers no button rather than a dead one.
     */
    describe("when the application composed no dataset editor", () => {
      it("offers no way to create one", () => {
        renderSelector({ datasets: [], isLoading: false, onCreateNew: void 0 });
