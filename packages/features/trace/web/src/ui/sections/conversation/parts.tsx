...
const textAlignForRole = ({
  ...
  role?: string;
  roleMode?: ConversationRoleMode;
}): "left" | "right" => (alignForRole({ role, roleMode }) === "flex-start" ? "left" : "right");
...
export function TextPart({
  ...
  const structured =
    shouldRenderStructuredOutput && part.role === "assistant"
      ? findStructuredOutput(part.content)
      : undefined;
...
export function MediaRow({
  ...
  return (
    <VStack align={align} data-align={align} width="100%">
      ...
      >
        {renderMediaPart({ part: part.part, projectId, audioPlayback })}
        {part.transcript && (
          <Text
            fontSize="xs"
            color="fg.muted"
...
export function ErrorPart({ part }: { part: Extract<DisplayPart, { kind: "error" }> }) {
  return (
    <Box width="100%">
