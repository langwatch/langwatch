...
function parseLiteralTag(
  ...
  if (tag.expression.type !== "LiteralExpression") {
    throw new Error(`Expected a literal Tag for ${query}`);
  }

  return { ...tag, expression: tag.expression };
}

function parseRangeTag(query: string): TagToken & { expression: RangeExpressionToken } {
  const tag = parseTag(query);
  expect(tag.expression.type).toBe("RangeExpression");

  if (tag.expression.type !== "RangeExpression") {
    throw new Error(`Expected a range Tag for ${query}`);
  }

  return { ...tag, expression: tag.expression };
}

function firstGroup(analysis: OrGroupAnalysis): OrGroup {
  expect(analysis.groups).not.toHaveLength(0);
  const group = analysis.groups.at(0);

  if (!group) {
    throw new Error("Expected an OR group");
  }
...
describe("parse operator matrix", () => {
  describe("comparison operators", () => {
    it.each([
      ...
      const tag = parseLiteralTag(query);
      expect(tag.operator.operator).toBe(op);
      expect(tag.expression.value).toBe(value);
    });

    it("parses `>=` with a decimal value", () => {
      const tag = parseLiteralTag("cost:>=0.05");
      expect(tag.expression.value).toBe(0.05);
    });
  ...
  describe("range form", () => {
    ...
    it("parses negative range bounds", () => {
      const tag = parseRangeTag("evaluatorScore:[-1 TO 1]");
      expect(tag.expression.range.min).toBe(-1);
      expect(tag.expression.range.max).toBe(1);
    });
  });
  ...
  describe("quoted values", () => {
    ...
    it("normalises single-quoted values to the same Literal value", () => {
      const single = parseLiteralTag("model:'gpt-4 turbo'");
      const dbl = parseLiteralTag('model:"gpt-4 turbo"');
      expect(single.expression.value).toBe(dbl.expression.value);
    });

    it("parses a bare quoted string as ImplicitField free text", () => {
      const tag = parseLiteralTag('"refund policy"');
      expect(tag.field.type).toBe("ImplicitField");
      expect(tag.expression.value).toBe("refund policy");
    });
  });
  ...
  describe("wildcards", () => {
    it.each([
      ...
      const tag = parseLiteralTag(query);
      expect(tag.expression.value).toBe(expected);
    });
  });
  ...
  describe("special characters in unquoted values", () => {
    ...
    it("accepts `.` and `_` in field names (e.g. attribute paths)", () => {
      const tag = parseLiteralTag("attribute.langwatch.user_id:abc");
      expect(tag.field.type).toBe("Field");

      if (tag.field.type !== "Field") {
        throw new Error("Expected an explicit field");
      }

      expect(tag.field.name).toBe("attribute.langwatch.user_id");
    });
  });
...
describe("parse silent miscarriages — non-obvious shapes the validator must catch", () => {
  ...
  describe("given `NOT-status:error` (no space after NOT)", () => {
    it("parses as a literal Tag with field name `NOT-status` — not a negation", () => {
      ...
      const tag = parseLiteralTag("NOT-status:error");
      expect(tag.field.type).toBe("Field");
      ...
      expect(tag.field.name).toBe("NOT-status");
    });
  });
...
describe("analyzeOrGroups", () => {
    ...
    it("marks that member as negated, peers as not-negated", () => {
      ...
      expect(members).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ field: "status", negated: false }),
          expect.objectContaining({ field: "model", negated: true }),
        ]),
      );
    });
  ...
  describe("given a multi-arm OR", () => {
    ...
    it("flattens parenthesised OR shapes as one group", () => {
      ...
      expect(firstGroup(a).members).toHaveLength(3);
      expect(firstGroup(b).members).toHaveLength(3);
    });
  });
  ...
  describe("given multiple disjoint OR groups", () => {
    ...
    it("registers a field that appears in multiple groups under fieldToGroupIds", () => {
      ...
      if (!statusGroups) {
        throw new Error("Expected status group ids");
      }
