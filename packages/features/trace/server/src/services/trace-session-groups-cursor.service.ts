/**
 * The Sessions lens page cursor: what it carries, how it travels, how a page's last row turns into the next. Its own module because it is the one part of the lens read that is a wire contract rather than a query: clients hold these strings across requests, so every rule about what a cursor may claim lives here.
 */
...
/**
 * Every sort dimension a cursor may name, keyed by the union so the compiler refuses a list that has drifted in EITHER direction. A new `SessionGroupSortColumn` missing from here would make the cursor schema reject a cursor this very service minted, and the caller would lose the page mid-walk.
 */
...
/**
 * The decoded cursor. It carries the sort it was minted under because `sortValue` is meaningless without it: the repository recomputes the keyset boundary from the CURRENT sort, so a cursor from a cost sort compared against a timestamp aggregate would silently page through nonsense.
 */
