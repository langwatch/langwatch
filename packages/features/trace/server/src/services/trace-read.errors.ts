...
/**
 * This process refused the statement itself: its concurrency slots were all taken and its wait queue was full, so the statement never reached ClickHouse. Distinct from {@link ClickHouseUnavailableError} on purpose, even though both are a 503 — "the database is down" and "we declined to add to a load we are already struggling with" call for different responses, and only one is fixed by looking at ClickHouse. Count lives in `clickhouse_statements_shed_total`.
 */
