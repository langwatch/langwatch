...
import { asNumber } from "./canonical-guard.service";
import {
  AI_SDK_SPAN_TYPE_MAP,
  canonicaliseVercelToolCall,
} from "./vercel-tool-call.service";
...
function canonicaliseUsage(ctx: ExtractorContext): void {
  ...
  if (canonicalInput !== null) {
    ...
    if (cacheWrite !== null && cacheWrite > 0) {
      ...
      ctx.recordRule(
        `${VERCEL_RULE_PREFIX}:ai.usage.cacheWrite->gen_ai.usage.cache_creation`,
      );
    }
