...
export class MastraValues {
   ...
   */
  static extractTextFromOutput(output: unknown): string | null {
   ...
   */
  static extractBodyFromModelStepInput(input: unknown): Record<string, unknown> | null {
   ...
   */
  static extractModelFromMetadata(attrs: { get: (key: string) => unknown }): string | null {
   ...
   */
  static normalizeContentToString(content: unknown): string | null {
   ...
   */
  static extractSystemPromptFromBody(body: Record<string, unknown> | null): string | null {
    ...
    for (const rawMessage of body.messages) {
      ...
      const text = MastraValues.normalizeContentToString(message.data.content);
      if (text) {
        return text;
      }
    }
