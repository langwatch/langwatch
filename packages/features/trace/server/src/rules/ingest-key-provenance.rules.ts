...
export function originForIngestSourceType(sourceType: string): string {
  return CODING_AGENT_SOURCE_TYPES[sourceType] === true
 ...
 */
export const COPILOT_VSCODE_ALLOWED_SCOPES: Readonly<Record<string, true>> = {
  "github.copilot": true,
  "@github/copilot": true,
};
...
function scopeAllowedForVscode(scope: OtlpScope | undefined): boolean {
  return COPILOT_VSCODE_ALLOWED_SCOPES[scope?.name ?? ""] === true;
}
