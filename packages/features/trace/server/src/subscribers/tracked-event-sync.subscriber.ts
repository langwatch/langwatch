...
export class TrackedEventSync {
   ...
   */
  private static readStringValue(value: OtlpAnyValue | undefined): string | undefined {
   ...
   * skipped.
   */
  private static collectMetricAttribute({
    metrics,
    key,
    value,
  }: {
    ...
    const num = TrackedEventSync.readMetricValue(value);
   ...
   */
  private static collectDetailAttribute({
    eventDetails,
    key,
    value,
  }: {
    ...
    const detailValue = TrackedEventSync.readStringValue(value);
   ...
   */
  private static reconstructTrackedEvent({
    event,
    occurrenceIndex,
  }: {
    event: OtlpSpanEvent;
    ...
    for (const attr of event.attributes) {
      ...
      if (attr.key === EVENT_TYPE_KEY) {
        eventType = TrackedEventSync.readStringValue(value) ?? eventType;
        continue;
      }
      if (attr.key.startsWith(METRICS_PREFIX)) {
        TrackedEventSync.collectMetricAttribute({ metrics, key: attr.key, value });
        continue;
      }
      if (attr.key.startsWith(DETAILS_PREFIX)) {
        TrackedEventSync.collectDetailAttribute({ eventDetails, key: attr.key, value });
      }
    }

    if (!TrackedEventSync.isRecordableEventType(eventType)) return undefined;
      ...
      (event) =>
        ...
        event.attributes.some(
          (attr) =>
            ...
            TrackedEventSync.isRecordableEventType(attr.value.stringValue),
        ),
    );
  }
  ...
  private static async recordReconstructedEvent({
    ...
    if (!TrackedEventSync.isValidTrackedEvent({ event: trackedEvent, traceId })) {
      logger.warn(
    ...
    const eventId = TrackedEventSync.deterministicEventId({
      traceId,
      spanId,
    ...
    try {
      ...
      const failure = TrackedEventSync.toError(error);
      logger.error(
        {
          tenantId,
          traceId,
  ...
  private static async syncTrackedEventsFromSpan({
    ...
    if (!TrackedEventSync.hasSyncableFeedback(event)) return;

    const { tenantId, aggregateId: traceId } = context;
    const spanId = event.data.span.spanId;

    const trackedEvents = TrackedEventSync.extractTrackedEventsFromSpan(event.data.span);
    if (trackedEvents.length === 0) return;

    const errors: Error[] = [];

    for (const trackedEvent of trackedEvents) {
      const failure = await TrackedEventSync.recordReconstructedEvent({
        deps,
        trackedEvent,
        tenantId,
        traceId,
  ...
  static trackedEventSyncDedupId(event: TraceProcessingEvent): string {
  ...
  static extractTrackedEventsFromSpan(span: OtlpSpan): ReconstructedTrackedEvent[] {
    ...
    for (const [occurrenceIndex, event] of (span.events ?? []).entries()) {
      ...
      const reconstructed = TrackedEventSync.reconstructTrackedEvent({ event, occurrenceIndex });
  ...
  static createTrackedEventSyncHandler(
    ...
    return (event, context) => TrackedEventSync.syncTrackedEventsFromSpan({ event, context, deps });
  }
}
