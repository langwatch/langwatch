...
import { ClaudeCodeResponse } from "../services/claude-code-response.service";
...
export class ClaudeCodeCanonicaliser implements CanonicalAttributesPort {
  readonly id = "claude-code";

  apply(ctx: ExtractorContext): void {
    // Gateway-proxied claude_code traffic already arrives as gen_ai.* spans
    // (GenAICanonicaliser's job) — only the CLI's own native span needs lifting.
  ...
  private liftApiResponseBodyUsage(ctx: LogExtractorContext): void {
    const usage = ClaudeCodeResponse.tryExtractCacheCreationTtlSplit(ctx.bag.attrs.get("body"));
    if (usage === null) {
      return;
    }
    let fired = false;
