...
import { ClaudeCodeCanonicaliser } from "../claude-code.canonicaliser.adapter";
import { isConversationalQuerySource } from "../../rules/claude-code-call-policy.rules";
import { ClaudeCodeRequest } from "../../services/claude-code-request.service";
...
describe("tryExtractAssistantTextFromResponseBody (exported helper)", () => {
  it("lifts concatenated assistant text from content[]", () => {
    ...
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(body)).toBe(
  ...
  it("returns null for non-string / empty / malformed / non-array content", () => {
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(void 0)).toBeNull();
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(null)).toBeNull();
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(42)).toBeNull();
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody("")).toBeNull();
    expect(
      ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody("{not valid json"),
    ).toBeNull();
    expect(
      ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(JSON.stringify({})),
    ).toBeNull();
    expect(
      ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(
...
describe("tryBuildInputMessagesFromRequestBody (exported helper)", () => {
  it("parses system + every turn into a role/content conversation", () => {
    ...
    expect(ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(body)).toEqual([
  ...
  it("returns null for truncated / malformed / message-less bodies", () => {
    expect(ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(void 0)).toBeNull();
    expect(ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody("")).toBeNull();
    // claude truncates large request bodies inline -> invalid JSON tail.
    expect(
      ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody('{"model":"x","messages":[{"role":"u'),
    ).toBeNull();
    expect(ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(JSON.stringify({}))).toBeNull();
    // messages present but every turn flattens to empty -> null.
    expect(
      ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(
...
describe("tryExtractAssistantOutputFromResponseBody", () => {
  it("renders a tool_use reply as the output so a tool-deciding call is not empty", () => {
    ...
    const out = ClaudeCodeResponse.tryExtractAssistantOutputFromResponseBody(body) ?? "";
  ...
  it("matches the text-only extractor for a plain text reply", () => {
    ...
    expect(ClaudeCodeResponse.tryExtractAssistantOutputFromResponseBody(body)).toBe("PONG-Z");
    expect(ClaudeCodeResponse.tryExtractAssistantTextFromResponseBody(body)).toBe("PONG-Z");
  });

  it("returns null for an empty or unparseable body", () => {
    expect(ClaudeCodeResponse.tryExtractAssistantOutputFromResponseBody("")).toBeNull();
    expect(ClaudeCodeResponse.tryExtractAssistantOutputFromResponseBody("{not json")).toBeNull();
    expect(ClaudeCodeResponse.tryExtractAssistantOutputFromResponseBody(null)).toBeNull();
  });
});
...
describe("tryExtractSessionTitleFromResponseBody", () => {
  ...
  describe("given the title generator's own reply", () => {
    ...
    it("reads the title out of the JSON text block", () => {
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(
    ...
    it("caps a title long enough to be something other than a title", () => {
      const title = ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(
  ...
  describe("given a body that is not a title", () => {
    ...
    it("answers null for every deviation instead of guessing", () => {
      ...
      expect(ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody("{not json")).toBeNull();
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(
      ...
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(titleBody("Done, pushed.")),
      ).toBeNull();
      // Right shape, wrong type or empty.
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(titleBody('{"title": 7}')),
      ).toBeNull();
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(titleBody('{"title": "   "}')),
      ).toBeNull();
      expect(
        ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody(titleBody('["a title"]')),
      ).toBeNull();
      ...
      expect(ClaudeCodeResponse.tryExtractSessionTitleFromResponseBody("")).toBeNull();
    });
  });
});
...
describe("tryBuildInputMessagesFromRequestBody tool definitions", () => {
  ...
  it("surfaces the tools array as a compact system-side message", () => {
    ...
    const out = ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(body)!;
...
describe("salvageTruncatedRequestBody (claude's 60KB inline cap)", () => {
  ...
  it("recovers every complete leading message when the cut lands mid-history", () => {
    ...
    const out = ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(cut)!;
  ...
  it("recovers the partial system prompt when the cut lands inside it", () => {
    ...
    const out = ClaudeCodeRequest.tryBuildInputMessagesFromRequestBody(raw)!;
