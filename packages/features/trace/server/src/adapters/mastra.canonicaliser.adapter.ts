...
import { MastraValues } from "../services/mastra-value.service";
...
export class MastraCanonicaliser implements CanonicalAttributesPort {
  ...
  apply(ctx: ExtractorContext): void {
    ...
    const modelStepBody = MastraValues.tryExtractBodyFromModelStepInput(rawModelStepInput);
  ...
  private extractModelInfo(
    ...
    if (!modelName) {
      modelName = MastraValues.tryExtractModelFromMetadata(attrs);
      if (
        modelName &&
        !attrs.has(ATTR_KEYS.GEN_AI_REQUEST_MODEL) &&
        !attrs.has(ATTR_KEYS.GEN_AI_RESPONSE_MODEL) &&
  ...
  private extractIO(
    ...
    if (mastraType === "agent_run") {
      ...
      if (!attrs.has(ATTR_KEYS.LANGWATCH_OUTPUT)) {
        ...
        if (rawOutput !== void 0) {
          const text = MastraValues.tryExtractTextFromOutput(rawOutput);
          if (text) {
  ...
  private setDisplayName(
    ...
    const displayName = MastraValues.tryDeriveDisplayName({
      mastraType,
      modelName,
