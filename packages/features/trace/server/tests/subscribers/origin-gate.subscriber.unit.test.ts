...
} from "../../src/adapters/eventing.deferred-origin.adapter";
...
function createDeps(overrides: Partial<OriginGateSubscriberDeps> = {}): OriginGateSubscriberDeps {
  return {
...
/** Adapts the test's plain callback to the scheduler port the handler takes. */
class TestDeferredOriginScheduler extends TraceDeferredOriginSchedulerPort {
  constructor(private readonly deps: OriginGateSubscriberDeps) {
    super();
  }

  schedule(payload: DeferredOriginPayload): Promise<void> {
    return this.deps.scheduleDeferred(payload);
  }
}
