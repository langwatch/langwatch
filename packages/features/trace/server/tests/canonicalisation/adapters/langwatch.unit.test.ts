...
import { LangWatchCanonicaliser } from "../../../src/services/canonicalisation/langwatch.canonicaliser";
import { createExtractorContext } from "./test-helpers";

describe("LangWatchCanonicaliser", () => {
  const extractor = new LangWatchCanonicaliser();
  ...
  describe("RAG contexts promotion", () => {
    describe("when span has langwatch.rag.contexts (canonical key)", () => {
      it("promotes to canonical output", () => {
        ...
        expect(ctx.setAttr).toHaveBeenCalledWith(ATTR_KEYS.LANGWATCH_RAG_CONTEXTS, contexts);
    ...
    describe("when span has langwatch.rag_contexts (legacy key)", () => {
      it("promotes to canonical output", () => {
        const contexts = [{ document_id: "doc-2", chunk_id: "chunk-2", content: "legacy data" }];
        const ctx = createExtractorContext({
    ...
    describe("when span has langwatch.contexts (old TS SDK key, pre-fix)", () => {
      it("does NOT promote — legacy/unrecognized key, SDK now emits langwatch.rag.contexts", () => {
        const contexts = [{ document_id: "doc-3", chunk_id: "chunk-3", content: "ts sdk data" }];
