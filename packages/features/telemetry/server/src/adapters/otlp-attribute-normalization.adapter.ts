...
function scalar(value: OtlpAnyValue): string | boolean | number | Uint8Array | undefined {
  ...
  if (
    value.arrayValue &&
    value.arrayValue.values.every((item) => scalar(item) !== undefined)
  ) {
...
function flatten(
  ...
  if (value.arrayValue) {
    value.arrayValue.values.forEach((child, index) =>
...
export class OtlpAttributeNormalizationAdapter {
  private constructor() {}

  static create(): OtlpAttributeNormalizationAdapter {
    return new OtlpAttributeNormalizationAdapter();
  }
}
