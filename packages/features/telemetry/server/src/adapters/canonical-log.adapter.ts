...
import { z } from "zod";
import { sha256, stableStringify } from "./metric-serialization.adapter";
import { normalizeOtlpAttributeMap } from "./otlp-attribute-normalization.adapter";
import {
...
type PIIRedactionLevel = "STRICT" | "ESSENTIAL" | "DISABLED";
type CanonicalLogPreparationInput = TelemetryLogPreparationInput;
...
export class CanonicalLogAdapter extends TelemetryLogPreparationPort {
  private constructor() {
    super();
  }

  static create(): CanonicalLogAdapter {
    return new CanonicalLogAdapter();
  }

  prepare(input: TelemetryLogPreparationInput): Promise<CanonicalLogPreparationResult> {
    return prepareCanonicalLogRecords(input);
  }
}
