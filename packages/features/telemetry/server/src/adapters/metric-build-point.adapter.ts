import { decodeBase64OpenTelemetryId } from "./otlp-id.adapter";
import { MAX_CANONICAL_METRIC_PAYLOAD_BYTES } from "@langwatch/telemetry-contract";
import type {
  CanonicalMetricDataPoint,
  MetricKind,
  MetricTraceCorrelation,
} from "@langwatch/telemetry-contract";
import { canonicalAttributes } from "./metric-attributes.adapter";
import { correlations } from "./metric-correlations.adapter";
import { aggregation } from "./metric-kinds.adapter";
import { integerDecimal, timestampDecimal, timestampMs } from "./metric-numbers.adapter";
import {
  isRecord,
  sha256,
  stableStringify,
...
import { validatePointShape } from "./metric-validate.adapter";
import { canonicalPointValues, canonicalValueSection } from "./metric-values.adapter";
...
export class MetricBuildPointAdapter {
  private constructor() {}

  static create(): MetricBuildPointAdapter {
    return new MetricBuildPointAdapter();
  }
}
