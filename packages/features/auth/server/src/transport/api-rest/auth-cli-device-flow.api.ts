/**
 * The CLI device grant (RFC 8628) as seven routes under `/api/auth/cli`: CLI half (device-code, exchange, refresh, logout) and browser half (lookup, approve, deny), one state machine over one device-code keyspace, never split across packages. ORDERING: must register BEFORE the `/api/auth/*` catch-all. See specs/ai-governance/cli-onboarding/.
 */
...
/**
 * The flag that gates the device-session journey. Declared here, not
 * imported, since the browser's holder is a browser module a server
...
export type AuthCliDeviceFlowRestPorts = Readonly<{
  ...
  /**
   * The typed client the identity and membership reads run on. Membership
   * is re-derived from rows, not trusted from the record: an admin can
   * disable a seat between approve and exchange.
   */
   ...
   * Idempotent, and not a credential: a device session ships the
   * workspace's existing key so the CLI never asks a person for one.
   */
  ...
  /**
   * Whether one person may write to one project. A port since it's an RBAC
   * decision on the deployment's own AuthZ graph, and the ONE gate between
   * a device session and a shared project's write credential.
   */
...
export function createAuthCliDeviceFlowRestApp(options: {
  ...
  const { service, policy } = security.createServiceVersionedApp({
    name: "auth-cli-device-flow",
    basePath: "/api/auth/cli",
    // Released `langwatch` CLI builds poll these exact paths through a device
    // flow they cannot renegotiate mid-authorization.
    staticGeneration: "v1",
    errorEnvelope: "legacy",
  });

  /** Every answer here is the device flow's own, as RFC 8628 shapes it. */
  const CLI_DEVICE_FLOW_ANSWER =
    "the device flow answers OAuth's own bodies — authorization_pending, slow_down, " +
    "expired_token and the token response — which released CLI builds parse as they stand";
   ...
   * Falls back to `https://app.langwatch.ai`, the same fallback the CLI
   * uses client-side, so the round-trip self-hosted experience stays consistent.
   */
  ...
  /**
   * The rule every endpoint handing back a project's key shares: a
   * personal project is honoured only as the caller's OWN explicit pick,
   * and the caller needs a write-capable project permission.
   */
  ...
  service.registerRoute(
    ...
    (b) => policy(cliPolicy)(b).withRawResponse(CLI_DEVICE_FLOW_ANSWER),
  );
  ...
  service.registerRoute(
    ...
    async (c: ServiceContext<EndpointVariables>) => {
      ...
      // code is consumed and answered with the same fatal 410 a removed
      // member's mint already gives, so the CLI stops polling for good.
      ...
      if (record.key_selection) {
        ...
        try {
          ...
          // A ceiling refusal is permanent — the approver has since lost
          // access — so leaving the record approved means an endless "keep
          // polling" loop. Burn the device code and answer with the one code
          // the CLI already treats as fatal.
  ...
  service.registerRoute(
    ...
    async (c: ServiceContext<EndpointVariables>) => {
      ...
      const maxDurationDays = await ports
        .directory()
  ...
  service.registerRoute(
    ...
    (b) => policy(cliApproveAuth)(b).withRawResponse(CLI_DEVICE_FLOW_ANSWER),
  );
  ...
  // Without the access token it expires naturally within the hour — a real
  // gap if stolen. The session's user-scoped key is revoked alongside.
  service.registerRoute(
    "post",
    "/logout",
    MANAGEMENT_API_VERSION,
