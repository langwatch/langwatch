...
export function useHandleSavePrompt() {
  ...
  const handleSaveVersion = useCallback(async () => {
    ...
    const onError = (error: Error) => {
      // No form bridge here on purpose. The only top-level (claimable) values on the prompt form
      // are `handle`, `scope` and `configId`, and the playground renders none of them as an
      // input — the handle is read-only in the header and changed through its own dialog.
      // `applyHandledErrorToForm` would claim a `handle` field error, set it on a field nobody
      // paints, and suppress this toast — the user would hit Save and see nothing at all.
