...
/**
 * Renders JSON data in the same framed block the trace details panel uses.
 *
 * THE COLLAPSIBLE TREE DID NOT TRAVEL, and it is the one recorded loss here.
 * `platform/app` rendered this through `RenderInputOutput`, whose closure is
 * `@microlink/react-json-view` behind a dynamic import, the media-part
 * collector, a Python-repr parser and the toast singleton — four modules with
 * seven other consumers between them, none of which this move may repoint. A
 * structured output is pretty-printed instead: same frame, same monospace, no
 * fold arrows and no copy button. It returns when the trace explorer's JSON
 * viewer becomes a surface on `@langwatch/trace-web`.
 */
