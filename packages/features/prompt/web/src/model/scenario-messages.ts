...
/**
 * A tool payload as JSON, or the raw string wrapped when it is not JSON.
 *
 * Nine lines, inlined rather than imported from `@langwatch/scenario-web`: a
 * screen closure that names another feature's WEB package pays a finding for
 * it, and this is the only thing that import was for.
 */
const safeJsonParseOrStringFallback = (json: string): Record<string, unknown> => {
  try {
...
/**
 * Turns a recorded conversation into the CopilotKit messages the chat renders.
 *
 * A family-local copy of
 * `platform/app/src/components/simulations/utils/convert-scenario-messages.ts`
 * (one other caller, the simulations run view). The playground seeds its chat
 * from a trace, and the trace's messages arrive in the same shape a scenario's
 * do, which is why this conversion is shared at all.
 */
export function convertScenarioMessagesToCopilotKit(
  ...
  // `toReversed` is es2023 and this package targets es2022; a copy reversed in
  // place is the same walk without widening the whole package's lib.
...
function createToolResultMessage(
  ...
  const resultMessage: ResultMessage & { traceId?: string } = new ResultMessage({
    ...
    actionName: "tool",
    // CopilotKit declares `result` as a string and the application has always
    // handed it the PARSED value here — `safeJsonParseOrStringFallback`
    // returned `any` in `platform/app`, so nothing said so. The cast keeps the
    // runtime behaviour identical rather than changing what a tool result
    // renders as inside a page move; the mismatch is the simulations feature's
    // to settle, and it predates this copy.
