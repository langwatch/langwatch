...
import { diffPromptVersions, type PromptVersionSnapshot } from "../prompt-version-diff.ts";

const version = (overrides: Partial<PromptVersionSnapshot> = {}): PromptVersionSnapshot => ({
  messages: [
  ...
  describe("given a version republished the previous version unchanged", () => {
    describe("when the two versions are compared", () => {
      ...
      it("reports no changes", () => {
        expect(diffPromptVersions({ previous: version(), version: version() })).toEqual([]);
      });
    });
  });
  ...
  describe("given a version carries no content at all", () => {
    describe("when it is compared against a populated version", () => {
      it("does not throw and reports the removals", () => {
        ...
        expect(changes.every((change) => change.status === "removed")).toBe(true);
      });
    });
  });
});
