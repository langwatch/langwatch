...
`contract/` is the sole portable Prompt domain boundary and owns Zod 4
schemas. `server/` owns persistence and service implementations. `web/` owns
browser-safe Prompt presentation primitives and pure UI helpers; it exposes a
root-only public surface and receives data/callbacks from application
composition. Page layout, tRPC clients, REST clients, and cross-feature
bridges remain application-owned adapters.
