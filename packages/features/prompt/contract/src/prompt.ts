...
export const promptInputSchema = z.object({
  ...
  type: z.enum(["str", "float", "bool", "image", "list", "list[str]", "list[float]", "list[int]", "list[bool]", "dict", "chat_messages"]),
}).strict();
...
export const promptDemonstrationsSchema = z.object({
...
export const versionedPromptSchema = z.object({
  ...
  author: z.object({ id: z.string(), name: z.string().nullable(), email: z.string().nullable(), image: z.string().nullable() }).nullable().optional(),
...
export const promptModifyPermissionSchema = z.object({ hasPermission: z.boolean(), reason: z.string().optional() }).strict();
export type PromptModifyPermission = z.infer<typeof promptModifyPermissionSchema>;

export const promptTagAssignmentSchema = z.object({
  configId: z.string().min(1), versionId: z.string().min(1),
  promptTag: promptTagSchema, updatedAt: z.date(),
}).strict();
export type PromptTagAssignment = z.infer<typeof promptTagAssignmentSchema>;

export const promptCopySummarySchema = z.object({
  id: z.string().min(1), handle: z.string().nullable(), projectId: z.string().min(1),
  projectName: z.string(), teamName: z.string(), organizationName: z.string(),
}).strict();
export type PromptCopySummary = z.infer<typeof promptCopySummarySchema>;

export const promptCopySourceSchema = z.object({ sourcePromptId: z.string().min(1), sourceProjectId: z.string().min(1) }).strict();
export type PromptCopySource = z.infer<typeof promptCopySourceSchema>;

export const promptSyncResultSchema = z.object({
  action: z.enum(["created", "updated", "conflict", "up_to_date"]),
  prompt: versionedPromptSchema.optional(),
  conflictInfo: z.object({
    localVersion: z.number(), remoteVersion: z.number(), differences: z.array(z.string()),
    remoteConfigData: z.record(z.string(), z.unknown()), remoteParameters: z.record(z.string(), z.unknown()).optional(),
  }).strict().optional(),
}).strict();
...
export const promptShorthandSchema = z.object({
  slug: z.string().min(1),
