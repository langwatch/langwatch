...
```text
packages/features/entitlements/
├── contract/                 # @langwatch/entitlements-contract
├── server/                   # @langwatch/entitlements-server
├── adrs/
└── specs/
```
...
consumers depend only on `@langwatch/entitlements-contract`. An application
...
Core consumers depend only on `@langwatch/entitlements-contract`. Billing and
...
`EntitlementsService` is a class created through `EntitlementsService.create`.
