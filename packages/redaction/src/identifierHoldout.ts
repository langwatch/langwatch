 ...
 * canonical name during canonicalisation, so anything that reasons about a
 * metadata key by its name has to accept every prefix on this list.
 *
 * It matters beyond canonicalisation because redaction runs BEFORE the fold:
 * the personal-data pass sees the prefixed spelling, and a namespace added to
 * the canonicaliser but not taught to {@link isReservedIdentifierAttributeKey}
 * is a namespace whose trace identifiers are handed to the recognizers. That is
 * why it is one exported constant and not a literal in each place — the copy
 * that gets forgotten is the one that silently stops protecting anything.
 *
 * It is declared HERE, in the leaf package, rather than beside the
 * canonicaliser: this package carries no workspace dependency at all (see the
 * shape constraints in `contentRedaction.ts`), so it cannot read a constant out
 * of a module, while every module can read one out of it. The canonicaliser\'s
 * own copy lives in `modules/trace/server/src/rules/langwatch-metadata.rules.ts`
 * and should import this one.
 */
