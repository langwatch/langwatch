...
describe("redactSecretsInText, given text that only looks like secrets", () => {
  const leaveAlone: [string, string][] = [
    ...
    [
      "a source path",
      "see modules/metric/process/src/services/metric-request-collection.service.ts",
    ],
...
describe("redactSecretsInText, given a payload the size of the scan budget", () => {
  ...
  it("completes well inside the ingestion budget", () => {
    const chunk =
      "The agent read modules/trace/process/src/services/trace-legacy-read.service.ts at " +
