...
const { envHttpProxyAgentMock, fetchMock } = vi.hoisted(() => ({
...
const makeProvider = ({ apiKey = "re_test_key" }: { apiKey?: string } = {}) =>
  ResendEmailProvider.create({
    configuration: { apiKey },
    outboundProxy: parseOutboundProxyConfig(process.env),
  });

const makeProviderWithNoApiKey = () =>
  ResendEmailProvider.create({
...
describe("ResendEmailProvider.send", () => {
  ...
  describe("given the API rejects the message", () => {
    ...
    it("keeps the response body out of the error, which gets logged", async () => {
      ...
      // addresses; the provider never reads it on the error path.
      fetchMock.mockResolvedValue({
        ok: false,
        status: 422,
        statusText: "Unprocessable Entity",
