...
describe("integration test lanes", () => {
  ...
  describe("given a test file declaring the jsdom environment", () => {
    ...
    it("runs in the component lane when it names no datastore", () => {
      const source = `${jsdomHeader}
        ...
        import { Button } from "@langwatch/trace-process";
    ...
    it.each(["prisma", "clickhouse", "redis", "bullmq", "groupQueue"])(
      ...
      (dependency) => {
        expect(laneForSource(`${jsdomHeader}import x from "${dependency}";`)).toBe("datastore");
      },
    );
  ...
  describe("when a lane's file list becomes include patterns", () => {
    ...
    it("escapes a Next.js route segment so it matches only itself", () => {
      const [pattern] = toIncludePatterns(["src/pages/[project]/__tests__/a.integration.test.tsx"]);
      ...
      expect(picomatch.isMatch("src/pages/p/__tests__/a.integration.test.tsx", pattern!)).toBe(
        false,
      );
      expect(
        picomatch.isMatch("src/pages/[project]/__tests__/a.integration.test.tsx", pattern!),
      ).toBe(true);
    });
