...
function offendersIn({
  ...
  for (const site of scanSourceForMockSpecifiers({ source })) {
    ...
    offenders.push(`${relativePath}:${site.line} vi.mock("${site.specifier ?? "<computed>"}")`);
  }
  return offenders;
}
...
function scanTrackedTestFiles(): {
  ...
  for (const { fileName, source } of parsed) {
    offenders.push(...offendersIn({ relativePath: fileName, source, aliasesByConfigDir }));
  }
...
describe("the mock-specifier rule", () => {
  ...
  describe("given a specifier built at runtime", () => {
    it("records the call without a module name", () => {
      expect(scan("vi.mock(modulePath, () => ({}));")).toEqual([{ line: 1, specifier: undefined }]);
    });
