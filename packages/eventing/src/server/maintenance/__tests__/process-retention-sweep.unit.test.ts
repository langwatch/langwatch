...
class RecordingRetentionMetrics extends ProcessRetentionMetricsPort {
  ...
  readonly failures: RetentionFamily[] = [];

  recordSweptRows(family: RetentionFamily, rows: number): void {
    this.sweptRows[family] += rows;
  }
...
function deps(overrides: Partial<ProcessRetentionSweepDeps> = {}): ProcessRetentionSweepDeps {
  return {
    deleteDispatchedOutboxBatch: backlog(0),
    deleteDeadOutboxBatch: backlog(0),
    deleteConsumedInboxBatch: backlog(0),
    metrics: new RecordingRetentionMetrics(),
    now: () => NOW,
    sleep: async () => {},
    ...overrides,
  };
...
describe("processRetentionSweep", () => {
  describe("given a scheduled sweep process", () => {
    ...
    describe("when it has woken before", () => {
      ...
      it("doubles the budget each wake until it reaches the ceiling", () => {
        expect(retentionSweepBatchBudget(0)).toBe(RETENTION_SWEEP_INITIAL_BATCHES_PER_WAKE);
        expect(retentionSweepBatchBudget(1)).toBe(RETENTION_SWEEP_INITIAL_BATCHES_PER_WAKE * 2);
        expect(retentionSweepBatchBudget(2)).toBe(RETENTION_SWEEP_INITIAL_BATCHES_PER_WAKE * 4);
  ...
  describe("given each family has a backlog", () => {
    describe("when the retention sweep runs", () => {
      ...
      it("deletes at most the per-wake budget and leaves the rest", async () => {
        ...
        const deleted = (await Promise.all(dispatched.mock.results.map((r) => r.value))).reduce(
      ...
      it("issues no second statement for a family that was already empty", async () => {
        ...
        await runProcessRetentionSweep(deps({ deleteConsumedInboxBatch: inbox }))(payload());
  ...
  describe("given the deletes are slow enough to outlast the wake budget", () => {
    ...
    describe("when the retention sweep runs", () => {
      ...
      it("stops draining at the deadline instead of running past the lease", async () => {
        ...
        expect(dispatched.mock.calls.length).toBeLessThan(RETENTION_SWEEP_MAX_BATCHES_PER_WAKE);
      });
  ...
  describe("given rows were deleted in more than one family", () => {
    describe("when the sweep reports what it deleted", () => {
      ...
      it("names the family each count belongs to", async () => {
        // The claim worth pinning is that an operator can tell dispatched-outbox
        // reaping apart from inbox reaping on the metric they actually scrape,
        // so the assertion is on what the sweep reports per family rather than
        // on a counter binding.
        const metrics = new RecordingRetentionMetrics();
        ...
        expect(metrics.sweptRows).toEqual({
          dispatched_outbox: 3,
          dead_outbox: 0,
          inbox: 7,
        });
  ...
  describe("given one family's delete statement fails", () => {
    describe("when the sweep runs", () => {
      it("reports that family's failure and still drains the others", async () => {
        const metrics = new RecordingRetentionMetrics();
        ...
        await expect(
          runProcessRetentionSweep(
            deps({
              ...
              deleteConsumedInboxBatch: backlog(4),
            }),
          )(payload()),
        ).resolves.toBeUndefined();

        expect(metrics.failures).toEqual(["dispatched_outbox"]);
        expect(metrics.sweptRows.inbox).toBe(4);
      });
    });
  });
});
