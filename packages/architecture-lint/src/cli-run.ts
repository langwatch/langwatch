...
function selectedViolations(): ArchitectureViolation[] {
  if (reviewCommentBlocks) return [];

  if (reviewTestQuality) return lintTestQuality(root, { files: changedFiles });

  return baselinePolicyViolations ?? fullWorkspaceViolations();
}

const violations = selectedViolations();
