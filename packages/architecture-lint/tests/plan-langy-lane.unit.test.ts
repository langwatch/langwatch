...
/**
 * The env files live at the workspace ROOT, which is also where the planner
 * derives the repo from and writes the python shim, so the fixture is one
 * directory rather than a nested `platform/app` the tree no longer has.
 */
...
`;
  ...
  const read = (key: string) => stdout.match(new RegExp(`__${key}=(.*)`))?.[1] ?? "";
  ...
  describe("given a lane that will start", () => {
    describe("when the lane is started", () => {
      ...
      it("caps the worker pool to the local size", () => {
        ...
        expect(r.command).toMatch(/LANGY_MAX_WORKERS="\$\{LANGY_MAX_WORKERS:-2\}"/);
      });
      ...
      it("reaps an idle worker in minutes rather than the production wait", () => {
        ...
        expect(r.command).toMatch(/LANGY_WORKER_IDLE_MS="\$\{LANGY_WORKER_IDLE_MS:-120000\}"/);
