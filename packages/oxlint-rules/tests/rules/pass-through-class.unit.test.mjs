...
import { passThroughClassRule } from "../../src/rules/pass-through-class.rule.mjs";
...
function report(code, filename = SERVICE) {
  return runRule(passThroughClassRule, { code, cwd: workspace.cwd, filename, options: [] });
}
...
describe("given a TypeScript source the over-abstraction policies read", () => {
  describe("when a class forwards every public method to one collaborator", () => {
    ...
    it("reports passThrough on the class, naming the class, the count and the receiver", () => {
      const found = report(`import type { Inner } from "./inner";
...
export class ExampleService {
  ...
  e(input: In): Out { return this.inner.e(input); }
      ...
      expect(found.map((finding) => [finding.messageId, finding.line])).toEqual([
        ["passThrough", 3],
      ]);
  ...
  e = (input: In): Out => this.inner.e(input);
      ...
      expect(found.map((entry) => entry.messageId)).toEqual(["passThrough"]);
    });
  ...
  e(input: In): Out { return this.inner.e(input); }
      ...
      expect(report(code, "modules/project/process/src/services/example.service.test.ts")).toEqual(
        [],
      );
      expect(report(code, "modules/project/process/src/generated/example.service.ts")).toEqual([]);
    });
  });
});
