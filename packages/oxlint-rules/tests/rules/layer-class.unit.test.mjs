...
const workspace = createFixtureWorkspace({
  features: { project: { layoutVersion: 0, roles: { process: {} } } },
});

afterAll(() => workspace.cleanup());

const SERVICE = "modules/project/process/src/services/example.service.ts";
const ALLOWED =
...
describe("given a TypeScript source the over-abstraction policies read", () => {
  ...
  e(input: In): Out { return this.deps.example.e(input); }
      ...
      expect(report(facade, "modules/project/process/src/app/example.app.ts")).toEqual([]);
      expect(
        report(
          facade,
          "modules/project/process/src/repositories/routed/routed.example.repository.ts",
        ),
      ).toEqual([]);
    });
  });
  ...
  e(input: In): Out { return this.inner.e(input); }
      ...
      expect(
        report(code, "modules/project/process/src/services/example.service.test.ts"),
      ).toEqual([]);
      expect(
        report(code, "modules/project/process/src/generated/example.service.ts"),
      ).toEqual([]);
    });
  });
});
