...
const asErrorBody = (value: unknown): ErrorBody | null => {
  ...
  // holding a literal `code` key would shadow the real discriminant on `error`
  ...
  // When `kind` named the failure, `error` is free to be the sentence (dialect
  // 3 always emits `kind` alongside `code`, so checking `kind` alone covers it;
  // a bare `code` without `kind` leaves `error` as the dialect-1 discriminant).
  const sentence =
  ...
  return {
    ...
    meta: asRecord(explicitMeta) ?? rest,
    traceId:
