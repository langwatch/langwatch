...
// ---------------------------------------------------------------------------
// An endpoint is one registration call (specs/fluent-registration.feature)
// ---------------------------------------------------------------------------
...
describe("registerRoute", () => {
      ...
      .registerRoute(
        ...
        (builder) =>
          builder
            ...
            .withOutput(z.object({ id: z.string(), dryRun: z.boolean(), name: z.string() })),
      )
      .build();
    ...
    expect(() =>
      Reflect.apply(service.registerRoute, service, [
        ...
        (builder: ChainBuilder) =>
          builder.withInput(z.object({ id: z.string() })).withOutput(z.object({ ok: z.boolean() })),
      ]),
  ...
  it("refuses a bare name: a route path must start with a slash", () => {
    expect(() =>
      buildTestService().registerRoute(
        "post",
        "things.create",
  ...
  it("rejects paths that squat on the reserved version namespace", () => {
    for (const path of ["/latest", "/preview/items", "/2025-03-15/items", "/2025-02-30/items"]) {
      expect(() =>
        buildTestService().registerRoute(
          "get",
          path,
...
describe("provide", () => {
  ...
  it("rejects provider names reserved by the framework", () => {
    expect(() => buildTestService().provide({ project: () => ({ id: "wrong" }) })).toThrow(
      /reserved by BaseApp/,
    );
  ...
  it("runs service middleware before endpoint middleware", async () => {
    ...
    const app = buildTestService()
      ...
      .registerRoute(
        ...
        (b) => b.withMiddleware(endpointMiddleware).withOutput(z.object({ ok: z.boolean() })),
      )
      .build();
...
describe("group", () => {
  it("applies its chain to everything registered through it", async () => {
    ...
    const things = app.group("things", (b) => b.withDocs({ tags: ["Things"] }).withRateLimit());
  ...
  it("lets the endpoint's re-declaration win over group and service defaults", async () => {
    ...
    things.registerRoute(
      ...
      "2025-03-15",
      async () => undefined,
      (b) => b.withMeta({ level: "endpoint" }).withOutput(z.void()),
    );
    things.registerRoute(
      "post",
      "/things.get",
  ...
  it("carries no version: every registration names its own", async () => {
    ...
    const dated = [
      ...new Set(mounted.filter((r) => r.path.endsWith("/things.create")).map((r) => r.version)),
    ].sort();
  ...
  it("uses registerRoute paths as-is", async () => {
    ...
    things.registerRoute(
      ...
      (b) => b.withParams(z.object({ id: z.string() })).withOutput(z.object({ id: z.string() })),
    );
    service.build();
