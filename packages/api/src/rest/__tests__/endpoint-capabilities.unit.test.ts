  ...
  it("keeps operations on the same path in separate limiter budgets", async () => {
    const app = createService({
      ...
      .registerRoute(
        ...
        "2026-08-07",
        async () => ({ method: "get" }),
        (b) => b.withOutput(z.object({ method: z.string() })).withRateLimit(),
      )
      .registerRoute(
        "delete",
        "/shared",
...
describe("withCache", () => {
  ...
  function buildCachedService(
    ...
    const app = createService({
      ...
      .registerRoute("post", "/things.get", "2026-08-07", handler, (b) =>
        b
  ...
  it("treats the cache key as the complete call", async () => {
    ...
    const app = createService({
      ...
      .registerRoute("post", "/things.get", "2026-01-15", handler, (b) =>
        b
          .withInput(z.object({ id: z.string() }))
          .withOutput(z.object({ id: z.string(), name: z.string() }))
          .withCache("things", 60),
  ...
  it("refuses the registration when no output is declared: unvalidated bytes may not be cached", () => {
    ...
    // Every route declares an output, so a cached endpoint can never carry
    // unvalidated bytes. The build-time cache check behind this stays as a
    // second line of defence for a caller that reaches the builder behind an
    // `any`; the route surface refuses it a step earlier.
    expect(() =>
