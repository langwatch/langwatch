...
import { getRoutePolicy } from "../route-registry.js";
import {
...
/**
 * A spine whose every port records the call that produced it.
 *
...
function recordingSpine() {
  ...
  const ports: SecuredAppPorts = {
    ...
    authenticateOrganization: (envelope) =>
      record(`authenticateOrganization:${envelope}`),
  ...
  return { calls, spine: createSecuritySpine<object, object>(ports) };
}
...
describe("SecuredApp", () => {
  ...
  describe("the credential class a family publishes", () => {
    it("takes the app's own family unless the app overrides it", () => {
      ...
      admin.access(internalSecret("the operator's instance key")).post("/x", (c) =>
        c.text("ok"),
      );
    ...
    it("keeps a public route on an overriding app at none", () => {
      ...
      expect(
        getRoutePolicy("GET", "/api/__pkg_class_discovery/ServiceProviderConfig")
          ?.credentialClass,
      ).toBe("none");
    });
  });
