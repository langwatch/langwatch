...
> Originally shipped as the standalone package `@langwatch/trpc`. It was folded
> into `@langwatch/api` as the `./trpc` entry point; the boundary this ADR
> draws is unchanged, and the package's other two entry points (`.` for the
> transport-agnostic vocabulary, `./rest` for the Hono service framework)
> import nothing from it.

`@langwatch/api/trpc` owns generic typed tRPC root construction **and the policy
