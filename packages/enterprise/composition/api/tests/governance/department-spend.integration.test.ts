 ...
 *  `CI_CLICKHOUSE_URL` is the job-wide server whose test database is the one
 *  the job migrates. */
function testClickHouseUrl(): URL | null {
...
describe.skipIf(!databaseUrl || !chUrl)("ActivityMonitorService.spendByDepartment", () => {
  ...
  beforeAll(async () => {
    ...
    org = await prisma.organization.create({ data: { name: ns, slug: `org-${ns}` } });
    crossOrg = await prisma.organization.create({ data: { name: `${ns}-x`, slug: `org-x-${ns}` } });
    ...
    // Same-named department in the other org — proves the rollup keys on this
    // org's ids, never a name collision across orgs.
    const crossEngineering = await prisma.department.create({
      data: { organizationId: crossOrg.id, name: "Engineering" },
    });

    const team = await prisma.team.create({
      data: { name: `platform-${ns}`, slug: `platform-${ns}`, organizationId: org.id },
    });
    const crossTeam = await prisma.team.create({
      data: { name: `x-team-${ns}`, slug: `x-team-${ns}`, organizationId: crossOrg.id },
    });

    // The agent project carries Engineering directly (autonomous traffic, no
    // principal user, so it attributes by project).
    ...
    await prisma.user.create({ data: { id: ROBIN, email: ROBIN_EMAIL, name: "Robin" } });
  ...
  afterAll(async () => {
    ...
    await prisma.organization
      ...
      .catch(() => undefined);
    for (const tenantId of [personalProjectId, agentProjectId, crossProjectId]) {
      if (!tenantId) continue;
      await ch
        .command({
          query: `ALTER TABLE trace_summaries DELETE WHERE TenantId = '${tenantId}'`,
        })
        .catch(() => undefined);
    }
    await ch.close();
  ...
  describe("given spend across personal, team, and agent projects", () => {
    ...
    it("rolls personal, agent, and unattributed spend up by department across all projects", async () => {
      ...
      // The whole org's spend rolls up (2 + 3 + 1 = 6), proving the card is not
      // limited to a single governance ingestion project.
  ...
  describe("given another org with spend under a like-named department", () => {
    ...
    it("never includes the other org's spend in this org's rollup", async () => {
      ...
      const total = rows.reduce((sum, r) => sum + Number(r.spendUsd), 0);
      // The cross-org $50 must not leak — this org's total stays at $6.
      expect(total).toBeLessThan(10);
      // The Engineering row is this org's $3 agent spend, not the cross org's
      // $50 under its own like-named Engineering department.
