...
export abstract class ImpersonationRepository {
  abstract findTarget(userId: string): Promise<ImpersonationTarget | null>;
...
export class ImpersonationService extends AdminImpersonation {
  private constructor(
  ...
  async start(input: StartImpersonationInput): Promise<void> {
    const target = await this.repository.findTarget(input.userIdToImpersonate);
    if (!target) {
