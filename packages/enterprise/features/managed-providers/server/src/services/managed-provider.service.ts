...
export class ManagedProviderService extends ManagedProviderServiceContract {
  ...
  isManagedProvider(organizationId: string, provider: string): boolean {
    return (
      provider === "bedrock" &&
      this.configuration.forOrganization(organizationId) !== null
    );
  }
  ...
  async buildLitellmParameters(
    ...
    const organizationId = await this.organizationForProject(input.projectId);
    if (!organizationId) return input.params;
    const config = this.configuration.forOrganization(organizationId);
  ...
  private async organizationForProject(projectId: string): Promise<string | null> {
