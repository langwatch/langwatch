 ...
 *
 * It answers from the same {@link ScimApp} the settings page reaches over
 * tRPC, so the two doors cannot drift on what minting a token means.
 */
...
type ScimTokensContext = ServiceContext<EndpointVariables & { scim: ScimApp }>;
...
export function createScimTokensRestApp(options: {
  ...
  const { security, enterpriseGate, app, audit } = options;
  ...
  const createTokenHandler = async (
    ...
    const created = await c.get("scim").generateToken({
      ...
      // The connection is the token's whole write authority, and the service
      // refuses without one. Dropping it here made every REST create answer
      // `scim_connection_required` no matter what the caller sent, while the
      // tRPC door passed it and worked.
      ...
      policy("organization:manage")(b)
        ...
        .withOutput(
          z.object({
            id: z.string(),
            token: z.string(),
            // Declared because the handler returns it and the framework
            // validates the answer against this schema: undeclared, the one
            // field naming what the token can reach was dropped on the way out.
            connectionId: z.string().nullable(),
            description: z.string().nullable(),
          }),
        )
