...
class FakeErrorReporter extends BillingErrorReporter {
  ...
  static create(): FakeErrorReporter {
    return new FakeErrorReporter();
  }
}

class FakeUsageLimitEmail extends UsageLimitEmailAdapter {
  private constructor(readonly send = vi.fn().mockResolvedValue(undefined)) {
    super();
  }

  static create(): FakeUsageLimitEmail {
    return new FakeUsageLimitEmail();
  }
}
...
describe("NotificationService", () => {
  ...
  let usageLimitEmail: FakeUsageLimitEmail;
  let config: {
    baseHost: string;
  ...
  describe("sendUsageLimitEmail()", () => {
    ...
    describe("when email sends successfully", () => {
      it("delegates to sendUsageLimitEmail mailer", async () => {
        ...
        expect(usageLimitEmail.send).toHaveBeenCalledWith({
          to: "admin@acme.com",
          organizationName: "Acme",
          usage: emailParams.usageData satisfies UsageLimitEmailData,
        });
      });
    });
  ...
  describe("sendHubspotPlanLimitForm()", () => {
    ...
    describe("when HubSpot request fails", () => {
      it("catches the error and captures exception", async () => {
        ...
        expect(errorReporter.capture).toHaveBeenCalled();
      });
    });
  });
