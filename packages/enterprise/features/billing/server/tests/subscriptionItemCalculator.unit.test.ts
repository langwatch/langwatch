...
describe("subscriptionItemCalculator", () => {
  describe("calculator.getItemsToUpdate()", () => {
    it("updates launch items and deletes stale items from other plans", () => {
      const items = calculator.getItemsToUpdate({
        currentItems: [
  ...
  describe("calculator.createItemsToAdd()", () => {
    it("creates launch add-on items when usage exceeds plan limits", () => {
      const items = calculator.createItemsToAdd(
        PlanTypes.LAUNCH,
        { quantity: 40_000 },
        { quantity: 8 },
      );
  ...
  describe("calculator.calculateQuantityForPrice()", () => {
    it("calculates quantity for user add-ons", () => {
      expect(
        calculator.calculateQuantityForPrice({
