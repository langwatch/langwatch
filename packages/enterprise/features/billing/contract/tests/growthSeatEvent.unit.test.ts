...
describe("growthSeatEvent", () => {
  describe("isGrowthSeatPrice()", () => {
    describe("when given a growth seat price ID", () => {
      it("returns true for EUR monthly", () => {
        expect(isGrowthSeatPrice("price_seat_eur_monthly", prices)).toBe(true);
      });

      it("returns true for EUR annual", () => {
        expect(isGrowthSeatPrice("price_seat_eur_annual", prices)).toBe(true);
      });

      it("returns true for USD monthly", () => {
        expect(isGrowthSeatPrice("price_seat_usd_monthly", prices)).toBe(true);
      });

      it("returns true for USD annual", () => {
        expect(isGrowthSeatPrice("price_seat_usd_annual", prices)).toBe(true);
      });
    });

    describe("when given a non-seat price ID", () => {
      it("returns false for an events price", () => {
        expect(isGrowthSeatPrice("price_events_eur_monthly", prices)).toBe(false);
      });

      it("returns false for an unknown price", () => {
        expect(isGrowthSeatPrice("price_unknown", prices)).toBe(false);
      });
    });
  });
  ...
  describe("isGrowthEventsPrice()", () => {
    describe("when given a growth events price ID", () => {
      it("returns true for EUR monthly", () => {
        expect(isGrowthEventsPrice("price_events_eur_monthly", prices)).toBe(true);
      });

      it("returns true for EUR annual", () => {
        expect(isGrowthEventsPrice("price_events_eur_annual", prices)).toBe(true);
      });

      it("returns true for USD monthly", () => {
        expect(isGrowthEventsPrice("price_events_usd_monthly", prices)).toBe(true);
      });

      it("returns true for USD annual", () => {
        expect(isGrowthEventsPrice("price_events_usd_annual", prices)).toBe(true);
      });
    });
    ...
    describe("when given a non-events price ID", () => {
      it("returns false for a seat price", () => {
        expect(isGrowthEventsPrice("price_seat_eur_monthly", prices)).toBe(false);
      });

      it("returns false for an unknown price", () => {
        expect(isGrowthEventsPrice("price_unknown", prices)).toBe(false);
      });
    });
  });
