import { describe, expect, it } from "vitest";
import {
  BillingPriceCatalogue,
  formatPrice,
  getAnnualDiscountPercent,
  getGrowthSeatPriceCents,
} from "../index";

const prices = BillingPriceCatalogue.create("test").prices;

describe("pricing", () => {
  describe("getGrowthSeatPriceCents()", () => {
    it("returns EUR monthly at 2900 cents", () => {
      expect(getGrowthSeatPriceCents(prices).EUR.monthly).toBe(2900);
    });

    it("returns EUR annual at 32000 cents", () => {
      expect(getGrowthSeatPriceCents(prices).EUR.annual).toBe(32000);
    });

    it("returns USD monthly at 3200 cents", () => {
      expect(getGrowthSeatPriceCents(prices).USD.monthly).toBe(3200);
    });

    it("returns USD annual at 35328 cents", () => {
      expect(getGrowthSeatPriceCents(prices).USD.annual).toBe(35328);
    });
  });

  describe("getAnnualDiscountPercent()", () => {
    it("returns 8 for EUR", () => {
      expect(getAnnualDiscountPercent("EUR", prices)).toBe(8);
    });

    it("returns 8 for USD", () => {
      expect(getAnnualDiscountPercent("USD", prices)).toBe(8);
    });
  });

  describe("formatPrice()", () => {
    it("formats whole EUR amount without decimals", () => {
      expect(formatPrice({ cents: 2900, currency: "EUR" })).toBe("\u20AC29");
    });

    it("formats fractional USD amount with decimals", () => {
      expect(formatPrice({ cents: 35328, currency: "USD" })).toBe("$353.28");
    });

    it("formats large EUR amount with thousands separator", () => {
      expect(formatPrice({ cents: 128000, currency: "EUR" })).toBe("\u20AC1,280");
    });

    it("formats zero correctly", () => {
      expect(formatPrice({ cents: 0, currency: "EUR" })).toBe("\u20AC0");
    });
  });
});
