...
function truncateValue(value: AuditLogJsonValue, maxStringLength: number): AuditLogJsonValue {
...
export class DefaultAuditLogService extends AuditLogServiceContract {
  ...
  async record(command: RecordAuditLogCommand): Promise<void> {
    ...
    await this.repository.create({
      ...parsed,
      args: parsed.args === undefined ? undefined : boundJson(parsed.args, this.maxArgsBytes),
    });
  }
}
