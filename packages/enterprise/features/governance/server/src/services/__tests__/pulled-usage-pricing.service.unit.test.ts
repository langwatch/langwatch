...
class TestRates extends PulledUsageRatePort {
  rate(input: Parameters<PulledUsageRatePort["rate"]>[0]): {
    ...
    if (input.model !== "anthropic/claude-sonnet-5") {
      return { costNanoUsd: 0, rateVersion: "test-rate" };
    }
    return {
...
const price = (input: PulledUsagePriceInput) =>
  PulledUsagePricingService.create(new TestRates()).price(input);

describe("given the provider reported its own cost", () => {
  describe("when the item is priced", () => {
...
describe("given the provider gave only quantities", () => {
  describe("when the item is priced", () => {
  ...
  describe("when the same item is priced more than once", () => {
    it("returns the identical integer, so no surface can disagree", () => {
      const input: PulledUsagePriceInput = {
        basis: "computed",
        model: "anthropic/claude-sonnet-5",
        quantities: QUANTITIES,
      };
