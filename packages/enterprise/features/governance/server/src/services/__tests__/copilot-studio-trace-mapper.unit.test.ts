...
function spansOf(events: NormalizedPullEvent[]) {
  const request = CopilotStudioTraceMapperService.tryToTraceRequest({ events, origin: ORIGIN });
...
describe("given two sources reading the same environment", () => {
  ...
  it("keeps their conversations apart", () => {
    ...
    const theirs =
      CopilotStudioTraceMapperService.tryToTraceRequest({
...
describe("given a conversation Microsoft stored across several rows", () => {
  ...
  it("groups batches into one thread with per-turn traces", () => {
    ...
    const threadIds = new Set(spans.map((s) => attrsOf(s)["langwatch.thread.id"]));
    expect(threadIds.size).toBe(1);
    const rendered = JSON.stringify(spans);
    expect(rendered).toContain("How do I reset my laptop?");
    expect(rendered).toContain("Hold the power button for ten seconds.");
...
describe("given a multi-turn conversation", () => {
  ...
  it("carries each turn's own input and output", () => {
    ...
    const outputs = turns
      ...
      .filter((v): v is string => typeof v === "string");
    expect(outputs.some((v) => v.includes("France is a country in Western Europe."))).toBe(true);
    expect(outputs.some((v) => v.includes("Santiago is the capital of Chile."))).toBe(true);
  });
});
...
describe("given events that are not Copilot conversations", () => {
  it("routes nothing", () => {
    ...
    expect(
      CopilotStudioTraceMapperService.tryToTraceRequest({ events: [foreign], origin: ORIGIN }),
    ).toBeNull();
  });
