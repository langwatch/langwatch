...
export const COPILOT_CONVERSATION_SPAN_NAME = "copilot_studio.conversation" as const;
 ...
 */
/**
 * Copilot Studio transcript rows as a trace.
 *
 * The mapping is one job with many steps, and every step but the entry point
 * exists only to serve it: grouping rows into conversations, ordering
 * activities within one, folding activities into turns, and turning those
 * into spans. Naming that as a class rather than thirty-four loose functions
...
export class CopilotStudioTraceMapperService {
   ...
   */
  private static roleOf(raw: unknown): number | null {
  ...
  private static activityMs(activity: Activity): number | null {
   ...
   */
  private static conversationKeyOf(row: TranscriptRow): string | null {
  ...
  private static hasBatchGap(batches: number[]): boolean {
    if (batches.length < 2) {
      return false;
    }
   ...
   */
  private static rememberBotFacts(params: { bot: BotFacts; extra: Record<string, unknown> }): void {
  ...
  private static bucketRowsByConversation(
   ...
   */
  private static byBatchThenArrival(a: IndexedRow, b: IndexedRow): number {
   ...
   */
  private static isActivityObject(value: unknown): value is Activity {
  ...
  private static activitiesOf(rows: IndexedRow[]): Activity[] {
    ...
    for (const { row } of rows) {
      ...
      for (const entry of list) {
        if (CopilotStudioTraceMapperService.isActivityObject(entry)) {
   ...
   */
  private static timeOrderActivities(activities: Activity[]): Activity[] {
  ...
  private static isDesignModeConversation(activities: Activity[]): boolean {
  ...
  private static conversationGroupOf(params: {
    key: string;
  ...
  private static textOf(activity: Activity): string {
  ...
  private static closeTurn(state: TurnAccumulator): void {
    if (state.open) {
   ...
   * conversations.
   */
  private static applyMessage(params: { state: TurnAccumulator; message: ReadableMessage }): void {
   ...
   */
  private static turnsOf(activities: Activity[]): {
    turns: Turn[];
    skipped: number;
  } {
    ...
    for (const activity of activities) {
      ...
      if (!message) {
        ...
        if (CopilotStudioTraceMapperService.textOf(activity)) {
          state.skipped += 1;
        }
  ...
  private static toolNameOf(value: ToolCallValue): string {
    return (
   ...
   * so stands alone.
   */
  private static callIdOf(params: { value: ToolCallValue; activityId: string }): string {
   ...
   * identified or dated.
   */
  private static toolCallTraceOf(activity: Activity): ToolCallTrace | null {
  ...
  private static chatValue(role: string, content: string): string {
   ...
   */
  private static agentChangedSince(bot: BotFacts, conversationEndMs: number): boolean {
    if (!bot.modifiedOn) {
      return false;
    }
  ...
  private static conversationAttrs(params: {
  ...
  private static turnSpan(params: {
    origin: RoutingOrigin;
    group: ConversationGroup;
    turn: Turn;
    traceId: string;
  ...
  private static toolSpan(params: {
    origin: RoutingOrigin;
    call: ToolCall;
    traceId: string;
    parentSpanId: string;
   ...
   */
  private static conversationSpans(params: {
    origin: RoutingOrigin;
    group: ConversationGroup;
  }): OtlpJsonSpan[] {
