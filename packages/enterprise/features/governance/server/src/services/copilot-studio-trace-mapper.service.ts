...
export class CopilotStudioTraceMapperService {
  ...
  private static conversationSpans(params: {
    ...
    const { turns, skipped } = CopilotTurnAssemblyService.turnsOf(group.activities);
    if (turns.length === 0) {
      return [];
    }

    const calls = CopilotTurnAssemblyService.toolCallsOf(group.activities);
  ...
  static tryToTraceRequest({
    ...
    const spans = CopilotTranscriptGroupingService.groupTranscriptRows(conversationEvents).flatMap(
