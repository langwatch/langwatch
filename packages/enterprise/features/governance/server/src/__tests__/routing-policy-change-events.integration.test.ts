...
async function createKey(args: { label: string; routingPolicyId: string | null }): Promise<string> {
...
describe.skipIf(!databaseUrl)(
  "given a routing policy the gateway has already materialised into bundles",
  () => {
    beforeAll(async () => {
      ...
      await prisma.user.create({
        data: { id: USER_ID, email: `rpce-${suffix}@example.com`, name: "RPCE Tester" },
      });
    ...
    afterAll(async () => {
      ...
      await prisma.virtualKeyScope.deleteMany({
        where: { virtualKeyId: { in: createdKeyIds } },
      });
    ...
    describe("when an admin edits the policy", () => {
      ...
      it("appends exactly one ROUTING_POLICY_UPDATED and bumps only the keys that reference it", async () => {
        ...
        const referencingA = await createKey({ label: "ref-a", routingPolicyId: policyId });
        const referencingB = await createKey({ label: "ref-b", routingPolicyId: policyId });
        const unrelated = await createKey({ label: "unrelated", routingPolicyId: null });
        const before = {
      ...
      describe("given the edit is rejected", () => {
        it("writes neither the policy change nor the event", async () => {
          ...
          const key = await createKey({ label: "rejected-ref", routingPolicyId: policyId });
    ...
    describe("when an admin deletes the policy", () => {
      ...
      it("appends ROUTING_POLICY_DELETED and moves the released key off policy routing", async () => {
        ...
        const key = await createKey({ label: "deleted-ref", routingPolicyId: policyId });
        const revisionBefore = await revisionOf(key);
        ...
        expect(await prisma.routingPolicy.findUnique({ where: { id: policyId } })).toBeNull();
      });
    });
