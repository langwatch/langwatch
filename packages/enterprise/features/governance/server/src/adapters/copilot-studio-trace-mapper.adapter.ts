 ...
 */
function rememberBotFacts(params: { bot: BotFacts; extra: Record<string, unknown> }): void {
...
function bucketRowsByConversation(events: NormalizedPullEvent[]): Map<string, ConversationBucket> {
...
function isDesignModeConversation(activities: Activity[]): boolean {
  return activities.some(
    (a) => a.valueType === "ConversationInfo" && asObject(a.value)?.isDesignMode === true,
  );
}
...
function conversationGroupOf(params: {
  ...
  const ordered = bucket.rows.map((row, index) => ({ ...row, index })).sort(byBatchThenArrival);
 ...
 * conversations.
 */
function applyMessage(params: { state: TurnAccumulator; message: ReadableMessage }): void {
 ...
 * so stands alone.
 */
function callIdOf(params: { value: ToolCallValue; activityId: string }): string {
...
function conversationSpans(params: {
  ...
  const spanEndMs = calls.reduce((latest, call) => Math.max(latest, call.endMs), conversationEndMs);
