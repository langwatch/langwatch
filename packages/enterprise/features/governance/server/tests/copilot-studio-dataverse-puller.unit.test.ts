...
import { GovernanceHttpPort } from "../src/ports/governance-http.port";
...
function evaluateFilter(filter: string, row: Record<string, unknown>): boolean {
  ...
  if (at !== tokens.length) throw new Error(`test bug: unread filter ${filter}`);
  return matched;
}
...
function byCursorOrder(a: Record<string, unknown>, b: Record<string, unknown>): number {
  const byTime = Date.parse(String(a.createdon)) - Date.parse(String(b.createdon));
  if (byTime !== 0) return byTime;
  return stringOrder(String(a.conversationtranscriptid), String(b.conversationtranscriptid));
}

function serveTranscriptPage(url: string, table: TranscriptTable): Record<string, unknown> {
...
async function newAdapter() {
  const { CopilotStudioDataversePuller } =
    await import("../src/adapters/copilot-studio-dataverse-puller.adapter");
...
class StubHttp extends GovernanceHttpPort {
  async fetch(url: string, init: RequestInit & { followRedirects?: boolean }): Promise<Response> {
    ...
    if (transcripts && url.includes("/conversationtranscripts")) {
      return new Response(JSON.stringify(serveTranscriptPage(url, transcripts)), {
        status: 200,
...
function transcriptCall() {
  const call = capturedCalls.find((c) => c.url.includes("/conversationtranscripts"));
...
describe("given a run against an environment holding one conversation", () => {
  ...
  it("names a conversation from its agent lookup and never from its metadata", async () => {
    ...
    queueSignInAndBots([botRow(), botRow({ botid: "dacfd251-bot", name: "wrong-agent" })]);
    responseQueue.push({
      status: 200,
      body: {
        value: [
          ...
          transcriptRow({
            ...
            _bot_conversationtranscriptid_value: "00000000-0000-4000-8000-00000000dead",
          }),
        ],
      },
    });
