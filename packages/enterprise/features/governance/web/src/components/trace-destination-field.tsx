...
import { routesConversations, type SourceType } from "../ingestion-source-catalog";
...
function ArchivedDestinationNotice() {
  return (
    <HStack gap={1.5} alignItems="center">
      <Text fontSize="xs" color="fg.muted" data-testid="ingestion-trace-destination-archived">
        This source&rsquo;s destination project has been archived, so its conversations are no
        longer being routed anywhere. Restore that project, or pick another destination below, to
        start again.
      </Text>
  ...
  return (
    <VStack align="start" width="full" gap={1.5} data-testid="ingestion-trace-destination">
      ...
      {value === null && !destinationArchived && (
        <Text fontSize="xs" color="fg.muted" data-testid="ingestion-trace-destination-empty">
          This source&rsquo;s conversations will not be readable in the trace explorer until you
