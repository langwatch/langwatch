/**
 * The frame every `/governance/*` page renders inside.
 *
 * `platform/app`'s `GovernanceLayout` wrapped `SectionNavigationLayout`, which
 ...
 * host rather than through a flag hook.
 *
 * KNOWN GAP, and the reason this file says so out loud: the outer
 * `DashboardLayout` does not come with it. A `/governance` page served from
...
import { governanceNavItems } from "../../model/governance-nav-items";
import { Link } from "../elements/governance-link";

const SECTION_LABEL = "AI Governance";

export default function GovernanceLayout({ children }: PropsWithChildren<{ pageTitle?: string }>) {
  const host = useGovernanceHost();
  const visibleItems = governanceNavItems.filter(
