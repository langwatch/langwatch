...
/**
 * `buildCreateInput` names the offending field when a pull config will not
 * build, and takes the toaster to name it with rather than reaching for one.
 * None of the composers below is meant to fail, so the recorder is here to
 * satisfy the signature — and to make a surprise refusal readable.
 */
const createInputFor = (composer: ComposerState) =>
  buildCreateInput({
    composer,
...
describe("given the Genie composer field definitions", () => {
  describe("when the form lays out its fields", () => {
    ...
    it("keeps the primary trio out of the Advanced group", () => {
      for (const key of ["workspaceUrl", "credentialsClientId", "credentialsClientSecret"]) {
...
describe("given the create input for a pull-mode source", () => {
  ...
  describe("when the admin picked an hourly cadence", () => {
    ...
    it("carries that schedule everywhere the schedule travels", () => {
      const input = createInputFor(genieComposer({ pullSchedule: "0 * * * *" }));
      ...
      expect((input?.pullConfig as { schedule?: string } | null)?.schedule).toBe("0 * * * *");
    });
  });
  ...
  describe("when a trace destination was picked", () => {
    ...
    it("carries it on the create request", () => {
      const input = createInputFor(genieComposer({ traceProjectId: "proj_analytics" }));
