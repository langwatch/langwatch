 ...
 * @integration
 * Reads the real server (the static unit test only reads migration files),
 * catching schema drift the files don't show. ClickHouse 26+ rejects a
 * rollup column without a merge rule at CREATE TABLE time.
 */
