Feature: Per-tenant soft cap on in-flight dispatch
  ...
  The static cap is the protective fallback for fair dispatch and an explicit
  operator clamp. Groups above it are parked so the dispatcher can continue
  scanning work for other tenants.
