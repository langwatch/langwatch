...
type TestPayload = {
  ...
  // group-queue.gq2.integration.test.ts). Optional so plain sends still work.
...
// > the 256 KiB s3 threshold once gzipped (see group-queue.gq2.integration.test.ts).
const OFFLOADABLE_S3_VALUE = () => incompressible(768 * 1024);

describe("GroupQueueProcessor — decode-drop durability (#5538)", () => {
  let redis: Redis;
  ...
  afterEach(async () => {
    ...
    // shared Redis (see group-queue.gq2.integration.test.ts).
  ...
  describe("given a group carrying a recorded error from a previous failure", () => {
    describe("when a worker claims the group and the staged job is dropped on decode failure", () => {
      ...
      it("leaves the group's recorded error intact", async () => {
        ...
        // one occurred.
        expect(await storedErrorMessage(name, groupId)).toBe("boom from a previous failure");
      });
    });
  });
