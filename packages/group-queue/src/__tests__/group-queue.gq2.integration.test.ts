...
describe("GroupQueueProcessor — GQ2 offload", () => {
  let redis: Redis;
      ...
      it("keys the blob by tenant namespace and content hash", async () => {
        ...
        await vi.waitFor(
          async () => {
            ...
            expect(keys[0]!.slice(prefix.length)).toMatch(/^[A-Za-z0-9_-]{22}$/);
          },
          { timeout: 5000, interval: 50 },
        );
      });
    ...
    describe("when they are staged", () => {
      it("stores one blob and records one lease per job", async () => {
        ...
        await vi.waitFor(async () => {
          ...
          expect(await redis.zcard(`${name}:gq:group:${TENANT_GROUP}:jobs`)).toBe(3);
        });
      });
    });
  });
