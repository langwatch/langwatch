...
// The list the installer uses, read from the installer, so a tool added to one
// and forgotten in the other cannot read as covered.
import { TOOLS } from "../../../dev/scripts/install-check-shims.mjs";
  ...
  describe("given the limit is already taken", () => {
    ...
    it("makes a lint wait for a typecheck, and wires every check through the queue", async () => {
      const scriptEnv = (script: string) => ({
        npm_package_name: "@langwatch/platform-api",
      ...
      for (const name of ["typecheck", "typecheck:all", "lint", "lint:fix", "format"]) {
        ...
        expect(Object.keys(scripts), `${name} is not a root script any more`).toContain(name);
        const script = scripts[name] ?? "";
        const reachesTheQueue =
          script.includes("check-queue.mjs") ||
          // A delegation to another root script or another workspace package is
          // followed by the tool the leaf runs, and every leaf ends at one of
          // these binaries.
          script.includes("pnpm ") ||
          [...queuedTools].some((tool) => new RegExp(String.raw`(^|\s)${tool}(\s|$)`).test(script));
        expect(reachesTheQueue, `${name} reaches no queued tool`).toBe(true);
      }
    ...
    it("repeats its position and names the holder while it waits", async () => {
      ...
      expect(second.stderr).toMatch(/Active: @langwatch\/platform-api typecheck \(.+\) for \d+s/);
    });
