...
// The workspace-wide rules span two files since the root config became a thin
// entry point: the langwatch/* rules live beside the plugin that implements
// them, and the handful of stock rules the repository turns on workspace-wide
// stay at the root. A rule declared in either is declared.
...
/** The top-level `rules` blocks of the oxlint configs, as raw lines. */
function workspaceRuleLines() {
  return OXLINT_CONFIGS.flatMap((config) => {
...
function specRecordFor(id, blocks) {
  ...
  if (policy && existsSync(join(packageRoot, policy.spec))) {
    return `packages/architecture-enforcer/${policy.spec}`;
  }
