...
describe("when the finding is already baselined", () => {
  /** @scenario "A finding matching a live baseline row is not reported" */
  it("does not report a leak the baseline already names", () => {
    writePackage("modules/trace/contract", "@langwatch/trace-contract");
    write(
      "modules/trace/contract/src/index.ts",
      'import crypto from "node:crypto";\nexport { crypto };\n',
    ...
    write(
      "packages/architecture-enforcer/src/browser-node-leak-baseline.json",
      JSON.stringify({
        version: 1,
        policy: "browser-node-leak",
        entries: [
          { key: "modules/trace/contract/src/index.ts|node:crypto", measured: "2026-09-18" },
        ],
      }),
    );
  ...
  it("reports a baseline row nothing still matches as stale", () => {
    writePackage("modules/trace/contract", "@langwatch/trace-contract");
    write("modules/trace/contract/src/index.ts", "export const clean = true;\n");
    write(
      "packages/architecture-enforcer/src/browser-node-leak-baseline.json",
      JSON.stringify({
        version: 1,
