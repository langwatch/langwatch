...
| **Passport** | **Designed, not built** (stage F). A signed, short-TTL (≤60s), epoch-bound token for stateless surfaces (Go gateway, collectors). Two postures, and only the first carries an answer: in **carry** mode it holds per-scope permission bitsets, so a surface with no database verifies with an HMAC check and an epoch compare; in **assert** mode it carries signed identity claims only and the verifier resolves the question itself. A surface that CAN answer live is meant to refuse a carried answer (`specs/rbac/authz-passports.feature`, "A carried answer is refused where a live answer is available"). No code mints or checks one today; the word appears here so the constraints it imposes on the registry are not lost. |
