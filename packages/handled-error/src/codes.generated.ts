...
export const goErrorCodes = {
   ...
   * event on the worker's own stream — e.g. its LLM call was rejected). The
   * worker is alive and answered deterministically, so the control plane must
   * fail the turn immediately with this code instead of the generic
   ...
   * control plane's presentation registry is keyed by it — so renaming it is a
   * breaking change to buy a tidier constant, exactly the trade
