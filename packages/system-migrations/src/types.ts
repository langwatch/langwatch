...
export type MigrationPassSummary = {
   ...
   *  outcome. Never "already done" - that is `alreadyTerminal`. */
  skipped: number;
  /** Finalized or rolled back BEFORE this pass ever touched the tenant.
   *  Split from `skipped` so a targeted run over tenants that are all
   *  already done reads as done, not as "nothing was in the cohort". */
