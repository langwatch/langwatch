...
/**
 * Derives a projection's state version from its zod schema (ADR-105 §4).
 *
 ...
 * promise someone has to remember to keep — today only 8 of them exist across
 * 12 folds, so most shape changes have never bumped anything. Deriving the
 * stamp from the schema itself removes the step a person can forget: change a
 * field's type and the hash moves with it, with no separate action required.
 *
 ...
 * one explicitly (ADR-105 §4). Two schemas that differ only in key order or
 * in a `.describe()` call hash identically; any difference in a field's type,
 * optionality, or nesting changes the hash.
 */
 ...
 * (ADR-105 §4). But the hash is still computed and returned alongside the
 * pin, because a pin that silently absorbed every future shape change would
 * recreate the exact failure derivation was built to prevent: a stale row
 * decoding into the current shape's meaning.
 */
