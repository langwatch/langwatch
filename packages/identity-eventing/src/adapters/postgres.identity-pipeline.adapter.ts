...
import {
  ...
  type PrismaIdentityProjectionDatabase,
} from "../repositories/prisma/prisma.identity-projection.repository";
import {
  PrismaMfaEnrollmentProjectionRepository,
  type PrismaMfaEnrollmentProjectionDatabase,
} from "../repositories/prisma/prisma.mfa-enrollment-projection.repository";

/** Every model the identity ledger reads or writes, and no other. */
export type IdentityPipelineDatabase = IdentityGuardsDatabase &
  PrismaIdentityProjectionDatabase &
  PrismaMfaEnrollmentProjectionDatabase;
...
export class PostgresIdentityPipelineAdapter {
  ...
  build(): IdentityPipeline {
    ...
    return createIdentityPipeline({
      identityProjectionStore: PrismaIdentityProjectionRepository.create(database, reservations),
      identityGuards,
      mfaProjectionStore: PrismaMfaEnrollmentProjectionRepository.create(database),
      mfaGuards,
    });
  }
}
