...
import type { JoinRequestPipeline } from "../../join-requests/pipeline";
...
describe("given a composed join-request pipeline", () => {
  describe("when a guard reads a request's state", () => {
    ...
    it("reads the rows the fold writes, through one repository", async () => {
      const { pipeline, findUnique, findFirst } = compose();
      ...
      // And through the SAME object the fold writes with. Two repositories
      // over one table would still compile and still route every key.
      const guards = (withdraw!.handlerInstance as unknown as { guards: { requests: unknown } })
        .guards;
      expect(guards.requests).toBe(requestStore(pipeline));
    });
  });
});
