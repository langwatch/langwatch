/**
 * The support inbox, composed as its own feature. `bugReports.*` — the reports `langwatch
 * report` and the MCP tool file, read by the back office.
 */
...
/**
 * The support inbox on a process that composed no database. The namespace still mounts
 * and both reads refuse by name, so the back office says the deployment cannot answer
 * rather than rendering an empty inbox that reads as "nobody has reported anything".
 */
