 ...
 * per request rather than at module scope.
 *
 * Every route shares the process-composed endpoint store instead of minting
 * its own, and the deployment's ClickHouse configuration is read at request
 * time, not once at import time.
 */
export interface WebhookRestServices {
   ...
   * the log has no fallback store — which the two routes that read it report
   * as a plain "not configured" refusal.
   */
...
/**
 * Resolved per call rather than at module scope, so the ClickHouse-backed
 * events log is read at request time. Undefined on a deployment without
 * ClickHouse, which this reports the same way the old inline resolver did: a
 * plain "not configured" refusal.
 */
function requireEventsService(services: WebhookRestServices): WebhookEventsService {
  const service = services.events;
...
export function createWebhookRestApp(options: {
  ...
  const requireWebhookPlan = async (c: Context, next: Next): Promise<void> => {
    ...
    try {
      await webhooks().assertEndpointsEntitled(organization.id);
    } catch (error) {
  ...
  secured.access(requires("webhookEndpoints:view")).get(
    ...
    async (c) => {
      ...
      // without an error.
      const page = await requireEventsService(webhooks()).getEmittedEvents({
        organizationId: organization.id,
        fromMs: query.from,
        toMs: query.to,
  ...
  secured.access(requires("webhookEndpoints:view")).get(
    ...
    async (c) => {
      const organization = c.get("organization");
      const event = await requireEventsService(webhooks()).tryGetEmittedEventById({
        organizationId: organization.id,
        id: c.req.param("id"),
      });
