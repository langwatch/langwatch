 ...
 * object IS.
 *
 * The stored-object services, the dual-auth verifier, the permission check and
 * the rate limiter all arrive as arguments. Each of the last three reads the
 * application's own database or Redis, which is why they are ports rather than
 * imports.
 */
...
export function createFilesRestApp<E extends Env = { Variables: FilesDualAuthVariables }>(options: {
  ...
  async function handleFileRead(
    ...
    if (projectIdFromUrl) {
      ...
      try {
        owner = await storedObjectOwners().resolve({ id });
      } catch (err) {
    ...
    let result;
    try {
      result = await storedObjects().getById({
