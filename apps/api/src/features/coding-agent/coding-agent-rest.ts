 ...
 * which of those they may price, named as the usage read names them.
 *
 * Derived from the service's own input rather than restated, so the two
 * cannot drift into disagreement about what a scope is.
 */
export type CodingAgentCallerScope = Omit<
  CodingAgentPullRequestUsageInput,
  "organizationId" | "repositoryHost" | "repositoryFullName" | "prNumber"
>;

/** What this family dispatches through, resolved per request. */
export interface CodingAgentRestServices {
  codingAgents: CodingAgentService;
  /** The GitHub web origin this instance is bound to, e.g. `https://github.com`. */
  githubWebBase(): string;
  /** The organization owning a project, or undefined for an orphan project. */
...
export function createCodingAgentRestApp(options: {
  ...
  const { security, codingAgents } = options;
  ...
  secured.access(requires("traces:view")).get(
    ...
    async (c) => {
      ...
      const { events, nextCursor } = await codingAgents().codingAgents.getSessionEvents({
  ...
  secured.access(requires("traces:view")).get(
    ...
    async (c) => {
      ...
      const query = usageQuerySchema.safeParse({
        ...
        host: c.req.query("host") ?? new URL(services.githubWebBase()).hostname,
      });
      if (!query.success) throw ValidationError.fromZodError(query.error);

      const organizationId = await services.resolveOrganizationId(project.id);
      if (!organizationId) {
      ...
      // resolved to nobody.
      const scope = await services.resolveCallerProjectScope({
        userId: callerUserId,
        organizationId,
      });

      const usage = await services.codingAgents.getPullRequestUsage({
        organizationId,
