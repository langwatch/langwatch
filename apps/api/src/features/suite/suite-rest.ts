 ...
 *
 * Every capability arrives as a per-request provider rather than off the Hono
 * context, so the family can be mounted into any process that has them and
 * built with none by the OpenAPI generator.
 */
export function createSuiteRestApp(options: {
  ...
  const { security, suites, scenarios, projects, platformUrl } = options;
  ...
  secured.access(requires("scenarios:view")).get(
    ...
    async (c) => {
      ...
      const listed =
        kind === "folder"
          ? (await scenarios().listFolders({ projectId: project.id })).map(toFolderResponse)
  ...
  secured.access(requires("scenarios:view")).get(
    ...
    async (c) => {
      ...
      try {
        suite = await suites().get({ id, projectId: project.id });
      } catch (error) {
        if (!(error instanceof SuiteNotFoundError)) throw error;
      }

      if (!suite) {
        const folder = await scenarios().tryGetFolder({
          folderId: id,
  ...
  secured.access(requires("scenarios:update")).patch(
    ...
    async (c) => {
      ...
      if (folder) {
        ...
        const updatedFolder = await scenarios().updateFolder({
          ...folderInput,
          folderId: id,
          projectId: project.id,
        });
  ...
  secured.access(requires("scenarios:create")).post(
    ...
    async (c) => {
      ...
      const projectWithTeam = await projects().tryGetWithTeam(project.id);
      if (!projectWithTeam) {
