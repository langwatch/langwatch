/**
 * Deletes the studio's quiet per-project NLP Lambda functions and the log groups
 * left behind when one is already gone. Same 7-day function cutoff, 365-day log
 * cutoff and `langwatch_nlp-` prefix as before; the cron family is its caller.
 */
import {
...
export async function cleanupOldLambdas(): Promise<void> {
  ...
  try {
    ...
    if (allFunctions.length === 0) {
      ...
      for (const func of nlpLambdas) {
        ...
        if (!lastEventTime) {
          logger.warn(`No last event time found for ${func.FunctionName}, skipping deletion`);
          continue;
        }

        logger.info(`Last event time for ${func.FunctionName}: ${lastEventTime.toISOString()}`);

        // Delete Lambda function if older than 7 days
        if (lastEventTime < sevenDaysAgo) {
          logger.info(`Function ${func.FunctionName} is older than 7 days, deleting Lambda...`);
        ...
        if (lastEventTime < oneYearAgo) {
          ...
          logger.info(`Log group for ${func.FunctionName} is recent enough, keeping logs`);
        }
      }
    }
    ...
    if (!logGroupsResponse.logGroups) {
      ...
      for (const logGroup of logGroupsResponse.logGroups) {
        ...
        logger.info(`Lambda function ${functionName} no longer exists, checking log group age`);
        ...
        if (lastEventTime < oneYearAgo) {
          ...
          logger.info(`Orphaned log group ${logGroup.logGroupName} is recent enough, keeping it`);
        }
      }
    }
