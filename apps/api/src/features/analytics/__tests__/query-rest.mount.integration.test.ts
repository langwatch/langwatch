/**
 * The query REST family, driven through the Hono app the API process mounts.
 *
 * NOTE (langwatch-ql lane, 2026-09-10): split out of the combined
 * langwatch-ql-rest.integration.test.ts, unchanged. The `query` door is a
 * separate baseline row (`analytics|legacy-transport-runtime` for `query`)
 * and out of this lane's scope; `../query-rest.mount.ts` still imports the
 * removed `createQueryRestApp` / `AppRestSecurity` names and needs the same
 * `runtime.mount` conversion `../langwatch-ql-rest.mount.ts` just got.
 */
