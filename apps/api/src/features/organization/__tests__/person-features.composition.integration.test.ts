...
const rateLimit = async () => ({ allowed: true, resetAt: Date.now() + 60_000 });
...
function composeApplication(
  ...
  const features = ApiTrpcFeaturesComposition.tryCompose({
    ...
    collaborators: stubCollaborators({ users: user.app, organizations: organization.app }),
  });
...
function composeSeatLicence(options: {
  ...
  return { organizations, prisma };
}

describe("given an API process composed with its person-shaped features", () => {
