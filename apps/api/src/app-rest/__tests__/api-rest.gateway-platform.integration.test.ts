...
async function expectCanonicalError(
  ...
  const parsed = apiErrorSchema.strict().safeParse(await json(res));
  if (!parsed.success) {
...
const app = {
  request: (path: string, init?: RequestInit) => harness!.request(path, init),
};

/** The response document, as the wire publishes it. */
...
function ch(): ClickHouseClient {
  if (!harness) throw new Error("test ClickHouse client not available");
  return harness.clickhouse;
}
...
async function seedUserWithRole(args: {
  ...
  await prisma.roleBinding.create({
    data: {
      id: generate("rolebinding").toString(),
      organizationId: args.orgId,
      userId: args.userId,
  ...
  const created = await harness!.apiKeys.create({
...
describe.skipIf(!databaseUrl || !clickHouseUrl)(
  "gateway platform REST API (real PG + real CH)",
  () => {
    beforeAll(async () => {
      await seedTenant({
        orgId: ORG_ID,
    ...
    describe("virtual key lifecycle", () => {
      ...
      it("revokes idempotently and retires the key's budget", async () => {
        ...
        expect((await json(first)).virtual_key.status).toBe("revoked");

        const second = await post(`/api/gateway/v1/virtual-keys/${id}/revoke`, {}, legacyAuth());
        expect(second.status).toBe(200);
        expect((await json(second)).virtual_key.status).toBe("revoked");
    ...
    describe("budgets", () => {
      ...
      it("serves a single budget in the list row shape", async () => {
        ...
        const id = (await json(created)).budget.id;
        ...
        const listed = (await json(list)).data.find((b: any) => b.id === id);
      ...
      it("echoes a cycle anchor and reports the anchored period, not the calendar one", async () => {
        ...
        expect(created.status).toBe(201);
        const budget = (await json(created)).budget;
        ...
        expect((await json(read)).budget).toEqual(budget);
        ...
        const after = (await json(patched)).budget;
      ...
      it("refuses a cycle anchor on the windows that never roll", async () => {
        ...
        expect((await json(ok)).budget.cycle_anchor_at).toBeNull();
      });
      ...
      it("labels a per-person template with end_users_seen and end_users_over", async () => {
        ...
        expect(createRes.status).toBe(201);
        const budgetId = (await json(createRes)).budget.id;
        ...
        const chRepo = harness!.budgetSpend;
      ...
      it("creates and echoes a provider-filtered budget", async () => {
        ...
        expect(res.status).toBe(201);
        expect((await json(res)).budget.provider_key).toBe(MP_OPENAI_ID);
      ...
      it("updates and archives a budget", async () => {
        ...
        const id = (await json(createRes)).budget.id;
        ...
        expect((await json(deleteRes)).budget.archived_at).not.toBeNull();
      });
    });
    ...
    describe("cursor pagination on the unbounded lists", () => {
      ...
      it("pages budgets without skipping or repeating a row", async () => {
        ...
        const all = (await json(oneShot)).data.map((b: any) => b.id);
    ...
    describe("virtual key spend read", () => {
      ...
      it("takes epoch-ms and echoes a window it would accept back", async () => {
        ...
        expect(res.status).toBe(200);
        expect((await json(res)).window).toEqual({ from, to });
    ...
    describe("external_id and metadata", () => {
      ...
      it("replaces metadata on patch and clears external_id with null", async () => {
        ...
        expect((await json(replaced)).virtual_key.metadata).toEqual({
          keep: "yes",
        });
        ...
        expect((await json(cleared)).virtual_key.external_id).toBeNull();
      ...
      it("round-trips external_id and metadata on a budget, and conflicts", async () => {
        ...
        expect((await json(byId)).budget.external_id).toBe(externalId);
        ...
        expect((await json(duplicate)).error.code).toBe("external_id_conflict");
      });
    ...
    describe("when a create carries an Idempotency-Key", () => {
      ...
      it("fences the replaced request out of the receipt", async () => {
        ...
        expect(harness!.encryption.decrypt(settled!.responseBody!)).toBe(
