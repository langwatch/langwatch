...
} from "./api-browser-session.ts";
import { canonicalErrorResponse } from "./api-canonical-error.ts";
...
export class ApiRestHost implements FeatureRestHost<MountableRestApp> {
  ...
  private bearerDoor(options: {
    ...
    const admit = (request: Request): RestCaller => {
      // Three answers, not two: no secret means this deployment does not run
      // the door and it answers as though unmounted; a blank one means it
      // meant to and mis-set the key, which is its mistake to hear about.
      if (secret === undefined) throw new ApiRestDoorUnconfiguredError(door);
      const configured = secret.trim();
      if (configured === "") throw new ApiRestDoorBlankSecretError(door);
