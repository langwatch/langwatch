...
import {
  ApiEnterpriseAuditAbsenceReport,
...
function recordingPrisma(): { rows: RecordedRow[]; client: PrismaClient } {
  const rows: RecordedRow[] = [];
  const client = {
    auditLog: {
      create: async ({ data }: { data: RecordedRow }) => {
        rows.push(data);
        return data;
      },
    },
...
class RecordingAbsence extends ApiEnterpriseAuditAbsenceReport {
...
describe("given the API process composed its own audit trail", () => {
  describe("when a completed mutation is recorded", () => {
    ...
    it("writes one row carrying the actor, the action and the arguments", async () => {
      const { rows, client } = recordingPrisma();
      const audit = composeApiEnterpriseAudit({ prisma: () => client });
  ...
  describe("when the connection is opened after the trail is composed", () => {
    ...
    it("records through the connection the process opened later", async () => {
      const { rows, client } = recordingPrisma();
      let opened: PrismaClient | undefined;
      const audit = composeApiEnterpriseAudit({ prisma: () => opened });
  ...
  describe("when the process opened no database", () => {
    ...
    it("names the absent collaborator instead of failing the call it was recording", async () => {
      ...
      const audit = composeApiEnterpriseAudit({ prisma: () => undefined, report });

      await expect(
        audit.record({ actorId: "user_1", path: "project.update", input: {}, error: null }),
      ).resolves.toBeUndefined();
      expect(report.reasons).toEqual(["database connection"]);
    });
  });
});
