/**
 * The API process's packaged tRPC record, composed. `createAppTrpcFeatures`
 * builds every converted namespace from one mount, the shared infrastructure
 * and the features this process composed; the rest are named one by one.
 */
...
export class ApiTrpcFeaturesComposition extends ApiTrpcFeatures {
  ...
  static tryCompose(
    ...
    // Named one by one, and only once the record itself is composed: an
    // absence list beside a record nothing mounted would read as the reason
    // the record is missing rather than as the queue that it is.
    for (const entry of ABSENT_API_TRPC_NAMESPACES) {
      options.report?.absent("unconverted-namespace", {
        namespace: entry.namespace,
        reason: absentNamespaceReason(entry),
      });
    }
...
/** Writes each namespace this process does not serve to the log, once. */
export class LoggedApiTrpcFeaturesAbsence extends ApiTrpcCollaboratorsAbsence {
  ...
  absent(
    ...
    if (namespace) {
      this.logger.warn({ namespace: namespace.namespace }, namespace.reason);
      return;
    }
