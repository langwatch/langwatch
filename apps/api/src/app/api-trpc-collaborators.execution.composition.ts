 ...
 *
 *   - `runEvaluationForTrace` is SATISFIED where the process composed an
 *     evaluator runtime (`api-evaluator-execution.composition.ts`), which is
 *     the same engine the gateway's guardrail check and the legacy evaluate
 *     doors run on. It arrives as a call-time resolution rather than a value
 *     because that runtime is built FROM this half's evaluator service and the
 *     observability half's trace reads — one composed on either side of it.
 *     Absent, a re-score still refuses by name rather than scoring against an
 *     empty trace.
 ...
 */
export class ExecutionCapabilityUnavailableError extends HandledError {
