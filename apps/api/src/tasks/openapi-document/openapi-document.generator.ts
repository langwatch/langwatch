/**
 * The OpenAPI document's producer: reads installed module declarations and
 * writes what they publish. The document itself is FROZEN — three routes
 * serve it, both SDKs generate clients from it, and this task never writes it.
 */
...
/**
 * Schemas the prompt and API-key families reference by a literal `$ref`
 * rather than through a `resolver()`-wrapped zod schema. Hand-maintained
 * here because nothing generates them.
 */
 ...
 * The output path is a REQUIRED decision, not a default pointing at the
 * artifact — this task cannot overwrite the frozen document by omission.
 */
...
/**
 * Stamps every operation with the security requirement and access policy its
 * declaration enforces, and DELETES the ones no requirement can express. A
 * top-level default is a claim about every operation that doesn't override it.
 */
 ...
 * `$ref` that already points at the shared schema — dangling until this
 * hoists the `$defs` entry there. Two `$defs` wanting the same name throws.
 */
 ...
 * and the patch script both name this shape `JsonValue` literally, so an
 * unstable counter would break both on regeneration. Two in one run throws.
 */
...
/**
 * A media object naming a type with no schema is what `withRawBody`/
 * `withRawResponse` leave behind. A response drops it; a request body keeps
 * its type but drops the default `required: true` — unstated shape can't be required.
 */
