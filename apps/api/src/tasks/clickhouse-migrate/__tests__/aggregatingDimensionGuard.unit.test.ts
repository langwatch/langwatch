...
function splitTopLevel({ text, separator }: { text: string; separator: string }): string[] {
  const parts: string[] = [];
  let depth = 0;
  let current = "";
...
function parseSortingKey(statement: string): string[] {
  ...
  if (!orderBy) return [];
  return [...orderBy[1]!.matchAll(/`?([A-Za-z_][A-Za-z0-9_]*)`?/g)].map((match) => match[1]!);
}
...
function collectAggregatingTables(): AggregatingTable[] {
  ...
  for (const file of files) {
    ...
    for (const statement of sql.split(";")) {
      ...
      const create = statement.match(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(\S+)\s*\(/i);
      if (!create) continue;
      const openIndex = statement.indexOf("(", create.index! + create[0].length - 1);
      tables.push({
        file,
...
function underlyingType(type: string): string {
  return type.split(/\s+(?:DEFAULT|CODEC|MATERIALIZED|ALIAS|TTL)\b/i)[0]!.trim();
}
...
function isHistorical({
  ...
  return HISTORICAL_DIMENSIONS.some(
    (entry) => entry.file === file && entry.table === table && entry.column === column,
  );
}
...
describe("given the ClickHouse migrations that create AggregatingMergeTree tables", () => {
  ...
  describe("when the create statements are parsed", () => {
    it("finds the rollup tables the migrations create", () => {
      ...
      for (const table of tables) {
        expect(table.columns.length, `${table.file}: ${table.table}`).toBeGreaterThan(0);
        expect(table.sortingKey.length, `${table.file}: ${table.table}`).toBeGreaterThan(0);
      }
    });
  });
  ...
  describe("when a column is neither in the sorting key nor an aggregate state", () => {
    ...
    it("keeps the historical list pointing at real declarations", () => {
      ...
      for (const entry of HISTORICAL_DIMENSIONS) {
        const table = tables.find((t) => t.file === entry.file && t.table === entry.table);
        expect(
          table,
  ...
  describe("when the converge migration is read", () => {
    const upSection = stripLineComments(
      readFileSync(resolve(MIGRATIONS_DIR, CONVERGE_MIGRATION), "utf8").split("-- +goose Down")[0]!,
    );
    ...
    it("modifies every historical column to merge by max on its own type", () => {
      for (const { table, column } of CONVERGED_COLUMNS) {
        ...
        expect(declared, `${table}.${column} is not created by any migration`).toBeDefined();
  ...
  describe("when the runner decides which migrations to relax the check for", () => {
    ...
    it("relaxes it up to the migration before the converge migration", () => {
      const goose = readFileSync(
        resolve(import.meta.dirname, "../goose.migration-runner.ts"),
        "utf8",
      );
      const declared = goose.match(/const LAST_MIGRATION_NEEDING_DIMENSION_COMPAT = (\d+);/);
      expect(
        declared,
        "goose.migration-runner.ts no longer declares LAST_MIGRATION_NEEDING_DIMENSION_COMPAT",
      ).not.toBeNull();
