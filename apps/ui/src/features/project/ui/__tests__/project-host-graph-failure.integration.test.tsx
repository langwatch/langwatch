/**
 * `ProjectHost` read the organization graph without checking for a refusal, so a failed `organization.getAll` left the settings screen with no organization and no error — same gap `TraceHost` and `OrganizationHost` had.
...
vi.mock("@langwatch/project-web/screens/project", async () => {
  const actual = await vi.importActual<typeof import("@langwatch/project-web/screens/project")>(
    ...
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/project-1/settings"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <ProjectHost>
            <div>the project settings screen</div>
          </ProjectHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the project settings shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the project settings screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the project settings shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the project settings screen")).toBeTruthy();
    });
  });
});
