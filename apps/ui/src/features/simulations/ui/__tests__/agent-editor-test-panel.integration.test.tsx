/**
 * Saved HTTP and code agents expose a test panel that sends one turn; drafts do not.
    ...
    agents: {
      getById: {
        useQuery: (args: { id: string }) => {
          const agents = [httpAgent, codeAgent];
          return { data: agents.find((agent) => agent.id === args.id), isLoading: false };
        },
      },
...
describe("the Test agent panel of the editor drawers", () => {
  ...
  describe("given a saved HTTP agent", () => {
    ...
    it("shows the panel below the form and sends one turn from it", async () => {
      ...
      const { AgentHttpEditorDrawer } = await import("../sections/agent-http-drawer.tsx");
  ...
  describe("given a saved code agent", () => {
    it("shows the panel below the form", async () => {
      ...
      const { AgentCodeEditorDrawer } = await import("../sections/agent-code-drawer.tsx");
...
vi.mock("@langwatch/ui-host/capabilities", async (importOriginal) => ({
  ...
  useUiCapabilities: () => ({
    session: { activeScope: () => ({ projectId: "project_1" }) },
