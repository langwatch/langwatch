 ...
 *
 * `ApiKeyHost` read the organization graph without checking for a refusal,
 * so a failed `organization.getAll` left the API key screens with no scope
...
vi.mock("../../../../behavior/ui-address", () => ({
  useUiAddress: () => "/project-1/settings/api-keys",
}));
...
import { ApiKeyHost } from "../sections/api-key-host";
...
function mountApiKeys() {
  ...
  render(
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/project-1/settings/api-keys"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <ApiKeyHost>
            <div>the api keys screen</div>
          </ApiKeyHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the api key shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the api keys screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the api key shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the api keys screen")).toBeTruthy();
    });
  });
});
