 ...
 *
 * `MonitorHost` read the organization graph without checking for a refusal,
 * so a failed `organization.getAll` left the online evaluations screen with
...
import { MonitorHost } from "../sections/monitor-host";
...
function mountMonitors() {
  ...
  render(
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/project-1/evaluations"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <MonitorHost>
            <div>the online evaluations screen</div>
          </MonitorHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the monitor shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the online evaluations screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the monitor shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the online evaluations screen")).toBeTruthy();
    });
  });
});
