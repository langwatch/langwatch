 ...
 *
 * `AuthorizeHost` read the organization graph without checking for a
 * refusal, so a failed `organization.getAll` left the hand-off screens with
 * no project and no error — same gap `TraceHost` and `OrganizationHost` had.
 *
...
vi.mock("../../../../behavior/ui-address", () => ({
  useUiAddress: () => "/project-1/authorize",
}));
...
import { AuthorizeHost } from "../sections/authorize-host";
...
function mountAuthorize() {
  ...
  render(
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/project-1/authorize"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <AuthorizeHost>
            <div>the authorize screen</div>
          </AuthorizeHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the authorize shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the authorize screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the authorize shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the authorize screen")).toBeTruthy();
    });
  });
});
