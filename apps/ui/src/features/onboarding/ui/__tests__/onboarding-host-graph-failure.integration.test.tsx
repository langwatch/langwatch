...
vi.mock("@langwatch/onboarding-web/onboarding", async () => {
  const actual = await vi.importActual<
    typeof import("@langwatch/onboarding-web/onboarding")
  >("@langwatch/onboarding-web/onboarding");
  return {
    ...actual,
    onboardingApi: {
      organization: {
...
vi.mock("../../../../behavior/ui-address", () => ({
  useUiAddress: () => "/onboarding/welcome",
}));

vi.mock("../../../../behavior/ui-reduced-motion", () => ({
  useUiPrefersReducedMotion: () => false,
}));
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the onboarding shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the onboarding screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the onboarding shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the onboarding screen")).toBeTruthy();
    });
  });
});
