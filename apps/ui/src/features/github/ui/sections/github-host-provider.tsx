...
import { GithubHostProvider } from "@langwatch/github-web/screens/integrations";
import { useMemo, type ComponentType, type ReactNode } from "react";

import { useUiCapabilities } from "../../../../behavior/ui-capabilities";
import { uiLeaveTo, uiOpenExternal } from "../../../../behavior/ui-departure";
import { UiGithubHost } from "../../behavior/github-host.adapter";

function GithubHost({ children }: { children: ReactNode }) {
...
/** Wraps the Integrations screen in the host its package asks for. */
export function withGithubHost<P extends object>(Screen: ComponentType<P>): ComponentType<P> {
  const Mounted = (props: P) => (
    <GithubHost>
      <Screen {...props} />
    </GithubHost>
  );
  Mounted.displayName = `withGithubHost(${Screen.displayName ?? Screen.name ?? "Screen"})`;
  return Mounted;
}
