...
import { evaluatorApi, EvaluatorHostProvider } from "@langwatch/evaluator-web/screens/evaluators";
import { useMemo, type ComponentType, type ReactNode } from "react";

import { useUiCapabilities } from "../../../../behavior/ui-capabilities";
import { uiCopyTargets } from "../../../../model/ui-copy-targets";
import { EVALUATOR_COPY_PERMISSION, UiEvaluatorHost } from "../../behavior/evaluator-host.adapter";

function EvaluatorHost({ children }: { children: ReactNode }) {
...
/** Wraps the evaluators screen in the host its package asks for. */
export function withEvaluatorHost<P extends object>(Screen: ComponentType<P>): ComponentType<P> {
  const Mounted = (props: P) => (
    <EvaluatorHost>
      <Screen {...props} />
    </EvaluatorHost>
  );
  Mounted.displayName = `withEvaluatorHost(${Screen.displayName ?? Screen.name ?? "Screen"})`;
  return Mounted;
}
