 ...
 *
 * `PromptHost` read the organization graph without checking for a refusal,
 * so a failed `organization.getAll` left Prompt Studio with no project and
 * no error — same gap `TraceHost` and `OrganizationHost` had.
 *
    ...
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/project-1/prompts"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <PromptHost>
            <div>the prompt studio screen</div>
          </PromptHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the prompt studio shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the prompt studio screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the prompt studio shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the prompt studio screen")).toBeTruthy();
    });
  });
});
