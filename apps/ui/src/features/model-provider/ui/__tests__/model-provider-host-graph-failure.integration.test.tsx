/**
 * `ModelProviderHost` read the organization graph without checking for a refusal, so a failed `organization.getAll` left the model provider screens with no scope and no error — same gap `TraceHost` and `OrganizationHost` had.
...
vi.mock("@langwatch/model-provider-web/screens/model-provider", async () => {
  const actual = await vi.importActual<
    typeof import("@langwatch/model-provider-web/screens/model-provider")
  >("@langwatch/model-provider-web/screens/model-provider");
  return {
    ...actual,
    modelProviderApi: {
      organization: {
...
import { ModelProviderHost } from "../sections/model-provider-host";
...
function mountModelProviders() {
  ...
  render(
    <ChakraProvider value={defaultSystem}>
      <MemoryRouter initialEntries={["/settings/model-providers"]}>
        <UiCapabilityContextProvider value={capabilities}>
          <ModelProviderHost>
            <div>the model providers screen</div>
          </ModelProviderHost>
        </UiCapabilityContextProvider>
      </MemoryRouter>
    </ChakraProvider>,
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the model provider shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the model providers screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the model provider shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the model providers screen")).toBeTruthy();
    });
  });
});
