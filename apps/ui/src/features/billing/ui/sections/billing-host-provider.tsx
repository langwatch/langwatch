...
import { UiBillingHost } from "../../behavior/billing-host.adapter";
...
function BillingHost({ children }: { children: ReactNode }) {
...
/** Wraps a billing screen in the host its package asks for. */
export function withBillingHost<P extends object>(Screen: ComponentType<P>): ComponentType<P> {
  const Mounted = (props: P) => (
    <BillingHost>
      <Screen {...props} />
    </BillingHost>
  );
  Mounted.displayName = `withBillingHost(${Screen.displayName ?? Screen.name ?? "Screen"})`;
  return Mounted;
}
