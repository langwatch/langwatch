 ...
 *
 * `BillingHost` read the organization graph without checking for a refusal,
 * so a failed `organization.getAll` left the billing screens with no
 * organization and no error — same gap `TraceHost` and `OrganizationHost`
 * had.
 *
...
describe("given the organization graph refuses for a reason the reader can read", () => {
  describe("when the billing shell renders", () => {
    ...
    it("renders the registered copy instead of hanging on an empty document", () => {
      ...
      expect(screen.queryByText("the billing screen")).toBeNull();
    });
  });
});
...
describe("given the organization graph answered", () => {
  describe("when the billing shell renders", () => {
    it("renders what it is mounted around", () => {
      ...
      expect(departures).toEqual([]);
      expect(screen.getByText("the billing screen")).toBeTruthy();
    });
  });
});
