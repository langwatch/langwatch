...
describe("readHandledError", () => {
  ...
  describe("given a payload from an older server", () => {
    ...
    it("resolves the discriminant from the deprecated kind alias", () => {
      const result = readHandledError(trpcError({ kind: "project_not_found", httpStatus: 404 }));
...
describe("handledShapeFromSerialized", () => {
  ...
  describe("given a docs link on an event payload", () => {
    ...
    it("keeps one on our own docs origin", () => {
      expect(serializedWith("https://docs.langwatch.ai/errors/provider-error").docsUrl).toBe(
        "https://docs.langwatch.ai/errors/provider-error",
      );
    });
...
describe("safeProse", () => {
  ...
  it("passes a sentence under the cap through unchanged", () => {
    expect(safeProse("Your credit balance is too low.")).toBe("Your credit balance is too low.");
  });
...
describe("readAuthoredMessage", () => {
  ...
  describe("given a plain non-5xx TRPCError", () => {
    ...
    it("returns the prose the procedure authored", () => {
      ...
      expect(
        readAuthoredMessage(trpcError(429, "Too many signup attempts. Please try again later.")),
  ...
  describe("given anything the registry or the generic state owns", () => {
    ...
    it("declines a 5xx — its message can carry internals", () => {
      expect(readAuthoredMessage(trpcError(500, "connect ECONNREFUSED 10.0.0.4"))).toBeUndefined();
    });
  ...
  describe("given a 4xx the boundary did not mark as authored", () => {
    ...
    it("declines a message tRPC defaulted to the code NAME", () => {
      ...
      expect(readAuthoredMessage(trpcError(404, "NOT_FOUND", null, false))).toBeUndefined();
    });
    ...
    it("declines a message inherited from a wrapped cause", () => {
      ...
      expect(readAuthoredMessage(trpcError(400, "fetch failed", null, false))).toBeUndefined();
      expect(
...
describe("given the canonical nested error envelope", () => {
  ...
  describe("when the gateway refuses a request", () => {
    ...
    it("keeps the server's own sentence for the registry to fall back on", () => {
      expect(readHandledError(gatewayVertexFailure)?.meta.message).toContain("were not accepted");
    });
  });
  ...
  describe("when the nested object is not ours", () => {
    it("refuses prose in the code slot", () => {
      expect(readHandledError({ error: { code: "Something went badly wrong" } })).toBeNull();
    });
    ...
    it("survives a docs link pointing somewhere that is not our docs", () => {
      ...
      expect(ours?.docsUrl).toBe("https://docs.langwatch.ai/platform/model-providers");
    });
  });
  ...
  describe("when the nested code is slug-shaped but not one of ours", () => {
    ...
    it("keeps reading remediation for a code we do recognise", () => {
      ...
      expect(handled?.docsUrl).toBe("https://docs.langwatch.ai/platform/model-providers");
    });
  });
