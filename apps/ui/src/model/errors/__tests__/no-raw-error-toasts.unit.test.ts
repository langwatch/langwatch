...
const USES_REACT_HOOK_FORM = /FORM_SERVER_ERROR|\buseForm\b|UseFormReturn|react-hook-form/;
...
describe("the raw-message detector", () => {
  describe("given a leak", () => {
    it.each([
      ...
      ["inside a template literal", "toaster.create({ description: `Failed: ${error.message}` })"],
      [
  ...
  describe("given something that only looks like one", () => {
    it.each([
      ...
      ["a map parameter named e", "items.map((e) => toaster.create({ title: `${e.label}` }))"],
      [
