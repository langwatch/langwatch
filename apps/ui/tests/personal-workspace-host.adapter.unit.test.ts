...
function host(
  ...
  return UiPersonalWorkspaceHost.create(
    ...
    },
    {
      ...
      registerPasskey: () => Promise.resolve({ ok: true as const }),
      renamePasskey: () => Promise.resolve({ ok: true as const }),
      removePasskey: () => Promise.resolve({ ok: true as const }),
      linkSignInMethod: () => Promise.resolve({ ok: true as const }),
      succeeded: () => {},
      failed: () => {},
      ...actions,
    },
