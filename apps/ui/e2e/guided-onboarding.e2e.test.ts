...
const VIDEO_DIR = path.resolve(__dirname, "../../../.claude/tmp/guided-onboarding/video");
...
async function pickPaths({
  ...
  for (const [index, pick] of picks.entries()) {
    ...
    await expect(page.getByTestId(`pick-order-${pick.id}`)).toHaveText(String(index + 1));
  }
  await page.getByTestId("takeover-next").click();
}
...
async function trpcQuery<T>({
  ...
  const response = await page.request.get(`/api/trpc/${procedure}?input=${encoded}`);
...
test.describe("guided onboarding", () => {
  ...
  test("a fresh sign-up is taken over by Langy, connects a provider, lands with the panel open and is toured", async ({
    ...
    await expect(page.getByTestId("provider-line")).toContainText("I'll help you set those up.");
    await page.getByLabel("OpenAI", { exact: true }).click();
    await page.getByLabel("API key").fill(key);
    const connect = page.getByRole("button", { name: "Connect" });
    await connect.click();
    await expect(page.getByRole("button", { name: /Connected|Checking the key/ })).toBeVisible();
    ...
    try {
      ...
      const state = await readGuidedState({ page, organizationName }).catch((reason) => ({
    ...
    await expect(page.locator('[data-testid="tour-spotlight"][data-handoff]')).toBeVisible({
      timeout: 15_000,
    });
    ...
    await expect
      .poll(async () => (await readGuidedState({ page, organizationName })).tourCompletedAt, {
        timeout: 30_000,
      })
      .toBeTruthy();
  });
  ...
  test("skipping the guided tour on the provider screen lands on the personal home with the panel asking for a model", async ({
    ...
    await expect(page.getByTestId("provider-line")).toContainText("I'll help you set that up.");
    ...
    await expect(dialog.getByRole("button", { name: "Keep the guide" })).toBeVisible();
  ...
  test("with the flag off the classic wizard follows the tailor step", async ({ page }) => {
    ...
    await expect(page.getByRole("radiogroup", { name: "What do you want to do?" })).toBeVisible({
      timeout: 30_000,
    });
