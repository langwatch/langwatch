/**
 * The ACME checkout demo, driven over HTTP through one guest checkout, no
 * platform involved. Needs `uv` and an OpenAI key (env or workspace `.env`).
...
import { createDemoRepo, type DemoRepo, freePort, openaiKey } from "./local-control-fixture";
...
async function chat(threadId: string, content: string): Promise<ChatReply> {
  ...
  if (response.status !== 200) {
    throw new Error(`POST /chat answered ${response.status}: ${await response.text()}`);
  }
...
async function waitForHealth(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // Not listening yet.
      console.debug(`[acme-checkout-demo] health check not ready: ${String(error)}`);
    }
...
describe("the ACME checkout demo", () => {
  beforeAll(async () => {
    ...
    await fs.writeFile(path.join(repo.root, ".env"), `OPENAI_API_KEY=${openaiKey()}\n`, "utf8");
