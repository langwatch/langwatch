...
import { getWorkbenchState, seedOptimizationWorkbench } from "./seed-optimization-workbench";
import {
    ...
    it(
      ...
      async () => {
        ...
        try {
          const result = await runScenarioAndLog({
            config: {
              ...
              agents: [
                ...
                scenario.judgeAgent({
                  model,
                  criteria: [...LANGY_OPTIMIZE_LOOP_CRITERIA, ...LANGY_LIVE_PAGE_CRITERIA],
                }),
              ],
              script: [
                scenario.user(
                ...
                scenario.agent(),
                scenario.user("looks right, go ahead. no need to ask me for small runs"),
                scenario.agent(),
          ...
          // document.
          const executed = tab.claimedActions.filter((action) => action.outcome === "executed");
          expect(
            executed.length,
          ...
          expect(
            beforeClose.filter((output) => output.includes(BROWSER_MARKER)).length,
            `no action reported "executedVia":"browser" while the page was open, out of ${beforeClose.length} dispatches`,
          ).toBeGreaterThan(0);

          const afterClose = dispatchOutcomes(langy.state.toolOutputs.slice(handover.dispatches));
          expect(
            afterClose.length,
          ...
          expect(
            afterClose.filter((output) => output.includes(BACKEND_MARKER)).length,
          ...
          const wrongCarrier = after.state.evaluators.filter(
            (evaluator) =>
              evaluator.comparison !== undefined && !evaluator.evaluatorType.includes("compare"),
          );
          expect(
            wrongCarrier,
