/**
 * The coding path of the guided onboarding. See README.md "Scenario notes"
 * (coding) for what it covers and why, and the "Run" section above for how
 * to run one file.
 */
...
describe("Langy sets up coding agent tracking from the kickoff", () => {
  ...
  describe("when the kickoff for the coding path arrives", () => {
    ...
    it("prints the npx command between its two lines and records the path", async () => {
      ...
      expect((await readGuidedState(org.organizationId)).donePaths).toEqual(["coding"]);
    }, 900_000);
  });
});
