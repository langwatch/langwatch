...
## Prompt optimization (langy-optimization-bootstrap.scenario.test.ts, langy-evaluator-inference.scenario.test.ts)

Both seed a support-bot experiment through the workbench-state REST surface
(`seed-optimization-workbench.ts`: prompt, inline dataset, optional
answer-match evaluator, no baseline run) and grade against
`specs/langy/langy-prompt-optimization-bootstrap.feature`: the bootstrap suite
...
platform self-ingest loop; see `packages/config/src/self-ingest-guard.ts`):
