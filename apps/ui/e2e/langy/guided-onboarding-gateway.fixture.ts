 ...
 * the tour mints, and the reveal-in-hand run, from two snapshots.
 */
...
/**
 * The reveal-in-hand case, run from either snapshot (see README.md
 * "Scenario notes", gateway). Asserts and returns whether it passed.
 */
export async function expectGatewayPathFromTheRevealInHand({
  ...
  expect(text).not.toMatch(/already (exists?|have|had|minted|created|set up|there)/i);
  ...
  expect(
    commands.some((command) => /langwatch onboarding complete-path gateway/.test(command)),
  ).toBe(true);
  assertPathCompletedAfterSkill({
    events: langy.state.toolEvents,
    path: "gateway",
