// A person of their own for a scenario, seeded like
// `scripts/seed-dogfood-password.ts` does (a verified user plus its
// credential account) since a scenario has no mailbox to confirm through.
// Only on loopback: app under test and database both.
...
// A standalone REPO_ROOT (not imported from local-control-fixture.ts, which
// would cycle back here through trpc.ts): apps/ui/e2e/langy -> workspace root.
 ...
 */
const hostForError = (host: string | null): string => host ?? "an address that does not parse";
...
export function resolveSeedDatabaseUrl({
  ...
  const databaseUrl = env.DATABASE_URL ?? (dotenv ? databaseUrlFromDotenv(dotenv) : undefined);
  if (!databaseUrl) {
    throw new Error(
      "A scenario's own account needs the local stack's database: set DATABASE_URL, or keep it in the workspace-root .env.",
    );
  }
...
export async function openLocalAccountStore({
  ...
  const prisma = new PrismaClient({
    adapter: PrismaDriverAdapterService.create().createOwnedAdapter(databaseUrl),
  });
  return {
