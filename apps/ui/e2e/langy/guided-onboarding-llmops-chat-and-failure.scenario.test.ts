 ...
 * scenario then fails. See README.md "Scenario notes" (llmops-chat-and-failure).
 */
  ...
  describe("when the developer picks Chat about this and asks for a scenario the agent cannot pass", () => {
    ...
    it(
      ...
      async () => {
        ...
        let chatTurnText = "";
        let scenariosAfterChat: { id: string; name: string }[] = [];
        let suitesAfterChat: { id: string; name: string }[] = [];
        ...
        expect(proposal!.options?.[1]?.label).toBe(GUIDED_OPTIONS.chatAboutThis);
        expect(proposal!.options?.[1]?.quiet).toBe(true);
        expect(scenariosAtProposal).toEqual([]);
        expect(saysVerbatim(chatTurnText, GUIDED_LINES.chatAboutThis)).toBe(true);
        ...
        expect(saysVerbatim(storedAssistantText(stored), GUIDED_LINES.proved)).toBe(true);
        ...
        const langyBranch = branches.find((branch) => branch.startsWith("langy/"));
...
function commandParts(messages: { parts: Record<string, unknown>[] }[]): Record<string, unknown>[] {
...
function storedCommands(messages: { parts: Record<string, unknown>[] }[]): string[] {
  return commandParts(messages).map((part) => String((part.input as { command: string }).command));
}
...
function storedOutputs(messages: { parts: Record<string, unknown>[] }[]): string[] {
