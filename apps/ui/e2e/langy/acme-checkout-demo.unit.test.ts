/**
 * The demo stays a blank slate for Langy: no tracing, tests or LangWatch
 * dependency, or the guided-onboarding scenarios that add them go no-op.
...
import { createDemoRepo, type DemoRepo, REPO_ROOT } from "./local-control-fixture";

const DEMO_DIR = path.join(REPO_ROOT, "dev", "dogfood", "acme-checkout", "python");
...
describe("the ACME checkout demo tree", () => {
  describe("when the source tree is scanned", () => {
    ...
    it("names no tracing or test dependency, imports none, and says so in the README", () => {
      const pyproject = readFileSync(path.join(DEMO_DIR, "pyproject.toml"), "utf8");
...
describe("the scenario harness", () => {
  ...
  describe("when a scenario creates a demo repository in the langgraph language", () => {
    ...
    it("makes a git repository on main with the sources and the manifest exactly as shipped", async () => {
      ...
      const shipped = readFileSync(path.join(DEMO_DIR, "pyproject.toml"), "utf8");
