/**
 * The llmops path when the folder never connects. See README.md
 * "Scenario notes" (llmops-never-connects) for what it covers and why, and
 * the "Run" section above for how to run one file.
 */
...
describe("Langy offers GitHub when the folder never connects", () => {
  ...
  describe("when the request runs out with no folder connected", () => {
    ...
    it("offers GitHub in one line, asks again, and creates nothing", async () => {
        ...
        config: {
          ...
          script: [
            ...
            async () => {
              ...
              expiredRequestId = (await waitForPendingRequest({ conversationId })).id;
      ...
      expect(langy.state.toolCommands.some((c) => /scenario create|test-suite/.test(c))).toBe(
        false,
      );
