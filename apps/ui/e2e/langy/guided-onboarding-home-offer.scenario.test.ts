/**
 * The Home offer continues the guided conversation. See README.md
 * "Scenario notes" (home-offer) for what it covers and why, and the "Run"
 * section above for how to run one file.
 */
...
describe("The Home offer continues the guided conversation", () => {
  ...
  describe("when the gateway path starts from the Home offer after the coding path", () => {
    ...
    it("sends the second kickoff into the attached conversation and Langy opens the gateway path", async () => {
      ...
      console.log("[layer2] kickoffs:", JSON.stringify(kickoffs.map((k) => k.part?.path)));
      expect(kickoffs.map((k) => k.part?.path)).toEqual(["coding", "gateway"]);
      expect(continuation.input?.path).toBe("gateway");
      expect(kickoffs[1]?.text.startsWith(guidedPathContinuationLine("gateway"))).toBe(true);
      expect(guidedPathContinuationLine("gateway")).toBe("Let's set up Gateway then.");

      const keys = await listVirtualKeys(org.organizationId);
      expect(keys.filter((key) => key.name === "production-app")).toHaveLength(1);
      ...
      expect((await readGuidedState(org.organizationId)).currentPath).toBe(undefined);
    }, 1_500_000);
  });
});
