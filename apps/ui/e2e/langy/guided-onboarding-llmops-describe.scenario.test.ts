/**
 * The describe fallback of the llmops path. See README.md "Scenario notes"
 * (llmops-describe) for what it covers and why, and the "Run" section above
 * for how to run one file.
 */
    ...
    it("asks for one line, then asks how to connect and offers the folder and GitHub again", async () => {
        ...
        config: {
          ...
          script: [
            ...
            async () => {
              ...
              firstRequestId = (await waitForPendingRequest({ conversationId })).id;
            },
      ...
      const codeAccessInputs = stored.flatMap((message) =>
        message.parts
          ...
          .map((part) => part.input as { offer_describe?: unknown } | undefined),
      );
      console.log("[layer2] code_access inputs:", JSON.stringify(codeAccessInputs));
      expect(codeAccessInputs.length).toBeGreaterThanOrEqual(2);
      expect(codeAccessInputs[0]?.offer_describe).toBe(true);
      expect(codeAccessInputs[codeAccessInputs.length - 1]?.offer_describe).not.toBe(true);
