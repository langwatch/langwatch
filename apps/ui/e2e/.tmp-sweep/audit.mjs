...
for (const route of ROUTES) {
  ...
  page.on("response", async (r) => {
    ...
    rpcFails.push({
      status: r.status(),
      url: u.replace(APP, "").split("?")[0],
      q: (u.split("?")[1] || "").slice(0, 120),
      detail,
    });
  });
  const rec = {
  ...
  try {
    ...
    const vis = await page.evaluate(() => {
      const out = [];
      const pat =
        /(something went wrong|unknown error|failed to|could not|error|unable to|not found|went wrong|doesn't load|does not load|try again)/i;
      for (const el of document.querySelectorAll(
        '[role="alert"],[role="status"],[data-part="root"][data-scope="toast"],[class*="chakra-alert"]',
      )) {
  ...
  results.push(rec);
  console.error(
    `${route} len=${rec.textLen} err=${rec.errors.length} fail=${rec.rpcFails.length} vis=${rec.visibleErrors.length}`,
  );
  await page.close();
}
