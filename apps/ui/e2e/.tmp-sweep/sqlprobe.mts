import { buildTimeseriesQuery } from "../../../../modules/analytics/process/src/repositories/clickhouse/clickhouse.aggregation-builder.mapper.ts";
import { buildSlimTimeseriesQuery } from "../../../../modules/analytics/process/src/repositories/clickhouse/clickhouse.slim-timeseries-query.mapper.ts";
import { checkTenantScope } from "../../../../packages/clickhouse-client/src/tenantGuard.ts";
...
for (const [bname, build] of Object.entries({
  aggregation: buildTimeseriesQuery,
  slim: buildSlimTimeseriesQuery,
})) {
  for (const [vname, input] of Object.entries(variants)) {
    ...
    try {
      ...
      sql = built.sql;
      verdict = checkTenantScope({
        sql,
        params: built.params as Record<string, unknown>,
        tenantId: "local-dev-project",
      });
    } catch (e) {
      verdict = { threw: String(e).slice(0, 80) };
    }
