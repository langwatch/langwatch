...
# 4. Run tests (from tests/agentic-e2e/ directory)
cd ../../tests/agentic-e2e
...
CLI e2e deliberately lives elsewhere — `sdks/typescript/__tests__/e2e/cli/`
...
`platform/app/src/server.mts` loads `.env` with `override: true`, so `.env` beats
...
```bash
# Run all tests (from tests/agentic-e2e/ directory)
pnpm test
