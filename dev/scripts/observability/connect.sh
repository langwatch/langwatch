...
#   1. points .env at the local collector (traces + logs + metrics),
...
# ---------------------------------------------------------------------------
# 1. Point .env at the local collector
# ---------------------------------------------------------------------------
...
if [ -f "$ENV_FILE" ]; then
  ...
  say "Pointed .env at ${OTLP_ENDPOINT} (TS traces+logs+metrics, Go dual-export)."
  say "Tagged telemetry with langwatch.worktree=${worktree_name}."
else
  warn ".env not found — skipping .env wiring. Set these yourself:"
  cat <<EOF
