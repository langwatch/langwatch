...
echo "dirty=$dirty legacy_files=$legacy modules_on_legacy=$mods feature_shape_rows=$rows families_absent=$absent api_side_files=$apiside ports_adapters_files=$portsfiles port_names=$portnames port_word_files=$portword main_unfolded=$main api_boot=$boot head=$(git log --format=%h -1)"
