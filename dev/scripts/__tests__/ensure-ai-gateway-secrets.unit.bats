#!/usr/bin/env bats
# Unit tests for dev/scripts/ensure-ai-gateway-secrets.sh — guards
...
@test "exits 0 cleanly when .env does not exist" {
  run bash "$TEST_DIR/dev/scripts/ensure-ai-gateway-secrets.sh"
  [ "$status" -eq 0 ]
  [ ! -f "$ENV_FILE" ]
}
...
SOMETHING_ELSE=keep-me
EOF
  bash "$TEST_DIR/dev/scripts/ensure-ai-gateway-secrets.sh" >/dev/null
