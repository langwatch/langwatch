#!/usr/bin/env bats
# Unit tests for write_dev_overrides() in dev/scripts/lib/write-dev-overrides.sh
# — the shared per-preset URL rewrite layer used by both `dev/scripts/dev.sh`
# (quickstart presets) and `dev/scripts/dev-up.sh` (per-worktree isolated
...
# connection-host). Credentials live in .env.
...
@test "dev-storage overlay NEVER contains credentials (S3_ACCESS_KEY_ID / S3_SECRET_ACCESS_KEY / S3_SESSION_TOKEN)" {
  # Credentials always come from .env (rotated by refresh-dev-s3-env.sh).
...
@test "dev-infra overlay NEVER contains S3 credentials" {
  # Credentials always come from .env, rotated by
