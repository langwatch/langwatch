...
const pkgs = execFileSync("sh", ["-c", "ls modules/*/web/package.json 2>/dev/null"], { encoding: "utf8" })
  .split("\n").filter(Boolean);
...
for (const f of files) {
  ...
  const kind = f.startsWith("apps/ui/") ? "app" : f.startsWith("apps/") ? "app" : "peer";
...
let appOnly = 0, peerToo = 0, unused = 0;
const peerEntries = [];
for (const p of pkgs) {
  const mod = p.split("/")[1];
  ...
  for (const key of Object.keys(exp)) {
    ...
    if (!who) { unused++; continue; }
    if (who.has("peer")) { peerToo++; peerEntries.push(`${mod}${key.slice(1) || " (root)"}`); }
    else { appOnly++; }
  }
}
...
console.log(peerEntries.slice(0, 40).map((e) => `  ${e}`).join("\n"));
