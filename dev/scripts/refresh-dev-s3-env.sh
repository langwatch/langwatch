#!/usr/bin/env bash
# refresh-dev-s3-env.sh — rotate the S3 SSO credentials in .env
#
...
# S3_SECRET_ACCESS_KEY / S3_SESSION_TOKEN lines in .env in
...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "ERROR: $ENV_FILE not found. Copy $REPO_ROOT/.env.example first." >&2
  exit 1
fi
