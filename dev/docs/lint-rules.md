...
Every rule the `langwatch` oxlint plugin registers (78 of them), rendered from the
...
- [`langwatch/design-system-export-collision`](#langwatchdesignsystemexportcollision)
...
- [`langwatch/positional-parameter-list`](#langwatchpositionalparameterlist)
...
- [`langwatch/rest-declares-input-output`](#langwatchrestdeclaresinputoutput)
- [`langwatch/rest-declares-its-answer`](#langwatchrestdeclaresitsanswer)
- [`langwatch/rest-handler-throws`](#langwatchresthandlerthrows)
- [`langwatch/rest-no-error-handler-override`](#langwatchrestnoerrorhandleroverride)
- [`langwatch/rest-schema-from-own-contract`](#langwatchrestschemafromowncontract)
...
## `langwatch/design-system-export-collision`

- Kind: problem
- Applies to: isFeatureWebSource
- Fixable: no
- Spec: `specs/tooling/lint-design-system-export-collision.feature`
...
- `noTryPrefix`
  ...
  - fix: Decide what it answers, then drop `try` either way. If it answers one thing that may not exist, name it `get<Noun>` — or `getBy<Key>` when the key is what distinguishes it — and throw the domain error instead of null. If it answers none or many, return an array and name it `find<Noun>`: the empty array is the absence. Renaming it to a `find*` that still answers null is the one move to avoid — `find` states cardinality, so that name would promise a list and hand back a maybe.
...
## `langwatch/positional-parameter-list`

- Kind: problem
- Applies to: applies
- Fixable: no
- Spec: `specs/tooling/lint-positional-parameter-list.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
...
## `langwatch/rest-declares-input-output`

- Kind: problem
- Applies to: isRestTransportSource
- Fixable: no
- Spec: `specs/tooling/lint-rest-declares-input-output.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
...
## `langwatch/rest-declares-its-answer`

- Kind: problem
- Applies to: isRestOrTransportSource
- Fixable: no
- Spec: `specs/tooling/lint-rest-declares-its-answer.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
...
## `langwatch/rest-handler-throws`

- Kind: problem
- Applies to: isRestTransportSource
- Fixable: no
- Spec: `specs/tooling/lint-rest-handler-throws.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
...
## `langwatch/rest-no-error-handler-override`

- Kind: problem
- Applies to: isServerSource
- Fixable: no
- Spec: `specs/tooling/lint-rest-no-error-handler-override.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
...
## `langwatch/rest-schema-from-own-contract`

- Kind: problem
- Applies to: isRestTransportSource
- Fixable: no
- Spec: `specs/tooling/lint-rest-schema-from-own-contract.feature`
- Enforced: **no** — registered here but not enabled in `oxlint.architecture.jsonc`, so it reports nothing
