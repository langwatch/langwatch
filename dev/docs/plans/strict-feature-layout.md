...
- prompt: `promptTags.rename`/`delete` and the cross-project refusals in `copy`/`syncFromSource`/`pushToCopies` answer 403 `permission_denied` (were hand-spelled 401 `UNAUTHORIZED`); a taken handle is a named 409 on REST (`PromptHandleTakenError`, plain until `prompt_handle_taken` is registered); `GET /api/prompts/:id` declares `version` and `tag`; the family is literal (`/api/prompts` + v1 twin, bareMount preserved). The playground execution door is still on the deleted builder: it needs a narrow door api beside `PromptApi` (stored-object precedent, allowed) and five registered refusal codes.
- workflow: the evaluate route's API-key ceiling is a required fact refusing 403 with `{ error: "This key cannot read evaluation runs" }` (was the ceiling's handled envelope; REST `withPermission` takes one permission, the `permission-all` form is a runtime edge to close); the run family's 401/403 bodies come from the mount's error handler; `optimization.toggleSaveAsEvaluator` on a missing workflow is NOT_FOUND (was 500); `post_event` invalid body is `400 { error: "Invalid body" }`; the CRUD family's operation ids are declared, so the frozen document moves.
- ops: the operator gate lives in the app (`admitOperator`/`admitStaff`/`admitBackOfficeStaff` over `isAdmin`, same refusals); `POST /api/bug-reports` answers 413 on an oversize body and 400 `malformed_request` on non-JSON (was a crash); `searchAggregates` applies its 365-day lookback in the app; the 92 `ops.*` procedures are five declarations on one namespace because the router type hits TS2589 at fifty, so the process merges them. `startReplay` still throws a raw CONFLICT for every failure (needs a code).
...
| `ui-family-move-manifests.md` | Eighteen manifests for the UI family moves | The drawer registry mechanism moves, not the drawers. `@langwatch/browser-host/drawer` owns the address vocabulary, the navigation stack, the stores, the lazy registry and `CurrentDrawer` | `best_practices/drawers.md`, section "The drawer registry" |
...
| `ui-host-capabilities-2026-09-05.md` | The browser host ports in `packages/ui-host` | Router, toaster, error presenter and link move to `@langwatch/browser-host` | `best_practices/ui-install.md`. The product ruling is section 10 item 23 |
