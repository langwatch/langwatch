...
| `POST /api/track_usage` (anonymous self-hosted instance telemetry receiver) | `server/routes/misc.ts:1288` | none | **Absent.** The *sender* survives (`packages/features/ops/server/.../ops-worker.contribution.ts`) but is itself uninstalled (see §3); the receiver has no counterpart. |
...
| `generate-license.ts` + `localDevLicense.ts` | `scripts/` | none (the licensing *feature* survives) | **Absent.** No license minting. |
