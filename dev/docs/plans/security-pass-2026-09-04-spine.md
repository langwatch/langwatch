...
| ------------------------------------------------------------------------------------------------------------------ | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| [C1](#c1--critical-sql-injection-into-clickhouse-through-seriesmetric)                                             | **Critical** | Free-string `series[].metric` reaches the ClickHouse SELECT list unsanitised — arbitrary cross-tenant read with an ordinary project API key                                                                                                   | `modules/analytics/process/src/clickhouse/metric-translator.ts:227, 291-296`                                                |
...
| [H2](#h2--high-x-project-id-re-points-a-scoped-key-at-any-sibling-project-and-two-doors-behind-it-have-no-ceiling) | High         | `X-Project-Id` re-points a scoped API key at any sibling project, and two families behind it enforce no ceiling                                                                                                                               | `modules/api-key/process/src/services/api-key-token-resolution.service.ts:134-176`                                          |
...
| [H6](#h6--high-an-api-key-writes-model-defaults-with-its-owners-permissions)                                       | High         | An API key writes model defaults with its **owner's** permissions; the key's ceiling is never consulted                                                                                                                                       | `modules/model-provider/process/src/transport/api-rest/model-defaults.routes.ts:120-131`                                    |
...
| [H9](#h9--high-the-organization-audit-log-returns-other-organizations-project-rows)                                | High         | The organization audit log matches `projectId: { not: null }` — it returns other organizations' project rows, payloads included                                                                                                               | `modules/organization/process/src/repositories/prisma/prisma.organization-membership.repository.ts:1516-1523`               |
...
| [M5](#m5--medium-timing-unsafe-comparison-of-the-scim-webhook-shared-secret)                                       | Medium       | Timing-unsafe `!==` on the SCIM webhook shared secret, and no replay window                                                                                                                                                                   | `enterprise/modules/scim/process/src/transport/api-rest/scim-webhook-intake.api.ts:47-50`                                   |
| [M6](#m6--medium-legacy-project-keys-are-plaintext-permanent-unrevocable-and-ceiling-exempt)                       | Medium       | Legacy project keys are stored in plaintext, never expire, cannot be revoked, and are exempt from every ceiling                                                                                                                               | `modules/api-key/process/src/repositories/prisma/prisma.api-key.repository.ts:133-146`                                      |
| [M7](#m7--medium-share-links-default-to-permanent-public-and-unlimited-and-minting-is-uncapped)                    | Medium       | Share links default to permanent, public and unlimited views; minting is neither capped nor throttled                                                                                                                                         | `modules/share/process/src/transport/api-trpc/share.api.ts:62-113`                                                          |
| [M8](#m8--medium-post-apiauthvalidate-is-an-unauthenticated-unthrottled-key-oracle)                                | Medium       | `POST /api/auth/validate` is an unauthenticated, unthrottled key-validity and project oracle                                                                                                                                                  | `modules/auth/process/src/transport/api-rest/auth.api.ts:114-132`                                                           |
| [M9](#m9--medium-the-origin-gate-exists-but-is-mounted-only-on-apiauth)                                            | Medium       | The origin gate is correct but mounted only on `/api/auth/*`; admin, CLI-approve and logout carry none                                                                                                                                        | `modules/auth/process/src/transport/api-rest/auth.api.ts:218-241`                                                           |
...
| [M13](#m13--medium-the-ops-explain-system-guard-is-bypassable-with-quoted-identifiers)                             | Medium       | The ops EXPLAIN `system.*` guard is bypassable with quoted identifiers, and misses `gcs` / `dictGet` / `mergeTreeIndex`                                                                                                                       | `modules/ops/process/src/services/ops-clickhouse-explain.core.ts:30, 124-143`                                               |
...
| [M16](#m16--medium-eleven-project-list-endpoints-have-no-page-size-at-all)                                         | Medium       | Eleven project list endpoints have no page size; three read tables that grow with usage and take no pagination parameters at all                                                                                                              | `modules/prompt/process/src/transport/api-rest/prompt.api.ts:335-337, 678-680` (+9)                                         |
...
**Sink** `modules/analytics/process/src/clickhouse/metric-translator.ts:227` and `:291-296`.
Verified independently against the source, not taken on report.

The wire schema accepts a free string —
`modules/analytics/process/src/model/analytics-input.ts:86-88`:
...
and executes at
`modules/analytics/process/src/repositories/clickhouse/clickhouse.analytics.repository.ts:100-102`
...
(`modules/analytics/process/src/transport/api-trpc/analytics.api.ts:145`, permission
...
`modules/auth/process/src/transport/better-auth/better-auth.api.ts:256-260` sets only
...
(`modules/auth/process/src/transport/api-rest/auth.api.ts:218-241`).
...
(`modules/project/process/src/transport/api-trpc/project.api.ts:264`) — the victim holds it,
...
in `modules/auth/process/src/transport/better-auth/origin-gate.ts` already implements it.
...
**File** `modules/api-key/process/src/services/api-key-token-resolution.service.ts:134-176`.
...
(`modules/stored-object/process/src/transport/api-rest/stored-object.api.ts:194-196, 429-431`).
...
`modules/auth/process/src/transport/better-auth/better-auth.api.ts:98-101` and `:600-660`.
...
(`modules/ops/process/src/services/admin-access.service.ts:27-30`).
...
`modules/auth/process/src/transport/better-auth/better-auth.api.ts:256-260`.
...
(`modules/trace/process/src/transport/api-trpc/shared-trace.api.ts:161-170`).
...
**File** `modules/model-provider/process/src/transport/api-rest/model-defaults.routes.ts:120-131`
...
`modules/model-provider/process/src/services/model-provider-defaults-write.service.ts:85-86`
...
(`modules/model-provider/process/src/transport/api-trpc/model-provider.api.ts:376`), then
...
**File** `modules/organization/process/src/repositories/prisma/prisma.organization-membership.repository.ts:1516-1523`
(read and confirmed):
...
- `modules/scenario/process/src/adapters/serialized-prompt-config.adapter.ts:24` (rendered at `:84, :89`)
...
`modules/langy/process/src/transport/api-rest/langy-rest.credentials.ts:104-107` do not.
...
`enterprise/modules/scim/process/src/transport/api-rest/scim-webhook-intake.api.ts:37-44` and
`enterprise/modules/billing/process/src/transport/api-rest/stripe-webhook.api.ts:53-56`. Both
...
**File** `enterprise/modules/scim/process/src/transport/api-rest/scim-webhook-intake.api.ts:47-50`.
...
**File** `modules/api-key/process/src/repositories/prisma/prisma.api-key.repository.ts:133-146`.
...
**Files** `modules/share/process/src/transport/api-trpc/share.api.ts:62-113`;
`modules/share/process/src/services/share.service.ts:183-240`.
...
(`modules/trace/process/src/transport/api-trpc/shared-trace.api.ts:110-114`).

---

## M8 — Medium: `POST /api/auth/validate` is an unauthenticated, unthrottled key oracle

**File** `modules/auth/process/src/transport/api-rest/auth.api.ts:114-132`.
...
**File** `modules/auth/process/src/transport/api-rest/auth.api.ts:218-241`.

`isAllowedAuthOrigin` (`modules/auth/process/src/transport/better-auth/origin-gate.ts`) is
...
(`modules/ops/process/src/transport/api-rest/admin.api.ts:171-174`), `POST /approve` and
`POST /deny` on the CLI device flow
(`modules/auth/process/src/transport/api-rest/auth-cli-device-flow.api.ts:801, 1019`), and
...
(`modules/auth/process/src/transport/better-auth/better-auth.api.ts:220-229`), so behind a
...
`modules/auth/process/src/transport/api-rest/auth-cli-device-flow.api.ts:254-258` declares
...
(`modules/workflow/process/src/transport/api-trpc/workflow-optimization.api.ts:126`) and the
...
**File** `modules/ops/process/src/services/ops-clickhouse-explain.core.ts:30` —
...
(`modules/ops/process/src/services/__tests__/ops-clickhouse-explain.core.unit.test.ts:116-119`).
...
`modules/gateway/process/src/repositories/prisma/prisma.gateway-guardrail.repository.ts:89-90`
...
(`modules/agent/process/src/repositories/prisma/prisma.agent.repository.ts:67-71`,
...
- `GET /api/prompts` — `modules/prompt/process/src/transport/api-rest/prompt.api.ts:335-337`,
...
- `GET /api/annotations` —
  `modules/annotation/process/src/transport/annotation.rest.ts:141, 150-153`;
...
`modules/auth/process/src/services/auth.service.ts:73-105` re-reads `Session.impersonating`,
...
(`modules/ops/process/src/services/impersonation.service.ts:11`). _Fix:_ re-assert
...
`modules/model-provider/process/src/services/model-provider-defaults-write.service.ts:45, 85,
...
`modules/authz/process/src/services/authz.service.ts:684-709` destructures
...
`modules/api-key/process/src/services/legacy-api-key-grant.service.ts:36-60` returns
...
`modules/api-key/process/src/services/api-key-lifecycle.service.ts:70-77` — when
...
(`modules/api-key/process/src/app/api-key.app.ts:262-269`), so this is an admin granting
...
`modules/model-provider/process/src/repositories/prisma/prisma.model-provider.repository.ts:57-77`
...
`modules/gateway/process/src/services/gateway-realtime-session.service.ts:445` — an
...
`modules/trace/process/src/services/trace-list-read.service.ts:342`
(`SORT_COLUMN_MAP[params.sort.columnId]`, where `columnId` is `z.string()` at `traces-v2.api.ts:358`)
and `modules/analytics/process/src/clickhouse/aggregation-builder.ts:698`. `columnId:
...
`modules/user/process/src/transport/api-rest/user-avatar.api.ts:223-228` declares
...
(`enterprise/modules/scim/process/src/services/scim-provisioning.service.ts:478-484`). Reading
...
(`modules/api-key/process/src/transport/api-trpc/api-key.api.ts:180-320`) take
`organizationId` from input with no declared permission — and every corresponding application method
opens with `ensureMember`
(`modules/api-key/process/src/app/api-key.app.ts:118, 149, 166, 256, 288, 306, 323, 332, 348`),
...
(`modules/api-key/process/src/adapters/api-key-token.api-key-token.adapter.ts:41-75`;
...
(`modules/ops/process/src/services/admin-access.service.ts:27-30`); audited before the window
...
(`modules/github/process/src/transport/api-rest/github.api.ts:440-450, 494-500`) and the
ElevenLabs receiver
(`modules/gateway/process/src/transport/api-rest/elevenlabs-webhook.api.ts:134-153`) both use
