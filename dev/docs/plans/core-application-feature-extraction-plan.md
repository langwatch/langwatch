...
| Configuration ownership        | UI owns the strict public schema and inert base64url codec. Eventing, logger, telemetry, Gateway virtual-key, evaluation concurrency, scenario-child input and all five Group Queue values are parsed at process composition and injected. Production reusable-package source has no executable ambient env read, and the architecture rule governs every `packages/**/src` production module. The combined private boot schema and public-config source projection remain in `platform/app`.                               | Finish separate executable projections and delete legacy config only after invalid, missing, ordering and role-specific boot parity.                                                            |
...
| Persistence foundations        | Prisma, Redis and ClickHouse have explicit App/task construction and exact shutdown owners. `server/db.ts` is construction-free. The App and Worker now compose canonical Eventing ProcessStore/EventStore/retention graphs, and the three displaced platform adapters are deleted. At `83cdb89996`, the non-test lexical sweep still finds 60 platform files mentioning the Prisma compatibility binding.                                                                                                                  | Continue dependency-closed feature persistence verticals. Keep Worker consumers disabled; the complete shared registry and consumer activation are Wave 4 work.                                 |
| Infrastructure clients         | Shared AWS credential/proxy/handler policy, Dataset S3 lifetime and NLP Lambda/CloudWatch client pairs are process-owned. SDK client disposal borrows the handler and cannot destroy the process pool; shutdown rejects new work and drains active requests. Lambda/Logs preserve static credentials, Lambda retries and standard/China partition proxy targets; task cleanup closes its clients before its local transport. Legacy S3 operations now reuse that transport with standard, China and custom endpoint parity. | Keep the object-storage migration and Enterprise S3 boundaries task-local/feature-local, then move the remaining direct storage clients behind their owning process adapters.                   |
...
| `F-CONFIG-01`     | At committed checkpoint `83cdb89996`, the non-test lexical sweep finds 63 platform source files mentioning `env.mjs` and 92 mentioning `process.env`, with overlap. Evaluation concurrency and scenario-child configuration are injected by `87fc7f4521`; SSO remains Wave 2. Preserve database empty fallback, credential/auth-secret chains, privileged internal-route secrets, mail unsubscribe differences and storage unsafe/test gates before deleting the compatibility proxy.                                                                                                                                                               | Wave 1                              |
...
At the working checkpoint, `platform/app` contains 6,339 tracked files,
including 5,982 under `src`. Counts include tests unless identified as
...
Current Wave 1 progress at `83cdb89996`:
...
- [ ] Keep the object-storage migration and Enterprise S3 as explicit residuals
      until their named adapters preserve task-local and feature-specific
      lifecycle boundaries. NLP Lambda/CloudWatch is complete in `ec1240fb37`.
