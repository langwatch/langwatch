...
gate closes. Struck rows landed on 2026-08-28. `F-BRANCH-01` applies across the
...
| 6     | Inject mail runtime configuration            | `F-MAIL-01`                     | Ready, and closes `F-MAIL-01`: the empty-`NEXTAUTH_SECRET` case now exercises the real path because the secret is a parameter.           |
| 7     | Model-client and Langevals payload config    | Infrastructure clients          | Ready but partial; each reaches one caller, so record the remaining env-reading paths rather than claiming the cut.                      |
...
| 9     | SCIM and Ops email-change revocation         | `F-USER-AUTH-01`                | Blocked on `F-LOCK-01`. Ship the two halves together with `@scenario` bindings.                                                          |
| 10    | apps/api API-key organization REST security  | `F-API-06`, `F-APIKEY-01/02`    | Blocked on the Organization settings slice, and on the two API-key findings.                                                            |
...
| 13    | Analytics/Dashboard persistence              | Analytics/Dashboard persistence | Blocked. The new service exceeds the 500-line ceiling and cannot be baselined, and 20 spec scenarios lost their bindings.                |
| 14    | Gateway persistence                          | Gateway persistence             | Blocked. Four new error codes are unregistered and the composition surface still imports generated Prisma, so the plan gate is unmet.    |
...
| `F-APIKEY-01`         | The standalone API-key security slice reimplements a live policy instead of porting it, and drops its mount-time guarantees. `buildSecretRestApi` passes no `onRouteMounted`, so `appendPermissionMiddleware` silently accepts an endpoint declaring no permission, and `ApiKeyManagementRestFeature` is a raw `new Hono()` rather than the incumbent `SecuredApp`, which withholds verbs until `.access(policy)` is called. All five management routes are therefore absent from the route-policy registry and the authorization audit. Nothing outside its own tests composes it, so two API-key ceilings now exist and the platform one serves traffic — the drift the incumbent's own comment records having already cost once.                                                                                                               | Wave 3 API cutover                  |
| `F-APIKEY-02`         | The organization REST security adapter dresses infrastructure failures as handled errors — a bare `catch` raising `internal_error` with no `cause`, `reasons` or log line — contradicting ADR-045 and the incumbent's explicitly recorded decision; its organization existence probe calls `organizations.getSettings`, which decrypts S3 credentials on the authentication hot path; and neither new adapter logs any refusal, dropping the incumbent `AuthDiagnostics` and the ceiling's `notDelegableReason` warn. The management error handler logs nothing, omits `fault` and casts `httpStatus`. Key comparison, header precedence, credential-derived project selection and exact organization scope match are faithful ports and are not at issue.                                                                                        | Wave 3 API cutover                  |
