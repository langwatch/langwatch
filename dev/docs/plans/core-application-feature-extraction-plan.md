...
| Worker physical activation     | Topic commands and Trace processing registration are package-owned. Trace installs before Topic, Dataset durable payloads are parsed, shutdown order is proven, and one dead app bridge is deleted. Consumers remain disabled because the shared queue still includes every other legacy pipeline. | Move the remaining registry groups and concrete durable dependencies into the worker, then enable the one complete shared-queue consumer and delete displaced worker registrations. |
| UI physical activation         | Chunk recovery, runtime behaviour, shell sections and Agent browser transport now follow the enforced global/private hierarchy. `apps/ui/src/app`, `platform` and `testing` are invalid roots and contain no production files.                                                                     | Move the actual browser entry, providers, router, overlays and route families out of `platform/app`, retaining the legacy shell adapter until URL/provider parity is proven.        |
...
| Process observability adoption | API listener and Worker/Eventing shutdown now drain intake/features before observability. The reusable package foundation is committed.                                                                                                                                                            | Align the LangWatch SDK/OTel graph and preserve metrics, profiling and disabled/no-key behaviour in executable API/worker boot tests.                                               |
...
At the working checkpoint, `platform/app` contains 6,305 tracked files,
including 5,949 under `src`. Counts include tests unless identified as
