...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API physical activation         | `apps/api` owns an injectable typed boot/process boundary, a real Node/Hono listener, request policy, graceful drain, Agent/Secret tRPC and all four Secret REST bases. `ab64885d6f` requires the canonical Auth service plus an injected Better Auth transport for browser-session resolution. It remains a library without a physical launcher or concrete project-key/PAT/admin adapters; the platform route graph remains live.                                                                                                                                                                                                          | Compose project-key/PAT/admin, rate-limit and audit adapters and executable boot, then migrate the remaining REST/tRPC route graph by vertical before deleting compatibility routers.                                                           |
| Worker physical activation      | `apps/worker` owns an injectable typed boot/process boundary, structural `ResourceScope` borrowing, signals and fail-safe startup cleanup. Its production composition constructs the canonical PostgreSQL/ClickHouse Eventing graph plus one Redis/AWS/Group Queue foundation. Stored Object owns dispatch/destination policy; `4bfb7bd679` adds concrete Worker S3/filesystem drivers, a typed BYOC source and lazy Azure factory port. Both Eventing consumer switches remain off because the shared queue still contains every legacy pipeline.                                                                                           | Parse and bind the remaining Worker storage/Redis/queue inputs plus physical BYOC/Azure sources, then in Wave 4 install the complete registry and intent ports before enabling the one shared-queue consumer.                                   |
...
| `F-API-06`            | `apps/api` owns a callable listener and injected runtime bootstrap that parses typed config/logger/telemetry once, retains one scope and drains listener → graph → telemetry. `ab64885d6f` requires the canonical Auth service and Better Auth transport for browser sessions. It deliberately has no `process.env` launcher/package start command: project-key/PAT/admin authentication, API-key ceiling/mark-used, rate-limit and audit adapters remain outside the graph, so a launcher would still create an incomplete second API process. Port aliases remain `LANGWATCH_API_PORT`, then `API_PORT`, then `PORT`. | Wave 1, Wave 2 and Wave 3           |
...
| `F-WORKER-STORAGE-01` | `4bfb7bd679` gives Worker concrete S3/filesystem drivers, BYOC-first selection, process AWS proxy wiring and a lazy Azure factory port behind the canonical Stored Object runtime. The physical Worker config/executable still does not supply Redis/queue/storage projections, the project BYOC source or Azure implementation, and the legacy worker remains live until Wave 4 installs the complete shared Eventing registry.                                                                                                                                                                                        | Wave 1 physical Worker activation   |
...
| `F-MAIL-01`           | `52ec8f2a41` moves provider settings to a typed private App projection and deletes the old mail config module. The unchanged `unsubscribeToken.unit.test.ts` empty-`NEXTAUTH_SECRET` case remains red because its mutation does not reach the validated config module; link/unsubscribe helpers still read app env. Invitation delivery also retains its historical `SENDGRID_API_KEY` gate even when another injected provider is available; changing that is a later compatibility decision, not part of the config cut.                                                                                              | Wave 1 configuration/boot           |
...
#### Active Wave 1 foundation lanes at `52ec8f2a41`
...
Current Wave 1 progress at `52ec8f2a41`:
...
- [ ] Finish Group Queue, storage and external client construction in the
      physical API/Worker roots. Worker S3/filesystem providers are committed;
      private config, BYOC/Azure sources and executable binding remain.
