...
- [ ] Recut `otlp-ingestion` into Trace, Log and Metric, then delete its root and
      catalogue entry rather than leaving a compatibility feature.
...
- [ ] Finish with exact catalogue-to-root equality, no production implementation
      duplicated in `platform/app`, and no package surface without a real caller.
