...
| `F-TYPECHECK-10`      | The whole-tree typecheck's last ten errors are decisions, each diagnosed in the drain (`baf5b18fed`): the model-provider connection test's three-verdict union collapsed to a boolean (the hook hits its exhaustiveness throw — Test Connection is non-functional); root.ts writes Monitor JSON through raw Prisma outside the sanctioned seam (wants `upsertForExperiment` on the monitor repository); `StoredObjectDeliveryPort`/`UploadTokenPort` have no non-throwing implementation behind four mounted REST operations (unfinished feature); two module identities for one `Project` declaration; the dataset backfill port over-promising Prisma's aggregate; and four contract/test-composition calls (`config.materialiser`, `tasks.generated`, `workflowEvaluation.service`, `pipelineRegistry`). Decide each before claiming a green tree. | Wave 3                              |
