...
**(h) THE FOUR REMAINING SYNTHESIZED WRAPPERS. IN PROGRESS 2026-09-02 (uncommitted at time of
writing): langy-conversation and scenario CONVERTED, gateway-spend and automation's settlement half
under way.** The method is the trace conversion's, applied verbatim: count the CALLS the pipeline's
...
PLATFORM: `runtime/worker/packaged-worker.capabilities.ts` lost the `langyConversation` and
`scenario` entries; `runtime/app/features/langy-analytics-event.clickhouse.adapter.ts` and its test
are deleted; `runtime/app/trace-read-derivation.adapter.ts` lost its scenario derivation.
