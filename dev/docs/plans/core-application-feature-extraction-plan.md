...
| `coding-agent`  | coding-agent sessions, conversations, transcript and pull-request usage                                          | `codingAgents`; `/api/coding-agent`                                                                                                                                       | `packages/features/coding-agent/{contract,server,web}`, Coding Agent routes and UI         | contract/server/web own portable reads and reusable presentation; app composition, durable producers, transcript/Trace collaboration and page/query composition remain                                               |
| `github`        | GitHub installation, webhook, repository and pull-request linkage lifecycle                                      | `github`; GitHub setup/install/webhook routes                                                                                                                             | `packages/features/github/{contract,server,web}`, GitHub routes and UI                     | contract/server/web own the capability; app routes and worker/eventing remain compatibility transports and process composition                                                                                      |
...
| `server/app-layer/{langy,topic-clustering}/**`                                                                                  | `langy/server` and `topic/server`; process transports and external clients remain adapters. Coding Agent server ownership is complete; its event producers and app transports remain in their process composition roots. |
