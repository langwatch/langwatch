...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| API physical activation        | `apps/api` has a real Node/Hono listener, typed config, request policy, graceful drain, Agent/Secret tRPC and all four Secret REST bases. It is still a library composition without an executable boot or production session/API-key adapters; the platform route graph remains live.                                                                 | Compose the concrete auth/session/API-key/audit adapters and executable boot, then migrate the remaining REST/tRPC route graph by vertical before deleting compatibility routers.               |
| Worker physical activation     | Topic commands and Trace processing registration are package-owned. Trace installs before Topic, Dataset durable payloads are parsed, normal and failed boots drain Eventing before feature and process resources, and one dead app bridge is deleted. Consumers remain disabled because the shared queue still includes every other legacy pipeline. | Move the remaining registry groups and concrete durable dependencies into the worker, then enable the one complete shared-queue consumer and delete displaced worker registrations.             |
...
| Configuration ownership        | UI owns the strict public schema and inert base64url codec. Eventing TTL and logger compatibility are now parsed at process composition and injected; Gateway pepper injection remains the last active package ambient-read cut. The combined boot schema and private public-config source projection remain in `platform/app`.                       | Finish Gateway, separate API/worker projections and executable install order, then delete legacy config only after invalid/missing/role-specific boot parity.                                   |
...
| Process observability adoption | Eventing resolves the workspace LangWatch SDK; logger construction receives typed semantic process config and equivalent repeated boot configuration is idempotent. Legacy web/worker/task/direct-script entry points configure before graph imports, while metrics/profiling and physical API/worker executable boot remain.                         | Preserve metrics, profiling, disabled/no-key behaviour and logger/tracer shutdown ordering in executable API/worker boot tests.                                                                 |
| Persistence foundations        | Prisma already owns explicit connection/readiness/migration/shutdown services. ClickHouse now owns typed fail-closed tenant routing, one cached client per endpoint and idempotent close-all, but physical vendor construction and legacy process adoption remain.                                                                                    | Compose the vendor factory, Prisma tenant directory, migrations and both process graphs, then delete the old global/lazy clients after caller parity.                                           |
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------- |
| `F-API-01`    | The checked-in branch OpenAPI artefacts are stale. Against `main`, `openapidiff` reports 129 changed operations, 30 added and five removed. The removed set is exactly GET/POST `/api/secrets` plus GET/PUT/DELETE `/api/secrets/{id}`; changed fields include 73 request bodies, 49 operation IDs, 26 responses, eight parameter sets, five descriptions and one tag set. Source/runtime aliases exist, so do not call artefact parity green until generation succeeds. | Wave 3 and Wave 9         |
...
| `F-API-06`    | `apps/api` now owns a callable listener but still has no executable boot and no concrete session/project-key/PAT/admin authentication, API-key ceiling/mark-used, rate-limit or audit adapters. It cannot replace the live platform API yet.                                                                                                                                                                                                                             | Wave 1, Wave 2 and Wave 3 |
...
| `F-EVENT-01`  | Eventing production typecheck and the changed 79-test slice are green, but the full suite still has pre-existing `StateProjectionStore.load`/`tryLoad` incompatibilities and one memory-store expectation that omits the returned `idempotencyKey`. Keep these diagnostics explicit until their owning projection/test cleanup; they did not block the injected runtime-policy cut.                                                                                      | Wave 1 Eventing cleanup   |
...
| `F-WORKER-01` | The shared `event-sourcing/jobs` queue contains every pipeline. Trace `assignTopic`, deferred origin, Dataset normalize and Topic are now package-owned, but a partial worker would still reject/redeliver every other legacy job. Keep consumers disabled until the complete registry is mounted.                                                                                                                                                                       | Wave 4                    |
...
listener and bounded HTTP drain in `f1baea7011`; `F-WORKER-02` is closed by the
drain ordering and first-error retention proof in `0d877db1d7`. Their remaining
executable-composition work is represented by `F-API-06`, `F-WORKER-01` and the
active process rows rather than keeping obsolete findings open.
