...
| UI and feature web     | `apps/ui` is the browser composition shell; reusable browser behaviour belongs in feature web packages. Agent web is the active extraction lane. | Finish the Agent web cut with its browser behaviour and coverage, leaving route and transport composition in `apps/ui`. |
