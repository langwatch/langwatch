...
**Snapshot:** committed tree at `b7244670ea`
...
| OTLP Ingestion                                      | OTLP trace, log and metric intake, validation, canonicalisation, sharding and collection                     | `otlp-ingestion/{contract,server}`, `apps/api`, `apps/worker`                  |
| Trace                                               | `features/traces-v2`, canonical trace storage, projections, reads, overlays, filters and trace UI            | `trace/{contract,server,web}`, process apps                                    |
...
| --------------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|             417 | Feature server packages: Automation, Enterprise Billing, Coding Agent, Evaluation, Experiment, Gateway, GitHub, Langy, OTLP Ingestion, Simulation, Suite, Topic and Trace |
...
The 128-file Trace pipeline needs an internal split. OTLP Ingestion owns raw
protocol input, validation, canonicalisation, caps and command sharding for all
three signals. Trace begins at canonical trace facts and owns stored trace
behaviour, projections and queries. Evaluation, Experiment, Simulation, project
metadata and broadcast reactions remain named feature or worker adapters.
...
|  05 | review      | Evaluation execution family           | Evaluation server now owns the processing pipeline, deterministic projections, retry-safe execution intent and Analytics-owned evaluation reads. Package checks pass: Evaluation 2 contract and 87 server tests; Analytics 2 contract and 19 server tests; focused app eventing 51 tests. Root review must verify the moved stores and registrations. The legacy evaluator execution engine remains behind one named port until Evaluator, Trace and Model Provider are ready; it is an explicit app residual, not a second implementation.                                                  |
|  06 | active      | OTLP Ingestion and Trace              | Commit `bf070ccd72` extracted the 57-file log/metric pipeline but used the reserved Telemetry name. Rename it to strict OTLP Ingestion and add raw trace intake before cutover. Trace keeps canonical storage, deterministic projections, queries and response compatibility. No Elasticsearch shape is retained; preserve every response field and keep `trace_analytics`, `trace_summaries` and timeseries rollups distinct.                                                                                                              |
...
The immediate integration order is **06, 05, 04, 02, 03, 01**. OTLP Ingestion
and Trace are the largest active cuts, so they land as independent reviewed
commits at their canonical boundary. Evaluation/Workflow follow from their
...
is roughly **three to five sustained migration days**, with OTLP Ingestion and
Trace the main uncertainty. The file count and completed chunks take precedence
