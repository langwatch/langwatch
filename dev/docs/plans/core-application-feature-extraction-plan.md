...
| Worker physical activation      | `apps/worker` owns an injectable typed boot/process boundary, structural `ResourceScope` borrowing, signals and fail-safe startup cleanup. Its production composition constructs the canonical PostgreSQL/ClickHouse Eventing graph plus one Redis/AWS/Group Queue foundation. Stored Object now owns scheme dispatch and destination policy, and Worker can adapt that runtime into Group Queue. Both Eventing consumer switches remain off because the shared queue still contains every legacy pipeline.                                                                                                                                  | Move the concrete storage provider factories and destination source into the executable Worker root, then in Wave 4 install the remaining registry groups and intent ports before enabling the one complete shared-queue consumer.              |
...
| Infrastructure clients          | Shared AWS credential/proxy/handler policy, Dataset S3 lifetime and NLP Lambda/CloudWatch pairs are process-owned. Langevals and Trace privacy clients have one typed graph. `4bba78994c` adds one lazy process-owned SES/SMTP/SendGrid/Resend mailer and an explicit Slack webhook client for automation test-fire. `11c84ce592` moves Stored Object registry/destination policy into its owner and makes the Worker queue adapter consume it.                                                                                                                                                                                              | Move the remaining concrete storage provider factories, tenant-dynamic Slack, Stripe, WebSocket and model-provider clients only with their owning process/feature callers.                                                                      |
...
| Mail delivery graph             | `4bba78994c` makes Better Auth/passkey registration factories, composes Auth and one lazy mailer on the request `App`, injects the mail delivery port into callers and registers provider shutdown once. Focused changed mailer/provider/runtime tests pass; one unchanged unsubscribe-token environment-mutation test remains red.                                                                                                                                                                                                                                                                                                          | Move the remaining mail configuration projection to the physical API root, remove residual raw app-env reads, and delete the app compatibility modules only with boot/config coverage.                                                          |
| Workspace reconciliation        | API/Worker lifecycle, UI foundations, physical config, AuthZ, Entitlement/Licensing, Auth/User/Mailer and Stored Object/Worker queue slices are committed. Unrelated Analytics/Dashboard, Gateway, Prompt, Evaluation, generated artefact, SDK, baseline and formatting changes remain in the shared tree and are not part of this checkpoint.                                                                                                                                                                                                                                                                                               | Keep attributing every later lockfile/baseline hunk to its owning slice, stage exact paths or hunks and leave unrelated work untouched.                                                                                                         |
...
| `F-WORKER-STORAGE-01` | `11c84ce592` gives Worker one AWS/Redis/Group Queue foundation and an adapter for the canonical Stored Object runtime, but no executable production composition supplies the concrete S3/filesystem/Azure factories yet. Those drivers and destination-source adapters remain app-owned, so this is a tested construction seam rather than a live Worker storage cutover.                                                                                                                                                                                                                                                                           | Wave 1 physical Worker activation   |
...
| `F-USER-AUTH-01`      | User email changes previously revoked browser sessions inside the old User implementation. Ops and two Enterprise SCIM callers now update through User without explicitly orchestrating Auth revocation. Characterise transaction/error ordering and add the cross-feature orchestration at those transports before calling email-change parity complete.                                                                                                                                                                                                                                                                                           | Wave 2 User/SCIM caller cutover     |
| `F-MAIL-01`           | The changed mailer/provider/runtime tests are green, but the unchanged `unsubscribeToken.unit.test.ts` case for an empty `NEXTAUTH_SECRET` remains red because its environment mutation does not reach the validated config module. Mail link/unsubscribe helpers also retain app-env reads until the private mail projection moves to the physical API root.                                                                                                                                                                                                                                                                                       | Wave 1 configuration/boot           |
...
#### Active Wave 1 foundation lanes at `11c84ce592`
...
3. **Infrastructure foundations committed in `036d93752f` and
   `11c84ce592`:** Worker owns one Redis/AWS/Group Queue graph and consumes the
   Stored Object-owned registry/destination policy. Concrete S3/filesystem/Azure
   provider factories and the executable composition input remain app-owned.
4. **Mailer committed in `4bba78994c`:** one lazy provider graph is composed
   with Better Auth and explicit callers. Stripe, tenant-dynamic Slack,
   WebSocket and model clients still require owning process adapters.

Current Wave 1 progress at `11c84ce592`:
...
- [ ] Finish Group Queue, storage and external client construction in the
      physical API/Worker roots. The Worker foundation and Stored Object runtime
      are committed; concrete storage providers and executable binding remain.
...
- [ ] Move storage, mail, Stripe, Slack, AWS, WebSocket, NLP/Langevals and model
      ...
      Trace privacy and mail have named runtimes; storage providers, Stripe,
      tenant-dynamic Slack, WebSocket and model clients remain.
...
cutover; the displaced credential-user module is deleted. API-key actor
semantics have additional characterization but no transport cutover. The
recorded Organization revocation and User email-change orchestration seams must
close with their owning callers. Organization/Project/Role preparations do not
