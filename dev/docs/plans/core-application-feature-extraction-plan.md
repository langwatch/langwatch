...
**Snapshot:** committed tree at `1d6fa819a9`
...
The working tree currently has 39 core feature roots and 50 catalogue entries.
No root under `packages/features` is uncatalogued. `auth` and `telemetry` are the
two declared core owners without roots; they are created only with their real
vertical slices. The temporary `otlp-ingestion` root and catalogue entry are
superseded by the Trace, Log and Metric ownership decision.

- [ ] Repair the current Trace handoff and every import of displaced app Trace
      sources before deleting anything else. The package command has no
      production caller, the old raw event still reaches no moved projection,
      and 38 production imports currently target deleted sources.
- [ ] Recut `otlp-ingestion` into Trace, Log and Metric, then delete its root and
      catalogue entry rather than leaving a compatibility feature. Its current
      `./testing` export also points outside the package's published `src` files.
- [ ] Add singular `log` and `metric` catalogue entries only when their production
      slices exist. Do not use `telemetry` for observability-signal code.
...
|  05 | repair      | Evaluation execution family           | Evaluation server owns the processing pipeline and deterministic projections, while Analytics owns evaluation reads. Stable provider, cost and reported-event keys now exist, package checks pass, and the retry test runs, but the second review still found three gates: the Automation dependency is a renamed two-callback holder rather than an Automation-owned eventing service; the restored ordering cases inspect SQL instead of proving which competing row wins; and the crash test does not cross the production receipt-finalisation/P2002 seam. Repair those exact gates before staging. The legacy evaluator engine remains one explicit app residual until Evaluator, Trace and Model Provider are ready.                                                                                                                                                           |
|  06 | repair      | Trace, Log and Metric                 | Commit `bf070ccd72` extracted the 57-file log/metric pipeline under the wrong owner. Current review confirms the new Trace command is not wired to either live OTLP route, the legacy app pipeline still processes production traffic, the app has direct migration compiler failures, and the horizontal `otlp-ingestion` package owns 179 mixed files. First land one real Trace vertical with both routes, canonical processing, durable events and exact existing-table projections; preserve every API field and `stored_spans`/`trace_analytics`/`trace_summaries`/rollup distinction. Then create separate Log and Metric verticals and delete the horizontal root, its ADR/spec and invalid test export. No Elasticsearch ingestion or `telemetry` naming is retained.                                                                                                                                                            |
...
|  11 | repair      | Automation, Notification and Presence | Automation package checks pass, but migration review rejected the cut: substantial trigger/delivery/settlement behaviour was merely moved into `platform/app/src/runtime/app/features/automation-adapters`; `apps/ui` still reaches back into `platform/app` and cannot typecheck independently; missing `BASE_HOST` now degrades to broken relative Slack links instead of failing boot; and the new boundary contains `any`, `Pick`, `Parameters`/`ReturnType` mirrors and a raw Prisma-shaped database port. Move the domain behaviour into Automation server, keep only thin process/UI composition, restore required semantic config, and close the independent UI check before staging. Presence web ownership remains complete.                                                                                                                                                |
...
|  16 | queued      | API composition                       | Move Hono/tRPC roots, middleware, static delivery, auth transport, OpenAPI and compatibility adapters to `apps/api`. Direct Hono routes must move onto `@langwatch/api`: REST keeps its URLs and verbs but handlers receive one schema-validated input and return schema-validated output. Middleware serializes every `HandledError` losslessly. Dashboard parsing keeps wire fields inert: links, actions, tips, trace URLs and long-form copy come only from one trusted remediation document keyed by a known error code, bundled or backend-pulled and cached; unknown codes get bounded generic presentation. Existing statuses, headers and complete JSON shapes remain compatibility contracts at the adapter.                                                                                                                                                                |
...
|  20 | active      | Feature Flag                          | Move the store, rules, cache and all named callers behind one `FeatureFlagService`; delete the app duplicate, hidden developer drawer and browser evaluation overrides. Authenticated transport exposes one validated flag-map query and derives the actor while authorising an explicit tenant target. Add bounded anonymous-browser evaluation for pre-auth surfaces using a random stored browser ID. A flag may be an experiment: users opt themselves in; authorised project/organisation owners set an exact-scope `inherit`/`enabled`/`disabled` policy; Ops availability and normal eligibility still win. The settings dropdown opens a small Experiments tab and shows a browser-local unread dot for a newly visible experiment. Persist only the discovery watermark locally; it never affects evaluation. Keep the Ops management page and existing transport behaviour. |
...
The immediate integration order is **06, 05, 04, 02, 03, 01**. Trace, Log and
Metric are the largest active cut, so they land as three independently reviewed
vertical commits. Evaluation/Workflow follow from their
current review, then Scenario, Gateway and the shared Model Provider cutover
are integrated without mixing their composition-root hunks.
