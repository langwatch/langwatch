...
**Snapshot:** committed tree at `0cf394039c`
...
At this snapshot it is **6,403 files**. `platform/app/src` contains 6,072.
...
|  12 | active          | Data and security utilities           | Commit `0cf394039c` moves retroactive mutation, progress and cancellation into the canonical Data Retention service, with one 19-table ClickHouse catalogue and unchanged tRPC response fields. Four legacy implementation/test files are deleted. Nine deliberate app residues remain: the transport, page shell, compatibility index, meter read/service, policy authz/read, resolver and legacy policy schema. Move the meter next, then place the transport/page/process wiring in the three process apps. Continue Data Privacy, Secret, Share and Stored Object afterwards. |
...
The reusable web slice and retroactive mutation slice are complete. Commit
`0cf394039c` deleted:
...
It added one temporary replay composition adapter and one transport parity
test, so the application moved from 6,405 to 6,403 files. Nine deliberate Data
Retention residues remain:
...
The next deletion batch is the meter service. The 19 managed retention tables,
12 metered tables and timeseries/analytics stores remain distinct parity
contracts. `trace_summaries`, `trace_analytics` and
