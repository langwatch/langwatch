...
| `evaluation` | evaluation definitions, execution, runs, results and DSPy evaluation steps | `evaluations`, `batchRecord`; legacy evaluation REST | `server/evaluations`, `app-layer/evaluations`, `app-layer/dspy-steps`, evaluation UI | the process owns one canonical service for execution, run persistence, trace reads, resolved deferred inputs and monitor performance; the compatibility read service and duplicate run/read/performance repositories are deleted, while the evaluator execution engine, stored-object adapter, DSPy steps and UI still need relocation |
