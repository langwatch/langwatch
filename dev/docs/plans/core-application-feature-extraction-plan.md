...
| Platform residue | **NONE. `platform/` is deleted** (Cutover C, 2026-09-03). 306 rows, `git diff --numstat -- platform` 0 insertions on every one. `pnpm install --offline` is green without it and `pnpm-workspace.yaml` never listed it. What survived moved out first — see the Cutover C record at the end of this document for the file-by-file table. | Nothing. The lane is closed. Cutover A has since repointed the OpenAPI target and the root `test:*` scripts, and **Cutover E has closed the rest** — the lockfile, the root `dev` / prepare / seed scripts, `pack-npm.sh`, the four repo-level guards and haven's seed presets (see its record at the end of this document). The one platform path left in a live file is `.gitleaks.toml`'s allowlist, cutover D's. |
...
## UI: `ui-feedback` reads the presentation registry, 2026-09-03

The client error-copy registry moved to `apps/ui/src/model/errors` in
`504d1517f7` and nothing read it. `apps/ui/src/behavior/ui-feedback.ts` — the
`UiFeedbackPort` implementation every moved family's `host.failed(...)` ends up
in — still carried the four-entry placeholder copy table its own header called
"deliberately minimal", so a customer read registered copy for
`insufficient_permissions`, `validation_error`, `not_found` and `rate_limited`,
and the action name plus one generic line for the other 490 (414 app codes,
52 generated Go, 28 generated node). This is the
slice that header named.

### What `ui-feedback` now does

One resolution, three rules, over the whole registry:

1. `readHandledError` lifts the payload off whichever boundary carried it —
   tRPC's `data.error`, a flat Hono REST body — and `explainHandledError` reads
   the code-keyed registry. Every code in `APP_ERROR_CODES`, `goErrorCodes` and
   `nodeErrorCodes` resolves; the four-entry table is deleted.
2. **Registry copy outranks the screen's `fallbackTitle` and `description`.**
   The registry names this exact failure; a screen names only the action. An
   unrecognised code inverts that, so the screen's title wins there, with the
   humanised code behind it.
3. **A server remediation tip is folded into the body when it adds something.**
   Same normalised comparison the platform used: `rate_limited`'s tip repeats
   its description and is dropped; `clickhouse_unavailable`'s "check the status
   page or contact support" is the only escalation path a customer is offered
   and is kept.

A failure with no handled payload keeps ADR-045's unknown state — the screen's
title, one calm generic line, the trace id, and nothing about the internals —
with one exception carried over from the platform: prose a non-5xx procedure
authored itself (`data.authored`, second-guarded by
`readAuthoredMessageOfUnhandled`) still reaches the reader, because replacing
"You've already used this invite" with "we've been notified" tells them to wait
for something that will never change.

The toast now carries `meta: { docsUrl, traceId }`, and
`ui/elements/ui-error-toaster.tsx` installs the Design System toaster with a
`renderMeta` that renders `ui/elements/ui-error-actions.tsx`: the docs link and
the copyable error id. Without it the trace id was resolved and then dropped on
the floor, which is the half of the unknown state that lets a customer quote the
failure to support.

### Screens

- **`packages/features/topic/web/.../topic-clustering.screen.tsx` — left as
  written.** It already hands the raw error to `host.failed` with a
  `fallbackTitle`; the server raises a plain `Error` there
  (`project.api.ts:375`), so the failure is genuinely unknown and its
  `description: "Please try again in a moment."` is exactly the channel that
  line is for. It now yields automatically the day that throw becomes a
  `HandledError`.
- **The `*-feedback.ts` re-binders — unchanged and already fixed.** Automations,
  gateway, ops and coding-agent each re-bind `toaster.create` /
  `useShowErrorToast` onto their host port, and every host adapter in
  `apps/ui/src/features/**` forwards `failed` to this capability. Fixing the
  capability fixed all four families and every screen behind them; the "what
  does not come with them, and is a later slice" paragraph in each of those
  headers is now stale copy, not a live absence.
- **Three `Failed to copy` clipboard sites — left as written**
  (`api-key`, `authorize`, `onboarding` host adapters). A clipboard refusal has
  no code and never crosses a wire, so the screen's own `description` is the
  right channel and is untouched by the registry.
...
- **30 files in eight `packages/features/*/web` packages import
  ...
  family a host port, which is a family-move slice rather than a copy one.
- **`<HandledErrorAlert>`, `<FormServerError>` and `applyHandledErrorToForm`
  have not moved.** So there is still no inline counterpart to the toast, and
  `meta.fieldErrors` is not mapped onto form fields in `apps/ui` — a rejected
  submit gets `validation_error`'s registry copy naming the user-visible fields
  ("Check your input. The name isn't valid."), which is a large improvement on
  the slug but is not the same as marking the field. The form bridge is the next
  slice of this harvest.
- **`isHandledByGlobalHandler` has not moved**, so the dedup that stops a
  license-limit failure toasting on top of the modal it already opened is not
  applied here. Nothing in `apps/ui` opens such a modal yet.

### Judgment calls

- **`UI_FAILURE_COPY` deleted rather than kept as an override.** All four
  entries are strictly worse than the registry's: `insufficient_permissions`
  names the required permission from `meta`, `validation_error` names the
  user-visible fields that were rejected, and `not_found` / `rate_limited` say
  the same thing in the registry's voice. An override table that overrides
  nothing is a second authoring surface for copy, which is what the registry
  exists to prevent.
- **`UNKNOWN_UI_FAILURE_DESCRIPTION` deleted too.** It was a second generic
  line ("Something went wrong on our side.") beside the registry's own
  (`UNKNOWN_ERROR_PRESENTATION`). One unknown state, one place it is written.
- **`readUiFailureCode` deleted.** It read `error.error` as a code with no
  validation, so an unhandled Hono failure — which sends
  `{ error: "Internal server error" }` — would have been treated as a code.
  `readHandledError` requires a slug or a known code and is the reader now.
- **`title` added to `UiFailureNotice`.** The automations, gateway, ops and
  coding-agent re-binders all already forward a `title` hard override, and the
  port silently dropped it: structural typing let it through and nothing read
  it. It is honoured now, and outranks the registry, matching the platform
  helper it was copied from.
- **An empty description stays empty for a registered code.** The registry's
  rule is that an empty description beats padding, and six codes
  (`api_key_already_revoked`, `api_key_not_found`, `api_key_not_owned`,
  `evaluator_not_found`, `filter_field_unknown`, `invalid_email_recipient`)
  deliberately have title-only copy. Falling through to the screen's line there
  would have re-padded exactly the entries someone chose not to pad.
- **The tip-folding rule lives inside `ui-feedback.ts`, not in a harvested
  `resolve-error-copy.ts`.** The platform module exists because three surfaces
  render it; here there is one. It is extracted when `<HandledErrorAlert>`
  arrives and needs the same answer, not before.
- **`ErrorActions` ported lean.** The platform component takes `color` /
  `accentColor` because the inline alert paints a saturated fill. No such
  surface is here, so the props are dropped rather than carried unused.

### Gates

- `apps/ui`: `tsc --noEmit -p tsconfig.json` **6 errors**, all in
  `packages/features/workflow/web` (`TS7006` on untyped `find`/`map`
  parameters), **0 in this lane's files**; `tsc --noEmit -p tsconfig.test.json`
  **11 errors** in 5 files, the same six plus five pre-existing rows in
  `tests/trpc-agent-browser.adapter`, `tests/ui-session-client` and
  `tests/ui-session.integration`, **0 in this lane's files**.
- `apps/ui`: `vitest run` **100 files / 999 tests passing**, from a baseline of
  99 / 989 — this lane adds one file and ten tests, and the
  `no-raw-error-toasts` guard, which scans this package, stays green.
- `architecture-lint` (`--no-declarations --no-legacy-application-migration`):
  **92 findings under `apps/ui`, byte-identical to the baseline** — no rule
  gained an occurrence.
- `oxfmt` / `oxlint`: clean on every file this lane touched.
...
## Cutover E — closing cutover A's and cutover B's lists, 2026-09-03

The two preceding lanes each handed the other a list of one-line changes in
files it was not allowed to edit: cutover A's "What this lane needs from
others" (three items) and cutover B's "What cutover A still has to wire" (six).
This lane is both lists, plus the four repo-level guards cutover A recorded as
lost and haven's seed presets. Nothing under `platform/` exists to touch.

```
  BEFORE                                  AFTER

  pnpm dev                                pnpm dev
    └─ pnpm --parallel --filter …           └─ bash dev/scripts/dev-stack.sh
         no port pre-flight                      ├─ ensure-ai-gateway-secrets
         no REDIS_DB_INDEX slot                  ├─ check-ports
         no BASE_HOST alignment                  ├─ REDIS_DB_INDEX per PORT slot
         no gateway secrets                      ├─ BASE_HOST / NEXTAUTH_URL
         no Go lanes                             └─ ui · api · workers · gateway · nlpgo
                                                    (concurrently, a ROOT devDependency)

  apps/{api,worker}  --env-file .env      --env-file .env  --env-file .env.portless

  haven db seed demo                      haven db seed demo
    ├─ prisma:seed            ✓             └─ prisma:seed            ✓
    ├─ seed:retention         ✗ gone
    ├─ seed:sample-traces     ✗ gone      haven db seed mass
    └─ seed:realistic-platform ✗ gone       └─ refused BY NAME, saying why
```
...
| File | Change |
| --- | --- |
| `pnpm-lock.yaml` | Regenerated. The stale `platform/app` importer and `apps/api`'s `prisma` dependency were already reconciled by an earlier lane — `pnpm install --frozen-lockfile --lockfile-only` was green on arrival — so this lane's own contribution is three lines: the root `concurrently`. **`pnpm install --frozen-lockfile` now passes.** That was cutover A's highest-priority follow-up and every CI job, both Dockerfiles and the npx first boot were failing on it. |
| `package.json` (root) | `dev` is `bash dev/scripts/dev-stack.sh`. `concurrently` (`^9.1.2`, the version `apps/server` already pinned) is a root devDependency, because `dev-stack.sh` runs `pnpm -s exec concurrently` from the workspace root. `ensure:ai-gateway-secrets` exists again as a named script (`bash dev/scripts/ensure-ai-gateway-secrets.sh`) and is the first step of `start:prepare:files`, so a `make quickstart` run reaches the containers with its three gateway secrets generated (issue #3902) instead of crashlooping on the env validator. |
| `apps/api/package.json`, `apps/worker/package.json` | Every `tsx` entry point takes `--env-file-if-exists=../../.env.portless` after `../../.env` — five scripts on the API (`start`, `dev`, and the ClickHouse / LangWatchQL / model-provider tasks), two on the worker. Same order the platform manifest used, so the overlay still wins. A hand-run `pnpm --filter @langwatch/platform-api dev` inside a portless worktree now reads that worktree's ports instead of the committed 5560. |
| `dev/scripts/pack-npm.sh` | The four `--include=*.cjs.map` re-includes named the monolith's `dist/server` bundles (server, workers, task, scenario child); none of those bundles exists, both Node processes ship as source through `tsx`, and the one bundle left (`apps/server/dist/cli.cjs`) is built with `sourcemap: false`. Removed, with the rule that replaces them stated where the next bundle author will read it. The exclusion guard's matching comment corrected. Everything else cutover A asked for was already true: the anchored excludes are `apps/ui/e2e/auth.json` and `packages/prisma-client/prisma/db.sqlite*`, and `apps/server/distribution-files.json` names `apps/{api,ui,worker}`, `packages/` (so the Prisma schema and history ship) and `apps/server/{dist,src}` with no monolith entry. |
| `apps/server/test/pack-npm-filters.test.ts` | Repointed off the monolith, which is what made it the one red test in cutover A's run (153/154). Its fixture distribution list is the three applications plus `packages/`; the collision probes are an `auth.json` and a `reports/` directory in the new trees; and the inverse probe is now exactly the two ANCHORED_EXCLUDES plus a stray log. **154/154.** |
| `tools/thuishaven/app/db.go` | `demo` keeps its env switches and loses its ingest list. `traces` and `mass` are gone from the registry and named in a new `retiredSeedPresets` map, so `haven db seed mass` fails saying the preset is retired and why — not "unknown seed preset", which reads as a typo to someone who has been typing it for months. `seedRetentionStep` and `defaultMassSeedMonths` deleted with them. |
| `tools/thuishaven/app/{db_test.go,play_seed_test.go}` | A `withIngestPreset` test helper registers an ingest-carrying preset for the length of one test and restores the registry after. Every assertion about the ingest machinery — the stack-not-running refusal, the not-answering refusal, the baseline-fallback refusal, the loopback endpoint and ingestion key, step order, the abandon-after-failure path, the sandbox that never serves — keeps running against it instead of being deleted with the data. |
| `tools/thuishaven/app/orchestrator.go`, `domain/overlay.go` | Comments that named the deleted scripts as live steps now say they are absent and what that costs (nothing raises the dev stack's 7-day retention any more, and nothing needs it to). |
| `tools/thuishaven/README.md` | Preset lists in the command summary, the `play --seed` summary and the Seeding section; a paragraph on why `traces` and `mass` are refused and what the surviving seam is. |
| `specs/setup/haven-seed-presets.feature` | The mass-preset and seed-retention sections deleted — seven `@unit` scenarios describing pure generators (`scripts/seed-lib/{mass-timeline,mass-metrics,retention}.ts`) that went with the platform application; the parity checker had them at **5/12 bound**. "The demo preset needs the stack for its traces" is replaced by "A preset that ingests needs the stack up" (the seam, not the data) and a new "Retired presets say so rather than reading as a typo". **6/6 bound.** |
| `specs/setup/haven-play.feature` | One step: a seeded sandbox comes up "with the demo preset's content", not "with its sample traces ingested". 27/27 still bound. |
| `packages/prisma-client/prisma/seed.ts` | One comment: the prompt-tag upsert no longer explains itself by naming `seed:langy-prompts`. |

### The four repo-level guards were already ported — verified, not re-ported

Cutover A recorded `env-example-sentinels`, `langyagent-shell-tools`,
`no-postinstall-network` and `noBinarySourceFiles` as lost with
`platform/app/src/__tests__/`, and suggested `apps/api/src/__tests__/` as their
home. Cutover C had in fact already moved all four into
`packages/architecture-lint/tests/` in the same change that deleted them, which
is the better home: their subjects are repository-level files, not API files.
Each resolves the repository root correctly and has a non-empty subject —
`.env.example`'s three gateway keys, `infra/docker/Dockerfile.langyagent`'s four
`apt-get install` blocks, 191 tracked manifests, 15,482 tracked source files.
Two of them scan strictly MORE than the platform originals did: the postinstall
guard read one manifest and now reads every tracked one, and the NUL-byte guard
resolved to `<repo>/platform` and now reaches the real root. 14 tests, green.

### Judgment calls, recorded

1. **The five `seed:*` ingest scripts get no root home, because there is
   nothing to point one at.** `seed-sample-traces.ts`, `seed-realistic-platform-data.ts`,
   `seed-mass.ts`, `seed-retention.ts` and `seed-langy-prompts.ts` were deleted
   with `platform/app/scripts/`, along with the `seed-lib/` generators behind
   them. A root script naming a missing file is a slower way to discover the
   same absence. `prisma:seed` — the one that survived, as
   `@langwatch/prisma-client`'s `db:seed` — is already wired at the root.
2. **`traces` and `mass` are refused by name, not silently dropped.** Same
   choice cutover B made for `±workers`: "unknown preset" reads as a typo and
   sends a developer looking for their own mistake. `demo` survives because its
   env half does real work — `HAVEN_SEED_PRESET=demo` is what makes
   `prisma/seed.ts` call `seedDemoPlatform`, which is the prompt, the HTTP agent
   and the dataset. `traces` and `mass` had no env half worth keeping: `traces`
   was ingest only, and `mass`'s `HAVEN_SEED_MONTHS` has no reader left, so both
   collapse into `demo` or into nothing.
3. **The ingest MACHINERY stays; only its data is empty.** `seedPreset.ingest`,
   `runSeedIngest`, `runIngestScript`, `liveSeedEnv`, `ingestPlaySeed` and
   `PlaySeedFailure` are ~150 lines that run a list of pnpm scripts against a
   live stack's collector, with a real liveness contract and five bound
   scenarios in `haven-play.feature`. Deleting them was the alternative, and
   `ensureAPIBundle` is the precedent for deleting — but that was permanent,
   and these seeds are a NAMED ABSENCE this migration intends to restore. An
   empty list is a no-op, not a silent failure, and the machinery stays honest
   because the tests exercise it against a preset they register themselves. The
   `ingest` field's comment states this so it cannot be mistaken for a wiring
   bug, and `retiredSeedPresets` says to delete an entry once its data is back.
4. **`prepare:db` was NOT added.** Cutover B offered it as a convenience so the
   Go side could name one script instead of composing a shell. Both scripts it
   would wrap (`prisma:migrate`, `clickhouse:migrate`) already exist at the
   root, and `prepareDBShell` documents in place why it names two: two stores,
   migrated by two different applications, chained with `&&` so a failed Prisma
   migration never lets the ClickHouse one report success. A third name for the
   same two calls adds an indirection that hides that.
5. **`dev:app` and `dev:concurrent` were left alone** even though `CLAUDE.md`
   documents only four dev scripts. `dev:app` is live — `dev/scripts/dev.sh`
   runs it for the `dev-infra` preset, where the workers are in a container and
   ui + api run on the host — and `dev:concurrent` is named in ADR-004's
   amendment. Neither is a monolith leftover.
6. **The two `.gitleaks.toml` platform allowlist paths were left alone.** Root
   dotfiles are cutover D's; they are the last live-file `platform/app`
   references in the tree.

### Named absences

- **The two unformatted files this lane edited stay unformatted.**
  `apps/server/test/pack-npm-filters.test.ts` and
  `packages/prisma-client/prisma/seed.ts` both fail `oxfmt --check` at `HEAD`,
  before any edit of this lane's (verified by stashing them and re-checking).
  Reformatting either would produce a whole-file diff across two lanes' work
  for a condition this lane did not create.
- **`haven-play.feature`'s five ingest scenarios now describe a seam no preset
  reaches.** They are honestly bound and the code path is real, but until a
  seed script exists again, no `haven play --seed <preset>` will run one.

### Gates

- `pnpm install --frozen-lockfile`: **green**, 183 workspace projects,
  "Lockfile is up to date, resolution step is skipped". `pnpm -s exec
  concurrently --version` → 9.2.4 from the workspace root.
- `apps/server`: `vitest run` **154/154** (23 files). Two `cli-doctor` failures
  appear under a shell exporting `FORCE_COLOR`; they are the shell, not the
  suite, and the run above is with it unset.
- The pack guard was sabotage-checked: deleting `apps/ui/e2e/auth.json` from
  `ANCHORED_EXCLUDES` turns the inverse probe red and nothing else. It asserts
  what it claims to.
- `@langwatch/architecture-lint`: `vitest run` **37 files / 479 tests, all
  passing**, including the four repo-level guards (14 tests).
- `check-feature-parity`: `specs/setup/haven-seed-presets.feature` **6/6
  bound** (from 5/12), `specs/setup/haven-play.feature` **27/27 bound**.
- `tools/thuishaven`: `go build ./...` clean, `go test ./...` **all packages
  ok**, `gofmt -l .` empty.
- `bash -n dev/scripts/pack-npm.sh`: clean. `make -n` on `start`, `quickstart`,
  `quickstart-help`, `down`, `haven`, `service`, `tsc-watch`, `refresh-dev-s3`
  and `sync-all-openapi`: all resolve; `make start` is `pnpm dev`.
- Every changed manifest parses: root `package.json`, `apps/api/package.json`,
  `apps/worker/package.json`.
- Per-application `tsc --noEmit`, source project then test project. The counts
  are a moving target — sibling lanes were editing `packages/features/*/web` and
  `packages/features/trace/server` while these ran — so each error is named with
  its owner rather than only counted.

  | Application | Source | Test | The errors, and whose they are |
  | --- | --- | --- | --- |
  | `@langwatch/worker` | **0** | **0** | — |
  | `@langwatch/platform-api` | **1** | **6** | Both projects: `apps/ui/src/behavior/public-config.ts:87` `TS2584 Cannot find name 'document'`, reached through the static handler import — the web lane's, and the one cutover A recorded. The test project adds: `src/app-static/__tests__/app-static-handler.unit.test.ts:167` `TS2304 Element` (a DOM name in a non-DOM test project, from cutover A's own app-static work); three `TS2322` at `src/app-trpc/__tests__/app-trpc.features.unit.test.ts:420-422` against `packages/api`'s `TRPCRootObject` shape; and `src/app/api-packaged-rest.composition.ts:38` `TS2724 extractInlineMediaFromEvent` — a `@langwatch/trace-server` export being renamed in the working tree as this ran. |
  | `@langwatch/ui` | **6** | **11** | Six `TS7006` implicit-any in `packages/features/workflow/web/src/{behavior/optimization_studio/use-component-version,ui/sections/optimization_studio/history}.tsx` — the web-toasts lane's. The test project adds five in `apps/ui/tests/{trpc-agent-browser.adapter,ui-session-client}.unit.test.ts` and `ui-session.integration.test.tsx`. |

  **No error, in any of the six runs, is in a file this lane wrote.**

## The worker's tenancy graph, and AuthZ's metrics as a port, 2026-09-03

**The wall was one required argument.** Every model path in `apps/worker` — the
four questions topic clustering asks, an online evaluation's `X_LITELLM_*`
environment, and the model a Langy conversation's title is written by — resolves
through `ModelProviderService`, and `tryCreateWorkerModelProviders` refuses to
compose one without a tenancy graph. So production ran with
`withoutModelGateway("no-tenancy")` in its logs, clustering refusing every model
resolution by name, and every conversation keeping whatever title it was given.
The chain behind that was recorded twice (db06a4adb3, b79df709b6) and both times
ended in the same place: `PostgresOrganizationAdapter` needs an `AuthzService`
and an `AuthzGrantsService`, only `PostgresAuthzAdapter` builds them, and its
options required a `cutoverReporter` and a `revocationTelemetry` whose only
implementations took `prom-client` counters off a `Registry` this process
deliberately does not hold.

### The metric registry stopped being a precondition

`packages/features/authz/server/src/ports/authz-metrics.port.ts` is new:
`AuthzMetricsPort` (`revocationCounter(reason)` and
`engineGateReadFailureCounter()`, both answering `{ inc(): void }`) plus
`UncountedAuthzMetrics`, the default. `PostgresAuthzAdapterOptions` drops
`cutoverReporter` and `revocationTelemetry` for one optional `metrics`, and
BUILDS both collaborators itself over whatever it is given —
`ObservabilityAuthzCutoverAdapter` still warns on every failed migration-state
read and `ObservabilityAuthzRevocationAdapter` still records every queue-
bypassing projection write, because those two describe WHEN a counter moves and
that must not be described twice. Neither imports `prom-client`.
`ObservabilityAuthzMetricsAdapter` now extends the port and is the ONE module in
the feature that names the library; the two series and their help strings are
unchanged, so no dashboard or alert moves.

`apps/api/src/app/api-authz.composition.ts` keeps passing its own registry, one
line further in: `metrics: ObservabilityAuthzMetricsAdapter.create({ registry })`
where it used to build the two reporters. That is the only edit this lane made
outside `apps/worker` and the AuthZ package.

Recorded rather than done: `prom-client` stays a `dependency` of
`@langwatch/authz-server`. Removing it would mean the metrics adapter leaves the
package, and the only place it could go is `apps/api` — which is three concurrent
lanes' file. What the port buys is what mattered: nothing on the required path
imports a registry, so a process that renders no scrape endpoint composes the
same AuthZ graph.

### What the worker now composes, and from where

`apps/worker/src/app/worker-tenancy.composition.ts` returns ONE `WorkerTenancy`
— `projects`, `organizations`, `authorization`, `grants` — mirroring
`apps/api/src/app/api-tenancy.composition.ts` shape for shape, with nothing
imported from `apps/api`:

| Member | Built by | Collaborators |
| --- | --- | --- |
| `authorization`, `grants` | `PostgresAuthzAdapter` | the typed client, the queue's Redis as the epoch cache, `KsuidAuthzBindingIdAdapter`, NO metrics |
| `organizations` | `PostgresOrganizationAdapter` | the three packaged identity minters, the two AuthZ services above, the stored-secret cipher, `PersonalWorkspaceDiagnosticsAdapter` |
| `projects` | `PostgresProjectAdapter` | `ProjectCredentialsAdapter`, `organizations`, this process's logger |

Every persisted format below the two adapters is the feature package's own —
the ksuid prefixes, the slug pre-pass, `sk-lw-` plus 48 characters — so this
root restates none of them. `keyMap` and `storedObjects` are absent for the API
tier's reason: both are reach-outs a project DELETION makes, and this process
deletes none.

`worker.config.ts` gains `authz.{epochCacheEnabled, demoProjectId}` from
`AUTHZ_EPOCH_CACHE` and `DEMO_PROJECT_ID`, at the API tier's spellings and with
its exact rule ("1 or true, anything else off"; blank is not a project id). Two
processes disagreeing about whether a permission read may be served from the
epoch cache is worse than either answer. `DEMO_PROJECT_USER_ID` is deliberately
NOT read: it is attribution for work the demo project does on a request path,
and this process serves none.

`WorkerProductionCompositionOptions` gains `connection?: PrismaConnection` — the
SAME object `database` already carries, under a second name. `database` is the
structural intersection every feature narrows for itself, which is what lets a
composition test hand in the delegates it exercises; the organization and
project adapters do not take a structural type, they declare
`database: PrismaClient`. Passing the connection through means no cast at the
repository seam, which is what the typed-Prisma rule asks for.
`WorkerStandaloneComposition` supplies it; a graph without it composes no
tenancy and says so.
...
| Absence | Before | After |
| --- | --- | --- |
| `withoutModelGateway("no-tenancy")` | reported by EVERY deployment | reported only where the root was given no typed connection |
| `withoutClusteringModels()` | conditional, but its condition never held in production | the condition now holds: a deployment with a database and `CREDENTIALS_SECRET` composes the gateway and clustering resolves through it |
| `withoutTitleGeneration()` | conditional on a gateway that was never composed | conditional on the deployment alone (a database, and `LANGWATCH_NLP_SERVICE`) |
| `withoutModelTranslation()` | the same | the same |

One absence is NEW and it is this lane's:
`WorkerTenancyAbsenceReportPort.withoutGrantWrites()`, logged at `info`. This
process registers the grants pipeline as a CONSUMER —
`AuthzWorkerFeatureInstaller` folds grant events into the read model and the
audit trail and resolves no command senders — so a grant change for an
organization ALREADY ON THE LEDGER refuses with `authz_ledger_unavailable`. An
organization that has not cut over is unaffected: the ledger writer takes its
imperative Postgres path for those, here as on the API tier. Nothing in this
process makes either write today; the write half is composed because
`PostgresOrganizationAdapter` requires it structurally.

**Worker absence count** (`grep -rn "abstract without" apps/worker/src`): **26
declared before, 27 after.** The count going up while the biggest blocker in the
worker blocker graph comes down is the same honest shape the model-gateway lane
recorded: a capability that could not be composed at all had nothing to declare.

### Judgment calls

**The grants dispatcher refuses immediately rather than waiting.** The packaged
`EventingAuthzCommandDispatcherAdapter` waits five seconds for a `connect` that
a producer's registration performs, then raises `AuthzLedgerUnavailableError`.
That is right where the connection is merely LATE. Here it never comes, so
`WorkerUnregisteredAuthzGrantCommands` raises the same contract error at once —
same code for the caller, no five-second stall on the way to it.

**The worker does NOT connect the dispatcher, although it could.** Its
registration returns command senders, so wiring them would make grant writes
work. `AuthzWorkerFeatureInstaller` records the opposite decision by name ("there
is no `connect` half here any more … this process writes no grants"), and
nothing in this process makes such a write. Reversing another lane's recorded
decision to enable a call site that does not exist is how a graph acquires a
second producer for one aggregate.

**`withoutSessionKeyMint()` was NOT closed, and the reason is not scope.** Two of
the three things a Langy session-key mint needs now exist here — effective
permissions and the org-membership read both come off this graph. The third is a
grant ATTACH, which is exactly what the paragraph above refuses for an enrolled
organization. Composing the minter over that would trade a named absence for a
mint that looks configured and fails on precisely the customers who have
migrated. It also wants an API-key pepper this process does not read.

**`tryCreateWorkerLangyTitleModel` still takes the READ half of Project**
(`ProjectMetadataService` from `createWorkerTraceCapabilityServices`) rather than
the wide `ProjectService` this lane composes. A model cascade asks a project
directory for nothing more, and a path that asked for the wide service would be
claiming a dependency it does not have.

**The tenancy graph is composed unconditionally on the cipher**, unlike the model
gateway, which gates on it. A deployment with no `CREDENTIALS_SECRET` gets a
tenancy graph whose organization-settings cipher refuses by name on the two BYOC
fields — which is the behaviour `resolveWorkerStoredSecretCipher` already
documents for every other vertical that reads under it — rather than no
organization service at all.

**`createWorkerProcessDatabase` grew nine delegates.** The model-provider
repositories check at CONSTRUCTION that the client names their models, so the
first composition test to supply a connection failed on `modelProvider`,
`gatewayChangeEvent`, `modelDefaultConfig` and `modelDefaultConfigScope`. That
check is the point of the double's own docstring — it is the answer to "which
tables does one worker process touch" — so the tenancy models (`organization`,
`group`, `roleBinding`, `auditLog`, `organizationAuthzMigration`) went in beside
them.

**One pre-existing test expectation was repaired**, not scrubbed:
`worker.config.unit.test.ts` was missing `infrastructure.modelProvider.nlpServiceUrl`
from the default-config assertion and had been failing in the working tree since
b79df709b6. It is asserted alongside this lane's `authz` block.

### Gates

`cd apps/worker && pnpm exec vitest run`: **60 files / 450 tests, all passing**
(working-tree baseline 59 / 445 with one failure, the `nlpServiceUrl`
expectation above; the committed baseline this lane was handed is 59 / 440).
This lane adds `src/app/__tests__/worker-tenancy.composition.unit.test.ts` with
5 and two model-gateway scenarios to `worker-production.composition.unit.test.ts`.
`tsc --noEmit` and `tsc --noEmit -p tsconfig.test.json` both **0 errors**.
`apps/worker/src/features/job-registry.json` **byte-unchanged**.

`@langwatch/authz-server`: `vitest run` **47 files / 461 tests, all passing**
(baseline 47 / 460; this lane adds one scenario). `tsc --noEmit` and
`tsc --noEmit -p tsconfig.test.json` both report the SAME single pre-existing
error, `src/repositories/prisma/__tests__/prisma.authz-audit.repository.unit.test.ts:44`
(`InMemoryAuditTable` against the generated `AuditLogDelegate`), in a file this
lane did not touch.

`apps/api`: `tsc --noEmit` **1 error**, `../ui/src/behavior/public-config.ts:87`
reaching for `document` — the same one cutover A and the identity lane both
