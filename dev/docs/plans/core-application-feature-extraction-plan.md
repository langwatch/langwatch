...
|  05 | repair      | Evaluation execution family           | Evaluation server owns the processing pipeline and deterministic projections, while Analytics owns evaluation reads, but review found three gates before staging: the effect executor can repeat evaluator work and an unkeyed cost write after redelivery; app composition still passes callback bags and inferred `Parameters`/`ReturnType` contracts; and the moved Analytics tests dropped material ClickHouse ordering, time-zone, window, insert and error-log parity. Repair those boundaries and restore the lost scenarios, then rerun package and app checks. The legacy evaluator engine remains one explicit app residual until Evaluator, Trace and Model Provider are ready.                                                                                                                                                                                            |
|  06 | active      | Trace, Log and Metric                 | Commit `bf070ccd72` extracted the 57-file log/metric pipeline under the wrong owner. Recut it into three vertical features: each signal owns intake through processing and its existing-table projections. Simplify Trace's helper-service and port explosion without splitting its lifecycle across horizontal packages. No Elasticsearch shape is retained; preserve every response field, durable event/version, correlation rule, and the distinction between `trace_analytics`, `trace_summaries` and timeseries rollups.                                                                                                                                                                                                                                                                                                                                                        |
...
|  11 | review      | Automation, Notification and Presence | Automation deleted 142 displaced app files across its old feature, app-layer and event-sourcing roots. Contract 261 tests, server 119 tests and moved web 9 tests pass; root review is pending. A 29-file runtime delivery adapter bundle remains because reports, mail, graph routes, bug reports and API callers share it. That is the next named delivery/worker boundary, not permission to leave Automation business logic in app composition. Presence web ownership remains complete.                                                                                                                                                                                                                                                                                                                                                                                          |
