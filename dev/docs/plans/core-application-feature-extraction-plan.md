...
| 6     | ~~Inject mail runtime configuration~~ **LOST, redo**            | `F-MAIL-01`                     | Ready, and closes `F-MAIL-01`: the empty-`NEXTAUTH_SECRET` case now exercises the real path because the secret is a parameter.           |
...
| 9     | SCIM and Ops email-change revocation         | `F-USER-AUTH-01`                | Unblocked: `F-LOCK-01` is closed. Ship the two halves together with `@scenario` bindings.                                                          |
