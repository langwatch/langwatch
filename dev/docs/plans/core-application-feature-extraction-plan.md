...
| Configuration ownership        | UI owns the strict public schema and inert base64url codec. Eventing, logger and Gateway virtual-key policy are parsed at process composition and injected. Production reusable-package source has no executable ambient env read, and the architecture rule governs every `packages/**/src` production module. The combined boot schema, queue policy, telemetry boot and private public-config source projection remain in `platform/app`. | Commit queue and telemetry projections, then separate API/worker executable install order and delete legacy config only after invalid/missing/role-specific boot parity.                        |
...
| Persistence foundations        | Prisma owns explicit connection/readiness/migration/shutdown services, but 66 production platform files still import the global proxy. ClickHouse package connections are process-owned; the live explicit runtime cutover is under review. Eventing server adapters are injected, but platform persistence callers and harnesses remain.                                                                                                    | Commit the ClickHouse runtime and fail-closed ProcessStore cuts, then introduce process-owned Prisma without moving feature repositories or enabling consumers.                                 |
...
| `F-EVENT-01`  | Eventing production source is package-owned, but the full suite still has pre-existing `StateProjectionStore.load`/`tryLoad` incompatibilities and one memory-store expectation that omits the returned `idempotencyKey`. ProcessStore fail-closed work must preflight before registration mutation and expose memory storage only through named test/local factories before commit.                                                                                                                                                                                                                                                                | Wave 1 Eventing cleanup   |
...
| `F-CONFIG-01` | The current working tree has 61 production files importing `env.mjs` and 98 production files mentioning `process.env`, with overlap. Import-time telemetry, scenario-child and evaluation-concurrency reads move first; SSO is Wave 2. Preserve database empty fallback, credential/auth-secret chains, privileged internal-route secrets, mail unsubscribe differences and storage unsafe/test gates before deleting the compatibility proxy.                                                                                                                                                                                                      | Wave 1                    |
| `F-QUEUE-01`  | The live Group Queue reads three codec/concurrency values directly, while validated tenant/global dispatch limits are not supplied and package defaults silently apply. Parse all five once into typed process config, keep the platform App as sole live registry owner and do not enable partial Worker consumers.                                                                                                                                                                                                                                                                                                                                | Wave 1                    |
| `F-PRISMA-01` | `@langwatch/prisma-client` has explicit connection/readiness/shutdown services, but `server/db.ts` still owns an env-derived `globalThis` proxy used by 66 production files. The legacy scope deadlock is fixed; process-owned adoption must inject one connection, remove App-level Prisma shutdown and retain only a construction-free compatibility binding until callers move.                                                                                                                                                                                                                                                                  | Wave 1                    |
