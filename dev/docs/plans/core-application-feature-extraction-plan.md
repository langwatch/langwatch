...
| Worker physical activation     | `apps/worker` owns an injectable typed boot/process boundary, structural `ResourceScope` borrowing, signals and fail-safe startup cleanup. Every installer must now complete before Eventing readiness, and a failed registration drains the partial graph. Topic commands and Trace/Dataset registrations are package-owned, but consumers are false-only because the shared queue still contains every other legacy pipeline.                 | Move the remaining registry groups and concrete PostgreSQL/Redis/ClickHouse/Group Queue dependencies, then enable the one complete shared-queue consumer and delete displaced registrations.    |
...
| Configuration ownership        | UI owns the strict public schema and inert base64url codec. Eventing, logger, telemetry, Gateway virtual-key and all five Group Queue values are parsed at process composition and injected. Production reusable-package source has no executable ambient env read, and the architecture rule governs every `packages/**/src` production module. The combined private boot schema and public-config source projection remain in `platform/app`. | Define the separate API/worker executable projections and delete legacy config only after invalid, missing and role-specific boot parity.                                                       |
...
| Persistence foundations        | Prisma, Redis and ClickHouse now have explicit App/task construction and exact shutdown owners. `server/db.ts` is construction-free, though the committed non-test lexical sweep still finds 61 platform files mentioning the compatibility binding. Eventing registration rejects process managers without an injected ProcessStore; four live platform ProcessStore constructions and the full registry remain.                               | Replace the four live Eventing persistence graphs and move the complete shared registry. Keep consumers disabled until that graph installs atomically.                                          |
| Infrastructure clients         | Shared AWS credential/proxy/handler policy and Dataset S3 client lifetime are process-owned. SDK client disposal borrows the handler and cannot destroy the process pool; shutdown rejects new work and drains active requests. Legacy S3 operations now reuse that transport with standard, China and custom endpoint parity.                                                                                                                  | Move Lambda/CloudWatch construction behind named process adapters. Keep the object-storage migration and Enterprise S3 boundaries task-local/feature-local.                                     |
...
| `F-AWS-01`        | `@langwatch/aws-client` owns shared credential/proxy/handler policy, and `fa1a759f47` prevents SDK client disposal from destroying a shared raw handler. `b6ee5f2906` routes legacy S3 operations through it with standard, China and custom endpoint parity. The object-storage migration task, Enterprise S3 and platform Lambda/CloudWatch constructors remain explicit residuals; preserve task-local and feature-local closure while moving them behind named adapters.                                                                                                                                                                        | Wave 1                    |
...
At the working checkpoint, `platform/app` contains 6,333 tracked files,
including 5,976 under `src`. Counts include tests unless identified as
...
Current Wave 1 progress at `b6ee5f2906`:
...
- [ ] Replace the four live platform Eventing persistence graphs.
...
- [ ] Keep the object-storage migration, Enterprise S3 and platform
      Lambda/CloudWatch clients as explicit residuals until their named adapters
      preserve task-local and feature-specific lifecycle boundaries.
