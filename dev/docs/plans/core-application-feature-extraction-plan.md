...
Shared prerequisites, by leverage: (1) **ProjectService worker-composable** — gates langy-conversation (via ApiKeyService), scenario (prefetcher), gateway-spend (debit graph) and part of automation; it needs organizations, the LWQL ClickHouse key-map and S3 stored-objects. (2) **Packaged mail capability** in notification-server — clears join-request, automation's settlement digests and evaluation's graph-trigger subscriber. (3) **Trace conversion** — supplies scenario's traceSummaryStore and the evaluator engine's tracesMapping. (4) Model-provider resolution (getVercelAIModel) harvested with explicit params. (5) The all-instance ClickHouse directory (gateway-spend settlement) — endgame. Wave order: mail+join-request → Trace → scenario/evaluation → ProjectService wave → automation/langy-conversation/gateway-spend.
