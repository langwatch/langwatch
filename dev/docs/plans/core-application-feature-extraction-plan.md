...
| Platform residue | **NONE. `platform/` is deleted** (Cutover C, 2026-09-03). 306 rows, `git diff --numstat -- platform` 0 insertions on every one. `pnpm install --offline` is green without it and `pnpm-workspace.yaml` never listed it. What survived moved out first — see the Cutover C record at the end of this document for the file-by-file table. | Nothing. The lane is closed. Cutover A has since repointed the OpenAPI target and the root `test:*` scripts (see its record at the end of this document); what is left is haven's `mass` ingest preset and the lockfile regeneration every lane now depends on. |
