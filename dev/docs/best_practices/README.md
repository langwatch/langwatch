...
Every path in these documents that starts `platform/app/` names a file that no
longer exists. The pattern each one illustrates is unchanged; only its address
is, so the documents are amended here rather than rewritten one by one — a
rewrite would touch dozens of examples whose point was never the path.

Read a `platform/app/...` path as its owner in the new layout:
