...
The integration ships in its own package, so it has to be installed before it
can be registered:
...
- **`ai@7` publishes no CommonJS entry point, and that is fine.** Node's
  `require(esm)` loads it — unflagged since Node 22.12, and `ai@7` already
  requires Node >=22. `@langwatch/web`'s own `dist/server/server.cjs` is a
  CommonJS bundle carrying eleven `require("ai")` calls, so this is load-bearing
  in production today, not theoretical. `@langwatch/scenario` stays dual
  CJS+ESM for the same reason.

  The one thing that would break it is **top-level await**. If a future `ai`
