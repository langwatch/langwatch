...
- `MfaEnrollment` aggregate in the identity pipeline via `identityEventsPlugin` endpoint hooks (`/two-factor/enable`, `/verify-totp`, `/disable`, `/generate-backup-codes`, `/verify-backup-code` — failures emit events too):
...
- better-auth 1.6.23: `twoFactor` is built in; plugin migrations are Kysely-only ⇒ hand-written Prisma models; `databaseHooks` don't fire for plugin tables ⇒ the endpoint-hook plugin is the interception point.
...
1. Prisma models + plugin registration; `identityEventsPlugin` matchers for the two-factor endpoints.
