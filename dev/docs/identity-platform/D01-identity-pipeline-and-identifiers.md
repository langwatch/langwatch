...
> identity pipeline it describes lives under `modules/identity/process/src/`
> (`eventing/`, which absorbed `processes/` and `projections/` on 2026-09-15;
> `adapters/` and `ports/` are inventoried-and-going). Verify current shape
