...
- `GrantsService` must be able to say a directory made the write. Its actor parameter is a user id today, which is why SCIM writes through the ledger writer instead; widening it is a precondition, not a D08 task (see Research).
...
- Three gaps the current code carries into this deliverable: `ScimService.deleteUser` calls the low-level `GrantsLedgerWriter.offboardMember`, **not** `GrantsService.offboard`, so the empty-proof postcondition and the `needsHumanDecision` manifest are never exercised; a `PATCH`/`PUT` setting `active: false` deactivates the user and **revokes no grants at all**; and the SCIM resource id for a Group resolves to our internal `Group.id` rather than the IdP's `externalId`, which is written only on create.
...
- **Does `GrantsService` grow a system-actor seam, or does SCIM keep writing through `GrantsLedgerWriter`?** The service cannot express a non-user actor today, so "all membership writes via `grants.*`" is currently only reachable through the writer. Either answer is defensible; it needs settling on the authz precondition checklist before implementation, because widening the service is a breaking change to this program.
