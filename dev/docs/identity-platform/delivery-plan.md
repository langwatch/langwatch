...
- **SCIM converges on `grants.*`** (D08): the reconciler shape is already landed (`scim-grants.reconciler.ts` diffs and writes `source: "scim"`); what D08 removes is the direct `OrganizationUser` write beside it, by turning the SCIM endpoints into command producers — same external protocol, every membership consequence a command landing an event, and the consequence itself dispatched through `GrantsService` (whose system-actor seam is being built on `feat/authz-grant-provenance`), never the ledger writer. **The actor is NOT the connection** — an earlier draft said `actor: { type: "system", id: <connectionId> }`, which is not buildable: system actors are a closed registry of named principals in `@langwatch/actor` and a connection id is a per-customer value. The actor stays the single `system:scim`; which connection pushed a change lives on the `ScimSync` event. Cross-organization safety comes from the token's connection scope, not the actor stamp.
- **Offboarding is a service seam**: identity deprovision paths call `GrantsService` (revoke/offboard, empty-proof postcondition); no identity imports inside authz packages, no cross-pipeline event subscriptions. **Not yet true in the tree** — `.offboard(` has no production call site, SCIM calls the ledger writer's `offboardMember` instead, and `active: false` revokes nothing at all (latent, since deactivation blocks sign-in and API-key verification, but it means reactivation restores everything with nobody deciding). D08 is where this bullet becomes true, on both removal paths.
...
Critical path: **D01 → D03+D13 → D04 → D05 → D08 → D09 → D10**. D11 forks off after D01; D06/D07 fork off after D03; D12 forks off after D03+D13; Wave 4 has no dates by design.
 ...
 D01 identifiers             D03 router  ═╗                 D05 self-serve SSO +             D09 per-customer
      ...
      └──────────────────►   └─► D04 SsoConnection    ──►   └─► D08 SCIM per          ──►    └─► D10 delete
                             ...
                             (needs only D01)               ║  D12 join requests + auto-join
                                                            ║     (needs D13's hook only)
                             authz precondition ════════════╝  hard — gates D05 and D08, not D12
```
...
| D06 | D03 | TOTP MFA on the account + backup codes, `mfaRequired` as an org membership condition, sessions that carry `amr` (no forced re-login, no session revoke) | Orgs can finally require MFA; impersonation becomes provable |
...
| D12 | D03/D13 | Join requests + domain auto-join: sign up with a work email → see your company's org → request (admin approves) or walk straight in where the org allows it | "I signed up and my company was invisible" is over; the join-before-create flow is what starves orphaned organizations |
...
| `specs/identity/sso-connection-lifecycle.feature` | Amend the verification set with a fourth method, `operator-attested`: platform operators only, replaces the DNS proof and never the ops approval, no expiry and no upgrade path. Eight new `@unimplemented` scenarios; the seventeen bound ones are untouched | D05 |
...
| `specs/licensing/sso-license-gating.feature` | Amend mechanism (hook → router policy); settle restart semantics (Open Q11); behavioral invariants survive. **D12 amendment landed 2026-08-24**: the vocabulary now names domain **auto**-join specifically — the gate holds `auto` and lets `request` through, because an admin approves every request and no federation is involved. The `:182` enforced scenario is unchanged | D03, D12 |
| `specs/features/scim-group-mapping.feature` | Amend deprovisioning to the proved-empty postcondition (most scenarios `@unimplemented` — cheap now; the checker answers how many, so no count is kept here) | D08 |
...
| `specs/ai-gateway/governance/sessions-and-devices.feature` | Session inventory gains the sign-in method and whether a second factor was proven. No conflict with `maxSessionDurationDays` and no interplay to settle: `mfaRequired` is a membership condition and revokes nothing, so the two policies never contend over the same session | D06 |
...
| A connection that asserts no second factor makes MFA unenforceable for SSO members | D06 | Spec'd explicitly: the member is held at the enrollment gate like anyone else and can enroll here, and the org admin is shown that the connection asserts nothing so they can fix it at the identity provider instead |
