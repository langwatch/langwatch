...
| D10 (program exit criterion — customer-paced) | — | Repository-wide `grep -ri auth0` → allowlist only (changelog, retained `dev/docs/` planning documents); secrets blob + deployment config verified Auth0-free; deploy pipeline green | Tagged restore point + secret escrow, retired only after the observation window (see D10) | Low |
