...
- **SCIM converges on `grants.*`** (D08): the reconciler shape is already landed (`scim-grants.reconciler.ts` diffs and writes `source: "scim"`); what D08 removes is the direct `OrganizationUser` write beside it. **The actor is NOT the connection** — an earlier draft said `actor: { type: "system", id: <connectionId> }`, which is not buildable: system actors are a closed registry of named principals in `@langwatch/actor` and a connection id is a per-customer value. The actor stays the single `system:scim`; which connection pushed a change lives on the `ScimSync` event. Cross-organization safety comes from the token's connection scope, not the actor stamp.
