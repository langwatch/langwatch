...
- **Ordering precedent — and the position identity does not share.** ADR-110 settled authz's answer: every write goes through the group queue, no inline fold, and if Redis is down authorization writes are down — with deny-only (revocation, offboarding) as the one synchronous projection write, counted and logged because its effect can exist without an event behind it. That is a stated position, and it is the wrong one for sign-in: the front door is the one write path that must survive Redis loss. D01's pinned dispatch order (durable append → calling-path apply → staging last, best-effort) is identity's answer, recorded as a deliberate divergence in ADR-101 (§Revision 2026-08-23, §Rationale) and justified there by what makes it safe: idempotent applies, a per-user aggregate that is a handful of rows rather than the unbounded one ADR-110 was fixing, and volume in the hundreds per day. D02 is what proves the divergence holds under a real outage.
