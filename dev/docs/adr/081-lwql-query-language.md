...
**9. The text front-end is built on an existing SQL parser; hand-rolling the grammar is rejected.** The spike's hand-rolled tokenizer + recursive-descent parser (777 lines) was probed with 15 ordinary SQL constructs and rejected all 15, with at least five outright bugs (negative literals `-1`, exponent literals `1e5`, leading-dot decimals `.5`, trailing `;`, alias without `AS`) — the tokenizer greedily consumes trailing letters, so `24h` is one token. `node-sql-parser` 5.4.0 (Apache-2.0) parsed 8/8 probes including every case the hand-rolled parser broke, and is adopted as the front-end. Alternatives considered: ANTLR (`antlr4ng`, BSD-3) is the most robust option but requires Java-based code generation and no maintained ClickHouse grammar exists on npm; Chevrotain (Apache-2.0) composes grammars declaratively but still means authoring and maintaining our own grammar; `sql-parser-cst` is rejected outright — GPL-2.0-or-later is incompatible with the commercial SaaS distribution. The discipline that must not slip when adopting a permissive general-purpose parser: **the AST-to-IR walker is default-deny on node kinds** — it enumerates the node types it accepts and rejects everything else, so the parser's breadth never widens the language. The same rule retires the other hand-rolled utilities: Levenshtein suggestions move to `fastest-levenshtein` and duration-literal parsing to `ms` (which also removes the `1e5`-class tokenizer bug).

**10. Expressions: standard SQL syntax, closed-dialect power.** LWQL results are the tabular backend for configurable chart widgets (the New Relic model: query per widget → table → visualization; issues [#5670](https://github.com/langwatch/langwatch/issues/5670)/[#5389](https://github.com/langwatch/langwatch/issues/5389) are the dashboard side). Charts need computed columns, so the language gains arithmetic (`+ - * /`, parentheses, standard precedence), column aliases (`AS` and bare — the alias is the returned column name, which chart bindings depend on), and `HAVING` over aggregate expressions. The surface is **standard SQL syntax, nothing else** — no NRQL keywords, no invented dialect. The power/safety resolution borrows the NRQL *pattern*, not NRQL's syntax: NRQL is a closed dialect, not full SQL — power comes from arbitrary expressions over a fixed field vocabulary — but where NRQL invents syntax to reach that power, LWQL spells it in standard SQL throughout. Expressions are IR expression trees whose leaves are catalogue fields or literals and whose operators/functions come from a closed set, so allowlist-totality (rationale section) is preserved. Still excluded at any expressive power: subqueries, joins, unions, open function names, anything naming a table or column outside the catalogue. Content gating extends to whole expressions — an expression referencing a gated field is refused entirely (`gated_field * 0 + 1` is the same one-bit oracle as the bare field, per decision 6's argument).

**11. Timeseries: time-bucketing with selectable granularity, in standard SQL.** Trace and span data is time-ordered in ClickHouse, and the primary chart shape is a metric over time. LWQL buckets timestamps in `SELECT`/`GROUP BY` via standard-SQL-shaped functions: `date_trunc('minute'|'hour'|'day'|'week', started_at)` (ANSI-standard) and `time_bucket(INTERVAL '5' MINUTE, started_at)` for arbitrary granularity. Granularity is selected by choosing the bucket size, not by a dedicated keyword — this is the point at which NRQL's `TIMESERIES` clause collapses into plain SQL. Buckets compile through the catalogue to the existing `getDateTruncFunction` machinery (decision 5) like any other expression, so the timezone-correctness already proven there is inherited rather than re-derived. The maximum queryable time-span and row cap (decision 7) bound bucket-count explosion: a granularity/range combination whose bucket count would exceed the row cap is refused with an explanatory error, not silently truncated — truncating a timeseries result invisibly changes the shape of the chart it feeds.
...
- **[ADR-028 — plan-based visibility windows](./028-visibility-blur-teaser-redaction.md) needs an amendment from its owner.** (Note: two ADRs share the number 028 in this directory; this is the visibility-blur one, not the trace-facet-sidebar one.) Its summary states that "existence, timestamps, and **aggregates stay fully visible**" — a carve-out written when aggregates meant fixed, developer-authored dashboard charts, and which predates any caller-supplied-filter aggregate surface. Decision 6 is LWQL's local resolution; the platform-level rule belongs in that ADR itself. That ADR is also still `Status: Proposed` while its service layer ships, which should be corrected independently.
...
- **The spike parser (`parser.ts`) is replaced wholesale under decision 9**, with its behaviour-level test suite carried over as the falsifiability harness for the `node-sql-parser` front-end.
