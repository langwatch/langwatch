...
concerns: a "Conditions" cell mixes _what the thing is about_ with _what makes it
run_; the Reports table led with a useless "Schedule → see Ops" column; the
...
- **Subject leads, cadence follows.** "Watches _error rate_" / "Sends the
  _analytics dashboard_" is the scannable identity; the schedule is secondary
...
"Cadence" deliberately owns _all_ timing — threshold re-notify intervals, digest
