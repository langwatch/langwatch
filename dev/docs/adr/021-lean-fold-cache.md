...
3. `RedisCachedFoldStore` caches the complete fold state. Projecting a slimmer
   cache shape is unsafe because a cache hit becomes the next `apply` input and
   stripped fields would then be lost from the durable row. Edge leaning is the
   single cache-size defence.
...
- The Redis cache MUST retain the complete fold state consumed by `apply`.
