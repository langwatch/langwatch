...
>
> - **Phases.** One phase. No schema change, no backfill, no Terraform-managed resource added or removed — the change is in-Redis bookkeeping under the existing queue prefix and hash tag (`blobholders:` SET → `blobleases:` ZSET). Old holder sets are left to their 5-day TTL. `LEGACY_HOLDER_LEASE_GUARD` keeps old and new pods safe alongside each other during the roll.
