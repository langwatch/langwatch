...
> **Status (2026-07-29): the suite half of this ADR shipped; this paragraph did
> not, and is now OPEN rather than in progress.** `TotalCost` and the
> `TraceMetrics` map are gone, but the ten counters listed above are still
> folded by the experiment-run projection.
>
> An attempt to complete the conversion was made and **withdrawn**, because the
> stated justification did not survive checking. The counters were believed to
> be incremented non-idempotently; they are not. `FoldProjectionExecutor`'s
> `dropAlreadyApplied` is a genuine redelivery guard, and `experimentRunState`
> is wrapped in `CachedFoldStore`, which serves an applied-event-id set from its
> cache entry — so on the warm path `+= 1` is correct.
>
> The real defect was narrower: that set lived **only** in the cache entry, so a
> committed write followed by a lost cache (TTL, eviction, restart) let a retry
> re-fold a batch the state already contained. With `refoldOnOutOfOrder: false`
> and no `refoldOnStoreMiss`, the drift was permanent — a run's progress bar
> reading 11/10 beside ten item rows, with nothing to re-derive it. That window
> is closed by migration `00064`, which rides the watermark next to the state
> row (the same change `00054` made for `coding_agent_sessions`).
>
> This leaves a real tension a reader should not have to infer: the durability
> fix **entrenches** the counters this paragraph says to delete, and if they are
> later derived, `00064`'s column becomes dead weight. The case for deletion is
> now purely the "materialising is worse than computing" principle, not a
> correctness bug — the correctness bug is fixed. Whether that principle is
> worth a second migration to honour is an open decision, not a scheduled task.
