...
- **Projection queue carries the lean events.** Projections (`stored_spans`, `trace_summaries`, the fold cache, all reactors) see lean shapes. The fold cache stays bounded because the dispatch lean step bounded it.
