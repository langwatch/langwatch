...
- **Write-time wiring.** The offload runs inside `emitReported` in `executeEvaluation.command.ts`, before the event is built, so the stored-object PUT precedes the `event_log` append (matching the PUT-then-row ordering the stored-objects service uses internally). It is gated per project on the `release_evaluation_payload_offload` feature flag and is fail-open: any error from the flag store or the PUT keeps the inputs inline and logs a warning. With the flag off, behavior is exactly today's, except the unconditional repository cap below.
