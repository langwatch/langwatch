...
| Erasure blanks | `userId` and `actorUserId` on link rows (and person references in `DiscoveredAgent.snapshot`); anchor `externalId` on `OrganizationUser`; email-kind `externalId` swapped for the keyed-hash token; `erasedAt` stamped on every touched row | rows, non-email login ids, `externalKind`, timestamps, org/connection keys survive — no timeline row ever disappears |
| Erased-email token | keyed hash of the email, key scoped per organization, kept server-side for the life of reporting | reports derive the same token from ClickHouse's raw emails at read time, so erased timelines keep matching (Decision 9) |
...
| Erasure is distinguishable and complete | an erased row is never mistaken for an unlink, and no identifier survives | `erasedAt` set on every touched row; test erases a person who also authored link rows and asserts `userId`, `actorUserId`, and snapshot references are all blanked |
