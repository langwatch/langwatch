...
- **v9 — 2026-08-21 — Decision 4 amendment: the lifecycle transaction does not revoke grants; captain: team-lead (rebase onto the grants ledger).** Decision 4's transaction was written before ADR-092 landed the grants ledger, and its `remove` branch deleted the departing member's `RoleBinding` rows alongside the membership row. ADR-092 decision 18 makes the ledger the only writer of grants, and main's guard test fails any `roleBinding.deleteMany` reached from SCIM — a real collision, resolved in ADR-092's favour. The reason it is safe: Decision 4's crash-in-the-gap hazard is about the **closing link row** landing with the membership change, not about grants, and grant revocation was always eventually consistent (`offboardMember`'s fold sweeps every grant the principal holds, including ones the caller's read could not see — which is strictly stronger than the id-scoped delete this removes). So: the lifecycle transaction owns membership state and closing link rows only; the caller appends the offboard fact through the ledger *before* calling in, as `scim.service.ts` already does on the DELETE path. No entry point loses a sweep — the `disable` branch never revoked grants on either lineage. Delegation only; nothing in Decision 4's transactional guarantee for link rows is relaxed.
