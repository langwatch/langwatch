...
- **Competitive window.** Content-type cost attribution for coding agents — splitting a single call's cost by system prompt / MCP / skills / tools / memory — is an emerging, largely unoccupied space; where it is attempted at all, the split is **byte-proportional** with no tokenizer involved. Shipping the same insight computed from **real token usage** is the differentiation play: token-accurate rather than byte-estimated.
...
1. **Classification runs at ingest-time enrichment.** A new pure-synchronous step in the `recordSpanCommand.ts` enrichment chain — **serial, after** the parallel `Promise.allSettled` block (PII redaction, cost enrichment, token estimation) because it consumes their outputs (per-tier rates `langwatch.model.*`, tokenizer), and before content drop. It runs **AFTER** the ingest attribute cap, not before: the cap exists to bound oversized *customer* payloads, whereas everything classification writes is system-generated and self-bounded (`MAX_CLASSIFIED_BLOCKS_PER_SPAN` caps the detail array, category keys are a fixed enum), so its reserved output is exempt from the cap by design. Accepted consequence: a span whose `langwatch.input` was truncated by the cap classifies on the truncated content (or degrades to the empty state if nothing survives) — consistent with the content-availability policy (Decision 7). Why: categories are stamped once, flow into folds and ClickHouse, are queryable and indexable forever, and replay via ADR-015. Cost honesty: this is not a free string match — it tokenizes every content block (N per-block tokenizer calls, real CPU on an 8 MiB body), which is why the hot-path invariant carries an explicit block cap and a perf test, not a hand-wave. Rejects read-time classification (re-scans millions of span rows per dashboard load — the exact heavy-column pattern `clickhouse-queries.md` bans) and an async reactor (2× event volume and eventual-consistency lag for a bounded synchronous CPU task).
   ...
   - Content-drop interaction (recorded, conscious): classification also runs **before** `dropSpanContent`, and its output — `langwatch.reserved.blocks.classification` (`{idx, category, tokens, cacheTier}`, **no text**) and the per-category `blockcat.<category>.{tokens,cost_usd}` **numbers** — survives the drop. This is aggregate cost/structure analytics, the same class as the span-level `TotalCost`/`Models` the drop path already leaves in place; **no customer content is retained** (the per-block `text` is consumed for tokenization and never stored). A policy-drop of, say, `system_prompt` content still leaves that lane's token/dollar total visible. That is intended — analytics-only structural metadata, not the content. If an org needs finer-grained scrubbing (drop the per-category totals when their source content is dropped), that is a follow-up on the `data-privacy/` drop catalog, not a change to this classifier.
...
3. **Taxonomy: 12 primary categories on two axes, plus catch-alls, extensible additively.**
   ...
   - Rejects both a large ~23-category taxonomy verbatim (half would be Claude-Code-internal artifacts that won't aggregate meaningfully) and a minimal-6 taxonomy (too coarse to be useful; splitting later forces reclassifying history).
...
- **Byte-proportional cost split** — violates the exact-tokens constraint; ±15–30% error on code/JSON-heavy content.
...
- **A large ~23-category taxonomy verbatim** — half the categories are Claude-Code-internal artifacts with no aggregate meaning.
- **Minimal 6-category taxonomy** — too coarse to be useful; later splits force history reclassification.
...
**Positive**
- Category cost numbers are allocation-of-truth: totals always equal the real bill, split at tokenizer-grade accuracy (~±1–3% on text) — the marketing claim is technically honest.
...
- Capture fidelity is **not uniform across paths**, and the design must not pretend it is: the gateway body is post-middleware (rewritten `cache_control` under org overrides), and the Claude OTel raw-bodies path (Path B) truncates at ~60 KB inline. The truncation is the more damaging of the two — a real coding-agent turn's full prompt (system + tools + history) far exceeds 60 KB, so the whole cached prefix would strand in the catch-all (`other_input`), observed in testing as ~73% `other_input`. PR #5353 mitigates by recovering the surviving front (system prompt + complete leading turns) from the truncated body; the truncated tail is still lost. The structurally correct capture for this feature is to **own the request bytes end-to-end** — which our AI Gateway (Path A) already does — so classifying from the gateway-captured body is full-fidelity and never truncated. See Open Questions.
...
| Prefer the gateway-captured body (Path A — full-fidelity, never truncated) as the classification source over the Claude OTel raw-bodies path (Path B — ~60 KB inline truncation). Treat Path B + the #5353 front-recovery as best-effort fallback. | eng, post-v1 | No |
...
- **v5 (2026-07-03)** — Third review pass + local dogfood. Cache-tier split corrected: a cached-prefix block was assigned wholly to one tier by comparing the local token *estimate* against the *provider's* absolute `cacheReadTokens` (different scales) — when the provider's read count reached/exceeded the prefix estimate, the `cache_creation` pool was left blockless and its non-empty total dumped into `other_input` (a live dashboard showed ~73% `other_input`). Replaced with a split PROPORTIONAL to the provider's read share, measured in estimate space, so every pool with a positive provider total keeps a prefix block (conservation still holds via per-pool rescale). Correctness fixes: cache breakpoint preserved past the block cap and on empty `cache_control` parts (was collapsing to null → whole cache pool to catch-all); `span-cost-enrichment` model resolution aligned response-first with the tokenizer/classifier/`computeSpanCost`; half-filled custom rate rejected at the tRPC write layer (was silently pricing the unset tier at $0). `skill_invocation` is now emitted — a `tool_use` named `Skill` classifies as a skill run rather than folding into `tool_call_builtin`; `memory_context` remains the sole deferred lane (#5332). Cleanups: shared kill-switch helper; dropped a redundant token/cost recompute in the session-step fold.
- **v6 (2026-07-03)** — Capture-fidelity finding from local dogfood. The Claude OTel raw-bodies path (Path B) truncates the request body at ~60 KB inline, so a real coding-agent turn's full prompt (system + tools + history) is invalid JSON and the whole cached prefix stranded in `other_input` (~73% observed). Recorded the correct direction as an Open Question: classify from the gateway-captured body (Path A), which owns the request bytes end-to-end and is never truncated; Path B + PR #5353's front-recovery (system + complete leading turns from the truncated body) is the fallback.
