...
- **Graph triggers off the cron**: the final ADR-052 path is a lightweight subscriber on the trace and evaluation pipelines, debounced per project for five seconds. Absence/resolve runs through the scheduled `graphAlertSweep` process manager with the same source-aware pre-filter against the right slim table; its no-data predicate remains exactly "would a zero value breach?" for parity with the old cron. The earlier `.withOutbox` reactors and ADR-039 heartbeat were retired with `ReactorOutbox`. **Migration complete (2026-07):** the event-sourced subscriber/process-manager path is the sole graph-alert path. It runs on the worker role, so a deployment MUST run the workers Deployment for graph alerts to fire.
