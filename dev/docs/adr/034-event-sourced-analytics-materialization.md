...
We want analytics + triggers cheap and real-time, using ClickHouse's native engines, with the **idempotency the platform actually requires**: delivery is at-least-once and replay re-applies events ([Eventing framework](../../../packages/eventing/adrs/20260820-eventing-framework-boundary.md), [ADR-022](./022-event-log-source-of-truth.md), [ADR-015](../../../packages/eventing/adrs/015-projection-replay-coordination.md)). This is the materialization for **traces, evaluations, scenarios, experiments** — a shared primitive, traces first.
...
**Fold-state continuity** _(amended by [ADR-066](../../../packages/eventing/adrs/066-clickhouse-cached-projections.md))_. Continuity is the store's job: the fold reads back its own last committed state from the platform ClickHouse-cached store (Redis read tier, ClickHouse point read on a miss), so the durable row must round-trip the fold's working state — the slim analytics columns ride alongside that lossless state, never in place of it. The original design here was different — the slim row was deliberately lossy, and a cache miss rebuilt state from `event_log` via the framework's refold option — and we don't do that anymore: replaying an aggregate's history on the delivery path is an unbounded read that starved ClickHouse merges and caused the 2026-07-23 outage; the read-back store removes the need for it. What stands unchanged: the slim fold is an independent projection with its own queue/retry lifecycle — one event fans out to two projections at the dispatch layer, each owning its own store — and `event_log` is read only by the offline replay tool (ADR-015). _(These folds are ADR-066's remaining pillar-1 adopters; the lossless-row migration is sequenced there.)_
...
**Why per-span (not per-trace).** A per-trace source _mutates_ — a trace becomes N versions as spans land, an MV over it fires N times → every trace N×-wrong (systematic). A **span is a single immutable event**, processed once; the only repeat is a rare crash/retry re-delivery → a fraction-of-a-percent of buckets, transient, non-systematic. **We accept that** (explicitly — it's negligible). Replay re-fires everything, so **replay = truncate the rollup and rebuild**, a deliberate op, not steady-state.

**Rollup keys = dims final at span-write only: `(TenantId, BucketStart[=span time], Model, SpanType)`.** A rollup key is stamped onto the increment when the span is written and can never be re-stamped. Topic is a _classified id_ computed after the spans. Origin is resolved _inside the fold_ and **flips** as spans arrive (a provisional `application` is overridden when the root span's real marker lands — see `trace-origin.service.ts:147-163`). Neither is final at span-write, so **neither can be a rollup key** — they live on the slim table. (Adding a late `0`-metric row for the resolved value does not move the already-summed cost off the provisional label; re-attributing it would require the signed correction we rejected.)
...
Generic fold + per-span-increment + read-routing machinery, parameterised per pipeline by field extractors. Traces first; evaluations, scenarios, experiments each plug in with their own `<x>_analytics` + `<x>_analytics_rollup`. Our own copy — **not** the governance substrate (ADR-018 maps to a _flat_ table; we map-with-aggregate, deliberately divergent).
...
5. **Done (2026-07):** every project cut over, so the K8s cron graph-alert path (`/api/cron/triggers` + `pages/api/cron/triggers/`) and the `release_es_graph_triggers_firing` flag were **removed** — the event-sourced path is now unconditional. See the Implementation status note below.

## Keep / drop (from the prior attempt)

**Keep, repointed:** recurring absence detection (now the ADR-052 scheduled process manager), `evaluateCustomGraphThreshold`, the trigger / notification / custom-graph services, and the active-trigger read model.
Graph-alert evaluation reads the unified slim projections and does not maintain a parallel `CustomGraphTriggerRollupBucket`, sign-collapsing store or graph-only projection pipeline.

## Related

[Eventing framework](../../../packages/eventing/adrs/20260820-eventing-framework-boundary.md), **[ADR-022](./022-event-log-source-of-truth.md) (event log source of truth)**, **[ADR-015](../../../packages/eventing/adrs/015-projection-replay-coordination.md) (projection replay coordination)**, [ADR-024](./024-cold-path-tiered-storage.md) (cold-path storage), and [ADR-052](./052-automations-on-process-manager-substrate.md) (graph-alert scheduling). ADR-018's governance substrate is deliberately not shared.
...
- **Slim fold continuity** shipped as `refoldOnStoreMiss` + a RedisCachedFoldStore wrapper (lossy slim row, no read-back). That mechanism is retired by ADR-066 — see "Fold-state continuity" in the Decision — and these folds adopt the lossless read-back store per its sequencing; the refold option remains wired only until that migration lands. The **eval rollup** additionally enables the map-projection `dedupeByIdempotencyKey` option: eval terminal events are re-reported by design (deterministic ids, SDK retries), so without it every duplicate append double-counted the bucket — unlike the accepted per-span crash-retry noise.
- **Read routing** lives in `packages/features/analytics/server/src/routing/route-table.ts` (`pickAnalyticsTable`); the union is `"trace_analytics_rollup" | "trace_analytics" | "trace_summaries" | "evaluation_analytics_rollup" | "evaluation_analytics" | "evaluation_runs"`. Optional comparison reads use the LangWatch (PostHog) feature flag `release_event_sourced_analytics_read_tripwire`.
- **Graph triggers use Eventing post-event work**: a lightweight subscriber on the trace and evaluation pipelines evaluates activity with a five-second per-project debounce. Absence and resolve run through the scheduled `graphAlertSweep` process manager with the same source-aware pre-filter against the appropriate slim table; its no-data predicate remains exactly "would a zero value breach?" The subscriber and process manager run on the worker role, so a deployment must run the workers Deployment for graph alerts to fire.
...
- **Alert template data is graph-shaped, not trace-shaped**: `GraphAlertTemplateContext` carries the metric's recent numeric `history` (the buckets the threshold read — no extra query), a prebuilt `sparkline`, `previousValue` (preceding window, same aggregation), and a `graph.url` deep link pinned to the incident window. Preview, variable autocomplete, and test-fire render the same alert shape (`buildExampleGraphAlertTemplateContext` + `ALERT_TRIGGER_DEFAULTS`) so what the author sees matches what dispatch sends. Rendering the history as a PNG chart attachment was considered and deferred — it needs image hosting for Slack and CID inlining for email; `history` keeps that door open.
