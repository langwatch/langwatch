...
- **Graph triggers off the cron**: real-time path is `.withOutbox("<aggregate>Analytics", "graphTriggerEvaluation", …)` on the trace AND eval pipelines, debounced per `(triggerId, projectId)` via `makeJobId` + 5 s ttl. Absence/resolve path is the 30 s outbox heartbeat (ADR-039) with a source-aware pre-filter against the right slim table per candidate; its no-data predicate is exactly "would a zero value breach?" for parity with the old cron (a `count < 10` alert still fires on total silence). **Migration complete (2026-07):** once every project had cut over, the K8s cron path and the `release_es_graph_triggers_firing` flag were removed — the event-sourced path is now the sole, unconditional graph-alert path. This runs on the worker role only (outbox drainer + heartbeat are `roleRunsWorkers`-gated), so a deployment MUST run the workers Deployment for graph alerts to fire.
