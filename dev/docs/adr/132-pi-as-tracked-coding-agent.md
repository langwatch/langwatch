...
| v13 | 2026-09-18 | Sergio Esteban | **A third decision reversed, and it was reversed on a fact nobody had checked.** Rung 18.3 settled pi's icon as "**no asset added.** No pi mark exists in `public/images/external-icons/` and we hold no license to redistribute one", and the ladder's Open questions carried "whether a pi icon asset exists or may be used … has checked neither the directory nor the licensing". The directory half was true. The licensing half was an assumption written in the voice of a finding, and it was wrong: pi is MIT — `license: MIT` on npm `@earendil-works/pi-coding-agent` 0.85.1 and on `github.com/earendil-works/pi` via the GitHub API — and its mark is published by pi itself at `https://pi.dev/logo-auto.svg`, cited from its own README. Fetched and compared: 334 bytes, byte-identical to what now ships at `public/images/external-icons/pi.svg`. Worth naming because it is the reusable part: the npm package does **not** contain the logo, so a `find` across the installed package returns nothing and reads as confirmation of "no mark available". It is not — it only means the mark is not shipped with the code. **Two further gaps fell out of the same root, and neither was cosmetic.** `ASSISTANT_BILLING` and `ASSISTANT_VENDOR` (`ee/governance/dashboard/components/toolCatalog/registeredTools.ts`) never learned about pi, so a registered pi tile rendered a catalog card reading "Vendor not recorded" with **no payment rows at all** — on a tool whose maker and billing model are both perfectly well known. The billing map's own doc comment had predicted this exact failure ("one added to the picker before this map catches up") and the prediction was not acted on. pi is now `consumption` — it runs on a key the person supplies, the same shape as opencode — and `Earendil Works`, named for the company rather than opencode's "Open source", because a customer putting pi on a renewal needs a maker and the licence is a separate fact from who publishes it. **The structural fix matters more than the two entries.** Both maps were `Record<string, …>`, which is why omitting a kind was invisible to the compiler rather than a build failure; they are now keyed on `Exclude<AssistantKind, "custom">`, the same shape `ASSISTANT_PRESETS` already uses and which this ADR calls an exhaustive record at §229. Proven: deleting `pi: "consumption"` now fails the build. The runtime fall-back stays, because the lookup key is unvalidated text off a JSON config column rather than an `AssistantKind` — typing closes the gap we control, the fall-back covers input we do not. **Recorded because the process point repeats v10's:** the first coverage test written for this passed vacuously. It asserted the vendor was "not the fall-back string" by comparing against a copy of that string's text rather than the constant, so it survived both a rewording of the constant and a vendor of three whitespace characters — which reaches the rendered card as a blank maker line. Found only by deliberately sabotaging the source and observing that nothing went red. The constant is now exported and asserted against, pi's two values are pinned outright, and the all-kinds check tests for present-but-empty, which is the only thing left that typing cannot see. The scenario was also in the wrong file and overclaimed: it read as being about pi while its tests iterate every kind the picker offers, so an unrelated assistant added later would fail a test printed under pi's name. The behaviour it actually guards now has its own scenario in `specs/ai-governance/dashboard/inventory-catalog.feature`, which is the spec `registeredTools.ts` names in its own header. |
