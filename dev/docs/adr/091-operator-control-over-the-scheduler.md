...
**A manual run is a first-class slot, not a bypass.** This is the load-bearing choice. "Run now" does not call the target's handler directly; it claims a slot through the same path the calendar loop uses, stamped as manually triggered, and lets the existing revision fencing decide whether it may proceed. Three properties fall out, and they are exactly the ones a bypass would lose:

- A manual run **races safely** with the scheduled one. If the calendar loop fires the same slot concurrently, one of the two claims wins and the other stands down — the same fencing ADR-052 adopted so racing wake workers do not need a lock. Without this, "run now" pressed a second before the cron tick double-sends.
...
**Run now targets a slot, and says which one.** The control does not mean "execute the job body"; it means "work slot S". Default is the next due slot. The confirm step names the slot, the target, and the project **by name**, because the whole risk of this control is doing the right thing to the wrong tenant. A manual run of a slot already marked fired requires a second, explicit opt-in — that is the double-send case, and it should feel different from the ordinary case.
...
**Why "claim a slot" rather than "invoke the handler".** Invoking directly is a smaller change and a much worse primitive: it has no interaction with the calendar loop, so it double-sends under a race; it is invisible in `currentSlot`, so nothing on the page reflects it; and it would need its own retry, error and audit story rather than inheriting three that already exist. Routing through the slot path means "run now" is the *same event* as a scheduled run with a different reason stamped on it.
