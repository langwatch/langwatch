...
- **Kill-switch residual (accepted):** with `ops_evaluation_payload_offload_disabled` flipped, inputs flow inline again and only the unconditional 8 MiB repository cap bounds the row. 8 MiB × ~8K rows per granule re-approaches the fat-row merge-memory class at a higher threshold - and the row cap is row-only: the offload runs before the event is built, so with it disabled the full inputs also flow into `event_log.EventPayload`, which has no cap of its own, reintroducing the event-log growth/OOM risk this ADR exists to bound. The switch is an *emergency* lever for object-storage trouble, not a steady state - revert it as soon as storage recovers.
- The marker resolve path verifies the stored row's `purpose` and the content SHA-256 against the marker before returning resolved inputs; a resolvable (non-empty-id) marker must carry a well-formed SHA-256 to begin with - the write path always records one - so a marker-shaped input that omits or malforms the hash is never resolved. Any mismatch (marker-shaped user input pointing at an unrelated same-project object, corrupted content, or a missing hash) degrades to the marker's preview instead of returning foreign bytes; the read services additionally project any still-marker fail-safe result to the compact preview shape at their boundary, so a degraded resolution never serializes the internal object id or hash out of the API.
