...
| I3 | Governance signup is ≤3 screens to actionable (org → intent → CLI setup) | Flow-config unit test asserting the governance track screen list |
...
| O3 | Should the governance CLI setup screen poll for the first session and auto-forward to /me (mirror of WaitingForTracesChip)? | implementer | No |
