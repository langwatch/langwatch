...
- **v5 (2026-07-08, post-lock revision, user-directed)** — The feature ships dark: the flag pair stays `defaultValue: false` (registry + auth-cli fallback) and `release_ui_ai_governance_enabled` additionally gates the onboarding intent screen (flag off = the exact pre-fork flow, no intent screen) and the org "Primary use" settings field. Decision 8's GA mechanics are unchanged in shape but deferred: GA = PostHog rollout to 100% (SaaS) + flipping both literals (self-hosted), as its own release. Trigger: user directive to keep everything behind the wall until launch. Pin tests now assert the pair is `false` and moves together.
