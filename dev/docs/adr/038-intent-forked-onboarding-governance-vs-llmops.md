...
3. **The org intent always decides the `/` landing when set** (R3-Q1, user overruled the pin-wins recommendation — hard rule chosen). Resolver contract:
   ...
   - `primaryIntent = LLM_OPS` → `/{project-slug}` (the specific project still picked by existing last-visited/first-membership logic — intent decides the _kind_ of home, not which project)
...
4a. **Governance signups create org + team only — no default project** (v6, superseding R1-Q2's "create silently"). The default project served no purpose for a governance org: usage data flows through per-user personal workspaces (provisioned during onboarding since v7, and by CLI login on any machine where onboarding did not run) and org ingestion uses ADR-018's hidden project. Supporting the project-less org took four flow fixes: the org bouncer skips project-based redirects for `AGENT_GOVERNANCE` orgs; the welcome screen sends an already-onboarded governance owner home instead of minting a duplicate org; `initializeOrganization` returns `projectSlug: null` for governance; and org settings render project-optional. **The project is created at the moment it's needed**: flipping the org to LLMOps opens the `CreateProjectDrawer`. LLMOps and legacy signups still create the default project exactly as before.

4b. **Every pre-existing organization gets `NULL` — nobody moves, ever** (R3-Q2). No backfill heuristics, no migration of behavior: legacy orgs keep today's resolver output bit-for-bit, including flag-on early-adopter orgs whose mixed-persona users land on `/me` today. An org opts into intent-based landing by setting the field in org settings. This also makes governance GA a routing non-event for every existing customer (original R1-Q3 lock, now satisfied more strongly).
...
7. **GA mechanics: one operator-store rollout; the CLI 403 stays as a flag-armed safety net; no CLI changes** (R2-Q4 + R3-Q3, C5). The `feature-flag` service resolves `release_ui_ai_governance_enabled` for the authorised organization. Its registry default is on, and an operator rule can switch one organization off to re-arm the 403 without changing the route.
...
9. **Self-hosted gets the same fork** (C4). Self-hosted welcome grows from 1 screen to org → intent: the governance track finishes at the intent screen and lands `/me` (v4 — no in-onboarding CLI screen), where the existing CLI install surface renders the self-hosted `langwatch login --endpoint <host>` guidance; the LLMOps track finishes as today.
...
| CLI setup commands     | `npm install -g langwatch` → `langwatch login [--endpoint <host>]` → `langwatch claude` | Rendered on the docs page + the existing `/me` install surface, NOT by onboarding (v4 dissolved the in-onboarding CLI screen); must match docs page `ai-governance/track-your-claude-code-usage` |
| Flag                   | `release_ui_ai_governance_enabled` — registry default on, with operator-store targeting | Decision 7                                                                                                                                                                                       |
| Kill-switch            | Per-organization operator rule on the same flag                                         | Re-arms the 403 and hides governance UI for that org                                                                                                                                             |
...
| I7        | Self-hosted renders the intent screen (the `--endpoint` CLI guidance lives on the existing install surface, not onboarding — v4)                                                                                          | Flow-config test on the `self_hosted` variant                                                                       |
...
- **~~No project for governance signups~~ — REVERSED in v6.** Originally rejected (R1-Q2) fearing every no-project code path becomes load-bearing; a dedicated sweep found nothing crashes on the reachable surfaces and only four redirect/flow fixes were needed. Decision 4a adopts it.
...
- GA moves nobody: legacy orgs are NULL, the resolver's legacy path is untouched, and an operator rule re-arms the 403 per organization without a deploy.
...
- The paying governance buyer (budgets/policies/anomalies at `/governance`) first sees their _personal_ usage page; the org-control story is one sidebar click away rather than the landing (accepted, R4-Q2; O10).
...
- LLMOps and legacy orgs still get the default project; governance orgs get none until they flip to LLMOps, at which point the create-project drawer makes one (v6).
...
- **v7 (2026-07-25, post-lock revision, user-directed)** — The governance track provisions the signer's personal workspace during onboarding, amending Decision 4a's "per-user personal workspaces (provisioned lazily at CLI login)". Only the timing changes: the shape is still `PersonalWorkspaceService.ensure()`, still one per (organization, user), and CLI login still calls it unchanged, so the two paths converge on the same workspace whichever runs first. The track lands on `/me`, which reads the personal workspace, so lazy provisioning meant it arrived empty and could only point at a command the user had not run yet. Provisioning is non-fatal (matching `organization.acceptInvite`): a failure leaves the organization intact and `user.personalContext` backfills on the next session. Held twice before shipping — once on plan accounting, where a provisioned workspace spent a project and a team out of the free plan's two and one until `getProjectCount` / `getTeamCount` learned to exclude `isPersonal` rows (#6189), and once on ambient-context selection, where a personal workspace could win the app's current project and take organization-scoped writes with it. Decision 4a's "no default project" is unchanged: the personal project is not a shared project, does not appear in the switcher's team list, and is never the ambient context. Spec: `specs/features/onboarding/intent-fork.feature`.
...
- **v2 (2026-07-08)** — Red-team findings replaced the global pin with nullable `Organization.primaryIntent`, kept the 403 as a flag-armed safety net, and added org settings. Its then-current rollout mechanics were superseded by feature-flag ADR-001.
- **v6 (2026-07-08, post-lock revision, user-directed)** — Governance signups no longer create the silent default project (reverses R1-Q2 after an investigation sweep showed only four flow-level blockers, none data-related: bouncer ejection, welcome-screen duplicate-org trap, `initializeOrganization` coupling, and the settings page's project guard). Org + team only; `projectSlug: null`; bouncer and welcome screen special-case `AGENT_GOVERNANCE` orgs; settings render project-optional; flipping to LLMOps opens `CreateProjectDrawer` so the customer creates the project exactly when they need it. I4 relaxed: result shape identical except `projectSlug` is null on the governance track.
- **v5 (2026-07-08, post-lock revision, user-directed)** — The feature shipped dark behind `release_ui_ai_governance_enabled`; later rollout mechanics are owned by feature-flag ADR-001.
