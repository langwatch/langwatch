...
>
> - **Phased rollout.** Single phase, standard rolling deploy of the workers deployment. The two schemes coexist by design: `LEGACY_HOLDER_LEASE_GUARD` writes a sentinel into the legacy holder set so an old pod can never observe an empty set and UNLINK a blob new code has just staged.
> - **Canary validation.** Roll one worker replica first and hold for 15 minutes. Pass criteria: `gq_jobs_dropped_total{reason="missing_blob"}` shows no increase attributable to the canary pod, `gq_jobs_completed_total` continues to advance on that pod, and no `"Failed to parse staged job data"` lines carry the canary's `hostname`. Fail any one and roll back before widening.
> - **Rollback.** `kubectl rollout undo` on the workers deployment; no data step. Reverting to holder ref-counting leaves lease ZSETs behind, which the old code path ignores and which expire on their own TTL. The reverse-direction risk is the same narrow cross-version window described above, so roll back during a trough rather than a burst where practical.
> - **Dashboards.** Watch `gq_jobs_dropped_total` by `reason` (the incident metric), `gq_jobs_completed_total`, `gq_jobs_staged_total`, and Redis `used_memory` on the queue instance — the last because reclaim is now lazy, so blob bytes are expected to trend *up* until the 4-day backstop reaches steady state. A rising `used_memory` alone is not a rollback signal; a rising `missing_blob` count is.
...
**Extends / supersedes in part:** [ADR-090](./090-groupqueue-payload-envelope.md) (GroupQueue payload envelope). The versioned-envelope + header-only routing decision stands. This ADR supersedes ADR-090's §"Blob lifecycle" — random blob ids, best-effort delete, and the 7-day pure-backstop TTL — for offloaded bodies.
...
1. **Most fan-out payloads never offload.** ADR-090 offloads only bodies above 32 KiB; the leaned projection events (IO attributes capped at a 64 KiB preview by ADR-022, most attributes far smaller) frequently sit below that line and inline N times into the per-group data hash.
2. **Even when they offload, they don't dedup.** ADR-090's blob id is a `randomUUID()` (`jobEnvelope.ts`), so N byte-identical payloads land under N distinct keys. There is no content identity, so there is nothing to collapse.

Two further forces motivate a rewrite rather than a patch:

- **Offload is split by provenance, and there are three of them.** ADR-090's Redis blob (queue jobs), ADR-022's edge S3 spool (oversized commands), and the `stored-objects` object store (externalized media) are three separate systems with separate thresholds, key shapes, and lifecycles. Where a payload's bulk lives depends on *which subsystem touched it*, not on *how big it is*. We want one answer, keyed on size alone, and we want the durable tier to reuse the one object-store abstraction the codebase already has.
...
The reshape is invisible above the serialization boundary, exactly as ADR-090's envelope was. The load-bearing subtlety: the dedup must key on the shared **component**, lifted at the fan-out point — not the whole job body.

### 3. Holder-set reclaim + TTL backstop

Because a content-addressed blob is shared, ADR-090's "delete on complete" is unsafe: one job's completion would yank a blob its siblings still reference. References are tracked with a **holder set** per blob and reclaimed eagerly when it empties, with a long TTL as the orphan backstop.
...
- **Missing blob is a fail-safe, never a wedge.** Should a backstop ever fire under a still-referenced job (TTL mis-sized, sweeper bug), decode finds the blob gone and — exactly as ADR-090 — completes the slot without invoking the handler; the work is recoverable via event replay. Content-addressing plus the holder set makes this rare; it never makes it fatal.
...
**Neutral.** Handlers, the queue-manager facades' public API, dedup-id squash semantics, and every key shape other than the new `blob` namespacing / `blobholders` / reclaim keys are untouched. This replaces ADR-090's blob lifecycle; ADR-090's envelope, header-routing, and two-phase-rollout decisions stand.

## References

- Extends / supersedes-in-part: [ADR-090](./090-groupqueue-payload-envelope.md) (GroupQueue payload envelope)
