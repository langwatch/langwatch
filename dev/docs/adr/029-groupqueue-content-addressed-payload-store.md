...
>
> **Retention amendment (2026-07-22): release grace window.** The "expected to trend up" note above was directionally right and quantitatively wrong. With the 4-day backstop as the *only* Redis-tier reclaim path, the steady state is four days of ingestion-volume payloads — several times the queue instance's memory, so it is not a steady state at all. Observed in production the day after the WP4 rollout: Redis memory left a flat two-day baseline within minutes of the deploy and climbed linearly, on track to exhaust the instance before the first blobs aged out.
>
