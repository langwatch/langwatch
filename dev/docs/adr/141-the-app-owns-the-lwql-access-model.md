...
A helm upgrade opens a short window where the access model exists nowhere. The new app image can boot against the OLD ClickHouse pod, which still serves `users.d/lwql.yaml`, so the access-model DDL is skipped as config-store-owned (495). Moments later the StatefulSet rolls to the new chart, which renders no access model — the XML identity disappears and, without intervention, nothing re-provisions until the next app boot. The running app watches the config store and re-provisions once the old pod's rendered model is gone: the deploy task is a short-lived process (it returns and exits, so unref'd timers in it would never fire), so the watch lives in the long-running app server (`src/start.ts`, armed once it is listening). It polls a stateless ownership probe on an exponential backoff (first poll within ~5s so a pod that rolled before the server listened is still caught, then doubling, capped at 5 minutes) and decides from each snapshot alone, never inferring from probe history: the probe reports whether the model is owned by the config store (old pod still rendering it — keep waiting), the SQL store (the app-owned model is already live — nothing to do), or neither. The server re-provisions only when a successful probe shows the model is neither config-store-owned nor present in the SQL store, and it does so exactly once. A probe that throws (the ClickHouse pod is mid-roll) is treated as still-waiting and never re-provisions on the failure alone, so a transient error can never fire a re-provision against a healthy install; a config store that never releases the model gives up at a ~30-minute budget with a warning. The poll timers are unref'd, so they never hold shutdown open, and a hard provisioning failure (as opposed to a config-store skip) is left fail-closed for the next boot rather than retried.
