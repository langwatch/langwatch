...
- License is stored **per-organization** (`Organization.license`, TEXT); for **plan limits**, no license and one we did not sign resolve to the uncapped open-source baseline on self-hosted (`ee/licensing/licenseHandler.ts`), and a license past its end date keeps the seats it sold (see the v8 amendment below). For the **SSO gate specifically**, "genuine" means **`verifySignature()` passes** (`ee/licensing/validation.ts`); expiry is deliberately not checked (Decision 1). Never derive either from the denormalized `licenseExpiresAt` column, which skips signature checks.

## Decision

1. **Binary entitlement on license *authenticity*, not activeness — no license schema change.** SSO is allowed iff `IS_SAAS === true`, OR the `LANGWATCH_LICENSE_KEY` env var holds a **signature-valid** license (Decision 5), OR at least one organization row holds a **signature-valid** license — **expiry is deliberately ignored for the SSO gate** ("once a customer, never blocked", locked v6). Why: the gate's target is the free-rider who never had a license and cannot forge a signature; an *expired* license is an ex-/renewal-limbo customer, and cutting their whole company's login on a routine upgrade (upgrade = restart = re-evaluation) is exactly the blast radius the owner ruled out. Amended in v8: self-hosted plan limits are signature-only too, so the two now agree. Cost owned in Consequences: churned customers retain SSO indefinitely; tightening later is a breaking change. No `enableSso` field is added to `LicensePlanLimitsSchema` — zero signature risk, zero re-issuance, addable later with `absent → true` semantics. Rejects: strict expiry (renewal-limbo lockout); N-day grace window (adds a second clock + still has a cliff); per-plan/field encoding (schema churn for no v1 gain).
...
4. **Denied state = email mode, exactly as if the SSO env vars were unset — but email routes are re-blocked on ALLOW.** The `before` hook reads the one memoized gate value and branches **both** directions from it:
   ...
   - **When the gate ALLOWS** on an SSO-capable install (`NEXTAUTH_PROVIDER !== "email"`): the hook refuses `/sign-in/email` and `/sign-up/email` (400 `EMAIL_PASSWORD_DISABLED`, the same refusal the pre-existing credential-mutation block uses; SSO-path refusals on DENY are 403 — implementation note settled post-review). This preserves `main`'s guarantee that an Auth0/Okta deployment cannot mint password accounts — a guarantee that would otherwise be **lost** by the `enabled` formula change (see Consequences; this is the v5 BLOCKER fix). The email-route block keys off **gate-ALLOW**; the SSO-path block keys off **gate-DENY**; both consult the same memoized value in the same async hook — no truth table.
   ...
   - The **better-auth credential-mutation block stays keyed off `env.NEXTAUTH_PROVIDER !== "email"`** (the *configured* mode) for `/set-password`, `/change-password`, `/change-email` and the verification routes: at the better-auth endpoint layer, in **every** gate state, no session can attach or change a password on an SSO-capable deployment. **Nuance (v7): the tRPC `user.changePassword` mutation — a separate endpoint the before-hook never sees — is keyed off the *resolved* provider.** On a denied (coerced email-mode) install a user who recovered via reset owns a real credential account and must be able to change it; `changePassword` requires the *current* password, so it is not the account-takeover vector `set-password` is. The two layers agree with the coerced UI instead of offering a dead button.
...
| **No SSO-account takeover** | On an SSO-capable deployment with a license, no password account can be minted or attached through any Better Auth endpoint; on a denied one, only via inbox-proof reset | Decision 4. Three anchors: (a) `/set-password` refused (400) in every gate state; (b) on ALLOW: `/sign-up/email`, `/sign-in/email`, `/request-password-reset` all refused (400, the v5 BLOCKER anchor); (c) on DENY: reset works end-to-end only with a valid emailed token |
...
| 3 | Better Auth `before` hook — email-path block | `src/server/better-auth/index.ts:546` (same hook) | on ALLOW + SSO-capable, refuse (400) `/sign-in/email` + `/sign-up/email` (BLOCKER fix) |
...
**Negative**
- **Churned customers keep SSO indefinitely** — any once-issued license grants SSO forever (signature-only). Accepted deliberately ("once a customer, never blocked"); commercial pressure for renewals lives in the seats the license sold (v8) and the renewal-nudge log, not in login lockout. Tightening this later is a breaking change requiring its own ADR revision.
...
- Password sign-in/up routes become **mounted** on self-hosted SSO deployments (the `enabled` formula change). On their own this is a bypass — a licensed Auth0 install could mint a password account. The **ALLOW-path email-route block (gate site #3) is the load-bearing guard** and its test anchor (`/sign-up/email` refused with a license) is mandatory, alongside the credential-mutation block. Regression caught in the v5 red-team.
...
- `isGatedSsoPath` is a blocklist, which is the right shape here (DENY must leave the whole email-mode surface working, and that surface is far larger than the federation set) but cannot notice a federating route that a better-auth upgrade adds. `ssoRouteTableCanary.test.ts` enumerates what the library actually mounts and requires every route to carry a reviewed classification, so an added or renamed route fails by name instead of passing silently.
...
- **v7 (2026-07-06)** — Implementation phase; four independent reviews (design, security, tests, correctness) folded in. The gate mechanics, memoization, and blocklists were confirmed correct against the vendored `better-auth@1.6.12` router source; two **recovery-path** defects that silently defeated the no-lockout invariant were fixed:
  - **`beforeAccountCreate` was not gated (Decision 7 completion).** Only `afterUserCreate` rode the platform gate; the account hook still wrote `pendingSsoSetup=true` for a `credential` account whose domain matched an org `ssoDomain`. On a denied install the v6 reset-recovery path creates exactly such an account → permanent, unclearable "Link your SSO account" banner (every SSO path 403s). Now gated on `platformSSOAllowed()` alongside the deactivation check.
  - **tRPC `user.changePassword` keyed off raw env (Decision 4 nuance).** The coerced settings UI offered a Change Password button that the mutation always rejected on a denied social-SSO install. Keyed off the resolved provider now — a recovered user with a real credential account can change it (current-password required, so not the `set-password` takeover vector). The better-auth `/change-password` *endpoint* block is unchanged (defense-in-depth; the app uses the tRPC path).
  - **Design cohesion:** the SSO path predicates were extracted from `better-auth/index.ts` into `better-auth/ssoPathGate.ts` (mirroring `originGate.ts`), making the "single source of truth" claim literal and the before-hook a flat sequence.
  - **Test coverage:** added the MINOR-5 concurrency contract (shared in-flight promise), SaaS `resolveAuthProvider`, denied-mode reset negative-path (bogus token → no credential minted), and the two recovery-path regressions above.
