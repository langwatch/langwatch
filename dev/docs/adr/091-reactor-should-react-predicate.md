# ADR-091: Pure `shouldReact` predicate gates reactor enqueue

**Date:** 2026-06-10

**Status:** Superseded by ADR-075 (post-event work is subscribers and process
managers). `shouldReact` and the reactor it gated no longer exist; the
successor is `EnqueueDispatchOptions.filter` on the event-subscriber
contract, or a process manager's own trigger. Retained for history.
...
- Related ADRs: [ADR-023](./023-orphan-sweep-reactor-chain.md) (reactor
  ...
  the handler does even on cache-rehydrated paths; ADR-088 held this
