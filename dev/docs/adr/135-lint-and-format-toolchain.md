...
**Status:** Proposed. Amended 2026-09-23: ast-grep and the baseline are gone, and
`pnpm lint` is oxlint alone (see the last section).
...
**Related:** [ADR-137](./137-module-source-grammar.md),
...
The three policies this table held (`oxlint`, `comment-block-root`,
`comment-block-review`) were deleted with the baseline on 2026-09-23.
...
## Amendment, 2026-09-23: one registry per linter, no baseline, no ast-grep

The lint review of 2026-09-23 changed four things this ADR describes above.

- **ast-grep is gone.** Layer 4 no longer exists: the rules worth keeping moved
  into the plugin (`clickhouse-tenant-id`, `clickhouse-no-version-order-limit`,
  `require-fetch-timeout`, `no-form-watch-in-child`,
  `no-tautological-assertion`) and the rest were deleted. The fixture guard
  went with it.
- **The baseline is gone**, with its shrink mode and every generated override.
  Every rule reports every finding; a rule the repository does not enforce is
  off by name in the config.
- **`pnpm lint` is oxlint only.** architecture-enforcer runs as
  `pnpm lint:architecture`, which is not part of `pnpm lint` until the tree is
  clean; CI runs the policies already at zero by id (`--policies a,b`). A
  policy whose anchor file is missing refuses the run by name.
- **The plugin registry is one map.** `rules` in
  `packages/oxlint-rules/src/index.mjs` keys each rule by the name its
  `defineRule` declaration carries; adding a rule is that entry plus one
  `"langwatch/<name>": "error"` line in
  `packages/architecture-enforcer/oxlint.architecture.jsonc`. The plugin-config
  guard refuses a rule at `warn`, a registered rule no config enables, and a
  configured rule the registry does not hold. `dev/docs/lint-rules.md` is
  regenerated by `pnpm --filter @langwatch/architecture-enforcer docs` and CI
  runs `docs:check`.

The registry table above is historical; the counts today are read from the
registries themselves, not written here.

### The built-ins enabled workspace-wide
