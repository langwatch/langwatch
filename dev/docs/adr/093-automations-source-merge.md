# ADR-093: one automation flow — Source replaces the kind split, and Slack becomes a project integration
...
```
   ...
   ├─ Alerts        ──┘ same columns       │    Source + Delivery columns,
   └─ Schedules                            │    filter chips
                                           └─ Schedules   (own tab, unchanged)
   Composer: three Type cards              Composer: two Source cards
   ┌────────────┐┌───────────┐┌──────────┐ ┌──────────────┐┌──────────────┐
   │ Automation ││ Alert     ││ Schedule │ │ Trace search ││ Graph metric │
   │ (trace)    ││ (graph)   ││ (clock)  │ │ (magnifier)  ││ (chart)      │
   └────────────┘└───────────┘└──────────┘ └──────────────┘└──────────────┘
                                           Schedules: created from their own
                                           entry point, never a third card
```

**Source** answers "where does the input come from":

- **Trace search** — the automation acts on every trace a search matches, as it arrives. Subject: conditions (Builder by default, Code as the toggle — decided, not open).
- **Graph metric** — the automation watches a series on a graph and acts when it crosses a threshold. Subject: graph + series + threshold rule.

The rename is honest labelling of what the field already is: the draft reducer's `ConditionSource` (`"trace" | "customGraph" | "report"`) does not change. This is UI copy, information architecture, and wire naming — not a semantic change.

**A saved automation's source is immutable.** This is stated as a rule of the merged flow, not a leftover: the graph slot (`customGraphId @unique` — one automation per graph) and the report calendar make a source conversion a create-plus-delete, which is exactly what the public API already enforces as `trigger_kind_immutable` (#6891). The #6716 "type-lock" finding dissolves — there is no type to lock, and the Source step simply renders locked on edit with the same explanation. Severity (`alertType`) keeps its current behaviour: offered for graph-metric automations, absent for trace-search ones.
...
**`kind` does not collapse.** It remains the stored and wire discriminator. `source` is a derived, bijective alias published alongside it:

| Stored `triggerKind` | Wire `kind` (unchanged) | Wire `source` (new, read + write) | UI label |
|---|---|---|---|
| `AUTOMATION` | `AUTOMATION` | `trace_search` | Trace search |
| `ALERT` | `ALERT` | `graph_metric` | Graph metric |
...
- **Error codes are wire contract:** `trigger_kind_immutable` keeps its code — clients match on codes, and renaming a shipped code is a break dressed up as tidiness. The presentation-registry copy for it changes to speak of the source ("this automation's source cannot change; create a new automation instead").
...
Steps were chosen over accordion-trimming. The step sequence, and what each asks:

```
   CREATE (linear, with a persistent step rail)

   ┌─────────┐   ┌──────────────┐   ┌──────────┐   ┌─────────┐
   │ 1 SOURCE│ → │ 2 SUBJECT    │ → │ 3 DELIVER│ → │ 4 REVIEW│ → Save
   └─────────┘   │   + RULE     │   └──────────┘   └─────────┘
                 └──────────────┘
   Source:   Trace search | Graph metric. Pre-picked and locked when
             entered from a graph page or a use-case template.
   Subject:  trace  → conditions (Builder default, Code toggle),
   ...
   Delivery: ONE channel (email | Slack | webhook | dataset | queue),
             ...
             (digest cadence + settle window) — timing rides with
             delivery because it modulates *sending*; the threshold's
             time window stays in the rule, where it modulates *firing*
   Review:   the whole automation on one screen — name (editable
             here and in the header throughout), source, rule,
...
The Aug-4 lesson is designed in rather than remembered: **the overview never disappears.** Creating shows a step rail with each completed step's one-line summary, clickable to go back. Editing opens the Review screen directly — never step 1 — and each section's edit affordance enters *that step alone*, returning to the overview on done. The facet vocabulary (ADR-043) is unchanged; steps are how the facets are *sequenced*, not a new model.
...
- Editing an automation that still carries its own token shows that fact with one action: **"Use the project integration"**, which clears the stored credential (delivery falls through to step 2 above). The settings card shows "N automations in this project still use their own Slack token" with the same action in bulk.
...
The #6716 finding: pick "Error spike" and the automation still has no graph behind it — the template saves no work. In the merged flow, graph-source use-case cards (`AutomationsEducation.tsx`) carry a **graph specification** in their prefill, not just a name and an action. The Subject step shows it as a pre-filled rule over a graph that does not exist yet ("Creates graph: *Error rate*"), editable like any other. Saving creates the graph and the automation in one service call; if the automation write is refused, the just-created graph is deleted — a template must not strand orphan graphs (#6896 already tracks orphan graphs as a defect class).

Templates always create a **new** graph rather than binding an existing one: `customGraphId` is `@unique`, so reusing a graph the user already has an automation on would be refused, and guessing which existing graph the user meant is exactly the dead end the finding describes. Trace-source cards are unchanged (they already seed working filters).
...
**Why most-specific-first for the token.** The alternative (project token wins) makes rotation instantly cover legacy rows — attractive — but lets connecting the integration silently repoint live deliveries at a different workspace, which is a data-exposure shape this same bug bash already produced once by another route (the trace-ID preview incident). Explicit migration is a click; silent retargeting is an incident.
...
**Why the delivery timing lives in the Delivery step.** ADR-043 made Cadence one facet so timing controls have a single home; the wizard keeps that home but seats it where the author is already thinking about the channel. The threshold's evaluation window is part of the rule's meaning, not of delivery, so it stays in Subject + Rule. The facet model is unchanged; only its seating in the step sequence is decided here.

## Consequences

- Customer vocabulary shrinks: "automation" (with a source) and "schedule". Copy, docs, and empty states stop needing to explain the automation/alert distinction; the delete-copy noun defect class (#6716) loses its fuel.
- The type-lock question closes as by-design: source is immutable, stated in the flow and enforced by the existing API contract.
...
- The analytics "create alert from a graph" affordance (#6716, both sessions) becomes an entry into the wizard with Source pre-locked to graph metric and the graph pre-picked — no special flow.
...
**Scope:** the rename (Source vocabulary, two-card picker, copy), the merged wizard for create and edit (both sources, Review-as-home), and the unified list as a **read-only view** (one table, Source + Delivery columns; Schedules tab untouched).

**Files (owned):**
- `platform/app/src/features/automations/components/AutomationTypePicker.tsx` → the two-card Source picker
...
| F6 | Unified list interactivity: source/delivery filter chips, row-action coherence with the View drawer, flag removal at the end | `pages/[project]/automations.tsx`, `features/automations/components/page/**` | R0 (+#6899 landed) |
