...
3. **Single installation per project — DB-enforced via a claim row.** A project's bound repos must all belong to one GitHub installation. The junction's `@@unique([repoBindingId, scopeType, scopeId])` cannot express this (it only blocks duplicate rows), so the schema adds `LangyGithubProjectInstallationClaim` with `projectId` as its primary key: binding creation atomically insert-or-gets the claim in the same transaction as the scope row — the `insertOrGetExisting` pattern already used in this exact service for the cross-tenant installation-takeover guard (`langy-github-installations.service.ts:152-159`) — and a claim held by a *different* installation rejects the bind. Never a check-then-write across two awaits; the DB primary key is the arbiter, so two concurrent first-binds from different installations cannot both win. The claim dies with the installation (cascade) and is released when the project's last scope row is removed (service-layer cleanup). If the invariant is ever found violated at mint time anyway, the mint **fails closed** (returns `null`, logs) rather than silently dropping repos from the union. Keeps `LangyCredentials.githubToken` a single string (no downstream contract change) and matches the picker UX — an admin picks from one connected GitHub account's repo list at a time.
...
7. **Organization ownership is derived, never caller-supplied.** `organizationId`, `installationId`, and the polymorphic `scopeId` are independent references, so the schema alone permits a binding whose org, installation owner, and project owner disagree. The write path therefore derives the organization from the **project** (`project.organizationId`), verifies `installation.organizationId` matches it, and stamps `binding.organizationId` from that derived value — a caller can never supply an org. The mint path re-anchors the same way: the bound-set query is `where: { organizationId: project.organizationId, scopes: { some: { scopeType: "PROJECT", scopeId: projectId } } }`, so a stray cross-org row can never reach `mintScoped` (the same defense `getModelsAllowed` documents for `VirtualKeyScope` at `LangyCredentialService.ts:500-505`).
8. **Validation timing: write-time AND mint-time, one listing per mint.** Creating a binding checks the repo is currently in the installation's grant (cached `repositories` list when `repositorySelection="selected"`, live listing when `"all"`). At mint time the installation's repo list is resolved **once** (same cached-or-live rule) and all N bound repos are matched against it in memory — never one lookup per repo. Bound repos that no longer resolve are excluded from the minted set (and logged); this closes the drift window where GitHub revokes a repo after it was bound.
9. **Write permission: `project:manage`.** Same convention as every other `*Scope` table (writing a scope row requires the matching manage-permission on the scope target). Red-team dissent recorded (see Revisions): this permits any project manager in the org to bind any repo the installation reaches. Accepted trade-off — see Consequences.
10. **Installation deletion cascades bindings — intentionally.** The App being uninstalled means the grant is gone, so bindings referencing it are meaningless; `onDelete: Cascade` from `LangyGithubInstallation` enforces the binding ⊆ grant invariant structurally. A reinstall gets a new `installationId` and requires re-binding (see Consequences).
11. **Tenancy registration follows the `ModelProvider` precedent exactly.** `LangyGithubRepoBinding` joins `ORG_TENANCY_EXEMPT` in `dbOrganizationIdProtection.ts` (its primary access path is the scope predicate, which the org guard would reject) and **both** tables get `SCOPED_MODELS` entries in `dbMultiTenancyProtection.ts` mirroring `ModelProvider`/`ModelProviderScope` (`dbMultiTenancyProtection.ts:237-294`): where-clauses require a row id, `organizationId`/parent id, or scope predicate; creates require the scope relation / `(scopeType, scopeId)` payload. The partition test enforces that every org-bearing model is either guarded or listed, so this registration is CI-checked, not convention.
...
| Single-installation-per-project | A project's bound repos never span two GitHub installations | `LangyGithubProjectInstallationClaim.projectId` primary key — the DB is the arbiter (insert-or-get in the bind transaction), not a service-layer check-then-write; mint fails closed if violated anyway |
| Org triangle agrees | `project.organizationId == binding.organizationId == installation.organizationId`, always | Org derived from the project server-side (never caller-supplied), installation checked against it at write-time; mint query anchored on `project.organizationId` so cross-org rows can't reach `mintScoped` |
...
| Binding-write path (permission, single-installation claim, grant check, org triangle) | Yes | Medium | Automated tests: permission-denied for non-manager; concurrent-bind race yields one installation (claim PK conflict); out-of-grant repo rejected; installation from another org rejected |
...
// DB arbiter for the single-installation-per-project invariant: the unique
// constraints above cannot express "all of a project's bindings share one
// installation," so binding creation insert-or-gets this claim in the same
// transaction as the scope row. A claim held by a different installation
// rejects the bind. Released by the service when the project's last scope
// row is removed; dies with the installation on uninstall.
model LangyGithubProjectInstallationClaim {
  projectId      String                  @id
  installationId String
  installation   LangyGithubInstallation @relation(fields: [installationId], references: [installationId], onDelete: Cascade)
  organizationId String
...
- v3 (2026-07-31) — Sergio Esteban, after PR review (CodeRabbit on langwatch/langwatch#6388): two of four findings valid, both folded in. **(a) Org-ownership triangle** — the schema's independent `organizationId`/`installationId`/`scopeId` references permitted a binding whose org, installation owner, and project owner disagree; Decision §7 now requires deriving the org from the project server-side, checking the installation against it at write-time, and anchoring the mint query on `project.organizationId`. **(b) Single-installation enforcement** — v2's "claims via the DB's unique index" over-claimed: no proposed index could express the invariant. Added `LangyGithubProjectInstallationClaim` (projectId as primary key) so the DB is the arbiter of concurrent first-binds. Dismissed: the date nitpick (the dates are correct) and the em-dash rule (it governs customer-facing copy per `copywriting.md`; 78 of 87 existing ADRs use em dashes).
