...
**Builds on:** the existing relay dual-export lane (`services/langyagent/adapters/otelrelay`, `otel/angelinajolie.go` — the content-stripped operational copy), ADR-070/ADR-053 (egress), ADR-055 (canonical OTLP pipelines), `specs/langy/langy-otel-tracing.feature` (amended by this ADR).
