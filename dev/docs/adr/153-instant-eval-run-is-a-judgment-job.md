# ADR-153: An Instant Eval run is a judgment job over the same statement, not a monitor run

> Numbered 153 here, not 137: ADR-137 is the module source grammar on this branch.
...
the answers.
[ADR-052](052-automations-on-process-manager-substrate.md): the loop is a
...
[ADR-051](051-event-sourced-topic-clustering.md) (the paging pipeline this copies),
...
- `instant_eval_stalled` is a code recorded on a row rather than thrown. Nothing
  ...
  the orphan check in `packages/error-presentation/src/__tests__/codes.unit.test.ts`
