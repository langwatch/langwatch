...
Key structural facts of the retention system the design exploits: the 11 retention-managed tables map to **3 retention categories** (traces = 7 tables, scenarios = 2, experiments = 2 — `retentionPolicy.schema.ts`), are partitioned by `toYearWeek`, and rows carry `_retention_days` (per-tenant, per-category). The billing cutoff `BILLABLE_AFTER_DAYS = 35` is a clean week-partition boundary. **Retention is deterministic — every row's entry into and exit from the billable window is knowable in advance.**

Two scope facts every budget below depends on (perf red-team F0):

- **The 35-day paid retention floor is a coupled product change, not current code.** Today's floor is 49d (`MIN_RETENTION_DAYS`, schema `.min(49)`, migration default 308d) — the drop to 35 ships **with** this billing work, kept from the superseded decision. Until it ships, *every* paid org carries a billable window of at least 49 − 35 = 14 days (≥ 2 partitions); grandfathered 308d orgs carry ~39. The "€0-by-construction, zero queries" state exists only once a customer can actually select 35. Metering GA therefore **depends on** the floor change (see Open questions).
...
**Query unit vs. emit unit (perf red-team F7):** the measurement queries run per physical **table** (11 — each `FROM <table>`, no cross-table scan possible); events are emitted per **category** (3) — the per-table results for one `(project, category, partition, slice, retentionDays)` are **summed before emit**, so the dedup key stays category-grained without collisions. CH query budgets scale with 11; PG row budgets scale with 3.
...
- **Rotating reference audit — ClickHouse, capped.** Re-measure at most **N partitions per org per day** (N sized so full coverage completes within ~12 days), always including partitions touched by a retention change. Catches measurement-side drift (the late-arrival and retroactive-ALTER classes) within the rotation period rather than next-day. The cap is an **invariant, not a tunable**.
...
|---|---|---|
| Entry transit queries (steady state) | ~11k/day ≈ 0.13 qps | orgs × projects × 11 tables × partitions-in-transit × 1/day |
| `evaluation_runs` measured exits | ~1/org-project-week (≈ 7/day for the worst org) | Same shape/caps as entry; exit query MUST carry the partition-key literal, or it cannot prune (perf F8) |
| Daily fold audit | 0 CH queries | Postgres event-log recompute + Stripe totals comparison |
| Rotating reference audit | ≤ N partitions/org/day, full coverage ≤ ~12 days | Invariant, not tunable (perf F1) |
...
| ⚠️ Coupled 49→35 paid-retention-floor change — must ship before (or with) metering GA; without it no org can reach the €0-by-construction state (perf F0) | Product + billing rollout | **Yes — GA gate** |
