...
ADR-049 (Postgres projections), ADR-072 (turn-phase machine — the proof of
...
layer that fights async timing by hand (ADR-072 already had to consolidate one
corner of it into a state machine).

ADR-072's `langyTurnPhase` proved a better shape: a **pure `(state, event) →
...
- `turnPhase` collapses into a **derivation**: recorded turn status ⊕ the
  ...
  `agent_turn_accepted`) ⊕ stop-requested. The ADR-072 reducer shrinks to the
...
- **Keep signal-then-refetch, just tidy the booleans.** Rejected as the
  ceiling: it leaves two models of a turn and re-downloads projections; ADR-072
...
- Server fold projections become thin wrappers over shared reducers (ADR-072
