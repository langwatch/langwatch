...
ADR-110 collapsed the grants rollout this ADR mirrored: the cutover table, the cutover flag and the cached gate in front of it are gone, and **the migration's own `finalized` status is the fork** (`engine-gate.ts`). Enrollment is a switch, not a programme — no cohorts, no sampling, no pacing ladder. A migration **does not wait**: it states its facts, checks once, and a tenant whose projection is behind is *held* with the outstanding facts named. This ADR now says the same things in identity's terms (§2, §6): the write gate opens on `finalized` only (`migrated` is held), org enrollment expands to member users and the everyone-else cohort is deleted, and the backfill restates every pass with compensating detaches. One ADR-110 rule identity does **not** adopt, on purpose: "every write goes through the group queue, no inline fold; Redis down ⇒ writes down". The front door must survive Redis loss (D02) and the router (D03) needs read-your-writes, so identity keeps the fold apply on the calling path — a recorded divergence, justified in §Rationale, not an oversight.
...
Command → event → projection, never a hand-written upsert in a handler. Protocol values (password hash, tokens) ride only on the command — transient, never durably stored in events — and land through the credentials repository as row-truth on `Account`. Read-your-writes holds because the fold apply runs on the calling path; the queue fold re-applies the same events later and converges because applies are idempotent. This is identity's one stated departure from ADR-110's queue-only rule (see §Revision 2026-08-23 and §Rationale). A thin endpoint-hook plugin stamps ceremony context (flow, actor, request metadata) onto request-scoped storage so the adapter knows why a row is written.
...
**Calling-path apply over ADR-110's queue-only rule.** ADR-110 states that every authorization write goes through the group queue and that Redis down means authorization writes are down. Identity cannot take that position: its writes *are* sign-in, and D02 exists precisely so a Redis outage stops locking every customer out. The fold apply therefore runs on the calling path (append waited, apply, then staging best-effort), which also gives D03's router read-your-writes without a convergence wait. The cost ADR-110 avoids — an inline apply racing the queue fold — is bounded here by idempotent applies, a per-user aggregate that is a handful of rows (never the unbounded aggregate ADR-110 was fixing), and identity volume in the hundreds per day. The divergence is recorded, not incidental.
