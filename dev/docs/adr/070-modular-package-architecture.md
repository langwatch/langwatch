# ADR-070: Modular Package Architecture — bounded-context packages for runtime footprint and type isolation

**Date:** 2026-07-21

**Status:** Proposed

**Related:** PR #6023 (runtime footprint: browser UI out of the backend, env-gated DLP/OTel), ADR-076 (single pnpm workspace — the directory restructure this ADR's package moves now build on), PR #6018 (`refactor/type-graph-split`: composite projects, `@langwatch/contracts`), ADR-004 (dev environment), ADR-052 (process-manager substrate), ADR-054 (observability-as-code). Supersedes the ad-hoc lazy-import experiments explored during the #6023 work.
...
**Runtime footprint (PR #6023).** Profiling `pnpm dev` / `pnpm start` (Node 24, tsx over `src/`, 2026-07-21) measured a single server process reaching **~8,500 modules / ~500 MB resident**, whether or not the endpoints that need them are ever hit. We landed three point fixes (keeping the browser UI stack out of backend processes, env-gated Google DLP, need-based OTel instrumentation) but every further attempt (lazy-loading the AI SDK, CopilotKit, stripe) turned into whack-a-mole: a dependency deferred at one call site was still pulled in through another.
...
Restructure `platform/app/` into a strict, acyclic DAG of bounded-context packages. Concretely:
...
| `observability` | `setupObservability`, `createLogger` | shared | (SDK, but small) | `packages/observability` (exists) |
...
- **`observability`** is the telemetry _emission_ SDK (exists as `packages/observability`). Distinct from `otel-api`, which is the _ingestion_ receiver. Everyone emits through `observability`; only the ingestion process depends on `otel-api`.
...
An emit probe (`platform/app/tsconfig.emit-probe.json`, prototyped in #6018 but never merged — see Phase 0) measures the gap; the remaining errors are the `AppRouter` cluster.
...
### Phase 0 — Foundations (attempted in #6018; **not** landed)

**#6018 was closed without merging.** Nothing in this phase exists on `main`:
there is no `packages/contracts`, no `tsconfig.emit-probe.json`, and no
`typecheck:packages` script. The work below is prior art to draw on, not a
foundation to build on — treat all of Phase 0 as still to do.

- **0.1** Composite projects + `tsgo -b` to stop triple-checking (editor tsconfig, app tsgo, tests tsgo each re-check the graph). _Attempted in #6018; must be redone._
- **0.2** Extract the `shared`/`@langwatch/contracts` leaf with declaration emit. _Attempted in #6018 (~460 import sites rewritten); must be redone._
...
**Validation:** a `typecheck:packages` script (tsgo `--build` of the composite packages) — to be added by 0.1 — green; eslint boundaries reports the current violation set as the migration backlog.
...
- **Whole-program typecheck: CI only.** The app typecheck is RAM-banned locally (~6 GB peak pins a laptop); each full-program fix round-trips through CI. Budget for that latency.
...
- **The repo is now a single pnpm workspace with one lockfile (ADR-076).** The app lives at `platform/app` and shared packages at `packages/*`, so the package moves this ADR proposes no longer have to invent a workspace layout — they inherit one.
