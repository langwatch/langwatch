# ADR-085: Scenario Orphaned-Run Reconciliation
...
>
> **Corrected 2026-07-29: not yet, and one file is back.** Deleting both specs
> ran ahead of deleting the code. The boot sweeps are still wired and still
> shipping — deliberately, as the one-release cutover drain described above —
> so for that release they were live, tested behaviour with no spec at all, and
> nothing would have checked their removal.
> [`specs/scenarios/orphaned-run-reconciliation.feature`](../../../specs/scenarios/orphaned-run-reconciliation.feature)
> is restored, trimmed to the retained sweeps only and bound to
> `scenario-orphan-reconciler.unit.test.ts`. It is deleted together with the
> drain, not before it. `queued-run-orphan-recovery.feature` stays gone — its
> scenarios are folded into the restored file — and the graceful-drain
> scenarios stay gone too, because `drainInFlightRuns` really is deleted.
>
