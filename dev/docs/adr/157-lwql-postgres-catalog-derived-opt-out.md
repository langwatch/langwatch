...
- **Positive.** Topics, datasets, workflows, gateway objects, and governance
  ...
  catalog is the single source of truth for all deployments (per [ADR-159](./159-the-app-owns-the-lwql-access-model.md)), so the published docs cannot drift from what the catalog actually contains.
