...
- Reuses `/api/otel`, the GenAI core, `IngestionKeyService`, and the scoped shell-function tier (`shell-rc.ts`) — no new delivery mechanism. The new code is small and follows the copilot-CLI pattern: a `code` entry in `SHELL_FUNCTION_TOOLS`, a `buildOtelEnvBlock` `code` case, a `code → copilot_vscode` sourceType-map entry, `envForTool` wiring, and a hidden `langwatch code` command. A settings.json writer is needed only for the deferred Dock/always-on path, not for v1.
...
### Open questions
- Env-only enable (verify before implementing): confirm the scoped `code()` env alone activates the extension's OTel — i.e. `COPILOT_OTEL_ENABLED`/`OTEL_EXPORTER_OTLP_ENDPOINT` env override the default-false `github.copilot.chat.otel.enabled` setting. The validation turn set both env and settings; if env-alone does NOT enable it, v1 also needs the settings.json writer on the `code .` path. One-line check (launch `code` with env, no settings, one turn).
...
- 2026-07-15 — GitHub Copilot in VS Code (§Extension: Copilot Chat): native `github.copilot.chat.otel.*` settings carry enable/exporter/endpoint/capture; the ingest token — the only piece with no settings key — rides `OTEL_EXPORTER_OTLP_HEADERS`, delivered by the **existing scoped shell-function tier** (`code` added to `SHELL_FUNCTION_TOOLS` + `buildShellReapply`), the same mechanism the copilot CLI uses; sourceType `copilot_vscode`, tokens-only v1 (cost + AI-units deferred as codenames are obfuscated). Empirically validated on VS Code 1.128.1 / Copilot Chat 0.56.0: a real Chat turn (`panel/editAgent`, model `oswe-vscode-prime`) landed with prompt, tokens, and content. Proposed, PR on the #5784 stack (issue #5813).
  ...
  - Function-alignment pass: named the real functions — the persisted `code()` builder is `buildScopedToolFunction` + `persistBlockToRc`/`toolMarkers` (not `buildShellReapply`, which is the live-run prefix). Surfaced the full v1 touch points (`SHELL_FUNCTION_TOOLS`, a `buildOtelEnvBlock` `code` case, `code → copilot_vscode` sourceType map, a hidden `langwatch code` command). Noted settings.json is redundant on the `code .` path (env carries it) pending an env-only enable check.
