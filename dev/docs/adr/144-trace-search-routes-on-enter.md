# ADR-144: Trace search routes on Enter

> Landed from upstream as ADR-139; renumbered here because 139 is taken by another record.
...
A text made of `field:value` terms is applied as typed, with no request. A text with bare words is a sentence, and a new tRPC procedure, `traces.routeSearch`, decides what it is. The router (`server/app-layer/traces/search-router/route-search.ts`) answers one of four kinds:
