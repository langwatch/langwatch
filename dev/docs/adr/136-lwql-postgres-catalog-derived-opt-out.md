# ADR-136: The PostgreSQL half of the LangWatchQL catalog is derived from the Prisma manifest, opt-out
...
- `platform/app/src/server/analytics/lwql/catalog/derivePostgresCatalog.ts` — the derivation
- `platform/app/src/server/analytics/lwql/catalog/postgresSkippedModels.ts` — the skip list and its four reason categories
- `platform/app/src/server/analytics/lwql/catalog/prismaSchema.ts` — the manifest parser
- `platform/app/src/server/analytics/lwql/catalog/__tests__/tenantModelCoverage.unit.test.ts` — the coverage guard
