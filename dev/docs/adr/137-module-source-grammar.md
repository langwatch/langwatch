...
| `langwatch/rest-route` | plugin | A REST declaration imports its schemas from its own contract, declares input and output, keeps path parameters semantic except that a route main already publishes (`docs/api-reference/openapiLangWatch.json`) keeps the names it published, and answers by returning or throwing, never by building a response. Replaces the six `rest-*` rules. |
