...
**Relates to:** [ADR-004](./004-docker-dev-environment.md) (process roles), [ADR-052](./052-automations-on-process-manager-substrate.md) (revision-fenced scheduling), [packages/features/ops/specs/queue-discovery.feature](../../../packages/features/ops/specs/queue-discovery.feature), [Group Queue tenant parking](../../../packages/group-queue/specs/tenant-soft-cap.feature), [Group Queue poison parking](../../../packages/group-queue/specs/poison-group-park-guard.feature), and [specs/ops/dashboard-latency.feature](../../../specs/ops/dashboard-latency.feature).
...
One word needs pinning down before the rest reads correctly. **"Parked" in this ADR always means tenant soft-cap parking** — a group moved out of the ready zset into a per-tenant parked set because its tenant is at its in-flight cap ([packages/group-queue/specs/tenant-soft-cap.feature](../../../packages/group-queue/specs/tenant-soft-cap.feature)). The GroupQueue has a second, unrelated "park": the poison-group guard parks a crash-looping group _into the blocked set_ ([packages/group-queue/specs/poison-group-park-guard.feature](../../../packages/group-queue/specs/poison-group-park-guard.feature)), where it counts toward Blocked and surfaces through `getBlockedSummary`. The two never mix on the dashboard, and the Parked panel this ADR adds shows only the first kind. Both documents keep their own vocabulary; this ADR does not rename either.
...
The pending-counter reconcile is deliberately **out of scope**. It looks like fleet-duplicated work — every pod runs the loop every 60s — but it is already coordinated: [packages/features/ops/specs/pending-counter-reconcile.feature](../../../packages/features/ops/specs/pending-counter-reconcile.feature) specifies a cross-instance single-flight marker, down to what a pass that loses the marker mid-run may publish (nothing) and what the write does without the marker (refuses). Every pod _attempts_; one _runs_. Gating it on the snapshot lease as well would stack two coordination mechanisms over one loop, and would make several of that spec's scenarios unreachable — a pod that never attempts can never decline, and can never lose a marker it never held. The reconcile keeps its marker and its interval, on every pod, unchanged by this ADR.
...
- "Parked: 60" is now clickable down to tenant and group — the count finally explains itself. Parked ≠ broken is legible on the page: a parked tenant row shows which tenant, which queue, how many groups and how long the oldest has waited, not an error. The cap itself is deliberately not on the row — it is per-queue configuration, so repeating it once per tenant would add width without adding an answer.
...
- Out of scope, and why: [packages/features/ops/specs/pending-counter-reconcile.feature](../../../packages/features/ops/specs/pending-counter-reconcile.feature) (single-flight marker, #4683)
