...
**Follow-up (2026-08-06): fixed by [ADR-087](./087-trace-summary-storage-anchor.md), migration 00072.** `trace_summaries.OccurredAt` now carries the same frozen `storageAnchorMs`, its span timing baseline moved to its own `EarliestSpanStartMs` column, and the pre-split stamp (`2026-05-07`) is decoded rather than refused so the population heals in place. The four helpers this ADR's rule is made of - `MAX_ANCHOR_FUTURE_SKEW_MS`, `isUsableAnchorMs`, `firstUsableAnchor`, `anchorStorageTime` - moved out of `traceAnalytics.foldProjection.ts` into `projections/services/storage-anchor.ts` and are now shared by both folds, which is where `evaluation_analytics` and `coding_agent_sessions` step 3 should take them from.
