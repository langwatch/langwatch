# ADR-026: Liquid templates for user-customizable trigger notifications
...
[ADR-025](./025-notify-persistent-action-classification.md) introduces cadence and digest semantics. The template system must work for both single-match (`length === 1`) and digest (`length > 1`) cases without forcing the customer to write two templates.
...
- `'block_kit'`: Liquid output is parsed as JSON and sent as a `blocks` payload. JSON parse failure → fall back to default template, log, surface in operator activity tab.
...
- On render: try/catch the Liquid call. On failure, render the default template, log + capture, surface "rendered with the default template due to template error" in the operator activity tab.
...
- **Operator-facing surfaces** (Phase 6, see plan):
  ...
  - Slack preview: in-app renderer for the allowlist + "Open in Slack Block Kit Builder" deep-link (`https://app.slack.com/block-kit-builder#{encodedJson}`).
...
- **`strictVariables: false` trade-off.** Missing variables render silently. Mitigation: the operator activity tab surfaces "rendered with N missing variables: [list]" so authors learn about typos without dispatch failures.
...
- [ADR-021](./021-transactional-outbox-for-stake-sensitive-dispatch.md) — outbox dispatch is the renderer's caller
- [ADR-025](./025-notify-persistent-action-classification.md) — cadence model that produces `matches[]` of varying length
