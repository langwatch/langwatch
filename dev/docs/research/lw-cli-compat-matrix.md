...
| 8   | Natural-language escape hatch routed to the product's assistant                                                                                                           | ✅ (`gcx assistant prompt`) | ✅ (`gh copilot`, `gh agent-task`) | ⚠️ Langy in-product only; **no CLI `ask` yet** — P5 investigation verdict: Langy's only entry points are tRPC procedures (`modules/langy/server/src/transport/api-trpc/langy.api.ts`) gated by a BetterAuth session cookie (no API-key plugin) plus the `release_langy_enabled` rollout flag, so a CLI holding only `LANGWATCH_API_KEY` cannot reach it. Scoped as a server follow-up — see Open items                                                                                                                                                                                                                                                                                                 | follow-up (server endpoint)                                |
...
- Until the backend ships `suggestions`, a code-keyed client-side fallback table
  ...
  `modules/langy/web/src/features/langy/behavior/logic/langy-error-explainer.ts`) covers the 10 most
...
- `lw ask` (NOT BUILT — needs a server endpoint): the investigation found no
  ...
  (`modules/langy/server/src/transport/api-trpc/langy.api.ts`) behind `protectedProcedure` →
  ...
  `modules/langy/server/src/transport/api-rest/`) are worker callbacks
...
- [ ] `lw ask "<question>"` — server endpoint needed first (P5 verdict: no
      ...
      `modules/langy/server/src/transport/api-rest/`'s existing routes, at
