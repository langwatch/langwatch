...
### LangWatch MCP (REQUIRED for experiments with evaluators)

The LangWatch MCP is **required** — not optional — when setting up experiments that use evaluators. SDK evaluators (e.g. `ragas/answer_relevancy`, `off-topic`, `llm-answer-match`) run on the LangWatch platform and need:
1. A **model provider** configured on the platform (the LLM API key that powers the evaluator)
2. The **evaluator to exist** on the platform

Without MCP access, you cannot create evaluators or configure model providers, and the experiment will silently fail with "Evaluator not found" errors on every row.

**Set up the MCP before proceeding:**

See [MCP Setup](_shared/mcp-setup.md) for installation instructions.

If MCP installation fails:
1. For documentation, see [docs fallback](_shared/llms-txt-fallback.md)
2. For evaluator/model provider setup, you MUST get MCP working or tell the user to manually create evaluators at https://app.langwatch.ai → Evaluators before running the experiment
...
4. **Set up evaluators on the platform BEFORE writing experiment code.** SDK evaluators run server-side — if the slug doesn't exist on the platform, every row fails silently with "Evaluator not found".

   a. `platform_list_model_providers` → if none, detect the user's LLM key from `.env` and configure it with `platform_set_model_provider`
   b. `platform_list_evaluators` → if none, call `discover_schema` category "evaluators", then `platform_create_evaluator` for evaluators suited to the agent's domain
   c. Note the **slugs** returned — use these exact slugs in the experiment code

5. Create the experiment file. CRITICAL: evaluator slugs in `evaluation.evaluate("slug", ...)` MUST match evaluators on the platform from step 4. Do NOT hardcode slugs like `ragas/answer_relevancy` — use the slugs you just created/verified.
...
for index, row in evaluation.loop(df.iterrows()):
    ...
    # Use the EXACT slug from the evaluator you created on the platform
    evaluation.evaluate(
        "your-evaluator-slug",  # ← slug from platform_create_evaluator
        index=index,
...
await evaluation.run(dataset, async ({ item, index }) => {
  ...
  // Use the EXACT slug from the evaluator you created on the platform
  await evaluation.evaluate("your-evaluator-slug", {  // ← slug from platform_create_evaluator
    index,
...
6. Run the experiment to verify it works
...
- Do NOT run an experiment with SDK evaluators without first ensuring the evaluators exist on the platform and a model provider is configured — otherwise every evaluation row will fail with "Evaluator not found" and the experiment will appear successful but have zero results
