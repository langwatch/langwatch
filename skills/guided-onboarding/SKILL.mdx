...
Work on a branch of your own, never on the branch the user has checked out: `git checkout -b langy/<slug> origin/<default>` (with no start point when the tree is dirty), as step 2 of `code-changes` says. When the workspace facts say `git: not a repository`, there is no branch to make: that is the first unlock question of "When a step fails", asked before any command of this step. On that branch, load the `tracing` and `connect-agent` skills with the `skill` tool, then, in this order:
