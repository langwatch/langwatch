...
1. `tracing` for the detected framework, so every call is traced. The docs page is `langwatch docs integration/<python|typescript>/integrations/<framework>`, for example `integration/python/integrations/langgraph`. Keep the order the tracing skill pins: the environment loads first and LangWatch initialises after it, so `langwatch.setup()` sits below the import that loads the env file, never at the top of the entry file.
