  ...
  describe("when the worker is killed", () => {
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        ...
        fs.writeFileSync(path.join(binDir, "claude"), fakeClaudeScript(workingDirectory), {
          mode: 0o755,
        });
        ...
        try {
          await waitUntil({
            condition: () => {
              ...
              claudeChildPid = readPid(path.join(workingDirectory, "claude-child.pid"));
          ...
          await waitUntil({
            condition: () =>
              !isRunning(claudePid as number) && !isRunning(claudeChildPid as number),
            timeoutMs: 30_000,
