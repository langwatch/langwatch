...
async function withDatasetWorkDir<T>({
  prefix,
  fixtureSubpath,
  run,
}: {
  ...
  try {
    ...
    copySkillToWorkDir(workingDirectory);
...
function findGeneratedFiles({ dir, extensions }: { dir: string; extensions: string[] }): string[] {
...
describe("Dataset Generation Skill", () => {
  it.skipIf(isCI)(
    ...
    async () => {
      await withDatasetWorkDir({
        prefix: "langwatch-skill-dataset-py-",
        fixtureSubpath: "python-openai",
        run: async (tempFolder) => {
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      await withDatasetWorkDir({
        prefix: "langwatch-skill-dataset-rag-",
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      await withDatasetWorkDir({
        prefix: "langwatch-skill-dataset-multiturn-",
        fixtureSubpath: "python-openai",
        run: async (tempFolder) => {
          // Only forward LANGWATCH_ENDPOINT when it's actually set in the parent
          // process. Otherwise the CLI uses its default (https://app.langwatch.ai/)
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      await withDatasetWorkDir({
        prefix: "langwatch-skill-dataset-hallucination-",
        fixtureSubpath: "python-rag-agent",
        run: async (tempFolder) => {
          // Only forward LANGWATCH_ENDPOINT when it's actually set in the parent
          // process. Otherwise the CLI uses its default (https://app.langwatch.ai/)
            ...
            script: [
              ...
              (state) => {
                ...
                const hasContextColumn =
                  headerLine.includes("context") || headerLine.includes("expected_context");
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      await withDatasetWorkDir({
        prefix: "langwatch-skill-dataset-ts-generic-",
