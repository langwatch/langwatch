import { type AgentAdapter, AgentRole, type ScenarioExecutionStateLike } from "@langwatch/scenario";
  ...
  return {
    ...
    call: async (state) => {
          ...
          .map((block: any) => {
            ...
            switch (block.type) {
              ...
              case "tool-result": {
                ...
                const inner = typeof value === "string" ? value : JSON.stringify(value);
      ...
      return new Promise<string>((resolve, reject) => {
        const claudeBin =
          process.env.CLAUDE_BIN || execSync("which claude", { encoding: "utf8" }).trim();
        ...
        const envVars =
          omittedKeys.size > 0
            ? Object.fromEntries(
                Object.entries(process.env).filter(([key]) => !omittedKeys.has(key)),
              )
            : process.env;
        ...
        child.on("close", (exitCode) => {
          if (exitCode === 0) {
            ...
            reject(
              new Error(
                `Claude Code failed with exit code ${exitCode}` + `${describeFailure(output)}`,
              ),
            );
          }
        });
...
function describeFailure(output: string): string {
  for (const line of output.split("\n").reverse()) {
    ...
    try {
      ...
      const reasons = [parsed.stop_reason, parsed.terminal_reason].filter(Boolean).join(", ");
...
function userMessagesFromBlocks({
  ...
  const texts = content
    ...
    .map((block: any) => (block?.type === "text" ? block.text : unsupportedBlockText(block)));
  if (texts.length > 0) {
 ...
 */
export function claudeCodeTranscriptToModelMessages(rawMessages: any[]): ModelMessage[] {
 ...
 */
export function assertSkillWasRead(state: ScenarioExecutionStateLike, skillName: string): void {
