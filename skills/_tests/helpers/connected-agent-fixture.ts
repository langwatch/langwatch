...
function listAgents(workingDirectory: string): AgentRow[] {
  const output = execFileSync("node", [cliDistPath, "agent", "list", "--format", "json"], {
...
async function stopAgentProcess(agent: AgentProcess): Promise<void> {
  ...
  await Promise.race([new Promise((resolve) => agent.child.once("exit", resolve)), sleep(10_000)]);
...
function deleteAgentRow({ workingDirectory, id }: { workingDirectory: string; id: string }): void {
  try {
