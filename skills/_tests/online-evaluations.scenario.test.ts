        ...
        const result = await scenario.run({
          ...
          script: [
            ...
            (state) => {
              ...
              expect(transcript).toMatch(/"command":"[^"]*langwatch monitor create/);
              expect(transcript).toMatch(/"command":"[^"]*langwatch monitor (get|list)/);
              toolCallFix(state);
      ...
      try {
        const result = await scenario.run({
          ...
          script: [
            ...
            (state) => {
              const transcript = executedCommandTranscript(state);
              expect(transcript).not.toMatch(/"command":"[^"]*langwatch monitor create/);
              toolCallFix(state);
              assertSkillWasRead(state, "online-evaluations");
            },
            scenario.judge(),
