...
async function listMonitors(): Promise<Monitor[]> {
  ...
  if (!response.ok) {
    throw new Error(`Unable to list real monitors: ${response.status} ${await response.text()}`);
  }
...
describe("Online Evaluations Skill", () => {
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      const tempFolder = createSkillTestWorkDir("langwatch-skill-online-evaluation-");
      installSkillToWorkDir({
      ...
      try {
        ...
        const created = (await listMonitors()).find((monitor) => monitor.name === monitorName);
        expect(created, "expected the agent to create a real monitor").toBeDefined();
        ...
        const leftovers = (await listMonitors()).filter((monitor) => monitor.name === monitorName);
        await Promise.all(leftovers.map((monitor) => deleteMonitor(monitor.id)));
  ...
  it.skipIf(isCI)(
    ...
    async () => {
      const tempFolder = createSkillTestWorkDir("langwatch-skill-online-to-experiments-");
      installSkillToWorkDir({
        workingDirectory: tempFolder,
        skillSubpath: "online-evaluations",
      });
