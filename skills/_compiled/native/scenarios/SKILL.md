...
- ALWAYS run the tests after writing them. If they fail, debug and fix the test or the agent code (see Improving the Agent When a Scenario Fails below before touching the agent's prompt).
...
3. **Prove the class.** Re-run the scenario several times, or with varied wording, before accepting a fix. The simulator improvises, so a fix that only survives one phrasing is a transcript patch wearing a principle's clothes.
