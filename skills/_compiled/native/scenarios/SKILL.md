...
- When the scenario reproduces a trace or a conversation the user pasted, `--situation` carries the identifiers the agent looks up (the names, emails and order, ticket or account ids that appear as tool-call inputs in `langwatch trace get <traceId> -f json`) verbatim into the situation. A stand-in makes the simulated user invent one, the agent's lookup misses, and the run fails for a reason the original never had. Only the identifiers the agent looks up travel verbatim; everything else keeps its shape with a stand-in. The situation is stored on the scenario and readable by anyone in the project who can see scenarios, which is a wider audience than a trace with restricted content: name the values you are about to copy, say who will be able to read them, and wait for the user's go-ahead before you run `scenario create`. Run parameters still travel in `--param`, never in the situation. A `[REDACTED]` value means the project redacts PII at ingestion: tell the user and ask for the value, never invent one.
