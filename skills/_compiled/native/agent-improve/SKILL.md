...
Keep the identifiers the agent looks up. The trace shows them as tool-call inputs in `langwatch trace get <traceId> -f json`: a name, an email, an order, ticket or account id. Copy them verbatim into the situation, because a stand-in reproduces a lookup miss, not the failure you are chasing. Everything else sensitive (the customer's wording, account data, secrets) keeps its STRUCTURE (length, language, format, the property that triggers the failure) with realistic stand-ins, and the original is referenced as a trace link in the test's comment instead of pasted. A `[REDACTED]` value in the trace means the project redacts PII at ingestion: tell the user and ask for the value, never invent one. A platform scenario is readable by anyone in the project who can see scenarios, a wider audience than a trace with restricted content: name the values you are about to copy, say who will be able to read them, and wait for the user's go-ahead before you create the scenario. Verbatim identifiers belong in platform scenarios; in a committed test only when the repository seeds that record, since raw customer content never goes into tests, fixtures, or PR descriptions.
...
- Do NOT invent test inputs when real failing traces exist; copy the identifiers the agent looks up and keep the structure of everything else
