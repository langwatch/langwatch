...
When you operate under a contract that forbids asking the user questions (for example an in-product agent with act-immediately rules), do not stall on the gate: present the ranked hypotheses, state which one you are executing and why in one line, and proceed with the top-ranked one. The explanation duty stays either way.
