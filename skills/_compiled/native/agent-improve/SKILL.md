...
Keep the identifiers the agent looks up. The trace shows them as tool-call inputs in `langwatch trace get <traceId> -f json`: a name, an email, an order, ticket or account id. Copy them verbatim into the situation, because a stand-in reproduces a lookup miss, not the failure you are chasing. Everything else sensitive (the customer's wording, account data, secrets) keeps its STRUCTURE (length, language, format, the property that triggers the failure) with realistic stand-ins, and the original is referenced as a trace link in the test's comment instead of pasted. A `[REDACTED]` value in the trace means the project redacts PII at ingestion: tell the user and ask for the value, never invent one. Verbatim identifiers belong in platform scenarios; in a committed test only when the repository seeds that record, since raw customer content never goes into tests, fixtures, or PR descriptions.
