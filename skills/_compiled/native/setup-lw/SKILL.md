...
| A command hangs waiting for input | Interactive prompt in a non-interactive context | Set `LANGWATCH_API_KEY` and re-run with the non-interactive flags (`-y`, `-o json`): the CLI never prompts when it detects an agent |
