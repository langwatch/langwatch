...
- If the `agent-improvement` skill is installed, offer to run `/agent-improvement` on the findings right away: it turns each finding into tested hypotheses, scenario tests, evaluators, and PR-ready changes.
- If it is not installed, tell the user to install it: `npx skills add langwatch/skills/agent-improvement`

Pass along the report: agent-improvement uses these findings and trace examples as its evidence base.
