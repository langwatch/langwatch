...
| **M** ⏳ (2026-05-04 dogfood — backend projection half) | `api.gatewayBudgets.list` returns `spentUsd: '0'` for budgets that genuinely have ledger rows (verified via direct CH query on `gateway_budget_ledger_events`). The list query's `spentUsd` projection isn't joining/aggregating the ledger rows for the just-seeded org — likely the same class of TenantId vs projectId vs orgId predicate-resolution mismatch as the original `/gateway/usage` empty-state observation. **Wider than originally framed**: this also surfaces on the `/gateway/budgets/[id]` list view, not just `/gateway/usage`. Sergey's row N fix (`90a3b2002`) closes the `personalUsage.service.ts` query path (`dailyBuckets` + `recentActivity` ILLEGAL_AGGREGATION) but does NOT close this `gatewayBudgets.list` projection issue. | S | ⏳ in-flight — Sergey investigating the `gatewayBudgets.list` projection predicate |
