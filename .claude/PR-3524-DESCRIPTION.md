...
> 1. **Governance-reactor registration gap on the trace_processing pipeline.** During Sergey's smoke run (post-Docker-recovery, post-CH-schema-drift-fix), workers boot cleanly + reactors load with no `MODULE_NOT_FOUND`, but only **8 of 11** reactors stage on the `trace_processing` pipeline (`originGate` / `evaluationTrigger` / `customEvaluationSync` / `traceUpdateBroadcast` / `projectMetadata` / `simulationMetricsSync` / `experimentMetricsSync` / `alertTrigger` ✅). The 3 governance reactors (`gatewayBudgetSync` / `governanceKpisSync` / `governanceOcsfEventsSync`) are conspicuously absent from the staged-reactor list. **This is a separate gap from the `@ee/*` alias miss already fixed in `40c7a4bbc`** — registration code path needs a fix in this PR, NOT a follow-up. Direct PG+CH integration tests (Sergey landing) prove per-reactor write semantics in isolation but do **NOT** prove the production-pipeline path. Phase 7 end-to-end is not actually working until the registration is fixed AND a real gateway-trace smoke fires through the in-flight pipeline producing rows in `gateway_budget_ledger_events` + `governance_kpis` + `governance_ocsf_events`. Sergey + Andre tracking the staged-reactor registration file path to identify the gate condition.
> 2. **Live `IngestionSourceComposer` capture for Phase 10 dogfood (P10-dogfood blocker).** The composer page is wrapped in `EnterpriseLockedSurface` — non-enterprise dogfood org sees only the upgrade upsell card. Lane-B captured the gate state itself (`admin/ingestion/00-license-gate-non-enterprise.png`) as compliance evidence (real / current / valid), but the **populated composer drawer** capture requires the org to be in enterprise mode. Per orchestrator 2026-05-04: do NOT substitute pre-iter-156 stale composer screenshots; instead use the dev/test license fixture or org-plan override pattern already used in the enterprise-gating screenshots to render the live composer against current code. If the license path is genuinely blocked, post the exact blocker + mark the cell as blocked-with-evidence in the §Screenshots grid.
>
