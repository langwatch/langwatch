...
| **Phase 8 — Sessions/Devices** (NEW) | Architecture-spine row above. Backend ✅ (`82ae4b666`); UI `/me/sessions` ✅ (`1e7360a8f`); BDD specs + docs scaffold ✅ (`bd4875f56`); ⏳ remaining: P8-ui-admin-ttl (section/card in existing `/settings/governance/index.tsx` per orchestrator placement call) + P8-dogfood. | Yes (rchaves explicit ask) | ✓ |
...
| P8-ui-admin-ttl | Section/card slot inside existing `/settings/governance/index.tsx` (NOT a new `/settings/governance/sessions-policy` route — orchestrator placement call 2026-05-03): "Maximum session duration" numeric days input/stepper + 0 = unbounded + Save/Reset + copy framing the cap as applying to CLI/device sessions. Reads/writes `Organization.maxSessionDurationDays` (already migrated in `82ae4b666`). | 🅑 | |
