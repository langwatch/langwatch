...
| **Phase 9 — Gateway no-spy mode** (NEW) | Architecture-spine row above. Backend ✅ (`6433e3e14` — P9-schema/strip-service/pipeline-wire/router/int-test landed); BDD specs + docs scaffold ✅ (`bd4875f56`); ⏳ remaining: P9-ui (3-radio picker — Lane-B) + P9-dogfood. | Yes (rchaves explicit ask) | ✓ |
...
| P9-ui | UI toggle — 3-radio (or segmented) picker as a **sibling slot in the existing `<SessionPolicySection />` card** in `/settings/governance/index.tsx` (per orchestrator placement call 2026-05-03 — same-card consolidation, NOT a separate Privacy section): **Full** ("All content stored — recommended for debugging-heavy teams") / **Strip inputs & outputs** ("Prompts + completions + system messages dropped before storage — recommended for orgs with conversational-content policies") / **Strip everything** ("Also drops tool-call arguments + tool-result payloads — strictest mode, may break some debugging flows"). Explicit forward-looking copy: "Forward-looking only — does not retroactively scrub history." Save fires `api.sessionPolicy.setContentMode` (already wired by `6433e3e14`). | 🅑 | ✓ |
