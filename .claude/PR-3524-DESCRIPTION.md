...
> 1. **Governance-reactor `handle()` produces 0 CH rows on the trace_processing pipeline — narrowed to foldState attribute propagation** (third-pass diagnosis 2026-05-04 post-Sergey breakthrough). Diagnosis arc: (1) initial "3 reactors absent" — wrong; (2) "registered + staged but handle() not executing" — also wrong; (3) **handle() DOES execute, but the early-return `if (!virtualKeyId || !gatewayRequestId) return;` fires silently producing 0 rows**. Evidence: `gatewayBudgetSync.reactor.integration.test.ts` (already shipped in `src/server/gateway/__tests__/`) **passes 5/5 in 12s against real PG + real CH** when called directly with a hand-crafted `ReactorContext` + `foldState` carrying `langwatch.virtual_key_id` + `langwatch.gateway_request_id`; Redis confirms production execution: `{event-sourcing/jobs}:gq:stats:completed:{gatewayBudgetSync,governanceKpisSync,governanceOcsfEventsSync} = 10` each, with `gq:stats:failed:*` for governance reactors = **none recorded**. Therefore: reactor SEMANTICS are proven; **the bug is in what `foldState.attributes` contains in the production path**, NOT in the reactor body. Three narrow hypotheses for the fold-attribute gap: (a) `foldState` is constructed differently from the persisted `trace_summaries.Attributes` column — the column has the keys but the in-memory state passed to handle() doesn't; (b) `trace-attribute-accumulation.service.ts` is filtering / namespacing the `langwatch.*` keys at fold time before reactor invocation; (c) the OTLP→span-attribute translation strips or normalizes the gateway-stamped keys between receipt and fold-state composition. Phase 7 end-to-end is not working until the foldState-attribute propagation gap is closed AND a real gateway-trace smoke fires producing rows in `gateway_budget_ledger_events` + `governance_kpis` + `governance_ocsf_events`. Sergey landing parity `governanceKpisSync` + `governanceOcsfEventsSync` integration tests next, then digging into `trace-attribute-accumulation.service.ts`. Smoke evidence script ready at `langwatch/scripts/dogfood/smoke-3-reactors.ts` (Sergey `60e7eb3d8` — **smoke-script tooling only, NOT evidence**); emits a JSON summary suitable for direct paste into §Smoke evidence once the reactors produce rows.
