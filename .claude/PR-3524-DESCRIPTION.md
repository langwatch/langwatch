...
| 📋 | 🅑 | **1B-followup-1: PRINCIPAL admin UI in `BudgetCreateDrawer`** — Lane-B claimed (post-Sergey-backend). Plan: add PRINCIPAL to scope picker + conditional user-picker (existing `OrganizationUser` query) + wire `principalUserId` into `api.gatewayBudgets.create` mutation + surface cross-org `BAD_REQUEST` inline. |
