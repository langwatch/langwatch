...
| **Phase 11 — CLI wrapper e2e in CI** (NEW) | Architecture-spine row above. **Approved scope** (orchestrator call 2026-05-03): pure Node e2e — fake control-plane (Express, in-memory fixtures) + fake gateway (Express, records inbound) + mocked tool binaries on PATH (shell scripts that echo env). No Docker, no live LLM, no Bifrost matrix. claude/codex/opencode first; cursor/gemini ONLY if assertion shape is line-for-line identical (no harness expansion). ⏳ — Sergey starting now (P11-harness + P11-per-wrapper + P11-spec-wrap-login-routing); Lane-A: P11-spec already shipped (`bd4875f56`) + new spec path stub + P11-docs; Lane-B: only P11-ui-handoff (browser-side device-flow page check). | Yes (rchaves explicit ask) | ✓ — Independent of all other phases |
...
| P11-harness | `typescript-sdk/__tests__/e2e/cli/governance-wrapper.e2e.test.ts` — single-file pure-Node harness (~3-5min runtime). Stands up: (a) **fake control-plane** Express app on a random port serving device-flow + budget endpoints from in-memory fixtures; (b) **fake gateway** Express app on a random port that records inbound request shape (Authorization header, base URL path, payload) and returns a deterministic OpenAI/Anthropic-shape response; (c) **mocked tool binaries** — tiny shell scripts on PATH that just echo `$ANTHROPIC_BASE_URL` etc. Spawns the CLI wrapper as a child process via existing `cli-runner.ts` helper with `LANGWATCH_CONFIG_DIR` pointed at a tmp dir + base-URL env vars pointed at the fake servers. Tests against `envForTool()` seam at `typescript-sdk/src/cli/utils/governance/wrapper.ts:32-70`. | 🅢 | ✓ |
| P11-per-wrapper | Three scenarios per tool (claude + codex + opencode in this slice; cursor + gemini follow ONLY if line-for-line identical, no harness expansion): **(a)** login ceremony writes config with the expected base_url + token shape; **(b)** wrap-tool injects the right env vars + base URL into the child env (claude → `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN`; codex → `OPENAI_BASE_URL` + `OPENAI_API_KEY`; opencode → equivalent); **(c)** budget pre-flight blocks with exit code 2 when over-limit. | 🅢 | ✓ |
| P11-ci | CI job registration — wires the e2e suite into the langwatch-app-ci workflow. **No Docker / no live LLM / no Bifrost matrix dependency** per orchestrator call — runs in default CI as a normal Node test. May land as a separate shard if runtime exceeds budget. | 🅢 | ✓ |
...
| P11-spec-wrap-login-routing | New spec at `specs/ai-governance/cli-wrappers/wrap-login-routing.feature` (Sergey to ship alongside `governance-wrapper.e2e.test.ts`) — pins the three scenarios per tool (login config write, env injection, budget pre-flight exit code 2). Will fold the path into the PR doc when commit lands. | 🅢 | |
...
Critical path: **P11-arch → P11-spec-wrap-login-routing → P11-harness → P11-per-wrapper (claude/codex/opencode) → P11-ci** (cursor/gemini are conditional follow-ons; not on critical path unless line-for-line identical).
