...
- **`/gateway/usage` — Vite route-resolution bug**. The page exists at `langwatch/src/pages/[project]/gateway/usage.tsx` AND is registered in `routes.tsx` (line 279), but still returns 404. Same class of bug as the `/settings/governance/tool-catalog` 404 fixed in Phase 7 `25dea5fdd`. **Lane-B claimed the repair as next dial** (mirror the tool-catalog routes.tsx fix); blocks captures #8 (VK detail Usage section, links to `/gateway/usage`) + #9 (project-wide usage). Will mark this row ✅ once Alexis's repair commit lands.
