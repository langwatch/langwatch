...
> - **Browser dogfood evidence (lane-B, @ai_gateway_alexis_2)**: post-fix browser walk verified — MEMBER signed in directly loading `/governance` → "Access Restricted" (UI gate fires, page guard wins); same for `/settings/governance/ingestion-sources` and `/settings/routing-policies`. tRPC layer probed via authenticated MEMBER session against the batched URL: `governance.setupState` 401, `ingestionSources.list` 401, `anomalyRules.list` 401, `activityMonitor.summary` 401 — all match the design (UNAUTHORIZED with "You do not have permission to access this organization resource" body). Screenshots bundled at `langwatch/.claude/screenshots/iter32-dogfood/` (gitignored, local-to-worktree per project convention; reviewers cloning the branch can browse 18 PNGs covering admin + MEMBER + persona-1 walks).
>
> **🟡 Open thread (post-`9e373c284` regression flag, NOT blocking)**: Alexis's MEMBER browser-session probe found `governance.resolveHome` returning `401 UNAUTHORIZED` (same body as the now-restricted endpoints), despite the `9e373c284` carve-out keeping it on `organization:view` and the integration test (`governance.rbac.integration.test.ts`) verifying MEMBER → 200 on `resolveHome`. Likely a session-vs-test-context drift (test stubs vs real Better-Auth session). Active investigation by @ai_gateway_sergey_2; **does NOT block the PR** — the UI gate (page guard "Access Restricted") still fires correctly so MEMBER UX is preserved. Will fold the resolution once Sergey confirms root cause + fix.
