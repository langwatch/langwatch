...
>
> **🚨 Pre-merge blockers** (orchestrator-flagged 2026-05-04, MUST close before merge — not just dogfood-evidence; these are production-path bugs):
> 1. **Governance-reactor `handle()` produces 0 CH rows on the trace_processing pipeline — root cause UNRESOLVED, escalating** (fourth-pass diagnosis 2026-05-04 post-Sergey debug-budget-exhaust). **Diagnosis arc**: (1) ❌ "3 reactors absent" → (2) ❌ "staged but handle() not executing" → (3) ❌ "handle() runs, early-return on missing foldState attrs" → (4) **the running reactor code in the production worker is NOT the file on disk; even `throw new Error("SMOKE-DEBUG-THROWN")` as the first statement of handle() does not increment `gq:stats:failed:*` for the smoke trace**. **Reactor SEMANTICS are proven** by the existing `gatewayBudgetSync.reactor.integration.test.ts` (5/5 passing in 12s against real PG + real CH). **Reactor GATING is now correctly understood**: `gatewayBudgetSync` requires `langwatch.virtual_key_id` + `langwatch.gateway_request_id` (gateway traces); `governanceKpisSync` + `governanceOcsfEventsSync` require `langwatch.origin.kind === "ingestion_source"` + `langwatch.ingestion_source.id` (Phase 10 puller traces, NOT gateway traces). `5ad02ed14` updates the smoke script to fire BOTH a gateway trace AND an ingestion-source trace — both land in `trace_summaries` with the correct attributes, both ✅ at the persistence layer, but all three reactors still produce 0 CH rows. **Five debug attempts on the production foldState path, all inconclusive**: (1) `logger.warn` never appeared in logs; (2) `console.error` direct stdout/stderr never appeared; (3) `fs.appendFileSync("/tmp/smoke-debug.log", ...)` — file never created; (4) **whole-body `throw new Error("SMOKE-DEBUG-THROWN")` — `failed` counter never incremented, `completed` counter still incremented +1 per smoke**; (5) `rm -rf /tmp/tsx-*` cache clear + full `docker restart` — same null result. File mtime in container postdates restart (verified `stat`), file content has the throw (verified `grep -c`), but the running code is something else. **Staleness hypotheses** (none confirmed): (a) pre-bundled / pre-compiled artifact loaded ahead of the source path; (b) reactor handler closure captured at app-init and persisted somehow (Redis? in-memory module cache?); (c) worker process forking that runs a snapshot of the module; (d) module-resolution alias colliding with a different file under the new `@ee/*` mapping. **Escalating beyond Lane-S debug budget** — Sergey requesting either a specific direction (e.g. patch `projectionRouter.ts:151` to log handler invocation) or fresh eyes on the problem. Per orchestrator gating rule: blocker resolves ONLY when real pipeline smoke writes all three CH tables for a trace_id — direct `handle()` semantics are insufficient. Smoke evidence script at `langwatch/scripts/dogfood/smoke-3-reactors.ts` (Sergey `5ad02ed14`) ready to emit the §Smoke evidence JSON the moment the reactors produce rows.
...
**Process learning** (folded into PR retrospective): when introducing a new path-alias prefix, audit ALL `tsconfig*.json` overrides + `vite.config.ts` resolve.alias + `vitest.config.ts` resolve.alias in the same change. The `@ee/*` alias was only added to root `tsconfig.json` + every `vitest.config.ts` initially — `tsconfig.workers.json` and `vite.config.ts` were missed because they don't get exercised by `pnpm typecheck` or `pnpm test:*`. Future open-core split work should use a "alias parity grep" check: `rg -l '"paths"|resolve.alias' --type json --type ts` to enumerate every config file that needs the new prefix.
