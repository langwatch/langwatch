...
| **H** | CodeRabbit review-driven fixes (waves 1+2+3+3b+3c+3d: 2 critical + 5 major + 12 minor + 5 nitpick across 10 commits + 1 bonus, **24 findings closed**, 1 stale-confirmed, all verified before fix) | S+B | `abd5fe5c6` + `0bf5781c4` + `fd800485e` + `49f81be4f` + `565be6b63` + `17047a301` + `7007bfa2b` + `d09ab7fa9` + `2942c26a2` + `0ff8ca117` (+ `a4db4f714` bonus `is80Pct` close-out by lane-B) — see sub-list below |
...
### H detail — CodeRabbit review-driven fixes (waves 1+2+3+3b+3c+3d)
...
**Out-of-scope (deferred follow-up, not in this PR)**: only `me/settings.tsx:64` notification-toggles persistence remains (lane-B/UI; needs design call before implementation). Already-fixed items dropped from the open list: `typescript-sdk/budget.ts:105` + earlier `wrapper.ts:126` were closed by rchaves in `45575b9c4` earlier in the branch; `me/index.tsx:59` `is80Pct` was closed by Alexis's `a4db4f714` D1+D2 bonus. All other CodeRabbit-flagged items across waves 1+2+3+3b+3c+3d resolved; **lane-S + lane-B CodeRabbit-triage queue is fully closed** (24/24 integration tests green post-wave-3 verification: governance.rbac 11/11 + routingPolicy.service 13/13).
