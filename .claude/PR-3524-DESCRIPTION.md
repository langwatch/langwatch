...
- **`/gateway/usage` — Vite import-analysis 500 (NOT a routing bug)**. Initial diagnosis ("missing route registration") was wrong. Lane-B (Alexis) traced the actual root cause: `vite.config.ts` was missing the `@ee` path alias, so when `routes.tsx` imported `@ee/governance/dashboard/pages/ingestion-sources` (added in Alexis's `515b4f4c0` Phase 4a-3 UI relocation), Vite's import-analysis pass blew up with `[plugin:vite:import-analysis] Failed to resolve import '@ee/governance/...'`. The Vite HMR error overlay then masked **every** route in the app — `/gateway/usage`'s "404" was the overlay, not a missing page. The page itself at `langwatch/src/pages/[project]/gateway/usage.tsx` is registered in `routes.tsx:279` and serves correctly once the alias resolves. Fix: one-line addition `'@ee': path.resolve(__dirname, './ee')` to `vite.config.ts` resolve.alias. Lane-B repair in flight; will fold SHA when commit lands.
- **Workers process: silent reactor crashloop since `ee/` relocation**. Same alias gap, different config: `tsconfig.workers.json` overrides the root `tsconfig.json` `paths` and was missing `@ee/*`. When Sergey moved `gatewayBudgetSync.reactor`, `governanceKpisSync.reactor`, `governanceOcsfEventsSync.reactor` into `langwatch/ee/governance/reactors/` earlier in this PR, the workers process started failing to load them with `Cannot find module '@ee/governance/reactors/gatewayBudgetSync.reactor'` — and crashlooped silently (no surfaced error in the UI, only in worker stderr). This means the Phase 7 trace→budget fold + governance KPI projections + OCSF event projections **have been DOA in the workers process** since the relocation, even though unit/integration tests passed (those don't use `tsconfig.workers.json`). Fix: one-line addition `"@ee/*": ["./ee/*"]` to `tsconfig.workers.json` `compilerOptions.paths`. Lane-B repair in flight; Sergey will smoke the three reactors end-to-end after the alias lands and drop a trace-id for §Smoke evidence capture.
