...
| 📋 | 🅑 | **4b-3: IngestionSource composer source-type / retention dropdown gate** — pushed out of this slice per master_orchestrator narrowing (composer-side restriction on `otel_generic` + `thirty_days` for non-enterprise still wanted but as separate follow-up commit). |
