...
| **Phase 10 — Pull-mode framework** (NEW) | Architecture-spine row above. Backend ✅ (`3fdf6626b` — P10-adapter-iface / http-adapter / copilot-studio-reference / worker / schema / unit-tests landed); BDD specs + docs scaffold ✅ (`bd4875f56`); ⏳ remaining: P10-s3-adapter (queued) + P10-event-sink-wire (currently a TODO in pullerWorker.ts) + P10-openai-compliance + P10-claude-compliance reference impls (queued) + P10-int-test + P10-ui (Lane-B) + P10-dogfood. | Yes (rchaves explicit ask — closes the Phase 2D gap) | ✓ |
...
| P10-s3-adapter ⏳ | `S3PollingPullerAdapter` — universal S3-polling adapter. Config: `{ bucket, prefix, region, credentialRef, parser: "ndjson" \| "csv" \| "json-array", schedule }`. Cursor = lexicographic-max key seen so far; resume from there. **Queued** — Sergey's next slice. | 🅢 | ✓ |
...
| P10-openai-claude-references ⏳ | `openaiCompliance.puller.ts` + `claudeCompliance.puller.ts` — additional reference impls following the Copilot Studio pattern. **Queued** — Sergey's follow-up after S3 + event-sink wiring. | 🅢 | |
...
| P10-event-sink-wire ⏳ | Wire `pullerWorker.ts` TODO into the existing OCSF event sink so pulled events flow into `governance_ocsf_events` (not just logs). **Highest priority of remaining P10 rows** (orchestrator call 2026-05-03) — without this, the framework is shape-correct but produces no usable governance events. | 🅢 | ✓ |
...
| P10-int-test | `governancePuller.integration.test.ts` — fixture-backed: spin up local HTTP server returning a paginated audit-log response, run the puller against it, assert events land in the trace store, assert cursor advances, assert restart resumes from cursor. Also covers S3 adapter against a localstack S3 fixture. **Pending S3 adapter + event-sink wiring** before integration test can be meaningful end-to-end. | 🅢 | ✓ |
...
| P10-ui | Admin "Add pull source" drawer at `/settings/governance/ingestion-sources` — supports `copilot_studio` (credentials-only since reference impl locks the rest) + `http_polling` (full pullConfig form with JSON-path field validation). Source-type dropdown populated from `pullerAdapterRegistry.ids()` (or hardcoded curated list for v1). Per-adapter config form per orchestrator call 2026-05-03. Schedule picker. | 🅑 | |
...
Critical path: **P10-arch → P10-adapter-iface → P10-http-adapter → P10-reference-impl → P10-worker → P10-schema → P10-event-sink-wire → P10-s3-adapter → P10-int-test → P10-ui → P10-dogfood**. (First five-then-schema chunk landed in `3fdf6626b`; `P10-event-sink-wire` is the highest-priority remaining backend row per orchestrator call 2026-05-03.)
