...
> 1. **Governance-reactor `handle()` produces 0 CH rows on the trace_processing pipeline — narrowed; orchestrator rejected stale-code; rerunning** (fifth-pass diagnosis 2026-05-04 post-orchestrator log inspection). **Diagnosis arc**: (1) ❌ "3 reactors absent" → (2) ❌ "staged but handle() not executing" → (3) ❌ "handle() runs, early-return on missing foldState attrs" → (4) ❌ "running code is NOT the file on disk" → (5) **orchestrator's worker-log inspection rejected the stale-code hypothesis: logs DO show `SMOKE-DEBUG-FACTORY`, `SMOKE-DEBUG-HANDLE-1`, and the unique thrown `gatewayBudgetSync` error, AND the thrown error includes `foldState.attributes` carrying `langwatch.virtual_key_id` + `langwatch.gateway_request_id` for the gateway trace**. So the running code IS the edited file; the gateway-trace foldState DOES carry the right attrs at handle() entry. The earlier "Redis completed counter without failed counter" + "no debug logs visible" reads were artifacts of where Sergey was looking, not actual stale code. **Reactor SEMANTICS proven** (existing `gatewayBudgetSync.reactor.integration.test.ts` 5/5 in 12s real PG+CH). **Reactor GATING confirmed correct** (gatewayBudgetSync → gateway traces; governanceKpisSync + governanceOcsfEventsSync → ingestion-source traces). **Orchestrator-directed next step** (2026-05-04): remove the debug throw + debug logs from gatewayBudgetSync, restart workers, rerun `smoke-3-reactors.ts` with fresh trace_ids, observe whether `gateway_budget_ledger_events` gets a row for the gateway trace. If `governance_kpis` / `governance_ocsf_events` still produce 0 for the ingestion-source trace, add the same first-line foldState-attrs dump to those two reactors next. Smoke evidence script at `langwatch/scripts/dogfood/smoke-3-reactors.ts` (Sergey `5ad02ed14`) ready to emit the §Smoke evidence JSON the moment the reactors produce rows. Per orchestrator gating rule: blocker resolves ONLY when real pipeline smoke writes all three CH tables for a trace_id.
