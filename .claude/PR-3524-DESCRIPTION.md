...
| **H** | CodeRabbit review-driven fixes (waves 1+2+3+3b+3c: 2 critical + 5 major + 9 minor + 5 nitpick across 8 commits + 1 bonus, **20 findings closed**, all verified before fix) | S | `abd5fe5c6` + `0bf5781c4` + `fd800485e` + `49f81be4f` + `565be6b63` + `17047a301` + `7007bfa2b` + `d09ab7fa9` (+ `a4db4f714` bonus `is80Pct` close-out by lane-B) — see sub-list below |
...
**Out-of-scope (deferred follow-up, not in this PR)**: only `me/settings.tsx:64` notification-toggles persistence remains (lane-B/UI; needs design call before implementation). Already-fixed items dropped from the open list: `typescript-sdk/budget.ts:105` + `wrapper.ts:126` were closed by rchaves in `45575b9c4` earlier in the branch; `me/index.tsx:59` `is80Pct` was closed by Alexis's `a4db4f714` D1+D2 bonus. All other CodeRabbit-flagged items resolved across waves 1+2.
...
| D3 | Write net-new `overview.mdx`: Personas (Admin / End-user) + 6-bullet feature map + Admin Prerequisites callout (default routing policy hard prereq per Sergey's `49f81be4f` flag) + cross-link to `ai-gateway/overview` (gateway = data plane; governance = controls) | 🅑 | |
| D4 | Refresh moved pages — `cli-debug.mdx` 409 error catalog (`no_default_routing_policy` entry) + "Pairs with `<gateway page>`" header on each moved page | 🅑 | |
| D5 | Capture / move 23 assets to `docs/images/ai-governance/<bucket>/` (personas 2 · admin 3 · drawers 2 · sources 6 · flows 2 · cli 3 incl. NEW `cli-login-409` · architecture 5) | 🅑 | ✓ |
| D6 | Cross-link 4 AI Gateway pages back into governance (overview callout, budgets→anomaly-rules, audit→compliance-architecture, cli/overview→cli-debug) | 🅑 | |
| D7 | Update `docs/introduction.mdx` (peer card) + main quickstarts (final-step pointer + governance-angle callout) | 🅑 | |
| D8 | `pnpm docs:build` + link check (catches redirect-stub mistakes early) | 🅑 | ✓ |
| D9 | 🅐 Andre folds each batch into PR body + re-PATCHes | 🅐 | |
