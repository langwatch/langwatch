...
> 1. **Governance-reactor `handle()` execution silently failing on the trace_processing pipeline** (corrected diagnosis 2026-05-04 post-`60e7eb3d8` debug). Initial framing was "3 reactors absent from the staged list" — **wrong**. Sergey's in-container probe of `_eventSourcing._definitions[].foldReactors` confirms all 11 reactor names ARE registered, including `gatewayBudgetSync` / `governanceKpisSync` / `governanceOcsfEventsSync`. Worker logs show all 3 ARE staged on each smoke trace (group IDs at `.../fold/traceSummary/reactor/<name>/trace:{traceId}` + Redis dedup keys at `{event-sourcing/jobs}:gq:dedup:trace_processing/reactor/<reactor>/...` set). **What's failing**: the reactor's `handle()` body doesn't appear to execute — debug `logger.warn` + `console.error` lines added to the 3 reactors don't show up in worker stdout/stderr even after `docker restart wise-mixing-zebra-workers-1`. Three hypotheses still to investigate: (a) `PINO_CONSOLE_LEVEL=warn` filtering EE-namespace outputs; (b) silent throw inside `handle()` that `captureException` swallows + the dedup key still gets set; (c) GroupQueue dedup key gets set on enqueue (not on processing), making the dedup-key signal misleading. `isReactorExcluded` (`projectionRouter.ts:215`) checks `reactor.options?.runIn` against `processRole`; none of the 3 governance reactors set `runIn`, so they shouldn't be excluded — confirmed not the gate. **Orchestrator-suggested next debug** (2026-05-04): inspect BullMQ/GroupQueue failed+completed job states + worker logs for exact groupIds (dedup keys = enqueue evidence, NOT processing evidence); temporarily throw a unique error inside one governance reactor's `handle()` or add an unmistakable side-effect, verify whether the worker records the failure; check Sentry / log sink for swallowed `captureException` errors; wrap `handle()` with an outer debug log in the **event-sourcing runner** (not just inside the reactor) so the runner-level invocation is observable independent of reactor body. Direct PG+CH integration tests (Sergey landing per orchestrator split #1) prove per-reactor write semantics in isolation but do **NOT** prove the production-pipeline path. Phase 7 end-to-end is not actually working until the `handle()` execution path is fixed AND a real gateway-trace smoke fires through the in-flight pipeline producing rows in `gateway_budget_ledger_events` + `governance_kpis` + `governance_ocsf_events`. Smoke evidence script ready at `langwatch/scripts/dogfood/smoke-3-reactors.ts` (Sergey `60e7eb3d8` — **smoke-script tooling only, NOT evidence**); emits a JSON summary suitable for direct paste into §Smoke evidence once the reactors fire.
