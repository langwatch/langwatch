...
> 1. **🟡 Governance-reactor pipeline smoke — pipeline IS green; smoke script poll-query was the bug** (sixth-pass diagnosis 2026-05-04 post-orchestrator CH inspection — **resolution PENDING smoke-script fix + JSON capture**). **Diagnosis arc**: (1) ❌ "3 reactors absent" → (2) ❌ "staged but handle() not executing" → (3) ❌ "handle() runs, early-return on missing foldState attrs" → (4) ❌ "running code is NOT the file on disk" → (5) ❌ "rerun smoke without debug throw" — actually unnecessary because → (6) **the reactors DID write rows; the smoke script was polling with `seeded.org.id` but `TenantId` in these CH tables is the trace-processing tenant/project id**. Orchestrator's direct CH inspection for the clean-rerun project `proj_smoke_1863ac3a1bbdd5e0`: **`gateway_budget_ledger_events = 1`, `governance_kpis = 2`, `governance_ocsf_events = 2`** ✅. The same queries keyed by `org_smoke_8bb4079685be4789` return 0 — explaining every prior "0 rows" observation. **Real pipeline smoke is green.** Pending closure: (a) Sergey fix `smoke-3-reactors.ts` to poll `seeded.project.id` (label `orgId` separately for clarity); (b) rerun once with fresh trace_ids; (c) capture the JSON summary as the canonical §Smoke evidence row that closes this blocker. Reactor SEMANTICS were always proven (existing `gatewayBudgetSync.reactor.integration.test.ts` 5/5 in 12s real PG+CH); reactor GATING was always correct (gatewayBudgetSync → gateway traces; governanceKpisSync + governanceOcsfEventsSync → ingestion-source traces). The 5 false-trail debug passes above are preserved here as history because the diagnostic-arc shape is itself valuable for future debugging — the lesson is "verify your TenantId predicate against the actual CH table schema before spending hours on reactor-internals hypotheses".
