...
| **H** | CodeRabbit review-driven fixes (waves 1+2: 2 critical + 5 major + 4 nitpick across 5 commits, all verified before fix) | S | `abd5fe5c6` + `0bf5781c4` + `fd800485e` + `49f81be4f` + `565be6b63` — see sub-list below |
...
- **Nitpick cleanups close-out** (`565be6b63`): 2 more unused-var nits flagged by github-code-quality bot — `organization` destructure leftover in `MainMenu.tsx` (post-merge edit residue) + `logRequestType` in `parseOtlpBody.test.ts` (pre-existing). Surgical removals only on peer-touched files.

**Out-of-scope (deferred follow-up, not in this PR)**: `me/settings.tsx:64` notification-toggles persistence (lane-B/UI; needs design call before implementation), `me/index.tsx:59` `is80Pct` unused (Alexis's FF-flash territory; covered by I or batchable in a follow-up nitpick sweep). Already-fixed items dropped from the open list: `typescript-sdk/budget.ts:105` + `wrapper.ts:126` were closed by rchaves in `45575b9c4` earlier in the branch. All other CodeRabbit-flagged items resolved across waves 1+2.
