...
| 27 | *(this commit)* | sergey iter 25 JWT rotation + alexis iter 18 UI sweep. config.mdx new §Secret rotation with dual-key `jwt.VerificationKeySet` explainer, 4-step flow, `jwt_secret_rotation_active` WARN log invariant, same-pattern applied to INTERNAL_SECRET. helm.mdx §Secret rotation (sibling of §Secrets) with values.yaml `jwtSecretPreviousName/Key` + conditional secretKeyRef + grep recipe. contract.md §13 first open question closed; v0.1.2 changelog entry. budgets.mdx new §Viewing budget spend (UI) covering /gateway/budgets/[id] detail page + byDay sparkline gating. |
