...
| 26 | *(this commit)* | NEW cookbooks/disaster-recovery.mdx (~210 lines) — enterprise-facing "what happens when X fails" playbook covering 6 scenarios (control plane down / gateway pod killed / provider down / Redis down / gateway region down / full LangWatch outage), degradation timelines, RTO/RPO, and quarterly drill recipes. Pairs with production-runbook (symptoms on healthy gateway) and prometheus-alerts (signals). Landed in docs.json between production-runbook and the end of cookbooks. |
