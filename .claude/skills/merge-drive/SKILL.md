...
a trap. Where your branch _created_ a tree (`enterprise/`, absent from both base
...
- **Never `git add` a directory while anything under it still holds markers.**
  It stages unresolved work as resolved _and_ destroys that file's `:1:/:2:/:3:`
...
A partial merge's index and operation metadata live in this worktree. They
survive an ordinary agent session ending, but a content commit does not carry
that index or metadata to another worktree or machine.

Do not finish the merge just to collect a lane or checkpoint. Normal `git commit`
refuses an unmerged index; after resolution it finishes the merge and removes
MERGE_HEAD. `commit-slice.sh` is also unsuitable: its plumbing would create a
one-parent commit and move HEAD during the merge. It now refuses active
operations. Follow the coordinator's collection rule: review, stage resolved
owned paths, record the evidence and leave the unfinished merge intact.

When a content checkpoint is needed, pause writers and use the tested helper:

```bash
bash dev/scripts/snapshot-merge.sh \
  "refs/heads/wip/merge-partial-$(date +%Y%m%d-%H%M%S)" \
  "[skip ci] partial merge content checkpoint"
```

The helper uses a throwaway index, records HEAD and all MERGE_HEAD parents, and
creates a new local WIP ref. It includes tracked worktree contents, including
remaining conflict markers, and newly added files already staged by the
coordinator. Untracked files must be reviewed and preserved separately. It does
not move HEAD, change the live index, or push anything. Verify live parents and
unmerged entries after checkpointing. Publish a checkpoint only when authorised.

A WIP ref is neither a reviewed merge result nor a portable unmerged index. To
resume elsewhere, preserve the original parents and resolved-path inventory in
the handoff, reconstruct the merge from those parents, then restore only the
reviewed resolved paths from the checkpoint and revalidate them. Do not use a
moving `origin/main` as a substitute for the recorded parent. Preserve unresolved
stage evidence separately when exact recovery is required.

Take a checkpoint before a risky operation or transfer when local state alone is
insufficient; an ordinary session handoff on the same intact worktree does not
require manufacturing a commit.
