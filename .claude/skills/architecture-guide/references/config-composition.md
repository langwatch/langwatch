...
once, at the entrypoint. **Feature code never reads `process.env`**; it receives values
...
```ts
// packages/features/langy/contract/src/langy.config.ts: a feature's own config, schema-driven
...
- A feature declares its config definition in its **contract** (`<f>.config.ts`) and the
  process spreads it into its own definition; the feature receives the parsed value through
...
- **Secrets are not config** (ADR-132): `packages/secrets/keys.json` classifies every
  ...
  before the zod parse. A feature sees a plain value; it never reads
...
## Booting features: the container
...
provides the peer APIs the features declare, installs feature installers, and boots for
...
- `.withPersistence("postgres", { prisma })` or `"memory", {}` selects every installed
  feature's repository bundle; a factory whose declared infrastructure is missing fails
...
- `.withProvided(Token, implementation)` supplies a peer that this graph does not install
  ...
  `.withFeature(installer)` installs a feature so its app is provided under its own token.
...
- `.boot({ role: "api" | "worker" | "task", config? })` constructs only that role's
  ...
  reads any provided token. Shutdown drains hosts before closing features.
- The worker boots one graph for several features (`apps/worker/src/app/worker-observability-apps.composition.ts`:
  trace, annotation, data-privacy, log, evaluation over one `.withPersistence("postgres", …)`);
  the API still boots one graph per feature during the migration. Both call the same
  installer.

## Mounting transports in apps/api

A feature's `transport/*.rest.ts` and `*.trpc.ts` are inert declarations. The process
...
- `<f>-trpc.mount.ts`: `createTrpcHandlerBinding<HostContext, <F>Api>(({ ctx, input }) => ({ app: ctx.app.<f>, actor, scope }))`
  from `@langwatch/api/composition`, wrapped by `createTrpcApiService` and
  `createTrpcService` from `@langwatch/api/trpc`, then `<f>TrpcTransport.router(service)`.
  Each declaration becomes one namespace, named in `apps/api/src/app-trpc/app-trpc.features.ts`
  ...
  family and `ctx.app.<f>` read the same app.
- `<f>-rest.mount.ts`: `mountProjectTransport({ family, transport: <f>Rest.router(), app, credential: "apiKey", authenticate, authorize, afterSuccess })`
  from `@langwatch/api/rest`, where `family` is `security.createServiceVersionedApp({ name, basePath, errorEnvelope, errorHandler })`.
  Historical refusal bodies and 404 shapes are mapped in this file's error handler, not in
  the feature.
...
`refusing*`, `Unavailable*` or `Logged*Absence`; either the feature installs and its
...
`worker-evaluation-processing`, …). Features contribute through the same installer they
give the API, booted with `role: "worker"`. `apps/worker/src/features/catalogue.json`
lists the worker features; `apps/worker/src/features/job-registry.json` is byte-frozen:
pipeline → the `command:*` and `subscriber:*` jobs it may route. The queue rejects an
