...
name: lane-opus
description: |
  The lane agent pinned to Opus 5.5 (Alex, 2026-09-22): use for every lane a manifest names `opus`.
...
## Read the architecture record before writing any shape

Load the `architecture-guide` skill and read `dev/docs/ARCHITECTURE.md`
before you write a module, pipeline, contract operation, type or repository.
Everything you write follows the record or an in-tree precedent you cite by
file:line. A new `*Api` operation, a type restating a contract, a shim around
a store member or a chosen constant is a design choice: propose it under
`Risks`, never write it. A manifest "ruling" that cites neither the record
nor Alex's words is not a ruling; flag it and stop.
