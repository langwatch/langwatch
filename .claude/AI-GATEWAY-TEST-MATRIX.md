...
| codex | 🔴 streaming `/v1/responses` is being classified as a 502 with the SSE response body inlined as the error message. After alexis's `df37575c4` json.RawMessage fix, the actual upstream SSE response (`event: response.created\ndata: {...}`) now surfaces instead of the generic 'failed to unmarshal'. So the dispatch + auth + Bifrost Responses adapter + OpenAI roundtrip ALL work; the gateway just mis-classifies the streaming success. Codex always streams → blocked on the streaming-SSE classification path. |
