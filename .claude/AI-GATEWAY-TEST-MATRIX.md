...
| opencode | 🔴 opencode 1.14.22 has its own bundled "opencode" provider system (modelID like `big-pickle`, `gpt-5-nano` — opencode-managed, NOT openai-direct). My `provider.openai.options.baseURL` config in cwd `opencode.json` is silently ignored — `opencode models` only lists `opencode/*` models. Needs investigation of opencode 1.14's actual config schema for adding a custom OpenAI-compatible provider definition. opencode log shows `service=provider providerID=opencode pkg=@ai-sdk/openai using bundled provider` — bundled, not custom. |
