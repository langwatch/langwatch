...
| gemini-cli | 🟡 NOT inherent skip — `GOOGLE_GEMINI_BASE_URL` env exists (alexis intel). Was blocked on gateway lacking Gemini-native `/v1beta/models/X:generateContent` route; alexis shipped `c513c399e` adding the passthrough route. **Sergey owns the matrix cell + verification post route shipping**. |
