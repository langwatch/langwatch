...
| codex | 🔴 `/v1/responses` Bifrost dispatch returns 504 `failed to unmarshal response from provider API`. Alexis's `d90b81db7` fix landed for `/v1/messages` but apparently doesn't cover the `/v1/responses` raw-forward path. Lane A — needs the same RawResponse extraction in the responses dispatch. |
