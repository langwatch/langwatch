...
//
// The flush is wrapped because a jsdom suite can tear down `URL` or `fetch`
// before the analytics client finishes its final network flush.
afterAll(async () => {
  try {
    const { shutdownPostHog } = await import("./src/server/posthog");
    await shutdownPostHog();
  } catch {
    // Teardown-phase flush can throw once test env globals are gone.
  }
