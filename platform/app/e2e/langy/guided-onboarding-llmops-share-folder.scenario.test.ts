            ...
            agents: [
              ...
              scenario.judgeAgent({
                ...
                criteria: [
                  ...
                  `Before proposing the first scenario, Langy says one of three lines: "${GUIDED_LINES.pullRequestOpened} <the address the command printed>. ${GUIDED_LINES.pullRequestMerge}" with a real address; or, when the folder has no remote or GitHub is not signed in, "${GUIDED_LINES.noRemoteStart} <the branch> holds the commit."; or, when the push worked and opening the pull request failed twice, "The branch <the branch> ${GUIDED_LINES.pushedOpenFailed} <the line the command printed>." Whichever it is, it never leaves the pull request to the end of the path.`,
