/**
 * The llmops path in a folder that is not a git repository.
 *
 * The shared folder is the ACME notes application with no `.git` in it, so the
 * folder facts say `git: not a repository` and no branch and no commit can be
 * made. That is a dead end with a known unlock, and an unlock is asked, never
 * stopped on: the turn ends on a question card offering to create the
 * repository or to pick another folder, and nothing is checked out before the
 * developer answers.
 *
 * Layer 2 comes before the judge: the card's own wording and options, the
 * commands that ran before it, and, after "Create a repository for me", the
 * repository the answer made and the branch the path carried on with.
 *
 * RUN (one file per vitest run, see README):
 *   cd platform/app/e2e/langy && npx vitest run guided-onboarding-llmops-no-repo.scenario.test.ts --reporter=verbose
 */
  ...
  describe("when the shared folder has no repository in it", () => {
    ...
    it(
      ...
      async () => {
        ...
        const unlock = watcher.questions
          ...
          .find((question) =>
            (question.options ?? []).some(
              (option) => option.label === CREATE_REPO_OPTION,
            ),
          );
        ...
        const checkoutAt = commands.findIndex((command) =>
          /git\s+checkout\s+-b/.test(command),
        );
        const initAt = commands.findIndex((command) =>
          /git\s+init/.test(command),
        );
