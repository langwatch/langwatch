  ...
  describe("when the folder's own package manager spelling is gone", () => {
    ...
    it(
      ...
      async () => {
        ...
        // The rung that is gone was tried once, and the ladder moved on
        // rather than retrying a spelling that is not there.
        const pipAttempts = commands.filter((command) =>
          /(^|[^\w-])pip\s+install\b/.test(command),
        );
        ...
        expect(
          asked.some((question) => /couldn't find pip or uv/i.test(question)),
        ).toBe(false);
