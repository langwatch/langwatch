 ...
 *
 * Every `langwatch` release published so far, 0.2.11 through 1.4.0, declares
 * `requires-python >=3.10,<3.14`. On a machine whose `python3` is 3.14, pip
 * walks past all of them and installs 0.1.32, the last release with no upper
 * bound and a release from before `setup()` and `connect_agent()` existed.
 * Nothing warns: the install reports success, and the tracing code written
 * against it cannot import. Homebrew's `python3` is 3.14 today, so this is
 * what a developer on a fresh Mac Python meets. The range in this repository
 * already reaches 3.14, but the scenario is about what a developer can
 * install, so it stands until a release ships.
 *
 * The venv here is built from the machine's own `python3` on purpose, unlike
 * every sibling scenario, because the interpreter IS the subject. The skill's
 * answer is an import check between the install and the first edit, and an
 * unlock question when the check fails: never a blind `--upgrade`, which
 * cannot help, and never an edit written against an API that is not there.
 *
 * Layer 2 comes before the judge: the card's own wording and options, that
 * `app/main.py` was still untouched when the card was answered, and that no
 ...
 *
 * The skill puts the version the interpreter reports in place of 3.14, so the
 * wording is fixed and the number is not. The test pins every word and checks
 * the number is this machine's interpreter, rather than hard-coding a version
 * that only holds until Homebrew moves.
 */
  ...
  describe("when pip can only reach a release without the tracing API", () => {
    ...
    it(
      ...
      async () => {
        ...
        const unlock = watcher.questions
          ...
          .find((question) =>
            (question.options ?? []).some(
              (option) => option.label === INSTALL_PYTHON_OPTION,
            ),
          );
        ...
        expect(
          asked,
          "the card asks the skill's question, word for word",
        ).not.toBeNull();
        ...
        console.log(
          "[layer2] main.py unchanged at the card:",
          mainAtQuestion === pristineMain,
        );
