...
export class SsoConnectionGuards {
  ...
  async registerConnection(
    ...
    await this.refuseCompetingConnection({
      organizationId: data.organizationId,
      connectionId: data.connectionId,
      kind: "direct",
    });
    await this.claimRegistrationSlot({
      organizationId: data.organizationId,
      connectionId: data.connectionId,
  ...
  /**
   * Register the one direct replacement an organization may run beside its
   * grandfathered connection. The database owns the concurrent uniqueness
   * rail; this guard owns the domain meaning and gives an early refusal.
   */
  async registerReplacementConnection(
    ...
    const predecessor = await this.connections.findConnection({
    ...
    return [
      {
        ...
        data: {
          ...
          arrivalPolicy: data.arrivalPolicy,
          replacesConnectionId: data.replacesConnectionId,
          inheritedDomainVerifications: predecessor.domainVerifications.flatMap(
            (proof) =>
              qualifySsoDomainOwnership({
                state: predecessor,
                domain: proof.domain,
  ...
  async selectMigrationRoute(
    ...
    const state = await this.require(data, SELECT_MIGRATION_ROUTE_COMMAND_TYPE);
    this.requireReplacementMigration(state);
    if (
      state.migrationPhase === "FINALIZING" ||
      state.migrationPhase === "FINALIZED"
    ) {
      throw new SsoConnectionInvalidTransitionError(
    ...
    const selectedPhase =
      data.route === "legacy" ? "GRACE_LEGACY" : "GRACE_DIRECT";
    if (state.migrationPhase === selectedPhase) return [];
    return [
      {
  ...
  async beginMigrationFinalization(
    ...
    this.requireReplacementMigration(state);
    if (state.migrationPhase === "FINALIZING") return [];
  ...
  async finalizeMigration(
    ...
    const state = await this.require(data, FINALIZE_MIGRATION_COMMAND_TYPE);
    this.requireReplacementMigration(state);
    if (state.migrationPhase === "FINALIZED") return [];
  ...
  private requireReplacementMigration(state: SsoConnectionState): void {
    ...
    throw new SsoConnectionInvalidTransitionError(
      `connection ${state.connectionId} is not a migration replacement`,
    );
  }

  private async refuseCompetingConnection({
    organizationId,
    connectionId,
    kind,
    allowedConnectionId,
    ...
    const all = await this.connections.findConnectionsForOrganization?.({
      organizationId,
    });
    const connections =
      all ??
      [
        await this.connections.findConnectionForOrganization({
          organizationId,
        }),
      ].filter((held): held is SsoConnectionState => held !== null);
    const competing = connections.find((held) => {
      if (held.connectionId === connectionId) return false;
      if (held.connectionId === allowedConnectionId) return false;
      return true;
    });
  ...
  private async claimRegistrationSlot({
    organizationId,
    connectionId,
    commandId,
    kind,
    ...
    const held = await this.registrationSlots.claim({
      organizationId,
      connectionId,
      commandId,
      kind,
    ...
    if (
      ...
      held.replacesConnectionId === replacesConnectionId
    ) {
      return;
    }
    throw new SsoConnectionAlreadyRegisteredError(
      `organization ${organizationId} already reserved its ${kind} connection slot`,
    );
  }
