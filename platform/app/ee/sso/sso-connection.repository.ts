...
export interface SsoConnectionReadRepository {
  ...
  /**
   * All nonterminal connections for an organization. Migration registration
   * uses this to distinguish the one grandfathered predecessor plus its one
   * direct replacement from an accidental third connection. Implementations
   * may omit it during rolling deployment; the database uniqueness rail is
   * still authoritative under concurrency.
   */
  findConnectionsForOrganization?(args: {
    organizationId: string;
...
export interface SsoDomainClaimQueueRepository {
  findAllDisputed(args: { limit: number }): Promise<SsoDomainClaimQueueEntry[]>;
}
