...
describe("the ingestion-ledger union's money", () => {
  ...
  describe("when a ledger read refuses to answer", () => {
    it("leaves the rest of the dashboard standing rather than failing the page", async () => {
      ...
      const repository = {
        ...
        findIngestionPrincipalSummary: vi
          .fn()
          .mockRejectedValue(new Error("summed nano-USD past the safe range")),
      };

      const summary = await new PersonalUsageService(
        repository as unknown as PersonalUsageClickHouseRepository,
      ).summary({
