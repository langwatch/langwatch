...
describe("given a Genie config naming a real workspace", () => {
  describe("when the destination is checked", () => {
    it.each([
      ...
      expect(() =>
        assertPullDestinationAllowed({ parserConfig: genie(url) }),
      ).not.toThrow();
    });
  });
});
  ...
  describe("when the destination is checked", () => {
    ...
    it("refuses a host the attacker controls", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: genie("https://attacker.example.com"),
        }),
    ...
    it("refuses a lookalike that merely contains the real domain", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: genie(
            "https://azuredatabricks.net.attacker.example.com",
          ),
        }),
      ).toThrow(/Databricks workspace address/);
    });
    ...
    it("refuses plain http, which would expose the token even to a real workspace", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: genie(
            "http://adb-1234567890123456.7.azuredatabricks.net",
          ),
        }),
      ).toThrow(/Databricks workspace address/);
    });
    ...
    it("refuses a URL smuggling credentials in its userinfo", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: genie("https://user:pass@adb-1.7.azuredatabricks.net"),
        }),
      ).toThrow(/Databricks workspace address/);
    });
...
describe("given a Copilot config naming a real Power Platform environment", () => {
  describe("when the destination is checked", () => {
    ...
    it.each([
      ...
      expect(() =>
        assertPullDestinationAllowed({ parserConfig: dataverse(url) }),
      ).not.toThrow();
    });
  });
});
...
describe("given a Copilot config pointed somewhere the secret must never go", () => {
  describe("when the destination is checked", () => {
    ...
    it("refuses a host the attacker controls", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: dataverse("https://attacker.example.com"),
        }),
    ...
    it("refuses a lookalike domain that merely ends in the same letters", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: dataverse("https://evildynamics.com"),
        }),
      ).toThrow(/Power Platform environment address/);
    });
    ...
    it("refuses plain http even to a real environment", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: dataverse("http://org12345.crm.dynamics.com"),
        }),
      ).toThrow(/Power Platform environment address/);
    });
    ...
    it("refuses credentials embedded in the address", () => {
      expect(() =>
        assertPullDestinationAllowed({
          parserConfig: dataverse(
            "https://user:pass@org12345.crm.dynamics.com",
          ),
        }),
      ).toThrow(/Power Platform environment address/);
    });
...
describe("given a config whose adapter is known only to the caller", () => {
  ...
  it("checks the destination using the adapter the caller supplies", () => {
    ...
    expect(() =>
      assertPullDestinationAllowed({ parserConfig: withoutAdapter }),
    ).not.toThrow();
    expect(() =>
      assertPullDestinationAllowed({
        parserConfig: withoutAdapter,
        adapterId: COPILOT_STUDIO_DATAVERSE_ADAPTER_ID,
      }),
    ).toThrow(/Power Platform environment address/);
  });
  ...
  it("does the same for a workspace being repointed", () => {
    expect(() =>
      assertPullDestinationAllowed({
        parserConfig: { workspaceUrl: "https://attacker.example.com" },
        adapterId: DATABRICKS_GENIE_ADAPTER_ID,
      }),
    ).toThrow(/Databricks workspace address/);
  });
...
describe("given a config for an adapter with no fixed destination", () => {
  describe("when the destination is checked", () => {
    ...
    it("ignores a config with no adapter at all", () => {
      expect(() =>
        assertPullDestinationAllowed({ parserConfig: { ottlStatements: [] } }),
      ).not.toThrow();
      expect(() =>
        assertPullDestinationAllowed({ parserConfig: null }),
      ).not.toThrow();
    });
  });
});
