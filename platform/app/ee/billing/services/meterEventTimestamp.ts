 ...
 * older than that, and the amount is still owed, so it is reported at the time
 * of reporting instead. The event identifier names the period it came from
 * either way, which is what keeps the line attributable on the invoice.
 */

/** How far back Stripe accepts a meter event, in days. */
 ...
 * `nowMs`, so the event is never refused for its timestamp and never dated
...
export function meterEventTimestampSeconds({
  ...
  const withinWindow =
    periodEndMs <= nowMs && nowMs - periodEndMs <= METER_EVENT_MAX_AGE_MS;
