...
/**
 * A submitted search is a place Back can return to. The query, the window and
 * the lens only change on a submit, so a change in any of them pushes a
 * history entry; the run keys that follow a submit, and the first write of a
 * mount, rewrite the entry in place.
 *
 * Spec: specs/traces-v2/search.feature ("Back returns to the search before").
 */
...
import { isNewSearchEntry, useURLSync } from "../useURLSync";
...
describe("isNewSearchEntry", () => {
  describe("when the page writes its address for the first time", () => {
    it("rewrites the entry the page was opened with", () => {
      expect(
        isNewSearchEntry({ previousSearch: null, nextSearch: "all?q=a" }),
      ).toBe(false);
    });
  });
  ...
  describe("when only the run keys moved", () => {
    it("rewrites the entry the submit made", () => {
      expect(
        isNewSearchEntry({ previousSearch: "all?q=a", nextSearch: "all?q=a" }),
      ).toBe(false);
    });
  });
  ...
  describe("when the query, the window or the lens moved", () => {
    it("is a new entry", () => {
      expect(
        isNewSearchEntry({ previousSearch: "all?q=a", nextSearch: "all?q=b" }),
      ).toBe(true);
    });
  });
});
