import { type ExplorerStore, useExplorerStore } from "../stores/explorerStore";
...
const sameRange = (
  a: ExplorerState["timeRange"],
  b: ExplorerState["timeRange"],
 ...
 *
 * A transform answers the page state only. The parsed query, the keyset
 * cursors and the lens drafts are the store's to derive, so each field that
 * moved goes through the action a click on the page would have used. The
 * store is re-read before every step because an action moves more than its
 * own field: a lens brings its filter, and a new sort returns to the first
 * page.
 */
