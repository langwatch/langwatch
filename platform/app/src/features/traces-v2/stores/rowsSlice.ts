...
/**
 * What the last list read answered for the page state as it stands, kept so
 * a reader of the Explorer (the header count, the pagination line, a Langy
...
/**
 * The rows slice of the Explorer store: which rows are open, and what the
 * last read answered.
 */
export interface RowsSlice {
  ...
  /**
   * Open or close one row. `exclusive` closes every other row first, which
   * is how the Conversations lens keeps one conversation open at a time.
   */
...
export const createRowsSlice: StateCreator<ExplorerStore, [], [], RowsSlice> = (
  set,
) => ({
