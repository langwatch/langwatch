...
describe("sseStatusStore", () => {
  describe("when reading the initial liveUpdatesMode", () => {
    ...
    it("honours an explicit persisted 'paused' choice", async () => {
      ...
      expect(useSseStatusStore.getState().sseConnectionState).toBe("disconnected");
    });
  ...
  describe("when toggling the mode", () => {
    ...
    it("persists each step so a reload resumes mid-cycle", async () => {
      ...
      expect(window.localStorage.getItem(LEGACY_LIVE_UPDATES_BOOL_KEY)).toBe("true");

      useSseStatusStore.getState().toggleLiveUpdates(); // → paused
      expect(window.localStorage.getItem(LIVE_UPDATES_STORAGE_KEY)).toBe("paused");
      expect(window.localStorage.getItem(LEGACY_LIVE_UPDATES_BOOL_KEY)).toBe("false");
    });
    ...
    it("flips sseConnectionState to disconnected when paused, connecting otherwise", async () => {
      ...
      expect(useSseStatusStore.getState().sseConnectionState).toBe("connecting");
    });
  });
});
