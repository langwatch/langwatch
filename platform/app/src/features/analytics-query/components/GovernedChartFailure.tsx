 ...
 */
export const GOVERNED_CHART_FAILURE_COPY: Record<
  VegaValidationErrorCode,
  FailureCopy
> = {
  "invalid-json": {
...
export interface GovernedChartFailureProps {
 ...
 */
export function GovernedChartFailure({ errors }: GovernedChartFailureProps) {
  const leading = errors[0];
  if (leading === undefined) return null;

  const copy = GOVERNED_CHART_FAILURE_COPY[leading.code];
  ...
  return (
    <VStack
      ...
      borderRadius="8px"
      role="status"
      data-testid="governed-chart-failure"
