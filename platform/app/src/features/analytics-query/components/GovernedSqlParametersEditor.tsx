 ...
 */
function valueOf(row: ParameterRow): GovernedSqlParameterValue | undefined {
  switch (row.kind) {
    case "text":
      return row.text;
    case "number": {
...
function recordOf(
  ...
  const record: Record<string, GovernedSqlParameterValue> = {};
...
export function GovernedSqlParametersEditor({
  ...
  return (
    <VStack
      ...
      gap={2}
      width="full"
      data-testid="governed-sql-parameters"
    >
      <HStack gap={2}>
        <Button
          size="xs"
