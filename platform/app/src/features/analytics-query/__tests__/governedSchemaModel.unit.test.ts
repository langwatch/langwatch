...
describe("mapping the governed schema response", () => {
  describe("given a response with datasets, columns, types, units and documentation", () => {
    describe("when it is mapped for the browser and the editor", () => {
      ...
      it("carries every dataset and column from the response and invents none", () => {
        ...
        expect(model.datasets.map((dataset) => dataset.name)).toEqual(
          SCHEMA_DATASET_NAMES,
        );
        expect(
          model.datasets.flatMap((dataset) =>
      ...
      it("offers nothing at all for a response that carries nothing", () => {
        const model = governedSchemaModel({
          database: "analytics",
          datasets: [],
        });

        expect(model.datasets).toEqual([]);
        expect(governedSqlCompletionItems(model)).toEqual([]);
        expect(
          governedSqlHoverFor({ model, identifier: "analytics.traces_daily" }),
        ).toBeUndefined();
      });
      ...
      it("suggests only names the response carried, and no withheld column", () => {
        const model = governedSchemaModel(SCHEMA_RESPONSE);
        const items = governedSqlCompletionItems(model);
  ...
  describe("given a member hovering an identifier", () => {
    describe("when the schema names it", () => {
      it("answers from the response and stays silent about withheld columns", () => {
        ...
        expect(
          governedSqlHoverFor({ model, identifier: "latency_ms" }),
        ).toEqual({
        ...
        expect(
          governedSqlHoverFor({ model, identifier: "traces_daily" })?.title,
  ...
  describe("given a search term", () => {
    describe("when the schema is narrowed", () => {
      it("keeps a matching dataset whole and trims a dataset matched through a column", () => {
        ...
        const byDataset = filterGovernedSchemaModel({
          model,
          search: "evaluations",
        });
        ...
        const byColumn = filterGovernedSchemaModel({
          model,
          search: "latency",
        });
        ...
        expect(
          filterGovernedSchemaModel({ model, search: "   " }).datasets,
        ).toHaveLength(2);
      });
    });
  });
