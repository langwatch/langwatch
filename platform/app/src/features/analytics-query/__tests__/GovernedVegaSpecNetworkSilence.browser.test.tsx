 ...
 * mount of `GovernedVegaLiteChart`, and watches four channels at once:
...
import { GOVERNED_FIXTURE_COLUMNS } from "./fixtures/governedDatasetRegistry";
...
const QUERY_RESULT_ROWS: GovernedDataset = Array.from(
...
const MODEL_CATALOG_ROWS: GovernedDataset = [
  "model-0",
  "model-1",
  "model-2",
  "model-3",
...
const DATASETS: Readonly<Record<string, GovernedDataset>> = {
...
interface CorpusEntry {
  ...
  /** Whether the governed policy admits it, and therefore whether it draws. */
...
function chartStatus(container: ParentNode): string | null {
  return (
    container
      .querySelector('[data-testid="governed-vega-chart-view"]')
...
async function attemptToRender(entry: CorpusEntry): Promise<CorpusOutcome> {
  ...
  try {
    ...
    const refusalShown = await poll(
      () =>
        container.querySelector('[data-testid="governed-chart-failure"]') !==
        null,
    );
...
describe("the governed chart surface with the browser's network recorded", () => {
