/**
 * The governed chart drawn on a page where nothing may be evaluated from a
 * string.
 *
...
import { governedVegaEmbedOptions } from "../hooks/useGovernedVegaView";
import { buildGovernedVegaSpec } from "../visualization/buildGovernedVegaSpec";
...
describe("the governed chart on a page that forbids string evaluation", () => {
  describe("given the workbench served under a policy without unsafe-eval", () => {
    describe("when a valid specification renders as a chart", () => {
      ...
      it("draws through Vega's expression interpreter, while the same specification with the interpreter disabled is refused", async () => {
        ...
        expect(
          governedVegaEmbedOptions({ themeConfig: {}, colorMode: "light" }).ast,
        ).toBe(true);
        ...
        // injected here.
        const handedToVega = buildGovernedVegaSpec({
        ...
        expect(
          document.querySelector('[data-testid="governed-vega-chart-view"]'),
