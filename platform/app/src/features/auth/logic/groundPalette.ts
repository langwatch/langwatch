...
export type AuthDoor = "signin" | "signup";
 ...
 */
export type AuthDepth =
  /** Being asked for an address. */
  | "entry"
  /** Being asked for a secret, or offered the ways to give one. */
  | "credential"
  /**
   * Past the secret and being asked to prove a second factor. Its own depth
   * rather than a second helping of `credential`, because the ground's whole
   * job is to move when the step does — and somebody who has just typed a
   * correct password and been asked for something else needs to see that the
   * screen registered the first answer.
   */
  | "challenge"
 ...
 */
const DEPTH_SHIFT: Record<AuthDepth, GroundShift> = {
  ...
  challenge: {
    // A step further round than the password it follows, and tighter: the
    // field pulls in around a card holding one short box, which is the whole
...
export function resolveGroundShift(stage: AuthStage): GroundShift {
