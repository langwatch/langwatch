...
/**
 * The address step: the whole of what the auth screens asks before it knows
 * anything (ADR-117 §2).
 *
          ...
          <AuthField label="Email" error={showEmailError}>
            {(id) => (
              <Input
                ...
                // Locked while the router is being asked. The answer decides
                // which screen comes next, so an address edited mid-flight
                // would land somebody on the decision for the address they
                // just stopped having — and the field going quiet is also the
                // clearest signal available that something is happening.
