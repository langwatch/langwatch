...
async function createAccountWithPasskey(
  ...
  try {
    const result = await authClient.passkey.addPasskey({
      ...
      // asks for no session here and the emailed link opens the first one.
      //
      // Where it IS asked for, the plugin mints it in the same transaction
      // that creates the account and writes the credential, so none of the
      // three can land without the others.
      createSession: addressIsConfirmed,
    });
