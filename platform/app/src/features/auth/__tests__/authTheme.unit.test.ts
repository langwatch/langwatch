/**
 * The auth screens's palette is a THEME, not a stylesheet.
 *
 * These pin the two properties that make that true and that a refactor could
 * quietly undo: the tokens actually reach the system (so `bg="auth.action"`
 ...
 * wide-gamut block in `auth.css` re-declares them by name — a rename
...
describe("given the auth screens' theme tokens", () => {
  describe("when the app's design system is built", () => {
    it("carries the whole namespace, so a component can style through it", () => {
      ...
      for (const token of [
        ...
        expect(variables).toContain(`--chakra-colors-auth-${token}`);
      }
    });
    ...
    it("resolves the action colour on both grounds", () => {
      ...
      const values = blocks
        .map((block) => block["--chakra-colors-auth-action"])
        .filter(Boolean);
     ...
     * tokens: the auth screens borrows the marketing site's orange, and the app
    ...
    it("touches no token any other surface reads", () => {
      const foreign = [...allVariables()].filter(
        (name) =>
          name.startsWith("--chakra-colors-auth-") === false &&
          name.includes("auth-screen"),
      );
