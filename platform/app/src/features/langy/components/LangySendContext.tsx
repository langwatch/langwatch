...
/**
 * The composer's send, handed to the cards a conversation renders, so a card
 * can offer a next step in words: the reader chooses the offer, the message
 * goes to Langy as if typed, and Langy does the work with everything it has
 * to ask (which target, which parameters) instead of the card scheduling
 * anything itself.
 */
...
export function LangySendProvider({
  ...
  return (
    <LangySendContext.Provider value={value}>
      {children}
    </LangySendContext.Provider>
  );
}
