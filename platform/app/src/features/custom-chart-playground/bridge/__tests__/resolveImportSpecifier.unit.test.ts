/**
 * How a widget's `import` specifiers are rewritten for the sandboxed ESM
 * frame: bare packages go to esm.sh with React externalised, the frame's own
 * built-ins and anything already a URL/path are left alone.
 *
...
describe("resolveImportSpecifier", () => {
  describe("given a bare package specifier", () => {
    ...
    it("resolves to esm.sh with React externalised", () => {
      expect(resolve("dayjs")).toBe(
        "https://esm.sh/dayjs?external=react,react-dom",
      );
    });
  });

  describe("given a scoped or deep package specifier", () => {
  ...
  describe("given a built-in module the frame provides", () => {
  ...
  describe("given a URL, data, blob or relative specifier", () => {
