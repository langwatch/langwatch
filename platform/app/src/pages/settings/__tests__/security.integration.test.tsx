...
vi.mock("~/utils/auth-client", () => ({
  ...
  linkAccount: vi.fn(),
  // Both the passkey and two-step sections reach the auth client at the top
  // of their bodies, since a hook cannot be conditional — so the double has to
  // answer even on a deployment that offers neither. Passkeys are no longer a
  // setting at all; two-step still is (`MFA_ENROLLMENT_OPEN`), and that is the
  // gate the call still sits above. Only the hooks are named: `passkey.*` is
  // reached from click handlers this suite never fires.
  authClient: {
