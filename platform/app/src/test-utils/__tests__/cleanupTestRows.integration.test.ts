...
const bystanderOrgId = generate("organization").toString();
const bystanderTeamId = generate("team").toString();
const suiteOrgId = generate("organization").toString();
...
describe("cleanupTestRows (real DB)", () => {
  beforeAll(async () => {
  ...
  describe("given a teardown whose ids were never assigned", () => {
    ...
    it("still cleans the entries that are identified", async () => {
      const doomedId = generate("organization").toString();
      await prisma.organization.create({
        data: {
          id: doomedId,
          name: `Doomed Org ${ns}`,
  ...
  describe("given a teardown whose ids were all assigned", () => {
    ...
    it("deletes exactly the matching rows and resolves", async () => {
      const ownId = generate("organization").toString();
      await prisma.organization.create({
        data: {
          id: ownId,
          name: `Owned Org ${ns}`,
  ...
  describe("given a delete that the database refuses", () => {
    ...
    it("reports the failure and still cleans the remaining entries", async () => {
      const survivorId = generate("organization").toString();
      await prisma.organization.create({
        data: {
          id: survivorId,
