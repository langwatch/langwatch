...
/**
 * Whether `TEST_DATABASE` actually holds the migrated schema. `event_log` is the
 * sentinel: it is created by the first migration, so its absence means the
 * container has no schema regardless of what the host-side hash claims.
 */
async function testSchemaIsPresent(baseUrl: string): Promise<boolean> {
  const url = new URL(baseUrl);
  url.searchParams.set(
    "query",
    `SELECT count() FROM system.tables WHERE database = '${TEST_DATABASE}' AND name = 'event_log'`,
  );
  try {
    const response = await fetch(url, { method: "GET" });
    if (!response.ok) return false;
    return Number.parseInt((await response.text()).trim(), 10) > 0;
  } catch {
    return false;
  }
}
...
export async function setup(): Promise<void> {
  ...
  // Run goose migrations to create database and tables. With reusable
  // containers we skip when nothing changed — but the hash lives on the HOST
  // while the schema lives in the CONTAINER, so an unchanged hash is not
  // evidence the schema is there. A container that was pruned, or a fresh one
  // started against a leftover hash file, otherwise skips migrations and every
  // test then fails on a missing table. Verify the schema, not just the hash.
  const schemaPresent = await testSchemaIsPresent(clickHouseBaseUrl);
  const migrationsChanged = needsMigrationRerun();
  if (migrationsChanged || !schemaPresent) {
    console.log(
      migrationsChanged
        ? "[globalSetup] Migration files changed, running ClickHouse migrations..."
        : "[globalSetup] Test schema absent from the container, running ClickHouse migrations...",
    );
    await migrateUp({
