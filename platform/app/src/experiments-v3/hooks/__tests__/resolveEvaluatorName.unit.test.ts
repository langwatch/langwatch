...
import { resolveEvaluatorName } from "../useEvaluatorName";
...
describe("given an evaluator to name on a chip", () => {
  describe("when it carries no name and no database evaluator", () => {
    ...
    it("reads the name of its type", () => {
      expect(resolveEvaluatorName({ evaluator: evaluator() })).toBe(
    ...
    it("never reads the config id", () => {
      expect(resolveEvaluatorName({ evaluator: evaluator() })).not.toContain(
        "evaluator_AzPF-HSd",
      );
    });
  });
    ...
    it("reads that name", () => {
      expect(
        resolveEvaluatorName({
          evaluator: evaluator({
            localEvaluatorConfig: { name: "l3 exact match" },
          }),
    ...
    it("reads the type, which still says more than the id", () => {
      expect(
        resolveEvaluatorName({
          evaluator: evaluator({
            evaluatorType:
              "custom/evaluator_abc" as EvaluatorConfig["evaluatorType"],
          }),
        }),
