...
describe.skipIf(process.env.CI)("Scenario Processor - OTEL Isolation", () => {
  ...
  it("sends traces to the configured LANGWATCH_ENDPOINT", async () => {
    ...
    expect(
      ...
      `Expected traces but got 0 requests. Child exit code: ${exitCode}, stderr: ${stderr.slice(0, 500)}`,
    ).toBeGreaterThan(0);
  }, 60000);

  it("sends traces to the OTEL traces endpoint", async () => {
    const jobData = createTestJobData();
    const env = {
      LANGWATCH_API_KEY: "isolated-test-key",
      LANGWATCH_ENDPOINT: `http://127.0.0.1:${mockCollector.port}`,
    ...
    expect(
      hasTracesRequest,
      `Expected traces endpoint request. Requests: ${mockCollector.requests.length}, urls: ${mockCollector.requests.map((r) => r.url).join(", ")}, child exit: ${exitCode}, stderr: ${stderr.slice(0, 500)}`,
    ).toBe(true);
  }, 60000);

  it("concurrent child processes have isolated traces with distinct scenario IDs", async () => {
  ...
  it("passes OTEL_RESOURCE_ATTRIBUTES for labels", async () => {
    const jobData = createTestJobData({
      scenario: {
        id: "scenario-abc123",
  ...
  it("includes setId in scenario events sent to collector", async () => {
