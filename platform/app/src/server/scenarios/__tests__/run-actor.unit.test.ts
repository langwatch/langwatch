...
describe("the actor of a REST run", () => {
  describe("when the key belongs to a person", () => {
    ...
    it("records that person, through the surface the request declared", () => {
      ...
      expect(
        runActorFromRequest({ userId: "user_lena", surfaceHeader: undefined }),
      ).toEqual({ id: "user_lena", label: "api" });

      // Only "cli" is honored, so a caller cannot claim the in-app surface.
      expect(
        runActorFromRequest({ userId: "user_lena", surfaceHeader: "user" }),
      ).toEqual({ id: "user_lena", label: "api" });
    });
  });
  ...
  describe("when the key belongs to no person", () => {
    ...
    it("names no actor, whatever surface the request declares", () => {
      expect(
        runActorFromRequest({ userId: null, surfaceHeader: "cli" }),
      ).toBeUndefined();
      expect(
        runActorFromRequest({ userId: undefined, surfaceHeader: undefined }),
      ).toBeUndefined();
    });
  });
});
