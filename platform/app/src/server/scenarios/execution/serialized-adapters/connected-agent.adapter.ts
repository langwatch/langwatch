...
export class SerializedConnectedAgentAdapter extends SerializedAgentAdapter {
  ...
  constructor({
    ...
    this.logger =
      logger ??
      createChildProcessLogger("langwatch:scenarios:connected-adapter");
    this.fetchImpl =
      fetchImpl ?? ((url, init) => fetch(url, init) as ReturnType<FetchLike>);
    this.sleep =
  ...
  private async post({
    ...
    const signal = AbortSignal.timeout(
      this.config.timeoutMs + REQUEST_SLACK_MS,
    );
    try {
