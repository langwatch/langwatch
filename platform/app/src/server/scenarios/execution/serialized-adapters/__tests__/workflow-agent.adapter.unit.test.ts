...
describe("SerializedWorkflowAgentAdapter", () => {
  ...
  describe("when the workflow execution fails", () => {
     ...
     * not `{detail}` alone. Reading only `detail` reduced a real 4xx to the raw
     * JSON blob, which is what the PR description previously claimed was
     * forwarded "verbatim". httpErrorMessage checks `detail` first (for the
     * older Python service's shape), then falls back to nlpgo's envelope.
     */
