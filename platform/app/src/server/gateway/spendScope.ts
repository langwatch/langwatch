...
/** Drops every cached organization. Exported for tests and for a caller that
 *  has just written a project and needs the next read to see it. */
export function clearSpendScopeCache(): void {
  projectCache.clear();
}
  ...
  return {
    ...
    virtualKeyIds:
      externalIds === undefined
        ? undefined
        : await virtualKeyIdsForExternalIds({ organizationId, externalIds }),
  };
}

async function virtualKeyIdsForExternalIds({
  organizationId,
  externalIds,
}: {
  organizationId: string;
