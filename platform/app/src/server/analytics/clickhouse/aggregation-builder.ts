...
/**
 * Sub-day (< 1 day) date truncation. Uses HOUR interval only when
 * timeScaleMinutes is an exact multiple of 60; otherwise uses MINUTE
 * interval to preserve precision (e.g., 90 minutes).
 */
...
/** Day-or-longer date truncation: day, week-of-days, week, or month buckets. */
function dateTruncByDays(
  timeScaleMinutes: number,
  validatedTimeZone: string,
): string {
...
function getDateTruncFunction(
  ...
  if (timeScaleMinutes <= 1) {
    ...
    return dateTruncSubDay(timeScaleMinutes, validatedTimeZone);
  } else {
    return dateTruncByDays(timeScaleMinutes, validatedTimeZone);
  }
}
...
/**
 * Translate each series entry to its metric SQL, mutating `allJoins` with
 * every table the translations require.
 */
function translateSeriesMetrics({
  series,
  allJoins,
}: {
  series: TimeseriesQueryInput["series"];
  allJoins: Set<CHTable>;
}): MetricTranslation[] {
  const metricTranslations: MetricTranslation[] = [];

  for (let i = 0; i < series.length; i++) {
    const s = series[i]!;
    const translation = s.pipeline
      ? translatePipelineAggregation({
          metric: s.metric,
          aggregation: s.aggregation,
          pipelineField: s.pipeline.field,
          pipelineAggregation: s.pipeline.aggregation,
          index: i,
          key: s.key,
          subkey: s.subkey,
        })
  ...
  return metricTranslations;
}

/** Resolved groupBy column plus the routing flags derived from it. */
interface TimeseriesGroupByState {
  groupByColumn: string | null;
  usesArrayJoin: boolean;
  groupByHandlesUnknown: boolean;
...
/**
 * Resolve the groupBy column expression and its routing flags, mutating
 * `allJoins` with any tables the groupBy expression requires.
 */
function resolveTimeseriesGroupByState({
  input,
  allJoins,
}: {
  input: TimeseriesQueryInput;
  allJoins: Set<CHTable>;
}): TimeseriesGroupByState {
  if (!input.groupBy) {
    return {
      groupByColumn: null,
      usesArrayJoin: false,
...
/**
 * Build JOIN clauses with column pruning: collect all SQL expressions that
 * reference columns from joined tables so each JOIN subquery only SELECTs
 * the columns actually needed.
 */
function buildTimeseriesJoinClauses({
  allJoins,
  allExpressions,
}: {
  allJoins: Set<CHTable>;
 ...
 * it is a scope restriction, so it is never negated. Mutates
 * `allTranslationParams` with the traceIds param when present.
 */
function buildTimeseriesFilterWhere({
  input,
  filterTranslation,
  ts,
...
/**
 * SELECT expression for the groupBy column.
 * - If handlesUnknown is true, the column expression already handles NULL/empty -> 'unknown'
 * - Otherwise, convert NULL to 'unknown' for ES `missing: "unknown"` behavior
 */
function buildGroupByColumnSelectExpr(
  groupByColumn: string,
  groupByHandlesUnknown: boolean,
): string {
  if (groupByHandlesUnknown) {
    return `${groupByColumn} AS group_key`;
  }
  return `if(${groupByColumn} IS NULL, 'unknown', toString(${groupByColumn})) AS group_key`;
}

/**
 * Route a translated timeseries query to the specialized builder its shape
 * requires (arrayJoin/CTE dedup, mixed-eval CTE, "full" subquery CTE,
 * date-bucketed pipeline), falling back to the standard single-scan builder.
 */
function routeTimeseriesQuery({
  input,
  ts,
  timeZone,
  groupByColumn,
  ...
  return routeNonArrayJoinTimeseriesQuery({
    input,
    ts,
    timeZone,
    groupByColumn,
...
/**
 * Route the non-arrayJoin timeseries paths: mixed trace+eval metrics (CTE),
 * "full" timeScale without groupBy (CTE), date-bucketed pipeline metrics,
 * falling back to the standard single-scan builder.
 */
function routeNonArrayJoinTimeseriesQuery({
  input,
  ts,
  timeZone,
  groupByColumn,
...
/**
 * Build the standard (non-arrayJoin, non-mixed-eval, non-CTE, non-pipeline)
 * timeseries query: a single GROUP BY period[, date][, group_key] scan.
 */
function buildStandardTimeseriesQuery({
  ...
  // (exclude empty strings via HAVING when it doesn't handle 'unknown' itself — ES terms excludes them)
  if (groupByColumn) {
    selectExprs.push(
      buildGroupByColumnSelectExpr(groupByColumn, groupByHandlesUnknown),
    );
  }
...
export function buildTimeseriesQuery(input: TimeseriesQueryInput): BuiltQuery {
  ...
  const metricTranslations = translateSeriesMetrics({
    series: input.series,
    allJoins,
  });
  ...
  const allTranslationParams: Record<string, unknown> = {
  ...
  const {
    ...
    groupByRequiresSpans,
    spanModelPartitioned,
  } = resolveTimeseriesGroupByState({ input, allJoins });
  ...
  const joinClauses = buildTimeseriesJoinClauses({ allJoins, allExpressions });
  ...
  const filterWhere = buildTimeseriesFilterWhere({
    input,
    filterTranslation,
    ts,
    allTranslationParams,
...
/**
 * Build the per-metric per-trace CTE plan: for each simple metric, the inner
 * CTE expression that collapses it to one value per trace, and the outer
 * expression that re-aggregates across traces.
 *
 ...
 *
 * Per-trace aliases start with the metric index digit (e.g. `0__…`), so they
 * are wrapped in `quoteIdentifier` to satisfy ClickHouse's identifier rules.
 */
/** A single metric's per-trace CTE plan: its inner and outer SQL fragments. */
interface MixedEvalMetricPlanEntry {
  innerExpr: string;
  outerExpr: string;
}

/** True for `count()` / `count(*)`-shaped expressions. */
function isCountLikeMetricExpr(expr: string): boolean {
  return /\bcount\s*\(\s*\*?\s*\)/.test(expr);
}

/** True for `uniq(...)` / `uniqExact(...)` over `TraceId`. */
function isUniqTraceIdMetricExpr(expr: string): boolean {
  return (
    (/\buniq\s*\(/.test(expr) || /\buniqExact\s*\(/.test(expr)) &&
    expr.includes("TraceId")
  );
}
 ...
 * outer re-aggregates across traces via mapEvalAggregationToOuter.
 */
function planEvalMetricPerTrace({
  metric,
  exprWithoutAlias,
  perTraceAlias,
  quotedAlias,
  ...
  exprWithoutAlias: string;
  perTraceAlias: string;
  quotedAlias: string;
}): MixedEvalMetricPlanEntry {
  ...
  return {
    innerExpr: `${exprWithoutAlias} AS ${perTraceAlias}`,
    outerExpr: `${outerAgg}(${perTraceAlias}) AS ${quotedAlias}`,
  };
}

/**
 * Trace metric: inner emits `any(<underlying column>)` per trace; outer
 * applies the original aggregation to the per-trace column, preserving
 * coalesce/quantile wrappers by substituting the column reference in the
 * original expression.
 */
function planTraceMetricPerTrace({
  exprWithoutAlias,
  perTraceAlias,
  quotedAlias,
}: {
  ...
  return {
    innerExpr: `any(${column}) AS ${perTraceAlias}`,
    outerExpr: `${outerExpr} AS ${quotedAlias}`,
  };
}

/**
 * Push the "date"/"group_key" column labels used identically by the mixed
 * eval query's inner GROUP BY, outer SELECT and outer GROUP BY lists.
 */
function pushDateAndGroupKeyLabels(
  target: string[],
  dateTrunc: string | null,
  groupKeyExpr: string | null,
): void {
  if (dateTrunc) target.push("date");
  if (groupKeyExpr) target.push("group_key");
}

/** Per-trace CTE plan for a single simple metric. See buildMixedEvalMetricPlan. */
function planMixedEvalMetric(
  metric: MetricTranslation,
): MixedEvalMetricPlanEntry {
  ...
  // uniq/uniqExact of TraceId is the same: 1 per trace row.
  if (
    isCountLikeMetricExpr(exprWithoutAlias) ||
    isUniqTraceIdMetricExpr(exprWithoutAlias)
  ) {
    return {
      innerExpr: `1 AS ${perTraceAlias}`,
      outerExpr: `sum(${perTraceAlias}) AS ${quotedAlias}`,
    };
  }

  return planTraceMetricPerTrace({
    exprWithoutAlias,
    perTraceAlias,
    quotedAlias,
  });
...
function buildMixedEvalMetricPlan(simpleMetrics: MetricTranslation[]): {
  innerExprs: string[];
  outerExprs: string[];
} {
  const innerExprs: string[] = [];
  const outerExprs: string[] = [];

  for (const metric of simpleMetrics) {
    const { innerExpr, outerExpr } = planMixedEvalMetric(metric);
    innerExprs.push(innerExpr);
    outerExprs.push(outerExpr);
  }

  return { innerExprs, outerExprs };
}
 ...
 */
function buildMixedEvalFullPeriodQuery({
  input,
  ts,
  traceColumns,
  innerSelectExprs,
...
function buildMixedEvalTimeseriesQuery({
  ...
  const { innerExprs: metricInnerExprs, outerExprs: outerMetricExprs } =
    buildMixedEvalMetricPlan(simpleMetrics);
  innerSelectExprs.push(...metricInnerExprs);

  const innerGroupBy: string[] = ["trace_id", "period"];
  pushDateAndGroupKeyLabels(innerGroupBy, dateTrunc, groupKeyExpr);

  // Outer SELECT: re-aggregate across traces.
  const outerSelectExprs: string[] = ["period"];
  pushDateAndGroupKeyLabels(outerSelectExprs, dateTrunc, groupKeyExpr);
  outerSelectExprs.push(...outerMetricExprs);

  const outerGroupBy: string[] = ["period"];
  pushDateAndGroupKeyLabels(outerGroupBy, dateTrunc, groupKeyExpr);
  ...
  // period has no data). This matches the pattern used by buildSubqueryTimeseriesQuery.
  if (input.timeScale === "full" && !groupByColumn) {
    return buildMixedEvalFullPeriodQuery({
      input,
      ts,
      traceColumns,
      innerSelectExprs,
...
/**
 * Aggregations for which a `pipeline: { field: trace_id }` metric is safe to
 * re-translate as a simple metric in the arrayJoin CTE path. See
 * `promoteTraceIdPipelineMetrics`.
 */
 ...
 *
 * Also separates eval and non-eval metrics so we can pre-aggregate evaluation
 ...
 * SELECT where the `es` alias no longer exists.
 *
 * @regression issue #3088
 */
function promoteTraceIdPipelineMetrics({
  input,
  metricTranslations,
}: {
  input: TimeseriesQueryInput;
...
/**
 * Build the CTE SELECT expressions for the hard-coded trace_* columns
 * (cost, duration, tokens, etc.) the outer query's metric transforms read.
 *
 ...
 */
function buildArrayJoinTraceColumnExprs({
  ts,
  simpleMetrics,
  traceColumnWrapper,
  partitionedOrTrace,
  ...
  ts: string;
  simpleMetrics: MetricTranslation[];
  traceColumnWrapper: (col: string) => string;
  partitionedOrTrace: (bucketExpr: string, traceExpr: string) => string;
}): string[] {
  ...
  if (needsCacheTokenColumns) {
    exprs.push(
      ...buildArrayJoinCacheTokenColumnExprs({
        ts,
        traceColumnWrapper,
        partitionedOrTrace,
      }),
...
/**
 * Cache/reasoning token trace_* columns, only materialized when a requested
 * metric reads one of them. toFloat64 wraps the UInt64 attribute read so
 * both if() branches share a supertype with the smd join's Float64 sums.
 */
function buildArrayJoinCacheTokenColumnExprs({
  ts,
  traceColumnWrapper,
  partitionedOrTrace,
}: {
 ...
 */
function buildEvalPerTraceColumns({
  simpleMetrics,
  hasEvalMixWithTrace,
}: {
  simpleMetrics: MetricTranslation[];
  hasEvalMixWithTrace: boolean;
}): { cteExprs: string[]; evalPerTraceAliases: Map<string, string> } {
  const cteExprs: string[] = [];
  ...
  if (hasEvalMixWithTrace) {
    for (const metric of simpleMetrics) {
      ...
      cteExprs.push(`${exprWithoutAlias} AS ${perTraceAlias}`);
      evalPerTraceAliases.set(metric.alias, perTraceAlias);
    }
  }

  return { cteExprs, evalPerTraceAliases };
}
 ...
 */
function buildEvalRawColumnExprs({
  simpleMetrics,
  hasEvalMixWithTrace,
}: {
  simpleMetrics: MetricTranslation[];
  ...
  return Array.from(referencedEvalCols, (col) => {
...
/**
 * Build the outer SELECT expression list: period[, date], group_key, then
 * each simple metric re-aggregated for the deduplicated CTE — eval metrics
 * via mapEvalAggregationToOuter, everything else via transformMetricForDedup.
 */
function buildArrayJoinOuterSelectExprs({
  dateTrunc,
  simpleMetrics,
  evalPerTraceAliases,
}: {
 ...
 */
function buildArrayJoinCteBody({
  ts,
  traceColumns,
  cteSelectExprs,
  cteJoinClauses,
 ...
 */
function buildArrayJoinGroupKeyFilter({
  input,
  groupByColumn,
}: {
  input: TimeseriesQueryInput;
  ...
  groupKeyFilterParams: Record<string, unknown>;
} {
  let groupKeyFilter = "";
  ...
  return { groupKeyFilter, groupKeyFilterParams };
}

/**
 * Build the arrayJoin CTE's SELECT expression list: trace_id, group_key,
 * period[, date], the hard-coded trace_* columns, per-trace eval columns,
 * and raw eval columns — everything the outer query re-aggregates from.
 */
function buildArrayJoinCteSelectExprs({
  ts,
  groupKeyExpr,
  periodCaseExpr,
  dateTrunc,
  ...
  ts: string;
  groupKeyExpr: string;
  periodCaseExpr: string;
  dateTrunc: string | null;
  simpleMetrics: MetricTranslation[];
  hasEvalMixWithTrace: boolean;
  spanModelPartitioned: boolean;
}): { cteSelectExprs: string[]; evalPerTraceAliases: Map<string, string> } {
  ...
  const partitionedOrTrace = (bucketExpr: string, traceExpr: string) =>
    spanModelPartitioned
      ...
      : traceExpr;
  cteSelectExprs.push(
    ...buildArrayJoinTraceColumnExprs({
      ts,
      simpleMetrics,
      traceColumnWrapper,
      partitionedOrTrace,
  ...
  const { cteExprs: evalPerTraceCteExprs, evalPerTraceAliases } =
    buildEvalPerTraceColumns({ simpleMetrics, hasEvalMixWithTrace });
  cteSelectExprs.push(...evalPerTraceCteExprs);

  cteSelectExprs.push(
    ...buildEvalRawColumnExprs({ simpleMetrics, hasEvalMixWithTrace }),
  );

  return { cteSelectExprs, evalPerTraceAliases };
}

/**
 * Resolve the arrayJoin CTE body: the columns the dedup subquery must
 * expose (derived from the assembled CTE SELECT expressions so no
 * referenced column is pruned), the span-model partition join riding
 * alongside the generic filter/metric joins, and the CTE body itself.
 */
function resolveArrayJoinCteBody({
  ts,
  cteSelectExprs,
  filterWhere,
  joinClauses,
...
function buildArrayJoinTimeseriesQuery({
  ...
  const simpleMetrics = promoteTraceIdPipelineMetrics({
    input,
    metricTranslations,
  });
  ...
  const { cteSelectExprs, evalPerTraceAliases } = buildArrayJoinCteSelectExprs({
    ts,
    groupKeyExpr,
    periodCaseExpr,
    dateTrunc,
  ...
  const outerSelectExprs = buildArrayJoinOuterSelectExprs({
    dateTrunc,
    simpleMetrics,
    evalPerTraceAliases,
  });
  ...
  const cteBody = resolveArrayJoinCteBody({
    ts,
    cteSelectExprs,
    filterWhere,
    joinClauses,
  ...
  const { groupKeyFilter, groupKeyFilterParams } = buildArrayJoinGroupKeyFilter(
    { input, groupByColumn },
  );
...
/** The current/previous column expressions and FROM clauses for one branch of buildGroupByUnionAllQuery. */
interface GroupByUnionAllSource {
  currentCols: string[];
  previousCols: string[];
  currentFrom: string;
  previousFrom: string;
...
function buildSimpleOnlyUnionSource(
  simpleAliases: string[],
): GroupByUnionAllSource {
  return {
    currentCols: simpleAliases,
    previousCols: simpleAliases,
...
function buildSingleSubqueryUnionSource(
  subqueryMetrics: MetricTranslation[],
  subqueryAliasExprs: string[],
): GroupByUnionAllSource {
  ...
  return {
    currentCols: [singleAliasExpr],
    previousCols: [singleAliasExpr],
...
function buildMixedUnionSource({
  simpleMetrics,
  simpleAliases,
  subqueryMetrics,
}: {
  ...
  return {
    currentCols: allCurrentCols,
    previousCols: allPreviousCols,
...
function buildGroupByUnionAllQuery({
  ...
  const source: GroupByUnionAllSource =
    simpleMetrics.length > 0 && subqueryMetrics.length === 0
      ? buildSimpleOnlyUnionSource(simpleAliases)
      : simpleMetrics.length === 0 && subqueryMetrics.length === 1
        ? buildSingleSubqueryUnionSource(subqueryMetrics, subqueryAliasExprs)
        : simpleMetrics.length > 0 || subqueryMetrics.length > 0
          ? buildMixedUnionSource({
              simpleMetrics,
              simpleAliases,
              subqueryMetrics,
            })
  ...
  const sql = `
    WITH
      ${ctes.join(",\n      ")}
    SELECT ${currentParts.join(", ")} ${source.currentFrom}
    UNION ALL
    SELECT ${previousParts.join(", ")} ${source.previousFrom}
  `;
  return {
    sql,
    params: {
...
/** Shared context for building one subquery metric's current/previous CTE pair. */
interface SubqueryMetricCteArgs {
  metric: MetricTranslation;
  subquery: NonNullable<MetricTranslation["subquery"]>;
  cteName: string;
  ts: string;
  traceColumns: readonly string[];
  joinClauses: string;
  filterWhere: string;
  groupByColumn: string | null;
  groupKeyInnerSelect: string;
  groupKeyInnerGroupBy: string;
}

/** 3-level aggregation (nested subquery), e.g. avg thread duration per user. */
function buildNestedSubqueryMetricCte({
  metric,
  subquery,
  cteName,
  ts,
  ...
  groupByColumn,
  groupKeyInnerSelect,
  groupKeyInnerGroupBy,
}: SubqueryMetricCteArgs): string[] {
...
function buildStandardSubqueryMetricCte({
  metric,
  subquery,
  cteName,
  ts,
...
function buildSubqueryMetricCtes({
  ...
  for (const metric of subqueryMetrics) {
    ...
    const cteArgs: SubqueryMetricCteArgs = {
      metric,
      subquery,
      cteName,
      ts,
      ...
      filterWhere,
      groupByColumn,
      groupKeyInnerSelect: groupKeyExpr ? `, ${groupKeyExpr}` : "",
      groupKeyInnerGroupBy: groupByColumn ? `, group_key` : "",
    };

    // Check if this is a nested subquery (3-level aggregation)
    ctes.push(
      ...(subquery.nestedSubquery
        ? buildNestedSubqueryMetricCte(cteArgs)
        : buildStandardSubqueryMetricCte(cteArgs)),
    );
  }
...
/**
 * Build the current/previous simple_metrics CTEs (only present when there
 * are simple metrics to aggregate).
 */
function buildSimpleMetricsCtes({
  simpleMetrics,
  groupKeyExpr,
  groupByColumn,
  ts,
  ...
  if (simpleMetrics.length === 0) return [];
...
/**
 * Build the current/previous SELECT expressions for the scalar-subquery
 * (no groupBy) path: simple metrics read from simple_metrics_current/previous,
 * subquery metrics read from their cte_<alias>_current/previous, both quoted
 * for aliases starting with a digit and wrapped via `scalarExpr`.
 */
function buildScalarSubquerySelectExprs({
  simpleMetrics,
  subqueryMetrics,
  scalarExpr,
}: {
  simpleMetrics: MetricTranslation[];
  subqueryMetrics: MetricTranslation[];
  scalarExpr: (subquery: string, alias: string, quotedAlias: string) => string;
}): { currentSelectExprs: string[]; previousSelectExprs: string[] } {
  ...
  return { currentSelectExprs, previousSelectExprs };
}
 ...
 */
function buildScalarSubqueryQuery({
  input,
  ctes,
  simpleMetrics,
  subqueryMetrics,
  ...
  const { currentSelectExprs, previousSelectExprs } =
    buildScalarSubquerySelectExprs({
      simpleMetrics,
      subqueryMetrics,
      scalarExpr,
    });
...
function buildSubqueryTimeseriesQuery({
  ...
  return buildScalarSubqueryQuery({
    input,
    ctes,
    simpleMetrics,
    subqueryMetrics,
 ...
 */
/**
 * Collect the joins needed by a date-bucketed simple-metrics CTE: only the
 * joins simple metrics, the groupBy column, and filters require — not
 * pipeline metrics.
 */
function collectDateBucketedSimpleJoins({
  input,
  simpleMetrics,
}: {
  input: TimeseriesQueryInput;
 ...
 * when mixed with pipeline metrics on numeric timeScale. Quote aliases that
 * start with digits for ClickHouse compatibility. Use only the joins
 * required by simple metrics (+ groupBy + filters) to avoid fan-out
 * inflation from evaluation_runs or stored_spans joins that are only
 * needed by pipeline metrics. Returns null when there are no simple metrics.
 */
function buildDateBucketedSimpleMetricsCte({
  input,
  simpleMetrics,
  ts,
  periodCase,
  ...
  const simpleJoins = collectDateBucketedSimpleJoins({ input, simpleMetrics });
...
/**
 * Build the final SELECT joining all CTEs on (period, date[, group_key]).
 */
function buildDateBucketedFinalSelect({
  groupByColumn,
  pipelineMetrics,
  hasSimple,
  simpleMetrics,
...
function buildDateBucketedPipelineQuery({
  ...
  const simpleMetricsCte = buildDateBucketedSimpleMetricsCte({
    input,
    simpleMetrics,
    ts,
    periodCase,
  ...
  if (simpleMetricsCte) {
    ctes.push(simpleMetricsCte);
  }

  const finalSelect = buildDateBucketedFinalSelect({
    groupByColumn,
    pipelineMetrics,
    hasSimple,
    simpleMetrics,
...
/** Precomputed shared bits for either pipeline-metric CTE shape (nested or standard). */
interface PipelineCTEComputed {
  subquery: NonNullable<MetricTranslation["subquery"]>;
  cteName: string;
  hasGroup: boolean;
  groupPrefix: string;
  quotedAlias: string;
  outerAggExpr: string;
  outerGroupByCols: string[];
  outerHaving: string;
  baseSelectCols: string[];
  baseFrom: string;
}

/** 3-level aggregation (e.g., threads per user). */
function buildNestedPipelineMetricCte({
  subquery,
  cteName,
  hasGroup,
  groupPrefix,
...
/** Standard 2-level aggregation. */
function buildStandardPipelineMetricCte({
  subquery,
  cteName,
  hasGroup,
  groupPrefix,
...
function buildPipelineMetricCTE(
  ...
  const computed: PipelineCTEComputed = {
    subquery,
    cteName,
    hasGroup,
    groupPrefix,
  ...
  return subquery.nestedSubquery
    ? buildNestedPipelineMetricCte(computed)
    : buildStandardPipelineMetricCte(computed);
}
...
/** count() / count(*) and uniq(TraceId) / uniqExact(TraceId) -> uniqExact(trace_id). */
function dedupCountLikeRewrite(
  selectExpression: string,
  alias: string,
): string | null {
 ...
 * Returns null when no substitution applied.
 */
function dedupTraceColumnRewrite(selectExpression: string): string | null {
  ...
  if (rewritten === selectExpression) return null;
 ...
 * Returns null when the expression references no evaluation columns.
 */
function dedupEvalColumnRewrite(selectExpression: string): string | null {
  const es = tableAliases.evaluation_runs;
  const referencedEvalCols = extractReferencedEvaluationColumns([
    selectExpression,
  ]);
  if (referencedEvalCols.size === 0) return null;
 ...
 */
function dedupEventColumnRewrite(
  selectExpression: string,
  alias: string,
): string | null {
...
function transformMetricForDedup(
  ...
  return (
    dedupCountLikeRewrite(selectExpression, alias) ??
    dedupTraceColumnRewrite(selectExpression) ??
    dedupEvalColumnRewrite(selectExpression) ??
    dedupEventColumnRewrite(selectExpression, alias) ??
    // Default: return as-is (may need extension for other metric types)
    selectExpression
  );
}

/** Shared context every dataForFilter per-field query handler builds its SQL from. */
interface DataForFilterQueryContext {
  ts: string;
  ss: string;
  es: string;
  field: FilterField;
...
function buildTopicsTopicsFilterQuery({
  ts,
  filterJoins,
  filterWhere,
  searchQuery,
}: DataForFilterQueryContext): string {
  return `
        SELECT
          ${ts}.TopicId AS field,
          ${ts}.TopicId AS label,
...
function buildTopicsSubtopicsFilterQuery({
  ts,
  filterJoins,
  filterWhere,
  searchQuery,
...
function buildMetadataUserIdFilterQuery({
  ts,
  filterJoins,
  filterWhere,
  searchQuery,
...
function buildMetadataThreadIdFilterQuery({
  ts,
  filterJoins,
  filterWhere,
  searchQuery,
...
function buildSpansModelFilterQuery({
  ts,
  ss,
  filterJoins,
  filterWhere,
...
function buildSpansTypeFilterQuery({
  ts,
  ss,
  filterJoins,
  filterWhere,
...
/** Handles both "evaluations.evaluator_id" and "...guardrails_only" (extra IsGuardrail predicate). */
function buildEvaluatorIdFilterQuery({
  ts,
  es,
  field,
  filterJoins,
...
function buildTracesErrorFilterQuery({
  ts,
  filterJoins,
  filterWhere,
}: DataForFilterQueryContext): string {
...
/**
 * Registry of dataForFilter query builders by field. Fields not listed here
 * (default case) fall back to the empty-result query in buildDataForFilterQuery.
 */
const dataForFilterHandlers: Partial<
  Record<FilterField, (ctx: DataForFilterQueryContext) => string>
> = {
  "topics.topics": buildTopicsTopicsFilterQuery,
  "topics.subtopics": buildTopicsSubtopicsFilterQuery,
  "metadata.user_id": buildMetadataUserIdFilterQuery,
  "metadata.thread_id": buildMetadataThreadIdFilterQuery,
  "spans.model": buildSpansModelFilterQuery,
  "spans.type": buildSpansTypeFilterQuery,
  "evaluations.evaluator_id": buildEvaluatorIdFilterQuery,
  "evaluations.evaluator_id.guardrails_only": buildEvaluatorIdFilterQuery,
  "traces.error": buildTracesErrorFilterQuery,
};
...
export function buildDataForFilterQuery({
  ...
  // Build query based on field type; fields with no registered handler fall
  // back to the empty-result query.
  const handler = dataForFilterHandlers[field];
  const sql = handler
    ? handler({ ts, ss, es, field, filterJoins, filterWhere, searchQuery })
    : `SELECT '' AS field, '' AS label, 0 AS count WHERE 1=0`;
