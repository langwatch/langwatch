/**
 * The catalog is opt-in: a ClickHouse table or Prisma model is queryable only
 * because `LWQL_VIEW_CATALOG` lists it, never because the manifest carries it.
 *
 * This is the negative guard the opt-out coverage tests
 * (`tenantTableCoverage`/`tenantModelCoverage`) were replaced by: it proves that
 * a table/model absent from the explicit list produces no view and is granted
 * nothing, and it pins the whole 129-view list so any add or removal is a
 * deliberate, reviewed edit.
 *
 * @see specs/lwql/catalog-inclusion.feature
 * @see ../lwqlViews.ts — the explicit catalog under test
 */
...
import { lwqlAllowedTables } from "../types";

const allowed = new Set(
  lwqlAllowedTables({ database: "analytics", views: LWQL_VIEW_CATALOG }),
);
...
describe("the LangWatchQL catalog is opt-in", () => {
  ...
  describe("when a Prisma model is not listed", () => {
    ...
    it("has no view and is granted nothing", () => {
      ...
      const baseRelations = new Set(
        LWQL_VIEW_CATALOG.map((view) => view.postgres?.baseRelation).filter(
          Boolean,
        ),
      );
    ...
    it("stays off the catalog even when the manifest carries it", () => {
      ...
      // ...yet nothing in the shipped catalog reads it, and the count is fixed.
      const baseRelations = new Set(
        LWQL_VIEW_CATALOG.map((view) => view.postgres?.baseRelation),
      );
  ...
  describe("when the catalogued view names are pinned", () => {
    ...
    const PINNED_VIEWS: readonly (readonly [string, string])[] = [
      ...
      [
        "gateway_budget_bucket_boundaries",
        "gateway_budget_bucket_boundaries_pg",
      ],
      ...
      [
        "github_branch_pull_request_checks",
        "github_branch_pull_request_checks_pg",
      ],
      ...
      [
        ...
        "langy_conversation_turn_projections_pg",
      ],
      ...
      [
        "topic_clustering_run_history_projections",
        "topic_clustering_run_history_projections_pg",
      ],
      [
        "topic_clustering_run_projections",
        "topic_clustering_run_projections_pg",
      ],
    ...
    it("equals the pinned list of 129 views, in order", () => {
      const actual = LWQL_VIEW_CATALOG.map(
        (view) => [view.name, view.sourceTable] as const,
      );
