...
function walkProjection({ value, node, frame, ctx }: FieldArgs): void {
  ...
  for (const element of value) {
    ...
    // An app function reaching the walk as a direct projection element is the
    // only position it is allowed in, so it is handled here rather than by the
    // ordinary function walk — which is also what lets `enterFunction` refuse
    // the name unconditionally wherever else it turns up.
    const appFunction = directAppFunctionCall(element);
    if (appFunction) {
      walkAppFunctionCall({
        node: element,
...
function admitAppFunctionCall({
  ...
  if (definition.kind === "eval" && !ctx.policy.instantEvalsEnabled) {
    return refuse(
      "APP_FUNCTION_GATED",
      `The function "${echoIdentifier(definition.name)}" is not available to you. Instant Evals are not switched on for this project; ask an administrator to enable them, or remove the call.`,
    );
  }
