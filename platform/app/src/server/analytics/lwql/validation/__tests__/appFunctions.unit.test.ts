/**
 * App functions through the real ClickHouse parser: where they are admitted,
 * and every position where they are not.
 *
 * The rule under test is a *correctness* rule, not a policy one, which is why
 * every refusal below is paired with a control. ClickHouse accepts a projection
 * UDF in `WHERE`, `GROUP BY`, `ORDER BY`, `HAVING`, a join condition, a CTE, a
 * subquery and inside a lambda, and answers with the raw key compared as if it
 * were the value — a wrong answer with no error. So a case that went green
 * because the query was unparseable, or the table wrong, or the column gated,
 * would be reporting a guard that is not there. Each one names the code it
 * expects and is followed by the same shape with the call in the projection,
 * asserted accepted.
 *
...
import { LWQL_APP_FUNCTION_CATALOG } from "../../appFunctions/catalog";
...
describe("given a statement that calls a LangWatchQL app function", () => {
  ...
  describe("when the call is an aliased element of the top-level SELECT list", () => {
    ...
    it("accepts any expression the policy already allows as the key", () => {
      ...
      expect(codesOf(result)).toEqual([]);
      expect(
        result.ok && result.appFunctions.map((call) => call.column),
      ).toEqual(["text"]);
    });
    ...
    it("records one entry per call, in projection order", () => {
      ...
      ).toEqual(["llm_messages", "trace_json"]);
    });
    ...
    it("records an empty plan for a statement that calls none", () => {
      const result = validate("SELECT TraceId FROM analytics.traces");
  ...
  describe("when the call sits anywhere but the top-level SELECT list", () => {
    ...
    it.each([
      ...
      [
        "a lambda body",
        "SELECT arrayMap(x -> llm_messages(x), ['a']) AS m FROM analytics.traces",
      ],
      [
  ...
  describe("when the call has no alias", () => {
    ...
    it("refuses it, because hydration finds the call by output column name", () => {
      const result = validate(
        "SELECT conversation(ConversationId) FROM analytics.trace_metrics",
      );
  ...
  describe("when the arguments do not match the one signature", () => {
    ...
    it.each([
      [
        "no arguments",
        "SELECT conversation() AS x FROM analytics.trace_metrics",
      ],
      [
        "too many arguments",
      ...
      [
        "a missing option",
        "SELECT llm_readable_trace(TraceId) AS x FROM analytics.traces",
      ],
      [
        "one option of two",
  ...
  describe("when the caller does not hold the permissions the function needs", () => {
    ...
    it("refuses every gated function for a caller holding nothing", () => {
      for (const definition of LWQL_APP_FUNCTION_CATALOG) {
        ...
        const result = validate(
          `SELECT ${definition.name}(${definition.parameters
            .map((parameter) =>
              parameter.role === "key"
                ? "TraceId"
            ...
            .join(", ")}) AS x FROM analytics.traces`,
          { heldPermissions: [] },
        );
    ...
    it("admits the ungated function for a caller holding nothing", () => {
      expect(
        codesOf(
          validate(
            "SELECT thread_traces(ConversationId) AS ids FROM analytics.trace_metrics",
            { heldPermissions: [] },
          ),
        ),
      ).toEqual([]);
  ...
  // -------------------------------------------------------------------------
  // The name is not in the ordinary allowlist
  // -------------------------------------------------------------------------

  describe("when an app function is spelled in another case", () => {
