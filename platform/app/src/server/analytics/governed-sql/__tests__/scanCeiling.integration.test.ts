...
describe("given the governed settings profile's scan ceilings", () => {
  let harness: GovernedClickHouseHarness;
  let executor: GovernedSqlExecutor;
  let database: string;
   ...
   */
  const provisionWith = async (limits: GovernedResourceLimits) => {
    await harness.applyAsAdmin(
      governedClickHouseSetupStatements({
        names: harness.names,
        password: harness.restrictedConnection().password,
        governedTables: harness.governedTables,
        limits,
      }),
    );
  };
  ...
  beforeAll(async () => {
    harness = await startGovernedClickHouse({ suite: "scanceiling" });
  ...
  describe("when the shipped ceilings are in force", () => {
    ...
    it("answers the query, over more rows than the tightened ceiling allows", async () => {
      ...
      const result = await runGovernedQuery();
