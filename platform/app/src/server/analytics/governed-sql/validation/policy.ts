/**
 * Governed analytics SQL — what a caller is allowed to name.
 *
...
/**
 * Databases no governed query may name, whatever the catalog says.
 *
 * The restricted database identity is granted `SELECT` on the governed objects
...
export interface GovernedSqlLimits {
  /**
 ...
 */
export const DEFAULT_GOVERNED_SQL_LIMITS: GovernedSqlLimits = {
  maxSubqueryDepth: 8,
  maxNodeDepth: 400,
};

/** What this caller may reference. */
export interface GovernedSqlPolicy {
  /**
   ...
   * sides are qualified with {@link GovernedSqlPolicy.defaultDatabase}. CTE
  ...
  /** Defaults to {@link DEFAULT_GOVERNED_SQL_LIMITS}. */
  readonly limits?: GovernedSqlLimits;
}

/** The policy in the form the walk compares against: lowercased and set-shaped. */
export interface ResolvedGovernedSqlPolicy {
...
export function resolveGovernedSqlPolicy(
  policy: GovernedSqlPolicy,
): ResolvedGovernedSqlPolicy {
  ...
  return {
    ...
    defaultDatabase,
    limits: policy.limits ?? DEFAULT_GOVERNED_SQL_LIMITS,
  };
}
