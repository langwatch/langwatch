/**
 * Governed analytics SQL — which functions a governed query may call.
 *
 ...
 * ## The admission rule
 *
 * **A function is listed because a question the governed schema exists to
 ...
 * governed views expose — and the comment on each says which.
 *
 ...
 * absent: no governed view exposes an `AggregateFunction` column, so no
 * governed question can need them.
 *
 ...
 * keeping the governed schema free of them. `now()` is the one deliberate
...
/**
 * Arrays, maps and tuples, because the governed views expose those types.
 *
...
/** Every name a governed query may call, lowercased. */
...
/**
 * Whether a governed query may call this function.
 *
 * @param name The name exactly as the parser reported it, in the caller's own
 *   spelling and case.
 */
export function isAllowedGovernedFunction(name: string): boolean {
 ...
 */
export function isGovernedAggregateFunction(name: string): boolean {
