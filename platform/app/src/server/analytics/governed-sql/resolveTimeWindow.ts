/**
 * Governed analytics SQL — what a surface may do with the reserved time-window
...
import {
  ...
  type GovernedSqlTimeWindow,
  type GovernedSqlTimeWindowParameter,
...
export interface GovernedSqlTimeWindowResolution {
  /**
...
function valueFor(
  name: GovernedSqlTimeWindowParameter,
  timeWindow: GovernedSqlTimeWindow,
): string {
  return formatGovernedDateTimeParameter(
 ...
 *
 * @throws {GovernedSqlReservedParameterTypeError} when a reserved name is
 ...
 * @throws {GovernedSqlReservedParameterSuppliedError} when the request carries a
 ...
 */
export function resolveGovernedTimeWindow({
  declared,
  parameters,
  timeWindow,
}: {
  /** Bound parameters the validated statement declares. */
  readonly declared: readonly GovernedSqlParameter[];
  /** Values the caller sent. */
  readonly parameters?: Readonly<Record<string, unknown>>;
  /** The period the surface is showing, when it has one. */
  readonly timeWindow?: GovernedSqlTimeWindow;
}): GovernedSqlTimeWindowResolution {
  const reserved = declared.filter((parameter) =>
    isGovernedSqlTimeWindowParameter(parameter.name),
  );

  const mistyped = reserved
    .filter((parameter) => !isGovernedSqlDateTimeParameterType(parameter.type))
    .map((parameter) => parameter.name)
    .sort();
  if (mistyped.length > 0) {
    throw new GovernedSqlReservedParameterTypeError(mistyped);
  }

  const supplied = Object.keys(parameters ?? {})
    .filter(isGovernedSqlTimeWindowParameter)
    .sort();
  if (supplied.length > 0) {
    throw new GovernedSqlReservedParameterSuppliedError(supplied);
  }
  ...
  const injected = Object.fromEntries(
    reserved.map((parameter) => [
      parameter.name,
      valueFor(parameter.name as GovernedSqlTimeWindowParameter, timeWindow),
    ]),
  );
