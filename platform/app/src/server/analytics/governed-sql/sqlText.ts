/**
 * Governed analytics SQL — the one home for SQL text safety.
 *
 * Every module under `governed-sql/` emits SQL as text. Neither ClickHouse nor
...
export function assertIdentifier(value: string, role: string): string {
  if (!SAFE_IDENTIFIER.test(value)) {
    throw new Error(
      `governed-sql: ${role} must match ${String(SAFE_IDENTIFIER)}, got "${value}"`,
    );
  }
  return value;
}
