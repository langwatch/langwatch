...
describe("Feature: SCIM entitlement is checked on every call", () => {
  ...
  let mockGetActivePlan: PlanProvider["getActivePlan"];
  let requestApp: App;

  beforeAll(async () => {
    mockGetActivePlan = vi.fn<PlanProvider["getActivePlan"]>();
    vi.mocked(mockGetActivePlan).mockResolvedValue(ENTERPRISE_TEST_PLAN);
    ...
    const scim = PostgresScimAdapter.create({
      database: prisma,
      writer: baseApp.authzGrants,
      users: baseApp.users,
      governance: baseApp.governance,
      entitlements: planProvider,
    }).build();
    requestApp = createTestApp({ planProvider, scim });
    ...
    const minted = await requestApp.scim.generateToken({
      organizationId,
  ...
  describe("given a SCIM token minted while the organization was on Enterprise", () => {
    ...
    it("serves the identity provider on Enterprise and refuses the same token once the plan lapses", async () => {
      ...
      const entitled = await app.request(
        "/api/scim/v2/Users",
      ...
      vi.mocked(mockGetActivePlan).mockResolvedValue({ ...FREE_PLAN });
