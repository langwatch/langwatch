/**
 * The recommendation against the committed catalog.
 *
 * The catalog is synced weekly, and the recommendation is derived from it
 * rather than typed, so these pins move whenever a newer main-tier model
 * lands. A pin that fails after a sync is either a release to accept here
 * or a grammar that stopped reading the provider's ids: check which before
 * updating the value.
 */
import { describe, expect, it } from "vitest";
import { recommendedChatModel, resolveLatestAlias } from "../latestAliases";
...
describe("given the committed model catalog", () => {
  ...
  describe("when the aliases are resolved", () => {
    ...
    it.each([
      ...
      "anthropic",
      "gemini",
    ])("resolves %s/latest to the recommendation", (provider) => {
      expect(resolveLatestAlias(`${provider}/latest`)).toBe(
        recommendedChatModel(provider),
      );
    });
