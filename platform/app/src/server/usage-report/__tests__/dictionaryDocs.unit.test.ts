/**
 * The docs page is the dictionary, rendered. A dictionary change that did
 * not regenerate the page fails here before it reaches a customer.
 *
  ...
  describe("when the committed page is compared with the dictionary", () => {
    ...
    it("matches byte for byte, or names the command that regenerates it", () => {
      const committed = readFileSync(
        path.join(repoRoot, USAGE_REPORT_DICTIONARY_DOC_PATH),
        "utf8",
      );
      expect(
        committed,
