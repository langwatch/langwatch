...
import { createLongPollApp } from "../long-poll.route";
...
async function startPod(podId: string): Promise<Pod> {
  ...
  const app = createLongPollApp({ transport: () => transport });
...
async function register(
  ...
  const response = await pod.app.request("/api/agents/connect/register", {
    method: "POST",
...
async function postFrames(
  ...
  const response = await pod.app.request("/api/agents/connect/frames", {
    method: "POST",
    headers: headers(projectApiKey, {
      "X-Agent-Instance-Token": instanceToken,
    }),
