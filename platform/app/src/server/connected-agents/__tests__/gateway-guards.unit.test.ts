...
import { CONNECT_PATH, ConnectGateway, readSdkFrame } from "../connect.gateway";
...
describe("readSdkFrame", () => {
  ...
  describe("when a result carries both an output and an error", () => {
    it("is an unreadable result with the refinement's message", () => {
      const read = readSdkFrame(
        resultFrame({ output: "ok", error: { code: "x", message: "y" } }),
      );
      expect(read).toMatchObject({
        kind: "unreadable_result",
        callId: "call_1",
  ...
  describe("when the frame names no call, or is not a result", () => {
    it("is dropped", () => {
      expect(readSdkFrame(resultFrame({ callId: "", output: {} }))).toEqual({
        kind: "dropped",
      });
      expect(
        readSdkFrame(JSON.stringify({ protocol: 1, type: "ack", callId: 7 })),
  ...
  describe("when the frame is well formed", () => {
    it("is the parsed frame", () => {
      expect(
        readSdkFrame(
          resultFrame({ output: { role: "assistant", content: "hi" } }),
        ),
      ).toMatchObject({
        kind: "frame",
        frame: { type: "result", callId: "call_1" },
      });
      expect(
        readSdkFrame(
          JSON.stringify({ protocol: 1, type: "ack", callId: "call_1" }),
        ),
