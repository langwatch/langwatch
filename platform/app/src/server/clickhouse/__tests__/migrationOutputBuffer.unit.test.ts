/**
 * The buffer a verbose migration run prints into.
 *
 * `spawnSync` kills a child that prints past `maxBuffer` and reports ENOBUFS,
 * which reads exactly like a migration that could not apply. These tests run
 * the real spawn rather than asserting on the option, because the option is
 * only worth anything if it actually carries output of that size.
 *
 * @see ../goose.ts
...
describe("given a verbose migration run", () => {
  ...
  describe("when the goose binary is absent", () => {
    ...
    it("names the install page", () => {
      const message = messageForSpawnError(
        "spawnSync goose-does-not-exist ENOENT",
      );
