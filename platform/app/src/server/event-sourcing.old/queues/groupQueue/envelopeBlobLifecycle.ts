 ...
 * the seams are exercisable without standing up the whole queue. See ADR-100.
 */
export class EnvelopeBlobLifecycle {
  ...
  async decode({
    ...
    if (tieredRef) {
      ...
      // refuse before fetching and let the missing-blob fail-safe run (ADR-100).
    ...
    await Promise.all(
      values.map(async (value) => {
        ...
        if (lease) {
          ...
          // tenant's blob on the fail-safe cleanup path (ADR-100).
