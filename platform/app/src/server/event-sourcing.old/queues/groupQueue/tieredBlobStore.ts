 ...
 * below it, in Redis. Derived from `COMMAND_INLINE_THRESHOLD` (ADR-099's
 ...
 * can't redirect a read across tenants (ADR-100).
 */
export type BlobRef =
 ...
 * mass-dropping every in-flight offloaded job (ADR-100).
 */
 ...
 * share a blob and a project purge is a delete-by-prefix. See ADR-100.
 *
...
export class TieredBlobStore {
  ...
  constructor(deps: {
    ...
    // Silent-fail-if-unset was the original ADR-100 issue Aryan flagged — this
  ...
  private async fetch(ref: BlobRef, refresh: boolean): Promise<Buffer | null> {
    ...
    // failure is transient (retry), never "missing" (ADR-100).
    let uri: string;
    try {
    ...
    try {
      ...
      // transient and must retry, not drop the job (ADR-100). Oversize is
