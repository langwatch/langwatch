...
// "Pre-ADR-080" below marks the staged-job-id-reuse migration by the number it
// shipped under; that ADR is now retired and folded into ADR-100, which is the
// current model.
describe.skipIf(!hasTestcontainers)(
  "GroupQueueProcessor — a staged job id is identity, not state (ADR-100)",
  () => {
    let redis: Redis;
     ...
     * "which intervals are currently armed", and the whole point of ADR-100's
    ...
    /**
     * Ticks every still-armed interval at the WORST moment ADR-100 describes:
