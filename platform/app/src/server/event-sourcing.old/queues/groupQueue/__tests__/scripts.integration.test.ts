...
import { encodeJobEnvelope, readJobSequence } from "../jobEnvelope";
import {
  type DispatchResult,
  GROUP_KEY_TTL_MS,
...
describe("GroupStagingScripts", () => {
  describe("stage", () => {
    ...
    // an offloaded blob (ADR-100) the blob would leak until its 7-day TTL (the
    ...
    describe("given GQ2 offloaded values with blob leases", () => {
      ...
      // Since ADR-100 the staged job id IS the source event's id, so an
    ...
    describe("delivery sequence (ADR-098 §5)", () => {
      ...
      it("does not share a sequence counter across groups", async () => {
        const groupA = await scripts.stage(
          makeJob({ stagedJobId: "j1", groupId: "group-a" }),
        );
        const groupB = await scripts.stage(
          makeJob({ stagedJobId: "j1", groupId: "group-b" }),
        );

        // Each group's first job starts its OWN counter — a shared counter
        // would make group-b's first sequence 2, not 1.
        expect(groupA.sequence).toBe(1);
        expect(groupB.sequence).toBe(1);
      });

      /** @scenario A job cannot be staged without a sequence */
      it("always assigns a sequence — the caller has no way to omit one", async () => {
        const result = await scripts.stage(makeJob({ stagedJobId: "j1" }));

        expect(result.isNew).toBe(true);
        expect(Number.isInteger(result.sequence)).toBe(true);
        expect(result.sequence!).toBeGreaterThan(0);

        // The assignment is not just returned to the caller — it is ON the
        // stored value itself, exactly as a dispatched consumer would read
        // it (readJobSequence never touches Redis; this reads the stored
        // string byte for byte).
        const dispatched = await scripts.dispatch({
          nowMs: Date.now() + 10_000,
          activeTtlSec: 60,
        });
        expect(readJobSequence(dispatched!.jobDataJson)).toBe(result.sequence);
      });

      /** @scenario A GQ1 envelope's sequence survives a compressed, offloaded body */
      it("stamps the sequence into the header of an envelope-encoded job", async () => {
        vi.stubEnv("GROUP_QUEUE_ENVELOPE_WRITES_ENABLED", "true");
        try {
          // >1 KiB so the body is gzip+base64 (ADR-100) — the stamp must land
          // in the header without ever touching (or needing to decompress)
          // the body.
          const jobDataJson = await encodeJobEnvelope({
            jobData: { bulk: "x".repeat(4096) },
          });

          const result = await scripts.stage(
            makeJob({ stagedJobId: "j1", jobDataJson }),
          );

          const stored = await inspectDataHash("group-a");
          expect(readJobSequence(stored.j1)).toBe(result.sequence);
        } finally {
          vi.unstubAllEnvs();
        }
      });
      ...
      it("preserves the original sequence when a dedup squash replaces an unconsumed job", async () => {
        const first = await scripts.stage(
          makeJob({
            stagedJobId: "j1",
            dedupId: "d1",
        ...
        // A second stage under the same dedup id, before j1 dispatches,
        // squashes into the SAME staged job — it must not mint a new
        // sequence for what is still, logically, one delivery slot.
        const second = await scripts.stage(
          makeJob({
            stagedJobId: "j2",
            dedupId: "d1",
        ...
        const stored = await inspectDataHash("group-a");
        expect(readJobSequence(stored.j1)).toBe(first.sequence);
      });
    });
  });
  ...
  describe("refreshActiveKey", () => {
    ...
    describe("when a retry re-stages a job", () => {
      // The group's retry chain and the re-stage are ONE script (ADR-100).
      ...
      it("carries the original delivery sequence forward across a retry", async () => {
        const staged = await scripts.stage(
          makeJob({
            stagedJobId: "j1",
            dispatchAfterMs: 100,
            jobDataJson: JSON.stringify({ hello: "world" }),
        ...
        const originalSequence = readJobSequence(dispatched!.jobDataJson);
        expect(originalSequence).toBe(staged.sequence);

        // Mirrors what GroupQueueProcessor's retry rebuild does: re-encode the
        // payload with a bumped __attempt, threading the SAME __sequence
        // through unchanged (RETRY_RESTAGE_LUA never re-derives it — it is a
        // verbatim HSET of whatever the caller hands it).
        ...
        const redelivered = await scripts.dispatch({
          nowMs: 10_000,
          activeTtlSec: 300,
        });
        expect(readJobSequence(redelivered!.jobDataJson)).toBe(
          originalSequence,
        );
      });
      ...
      it("advances neither the chain nor the staging set when it does not own the slot", async () => {
        ...
        // writes nothing. Pre-ADR-080 (now retired, folded into ADR-100) the
        // attempt was written by the CALLER before this call, so it advanced
        // even when the re-stage did not — shortening the next job's budget
        // for a job that was never staged.
  ...
  describe("dispatch", () => {
    ...
    // ADR-100: staged values are GQ1 envelopes whose routing fields live in a
    ...
    // ADR-100: GQ2 envelopes carry the same p/t/n routing header as GQ1. The
...
describe("GroupStagingScripts.drainGroupReady", () => {
  ...
  // ADR-099: the drain is bounded by bytes as well as count. Each
