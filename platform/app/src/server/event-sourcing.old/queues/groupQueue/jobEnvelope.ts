 ...
 * already caps both codecs' output at the encode ceiling (ADR-100) so a
 ...
 * carries.
 *
 * - **GQ1** (ADR-100): offloads bodies > 32 KiB to a `randomUUID()` Redis key
 *   via {@link JobBlobStore} (`e:"ref"`, header `r`). No content identity.
 * - **GQ2** (ADR-100): offloads bodies > 4 KiB to a **content-addressed,
 ...
 * rather than inlining N× (ADR-100).
 */
const INLINE_CEILING_BYTES = 4 * 1024;

/**
 * Phase gate for the format rollout (ADR-100). Readers are always
...
export interface EnvelopeHeader {
  ...
  n?: string;
  /**
   * Per-group monotonic delivery sequence (ADR-098 §5), assigned by the Lua
   * staging script — never by this encoder. Lives in a dedicated header field
   * rather than `m`: the header is always present and inline regardless of
   * whether the body is offloaded or compressed, which is what lets the
   * staging eval stamp it without ever needing to touch (or be able to
   * reach) the body. A retry that rebuilds the envelope through the normal
   * encode path instead carries the sequence forward as an ordinary `__*`
   * body/machinery field (see {@link readJobSequence}, which checks both).
   */
  sq?: number;
  /**
   ...
   * the payload alone, so the same event fanned out to N subscribers collapses
   * to one stored blob (ADR-100). Allowlist-free: any future `__*` field is
 ...
 * `__attempt`, `__context`) or per-consumer (`__jobName`, `__jobType`,
...
/**
 * Folds `header.sq` onto the decoded body as `__sequence`, when present and
 * the body doesn't already carry one.
 *
 * The Lua staging script stamps the ORIGINAL delivery's sequence into this
 * dedicated header field (jobEnvelope never writes it — see
 * {@link EnvelopeHeader.sq}), reachable regardless of whether the body ended
 * up inline or offloaded. A retry that rebuilds the envelope through the
 * normal encode path instead carries `__sequence` forward as an ordinary
 * `__*` field, which lands in the body (GQ1/legacy) or `header.m` (GQ2,
 * already folded in by {@link mergeMachinery}) — so the `undefined` guard
 * here is what lets a retried job's own sequence win over a stale `header.sq`
 * a squash may have left behind on an earlier delivery of the same slot.
 */
function withSequenceFallback(
  body: Record<string, unknown>,
  header: EnvelopeHeader,
): Record<string, unknown> {
  if (typeof header.sq === "number" && body.__sequence === undefined) {
    return { ...body, __sequence: header.sq };
  }
  return body;
}
...
/** Guards the payload-size ceiling (ADR-100). Emits a tenant-attributed warn before rejecting. */
...
export async function encodeJobEnvelope({
  ...
  if (enabled && tieredBlobs && projectId) {
    ...
    // (ADR-100). The body now contains only the user payload.
    ...
    if (payloadBytes > INLINE_CEILING_BYTES) {
      ...
      const ref = await tieredBlobs.put({
        ...
        // not depend on gzip/zstd determinism (library version/level; ADR-100).
        // The codec is folded in so a JSON-encoded and a msgpack-encoded
...
export async function decodeJobEnvelope({
  ...
  if (header.e === "redis" || header.e === "s3") {
    ...
    return withSequenceFallback(mergeMachinery(parsedBody, header), header);
  }
  ...
  if (header.e === "ref") {
    ...
    return withSequenceFallback(await decodeBody(data), header);
  }
  ...
  const merged =
    header.v === 2 ? mergeMachinery(parsedBody, header) : parsedBody;
  return withSequenceFallback(merged, header);
}
...
/**
 * Reads the per-group monotonic delivery sequence (ADR-098 §5) from the
 * header alone (envelope values) or via a full parse (legacy bare JSON).
 * Never throws; an absent or unreadable sequence is null.
 *
 * Unlike {@link readJobAttempt}, this checks `header.sq` FIRST regardless of
 * envelope version: the Lua staging script stamps the sequence there for
 * both GQ1 and GQ2 (it is a dedicated field, not lifted machinery — see
 * {@link EnvelopeHeader.sq}), because the header is always inline and
 * reachable even when the body is offloaded or compressed. `header.m` is
 * only checked as a fallback, for a value that reached this reader after a
 * retry rebuilt it through the normal encode path (which puts `__sequence`
 * back with the rest of GQ2's lifted machinery).
 */
export function readJobSequence(value: string): number | null {
  try {
    if (isEnvelope(value)) {
      const { header } = splitEnvelope(value);
      if (
        typeof header.sq === "number" &&
        Number.isInteger(header.sq) &&
        header.sq > 0
      ) {
        return header.sq;
      }
      const machinery = (header.m ?? {}) as Record<string, unknown>;
      const seq = machinery.__sequence;
      return typeof seq === "number" && Number.isInteger(seq) && seq > 0
        ? seq
        : null;
    }
    const parsed = JSON.parse(value) as Record<string, unknown>;
    const seq = parsed.__sequence;
    return typeof seq === "number" && Number.isInteger(seq) && seq > 0
      ? seq
      : null;
  } catch {
 ...
 */
function readEnvelopeBlobIdFromHeader(header: EnvelopeHeader): string | null {
 ...
 * Validate the ref; use the lease only for renewal (ADR-100).
 */
