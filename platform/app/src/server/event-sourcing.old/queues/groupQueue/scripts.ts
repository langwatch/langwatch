...
// Per-group monotonic delivery sequence (ADR-098 §5). Stamped onto an
// already-serialized staged value, in the SAME atomic eval that inserts the
// job — a job cannot exist without a sequence or share one with a sibling,
// and it is stable across a retry (RETRY_RESTAGE_LUA re-stages a JS-rebuilt
// value that already carries the same number forward, exactly as it does
// __attempt; no Lua stamping happens there). That is what lets a fold's
// dedup tell a redelivery from a genuinely new one: same sequence -> skip.
//
// Unlike the tenant in-flight ZSET this replaced a scalar INCR/DECR pair for
// (see the comment above TENANT_ACTIVE_HELPER_LUA, formerly at scripts.ts:88):
// that counter needed a compensating decrement and leaked UP when a worker
// died before it could fire. This one has no decrement to miss — assigned
// once per group member, monotonically increasing, never decremented — so an
// ungraceful death has nothing to strand.
//
// gqStampSequence writes by STRING SURGERY, never cjson.encode: this file
// only ever cjson.decodes (see gqParseLease, gqRoutingMeta below) because
// cjson.decode + cjson.encode round-trips the ENTIRE object through Lua's
// JSON library, which cannot always tell an empty array from an empty object
// on re-encode (`[]` and `{}` decode to the same empty Lua table) — silent
// corruption of a field this staging path has no business touching, for
// bytes that came from arbitrary user payloads (the legacy bare-JSON path)
// or job context metadata (GQ1/GQ2 headers). A header or a bare-JSON job
// object is a well-formed JSON object produced by JSON.stringify, so it is
// KNOWN to end in `}`; splicing one more key in before that closing brace is
// exact and touches nothing else in the value.
//
// Envelope values (GQ1/GQ2) get the sequence in a dedicated header field
// ("sq"), never inside the body: the body may be gzip-compressed or offloaded
// to a blob the eval cannot reach, but the header is always present, always
// inline, regardless of body encoding. Legacy bare JSON (envelope writes
// disabled — the default today) has no header at all, so the sequence is
// spliced directly onto the job object as "__sequence", exactly where a
// JS-rebuilt retry envelope would carry it forward too (jobEnvelope.ts reads
// both locations — see readJobSequence/decodeJobEnvelope).
const GQ_SEQUENCE_HELPER_LUA = `
local function gqStampSequence(jobDataJson, seq)
  local prefix = string.sub(jobDataJson, 1, 4)
  if prefix == "GQ1|" or prefix == "GQ2|" then
    local barIdx = string.find(jobDataJson, "|", 5, true)
    if not barIdx then return jobDataJson end
    local headerLen = tonumber(string.sub(jobDataJson, 5, barIdx - 1))
    if not headerLen or headerLen <= 0 then return jobDataJson end
    local headerJson = string.sub(jobDataJson, barIdx + 1, barIdx + headerLen)
    local body = string.sub(jobDataJson, barIdx + headerLen + 1)
    local newHeaderJson = string.sub(headerJson, 1, -2) .. ',"sq":' .. seq .. '}'
    return prefix .. #newHeaderJson .. "|" .. newHeaderJson .. body
  end
  return string.sub(jobDataJson, 1, -2) .. ',"__sequence":' .. seq .. '}'
end

-- Read-only twin of gqStampSequence: the sequence a stored value ALREADY
-- carries, or nil. Used to preserve the original assignment when an
-- unconsumed staged job is overwritten in place (a dedup squash, or an
-- at-least-once re-send landing on the same stagedJobId) — that job has not
-- been dispatched yet, so nothing has consumed a sequence for it, and
-- reassigning one on every overwrite would both waste the counter and let a
-- job's sequence change while it sits in staging.
local function gqReadSequence(value)
  ...
  if prefix == "GQ1|" or prefix == "GQ2|" then
    ...
    if ok and type(header) == "table" and type(header["sq"]) == "number" then
      return header["sq"]
    end
    return nil
  end
  local ok, data = pcall(cjson.decode, value)
  if ok and type(data) == "table" and type(data["__sequence"]) == "number" then
...
// value. Envelope values (ADR-100, "GQ<v>|<headerLen>|<headerJson><body>")
...
-- ARGV[12] — appended after shouldSurviveDispatch so no existing index
-- shifts. Needed to derive this group's sequence key (gqStampSequence/
-- gqReadSequence never touch Redis themselves; only the per-group INCR does).
local keyPrefix = ARGV[12]
local seqKey = keyPrefix .. "group:" .. groupId .. ":seq"
...
if dedupId ~= "" and dedupTtlMs > 0 then
  ...
  if existingJobId then
    ...
    if rank then
      ...
      local squashedSeq = nil
      if shouldReplace == 1 then
        orphanedValue = redis.call("HGET", dataKey, existingJobId) or ""
        -- existingJobId is not yet dispatched (it is still a groupJobsKey
        -- member — that is what "rank" found above), so no delivery has
        -- consumed a sequence for it. Carry its ALREADY-assigned sequence
        -- into the replacement rather than minting a new one: see
        -- gqReadSequence.
        squashedSeq = gqReadSequence(orphanedValue)
        if squashedSeq == nil then
          squashedSeq = redis.call("INCR", seqKey)
        end
        local stampedJobDataJson = gqStampSequence(jobDataJson, squashedSeq)
        redis.call("HSET", dataKey, existingJobId, stampedJobDataJson)
        gqSquashTransferLeases(parkKeyPrefixOf(readyKey), parkTenantOf(groupId), stampedJobDataJson, orphanedValue)
      else
      ...
      return {0, orphanedValue, squashedSeq}
    end
...
-- The staged value may DISPLACE one. Since ADR-100 the staged job id is the
...
if inserted == 0 then
  ...
  -- Same job id, still staged (never dispatched — dispatch/drain remove the
  -- data field). Nothing has consumed a sequence for it yet: carry the
  -- ALREADY-assigned one forward instead of minting a new one.
  seq = gqReadSequence(displacedValue)
end
if seq == nil then
  -- Assigned inside this atomic eval, in the SAME script that inserts the
  -- job (ADR-098 §5) — a job cannot exist without one or share one with a
  -- sibling. Monotone per group only: seqKey is scoped by groupId, so two
  -- different groups never contend on the same INCR.
  seq = redis.call("INCR", seqKey)
end
jobDataJson = gqStampSequence(jobDataJson, seq)
...
-- Per-job (index-aligned) assigned sequence; 0 for an index that staged
-- nothing (the post-dispatch squash — see squashedPostDispatch below). Never
-- left as a Lua nil: a table with a nil hole truncates the RESP array on the
-- way out, silently dropping every index after it.
local sequences = {}
...
for i = 1, count do
  ...
  local orphanedValue = ""
  -- nil until a branch below resolves it (existing, preserved, or freshly
  -- assigned). Converted to the 0 "nothing staged" sentinel only when written
  -- into 'sequences' below — Lua nil itself cannot go into that table (a nil
  -- hole truncates the RESP array conversion) but must stay nil here so
  -- "still unresolved" and "resolved to 0" can never be confused mid-branch.
  local seq = nil
  ...
  if dedupId ~= "" and dedupTtlMs > 0 then
    ...
    if existingJobId then
      ...
      if rank then
        ...
        if shouldReplace == 1 then
          orphanedValue = redis.call("HGET", dataKey, existingJobId) or ""
          -- existingJobId is still staged (rank found above, never
          -- dispatched), so carry its already-assigned sequence forward
          -- instead of minting a new one — see STAGE_LUA's squash branch.
          seq = gqReadSequence(orphanedValue)
          if seq == nil then
            seq = redis.call("INCR", seqKey)
          end
          local stampedJobDataJson = gqStampSequence(jobDataJson, seq)
          redis.call("HSET", dataKey, existingJobId, stampedJobDataJson)
          gqSquashTransferLeases(keyPrefix, parkTenantOf(groupId), stampedJobDataJson, orphanedValue)
        else
  ...
  if not isDeduped then
    ...
    if seq == nil then
      -- Assigned inside this atomic eval, in the SAME script that inserts
      -- the job (ADR-098 §5), scoped by groupId so two groups never contend
      -- on the same INCR.
      seq = redis.call("INCR", seqKey)
    end
    jobDataJson = gqStampSequence(jobDataJson, seq)
...
return {newStagedCount, orphanedValues, sequences}
`;
 ...
 *
 * Two bounds, whichever binds first (ADR-099):
 ...
 *     the pre-ADR-099 behaviour). Sizes are the stored envelope's `#value`,
...
-- 2. Re-stage the failed job under the id it was dispatched under (ADR-100).
...
-- (ADR-100) a duplicate delivery can have re-staged this same id while the job
...
-- with the staged job id no longer carrying a /r/<n> marker (ADR-100), this
...
-- id (ADR-100) while the job was in flight. Release what this HSET displaces,
...
--    naming the retired id no longer matched. Since ADR-100 the id is reused,
...
export class GroupStagingScripts {
   ...
   *   `orphanedValue`, and the `sequence` this staged job now carries — the
   *   per-group monotonic delivery sequence assigned inside the same atomic
   *   eval (ADR-098 §5). A squash that reuses an existing, not-yet-dispatched
   *   job reports that job's already-assigned sequence, never a fresh one.
   */
  async stage({
    stagedJobId,
    groupId,
  ...
  }): Promise<{ isNew: boolean; orphanedValue: string; sequence: number | null }> {
    ...
    const result = await stageScript.run(
      ...
      // ARGV[12] — appended after shouldSurviveDispatch, same reason. Lets
      // the Lua derive this group's sequence key without a 9th KEYS entry.
      this.keyPrefix,
    );

    // STAGE_LUA returns [code, orphanedValue, sequence]. Tolerate a bare
    // integer reply defensively (e.g. a stale cached script during a rolling
    // deploy).
    const [code, orphanedValue, sequence] = Array.isArray(result)
      ? (result as [number, unknown, unknown])
      : [result as number, "", null];
    return {
      isNew: Number(code) === 1,
   ...
   *
   * @returns `newStagedCount` (new jobs, excluding deduped), the
   *   index-aligned `orphanedValues` of squashed jobs (`""` where nothing was
   *   displaced), and the index-aligned `sequences` each job now carries (0
   *   for an index that staged nothing — see {@link stage}). Leases move
   *   inside the eval — see {@link stage}.
   */
  async stageBatch(
    jobs: Array<{
      stagedJobId: string;
    ...
    if (jobs.length === 0)
      return { newStagedCount: 0, orphanedValues: [], sequences: [] };
    ...
    // STAGE_BATCH_LUA returns [newStagedCount, orphanedValues[], sequences[]].
    // Tolerate a bare integer reply defensively (stale cached script mid-deploy).
    if (Array.isArray(result)) {
      const [count, orphans, seqs] = result as [number, unknown, unknown];
      return {
        newStagedCount: Number(count),
        orphanedValues: Array.isArray(orphans)
          ? orphans.map((v) => (v == null ? "" : String(v)))
          : [],
        sequences: Array.isArray(seqs) ? seqs.map((v) => Number(v)) : [],
      };
    }
    return { newStagedCount: Number(result), orphanedValues: [], sequences: [] };
  }
   ...
   * job in staging for its own later dispatch (ADR-099). `maxBytes <= 0`
