...
const metricNames = [
  ...
  // ADR-100 hardening + review 2026-06-24
  ...
  "gq_blob_sweep_total",
  // ADR-099 mixed-command isolation
...
// --- Blob lifecycle observability (ADR-100 hardening + review 2026-06-24) ---
...
/** Producer rejected a payload at the encode cap — bounds worker memory (ADR-100). */
 ...
 * dispatched job's (ADR-099 mixed-command isolation).
 *
...
export const gqForeignSiblingsRestagedTotal = new Counter({
  ...
  help: "Drained siblings restaged untouched because their __jobName differed from the dispatched job (ADR-099 mixed-command isolation) — excludes the batch-failure restage paths",
  labelNames: ["queue_name"] as const,
});
