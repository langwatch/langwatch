 ...
 * supplies no `coalesceMaxBytes` resolver (ADR-099).
 *
 ...
 * is generous enough that the fold/map batches that predate ADR-099 stay bounded
 ...
 * clobber on decode (because the strip is allowlist-free; ADR-100). Reject at
  ...
  implements EventSourcedQueueProcessor<Payload>
{
  ...
  async send(
    ...
    if (isNew) {
      ...
      // Audit hook (ADR-098): only on the new-stage path, not on
  ...
  async sendBatch(
    ...
    // Audit hooks (ADR-098). The Lua's stageBatch returns a count,
   ...
   * the queue stays available. See ADR-098 for the "audit lags
  ...
  private async processClaimedJob(dispatched: DispatchResult): Promise<void> {
    ...
    // ADR-098 §5: the per-group monotonic delivery sequence, assigned once by
    // the Lua staging script and carried on the envelope (decodeJobEnvelope
    // folds header.sq onto jobData — see jobEnvelope.ts). null only for a job
    // staged before this field existed, or the degraded GQ1-offloaded-body
    // corner readJobSequence already tolerates for __attempt.
    const jobSequence =
      typeof jobData.__sequence === "number" ? jobData.__sequence : null;
    ...
    if (maxBatch > 1 && this.processBatch) {
      // Byte bound (ADR-099): the drain also stops before a job that
      ...
      // Mixed-command groups (ADR-099): with `serializeByAggregate` the
      ...
      if (drainedSiblings.length > 0) {
        try {
          ...
          if (err instanceof TransientBlobStoreError) {
            ...
            // failure — the body is unreachable, not gone (ADR-100).
            ...
            try {
              ...
              await runWithContext(requestContext, async () => {
                if (batchPayloads && this.processBatch) {
                  ...
                  // The dispatched job's own sequence — coalesced siblings
                  // drained alongside it are not individually tracked here.
              ...
              if (
                ...
                // The job keeps the id it was dispatched under (ADR-100). Its
                ...
                try {
                  retryJobData = await this.blobLifecycle.encode({
                    jobData: {
                      ...
                      __context: contextMetadata,
                      __attempt: attempt + 1,
                      // Carried forward UNCHANGED, never incremented: a
                      // retry is a redelivery of the SAME logical job, and
                      // dedup (ADR-098 §5) depends on it presenting the
                      // sequence it was first staged with. Omitted (not
                      // re-derived) when the original was unreadable, rather
                      // than minting a fresh one that would defeat dedup.
                      ...(jobSequence !== null
                        ? { __sequence: jobSequence }
                        : {}),
                    },
                    groupId,
                  });
  ...
  private async parseDrainedPayload({
    ...
    try {
      ...
      // `handleTransientDecode` so the whole batch can retry (ADR-100).
  ...
  private async handleExhaustedRetries({
    ...
    /** ADR-098 §5 delivery sequence, carried forward unchanged — see handleRetry. */
    ...
    // Re-stage under the id the job was dispatched under (ADR-100), so the
    ...
    const jobDataJson = await this.blobLifecycle.encode({
      jobData: {
        ...(payload as Record<string, unknown>),
        __context: contextMetadata,
        // A blocked job can be unblocked and redispatched later, which is
        // still the SAME delivery — see the retry rebuild above for why this
        // is never re-derived.
        ...(jobSequence !== null ? { __sequence: jobSequence } : {}),
      },
      groupId,
    });
  ...
  private async parkPoisonGroup({
    ...
    await this.scripts.restageAndBlock({
      groupId,
      // Parked under the id it was dispatched under (ADR-100). This used to
   ...
   * cannot read (ADR-100): the attempt on the message's header, the group's
