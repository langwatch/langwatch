 ...
 *
 * `sequence` is the per-group monotonic delivery identity ADR-098 §5 dedup is
 * built on: a projection records the last sequence it applied, and a
 * redelivered job — SAME sequence as the delivery already applied — is
 * skipped, regardless of `attempt`. Unlike `attempt` it is assigned once, at
 * stage time, by GroupQueue itself (never by the caller), and stays constant
 * across every redelivery of that job. `null` only for a job staged before
 * this field existed, or the narrow degraded envelope corner documented on
 * `readJobSequence` (jobEnvelope.ts).
 */
...
> {
   ...
   * batch (ADR-099). The drain stops before taking a same-group job
   ...
   * per ADR-098: the queue owns scheduling and execution, and
 ...
 * side-store. Used by the outbox dispatch queue (ADR-098).
 *
 * The queue invokes:
