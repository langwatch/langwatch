...
import { trimAttributesForAnalytics } from "~/server/event-sourcing.old/pipelines/trace-processing/projections/services/analytics-attribute-trim.service";
import {
...
/**
 * ADR-099 — slim per-evaluation fold projection.
 *
 ...
 *
 * Re-fold safety (ADR-099): same state → same canonical projection →
 ...
 *  change is exactly what this stamp records (ADR-099), and the store's
...
export function projectEvaluationAnalyticsStateToRow({
  ...
  return {
    ...
    // Read-back state (ADR-099) — the operands DurationMs is derived from.
