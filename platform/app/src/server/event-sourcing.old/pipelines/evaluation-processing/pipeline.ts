...
import { createEvaluationAlertTriggerMatchSubscriber } from "~/server/event-sourcing.old/pipelines/automations/subscribers/evaluationAlertTriggerMatch.subscriber";
...
export interface EvaluationProcessingPipelineDeps {
  ...
  /** ADR-099: slim per-evaluation fold writer (eval mirror of
   *  `traceAnalyticsStore`). */
  evaluationAnalyticsStore: FoldProjectionStore<EvaluationAnalyticsData>;
  /** ADR-099: per-evaluation rollup writer (eval mirror of
   ...
   * here is more honest than hiding them in the composition root
   * (ADR-102 "What does not move"). Typed as the command's own
   * published contract, so this file cannot drift from what it takes.
   */
  executeEvaluation: ExecuteEvaluationCommandDeps;
  /** ADR-102 — the cross-pipeline port the billing poke dispatches through. */
  commands: CommandBus;
  ...
  automations: {
     ...
     * (ADR-102). The subscriber that reads them is constructed below;
     * only the service it asks comes in as a dep.
     */
    triggers: TriggerService;
    /**
     * The committed trace summary, read by identity — the same narrow port the
     * evaluation trigger's dispatch takes (ADR-102). A function rather
...
export function createEvaluationProcessingPipeline(
  ...
  const builder = definePipeline<EvaluationProcessingEvent>()
    ...
    // (ADR-102). `recordTriggerMatch` is a command-bus port into the
