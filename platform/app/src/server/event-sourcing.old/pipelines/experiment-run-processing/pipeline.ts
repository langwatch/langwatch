...
/**
 * ADR-102 — the run-state store adapter and its cache tier are composed
...
export interface ExperimentRunProcessingPipelineDeps {
  ...
  /**
   * ADR-102 — the reaper's terminal write is a command on THIS pipeline.
   ...
   * Required-but-nullable (ADR-102) — where it is `null` runs have no
 ...
 * writes the terminal state itself. See ADR-103 and
 ...
 *
 * Process manager: experimentRunExecution (ADR-103)
