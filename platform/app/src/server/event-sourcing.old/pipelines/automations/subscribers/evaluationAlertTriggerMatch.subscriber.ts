...
import type { EvaluationProcessingEvent } from "~/server/event-sourcing.old/pipelines/evaluation-processing/schemas/events";
...
interface EvaluationAlertTriggerMatchDeps {
   ...
   * (ADR-102). Handing the whole `FoldProjectionStore` over would give
 ...
 * ADR-098): a terminal status, and the trace the outcome belongs to.
 *
 ...
 * (ADR-098): a plain event subscriber, with no fold bound to it.
 *
