...
/**
 * The store adapter's half of the durable dedup watermark (ADR-099): it threads
...
/**
 * The read-back gate (ADR-099). The columns of migrations 00053/00054 are only
...
describe("CodingAgentSessionStore read-back gate", () => {
  ...
  describe("given a pre-bump row whose read-back columns are populated", () => {
     ...
     * `event_log` history for no gain — the exact cost ADR-099 exists to remove,
