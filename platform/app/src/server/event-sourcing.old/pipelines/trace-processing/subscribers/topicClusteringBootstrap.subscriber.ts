...
export interface TopicClusteringBootstrapSubscriberDeps {
  ...
  /**
   * ADR-098: ensures the project's topic clustering process exists and has a
 ...
 * (ADR-098).
 *
 * **Split out of `projectMetadata` (ADR-098).** The two rode one handler
 * because the original bootstrap bolted onto whatever already fired on every
 ...
 *
 * **A subscriber, not a process manager.** ADR-098's split would suggest a
 * process manager here — deferred re-check, wake — alongside `originGate`. It is
 ...
 * that is missed is missed forever, which is why the original
 * first-message-transition bootstrap (ADR-098) needed a deploy-time backfill
 * to repair it. Asking on every ingest instead means a lost invocation is
 * re-asserted by the project's next trace.
 */
