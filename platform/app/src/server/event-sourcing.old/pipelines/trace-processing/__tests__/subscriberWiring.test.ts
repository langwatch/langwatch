...
import { GRAPH_TRIGGER_REAL_TIME_DEBOUNCE_MS } from "~/server/event-sourcing.old/pipelines/automations/subscribers/graphTriggerActivity.subscriber";
 ...
 * the composition root mounts what ADR-098/ADR-102 say it should — the three
...
describe("trace-processing pipeline wiring", () => {
  describe("given the deferred work the pipeline owns", () => {
    ...
    it("gives every one of them a gate before a job is staged", () => {
      ...
      // job, an inbox row and a durable transition per span (ADR-098). What
  ...
  describe("given the coding-agent span-facts dispatch", () => {
    it("mounts it from this pipeline, bound through the command bus", () => {
      ...
      // ADR-102: the port is bound while this pipeline is being built, so
