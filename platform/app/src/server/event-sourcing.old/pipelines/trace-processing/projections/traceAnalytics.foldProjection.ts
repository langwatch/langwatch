...
} from "~/server/event-sourcing.old/projections/abstractFoldProjection";
import type {
  FoldProjectionOptions,
  FoldProjectionStore,
} from "~/server/event-sourcing.old/projections/foldProjection.types";
...
/**
 * ADR-099 Phase 2: slim per-trace fold projection.
 *
 ...
 *
 * Storage anchor (ADR-099): `OccurredAt` is the partition key, the lead
 ...
 *
 * Re-fold safety (ADR-099): a re-fold produces the same canonical state,
 ...
 *
 * State continuity (ADR-099): the store reads its own last committed row back
 ...
 *  (ADR-099), and the store's read-back path uses it as the discriminator:
 ...
 *
 *  2026-07-29 — the storage anchor split (ADR-099, migration 00061).
 ...
 * previous version until TTL, and can cross a `toYearWeek` boundary — the
 * consequences ADR-099 names, reintroduced at population scale by the fix for them.
 *
...
/**
 * How far a trace's OccurredAt (the partition column, and since ADR-099
 ...
 * rows sharing the full sort key; the frozen anchor (ADR-099) keeps a
...
export interface TraceAnalyticsRow {
   ...
   *
   * Since ADR-099 this is `state.storageAnchorMs` — the first business
  ...
  // ── Read-back state (ADR-099, migration 00056) ─────────────────────────
...
export interface TraceAnalyticsData {
   ...
   * moved afterwards (ADR-099: a storage anchor is written once).
   *
 ...
 * business time (ADR-099). First-observed, never `min` and never `max`: the
 * anchor's whole job is to hold a row's partition, sort position and TTL
 * deadline still, and every consequence ADR-099 names comes from the column
...
export function projectAnalyticsStateToRow({
  ...
  return {
    ...
    version,
    // The anchor, not the timing baseline (ADR-099).
    //
    ...
    //
    // ADR-099 names `CreatedAt` as a trap for exactly this use, and it is right:
    // it is fold time, so a rebuild re-stamps it. That is accepted here
    ...
    // first write. What ADR-099 argues for instead (the event log's accept time
    // threaded into the row) is a further re-key this change does not take;
    ...
    // Read-back state (ADR-099) — round-trips the fold's working bookkeeping.
 ...
 * the `fromRow` inverse of {@link projectAnalyticsStateToRow} (ADR-099).
 *
...
function applyLogContribution({
  ...
  return {
    ...
    // without any of the above — ADR-099, landed.
  ...
  implements FoldEventHandlers<typeof traceAnalyticsEvents, TraceAnalyticsData>
{
  ...
  /**
   * The store reads its own last committed state back (ADR-099): the row now
   ...
   * the first contribution that carried a usable business time (ADR-099,
