...
/**
 * The slim fold's STORAGE ANCHOR (ADR-099, migration 00061).
 *
...
describe("traceAnalytics storage anchor", () => {
  ...
  describe("given an earlier-starting span that arrives after the trace was anchored", () => {
    ...
    it("leaves the anchor exactly where it froze", () => {
      // ADR-099's consequences of a moving anchor — orphaned versions,
      // cross-partition orphans, a TTL deadline that moves towards the row,
      // and a dedup scope that can miss the true latest — all come from this
      // value moving.
  ...
  describe("given a log record followed by the span whose work it describes", () => {
    ...
    it("keeps the anchor of whichever contribution was folded first", () => {
      // First-observed, not min: the anchor is a storage address, and ADR-099's
