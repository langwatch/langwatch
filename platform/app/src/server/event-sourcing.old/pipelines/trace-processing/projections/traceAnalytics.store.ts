...
/**
 * FoldProjectionStore adapter for the slim trace_analytics fold (ADR-099
 * Phase 2, read-back per ADR-099). Mirrors the trace-summary store's shape —
 ...
 *
 * Read-back (ADR-099): `get`/`getWithApplied` decode the last committed row
  ...
  implements FoldProjectionStore<TraceAnalyticsData>
{
   ...
   * applied-event-id watermark (ADR-099) — the CH-fallthrough behind the Redis
   ...
   *
   * The storage-anchor split (ADR-099, migration 00061) is the ONE stamp
 ...
 *
 * `storageAnchorMs` is deliberately NOT a third door, even though ADR-099
