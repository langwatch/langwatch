...
export interface RecordSpanCommandDependencies {
  ...
  /**
   * ADR-099: Optional BlobStore for spool fetch (when command carries spoolRef)
  ...
  implements CommandHandler<Command<RecordSpanCommandData>, SpanReceivedEvent>
{
   ...
   *   composition root for ADR-099 spool support) without having to also
  ...
  async handle(
    ...
    return await this.tracer.withActiveSpan(
      ...
      async () => {
        ...
        // ADR-099: Oversized command path — reconstitute span from S3 spool.
        ...
        // ADR-099: Guard — if the command carries a spoolRef but this handler
        ...
        if (commandData.spoolRef && !this.blobStore) {
          throw new Error(
            `ADR-099: command carries spoolRef "${commandData.spoolRef}" but this handler has no blobStore configured to reconstitute the span. Refusing to emit a span with cleared attributes (would be permanent data loss in event_log).`,
          );
        }
        ...
        if (commandData.spoolRef && this.blobStore) {
          ...
          // ADR-099: spool body is the full serialized RecordSpanCommandData.
        ...
        //
        // ADR-099: Skip the cap for spool-reconstituted spans. When a command
  ...
  /**
   * ADR-099: Best-effort spool deletion, invoked by processCommand() AFTER
