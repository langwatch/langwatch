 ...
 *
 * ADR-099: When a command carries `spoolRef`, the worker must:
...
// ---------------------------------------------------------------------------
// Race-condition regression test (ADR-099 fix)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ADR-099 guard: spoolRef present but no blobStore configured
// ---------------------------------------------------------------------------
...
describe("given a command carrying a spoolRef but a handler with no blobStore", () => {
  ...
  describe("when handle() is called", () => {
    it("throws rather than emitting a span with cleared attributes", async () => {
      ...
      // Must reject with the ADR-099 guard message
      await expect(handler.handle(command)).rejects.toThrow(
        `ADR-099: command carries spoolRef "${SPOOL_REF}" but this handler has no blobStore configured to reconstitute the span. Refusing to emit a span with cleared attributes (would be permanent data loss in event_log).`,
      );
