...
import { LANGY_CONVERSATION_PROCESS_NAME } from "~/server/event-sourcing.old/pipelines/langy-conversation-processing/process-manager";
import {
  agentRespondedEvent,
  CONVERSATION_ID,
  PROJECT_ID,
} from "../../../../event-sourcing.old/pipelines/langy-conversation-processing/process-manager/__tests__/helpers/langyEventFixtures";
 ...
 * (ADR-098): the operational read models are two `withProjection` state folds
...
// ADR-102 the pipeline constructs them itself, so this list is the production
  ...
  describe("given the pipeline built from its public static definition", () => {
    ...
    describe("when inspecting the declared process manager", () => {
      it("declares the conversation process on the pipeline", () => {
        // ADR-098: the topology lives here, not in the registry. If this
    ...
    describe("when inspecting live event subscribers", () => {
      ...
      it("mounts the real handlers, not a caller-supplied set", () => {
        // ADR-102: the pipeline constructs its own live consumers, so
