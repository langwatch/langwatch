...
/**
 * Langy conversation process manager (ADR-099) — typed contracts for the
...
export interface LangyConversationProcessState {
  ...
  archived: boolean;
  /** ADR-098: id of the turn whose resume handoff is pending — identity only. */
...
export const langyWorkerDispatchIntentSchema = z.object({
  ...
  turnId: z.string(),
  /**
   * ADR-098: the handed-off turn whose worker-authored resume token the next
