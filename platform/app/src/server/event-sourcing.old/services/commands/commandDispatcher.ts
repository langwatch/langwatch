...
export async function processCommand<EventType extends Event>(
  ...
  try {
    ...
    if (events.length > 0) {
      ...
      // ADR-099: Post-store cleanup. Invoked AFTER the event_log INSERT is durable.
 ...
 * best-effort post-store cleanup (ADR-099). An empty event set skips the store.
...
/**
 * Batched sibling of {@link processCommand} (ADR-099).
 *
