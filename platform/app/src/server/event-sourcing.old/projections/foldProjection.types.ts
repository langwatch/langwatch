...
export interface FoldProjectionOptions {
   ...
   * bound, walking cold partitions. Under ADR-099 that is the behaviour behind
...
export interface FoldProjectionStore<State> {
   ...
   * stamp the store's version gate compares a committed row against (ADR-099).
   *
  ...
  getWithApplied?(
     ...
     * ADR-099 adopter means its version gate rejected an older shape.
     *
