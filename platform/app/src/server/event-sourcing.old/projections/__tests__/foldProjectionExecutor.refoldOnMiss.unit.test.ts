 ...
 * ADR-099). On a store miss the executor rebuilds state from the event log
...
describe("FoldProjectionExecutor refoldOnStoreMiss", () => {
  ...
  describe("given the option is not set", () => {
    ...
    // unbounded history scan is the expensive path (ADR-099), so it stays
...
/**
 * The ADR-099 transitional net has to be observable.
 *
 ...
 * transitional refold and a regression to the pre-ADR-099 steady state (every
