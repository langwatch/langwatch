 ...
 * the durable read-back shape (`getWithApplied`) a store gains under ADR-099.
 ...
 * ADR-099 read-back store publishes so the tier in front of it can key by it.
 */
function versioned(
...
describe("CachedFoldStore", () => {
  ...
  // Since ADR-099 the executor (`FoldProjectionExecutor.appliedIdsForCommit`)
  ...
  describe("given the context carries an applied-event-id set", () => {
    describe("when the state is stored", () => {
      ...
      it("does not re-read the cache to re-merge, even on a retry", async () => {
        ...
        // redundant read-back ADR-099 removed).
        await store.store(
  ...
  // The TTL is a correctness invariant (ADR-099): a cache miss is authoritative
