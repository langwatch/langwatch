...
> {
    ...
    for (const [name, subscriber] of this.eventSubscribers) {
      ...
      for (const event of matching) {
        try {
          ...
          // Enqueue-time filter (ADR-098): a declined event never
          ...
          // Claim-check staging (ADR-098): the subscriber may swap the staged
          ...
          // `EnqueueDispatchOptions.stage` and ADR-098.
   ...
   * staged job's id is `<eventId>/<jobType>/<jobName>` (ADR-100,
