...
The id of anything a command creates is **derived from the work's natural key**, never minted per attempt (ADR-098). That is what makes two deliveries of one unit of work collapse in the event log rather than duplicate.
...
A store that reads its own committed rows back carries a `projectionVersion` and reports an older stamp as a miss, so rows written before a column existed are rebuilt once rather than decoding their defaults into permanently wrong state (ADR-099).
...
`options.enqueue` is the ADR-098 seam evaluated *before* a job is staged: `filter` declines an event outright (the cheapest job is the one that never exists) and `stage` swaps the payload for a small claim-check reference. Both run on the no-retry path, so both must be total.
...
See: [`process-manager/`](./process-manager/), ADR-098, ADR-103
...
- [ADR-099](../../../../dev/docs/adr/099-projection-storage-and-table-definition.md) — fold read-back and the projection version gate
- [ADR-098](../../../../dev/docs/adr/098-event-sourcing-core.md) — the enqueue seam: filter and claim-check staging; which substrate for which job; derived identity for dispatched work
- [ADR-102](../../../../dev/docs/adr/102-package-topology-and-composition.md) — pipelines own their composition; the command bus
