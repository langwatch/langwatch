...
A pipeline is a factory function. It takes store interfaces, ports and pre-built artifacts, constructs everything it registers from imported factories, and returns a static definition built by `definePipeline()`. The whole topology is readable in `pipeline.ts` without opening the composition root (ADR-102).
...
A store that reads committed rows back **must gate on `projectionVersion`** and report an older stamp as a miss (ADR-099) -- otherwise a row written before a column existed decodes its default into permanently wrong state.
...
`options.enqueue` carries the ADR-098 hooks -- `filter` decides whether a job is staged at all, `stage` swaps the payload for a claim-check reference. **Both must be total.** A throw there loses this subscriber's job for this event permanently.
...
The runtime owns the manager, the shared process outbox and the wake workers ([`process-manager/`](./process-manager/)). See ADR-098 and ADR-103.

### Choosing a substrate

ADR-098 decides it with one question -- *if this work is lost, does anything need to be able to tell?*
...
Derive the aggregate id and the `idempotencyKey` from the work's natural key, never mint one per attempt (ADR-098) -- deriving the id *is* the idempotency, and two retries of one unit of work must collapse in the event log.
...
`commandBus.assertPortsResolvable()` runs once registration completes, so a port bound to a command no pipeline registers fails at boot rather than on the first production dispatch (ADR-102).
...
| `subscribers/` | `EventSubscriberDefinition` and its options (ADR-098 enqueue hooks) |
