...
/**
 * Queue serialization and append-coalescing options (ADR-099), shared
...
export interface CommandSerializationOptions {
  ...
  /**
   * Coalesce this producer's appends (ADR-099). When one aggregate can
  ...
  /**
   * Optional byte cap for a coalesced batch (ADR-099). The drain stops
...
> {
  ...
  /** Process managers mounted on this pipeline (ADR-099/098). */
