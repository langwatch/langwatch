...
/** The builder methods that register a value — ADR-102's list. */
 ...
 * may be handed to them whole. ADR-102 calls the two lists together
 * exhaustive over the builder; `builderWithMethodNames()` below reads the
 * class so a method neither list names fails rather than passes.
 */
 ...
 * accepting one as "the ADR-102 hole", so a dep typed that way is a
...
function describeDepsTypeViolation({
  ...
  return (
    `${pipeline}: dep \`${dep}\` is a ${typeName}, which ADR-102 forbids — ` +
...
function describeCallSiteViolation({
  ...
  return (
    ...
    `(pipeline.ts:${line}), which ADR-102 forbids — the argument must be ` +
...
/** Every ADR-102 violation in one `pipelines/<x>/pipeline.ts`. */
...
/** Every ADR-102 violation across every pipeline, sorted for stability. */
