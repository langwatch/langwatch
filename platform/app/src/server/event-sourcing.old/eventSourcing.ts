...
export class EventSourcing {
  ...
  /**
   * ADR-102 — the command bus' identity index. Keyed on the command class
   ...
   * (ADR-102). Replaces `getPipeline(name).commands.x.send(...)`, which is
  ...
  private createGlobalQueue(): void {
    ...
    // ADR-052 cutover tombstone (ADR-052's ground is now ADR-098): the legacy ReactorOutbox stack staged
