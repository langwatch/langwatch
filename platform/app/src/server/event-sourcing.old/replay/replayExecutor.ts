...
export class FoldAccumulator {
  ...
  apply(event: ReplayEvent): void {
    ...
    // ADR-099: Apply leanForProjection before the projection handler — same utility
...
export class MapAccumulator {
  ...
  async apply(event: ReplayEvent): Promise<void> {
    ...
    // ADR-099: lean the event before the map handler — the same interposition
 ...
 * projections this path targets (ADR-099).
 */
export class StateAccumulator {
  ...
  apply(event: ReplayEvent): void {
    ...
    // ADR-099: lean before the reducer, exactly as live dispatch and the fold
