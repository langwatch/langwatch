...
import type { DataplaneS3Config } from "./dataplane-s3";
...
export class StorageService {
  ...
  async getObject(
    ...
    if (env.DATASET_STORAGE_LOCAL) {
      return this.getLocalObject(projectId, datasetId);
    } else {
      return this.getS3Object(projectId, datasetId);
    }
  }
...
function resolveS3Bucket(
  ...
  return destination.kind === "s3" ? destination.bucket : "langwatch";
}
...
function resolveS3Credentials(
  privateConfig: DataplaneS3Config | null,
  ...
  return { accessKeyId, secretAccessKey, sessionToken, hasExplicitKeys };
}
...
function resolveS3Region(params: {
  endpoint: string | undefined;
  hasExplicitKeys: boolean;
}): string | undefined {
  const { endpoint, hasExplicitKeys } = params;
...
export const createS3Client = async (projectId: string) => {
  ...
  const s3Bucket = resolveS3Bucket(projectId, destination);
  ...
  const { accessKeyId, secretAccessKey, sessionToken, hasExplicitKeys } =
    resolveS3Credentials(privateConfig);

  const endpoint = privateConfig?.endpoint ?? env.S3_ENDPOINT;
  const region = resolveS3Region({ endpoint, hasExplicitKeys });
