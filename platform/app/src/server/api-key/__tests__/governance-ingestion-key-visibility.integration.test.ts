  ...
  describe("given user A and user B each own a personal ingest key plus an org-owned and a regular service key", () => {
    describe("when user A lists API keys (non-admin list path)", () => {
      ...
      it("returns A's own ingest key and the regular service key, but not B's ingest key or the org-owned ingest key", async () => {
        const aKey = await ingestKeys.ingestionKeyEnsureForProject({
          callerUserId: USER_A,
          ownerUserId: USER_A,
          organizationId: ORG_ID,
          projectId: PROJECT_ID,
        ...
        const bKey = await ingestKeys.ingestionKeyEnsureForProject({
          callerUserId: USER_B,
          ownerUserId: USER_B,
          organizationId: ORG_ID,
          projectId: PROJECT_ID,
        ...
        const orgOwned = await ingestKeys.ingestionKeyEnsureForProject({
          callerUserId: USER_A,
          ownerUserId: null,
          organizationId: ORG_ID,
          projectId: PROJECT_ID,
