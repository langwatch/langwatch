...
describe("IngestionKey issuance + self-scoping resolution", () => {
  const ingestKeys = getApp().governance;
  ...
  describe("when an ingest key is issued for a project", () => {
    it("mints through the org-tenancy guard and self-scopes on resolution with no projectId", async () => {
      const issued = await ingestKeys.ingestionKeyEnsureForProject({
  ...
  describe("when ingest keys are issued create-only for one project", () => {
    ...
    it("leaves every token live so two machines can both write", async () => {
      const first = await ingestKeys.ingestionKeyIssueForProject({
        callerUserId: USER_ID,
        ownerUserId: null,
        organizationId: ORG_ID,
        projectId: PROJECT_ID,
      ...
      const second = await ingestKeys.ingestionKeyIssueForProject({
        callerUserId: USER_ID,
        ownerUserId: null,
        organizationId: ORG_ID,
        projectId: PROJECT_ID,
    ...
    it("names each key after its source type and minting device", async () => {
      const issued = await ingestKeys.ingestionKeyIssueForProject({
        callerUserId: USER_ID,
        ownerUserId: null,
        organizationId: ORG_ID,
        projectId: PROJECT_ID,
    ...
    it("does not revoke a key the rotating path issued for the same pair", async () => {
      const rotating = await ingestKeys.ingestionKeyEnsureForProject({
        callerUserId: USER_ID,
        ownerUserId: USER_ID,
        organizationId: ORG_ID,
        projectId: PROJECT_ID,
      ...
      await ingestKeys.ingestionKeyIssueForProject({
        callerUserId: USER_ID,
        ownerUserId: null,
        organizationId: ORG_ID,
        projectId: PROJECT_ID,
  ...
  describe("when an ingest key is rotated in place", () => {
    it("revokes the previous token so it stops resolving", async () => {
      const first = await ingestKeys.ingestionKeyEnsureForProject({
        callerUserId: USER_ID,
        ownerUserId: USER_ID,
        organizationId: ORG_ID,
        projectId: OTHER_PROJECT_ID,
      ...
      const second = await ingestKeys.ingestionKeyEnsureForProject({
        callerUserId: USER_ID,
        ownerUserId: USER_ID,
        organizationId: ORG_ID,
        projectId: OTHER_PROJECT_ID,
