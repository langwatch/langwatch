...
export const sendTriggerEmail = async ({
      ...
      <Html lang="en" dir="ltr">
        ...
        >
          <Img
            ...
            width="36"
          />
          <Heading as="h1">{triggerType ? `(${triggerType}) ` : ""}LangWatch Trigger</Heading>
          <Heading as="h3">{triggerName}</Heading>
          <p>
            This is an automated email generated by your configured triggers. Below, you will find
  ...
  const subject = `${triggerType ? `(${triggerType}) ` : ""}Trigger - ${triggerName}`;
  try {
...
async function sendPerRecipient({
  ...
  for (const recipient of recipients) {
    ...
    if (!EMAIL_RX.test(recipient)) {
      logger.warn({ triggerId, projectId }, "Skipping malformed trigger email recipient");
      continue;
    }
    ...
    if (isSentinel) {
      await sendEmail({
        mailer,
        content: { to: noReplyTo, bcc: [recipient], subject, html },
      });
