...
function buildUnsubscribe({
  ...
  // Written as a string rather than as part of the tree above it on purpose
  // (ADR-031): it is appended after the template has rendered, so a template
  // author cannot strip it. The values are the same ones `emailTheme.ts`
  // holds, interpolated rather than imported as a style object because this
  // never passes through React.
  const footerHtml = `
    <div style="max-width:${EMAIL_WIDTH.card};margin:0 auto;padding:0 ${EMAIL_SPACE.textInset} 32px ${EMAIL_SPACE.textInset};font-family:${EMAIL_FONT.body};color:${EMAIL_COLOR.textSubtle};font-size:${EMAIL_TYPE.finePrint.size};line-height:${EMAIL_TYPE.finePrint.leading};">
      <a href="${triggerFooterUrl}" style="color:${EMAIL_COLOR.textSubtle};text-decoration:underline;">Stop receiving this notification</a>
      &nbsp;·&nbsp;
      <a href="${projectFooterUrl}" style="color:${EMAIL_COLOR.textSubtle};text-decoration:underline;">Stop all notifications from this project</a>
    </div>`;
...
export const sendTriggerEmail = async ({
  ...
  try {
    emailHtml = await render(
      <EmailShell
        title={`${triggerType ? `(${triggerType}) ` : ""}LangWatch Trigger`}
      >
        <EmailSectionHeading>{triggerName}</EmailSectionHeading>
        <EmailParagraph>
        ...
        {triggerMessage ? (
          <EmailParagraph>{triggerMessage}</EmailParagraph>
        ) : null}
    ...
    <Section>
      {triggerData.slice(0, 10).map((data, index) => (
        <Row key={index}>
          ...
          >
            <Link href={getLink(data)} style={emailLinkStyle}>
              {getDisplayText(data)}
            </Link>
          </Column>
        </Row>
