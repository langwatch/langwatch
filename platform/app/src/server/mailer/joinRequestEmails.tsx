...
export const sendJoinRequestArrivedEmail = async ({
  ...
  const html = await render(
    <EmailShell title="Someone asked to join your organization">
      <EmailParagraph>
      ...
      <EmailParagraph>
        Approving adds them with your organization&apos;s default role. If they
        need more than that, send them an invitation instead — that is the flow
        that carries roles and teams.
      </EmailParagraph>
...
export const sendJoinRequestReminderEmail = async ({
  ...
  const html = await render(
    <EmailShell title="A request to join is still waiting">
      <EmailParagraph>
        <strong>{requesterName}</strong> asked to join{" "}
        <strong>{organizationName}</strong> a week ago and nobody has answered
        yet.
      </EmailParagraph>
      <EmailParagraph>
...
export const sendJoinRequestApprovedEmail = async ({
  ...
  const html = await render(
    <EmailShell title={`You are in ${organizationName}`}>
      <EmailParagraph>
        Your request to join <strong>{organizationName}</strong> on LangWatch
        was approved. You are a member now, with the organization&apos;s default
        role.
      </EmailParagraph>
      <EmailAction href={organizationUrl} label={`Open ${organizationName}`} />
    </EmailShell>,
  );
  await sendEmail({
    to: requesterEmail,
...
export const sendJoinRequestRejectedEmail = async ({
  ...
  const html = await render(
    <EmailShell title="Your request was not approved">
      <EmailParagraph>
        Your request to join <strong>{organizationName}</strong> on LangWatch
        was not approved.
      </EmailParagraph>
      <EmailParagraph tone="muted" style={{ margin: 0 }}>
        If you think that is a mistake, the people who can change it are your
        colleagues there — ask one of them for an invitation.
      </EmailParagraph>
    </EmailShell>,
  );
  await sendEmail({
...
export const sendJoinRequestExpiredEmail = async ({
  ...
  const html = await render(
    <EmailShell title="Your request lapsed">
      <EmailParagraph>
        Nobody answered your request to join <strong>{organizationName}</strong>{" "}
        on LangWatch within two weeks, so it lapsed.
      </EmailParagraph>
      <EmailParagraph tone="muted" style={{ margin: 0 }}>
        You can ask again whenever you like.
      </EmailParagraph>
    </EmailShell>,
  );
  await sendEmail({
...
export const sendDomainAutoJoinedEmail = async ({
  ...
  const html = await render(
    <EmailShell title="A colleague joined automatically">
      <EmailParagraph>
        <strong>{memberName}</strong> verified a <strong>{domain}</strong>{" "}
        address and joined <strong>{organizationName}</strong> on LangWatch with
        the organization&apos;s default role.
      </EmailParagraph>
      <EmailParagraph>
        They were admitted by your automatic joining setting for that domain,
        not by anybody clicking approve. You can change that setting, or remove
        them, from members settings.
      </EmailParagraph>
      <EmailAction href={membersSettingsUrl} label="Open members settings" />
    </EmailShell>,
  );
