...
describe("given an organization whose connection is live", () => {
  ...
  it("does not implicitly roll back a suspended selected connection", async () => {
    const suspended = { ...projectedConnection, state: "SUSPENDED" } as const;
    const repository = new ConnectionFirstDomainRoutingRepository({
      legacy: port(legacyConnection, [legacyConnection]),
      connections: port(suspended, [suspended]),
    });

    expect(await repository.listActiveConnections()).toEqual([suspended]);
  });
});
