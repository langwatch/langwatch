 ...
 *
 * A connection's history, turned into words a customer or an operator can
 * read — never an internal event name, and an attested domain never read as
 * the customer's own proof (D05 amendment).
 *
 * Corresponds to specs/identity/sso-connection-history.feature.
 */
import { describe, expect, it, vi } from "vitest";
import type { SsoConnectionHistoryEntry } from "../repositories/sso-connection-event-log.repository";
import {
  type SsoConnectionHistoryReadsPort,
  SsoConnectionHistoryService,
} from "../sso-connection-history.service";
...
function serviceOver(entries: readonly SsoConnectionHistoryEntry[]) {
  const findHistory = vi
    .fn<SsoConnectionHistoryReadsPort["findHistory"]>()
    .mockResolvedValue(entries);
  return {
    service: new SsoConnectionHistoryService({ history: { findHistory } }),
    findHistory,
  };
}
...
describe("given an administrator reading a connection's history", () => {
  describe("when a fact this surface has words for is read", () => {
    ...
    it("reads as a sentence, never the event's own wire type", async () => {
      ...
      expect(history[1]?.summary).toContain(
        "the requester could not be reached at that domain",
      );
    });
    ...
    it("marks a grandfathered fact as carried over, and a self-served one as not", async () => {
      ...
      expect(
        history.find((v) => v.eventId === "evt_migrated")?.carriedOver,
      ).toBe(true);
      expect(
        history.find((v) => v.eventId === "evt_self_served")?.carriedOver,
      ).toBe(false);
    });
  });
  ...
  describe("when a domain was proved by a platform operator's attestation", () => {
    ...
    it("names the operator outright, never the customer's own proof", async () => {
      ...
      // Never worded as though "acme.com" itself did the proving — the
      // sentence names who verified it, not a record the customer published.
