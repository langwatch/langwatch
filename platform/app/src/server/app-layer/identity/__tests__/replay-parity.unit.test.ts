...
import {
  factToRow,
  type IdentifierRowShape,
  rowToFact,
} from "../repositories/identifier-row";
...
function maintainIncrementally(facts: IdentityFact[]): IdentifierRowShape[] {
  const rows = new Map<string, IdentifierRowShape>();
  for (const fact of facts) {
    const heads: IdentityHeads = {
      userId: USER,
      identifiers: Object.fromEntries(
        [...rows.values()].map((row) => {
          const loaded = rowToFact(row);
    ...
    for (const head of Object.values(next.identifiers)) {
      rows.set(head.identifierId, factToRow(head));
    }
  }
...
function rebuildFromScratch(facts: IdentityFact[]): IdentifierRowShape[] {
  const heads = facts.reduce(
    (state, fact) => reduceIdentity({ heads: state, fact }),
    emptyIdentityHeads({ userId: USER }),
  );
  return sorted(Object.values(heads.identifiers).map(factToRow));
}

const sorted = (rows: IdentifierRowShape[]): IdentifierRowShape[] =>
  ...
  describe("when a fact is re-applied to rows that already carry it", () => {
    it("never regresses a later lifecycle state", () => {
      ...
      const email = replayed.find(
        (row: IdentifierRowShape) => row.id === "idf_email",
      );
