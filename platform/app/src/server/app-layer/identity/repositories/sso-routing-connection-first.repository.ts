  ...
  implements SignInDomainRoutingPort
{
  ...
  async listActiveConnections(): Promise<readonly RoutableConnection[]> {
    ...
    // A projected answer means the persisted per-organization route has
    // already selected the predecessor or replacement. Adding the legacy
    // environment row beside it would count the same migration twice and,
    // worse, silently restore legacy when the selected direct side is
    // suspended. Rollback is an explicit migration command, never a union.
    return projected.length > 0 ? projected : legacy;
  }
}
