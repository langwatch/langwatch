/**
 * When `withSerializationRetry` runs a transaction again, how long it waits
 * before it does, and what it throws when it stops (#8200).
 *
 * The race itself is not reproduced here — 31 local runs of the integration
 * test never managed to — so the waits are pinned directly: a `run` that
 * loses a fixed number of races, a `wait` that records what it was asked for,
 * and a `random` held still.
 *
 * Feature: specs/identity/authentication-settings.feature
 */
...
describe("withSerializationRetry", () => {
  describe("given a transaction that loses three races before it commits", () => {
    ...
    it("runs four times, waiting a draw from a doubling window between each, and never after the last", async () => {
      ...
      await expect(withSerializationRetry(losing(3), deps)).resolves.toBe(
        "would_strand_user",
      );
    ...
    ])("scales the draw $draw over windows of 10, 20 and 40 ms", async ({
      draw,
      waits,
    }) => {
  ...
  describe("given a transaction that commits first time", () => {
    it("returns its answer without waiting", async () => {
      ...
      await expect(
        withSerializationRetry(losing(0, "deleted"), deps),
