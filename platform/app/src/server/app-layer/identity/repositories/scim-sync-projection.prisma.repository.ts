...
import type { ScimSyncFoldState } from "~/server/event-sourcing/pipelines/scim-sync/projections/scimSyncState.foldProjection";
  ...
  implements StateProjectionStore<ScimSyncFoldState>, ScimSyncReadRepository
{
  ...
  async store(
    ...
    const columns = {
      ...
      state: state.state,
      lastPushedAt:
        state.lastPushedAtMs === null ? null : new Date(state.lastPushedAtMs),
