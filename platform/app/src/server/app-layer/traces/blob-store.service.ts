 ...
 */
function eventLogOccurredAtWindow(eventId: string): {
  predicate: string;
...
export class BlobStore {
  ...
  private async queryEventLogRow({
    clickHouseClient,
    tenantId,
    aggregateType,
    aggregateId,
    ...
    const { predicate: occurredAtPredicate, params: occurredAtParams } =
      eventLogOccurredAtWindow(eventId);
  ...
  // Parses the raw event_log row into its EventPayload shape (ADR-022:
  // EventPayload IS event.data, span/body at top level, no outer `data`
  // wrapper).
  private parseEventLogPayload({
    rawRows,
    eventId,
    field,
    tenantId,
  ...
  private resolveEventLogSpanAttribute({
    payload,
    field,
    eventId,
  }: {
    ...
    const spanAttributes = payload.span?.attributes;
  ...
  // Resolves the requested field out of a parsed EventPayload — either the
  // log-record body (leanForProjection's "body" eventref) or a span
  // attribute by key.
  private resolveEventLogField({
    payload,
    field,
    eventId,
  }: {
    ...
    if (field === "body") {
      return this.resolveEventLogBody({ payload, field, eventId });
    }
    return this.resolveEventLogSpanAttribute({ payload, field, eventId });
  }
  ...
  async getFromEventLog({
    ...
    const rawRows = await this.queryEventLogRow({
      clickHouseClient,
      tenantId,
      aggregateType,
      aggregateId,
    ...
    const payload = this.parseEventLogPayload({
      rawRows,
      eventId,
      field,
      tenantId,
    ...
    return this.resolveEventLogField({ payload, field, eventId });
  }
