 ...
 *
 * Spec: specs/traces-v2/instant-eval-search.feature ("An eval chip filters by
 * a run's verdicts").
 */
...
const run = (
  overrides: Partial<ResolvedInstantEvalRun> = {},
...
describe("given a trigger evaluating an eval chip in memory", () => {
  describe("when the field is evaluated", () => {
    ...
    it("answers unsupported for a forcing spelling", () => {
      const tag = parse('eval.trace:"the user is annoyed"') as TagToken;
