/**
 * Worked trace filter queries, one definition for every reader.
 *
 * `SEARCH_FIELDS` says which fields exist and `QUERY_SYNTAX_DOC` says how the
 * grammar works; neither answers "what does a real query for my question look
 * like". A model handed the field list writes `status:failed` because nothing
 ...
 * the suite instead of teaching a query the product refuses.
 *
 * Consumed by the query reference (`~/server/analytics/query-reference`), which
 * is what the REST reference endpoint, the CLI and the MCP server all read.
 *
 * @see ./metadata.ts — the fields these name
 * @see ./grammar.ts — the syntax these are written in
 * @see specs/analytics/query-reference.feature
 */

/**
 * What kind of question an example answers. Drives the reference's grouping.
 *
 * A const array rather than a bare union, because the REST boundary publishes
 * the set: an agent choosing an example branches on the intent, and a spec that
 * would not enumerate the intents makes it guess them from prose.
 */
...
export type QueryExampleIntent = (typeof QUERY_EXAMPLE_INTENTS)[number];
 ...
 * The library.
 *
 * Values are real ones (`error`, `application`, `gpt-*`) except where an
 * example is necessarily about the caller's own data — a conversation id, a
 * user id, an attribute key — and then they are obvious placeholders. The
 * reference points at the facets endpoint for the real ones, because those are
 * tenant data and this file is not.
 */
export const TRACE_FILTER_EXAMPLES: readonly TraceFilterExample[] = [
  {
    ...
    text: "status:error",
    notes:
      "The error lives on a span, not in the trace's text, so free text does not find it.",
  },
  {
  ...
  },
  {
    ...
    notes:
      "Matches when at least one span in the trace carries the key with that value.",
  },
  {
