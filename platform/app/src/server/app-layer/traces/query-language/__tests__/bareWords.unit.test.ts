/**
 * The bare-word split behind Enter: which typed text is a sentence for the
 * router and which is a filter applied as typed, plus the phrase form the
 * "search it as one phrase" fix and the undo restore, and the node ceiling
 * the client checks before a query leaves the browser.
 *
 * Spec: specs/traces-v2/search.feature ("Enter routes a sentence", "A
 * sentence past the term ceiling").
 */
...
describe("splitBareWords", () => {
  ...
  describe("given bare words next to explicit terms", () => {
    ...
    it("joins the words in order and keeps the explicit terms", () => {
      expect(
        splitBareWords("annoyed users status:error asking refunds"),
      ).toEqual({
...
describe("combineQueries", () => {
  it("returns the other side when one is empty", () => {
    expect(combineQueries({ base: "", addition: "status:error" })).toBe(
      "status:error",
    );
    expect(combineQueries({ base: "status:error", addition: "" })).toBe(
      "status:error",
    );
  });
...
describe("requoteBareTerms", () => {
  describe("given a sentence past the term ceiling", () => {
    ...
    it("collapses the words into one phrase and keeps the explicit terms", () => {
      ...
      expect(requoteBareTerms(`status:error ${sentence}`)).toBe(
        `status:error AND "${sentence}"`,
      );
    });

    it("produces a query under the ceiling", () => {
      const requoted = requoteBareTerms(words(30));
      expect(validateAst(parse(requoted))).toBeNull();
    });
  });
