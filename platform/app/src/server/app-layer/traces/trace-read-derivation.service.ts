...
} from "~/server/app-layer/traces/scenario-role-metrics.derivation";
import { SpanCostService } from "~/server/app-layer/traces/span-cost.service";
 ...
 * version) so a batch reads stored spans once regardless of how many subscribers
 * or events it dispatches, while a fold that has advanced (new spans) re-reads.
 */
