...
export interface GraphTriggerSentRepository {
  ...
  markResolvedById(params: { id: string; projectId: string; now: Date }): Promise<void>;
}

/** Narrow app-only persistence adapter for graph-alert incident state. */
export class PrismaGraphTriggerSentRepository implements GraphTriggerSentRepository {
  ...
  deleteOpenClaim({ id, projectId }: { id: string; projectId: string }): Promise<void> {
