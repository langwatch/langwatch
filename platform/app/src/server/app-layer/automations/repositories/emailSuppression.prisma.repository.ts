  ...
  implements EmailSuppressionRepository
{
   ...
   * migration. Prisma's upsert also cannot target a partial unique index
   * (WHERE clause on a non-null column), which is what
   * `projectId_email_triggerId` resolves to when triggerId IS NOT NULL. Both
   * cases therefore attempt the create and treat a unique-violation (P2002)
   * as "already suppressed" — idempotent and race-safe (no findFirst+create
   * gap) — via the same create + catch-P2002 + findFirst pattern rather than
  ...
  private async createIdempotent({
    ...
    where: { projectId: string; email: string; triggerId: string | null };
  }): Promise<EmailSuppressionRow> {
    try {
      return await this.prisma.emailSuppression.create({ data });
    } catch (error) {
      if (
