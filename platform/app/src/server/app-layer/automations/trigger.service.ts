...
export class TriggerService {
   ...
   * ingested trace: the `triggerMatch` subscribers record a match for any
   * NOTIFY trigger with no evaluation filters, and the settle dispatcher skips
   * the filter guard entirely when `filters` is empty. A report fires from its
   * scheduler calendar entry (`syncReportSchedule`) and nowhere else.
   */
   ...
   * false if another dispatch already claimed it. Side effects (email,
   * slack, dataset write) must run only when this returns true to avoid
   * double-fire on concurrent dispatch or an outbox retry.
   */
