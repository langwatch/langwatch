...
import { IngestionCredentialsService } from "@langwatch/enterprise-governance-server";
import { AppGovernanceEncryptionPort } from "@langwatch/enterprise-api/governance/governance-infrastructure.adapter";
import { decrypt, encrypt } from "~/utils/encryption";

const ingestionCredentials = IngestionCredentialsService.create(
  AppGovernanceEncryptionPort.create({ encrypt, decrypt }),
);
...
describe("given an admin saves a Genie source carrying a workspace token", () => {
  describe("when the source is saved through the service", () => {
    ...
    it("stores the token encrypted and unreadable from the source's configuration", async () => {
      ...
      const { source } = await service.ingestionSourceCreate({
        organizationId,
