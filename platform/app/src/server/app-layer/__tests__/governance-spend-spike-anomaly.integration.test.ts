/** Verifies the anomaly evaluator against composed ClickHouse and Postgres adapters. */
...
describe("SpendSpikeAnomalyEvaluator — I/O integration against governance_kpis + AnomalyAlert", () => {
  ...
  let app: App;
  let kpisRepository: AppGovernanceKpisAdapter;
  let org: Organization;
  let govProject: InternalProject;
  ...
  function createEvaluator() {
    return PostgresSpendSpikeAnomalyAdapter.create({
      ...
      dispatcher: AnomalyAlertDispatcherService.create({
        http: SsrfSafeAnomalyAlertHttpAdapter.create(fetch),
      }),
    }).build();
  }
  ...
  beforeAll(async () => {
    ...
    kpisRepository = new AppGovernanceKpisAdapter(async () => ch);
