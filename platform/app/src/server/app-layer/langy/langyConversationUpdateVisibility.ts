 ...
 *
 * The freshness subscriber (`langy-conversation-update-broadcast.subscriber.ts`)
 * publishes on the tenant-wide (project) broadcast channel, so every project
 * member's SSE subscription receives every conversation's signal locally. That
 * is only safe once each subscription drops signals the user may not see:
