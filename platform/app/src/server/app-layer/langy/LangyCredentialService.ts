...
/**
 * The requesting user's GitHub-login-like handle for bot-authored attribution,
 * resolved from the LangWatch profile ONLY (never a GitHub call) — bot-authored
 * PRs need no user OAuth. Sanitised to the GitHub username charset so the
 * `Co-authored-by` trailer the worker writes is always well-formed. Falls back
 * to a stable handle when the profile carries nothing usable.
 *
 * TODO(seam): when the login flow captures the user's real GitHub login + id
 * (e.g. from the identity `Account` row), prefer that here so the trailer's
 * noreply email is exact.
 */
export function resolveActingGithubLogin(session: Session): string {
...
export type LangyCredentials = {
  ...
  /**
   * Short-lived GitHub App INSTALLATION access token (1h, scoped to the
   * installation's repositories) for a bot-authored PR. Absent when the GitHub
   * App is not configured on the instance, the org has no usable installation,
   * or the mint failed. The worker injects this as `GH_TOKEN` so `gh` and the
   * credential helper read it from env only — never written to disk, never
   * persisted control-plane side.
   */
  githubToken?: string;
  /**
   * The requesting user's GitHub-login-like handle. Threaded to the worker for
   * the `Co-authored-by` commit trailer and the "Requested by @<login> via
   * LangWatch" PR note, and surfaced in the sidebar chip. Resolved from the
   * user's LangWatch profile (NOT a GitHub call) — bot-authored attribution
   * needs no user OAuth.
   */
  githubLogin?: string;
  /**
   * Stable key for the minted token's repository/permission scope. Folded into
   * the worker credential signature so a scope change re-warms the worker
   * rather than reusing one holding a differently-scoped token.
   */
