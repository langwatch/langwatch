...
import { ScenarioRepository, type ScenarioRunConfig } from "../scenarios/scenario.repository";
...
async function resolveParameterMaps(params: {
  ...
  return {
    ...
    secretParametersByScenarioId: new Map(
      [...resolved]
        .filter(
          ([, scenarioParameters]) => Object.keys(scenarioParameters.secretParameters).length > 0,
        )
...
export class SuiteService {
   ...
   */
  static create(params: { prisma: PrismaClient; suiteRunService: SuiteRunService }): SuiteService {
  ...
  async create(input: Omit<CreateSuiteInput, "slug">): Promise<SimulationSuite> {
   ...
   */
  async getAll(params: { projectId: string; kinds?: SuiteKind[] }): Promise<SimulationSuite[]> {
    return tracer.withActiveSpan(
      "SuiteService.getAll",
      {
        kind: SpanKind.INTERNAL,
   ...
   */
  async createFolder(params: { projectId: string; name: string }): Promise<SimulationSuite> {
    return tracer.withActiveSpan(
  ...
  async getAllFolders(params: { projectId: string }): Promise<SimulationSuite[]> {
   ...
   */
  async archiveFolder(params: { projectId: string; folderId: string }): Promise<SimulationSuite> {
    return tracer.withActiveSpan(
  ...
  async getById(params: { id: string; projectId: string }): Promise<SimulationSuite | null> {
    return tracer.withActiveSpan(
      ...
      async (span) => {
        logger.debug({ projectId: params.projectId, suiteId: params.id }, "Fetching suite by id");
  ...
  async update(params: {
    ...
    return tracer.withActiveSpan(
      ...
      async () => {
        logger.debug({ projectId: params.projectId, suiteId: params.id }, "Updating suite");
  ...
  async duplicate(params: { id: string; projectId: string }): Promise<SimulationSuite> {
    return tracer.withActiveSpan(
      ...
      async (span) => {
        logger.debug({ projectId: params.projectId, suiteId: params.id }, "Duplicating suite");
  ...
  async archive(params: { id: string; projectId: string }): Promise<SimulationSuite | null> {
    return tracer.withActiveSpan(
      ...
      async (span) => {
        logger.debug({ projectId: params.projectId, suiteId: params.id }, "Archiving suite");
  ...
  async run(params: {
    ...
    return tracer.withActiveSpan(
      ...
      async (span) => {
        ...
        const { parametersByScenarioId, secretParametersByScenarioId } = await resolveParameterMaps(
          {
