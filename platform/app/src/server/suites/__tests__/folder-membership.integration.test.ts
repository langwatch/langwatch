...
async function invariantBreaks(): Promise<string[]> {
  ...
  const folderIdByCaseId = new Map(activeCases.map((scenario) => [scenario.id, scenario.folderId]));
  ...
  for (const folder of folders) {
    ...
    const strangers = heldMembers.filter(
      (caseId) => folderIdByCaseId.has(caseId) && folderIdByCaseId.get(caseId) !== folder.id,
    );
    if (strangers.length > 0) {
      breaks.push(`folder ${folder.id} holds [${strangers.join(", ")}], which name another folder`);
    }
  }
  return breaks;
}
...
describe("folder membership", () => {
  ...
  describe("when a folder is archived", () => {
    ...
    it("keeps the final member list as a snapshot and archives the cases", async () => {
      ...
      expect([...(archivedFolder?.scenarioIds ?? [])].sort()).toEqual([...caseIds].sort());
  ...
  describe("when a duplicate is made", () => {
    ...
    it("files the copy in the same folder with the same definition", async () => {
      ...
      expect(await folderScenarioIds(refunds.id)).toEqual([scenario.id, copy.id]);
      expect(await invariantBreaks()).toEqual([]);
    });
  });
  ...
  describe("when two cases are filed into one folder at the same time", () => {
    ...
    it("holds both of them", async () => {
      ...
      expect(new Set(await folderScenarioIds(refunds.id))).toEqual(new Set([first.id, second.id]));
      expect(await invariantBreaks()).toEqual([]);
    });
  });
});
