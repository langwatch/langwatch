...
async function createCase({ name, folderId }: { name: string; folderId?: string }) {
...
describe("running a folder", () => {
  describe("when the folder holds active and archived cases", () => {
    ...
    it("schedules active cases x targets, reports the archived case skipped, and lands in the folder's run set", async () => {
      ...
      expect(scheduledScenarioIds).toEqual(new Set(active.map((scenario) => scenario.id)));
    });
  });
  ...
  describe("running all test cases", () => {
    it("creates the managed suite once, refreshes it, and schedules every active case", async () => {
      ...
      const scheduled = new Set(queueSimulationRun.mock.calls.map((call) => call[0].scenarioId));
      ...
      expect(new Set(queueSimulationRun.mock.calls.map((call) => call[0].scenarioId))).toEqual(
      ...
      expect(new Set(refreshed.scenarioIds)).toEqual(new Set([filed.id, added.id]));
    });
