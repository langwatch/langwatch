...
describe("ProjectionRouter map-subscriber dispatch over a coalesced batch", () => {
  ...
  describe("when the subscriber's deduplication id is the same for every event", () => {
    it("dispatches once, with the last event in occurredAt order", async () => {
      ...
      const payloads = await dispatchMapBatch(subscriber, batch());
  ...
  describe("when the subscriber's deduplication id varies per event", () => {
  ...
  describe("when a shouldDispatch predicate filters part of the batch", () => {
    it("reads each event's own record", async () => {
      const seen: unknown[] = [];
      const subscriber: SubscriberDispatchDefinition<Event> = {
        name: "picky",
        shouldDispatch: (_event, context) => {
    ...
    it("collapses only what survived the predicate", async () => {
      const subscriber: SubscriberDispatchDefinition<Event> = {
        name: "picky",
        shouldDispatch: (event) => event.id !== `event-${BATCH_SIZE - 1}`,
        options: {
