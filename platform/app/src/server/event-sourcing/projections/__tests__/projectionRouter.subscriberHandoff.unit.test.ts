 ...
 *
 * Retiring the reactor (ADR-075) moved every post-event handler onto this seam.
 ...
 * subscriber's job, including every ADR-073 process manager, which the runtime
 ...
 *      with the identical payload so the queue's staged-job id (ADR-080)
  ...
  describe("given a queue that fails one attempt and then accepts", () => {
    describe("when an event a subscriber must see is published", () => {
      ...
      it("re-sends the identical payload, so the queue resolves it to the job already staged", async () => {
        ...
        // <jobName>`, ADR-080), so re-sending the SAME payload is what makes a
