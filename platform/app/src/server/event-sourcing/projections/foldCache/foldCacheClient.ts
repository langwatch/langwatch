...
export interface FoldCacheClient {
   ...
   * correctness invariant, not housekeeping — ADR-066 §5: a miss is treated as
 ...
 * answered for fold projections (ADR-082 §3).
 *
 ...
 * the union stops at this class — layer 1, where ADR-082 says it is legitimate
 * and where it must end.
 *
 * Topology therefore cannot decide whether folds are cached. That matters
 * because ADR-066 §5 makes the fold cache the event processor's read-your-write
