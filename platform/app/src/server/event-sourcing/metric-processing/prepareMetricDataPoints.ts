...
/**
 * OTLP containers arrive as untrusted JSON, so a wrong-shaped container is
 * rejected in place rather than thrown past the per-point `catch` — that
 * would abort the whole request instead of reporting the one malformed
 * container (`specs/otlp/canonical-metric-ingestion.feature`: "A malformed
 * batch is counted, not crashed on").
 */
function containerArray(args: {
  value: unknown;
  label: string;
  rejections: RejectionLog;
}): unknown[] {
  const { value, label, rejections } = args;
...
async function prepareMetric(args: {
  ...
  for (const pointRaw of metricData.dataPoints) {
    ...
    // Redactors mutate in place. Isolate a fresh resource/scope clone per
    // sibling point so a non-idempotent redaction policy cannot compound its
    // own output and produce a different SeriesId within one OTLP request.
    ...
    try {
      await redactTypedAttributes({
        ...
        redactionService: prepArgs.redactionService,
        piiRedactionLevel: prepArgs.piiRedactionLevel,
        tenantId: prepArgs.tenantId,
      });
      accepted.push(
        buildPoint({
          tenantId: prepArgs.tenantId,
          organizationId: prepArgs.organizationId,
...
/**
 * Canonicalises an OTLP metric export request into immutable data points.
 * Nothing here throws for bad input: every malformed container or point
 * becomes a counted rejection, so the caller can answer with OTLP partial
 * success rather than failing the whole request for one bad point.
 */
