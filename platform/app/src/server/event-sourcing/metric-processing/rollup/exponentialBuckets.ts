...
export function downscaleBuckets(args: {
  offset: number | null;
  counts: string[];
  fromScale: number;
  toScale: number;
}): BucketMap {
  const { offset, counts, fromScale, toScale } = args;
 ...
 * range — an array sized to that span is an OOM waiting for one hostile OTLP
 * batch. Layout selection downscales to fit this cap first; the fold below
 * only fires for a span no legal scale can absorb.
 */
...
export function denseBuckets(map: BucketMap): {
  ...
  // count-preserving, only the folded counts' magnitudes are clamped down.
...
export function subtractMaps(args: {
  current: BucketMap;
  previous: BucketMap;
}): BucketMap | null {
 ...
 */
function bucketBounds(args: { index: number; scale: number }): {
  lower: number;
  upper: number;
} {
  const exponent = 2 ** -args.scale;
  return {
    lower: 2 ** (args.index * exponent),
    upper: 2 ** ((args.index + 1) * exponent),
  };
}
 ...
 * upper boundary whenever it would otherwise bisect a populated bucket — a
 * threshold inside a bucket has no well-defined count to split.
 */
export function commonZeroThreshold(args: {
  thresholds: Array<number | null>;
  bucketMaps: BucketMap[];
  scale: number;
}): number {
  const { thresholds, bucketMaps, scale } = args;
  ...
  for (;;) {
    ...
    // Widening only ever raises the threshold onto an existing boundary, so
    // the fixpoint is reached in at most one pass per distinct boundary.
...
export function absorbZeroBuckets(args: {
  buckets: BucketMap;
  threshold: number;
  scale: number;
}): { buckets: BucketMap; absorbed: bigint } {
  const { buckets, threshold, scale } = args;
