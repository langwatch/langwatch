...
/** Matches `canonical/validate.ts`'s floor for exponential histogram scales. */
...
function mergedIndexSpan(args: {
  points: CanonicalMetricDataPoint[];
  scale: number;
}): number {
  const { points, scale } = args;
  let span = 0;
...
function normalizePoint(args: {
  point: CanonicalMetricDataPoint;
  layout: CommonLayout;
}): NormalizedPoint {
  const { point, layout } = args;
...
function differenceExponentialPoint(args: {
...
function collectContributors(args: {
  entries: BucketEntry[];
  all: CanonicalMetricDataPoint[];
}): Map<string, CanonicalMetricDataPoint> {
  const predecessors = new Map<string, CanonicalMetricDataPoint>();
  for (const { point, index } of args.entries) {
    const previous = usablePredecessor({ point, all: args.all, index });
 ...
 * chosen across every contributing point — including the predecessors
 * cumulative points are differenced against — so the stored row is a valid
 * exponential histogram, not a union of incompatible layouts.
 */
export function buildExponentialHistogramRow(args: {
  row: MetricRollupRow;
  entries: BucketEntry[];
  all: CanonicalMetricDataPoint[];
}): void {
