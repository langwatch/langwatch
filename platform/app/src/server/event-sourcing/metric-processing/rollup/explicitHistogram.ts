 ...
 */
function coarsenExplicit(args: {
  point: CanonicalMetricDataPoint;
  targetBounds: number[];
}): bigint[] {
  const { point, targetBounds } = args;
...
/**
 * The predecessor a cumulative point is differenced against, or `undefined`
 * when the sequence reset or gapped. Bounds selection and differencing share
 * this predicate so they can never disagree about which points contribute.
 */
...
function usablePredecessors(args: {
  entries: BucketEntry[];
  all: CanonicalMetricDataPoint[];
...
function differenceHistogramPoint(args: {
  point: CanonicalMetricDataPoint;
  index: number;
  all: CanonicalMetricDataPoint[];
  bounds: number[];
}): { counts: bigint[]; count: bigint; sum: number | null } | null {
  const { point, index, all, bounds } = args;
...
export function buildHistogramRow(args: {
  row: MetricRollupRow;
  entries: BucketEntry[];
  all: CanonicalMetricDataPoint[];
}): void {
  ...
  // coarsened onto the same exactly-mergeable boundary set, so each usable
  // predecessor also gets a say in choosing that set.
