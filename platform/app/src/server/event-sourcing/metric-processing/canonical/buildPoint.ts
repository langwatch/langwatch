...
import { MAX_CANONICAL_METRIC_PAYLOAD_BYTES } from "../constants";
import type {
...
function canonicalExemplars(exemplars: unknown): unknown[] {
  ...
  return exemplars.map((raw) => {
    ...
    // asInt/asDouble: 0 is a real exemplar value and must render as one, not
    // fall through to the "no value" branch.
    const value =
      exemplar.asInt !== undefined && exemplar.asInt !== null
        ? {
            type: "int",
...
/**
 * Turns one validated OTLP data point into its canonical, lossless form: a
 * stable SeriesId over the point's identity fields, a PointId over the whole
 * canonical payload (so two receivers of the same measurement always derive
 * the same PointId — ADR / `specs/otlp/canonical-metric-ingestion.feature`),
 * and the queryable columns rendered from that same payload.
 */
