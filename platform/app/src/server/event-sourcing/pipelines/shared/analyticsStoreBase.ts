...
interface SlimSideRepo<TRow> {
  ...
  upsertBatch?(entries: Array<{ row: TRow; retentionDays?: number }>): Promise<void>;
}
...
interface SlimFoldStoreConfig<TState, TRow> {
   ...
   */
  hasPersistableSignal: (state: TState, context: ProjectionStoreContext) => boolean;
  /**
  ...
  project: (params: { state: TState; tenantId: string; version: string }) => TRow;
}
 ...
 */
export abstract class BaseAnalyticsFoldStore<
  TState,
  TRow,
> implements FoldProjectionStore<TState> {
  ...
  constructor(repo: SlimSideRepo<TRow>, config: SlimFoldStoreConfig<TState, TRow>) {
    this.repo = repo;
    this.config = config;
  }
  ...
  async store(state: TState, context: ProjectionStoreContext): Promise<void> {
    ...
    const stateWithId = this.config.stampAggregateId(state, String(context.aggregateId));
  ...
  async storeBatch(
    ...
    const batchRows = entries
      .filter(({ state, context }) => this.config.hasPersistableSignal(state, context))
      .map(({ state, context }) => {
        ...
        return {
          ...
          retentionDays: retentionDaysFrom(context, this.config.retentionCategory),
        };
      });
    ...
    if (this.repo.upsertBatch) {
      ...
      await Promise.all(
        batchRows.map(({ row, retentionDays }) => this.repo.upsert(row, retentionDays)),
      );
    }
  }
 ...
 * category via `super(...)`.
 */
export abstract class BaseAnalyticsRollupAppendStore<TRow> implements AppendStore<TRow> {
  ...
  async append(record: TRow, context: ProjectionStoreContext): Promise<void> {
    const retentionDays = retentionDaysFrom(context, this.config.retentionCategory);
