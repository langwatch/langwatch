...
/**
 * Syncs a RunStarted event to the suite run pipeline, when the run belongs
 * to a suite.
 */
async function handleRunStarted(
  ...
  if (!isSuiteSetId(scenarioSetId)) return;
...
/**
 * Syncs a RunFinished event to the suite run pipeline, when the run belongs
 * to a suite and its ECST identity fields have been backfilled.
 */
...
/**
 * Syncs a RunEvaluated event to the suite run pipeline, when the run belongs
 * to a suite, its ECST identity fields have been backfilled, and the
 * evaluation actually moved the verdict or status.
 */
async function handleRunEvaluated(
  ...
  return {
    ...
    async handler(event: SimulationProcessingEvent): Promise<void> {
      if (isSimulationRunStartedEvent(event)) {
        await handleRunStarted(event, deps);
        return;
      }

      if (isSimulationRunFinishedEvent(event)) {
        await handleRunFinished(event, deps);
        return;
      }

      if (isSimulationRunEvaluatedEvent(event)) {
        await handleRunEvaluated(event, deps);
      }
    },
  };
}
