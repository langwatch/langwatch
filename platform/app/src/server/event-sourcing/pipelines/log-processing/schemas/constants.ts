...
export const LOG_PROCESSING_COMMAND_TYPES = [RECORD_CANONICAL_LOG_COMMAND_TYPE] as const;
 ...
 * against that rather than the stored length. A batch of records near
 * {@link MAX_CANONICAL_LOG_PAYLOAD_BYTES} is therefore stopped by bytes long
 * before this count; the count bounds a burst of small records. (This
 * previously read "the count is the only bound you can rely on", which was
 * true before the `s` header existed and is not now.)
 */
