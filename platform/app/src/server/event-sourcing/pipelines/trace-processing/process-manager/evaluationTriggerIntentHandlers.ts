...
import type { QueueSendOptions } from "~/server/event-sourcing/queues";
...
export interface EvaluationTriggerDispatchDeps {
   ...
   * the instance row and the outbox (ADR-069/ADR-073). And the summary read at
 ...
 * has no retry behind it (ADR-075, "The one migration hazard"). Moving the
