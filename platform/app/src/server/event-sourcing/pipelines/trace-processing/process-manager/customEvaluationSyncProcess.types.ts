 ...
 * (ADR-081: derived, never minted), and there is no deadline, no latch and no
 ...
 * the verdicts are read back from the span store at dispatch time (ADR-069's
 * claim-check).
 */
 ...
 *
 * Derived from the work itself, never minted (ADR-081, which names this
 * process as the exemplar of that rule). The span is what carried the
