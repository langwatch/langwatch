 ...
 * collapses rather than double-billing (migration `00002_create_schema.sql`
 * §8). Two writes for the same dedup key land on the same sort key; the read
 * side counts `countDistinct(DeduplicationKeyHash)`, so even an unmerged
 * duplicate contributes exactly one.
 *
