...
function stubDeps(
  overrides: Partial<BillingReportingPipelineDeps> = {},
): BillingReportingPipelineDeps {
  return {
    client: fakeClient(),
...
describe("billing-reporting pipeline topology", () => {
  ...
  it("derives the dotted event type string", () => {
    ...
    expect(built.eventTypes).toEqual([
      "lw.billing_report.billable_event_recorded",
    ]);
  });
  ...
  it("mounts the poke disabled on a self-hosted build", () => {
    ...
    expect(
      built.processManagers[BILLING_METER_POKE_PROCESS_NAME]!.enabled,
    ).toBe(false);
  });
