...
import { type RecordBillableEventPorts, recordBillableEvent } from "../recordBillableEvent.command";
...
describe("recordBillableEvent command", () => {
  describe("given the project resolves to an organization", () => {
    it("emits billableEventRecorded stamped with the resolved organization and the project as tenant", async () => {
      const ports: RecordBillableEventPorts = { resolveOrganizationId: vi.fn().mockResolvedValue("org-1") };

      const events = await recordBillableEvent(ports)(
        { id: "event-1", type: "lw.obs.trace.span_received", createdAt: 1_000, idempotencyKey: "idem-1" },
        ctx,
      );
    ...
    it("falls back to the event id when no idempotency key is supplied", async () => {
      ...
      const events = await recordBillableEvent(ports)({ id: "event-1", type: "lw.obs.trace.span_received", createdAt: 1_000 }, ctx);
  ...
  describe("given the project has no organization", () => {
    it("emits nothing rather than failing", async () => {
      const ports: RecordBillableEventPorts = { resolveOrganizationId: vi.fn().mockResolvedValue(undefined) };
