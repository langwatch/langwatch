/**
 * Unit tests for the billable-events meter store: organization resolution,
 * the orphan-project and ClickHouse-unconfigured skip paths, and — the
 * behaviour this task calls out explicitly — that a redelivery cannot
 * double-bill because two writes for the same dedup key land on the same row
 * identity.
 *
...
import { createBillableEventsMeterStore } from "../billableEventsMeter.store";
import type { BillableEventMeterRecord } from "../billableEventsMeter.mapProjection";

function makeClient(overrides: Partial<ClickHouseClient> = {}): ClickHouseClient {
  return {
    query: vi.fn(),
    stream: vi.fn(),
...
function makeRecord(overrides: Partial<BillableEventMeterRecord> = {}): BillableEventMeterRecord {
  return {
    eventId: "evt-1",
...
describe("createBillableEventsMeterStore", () => {
  describe("given the project resolves to an organization and ClickHouse is configured", () => {
    it("resolves the organization once per batch and inserts one row per record", async () => {
      ...
      const getClickHouseClientForOrganization = vi.fn().mockResolvedValue(client);

      const store = createBillableEventsMeterStore({
        resolveOrganizationId,
        getClickHouseClientForOrganization,
      });

      await store.writeBatch(
        [makeRecord({ eventId: "evt-1" }), makeRecord({ eventId: "evt-2", deduplicationKey: "evt-2" })],
        { tenantId: "proj-1" },
      );
  ...
  describe("given the same record is written twice (a redelivery)", () => {
    it("produces rows whose identity columns are byte-identical, so ClickHouse collapses them to one", async () => {
      ...
      const record = makeRecord({ eventId: "evt-1", deduplicationKey: "biz-key-1" });
      await store.writeBatch([record], { tenantId: "proj-1" });
      await store.writeBatch([{ ...record }], { tenantId: "proj-1" });

      const [firstCall, secondCall] = vi.mocked(client.insert).mock.calls;
      // Row layout is [OrganizationId, TenantId, EventId, EventType, DeduplicationKey, EventTimestamp, UpdatedAt].
      // The sort-key identity (OrganizationId, TenantId, DeduplicationKey) must match exactly across both writes.
      const identityOf = (row: unknown[]) => row.slice(0, 5);
      expect(identityOf(firstCall![0].rows[0]!)).toEqual(identityOf(secondCall![0].rows[0]!));
    });
  });
  ...
  describe("given ClickHouse is not configured for this organization", () => {
    it("skips the insert gracefully", async () => {
      ...
      await expect(store.writeBatch([makeRecord()], { tenantId: "proj-1" })).resolves.toBeUndefined();
    });
  });
  ...
  describe("given the ClickHouse insert fails", () => {
    it("propagates the failure so the delivery retries", async () => {
      const client = makeClient({
        insert: vi.fn().mockRejectedValue(new Error("clickhouse connection timeout")),
      });
      ...
      await expect(store.writeBatch([makeRecord()], { tenantId: "proj-1" })).rejects.toThrow(
