...
} from "../billableEventsMeter.mapProjection";

function makeEvent(overrides: Partial<BillableSourceEvent> = {}): BillableSourceEvent {
  return {
    id: "evt-1",
    tenantId: "proj-1",
...
describe("mapBillableEvent", () => {
  ...
  describe("given an event with an idempotencyKey", () => {
    it("uses it as the record's dedup key", () => {
      const result = mapBillableEvent(
        makeEvent({ id: "evt-1", idempotencyKey: "idem-key-abc", type: "lw.evaluation.reported" }),
      );

      expect(result).toEqual(
        expect.objectContaining({ deduplicationKey: "idem-key-abc", eventType: "lw.evaluation.reported" }),
      );
    });
  });
...
describe("billableEventsMeterGroupKey", () => {
  describe("given events from the same project", () => {
    it("renders the same group key", () => {
      const first = renderBillableEventsMeterGroupKey(makeEvent({ tenantId: "proj-1", id: "evt-1" }));
      const second = renderBillableEventsMeterGroupKey(makeEvent({ tenantId: "proj-1", id: "evt-2" }));
      expect(first).toBe(second);
    });
  });
  ...
  describe("given events from different projects", () => {
    it("renders different group keys, so the two projects never share a lane", () => {
      const first = renderBillableEventsMeterGroupKey(makeEvent({ tenantId: "proj-1" }));
      const second = renderBillableEventsMeterGroupKey(makeEvent({ tenantId: "proj-2" }));
  ...
  describe("given the descriptor itself", () => {
    it("is a map lane scoped to a partition of the project id, not per-event", () => {
      const key = billableEventsMeterGroupKey(makeEvent({ tenantId: "proj-1" }));
      expect(key).toEqual({
        tenantId: "proj-1",
...
describe("createBillableEventsMeterProjection", () => {
  ...
  describe("given the store rejects the write", () => {
    it("propagates the failure rather than swallowing it", async () => {
      const writeBatch = vi.fn().mockRejectedValue(new Error("clickhouse unavailable"));
      const projection = createBillableEventsMeterProjection({
        store: { kind: "append", writeBatch },
      });
