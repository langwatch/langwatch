...
/**
 * One item row per result event. The columns the store owns — the tenant, the
 * deterministic `ProjectionId` it derives from them, and the write-time
 * bookkeeping — are stamped by the store's own `toRow`.
 */
 ...
 *
 * `experimentId` is in the hash. The old generator omitted it, and since the
 * deployed sort key is `(TenantId, RunId, ProjectionId)`, two experiments
 * sharing a `runId` produced items with identical sort keys — silent data loss
 * the moment a background merge collapsed them. Including it closes that
 * without a re-key migration, at the cost of not colliding with ids minted
 * before this change.
 */
