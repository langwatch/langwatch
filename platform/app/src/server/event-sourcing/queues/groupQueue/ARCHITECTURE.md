...
A job's payload can range from "a 200-byte status update" to "a 30 MiB trace dump." Inlining all of it in the staged value works for the small case but bloats the Redis-side hash, blows past the Lua script's reply-size budget, and replicates the same bytes 30× when an event fans out to 30 subscribers. So the envelope **tiers** the body across four progressively-more-durable storage targets, picked at encode time based on the body's serialized size.
...
- **Inline gzip (1–4 KiB)** — gzip wins for most real payloads in this range; the encoder verifies (`gzip+base64 < raw`) and falls back to raw if not. The inline ceiling (`INLINE_CEILING_BYTES = 4 KiB`) is set low so ordinary fan-out events cross into the dedup tier rather than inlining N× across subscribers. Tighter than the GQ1 32 KiB threshold (`BLOB_OFFLOAD_THRESHOLD_BYTES`), which only offloaded for the very-large case.
...
- **Identical bytes → one stored blob.** A 30-subscriber fan-out of the same event stages 30 envelopes, each carrying a distinct lease-holder identity, but the underlying blob is a single stored copy. PUTs are idempotent — racing or retrying just overwrites the same key with the same content.
...
### Routing-exclusion: keeping the hash stable across subscribers

The same event fanned out to subscriber A (`__jobName: "rollup-by-day"`) and subscriber B (`__jobName: "rollup-by-hour"`) would naively hash to different blobs because the per-subscriber routing fields perturb the body bytes. To prevent this, the GQ2 encoder splits `jobData` into:
