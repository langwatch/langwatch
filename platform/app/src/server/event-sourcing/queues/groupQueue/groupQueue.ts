  ...
  implements EventSourcedQueueProcessor<Payload>
{
  ...
  async send(
    ...
    if (isNew) {
      ...
      // Audit hook (ADR-095 revision): only on the new-stage path, not on
  ...
  async sendBatch(
    ...
    // Audit hooks (ADR-095 revision). The Lua's stageBatch returns a count,
   ...
   * the queue stays available. See ADR-095 revision for the "audit lags
