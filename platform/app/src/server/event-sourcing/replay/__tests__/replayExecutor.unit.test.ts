 ...
 * The store has `storeBatch` so the accumulator will use batch writes.
 */
...
describe("FoldAccumulator", () => {
  ...
  describe("when events span multiple aggregates", () => {
    it("produces separate state per aggregate", async () => {
      ...
      accumulator.apply(makeEvent({ aggregateId: "agg-A", data: { value: 10 } }));
      accumulator.apply(makeEvent({ aggregateId: "agg-B", data: { value: 20 } }));
      accumulator.apply(makeEvent({ aggregateId: "agg-A", data: { value: 30 } }));
  ...
  describe("when the projection has a custom key function", () => {
    it("groups events by custom key instead of aggregateId", async () => {
      ...
      accumulator.apply(
        makeEvent({ aggregateId: "agg-1", data: { value: 10, group: "X" } }),
      );
      accumulator.apply(
        makeEvent({ aggregateId: "agg-2", data: { value: 20, group: "X" } }),
      );
      accumulator.apply(
        makeEvent({ aggregateId: "agg-3", data: { value: 30, group: "Y" } }),
      );
  ...
  describe("when writeBatchSize is smaller than the number of entries", () => {
    it("chunks store calls by writeBatchSize", async () => {
      const { projection, storeBatchSpy } = createTestProjection();
      const accumulator = new FoldAccumulator(projection);

      // Five different aggregates in one tenant.
      for (let i = 0; i < 5; i++) {
        accumulator.apply(
  ...
  describe("when a rebuild re-folds an aggregate that already has committed state", () => {
    ...
    it("always starts from init state, never loads existing", async () => {
      ...
      accumulator.apply(makeEvent({ data: { value: 5 } }));
      accumulator.apply(makeEvent({ data: { value: 7 } }));
