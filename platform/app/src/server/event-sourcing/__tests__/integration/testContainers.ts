...
export async function stopTestContainers(): Promise<void> {
  ...
  // that left a BRPOP/SUBSCRIBE on the shared test connection (groupQueue
  // dispatchers, pub/sub subscribers) wedges the entire shard's teardown
...
export async function cleanupTestData(tenantId?: string): Promise<void> {
  // Clean up Redis queues (groupQueue stores its staging layer in Redis)
