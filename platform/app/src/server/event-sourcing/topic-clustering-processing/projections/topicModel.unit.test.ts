...
function topic(id: string, overrides: Partial<TopicModelEntry> = {}): TopicModelEntry {
  return {
    id,
    name: `topic-${id}`,
    parentId: null,
...
const recorded = (overrides: Partial<TopicsRecordedData> = {}): TopicsRecordedData => ({
  projectId: "project-1",
  mode: "replace",
  source: "clustering",
...
const apply = (state: TopicModelState, data: TopicsRecordedData) => applyTopicsRecorded(state, data);

function topicIds(state: TopicModelState): string[] {
  return deriveTopicModelView(state).topics.map((t) => t.id).sort();
}
...
describe("topicModel fold", () => {
  ...
  it("replaces the model wholesale on a replace-mode event", () => {
    ...
    state = apply(state, recorded({ topics: [topic("a"), topic("b")], occurredAt: 1_000 }));
  ...
  it("merge-mode upserts into the existing model without dropping other topics", () => {
    let state = initTopicModelState();
    state = apply(state, recorded({ topics: [topic("a"), topic("b")], occurredAt: 1_000 }));
    state = apply(state, recorded({ mode: "merge", topics: [topic("c")], occurredAt: 2_000 }));
    expect(topicIds(state)).toEqual(["a", "b", "c"]);
  });

  it("merge-mode updates an existing topic's fields, keeping its id", () => {
    let state = initTopicModelState();
    state = apply(state, recorded({ topics: [topic("a", { name: "old-name" })], occurredAt: 1_000 }));
    state = apply(state, recorded({ mode: "merge", topics: [topic("a", { name: "new-name" })], occurredAt: 2_000 }));
    expect(deriveTopicModelView(state).topics.find((t) => t.id === "a")?.name).toBe("new-name");
  });
  ...
  it("recording the identical topics again is a no-op on the projected model", () => {
    let state = initTopicModelState();
    const data = recorded({ topics: [topic("a"), topic("b")], occurredAt: 1_000 });
  ...
  describe("firstRecordedAt", () => {
    it("takes an explicit firstRecordedAt over the event's own occurredAt", () => {
      const state = apply(initTopicModelState(), recorded({ topics: [topic("a", { firstRecordedAt: 500 })], occurredAt: 1_000 }));
    ...
    it("preserves an already-projected topic's firstRecordedAt across an update", () => {
      let state = initTopicModelState();
      state = apply(state, recorded({ topics: [topic("a")], occurredAt: 1_000 }));
      state = apply(state, recorded({ mode: "merge", topics: [topic("a", { name: "renamed" })], occurredAt: 5_000 }));
      expect(deriveTopicModelView(state).topics[0]?.firstRecordedAt).toBe(1_000);
    });

    it("falls back to the event's occurredAt for a genuinely new topic", () => {
      const state = apply(initTopicModelState(), recorded({ topics: [topic("a")], occurredAt: 1_000 }));
      expect(deriveTopicModelView(state).topics[0]?.firstRecordedAt).toBe(1_000);
    });
  });
  ...
  describe("given a stale replace arrives after a newer one (the watermark's job)", () => {
    ...
    it("does not let a stale replace resurrect a topic a newer replace already dropped", () => {
      ...
      const backward = [...t3ReplaceFirst].reverse().reduce(apply, initTopicModelState());

      expect(topicIds(forward)).toEqual(["a", "d"]);
      expect(topicIds(backward)).toEqual(["a", "d"]);
      expect(deriveTopicModelView(forward)).toEqual(deriveTopicModelView(backward));
    });
    ...
    it("still lets a chronologically-later merge survive a chronologically-earlier replace, regardless of delivery order", () => {
      ...
      const backward = [...events].reverse().reduce(apply, initTopicModelState());
  ...
  describe("the seed guard — documented exception (see module docblock)", () => {
    ...
    it("applies a seed normally when the model is still empty", () => {
      const state = apply(
        initTopicModelState(),
        recorded({ source: "seed", topics: [topic("legacy-1"), topic("legacy-2")], occurredAt: 1_000 }),
      );
    ...
    it("no-ops a seed once the model has any topics, regardless of the seed's own occurredAt", () => {
      let state = initTopicModelState();
      state = apply(state, recorded({ topics: [topic("a")], occurredAt: 5_000 }));
      state = apply(
        state,
        ...
        recorded({ source: "seed", topics: [topic("legacy-1")], occurredAt: 9_000 }),
      );
    ...
    it("is content-gated, not timestamp-ordered — the one documented order-dependent case", () => {
      const seedAt9000 = recorded({ source: "seed", topics: [topic("legacy")], occurredAt: 9_000 });
      const clusteringAt5000 = recorded({ topics: [topic("a")], occurredAt: 5_000 });

      const seedFirst = [seedAt9000, clusteringAt5000].reduce(apply, initTopicModelState());
      const clusteringFirst = [clusteringAt5000, seedAt9000].reduce(apply, initTopicModelState());
  ...
  describe("order invariance (ADR-098 decision 4) — excluding the documented seed exception", () => {
    ...
    it("reaches the same model regardless of the order two replaces are delivered in", () => {
      ...
      const report = checkOrderInvariance({ init: initTopicModelState, apply, events });
    ...
    it("reaches the same model regardless of the order a replace and several merges are delivered in", () => {
      ...
      const report = checkOrderInvariance({ init: initTopicModelState, apply, events, maxPermutations: 24 });
      expect(report.invariant).toBe(true);
    });

    it("reaches the same model regardless of the order repeated merges to one topic are delivered in", () => {
      const events: TopicsRecordedData[] = [
        recorded({ mode: "merge", topics: [topic("a", { name: "v1" })], occurredAt: 1_000 }),
        recorded({ mode: "merge", topics: [topic("a", { name: "v2" })], occurredAt: 2_000 }),
        recorded({ mode: "merge", topics: [topic("a", { name: "v3" })], occurredAt: 3_000 }),
      ];
      const report = checkOrderInvariance({ init: initTopicModelState, apply, events });
      expect(report.invariant).toBe(true);
    });
