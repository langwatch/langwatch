...
/**
 * Both tests here `await import("~/start")`, which pulls the whole server graph
 * in. That import, not the streaming, is what they spend their wall clock on -
 * measured at ~17s cold - and it is paid or not depending on whether something
 * earlier in the same shard already warmed the module cache. Riding the default
 * timeout therefore makes these tests a function of shard composition: they
 * passed for months, then a new unrelated test file changed the split and this
 * one timed out at 30s having done nothing wrong.
 *
 * So the budget is stated, and stated generously. It is not a performance
 * assertion - the ordering assertions below are the test - and a real hang
 * still fails it, just later.
 */
...
describe("the Node→Hono bridge with streaming request bodies", () => {
  ...
  describe("given an ndjson relay-style connection whose body arrives in delayed chunks", () => {
    describe("when the first chunk is pushed while the connection stays open", () => {
      it(
        "hands the first line to the route handler before the second chunk is sent",
        async () => {
  ...
  describe("given a regular buffered-read route", () => {
    describe("when its body arrives in two chunks", () => {
      it(
        "still delivers the whole body to the handler",
        async () => {
          const { honoFetchForNode } = await import("~/start");
