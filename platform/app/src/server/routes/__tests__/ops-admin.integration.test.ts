...
import type { AppContextBindings } from "~/app/api/middleware/app-context";
...
import { app } from "../ops/admin";

const adminOperation = vi.fn();

const requestApp = {
  ops: {
    isAdmin: ({ email }: { email?: string | null }) =>
      email?.toLowerCase() === "ada@example.com",
    startImpersonation: (...args: unknown[]) => impersonationStart(...args),
    stopImpersonation: (...args: unknown[]) => impersonationStop(...args),
    adminOperation: (...args: unknown[]) => adminOperation(...args),
  },
} as unknown as AppContextBindings["langwatchApp"];
...
describe("admin routes", () => {
  ...
  describe("given an admin asking for a supported resource", () => {
    beforeEach(signInAsAdmin);

    it("delegates the operation to the composed Ops service and preserves its wire result", async () => {
      adminOperation.mockResolvedValue({ data: [], total: 0 });
      ...
      expect(await bodyOf(response)).toEqual({ data: [], total: 0 });
      expect(adminOperation).toHaveBeenCalledWith(
        expect.objectContaining({
          resource: "user",
          method: "getList",
          actorId: "user_admin",
          req: { headers: expect.any(Object) },
        }),
      );
    });
  });
