...
import {
  attachComparisonResponseSchema,
...
secured.access(apiKeyAuthWrite).post(
  "/:slug/run",
  describeRoute({
...
secured.access(apiKeyAuthWrite).post(
  "/:slug/comparison",
  describeRoute({
    summary: "Attach a comparison to an experiment",
    description:
      "Add a comparison judge to a saved experiment, addressed by slug. Each variant names a target the experiment already has, or a prompt or agent to build one from; a variant that matches an existing target reuses it rather than adding a second column for the same thing.",
    tags: ["Experiments"],
    // Declared by hand rather than through zValidator: the handler
    // authenticates before it reads the body, so it parses with
    // `validateJsonBody` itself and there is no validator schema for the
    // generator to read.
    requestBody: {
      required: true,
      content: {
        "application/json": {
          schema: requestBodySchema(attachComparisonBodySchema),
        },
      },
    },
    responses: {
      ...
      400: {
        description:
          "A variant could not be resolved, or a named field is not a column on the dataset",
        content: {
          "application/json": {
            schema: resolver(handledErrorEnvelopeSchema),
          },
      ...
      409: {
        description:
          "The experiment changed while the update was being prepared; read it again and retry",
        content: {
          "application/json": {
            schema: resolver(handledErrorEnvelopeSchema),
          },
      ...
      422: {
        description: "The body did not match the expected shape",
        content: {
          "application/json": {
            schema: resolver(handledErrorEnvelopeSchema),
          },
      ...
      200: {
        description: "Comparison attached",
        content: {
          "application/json": {
            schema: resolver(attachComparisonResponseSchema),
          },
        },
      },
    },
