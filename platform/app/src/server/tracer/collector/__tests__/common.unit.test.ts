...
import type { BaseSpan, Span, SpanInputOutput } from "@langwatch/trace-contract";
import {
  flattenSpanTree,
  getFirstInputAsText,
  getLastOutputAsText,
...
// The collector functions under test operate on `Span`, whose input/output are
// already-parsed `SpanInputOutput` (value is a real object, not a JSON string).
// Several fixtures below are written in the legacy ElasticSearch span shape,
// where `input.value` / `output.value` may be a JSON-encoded string. This helper
// reproduces exactly what the (now-removed) `transformElasticSearchSpanToSpan`
// did for those fields: pass every other field through untouched, and convert
// input/output via `elasticSearchToTypedValue` — JSON.parse string values,
// leave non-string values as-is, and fall back to `{ type: "raw", value }` on a
// parse failure. Protections are the all-visible / no-redaction case, so values
// are otherwise unchanged.
const esTypedValueToTypedValue = (typed: any): SpanInputOutput => {
  try {
    return {
      type: typed.type,
...
const elasticSearchSpanToSpan = (esSpan: any): Span => {
  const { input, output, ...spanFields } = esSpan;
  return {
    ...spanFields,
    params: spanFields.params ?? {},
    input: input ? esTypedValueToTypedValue(input) : null,
    output: output ? esTypedValueToTypedValue(output) : null,
  } as Span;
};
