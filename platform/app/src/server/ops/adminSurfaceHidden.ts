 ...
 *
 * It lives here rather than beside one surface because there are two — the
 * flat REST admin API and the back office's tRPC procedures — and a denial
 * that differed between them would be the oracle this error exists to remove.
 * One class, one answer, both surfaces. It sits under `server/` rather than
 * `components/` because both callers are server code, and server code may not
