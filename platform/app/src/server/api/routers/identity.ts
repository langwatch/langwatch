/**
 * Process wiring for the `identity.*` tRPC surface.
 *
 * The transport itself is package-owned — `IdentityTrpcApi` in
 * `@langwatch/user-server`, mounted through `@langwatch/platform-api/app-trpc`.
 * What is left here is the composition this application still owns: its tRPC
 * root, its authenticated procedure, its authorization middlewares, and the
 * verification ceremony the mount reaches through a port.
 *
...
export const identityRouter = createIdentityTrpcRouter({
  root: appTrpcRoot,
