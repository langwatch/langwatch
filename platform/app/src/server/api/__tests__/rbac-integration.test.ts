...
describe("RBAC Integration Tests", () => {
  ...
  describe("hasProjectPermission", () => {
    ...
    it("grants access via group membership when user has no direct teamUser row", async () => {
      ...
      mockPrisma.groupMembership.findMany.mockResolvedValue([{ groupId: "group-abc" }]);
  ...
  describe("hasOrganizationPermission", () => {
    ...
    describe("when user is OrgUser+TeamUser ADMIN but has no RoleBindings (legacy migration)", () => {
      // Finding #28: /gateway/audit + /gateway/budgets.list + cache-rules.list
      // all 401 for legacy ADMINs because hasOrganizationPermission had no
      // TeamUser fallback for org-scoped permission checks.
      beforeEach(() => {
        mockPrisma.organizationUser.findFirst.mockResolvedValue({
          role: OrganizationUserRole.ADMIN,
        });
        mockPrisma.roleBinding.findMany.mockResolvedValue([]);
        mockPrisma.teamUser.findMany.mockResolvedValue([
          { role: TeamUserRole.ADMIN, assignedRoleId: null },
        ]);
      });
    ...
    describe("when user has only TeamUser MEMBER role (no RoleBindings, no admin)", () => {
      beforeEach(() => {
        mockPrisma.organizationUser.findFirst.mockResolvedValue({
          role: OrganizationUserRole.MEMBER,
        });
        mockPrisma.roleBinding.findMany.mockResolvedValue([]);
        mockPrisma.teamUser.findMany.mockResolvedValue([
          { role: TeamUserRole.MEMBER, assignedRoleId: null },
        ]);
      });
  ...
  describe("Permission Middleware", () => {
    ...
    describe("checkProjectPermission", () => {
      it("throws UNAUTHORIZED when user lacks permission", async () => {
        ...
        const middleware = checkProjectPermission("workflows:manage" as Permission);
      ...
      it("calls next when user has permission", async () => {
        ...
        const middleware = checkProjectPermission("workflows:view" as Permission);
    ...
    describe("checkTeamPermission", () => {
      it("throws UNAUTHORIZED when user lacks permission", async () => {
        ...
        const middleware = checkTeamPermission("workflows:manage" as Permission);
    ...
    describe("checkOrganizationPermission", () => {
      ...
      it("calls next when user has permission", async () => {
        ...
        const middleware = checkOrganizationPermission("organization:view" as Permission);
  ...
  describe("middleware org role propagation", () => {
    describe("when checkProjectPermission middleware runs for a permitted user", () => {
      it("sets organizationRole on context", async () => {
        ...
        const mockNext = vi.fn().mockResolvedValue("success");
        const middleware = checkProjectPermission("analytics:view" as Permission);
  ...
  describe("EXTERNAL (lite member) restriction logic", () => {
    ...
    describe("when non-EXTERNAL user (MEMBER) has VIEWER team role", () => {
      it.each(["datasets:view", "prompts:view", "secrets:view"] as Permission[])(
        "grants %s",
        async (permission) => {
          mockPrisma.project.findUnique.mockResolvedValue({
            team: {
