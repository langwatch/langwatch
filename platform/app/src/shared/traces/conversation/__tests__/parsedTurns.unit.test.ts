...
import { buildParsedTurns, type ConversationTurnSource } from "../parsedTurns";

const turn = (
  overrides: Partial<ConversationTurnSource> = {},
...
describe("buildParsedTurns", () => {
  ...
  describe("given a turn that started well after the previous one finished", () => {
    ...
    it("measures the gap from the previous turn's end", () => {
      const parsed = buildParsedTurns({
        turns: [
          turn({ timestamp: 1_000_000, durationMs: 2_000 }),
          turn({ timestamp: 1_032_000 }),
        ],
      });
    ...
    it("does not show a gap short enough to read as the same exchange", () => {
      const parsed = buildParsedTurns({
        turns: [
          turn({ timestamp: 1_000_000, durationMs: 1_000 }),
          turn({ timestamp: 1_003_000 }),
        ],
      });
