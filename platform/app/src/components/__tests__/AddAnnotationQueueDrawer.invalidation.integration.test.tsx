...
vi.mock("~/utils/api", () => ({
  api: {
    ...
    annotation: {
      createOrUpdateQueue: {
        useMutation: () => ({ mutate: mocks.mutate, isLoading: false }),
      },
      getQueueBySlugOrId: {
...
vi.mock("../ui/toaster", () => ({ toaster: { create: mocks.toastCreate } }));
vi.mock("~/features/errors", () => ({
  applyHandledErrorToForm: () => false,
  FormServerError: () => null,
  showErrorToast: vi.fn(),
}));
vi.mock("~/components/annotations/AddOrEditAnnotationScore", () => ({
