...
export function LLMSpanEditor({ span }: { span: SpanConfig }) {
  ...
  return (
    ...
    >
      ...
      <Flex gap={3} wrap="wrap" mb={3}>
        <Box flex={1} minW="140px">
          ...
          <Input
            ...
            onChange={(e) => updateLLM({ requestModel: e.target.value })}
            placeholder="gpt-4o"
            list="llm-models"
          />
      ...
      <Text fontSize="xs" fontWeight="medium" color="fg.subtle" mb={1}>
        Messages
      </Text>
      <Flex direction="column" gap={2} mb={2}>
        {messages.map((msg, i) => (
          <Flex
            ...
            borderColor="border"
            borderLeftWidth="2px"
            borderLeftColor={ROLE_COLORS[msg.role] ?? "border.emphasized"}
            bg="bg.subtle"
            p={2}
          >
            <select
      ...
      <Flex gap={3}>
        <Box flex={1}>
          ...
          <Input
            size="sm"
            type="number"
            value={llm.metrics?.promptTokens ?? ""}
            onChange={(e) =>
              updateLLM({
                metrics: {
                  ...llm.metrics,
                  promptTokens: parseInt(e.target.value) || undefined,
                },
              })
            }
            placeholder="0"
        ...
        <Box flex={1}>
          ...
          <Input
            size="sm"
            type="number"
            value={llm.metrics?.completionTokens ?? ""}
            onChange={(e) =>
              updateLLM({
                metrics: {
                  ...llm.metrics,
                  completionTokens: parseInt(e.target.value) || undefined,
                },
              })
            }
            placeholder="0"
