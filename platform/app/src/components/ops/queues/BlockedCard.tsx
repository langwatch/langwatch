...
export function BlockedCard({ queueNames }: { queueNames: string[] }) {
  ...
  const unblockAllMutation = api.ops.unblockAll.useMutation({
    onSuccess: (data) => {
      toaster.create({
        title: `Unblocked ${data.unblockedCount} groups`,
        type: "success",
      });
    ...
    onError: (error) =>
      showErrorToast({ error, fallbackTitle: "Couldn't unblock the groups" }),
  });
  ...
  const moveAllToDlqMutation = api.ops.moveAllBlockedToDlq.useMutation({
    onSuccess: (data) => {
      toaster.create({
        title: `Moved ${data.movedCount} groups to DLQ`,
        type: "success",
      });
      setMoveToDlqTarget(null);
      void utils.ops.invalidate();
    },
    onError: (error) =>
      showErrorToast({
        error,
        fallbackTitle: "Couldn't move the groups to the DLQ",
      }),
  });
  const canaryUnblockMutation = api.ops.canaryUnblock.useMutation({
    onSuccess: (data) => {
      toaster.create({
        title: `Canary unblocked ${data.unblockedCount}`,
        type: "success",
      });
      setCanaryQueueTarget(null);
      void utils.ops.invalidate();
    },
    onError: (error) =>
      showErrorToast({
        error,
        fallbackTitle: "Couldn't run the canary unblock",
      }),
  });

  const queuesWithBlocked = (queuesQuery.data ?? []).filter(
    (q) => q.blockedGroupCount > 0,
  );
  ...
  return (
    <>
                            ...
                            <Button
                              ...
                              onClick={() =>
                                setDrainTarget({
                                  ...
                                  groupId: cluster.sampleGroupIds[0]!,
                                })
                              }
                            >
                              Drain
      ...
      <ConfirmDialog
        open={!!unblockAllTarget}
        onClose={() => setUnblockAllTarget(null)}
        onConfirm={() => {
          if (unblockAllTarget)
            unblockAllMutation.mutate({ queueName: unblockAllTarget });
        }}
        title="Unblock All"
        description={`Unblock all blocked groups in "${unblockAllTarget}". They will retry immediately.`}
        isLoading={unblockAllMutation.isPending}
      />
      <ConfirmDialog
        open={!!drainTarget}
      ...
      />
      <ConfirmDialog
        open={!!moveToDlqTarget}
        onClose={() => setMoveToDlqTarget(null)}
        onConfirm={() => {
          if (moveToDlqTarget)
            moveAllToDlqMutation.mutate({ queueName: moveToDlqTarget });
        }}
        title="Move All to DLQ"
        description={`Move all blocked groups in "${moveToDlqTarget}" to DLQ. They can be replayed later.`}
        isLoading={moveAllToDlqMutation.isPending}
      />
      <ConfirmDialog
        open={!!canaryQueueTarget}
        onClose={() => setCanaryQueueTarget(null)}
        onConfirm={() => {
          if (canaryQueueTarget)
            canaryUnblockMutation.mutate({
              queueName: canaryQueueTarget,
              count: canaryCount,
            });
        }}
        title="Canary Unblock"
        description={`Unblock ${canaryCount} random groups in "${canaryQueueTarget}" as a canary test.`}
        isLoading={canaryUnblockMutation.isPending}
      />
    </>
  );
}
