...
export function targetColumnWidth(
  targets: readonly { label: string }[],
): number {
  const longest = targets.reduce(
    (width, target) => Math.max(width, target.label.length),
    0,
  );
  const needed =
    Math.ceil(longest * LABEL_CHARACTER_WIDTH) + LABEL_CHROME_WIDTH;
  return Math.min(
    Math.max(TARGET_COLUMN_MIN_WIDTH, needed),
