...
describe("connected agents in the run dialog", () => {
  ...
  describe("given a connected agent in production", () => {
    ...
    it("reads the name with the environment", () => {
      ...
      expect(
        screen.getByText("support-agent · production"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("given the chosen agent has no process behind it", () => {
    ...
    it("says that no process is running the agent", () => {
      const agents = agentsFor([
        connectedRow({ id: "agent_off", status: "offline" }),
      ]);

      render(
        <OfflineTargetsNotice
          agents={agents}
          targets={[{ type: "connected", id: "agent_off" }]}
        />,
        { wrapper: Wrapper },
      );
    ...
    it("says nothing when every chosen agent is online", () => {
      ...
      render(
        <OfflineTargetsNotice
          agents={agents}
          targets={[{ type: "connected", id: "agent_on" }]}
        />,
        { wrapper: Wrapper },
      );
