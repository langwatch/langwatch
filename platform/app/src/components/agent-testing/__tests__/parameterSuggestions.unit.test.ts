...
import {
  ...
  valueSuggestions,
} from "../run/parameter-suggestions";
...
describe("valueSuggestions", () => {
  describe("when the parameter has a closed list of options", () => {
    ...
    it("lists the options, narrowed by the query", () => {
      expect(
        valueSuggestions({ definition: MODEL, query: "" }).map((r) => r.value),
      ).toEqual(["gpt-5-mini", "gpt-5"]);
      expect(
        valueSuggestions({ definition: MODEL, query: "gpt-5-m" }).map(
          (r) => r.value,
        ),
      ).toEqual(["gpt-5-mini"]);
    });
  ...
  describe("when the parameter has no options", () => {
    ...
    it("offers the typed text once when it is the default", () => {
      expect(
        valueSuggestions({ definition: LOCALE, query: "en" }).map(
          (r) => r.value,
        ),
      ).toEqual(["en"]);
    });
  ...
  describe("when the name is not declared", () => {
    it("offers the typed text alone", () => {
      expect(
        valueSuggestions({ definition: undefined, query: "x" }).map(
          (r) => r.value,
        ),
      ).toEqual(["x"]);
    });
...
describe("acceptParameterSuggestion", () => {
  describe("when a key is accepted", () => {
    it("writes name= over the token and reopens on the values", () => {
      ...
      expect(
        acceptParameterSuggestion({ text, cursor: text.length, state, row }),
      ).toEqual({ text: "locale=de, model=", cursor: 17, reopens: true });
    });
  });
...
describe("parameterFieldState", () => {
  describe("when the field edits a row's name", () => {
    it("is in key mode over the whole text", () => {
      expect(
        parameterFieldState({ mode: { kind: "name" }, text: "mo", cursor: 2 }),
      ).toEqual({ open: true, mode: "field", query: "mo", tokenStart: 0 });
    });
...
describe("parameterPlaceholder", () => {
  describe("when parameters are declared", () => {
    ...
    it("reads the first plain parameter with its default", () => {
      expect(parameterPlaceholder([TOKEN, MODEL, LOCALE])).toBe(
        "model=gpt-5-mini",
      );
    });
    ...
    it("reads the first option when there is no default", () => {
      expect(
        parameterPlaceholder([
          { ...MODEL, defaultValue: undefined, options: ["a", "b"] },
        ]),
      ).toBe("model=a");
    });
  });
...
describe("errorOnLine", () => {
  ...
  describe("when the line holds the refused pair", () => {
    it("reads the message", () => {
      expect(errorOnLine({ line: "model=claude, locale=de", error })).toBe(
        error.message,
      );
    });
  });
  ...
  describe("when a row is checked", () => {
    it("reads the message on the refused row alone", () => {
      expect(errorOnRow({ name: "model", value: "claude", error })).toBe(
        error.message,
      );
      expect(
        errorOnRow({ name: "model", value: "gpt-5", error }),
      ).toBeUndefined();
    });
  });
});
