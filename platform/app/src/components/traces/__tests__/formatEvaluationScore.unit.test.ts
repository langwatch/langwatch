 ...
 *
 * `formatEvaluationScore` is the one wrapper in the tree that returns "N/A"
 * before handing a value to `numeral`, and it had no test file at all — so
 * "existing suites pass" asserted nothing about it while five other sites were
 * being routed through it as the fix. The zero case is the load-bearing one:
 * `llm_boolean` returns `score = 1 if passed else 0`, so a genuine 0.0 is real
 * data, and any guard rewritten as a falsy check (`score ? … : "N/A"`) would
 * silently start reporting real zeros as not-scored.
 */
