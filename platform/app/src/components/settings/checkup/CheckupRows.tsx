...
import type {
  CheckGroup,
  CheckRow as CheckRowData,
} from "~/server/checkup/verdict";

import { CheckRow } from "./CheckRow";

/**
 * The four bands of the checkup, in the order an operator reads them: the
 * install itself, the connection to LangWatch, the integrations, and the
 * pipelines that only prove themselves by running something.
 *
 * The bands that hold checks costing egress or money carry the button that
 * runs them; the others have no button because their rows ran on load.
 */
const GROUPS: readonly {
...
}[] = [
  {
    ...
    title: "Install",
    description:
      "The release, the data stores and the gateway this install runs on.",
    icon: <Server size={18} />,
  },
  {
    id: "langwatch",
    title: "LangWatch",
    description:
      "The license, the connection to LangWatch, and the daily report.",
    icon: <Cloud size={18} />,
  },
  {
    id: "integrations",
  ...
  },
  {
    ...
    title: "Pipelines",
    description:
      "Canaries that send one real trace, evaluation, scenario or turn.",
                ...
                <HStack gap={2}>
                  {group.id === "pipelines" ? (
                    <Input
                      ...
                      value={runPlanId}
                      onChange={(event) =>
                        onRunPlanIdChange(event.target.value)
                      }
