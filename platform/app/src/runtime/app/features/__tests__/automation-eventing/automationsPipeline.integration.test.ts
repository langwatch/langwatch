...
class InertScheduledIntents extends AutomationScheduledIntentPort {
...
class InertIntentRetention extends AutomationIntentRetentionPort {
...
describe("automations pipeline", () => {
  ...
  describe("given a settled trigger and trace receive later activity", () => {
    describe("when the later activity lands in a new settle window", () => {
      it("records and consumes a second evaluation round", async () => {
        ...
        expect(process?.state.pendingMatches["trace-1"]?.settleWindowBucket).toBe("30000-1");
      });
    });
  });
  ...
  describe("given several matches for one trigger", () => {
    describe("when coalescing is configured on the producer", () => {
      it("registers recordTriggerMatch with append coalescing enabled", () => {
        ...
        const recordMatch = builtPipeline.commands.find((c) => c.name === "recordTriggerMatch");

        expect(recordMatch?.options?.coalesceMaxBatch).toBe(TRIGGER_MATCH_COALESCE_MAX_BATCH);
  ...
  describe("given two triggers in one project match the same trace", () => {
    it("keeps their process-outbox identities isolated", async () => {
      ...
      expect(processes.map((process) => Object.keys(process?.state.pendingMatches ?? {}))).toEqual([
        ["trace-1"],
        ["trace-1"],
      ]);
    });
...
describe.skipIf(!hasRedis)("automations pipeline — coalesced redis-backed dispatch", () => {
  let redis: Redis;
    ...
    describe("when the coalescing consumer drains them", () => {
      it("stores every match in one multi-row call with distinct idempotency keys", async () => {
        ...
        try {
          ...
          await vi.waitFor(
            async () => {
              const events = await eventStore.getEvents("trigger-1", { tenantId }, "trigger");
          ...
          expect(
            (batchedEvents as readonly AutomationEvent[]).map((event) => event.idempotencyKey),
          ).toEqual([
            "trigger-1:trace-1:30000-0",
            "trigger-1:trace-2:30000-0",
            "trigger-1:trace-3:30000-0",
