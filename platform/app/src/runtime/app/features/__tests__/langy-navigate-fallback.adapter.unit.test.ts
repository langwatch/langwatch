...
const app = {
  ...
  experiments: { tryGetById: tryGetExperimentById },
...
const resolve = (resourceId: string) =>
  AppLangyNavigateFallbackAdapter.create(app as never).resolve({
    projectId: "proj_1",
    resourceId,
  });
...
describe("AppLangyNavigateFallbackAdapter", () => {
  ...
  describe("given an experiment id the project can see", () => {
    it("resolves the experiment's page by its slug: the address the app's own links use", async () => {
      tryGetExperimentById.mockResolvedValue({
        id: "experiment_1",
        slug: "my-batch-eval",
      });
      ...
      expect(tryGetExperimentById).toHaveBeenCalledWith({
        projectId: "proj_1",
        id: "experiment_1",
      });
    });
    ...
    it("returns null when the experiment does not resolve in this project", async () => {
      tryGetExperimentById.mockResolvedValue(null);
