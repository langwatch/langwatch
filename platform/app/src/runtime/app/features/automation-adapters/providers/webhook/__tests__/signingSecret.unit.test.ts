...
import { automationWebhookProvider } from "../../registry";
...
describe("the automations signing secret", () => {
  describe("when no secret is configured", () => {
    it("stores nothing and signs nothing, which is the pre-existing behavior", () => {
      const stored = automationWebhookProvider.persist({ incoming: { ...BASE } });
      expect(stored.signingSecretEncrypted).toBeUndefined();
      expect(automationWebhookProvider.decryptSigningSecrets(stored)).toEqual([]);
    });

    it("reports no secret to the drawer", () => {
      const stored = automationWebhookProvider.persist({ incoming: { ...BASE } });
      expect(automationWebhookProvider.redact(stored).signingSecret).toBeNull();
    });
  });

  describe("when a secret is typed", () => {
    it("encrypts it and never keeps the plaintext", () => {
      const stored = automationWebhookProvider.persist({
      ...
      expect(automationWebhookProvider.decryptSigningSecrets(stored)).toEqual(["whsec_first"]);
    });
    ...
    it("returns the sentinel to the drawer instead of the value", () => {
      ...
      const redacted = automationWebhookProvider.redact(stored);
  ...
  describe("when the drawer saves without touching the field", () => {
    it("round-trips the sentinel and keeps the stored secret", () => {
      const existing = automationWebhookProvider.persist({
        incoming: { ...BASE, signingSecret: "whsec_first" },
      });
      const stored = automationWebhookProvider.persist({
    ...
    it("keeps an in-flight rotation window intact", () => {
      const first = automationWebhookProvider.persist({
        incoming: { ...BASE, signingSecret: "whsec_first" },
      });
      const rotated = automationWebhookProvider.persist({
        incoming: { ...BASE, signingSecret: "whsec_second" },
        existing: first,
      });
      const resaved = automationWebhookProvider.persist({
        incoming: { ...BASE, signingSecret: WEBHOOK_HEADER_VALUE_KEPT },
        existing: rotated,
      });
      expect(automationWebhookProvider.decryptSigningSecrets(resaved)).toEqual([
        "whsec_second",
        "whsec_first",
      ]);
    });
  ...
  describe("when a different secret is typed over a stored one", () => {
    it("signs with both while the old one's window is open, newest first", () => {
      ...
      expect(automationWebhookProvider.decryptSigningSecrets(rotated)).toEqual([
        "whsec_second",
        "whsec_first",
      ]);
    });
    ...
    it("drops the old one once its window closes", () => {
      ...
      expect(automationWebhookProvider.decryptSigningSecrets(rotated, afterWindow)).toEqual([
        "whsec_second",
      ]);
    });
    ...
    it("opens no window when the same value is submitted again", () => {
      ...
      expect(automationWebhookProvider.decryptSigningSecrets(resaved)).toEqual(["whsec_first"]);
    });
  });
  ...
  describe("when the field is cleared", () => {
    it("stops signing and drops the old secret with it", () => {
      ...
      const cleared = automationWebhookProvider.persist({
      ...
      expect(automationWebhookProvider.decryptSigningSecrets(cleared)).toEqual([]);
    });
  });
});
