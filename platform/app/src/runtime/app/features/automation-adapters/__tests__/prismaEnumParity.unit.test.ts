...
const _triggerActionParity: AssertMutuallyAssignable<PackageTriggerAction, PrismaTriggerAction> =
  true;
const _alertTypeParity: AssertMutuallyAssignable<PackageAlertType, PrismaAlertType> = true;
...
describe("prisma enum parity", () => {
  describe("when the package enums are compared to the Prisma enums", () => {
    ...
    it("keeps AlertType values identical", () => {
      expect(Object.values(PackageAlertType).sort()).toEqual(Object.values(PrismaAlertType).sort());
    });
  });
});
