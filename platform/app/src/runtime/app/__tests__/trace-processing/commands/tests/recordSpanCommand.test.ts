...
function createMockDependencies(): {
  ...
  const mockDropSpanContent = vi.fn().mockResolvedValue({ droppedCount: 0, droppedCategories: [] });

  return {
    deps: {
      piiRedaction: {
        redact: mockRedactSpan,
      },
      costEnrichment: {
        enrich: mockEnrichSpan,
      },
      tokenEstimation: {
        estimate: mockEstimateSpanTokens,
      },
      contentDrop: {
        drop: mockDropSpanContent,
      },
    },
    mockRedactSpan,
    mockEnrichSpan,
...
describe("RecordSpanCommand", () => {
  ...
  beforeEach(() => {
    ...
    handler = RecordSpanCommand.create(deps);
  });
  ...
  describe("handle", () => {
    describe("PII redaction", () => {
      ...
      it("uses DISABLED level when specified", async () => {
        const command = createMockCommand("project-789", "trace-1", "span-1", [], "DISABLED");
    ...
    describe("when pre-processing steps fail", () => {
      it("throws when PII redaction rejects", async () => {
        ...
        const cmd = RecordSpanCommand.create(deps);
    ...
    describe("event creation", () => {
      it("creates SpanReceivedEvent with redacted span data", async () => {
        ...
        mockRedactSpan.mockImplementation(
          async (span: { attributes: Array<{ key: string; value: { stringValue?: string } }> }) => {
    ...
    describe("when an attribute value is oversized", () => {
      it("caps a multi-MB base64 image attribute on the emitted span", async () => {
        ...
        expect(inputAttr!.value.stringValue).toMatch(/^\[truncated: \d+ bytes, image\/png\]$/);
