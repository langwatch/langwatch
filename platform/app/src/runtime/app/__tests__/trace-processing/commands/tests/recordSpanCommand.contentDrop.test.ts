...
import { TraceCanonicalisationService, TraceSummaryFoldProjection } from "@langwatch/trace-server";
/**
...
import type { PIIRedactionLevel, RecordSpanCommandData } from "@langwatch/trace-contract";
import { RECORD_SPAN_COMMAND_TYPE } from "@langwatch/trace-contract";
import type { OtlpKeyValue } from "@langwatch/trace-contract";
import { RecordSpanCommand, type RecordSpanCommandOptions } from "@langwatch/trace-server";
...
function makeHandler(dropPolicy: ResolvedDataPrivacy | null): RecordSpanCommand {
  const deps: RecordSpanCommandOptions = {
  ...
  return RecordSpanCommand.create(deps);
}
