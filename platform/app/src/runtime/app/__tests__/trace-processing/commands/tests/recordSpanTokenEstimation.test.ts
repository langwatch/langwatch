...
function createDeps(): RecordSpanCommandOptions {
  return {
    piiRedaction: { redact: vi.fn() },
    costEnrichment: { enrich: vi.fn() },
    contentDrop: {
      drop: async () => ({
        droppedCount: 0,
        droppedCategories: [],
    ...
    tokenEstimation: {
      estimate: vi.fn(async (span: OtlpSpan) => {
        ...
        const hasInputTokens = span.attributes.some(
          (a) => a.key === "gen_ai.usage.input_tokens" || a.key === "gen_ai.usage.prompt_tokens",
        );
        const hasOutputTokens = span.attributes.some(
          (a) =>
            a.key === "gen_ai.usage.output_tokens" || a.key === "gen_ai.usage.completion_tokens",
        );
...
describe("RecordSpanCommand token estimation", () => {
  ...
  let deps: RecordSpanCommandOptions;
  ...
  describe("when LLM span has input/output but no token counts", () => {
    it("calls token estimation service", async () => {
      ...
      expect(deps.tokenEstimation.estimate).toHaveBeenCalled();
    });
  ...
  describe("when token estimation fails", () => {
    it("continues without tokens (non-critical)", async () => {
      deps.tokenEstimation.estimate = vi.fn().mockRejectedValue(new Error("tiktoken unavailable"));
      handler = RecordSpanCommand.create(deps);
