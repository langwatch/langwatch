...
import { type Command, createTenantId, type TenantId } from "@langwatch/eventing";
 ...
 */
function makeOversizedCommand({ spoolRef }: { spoolRef: string }): Command<RecordSpanCommandData> {
  return {
...
type LegacyDependencies = {
  ...
  tokenEstimationService: {
    estimateSpanTokens: (input: { span: OtlpSpan; tenantId?: string }) => Promise<void>;
  };
  contentDropService: {
    dropSpanContent: (input: {
      span: OtlpSpan;
      projectId: string;
    }) => Promise<{ droppedCount: number; droppedCategories: string[] }>;
  };
};

function makeDeps(): LegacyDependencies {
  return {
    ...
    contentDropService: {
      dropSpanContent: vi.fn().mockResolvedValue({ droppedCount: 0, droppedCategories: [] }),
    },
  };
}
...
function createHandler(
  ...
  blobStore?: Pick<BlobStore, "getSpool" | "deleteSpool">,
): RecordSpanCommand {
  const spool: TraceSpanSpoolPort | undefined = blobStore
    ? {
  ...
  return RecordSpanCommand.create({
    piiRedaction: { redact: deps.piiRedactionService.redactSpan },
    costEnrichment: { enrich: deps.costEnrichmentService.enrichSpan },
    tokenEstimation: {
      estimate: (span, tenantId) =>
        deps.tokenEstimationService.estimateSpanTokens({ span, tenantId }),
    },
    contentDrop: {
      drop: (span, projectId) => deps.contentDropService.dropSpanContent({ span, projectId }),
    },
    spool,
  });
}
...
describe("given a RecordSpanCommand that carries a spoolRef (oversized path)", () => {
  ...
  describe("when the command worker handles the oversized command", () => {
    it("calls BlobStore.get exactly once with the spool key and the reconstituted span flows through", async () => {
      ...
      const deps = makeDeps();
      const handler = createHandler(deps, blobStore);
...
describe("given the edge spool PUT failed and the command fell back to inline", () => {
  ...
  async function inlineCommandAfterFailedSpool(): Promise<Command<RecordSpanCommandData>> {
  ...
  describe("when the command worker handles that command", () => {
    ...
    it("emits a span whose oversized value carries the truncation marker and fits the cap", async () => {
      const handler = createHandler(makeDeps(), makeFailingBlobStore());
...
describe("given a command carrying a spoolRef but a handler with no blobStore", () => {
  ...
  describe("when handle() is called", () => {
    it("throws rather than emitting a span with cleared attributes", async () => {
      ...
      const depsWithoutBlobStore: LegacyDependencies = {
        piiRedactionService: { redactSpan: vi.fn() },
        costEnrichmentService: { enrichSpan: vi.fn() },
        tokenEstimationService: { estimateSpanTokens: vi.fn() },
        contentDropService: {
      ...
      const handler = createHandler(depsWithoutBlobStore);
      const command = makeOversizedCommand({ spoolRef: SPOOL_REF });
