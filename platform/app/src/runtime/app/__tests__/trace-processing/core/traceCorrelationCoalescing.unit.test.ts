  ...
  describe("given the trace-processing pipeline is defined", () => {
    ...
    describe("when the real registrations are wired into the queue manager", () => {
      ...
      function registryFor() {
        ...
        const registrations = createTraceProcessingPipeline(buildTraceDeps()).commands.filter(
          (candidate) => wired.has(candidate.name),
        );
        if (registrations.length !== wired.size) {
          ...
          throw new Error(`expected ${wired.size} registrations, resolved ${registrations.length}`);
        }

        manager.initializeCommandQueues(registrations as never, vi.fn(), "trace_processing");
      ...
      it("installs a batch processor for both correlation commands", () => {
        ...
        for (const name of ["recordLogContribution", "recordMetricCorrelation"]) {
          expect(registry.get(`trace_processing:command:${name}`)?.processBatch).toBeDefined();
        }
      });
  ...
  describe("given several queued log contributions from one trace", () => {
    describe("when the coalesced batch is processed", () => {
      ...
      it("appends them as one insert holding every contribution in dispatch order", async () => {
        ...
        expect(
          (events as Event[]).map((event) => (event.data as { recordId: string }).recordId),
