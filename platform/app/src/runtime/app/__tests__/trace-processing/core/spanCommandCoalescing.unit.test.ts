...
function stubbedHandler(blobStore?: {
  getSpool(input: {
    spoolRef: string;
    projectId: string;
    traceId: string;
  ...
  return RecordSpanCommand.create({
    ...
    spool: blobStore
      ? {
          read: async (identity) => (await blobStore.getSpool(identity)).toString("utf8"),
          delete: async (identity) => await blobStore.deleteSpool(identity),
        }
      : void 0,
  });
}
  ...
  describe("given the trace-processing pipeline is defined", () => {
    describe("when recordSpan is registered without sharding", () => {
      ...
      it("installs a bound resolved per span rather than one fixed for the command", () => {
        expect(typeof recordSpanRegistration()?.options?.coalesceMaxBatch).toBe("function");
      });

      it("folds an inline span up to the span batch bound", () => {
        const bound = coalesceResolverOf();

        expect(bound(spanPayload({ spanId: spanId(0) }))).toBe(RECORD_SPAN_COALESCE_MAX_BATCH);
      });
      ...
      it("leaves the existing span deduplication untouched", () => {
        expect(recordSpanRegistration()?.options?.deduplication).toBe(RECORD_SPAN_DEDUPLICATION);
      });
    });
  ...
  describe("given several queued spans from one trace", () => {
    describe("when the coalesced batch is processed", () => {
      ...
      it("appends them as one insert holding every span in dispatch order", async () => {
        ...
        const payloads = [0, 1, 2, 3].map((index) => spanPayload({ spanId: spanId(index) }));
        ...
        expect((events as Event[]).every((event) => event.type === SPAN_RECEIVED_EVENT_TYPE)).toBe(
          true,
        );
      ...
      it("keeps each span's idempotency key so a retry cannot duplicate it", async () => {
        ...
        expect((events as Event[]).map((event) => event.idempotencyKey)).toEqual(
          payloads.map(
            (payload) => `${FIXTURE_TENANT_ID}:${FIXTURE_TRACE_ID}:${payload.span.spanId}`,
          ),
        );
      });
  ...
  describe("given spooled spans that reached the batch anyway", () => {
    describe("when the batch is stored", () => {
      it("deletes each span's spool once, after the single append", async () => {
        ...
        expect(order).toEqual(["store", `delete:spool/${spanId(0)}`, `delete:spool/${spanId(1)}`]);
      });
    });
