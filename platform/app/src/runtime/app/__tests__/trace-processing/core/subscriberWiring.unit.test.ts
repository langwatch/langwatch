...
import { buildTraceDeps as buildBaseTraceDeps } from "./support/traceProcessingFixtures";
...
describe("trace-processing pipeline subscriber wiring", () => {
  describe("given the triggerMatch subscriber", () => {
    ...
    it("reacts only to span_received and origin_resolved events", () => {
      ...
      expect(shouldDispatch(fakeEvent({ type: SPAN_RECEIVED_EVENT_TYPE }), context)).toBe(true);
      expect(shouldDispatch(fakeEvent({ type: ORIGIN_RESOLVED_EVENT_TYPE }), context)).toBe(true);
      expect(shouldDispatch(fakeEvent({ type: "lw.obs.trace.something_else" }), context)).toBe(
        false,
      );
    });
  ...
  describe("given the customEvaluationSync subscriber", () => {
    ...
    it("rejects events without syncable evaluations before enqueue", () => {
      ...
      const context = {} as never;
      expect(shouldDispatch(fakeEvent({ type: SPAN_RECEIVED_EVENT_TYPE }), context)).toBe(false);
      expect(shouldDispatch(fakeEvent({ type: ORIGIN_RESOLVED_EVENT_TYPE }), context)).toBe(false);
    });
  });
  ...
  describe("given the graphTriggerActivity subscriber", () => {
    ...
    it("registers for span_received and origin_resolved with the real-time debounce delay", () => {
      ...
      expect(graphTriggerActivity!.options?.delay).toBe(GRAPH_TRIGGER_REAL_TIME_DEBOUNCE_MS);
    });
  ...
  describe("given a direct trace subscriber", () => {
    it("keeps its raw span event filter when the Trace pipeline is composed", () => {
      const directSubscriber = {
        name: "directSpanSubscriber",
        eventTypes: [SPAN_RECEIVED_EVENT_TYPE],
        handle: handlerStub(),
      };
      const pipeline = createTraceProcessingPipeline(
        buildTraceDeps({ subscribers: [directSubscriber] }),
      );

      expect(pipeline.eventSubscribers.get(directSubscriber.name)?.eventTypes).toEqual([
        SPAN_RECEIVED_EVENT_TYPE,
      ]);
    });
  });
