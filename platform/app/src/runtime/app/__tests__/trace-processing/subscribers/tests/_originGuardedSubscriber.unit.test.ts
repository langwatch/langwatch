...
describe("passesTraceOriginGuards", () => {
  ...
  describe("given a recent message event on a recent resolved trace", () => {
    it("admits span and origin-resolution events", () => {
      ...
      expect(passesTraceOriginGuards(event({ type: ORIGIN_RESOLVED_EVENT_TYPE }), fold())).toBe(
        true,
      );
    });
  });
  ...
  describe("given an event older than one hour", () => {
    it("rejects it", () => {
      expect(passesTraceOriginGuards(event({ occurredAt: NOW - 60 * 60 * 1000 - 1 }), fold())).toBe(
        false,
      );
    });
  });
  ...
  describe("given a derived trace event", () => {
    it("rejects it", () => {
      expect(passesTraceOriginGuards(event({ type: "lw.obs.trace.topic_assigned" }), fold())).toBe(
        false,
      );
    });
  });
  ...
  describe("given a trace older than the 24-hour trace-age cap", () => {
    it("rejects a fresh span while admitting a trace just inside the cap", () => {
      expect(
        passesTraceOriginGuards(event(), fold({ occurredAt: NOW - 24 * 60 * 60 * 1000 - 1 })),
      ).toBe(false);
      expect(
        passesTraceOriginGuards(event(), fold({ occurredAt: NOW - 24 * 60 * 60 * 1000 + 1 })),
      ).toBe(true);
  ...
  describe("given a trace without a usable resolved origin", () => {
    it("rejects unresolved and guardrail-blocked folds", () => {
      ...
      expect(
        passesTraceOriginGuards(event(), fold({ blockedByGuardrail: true, computedOutput: null })),
      ).toBe(false);
    });
  });
});
