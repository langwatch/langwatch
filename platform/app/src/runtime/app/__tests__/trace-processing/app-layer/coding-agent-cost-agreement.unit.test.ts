...
function analyticsRollupCost(extra: CallExtra = {}): number {
  return AppTraceProjectionsAdapter.createAnalyticsRollupProjection({
...
function storedSpanCost(extra: CallExtra = {}): number | null {
  return AppTraceProjectionsAdapter.createSpanStorageProjection({
    canonicalisation: traceCanonicalisation,
    store: noopAppendStore as never,
...
function recomputedSummaryRowCost(extra: CallExtra = {}): number | null {
  const attrs = AppTraceProjectionsAdapter.createSpanStorageProjection({
    canonicalisation: traceCanonicalisation,
    store: noopAppendStore as never,
...
function terminalFooterCost(extra: CallExtra = {}): number {
  const stored = AppTraceProjectionsAdapter.createSpanStorageProjection({
    canonicalisation: traceCanonicalisation,
    store: noopAppendStore as never,
  ...
  return buildEntryTimeline({ entries: transcript.entries }).at(-1)?.cumulativeCostUsd ?? 0;
}
