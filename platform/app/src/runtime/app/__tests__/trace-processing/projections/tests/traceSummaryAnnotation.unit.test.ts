  ...
  describe("apply()", () => {
    ...
    describe("when AnnotationRemovedEvent is received", () => {
      it("removes the annotation ID from the array", () => {
        ...
        state = projection.apply(state, makeAnnotationAddedEvent({ annotationId: "ann-1" }));
        state = projection.apply(state, makeAnnotationAddedEvent({ annotationId: "ann-2" }));
    ...
    describe("when AnnotationsBulkSyncedEvent is received", () => {
      it("merges with existing annotation IDs without duplicates", () => {
        let state = makeInitState();
        state = projection.apply(state, makeAnnotationAddedEvent({ annotationId: "old-ann" }));
    ...
    describe("when add then remove occurs", () => {
      it("results in empty array", () => {
        let state = makeInitState();
        state = projection.apply(state, makeAnnotationAddedEvent({ annotationId: "ann-1" }));
        state = projection.apply(state, makeAnnotationRemovedEvent({ annotationId: "ann-1" }));
    ...
    describe("when bulk sync then add occurs", () => {
      it("combines the results", () => {
        ...
        state = projection.apply(state, makeAnnotationAddedEvent({ annotationId: "new-ann" }));
