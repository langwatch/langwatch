...
/**
 * FOLD-EQUIVALENCE for the slim trace fold's read-back (ADR-066).
 *
...
describe("traceAnalytics fold-equivalence across the read-back boundary", () => {
  ...
  describe("given accumulated prompt ids past the 4096-char cap re-sent by a later span", () => {
    ...
    const committed = foldAll(overCapSpans.slice(0, OVER_CAP_SPANS - 3), projection.init());
    ...
    it("restarts the union from the spans after the truncation", () => {
      ...
      expect(ids).toEqual(
        tail.map((_, index) => `prompt_2Zx9QwErTyUiOpAsDfGhJ${OVER_CAP_SPANS - 3 + index}:3`),
      );
    });
    ...
    it("does not nest the truncated fragment back into the array", () => {
      ...
      const ids = JSON.parse(project(state).attributes["langwatch.prompt_ids"] ?? "[]") as string[];
...
describe("traceAnalytics read-back field coverage", () => {
  ...
  describe("given a state folded from the full event sequence", () => {
    ...
    it("accounts for every field the fold actually carries", () => {
      ...
      expect(Object.keys(state).sort()).toEqual(Object.keys(TRACE_STATE_DISPOSITION).sort());
      expect(Object.keys(decoded).sort()).toEqual(Object.keys(TRACE_STATE_DISPOSITION).sort());
    });
