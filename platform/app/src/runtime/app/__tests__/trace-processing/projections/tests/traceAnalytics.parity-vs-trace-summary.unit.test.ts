...
const summaryProjection = TraceSummaryFoldProjection.create({
  runtime: AppTraceProjectionsAdapter.createRuntime(TraceCanonicalisationService.create()),
  traceCanonicalisation: TraceCanonicalisationService.create(),
  store: { store: async () => {}, get: async () => null },
} as never);
...
function topicAssignedEvent(topicId: string | null, subtopicId: string | null): TopicAssignedEvent {
  return {
    id: "evt-topic-1",
...
function assertSharedFieldsParity({
  ...
  expect(slim.attributes["langwatch.user_id"]).toBe(summary.attributes["langwatch.user_id"]);
  ...
  expect(slim.attributes["langwatch.origin"]).toBe(summary.attributes["langwatch.origin"]);
  expect(slim.attributes["langwatch.labels"]).toBe(summary.attributes["langwatch.labels"]);
