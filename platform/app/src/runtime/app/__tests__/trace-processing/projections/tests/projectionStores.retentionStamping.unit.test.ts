...
import { EvaluationRunStore } from "@langwatch/evaluation-server/internal";
import {
  type NormalizedSpan,
  NormalizedSpanKind,
...
describe("trace-pipeline projection stores retention stamping", () => {
  ...
  describe("when the projection context carries no resolved policy", () => {
    it("stamps stored_spans with the platform default, not indefinite", async () => {
      ...
      const store = SpanStorageStore.create({
        storage: { insertSpan, insertSpans: async () => {} },
      ...
      expect(insertSpan.mock.calls[0]![0]!.retentionDays).toBe(PLATFORM_DEFAULT_RETENTION_DAYS);
    });
    ...
    it("stamps evaluation_runs with the platform default", async () => {
      ...
      const store = EvaluationRunStore.create({
        service,
        defaultRetentionDays: PLATFORM_DEFAULT_RETENTION_DAYS,
      });

      await store.store({ evaluationId: "eval_1" } as EvaluationRunData, nullPolicyContext);
