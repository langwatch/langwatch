...
import { DatasetTrpcApi } from "@langwatch/dataset-server";
...
export const datasetRouter = DatasetTrpcApi.create(
  appTrpcRoot,
  { protected: authProtectedProcedure, policy },
  {
    // `copy` names a SECOND project the declared check never covers. The
    // process owns the decision, exactly as it did when the probe was inline.
