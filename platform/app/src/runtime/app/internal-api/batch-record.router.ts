...
import { BatchRecordTrpcApi } from "@langwatch/dataset-server";
...
/**
 * `BatchEvaluation` is process-owned state with no feature package of its own
 * yet, so the two reads stay here and the transport takes them as ports. Both
 * are the same Prisma queries the router made inline, through the client the
 * request already carries.
 */
const summariseByExperiment = (ctx: TRPCContext, { projectId }: { projectId: string }) =>
  ctx.prisma.batchEvaluation.groupBy({
...
const listByExperiment = (
  ctx: TRPCContext,
  { projectId, experimentId }: { projectId: string; experimentId: string },
) =>
  ctx.prisma.batchEvaluation.findMany({
    where: {
      projectId: projectId,
...
export const batchRecordRouter = BatchRecordTrpcApi.create(
  appTrpcRoot,
