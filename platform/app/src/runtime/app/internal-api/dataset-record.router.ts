import { type AuthzPermission, authzDeclarationOf } from "@langwatch/authz-contract";
import { DatasetRecordTrpcApi } from "@langwatch/dataset-server";
 ...
 */
const policy = (permission: AuthzPermission) => {
  const check = checkDeclaredPermission({ permission });
...
export const datasetRecordRouter = DatasetRecordTrpcApi.create(appTrpcRoot, {
