...
export const teamRouter = TeamTrpcApi.create(
  appTrpcRoot,
  { protected: authProtectedProcedure, policy },
  {
    probeOrganizationPermission: (ctx: TRPCContext, organizationId, permission) =>
      probeOrganizationPermission(ctx, organizationId, permission),
    // Custom team roles are an Enterprise capability, and the plan is read per
    // organization out of this process's billing store.
    assertCustomRolesAllowed: async (ctx: TRPCContext, { organizationId, members }) => {
      if (!members.some(({ role }) => isCustomRole(role))) return;
