...
type ReplayFoldStore = RegisteredFoldProjection["definition"]["store"];
...
export function createReplayRuntime(config: {
  ...
  if (!redis) {
    throw new Error("Replay requires a Redis URL — none was resolved from the supplied config.");
  }

  const clientResolver = getApp().clickhouse.resolveClient;

  // Raw CH stores (no Redis cache) — keyed by pipeline name
  const storeByPipeline = new Map<string, ReplayFoldStore>([
    [
      "trace_processing",
    ...
    [
      ...
      EvaluationEventingAdapter.createRunStore({
        evaluation: getApp().evaluations,
