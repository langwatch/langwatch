...
| 9 | `legacy-top-topics.json` | Topics page top-topics chart, `traces`/`trace_metrics` `TopicId`/`SubTopicId` resolved via the `topics` view (`platform/app/src/server/analytics/lwql/catalog/postgresOverrides/topics.ts`) | Joins `trace_metrics` to `topics` twice (once for `TopicId`, once for `SubTopicId`) to resolve names, matching the pattern of `legacy-top-models.json`. Unmatched joins return `''` (empty string), hence the `coalesce()` calls in the SQL; traces with no `TopicId` show as "unknown" until topic clustering has run. |
