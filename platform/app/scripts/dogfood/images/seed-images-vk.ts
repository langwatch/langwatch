/**
 * Image dogfood seeder: provisions everything the gateway image endpoints
 * need for a live local run, in one shot:
 *
 *   1. An OpenAI ModelProvider row on the user's org (key from
 *      OPENAI_API_KEY in env). Read that from platform/app/.env, which is
 *      where this app's model provider keys live. An env file from another
 *      repository drifts, and a stale key is indistinguishable from a fresh
 *      one once stored.
 *   2. An org-default routing policy carrying the provider with NO model
 *      allowlist (image model ids like gpt-image-2 must not be filtered by a
 *      chat-shaped allowlist).
 *   3. A personal VK for the user, printed once to stdout.
 *
 * Pairs with scripts/dogfood/images/probe-image-spend.ts, which issues its
 * own key and budget but needs the provider and the policy this seeder
 * writes.
 *
 * Usage:
 *   pnpm tsx scripts/dogfood/images/seed-images-vk.ts --email you@example.com
 *
 * Idempotent: reuses an existing provider row (matched by org + provider) and
 * updates the default policy in place.
 *
 * Guardrails, because the provider key and the default policy are org-wide
...
/**
 * This seeder mutates org-wide records (the provider key, the default routing
...
async function ensureProvider({
  ...
  if (existing) {
    if (existingRows.length > 1) {
      process.stderr.write(
        `[seed-images] WARNING: ${existingRows.length} ${provider} provider rows on this org, using the oldest (${existing.id}) and leaving the rest untouched\n`,
      );
    }
  ...
  if (!keys) {
    process.stderr.write(
      `[seed-images] no ${provider} provider row and no key in env, nothing to seed\n`,
    );
    return null;
  }
...
async function main() {
  ...
  process.stderr.write(
    `[seed-images] user=${user.id} org=${org.id} (${org.name})\n`,
  );
  ...
  // serve a request drops out.
  const openaiKey = process.env.OPENAI_API_KEY;
  if (!openaiKey) {
    process.stderr.write(
      "[seed-images] OPENAI_API_KEY unset, keeping whatever openai credential the org already holds\n",
    );
  }
  ...
  if (providerIds.length === 0) {
    throw new Error(
      "no usable provider row: OPENAI_API_KEY was not in env and the org holds no readable openai credential",
    );
  }

  // Org-default policy carrying the provider and no model allowlist: the
  // image endpoints route by explicit provider/model.
  ...
  if (existingPolicy) {
    ...
    process.stderr.write(
      `[seed-images] updated default policy ${existingPolicy.id}: providers=${merged.length}\n`,
    );
  } else {
    const policy = await prisma.routingPolicy.create({
      data: {
        ...
        name: "images-dogfood-default",
        isDefault: true,
        modelProviderIds: providerIds,
      },
    });
    process.stderr.write(`[seed-images] created default policy ${policy.id}\n`);
  }
