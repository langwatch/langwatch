...
/**
 * One day's scenario runs. Consumes `random` first, before the organic and
 * experiment builders, so the whole day stays reproducible from its seed.
 */
function buildDayScenarioRuns({
  day,
  dayStart,
  trend,
  random,
  ...
  for (let ordinal = 0; ordinal < dailyRuns; ordinal++) {
    ...
    const startedAt =
      dayStart + (8 * 60 + ordinal * 95 + Math.floor(random() * 40)) * 60_000;
...
/** One day of organic support traffic, every day of the window. */
function buildDayOrganicTraces({
  day,
  dayStart,
  random,
}: {
...
/** The baseline/improved pair for one experiment week, scores drifting up. */
function buildExperimentPair({
  dayStart,
  trend,
  random,
}: {
...
export function buildMassTimeline(options: MassTimelineOptions): MassTimeline {
  if (!Number.isFinite(options.months)) {
    throw new Error(
      `buildMassTimeline: months must be a finite number, got ${options.months}`,
    );
  }
  ...
  for (let dayIndex = 0; dayIndex < days; dayIndex++) {
    ...
    // One stream per day, drawn from in a fixed order by the three builders
    // below — that order is what makes the same (now, months) reproduce the
    // same timeline.
    ...
    if (dayIndex % 7 === 3) {
      experimentRuns.push(...buildExperimentPair({ dayStart, trend, random }));
    }
  }
