...
You are an audit soldier. Goal: classify every `@unimplemented` scenario in `specs/tail/`.
...
For every occurrence of `@unimplemented` in `specs/tail/`, append a row to a new file at `specs/tail/AUDIT_MANIFEST.md`:

```markdown
| File | Scenario | Class | Rationale |
|------|----------|-------|-----------|
| specs/tail/foo.feature | "Open the modal" | KEEP | Behavior still exists, no test bound |
```
...
```bash
mc=$(awk '/^\| specs\//' specs/tail/AUDIT_MANIFEST.md | wc -l)
uc=$(grep -rh '@unimplemented' specs/tail/ | wc -l)
[ "$mc" -ge "$uc" ] && echo "AUDIT COMPLETE"
```

## Process

1. Read every `.feature` file in `specs/tail/`.
...
3. Append rows to `specs/tail/AUDIT_MANIFEST.md` as you go.
4. When manifest count matches @unimplemented count, commit + open a PR titled "audit(tail): classify @unimplemented scenarios (#3458)".
5. PR body = TL;DR breakdown by class + link to plan doc.

## Rules

- Branch is `audit/unimpl-tail-2026-04-25` (already checked out).
