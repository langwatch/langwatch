# Audit brief — scenarios

You are an audit soldier. Goal: classify every `@unimplemented` scenario in `specs/scenarios/`.
...
For every occurrence of `@unimplemented` in `specs/scenarios/`, append a row to a new file at `specs/scenarios/AUDIT_MANIFEST.md`:

```markdown
| File | Scenario | Class | Rationale |
|------|----------|-------|-----------|
| specs/scenarios/foo.feature | "Open the modal" | KEEP | Behavior still exists, no test bound |
```
...
```bash
mc=$(awk '/^\| specs\//' specs/scenarios/AUDIT_MANIFEST.md | wc -l)
uc=$(grep -rh '@unimplemented' specs/scenarios/ | wc -l)
[ "$mc" -ge "$uc" ] && echo "AUDIT COMPLETE"
```

## Process

1. Read every `.feature` file in `specs/scenarios/`.
...
3. Append rows to `specs/scenarios/AUDIT_MANIFEST.md` as you go.
4. When manifest count matches @unimplemented count, commit + open a PR titled "audit(scenarios): classify @unimplemented scenarios (#3458)".
5. PR body = TL;DR breakdown by class + link to plan doc.

## Rules

- Branch is `audit/unimpl-scenarios-2026-04-25` (already checked out).
