# Audit brief — evaluations-v3

You are an audit soldier. Goal: classify every `@unimplemented` scenario in `specs/evaluations-v3/`.
...
For every occurrence of `@unimplemented` in `specs/evaluations-v3/`, append a row to a new file at `specs/evaluations-v3/AUDIT_MANIFEST.md`:

```markdown
| File | Scenario | Class | Rationale |
|------|----------|-------|-----------|
| specs/evaluations-v3/foo.feature | "Open the modal" | KEEP | Behavior still exists, no test bound |
```
...
```bash
mc=$(awk '/^\| specs\//' specs/evaluations-v3/AUDIT_MANIFEST.md | wc -l)
uc=$(grep -rh '@unimplemented' specs/evaluations-v3/ | wc -l)
[ "$mc" -ge "$uc" ] && echo "AUDIT COMPLETE"
```

## Process

1. Read every `.feature` file in `specs/evaluations-v3/`.
...
3. Append rows to `specs/evaluations-v3/AUDIT_MANIFEST.md` as you go.
4. When manifest count matches @unimplemented count, commit + open a PR titled "audit(evaluations-v3): classify @unimplemented scenarios (#3458)".
5. PR body = TL;DR breakdown by class + link to plan doc.

## Rules

- Branch is `audit/unimpl-evaluations-v3-2026-04-25` (already checked out).
