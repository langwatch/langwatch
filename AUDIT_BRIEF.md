# Audit brief — model-providers

You are an audit soldier. Goal: classify every `@unimplemented` scenario in `specs/model-providers/`.
...
For every occurrence of `@unimplemented` in `specs/model-providers/`, append a row to a new file at `specs/model-providers/AUDIT_MANIFEST.md`:

```markdown
| File | Scenario | Class | Rationale |
|------|----------|-------|-----------|
| specs/model-providers/foo.feature | "Open the modal" | KEEP | Behavior still exists, no test bound |
```
...
```bash
mc=$(awk '/^\| specs\//' specs/model-providers/AUDIT_MANIFEST.md | wc -l)
uc=$(grep -rh '@unimplemented' specs/model-providers/ | wc -l)
[ "$mc" -ge "$uc" ] && echo "AUDIT COMPLETE"
```

## Process

1. Read every `.feature` file in `specs/model-providers/`.
...
3. Append rows to `specs/model-providers/AUDIT_MANIFEST.md` as you go.
4. When manifest count matches @unimplemented count, commit + open a PR titled "audit(model-providers): classify @unimplemented scenarios (#3458)".
5. PR body = TL;DR breakdown by class + link to plan doc.

## Rules

- Branch is `audit/unimpl-model-providers-2026-04-25` (already checked out).
